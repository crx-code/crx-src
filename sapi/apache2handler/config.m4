...
if test "$CRX_APXS2" != "no"; then
  if test "$CRX_APXS2" = "yes"; then
    APXS=apxs
    ...
    CRX_EXPAND_PATH($CRX_APXS2, APXS)
  fi
  ...
  APACHE_CFLAGS="$APACHE_CPPFLAGS -I$APXS_INCLUDEDIR $APR_CFLAGS $APU_CFLAGS -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1"

  dnl Test that we're trying to configure with apache 2.x
  CRX_AP_EXTRACT_VERSION($APXS_HTTPD)
  ...
  *aix*)
    ...
    CRX_SELECT_SAPI(apache2handler, shared, mod_crx.c sapi_apache2.c apache_config.c crx_functions.c, $APACHE_CFLAGS)
  ...
  *darwin*)
    ...
    CRX_SUBST(MH_BUNDLE_FLAGS)
    CRX_SELECT_SAPI(apache2handler, bundle, mod_crx.c sapi_apache2.c apache_config.c crx_functions.c, $APACHE_CFLAGS)
    SAPI_SHARED=libs/libcrx.so
