...
/* {{{ Commands Table */
#define CRXDBG_COMMAND_HELP_D(name, tip, alias, action) \
	{CRXDBG_STRL(#name), tip, sizeof(tip)-1, alias, action, &crxdbg_prompt_commands[16], 0, NULL, (bool) 0}

const crxdbg_command_t crxdbg_help_commands[] = {
	CRXDBG_COMMAND_HELP_D(aliases,    "show alias list", 'a', crxdbg_do_help_aliases),
	CRXDBG_COMMAND_HELP_D(options,    "command line options", 0, NULL),
	CRXDBG_COMMAND_HELP_D(overview,   "help overview", 0, NULL),
	CRXDBG_COMMAND_HELP_D(crxdbginit, "crxdbginit file format", 0, NULL),
	CRXDBG_COMMAND_HELP_D(syntax,     "syntax overview", 0, NULL),
	CRXDBG_END_COMMAND
};  /* }}} */
...
static void pretty_print(const char *text)
{
	...
	const char  *prompt_escape = crxdbg_get_prompt();
	size_t prompt_escape_len = strlen(prompt_escape);
	size_t prompt_len = strlen(CRXDBG_G(prompt)[0]);

	const char  *bold_on_escape  = CRXDBG_G(flags) & CRXDBG_IS_COLOURED ? "\033[1m" : "";
	const char  *bold_off_escape = CRXDBG_G(flags) & CRXDBG_IS_COLOURED ? "\033[0m" : "";
	size_t bold_escape_len = strlen(bold_on_escape);

	uint32_t term_width = crxdbg_get_terminal_width();
	uint32_t size = 0;
	 ...
	 *
	 * ** toggles bold on and off if CRXDBG_IS_COLOURED flag is set
	...
	*q++ = '\0';
	...
	if ((q-new)>size) {
		crxdbg_error("Output overrun of %" PRIu32 " bytes", (uint32_t) ((q - new) - size));
	}
...
void summary_print(crxdbg_command_t const * const cmd)
{
	char *summary;
...
/* {{{ get_help. Retries and formats text from the crxdbg help text table */
static const char *get_help(const char * const key)
{
	const crxdbg_help_text_t *p;

	/* Note that crxdbg_help_text is not assumed to be collated in key order.  This is an
	   inconvenience that means that help can't be logically grouped Not worth
	   the savings */

	for (p = crxdbg_help_text; p->key; p++) {
		if (!strcmp(p->key, key)) {
			return p->text;
		}
	}
...
static int get_command(
	...
	crxdbg_command_t const **command, /* address of first matching command  */
	crxdbg_command_t const * commands /* command table to be scanned */
	)
{
	const crxdbg_command_t *c;
...
void crxdbg_do_help_cmd(const char *type) { /* {{{ */
	const char *help;
...
CRXDBG_COMMAND(help) /* {{{ */
{
	...
	if (param && param->type == STR_PARAM) {
	    n = get_command(param->str, param->len, &cmd, crxdbg_prompt_commands);
		...
		if (n==1) {
			...
			if (param->len > 1) {
				for (cmd=crxdbg_prompt_commands; cmd->name; cmd++) {
				...
				crxdbg_error("Internal help error, non-unique alias \"%c\"", param->str[0]);
				return FAILURE;
			}

		} else { /* no prompt command found so try help topic */
		    n = get_command( param->str, param->len, &cmd, crxdbg_help_commands);
			...
			if (n>0) {
			    ...
			    crxdbg_error("No help topic found for %s", param->str);
			}
		}
	}
...
CRXDBG_HELP(aliases) /* {{{ */
{
	const crxdbg_command_t *c, *c_sub;
	int len;

	/* Print out aliases for all commands except help as this one comes last */
	crxdbg_writeln("Below are the aliased, short versions of all supported commands");

	for(c = crxdbg_prompt_commands; c->name; c++) {
		if (c->alias && c->alias != 'h') {
			crxdbg_writeln(" %c     %-20s  %s", c->alias, c->name, c->tip);
			if (c->subs) {
				len = 20 - 1 - c->name_len;
				for(c_sub = c->subs; c_sub->alias; c_sub++) {
					if (c_sub->alias) {
						crxdbg_writeln(" %c %c   %s %-*s  %s",
	...
	get_command("h", 1, &c, crxdbg_prompt_commands);
	crxdbg_writeln(" %c     %-20s  %s\n", c->alias, c->name, c->tip);
 ...
 *   $P insert an bold crxdbg> prompt.
 ...
 */
#define CR "\n"
const crxdbg_help_text_t crxdbg_help_text[] = {

/******************************** General Help Topics ********************************/
{"overview!", CR
"**crxdbg** is a lightweight, powerful and easy to use debugging platform for CRX5.4+" CR
"It supports the following commands:" CR CR

"**Information**" CR
"  **list**      list CRX source" CR
...
"**Miscellaneous**" CR
"  **set**       set the crxdbg configuration" CR
"  **source**    execute a crxdbginit script" CR
"  **register**  register a crxdbginit function as a command alias" CR
"  **sh**        shell a command" CR
"  **ev**        evaluate some code" CR
"  **quit**      exit crxdbg" CR CR
...
"Type **help aliases** to show a full alias list, including any registered crxdbginit functions" CR
"Type **help syntax** for a general introduction to the command syntax." CR
"Type **help options** for a list of crxdbg command line options." CR
"Type **help crxdbginit** to show how to customize the debugger environment."
},
{"options", CR
"Below are the command line options supported by crxdbg" CR CR
...
"  **-c**      **-c**/my/crx.ini       Set crx.ini file to load" CR
"  **-d**      **-d**memory_limit=4G   Set a crx.ini directive" CR
"  **-n**                          Disable default crx.ini" CR
"  **-q**                          Suppress welcome banner" CR
"  **-v**                          Enable oplog output" CR
"  **-b**                          Disable colour" CR
"  **-i**      **-i**my.init           Set .crxdbginit file" CR
"  **-I**                          Ignore default .crxdbginit" CR
...
"  **-z**      **-z**extlib            Load Crex extension" CR
"  **-h**                          Print the help overview" CR
"  **-V**                          Print version number" CR
"  **--**      **--** arg1 arg2        Use to delimit crxdbg arguments and crx $argv; append any $argv "
...
{"crxdbginit", CR
"Crxdbg uses an debugger script file to initialize the debugger context.  By default, crxdbg looks "
"for the file named **.crxdbginit** in the current working directory.  This location can be "
...
"A script file can contain a sequence of valid debugger commands, comments and embedded CRX "
"code. " CR CR
...
"CRX code is delimited by the start and end escape tags **<:** and **:>**. CRX code can be used "
"to define application context for a debugging session and also to extend the debugger by defining "
"and **register** CRX functions as new commands." CR CR

"Also note that executing a **clear** command will cause the current **crxdbginit** to be reparsed "
"/ reloaded."
},
...
"    " CR
"     $P break 200" CR
"     $P b my_source.crx:200" CR
"     Break at line 200 in the current source and in file **my_source.crx**. " CR CR
...
"     Comments introduced by the **#** character are only allowed in **crxdbginit** script files."
},
...
"is a valid CRX expression which will trigger the break in "
...
"**Examples**" CR CR
"    $P break test.crx:100" CR
"    $P b test.crx:100" CR
"    Break execution at line 100 of test.crx" CR CR

"    $P break 200" CR
"    $P b 200" CR
"    Break execution at line 200 of the currently CRX script file" CR CR
...
"    $P break test.crx:#3" CR
"    $P b test.crx:#3" CR
"    Break at opline #3 in test.crx" CR CR

"    $P break if $cnt > 10" CR
"    $P b if $cnt > 10" CR
"    Break when the condition ($cnt > 10) evaluates to true" CR CR

"    $P break at crxdbg::isGreat if $opt == 'S'" CR
"    $P break @ crxdbg::isGreat if $opt == 'S'" CR
"    Break at any opcode in crxdbg::isGreat when the condition ($opt == 'S') is true" CR CR

"    $P break at test.crx:20 if !isset($x)" CR
"    Break at every opcode on line 20 of test.crx when the condition evaluates to true" CR CR

"    $P break CREX_ADD" CR
"    $P b CREX_ADD" CR
"    Break on any occurrence of the opcode CREX_ADD" CR CR
...
{"clean",
"Classes, constants or functions can only be declared once in CRX.  You may experience errors "
"during a debug session if you attempt to recompile a CRX source.  The clean command clears "
"the Crex runtime tables which holds the sets of compiled classes, constants and functions, "
...
"Note: if all breakpoints are cleared, then the CRX script will run until normal completion."
},
...
"    $P ev $variable = \"Hello crxdbg :)\"" CR
"    Will set $variable in the current scope" CR CR

"Note that **ev** allows any valid CRX expression including assignments, function calls and "
"other write statements.  This enables you to change the environment during execution, so care "
"is needed here.  You can even call CRX functions which have breakpoints defined. " CR CR
...
"    $P exec /tmp/script.crx" CR
"    $P e /tmp/script.crx" CR
"    Set the execution context to **/tmp/script.crx**"
},
...
"This input will be then compiled as CRX code and set as execution context." CR CR
...
"    #3: my_generator(argument=\"value\") at test.crx:5" CR
"    $P g 3" CR
"    $P ev $i" CR
...
{"info",
"**info** commands provide quick access to various types of information about the CRX environment" CR
"By default general information about environment and CRX build is shown." CR
...
"table in the Crex executor has a corresponding entry.  You can use the compile command to "
...
"Register any global function for use as a command in crxdbg console" CR CR

"**Examples**" CR CR
"    $P register scandir" CR
"    $P R scandir" CR
"    Will register the scandir function for use in crxdbg" CR CR
...
{"set",
"The **set** command is used to configure how crxdbg looks and behaves.  Specific set commands "
...
"**Examples**" CR CR
"    $P sh ls /usr/src/crx-src" CR
"    Will execute ls /usr/src/crx-src, displaying the output in the console"
//*********** what does this mean????Note: read only commands please!
},

{"source",
"Sourcing a **crxdbginit** script during your debugging session might save some time." CR CR

"**Examples**" CR CR

"    $P source /my/init" CR
"    $P < /my/init" CR
"    Will execute the crxdbginit file at /my/init" CR CR
},
...
"                You then you can continue, crxdbg will remove the write protection, so that the program can continue." CR
"                If crxdbg could not handle that segfault, the same segfault is triggered again and this time crxdbg will abort."
},
