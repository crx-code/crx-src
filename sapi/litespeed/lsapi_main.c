...
crex_module_entry litespeed_module_entry;
...
static void init_sapi_from_env(sapi_module_struct *sapi_module)
{
    ...
    p = getenv("LSCRXRC");
    if (p)
        sapi_module->crx_ini_path_override = p;
}

/* {{{ crx_lsapi_startup */
static int crx_lsapi_startup(sapi_module_struct *sapi_module)
{
    if (crx_module_startup(sapi_module, NULL)==FAILURE) {
        return FAILURE;
    }
...
#define INI_DEFAULT(name,value)\
    ...
    crex_hash_update(configuration_hash, name, sizeof(name), tmp, sizeof(zval), (void**)&entry);\
    C_STRVAL_P(entry) = crex_strndup(C_STRVAL_P(entry), C_STRLEN_P(entry))

static void sapi_lsapi_ini_defaults(HashTable *configuration_hash)
{
#if CRX_MAJOR_VERSION > 4
/*
    zval *tmp, *entry;
...
static int add_variable( const char * pKey, int keyLen, const char * pValue, int valLen,
                         void * arg )
{
    int filter_arg = (C_ARR_P((zval *)arg) == C_ARR(PG(http_globals)[TRACK_VARS_ENV]))
    ...
    if (sapi_module.input_filter(filter_arg, (char *)pKey, &new_val, valLen, &new_val_len)) {
        crx_register_variable_safe((char *)pKey, new_val, new_val_len, (zval *)arg );
    }
    return 1;
}

static void litespeed_crx_import_environment_variables(zval *array_ptr)
{
    char buf[128];
    char **env, *p, *t = buf;
...
static void sapi_lsapi_register_variables(zval *track_vars_array)
{
    ...
    if ( lsapi_mode ) {
        if ( (SG(request_info).request_uri ) )
            crx_self = (SG(request_info).request_uri );

        litespeed_crx_import_environment_variables(track_vars_array);

        LSAPI_ForeachHeader( add_variable, track_vars_array );
        LSAPI_ForeachEnv( add_variable, track_vars_array );
        add_variable("CRX_SELF", 8, crx_self, strlen( crx_self ), track_vars_array );
    } else {
        ...
        crx_register_variable("DOCUMENT_ROOT", "", track_vars_array);
...
static int sapi_lsapi_send_headers(sapi_headers_struct *sapi_headers)
{
    ...
    if ( lsapi_mode ) {
        ...
        h = crex_llist_get_first_ex(&sapi_headers->headers, &pos);
        while (h) {
            if ( h->header_len > 0 ) {
                LSAPI_AppendRespHeader(h->header, h->header_len);
            }
            h = crex_llist_get_next_ex(&sapi_headers->headers, &pos);
        }
...
#if CRX_MAJOR_VERSION > 7 || (CRX_MAJOR_VERSION == 7 && CRX_MINOR_VERSION >= 1)
                               , 0
#endif
                                  );
}
...
static void init_request_info( void )
{
    ...
    /* It is not reset by crex engine, set it to 200. */
    SG(sapi_headers).http_response_code = 200;

    pAuth = LSAPI_GetHeader( H_AUTHORIZATION );
    crx_handle_auth_data(pAuth);
}
...
static int lsapi_execute_script(void)
{
    ...
    char *p;
    int len;
	crex_stream_init_filename(&file_handle, SG(request_info).path_translated);
...
static void lsapi_clean_shutdown(void)
{
    ...
    crex_alter_ini_entry_chars_ex(key, "0", 1,
                        CRX_INI_SYSTEM, CRX_INI_STAGE_SHUTDOWN, 1);
    crex_string_release(key);
#else
    crex_alter_ini_entry("error_reporting", 16, "0", 1,
                        CRX_INI_SYSTEM, CRX_INI_STAGE_SHUTDOWN);
#endif
                ...
                void * arg )
{
    ...
    int type = CREX_INI_PERDIR;
    int stage = CRX_INI_STAGE_RUNTIME;
    if ( '\001' == *pKey ) {
        ++pKey;
        if ( *pKey == 4 ) {
            type = CREX_INI_SYSTEM;
            /*
            ...
            */
            if(!mod_lsapi_mode)
            {
                stage = CRX_INI_STAGE_ACTIVATE;
            }
        }
        else
        {
            stage = CRX_INI_STAGE_HTACCESS;
        }
        ++pKey;
        --keyLen;
        ...
        {
            --keyLen;
            psKey = crex_string_init(pKey, keyLen, 1);
            crex_alter_ini_entry_chars(psKey,
                             (char *)pValue, valLen,
                             type, stage);
            crex_string_release_ex(psKey, 1);
        }
    }
    return 1;
}
...
{
    crex_hash_init(&user_config_cache, 0, NULL, user_config_cache_entry_dtor, 1);
}
                                           ...
                                           void* next)
{
    ...
    ctx->path_len = crex_dirname(path, ctx->path_len);
                                                  ...
                                                  void* next)
{
    ...
    ctx->entry = crex_hash_str_find_ptr(&user_config_cache, ctx->path, ctx->path_len);

    if (!ctx->entry)
    {
        ctx->entry = pemalloc(sizeof(user_config_cache_entry), 1);
        ctx->entry->expires = 0;
        crex_hash_init(&ctx->entry->user_config, 0, NULL,
                       config_zval_dtor, 1);
        crex_hash_str_update_ptr(&user_config_cache, ctx->path,
                                        ...
                                        void* data)
{
    ...
    crx_parse_user_ini_file(begin, PG(user_ini_filename), &ctx->entry->user_config);
    end[0] = tmp;
}
                                                      ...
                                                      void* next)
{
    ...
    if (!ctx->entry->expires || request_time > ctx->entry->expires)
    {
        ...
        if (is_outside_of_docroot) {
            crx_parse_user_ini_file(ctx->path, PG(user_ini_filename),
                                           ...
                                           void* next)
{
    ...
    crx_ini_activate_config(&ctx->entry->user_config, CRX_INI_PERDIR,
                            CRX_INI_STAGE_HTACCESS);
...
static int processReq(void)
{
    ...
    crex_first_try {
        ...
        if ( engine ) {
            ...
            LSAPI_Write( "Forbidden: CRX engine is disable.\n", 34 );
        }
    } crex_end_try();
    return ret;
}
...
static void cli_usage(void)
{
    ...
    crx_output_activate();
    crx_printf( "%s", usage );
#ifdef CRX_OUTPUT_NEWAPI
    crx_output_end_all();
#else
    crx_end_ob_buffers(1);
#endif
}

static int parse_opt( int argc, char * argv[], int *climode,
                        char **crx_ini_path, char ** crx_bind )
{
    char ** p = &argv[1];
...
static int cli_main( int argc, char * argv[] )
{
    ...
    crex_first_try     {
        ...
        crex_uv.html_errors = 0; /* tell the engine we're in non-html mode */
        ...
        for( ini = ini_defaults; *ini; ini+=2 ) {
            psKey = crex_string_init(*ini, strlen( *ini ), 1);
            crex_alter_ini_entry_chars(psKey,
                                (char *)*(ini+1), strlen( *(ini+1) ),
                                CRX_INI_SYSTEM, CRX_INI_STAGE_ACTIVATE);
            crex_string_release_ex(psKey, 1);
        }
        ...
        if ( ret == -1 ) {
            if ( *p ) {
				crex_file_handle file_handle;
				crex_stream_init_fp(&file_handle, VCWD_FOPEN(*p, "rb"), NULL);
                ...
                if ( file_handle.handle.fp ) {
                    ...
                    if (crx_request_startup() == FAILURE ) {
                        ...
                        if (source_highlight == 1) {
                            ...
                            file_handle.filename = crex_string_init(*p, strlen(*p), 0);
                            file_handle.opened_path = NULL;
                            ret = crx_lint_script(&file_handle);
                            if (ret==SUCCESS) {
...
int main( int argc, char * argv[] )
{
...
#if CRX_MAJOR_VERSION >= 7
#if defined(CREX_SIGNALS) || CRX_MINOR_VERSION > 0
    crex_signal_startup();
#endif
#endif
    ...
    if (argc > 1 ) {
        if ( parse_opt( argc, argv, &climode,
                &crx_ini_path, &crx_bind ) == -1 ) {
            return 1;
        }
    }
    if ( climode ) {
        lsapi_sapi_module.crxinfo_as_text = 1;
    } else {
        setArgv0(argc, argv );
    }
    ...
    if ( ignore_crx_ini )
        lsapi_sapi_module.crx_ini_ignore = 1;

    if ( crx_ini_path ) {
        lsapi_sapi_module.crx_ini_path_override = crx_ini_path;
    }


    lsapi_sapi_module.ini_defaults = sapi_lsapi_ini_defaults;

    if (crx_module_startup(&lsapi_sapi_module, &litespeed_module_entry) == FAILURE) {
#ifdef ZTS
        tsrm_shutdown();
#endif
        return FAILURE;
    ...
    if ( crx_bind ) {
        bindFd = LSAPI_CreateListenSock( crx_bind, 10 );
        if ( bindFd == -1 ) {
            fprintf( stderr,
                     "Failed to bind socket [%s]: %s\n", crx_bind, strerror( errno ) );
            exit( 2 );
        }
        if ( bindFd != 0 ) {
            dup2( bindFd, 0 );
    ...
    while( ( result = LSAPI_Prefork_Accept_r( &g_req )) >= 0 ) {
        ...
        if ( slow_script_msec ) {
            ...
            if ( n > slow_script_msec )
            {
                ...
                fprintf( stderr, "[%s] Slow CRX script: %d ms\n  URL: %s %s\n  Query String: %s\n  Script: %s\n",
...
/*   LiteSpeed CRX module starts here */

CRX_MINFO_FUNCTION(litespeed);

static CRX_MINIT_FUNCTION(litespeed)
{
    user_config_cache_init();

    const char *p = getenv("LSCRX_ENABLE_USER_INI");
...
static CRX_MSHUTDOWN_FUNCTION(litespeed)
{
    crex_hash_destroy(&user_config_cache);
...
CRX_FUNCTION(litespeed_request_headers)
{
...
CRX_FUNCTION(litespeed_response_headers)
{
    sapi_header_struct  *h;
    crex_llist_position pos;
    char *       p;
    ...
    h = crex_llist_get_first_ex(&SG(sapi_headers).headers, &pos);
    while (h) {
        if ( h->header_len > 0 ) {
            p = strchr( h->header, ':' );
            len = p - h->header;
        ...
        h = crex_llist_get_next_ex(&SG(sapi_headers).headers, &pos);
    }
}
...
CRX_FUNCTION(litespeed_finish_request)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}
