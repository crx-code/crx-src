...
static void print_modules(void)
{
	...
	crex_hash_init(&sorted_registry, 64, NULL, NULL, 1);
...
static char *cgi_getenv_win32(const char *name, size_t name_len)
{
	...
	keyw = crx_win32_cp_conv_any_to_w(name, name_len, CRX_WIN32_CP_IGNORE_LEN_P);
	if (!keyw) {
		return NULL;
	}
	...
	if (!rc) {
		ret = crx_win32_cp_w_to_any(valw);
	}
...
{
	/* when crx is started by mod_fastcgi, no regular environment
	 * is provided to CRX.  It is always sent to CRX at the start
...
static void cgi_crx_import_environment_variables(zval *array_ptr)
{
	...
	if (CGIG(fix_pathinfo)) {
		...
		if (path_info) {
			...
			if (script_name) {
				...
				crx_self = do_alloca(crx_self_len + 1, use_heap);
				memcpy(crx_self, script_name, script_name_len + 1);
				memcpy(crx_self + script_name_len, path_info, path_info_len + 1);
				free_crx_self = 1;
			}  else {
				crx_self = path_info;
				crx_self_len = path_info_len;
				free_crx_self = 0;
			}
		} else if (script_name) {
			crx_self = script_name;
			crx_self_len = strlen(script_name);
			free_crx_self = 0;
		} else {
			crx_self = "";
			crx_self_len = 0;
...
static void crx_cgi_ini_activate_user_config(char *path, size_t path_len, const char *doc_root, size_t doc_root_len)
{
	user_config_cache_entry *new_entry, *entry;
	time_t request_time = (time_t)sapi_get_request_time();

	/* Find cached config entry: If not found, create one */
	if ((entry = crex_hash_str_find_ptr(&CGIG(user_config_cache), path, path_len)) == NULL) {
		new_entry = pemalloc(sizeof(user_config_cache_entry), 1);
		new_entry->expires = 0;
		new_entry->user_config = (HashTable *) pemalloc(sizeof(HashTable), 1);
		crex_hash_init(new_entry->user_config, 8, NULL, (dtor_func_t) config_zval_dtor, 1);
		entry = crex_hash_str_update_ptr(&CGIG(user_config_cache), path, path_len, new_entry);
	}
	...
	if (crx_ini_has_per_dir_config() ||
		...
		if (PG(user_ini_filename) && *PG(user_ini_filename)) {
			...
			if (doc_root) {
				...
				crex_str_tolower(doc_root, doc_root_len);
#endif
...
static void crx_cgi_usage(char *argv0)
{
	...
	crx_printf(	"Usage: %s [-q] [-h] [-s] [-v] [-i] [-f <file>]\n"
				"       %s <file> [args...]\n"
				"  -a               Run interactively\n"
		...
		if (CGIG(fix_pathinfo)) {
			...
			if (script_path_translated &&
				...
				while ((ptr = strrchr(pt, '/')) || (ptr = strrchr(pt, '\\'))) {
					*ptr = 0;
					if (crex_stat(pt, &st) == 0 && S_ISREG(st.st_mode)) {
						/*
...
CRX_INI_BEGIN()
	...
	STD_CRX_INI_BOOLEAN("cgi.check_shebang_line",  "1",  CRX_INI_SYSTEM, OnUpdateBool,   check_shebang_line, crx_cgi_globals_struct, crx_cgi_globals)
...
#ifdef CRX_WIN32
	STD_CRX_INI_BOOLEAN("fastcgi.impersonate",     "0",  CRX_INI_SYSTEM, OnUpdateBool,   impersonate, crx_cgi_globals_struct, crx_cgi_globals)
#endif
CRX_INI_END()
...
static void crx_cgi_globals_ctor(crx_cgi_globals_struct *crx_cgi_globals)
{
	...
	crx_cgi_globals->check_shebang_line = 1;
...
#ifdef CRX_WIN32
	crx_cgi_globals->impersonate = 0;
#endif
	crex_hash_init(&crx_cgi_globals->user_config_cache, 8, NULL, user_config_cache_entry_dtor, 1);
}
/* }}} */
...
CRX_FUNCTION(apache_child_terminate) /* {{{ */
{
	if (crex_parse_parameters_none()) {
		RETURN_THROWS();
	}
	if (fcgi_is_fastcgi()) {
		fcgi_terminate();
...
# endif
	...
	if((query_string = getenv("QUERY_STRING")) != NULL && strchr(query_string, '=') == NULL) {
		...
		crx_url_decode(decoded_query_string, strlen(decoded_query_string));
	...
	while (!skip_getopt && (c = crx_getopt(argc, argv, OPTIONS, &crx_optarg, &crx_optind, 0, 2)) != -1) {
		switch (c) {
			case 'c':
...
<a href=\"http://crx.net/install.windows\">the manual page</a>.</p>\n");
...
#if defined(ZTS) && !defined(CRX_DEBUG)
	...
	if (bindpath) {
		int backlog = 128;
		if (getenv("CRX_FCGI_BACKLOG")) {
			backlog = atoi(getenv("CRX_FCGI_BACKLOG"));
		}
	...
	if (fastcgi) {
		/* How many times to run CRX scripts before dying */
		if (getenv("CRX_FCGI_MAX_REQUESTS")) {
			max_requests = atoi(getenv("CRX_FCGI_MAX_REQUESTS"));
			if (max_requests < 0) {
				fprintf(stderr, "CRX_FCGI_MAX_REQUESTS is not valid\n");
				return FAILURE;
			}
		}
		...
		if (getenv("CRX_FCGI_CHILDREN")) {
			char * children_str = getenv("CRX_FCGI_CHILDREN");
			children = atoi(children_str);
			if (children < 0) {
				fprintf(stderr, "CRX_FCGI_CHILDREN is not valid\n");
				return FAILURE;
			}
			...
			/* If this env var is set, the process was invoked as a child. Let
				it show the original CRX_FCGI_CHILDREN value, while don't care
				otherwise. */
			char * children_str = getenv("CRX_FCGI_CHILDREN_FOR_KID");
			if (children_str) {
				char putenv_buf[sizeof("CRX_FCGI_CHILDREN")+5];

				snprintf(putenv_buf, sizeof(putenv_buf), "%s=%s", "CRX_FCGI_CHILDREN", children_str);
				putenv(putenv_buf);
				putenv("CRX_FCGI_CHILDREN_FOR_KID=");

				SetEnvironmentVariable("CRX_FCGI_CHILDREN", children_str);
				SetEnvironmentVariable("CRX_FCGI_CHILDREN_FOR_KID", NULL);
			}
#endif
...
#else
		if (children) {
			wchar_t *cmd_line_tmp, cmd_line[CRX_WIN32_IOUTIL_MAXPATHLEN];
			size_t cmd_line_len;
			char kid_buf[16];
			int i;
			...
			SetEnvironmentVariable("CRX_FCGI_CHILDREN", NULL);
			...
			SetEnvironmentVariable("CRX_FCGI_CHILDREN_FOR_KID", kid_buf);
			...
			if (!cmd_line_tmp) {
				...
				char *err_text = crx_win32_error_to_msg(err);

				fprintf(stderr, "unable to get current command line: [0x%08lx]: %s\n", err, err_text);
				crx_win32_error_msg_free(err_text);
...
parent_loop_end:
			/* restore my env */
			SetEnvironmentVariable("CRX_FCGI_CHILDREN", kid_buf);
	...
	crex_first_try {
		while (!skip_getopt && (c = crx_getopt(argc, argv, OPTIONS, &crx_optarg, &crx_optind, 1, 2)) != -1) {
			switch (c) {
				case 'T':
					benchmark = 1;
					{
						char *comma = strchr(crx_optarg, ',');
						if (comma) {
							warmup_repeats = atoi(crx_optarg);
				...
				while ((c = crx_getopt(argc, argv, OPTIONS, &crx_optarg, &crx_optind, 0, 2)) != -1) {
					switch (c) {
						...
						case 'f': /* parse file */
							...
							script_file = estrdup(crx_optarg);
							no_headers = 1;
							break;
...
do_repeat:
				...
				} else if (argc > crx_optind) {
					/* file is on command line, but not in -f opt */
					if (SG(request_info).path_translated) efree(SG(request_info).path_translated);
					SG(request_info).path_translated = estrdup(argv[crx_optind]);
					/* arguments after the file are considered script args */
					SG(request_info).argc = argc - crx_optind;
					SG(request_info).argv = &argv[crx_optind];
				}
				 ...
				 *
				 *  test.crx v1=test v2=hello+world!
				 *  test.crx "v1=test&v2=hello world!"
				 *  test.crx v1=test "v2=hello world!"
				*/
				if (!SG(request_info).query_string && argc > crx_optind) {
					size_t slen = strlen(PG(arg_separator).input);
					len = 0;
					for (i = crx_optind; i < argc; i++) {
						if (i < (argc - 1)) {
			...
			*/
			if (cgi || fastcgi || SG(request_info).path_translated) {
				if (crx_fopen_primary_script(&file_handle) == FAILURE) {
					crex_try {
						if (errno == EACCES) {
				...
				crex_stream_init_fp(&file_handle, stdin, "Standard input code");
...
fastcgi_request_done:
			...
			if (!fastcgi) {
				...
				if (behavior == CRX_MODE_LINT && argc - 1 > crx_optind) {
					crx_optind++;
					script_file = NULL;
					goto do_repeat;
				}
...
#if defined(CRX_WIN32) && CREX_DEBUG && 0
	_CrtDumpMemoryLeaks();
#endif
