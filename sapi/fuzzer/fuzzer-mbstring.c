...
crex_string* convert_encoding(const uint8_t *Data, size_t Size, const mbfl_encoding *FromEncoding, const mbfl_encoding *ToEncoding, size_t BufSize, unsigned int *NumErrors)
{
	...
	while (Size) {
		...
		CREX_ASSERT(out_len <= BufSize);
		ToEncoding->from_wchar(wchar_buf, out_len, &buf, !Size);
	}

	*NumErrors = buf.errors;
	crex_string *result = mb_convert_buf_result(&buf, ToEncoding);
	efree(wchar_buf);
	return result;
}

void assert_crex_string_eql(crex_string *str1, crex_string *str2)
{
	CREX_ASSERT(ZSTR_LEN(str1) == ZSTR_LEN(str2));
	for (int i = 0; i < ZSTR_LEN(str1); i++) {
		CREX_ASSERT(ZSTR_VAL(str1)[i] == ZSTR_VAL(str2)[i]);
	}
}
...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t Size) {
	...
	crex_string *Result1 = convert_encoding(Data, Size, FromEncoding, ToEncoding, bufsize1, &errors1);
	crex_string *Result2 = convert_encoding(Data, Size, FromEncoding, ToEncoding, bufsize2, &errors2);

	assert_crex_string_eql(Result1, Result2);
	CREX_ASSERT(errors1 == errors2);
	...
	crex_string_release(Result1);
	crex_string_release(Result2);
	efree(ToEncodingName);
	efree(FromEncodingName);
