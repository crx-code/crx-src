...
int LLVMFuzzerTestOneInput(const uint8_t *Data, size_t FullSize) {
...

	{
		...
		if (C_TYPE(result) == IS_OBJECT
			&& crex_string_equals_literal(C_OBJCE(result)->name, "HashContext")) {
			zval args[2];
			ZVAL_COPY_VALUE(&args[0], &result);
			ZVAL_STRINGL(&args[1], (char *) Data, (Start - Data) - 1);
			fuzzer_call_crx_func_zval("hash_update", 2, args);
			zval_ptr_dtor(&args[1]);
			fuzzer_call_crx_func_zval("hash_final", 1, args);
		}
