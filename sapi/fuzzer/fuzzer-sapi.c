...
const char HARDCODED_INI[] =
	...
	"output_buffering=0\n"
	"error_reporting=0\n"
	/* Let the timeout be enforced by libfuzzer, not CRX. */
...
int fuzzer_init_crx(const char *extra_ini)
{
#ifdef __SANITIZE_ADDRESS__
	...
	fuzzer_module.crxinfo_as_text = 1;
	...
	*p = '\0';
	...
	/*
	 * TODO: we might want to test both Crex and malloc MM, but testing with malloc
	 * is more likely to find bugs, so use that for now.
	 */
	putenv("USE_CREX_ALLOC=0");
...
{
	static crex_execute_data execute_data;
	static crex_function func;

	memset(&execute_data, 0, sizeof(crex_execute_data));
	memset(&func, 0, sizeof(crex_function));
...
{
	if (fuzzer_module.crx_ini_path_override) {
		free(fuzzer_module.crx_ini_path_override);
	}
	fuzzer_module.crx_ini_path_override = strdup(file);
}


int fuzzer_shutdown_crx(void)
{
	crx_module_shutdown();
	sapi_shutdown();
		...
		void (*before_shutdown)(void))
{
	...
	crx_register_variable("CRX_SELF", filename, NULL);

	crex_first_try {
		crex_file_handle file_handle;
		crex_stream_init_filename(&file_handle, filename);
		file_handle.primary_script = 1;
		file_handle.buf = emalloc(data_len + CREX_MMAP_AHEAD);
		memcpy(file_handle.buf, data, data_len);
		memset(file_handle.buf + data_len, 0, CREX_MMAP_AHEAD);
		file_handle.len = data_len;
		/* Avoid CREX_HANDLE_FILENAME for opcache. */
		file_handle.type = CREX_HANDLE_STREAM;

		crex_op_array *op_array = crex_compile_file(&file_handle, CREX_REQUIRE);
...
// Call named CRX function with N zval arguments
void fuzzer_call_crx_func_zval(const char *func_name, int nargs, zval *args) {
	zval retval, func;
...
// Call named CRX function with N string arguments
void fuzzer_call_crx_func(const char *func_name, int nargs, char **params) {
	zval args[nargs];
	int i;
	...
	fuzzer_call_crx_func_zval(func_name, nargs, args);
