...
void vzlog(const char *function, int line, int flags, const char *fmt, va_list args) /* {{{ */
{
	...
	if (zlog_fd == ZLOG_SYSLOG) {
		buf[len] = '\0';
		crx_syslog(syslog_priorities[zlog_level], "%s", buf);
		buf[len++] = '\n';
	} else
#endif
	{
		buf[len++] = '\n';
		crex_quiet_write(zlog_fd > -1 ? zlog_fd : STDERR_FILENO, buf, len);
	}

	if (zlog_fd != STDERR_FILENO && zlog_fd != -1 &&
			!launched && (flags & ZLOG_LEVEL_MASK) >= ZLOG_NOTICE) {
		crex_quiet_write(STDERR_FILENO, buf, len);
	}
}
/* }}} */
...
{
	if (stream->use_fd) {
		crex_quiet_write(stream->fd, buf, len);
		if (append_len > 0) {
			crex_quiet_write(stream->fd, append, append_len);
		}
	}
	...
	if (stream->use_stderr) {
		crex_quiet_write(STDERR_FILENO, buf, len);
		if (append_len > 0) {
			crex_quiet_write(STDERR_FILENO, append, append_len);
		}
	}
...
static ssize_t zlog_stream_buf_flush(struct zlog_stream *stream) /* {{{ */
{
	...
	if (stream->use_syslog) {
		...
		crx_syslog(syslog_priorities[zlog_level], "%s", stream->buf.data);
		--stream->len;
	}
#endif
