...
/*
 * Please keep the same order as in fpm_conf.h and in crx-fpm.conf.in
 */
...
static char *fpm_conf_set_boolean(zval *value, void **config, intptr_t offset) /* {{{ */
{
	crex_string *val = C_STR_P(value);
	bool value_y = crex_string_equals_literal(val, "1");
...
static char *fpm_conf_set_string(zval *value, void **config, intptr_t offset) /* {{{ */
{
	...
	*config_val = strdup(C_STRVAL_P(value));
	if (!*config_val) {
...
static char *fpm_conf_set_log_level(zval *value, void **config, intptr_t offset) /* {{{ */
{
	...
	if (crex_string_equals_literal_ci(val, "debug")) {
		log_level = ZLOG_DEBUG;
	} else if (crex_string_equals_literal_ci(val, "notice")) {
		log_level = ZLOG_NOTICE;
	} else if (crex_string_equals_literal_ci(val, "warning") || crex_string_equals_literal_ci(val, "warn")) {
		log_level = ZLOG_WARNING;
	} else if (crex_string_equals_literal_ci(val, "error")) {
		log_level = ZLOG_ERROR;
	} else if (crex_string_equals_literal_ci(val, "alert")) {
...
#ifdef LOG_AUTH
	if (crex_string_equals_literal_ci(val, "AUTH")) {
		*conf = LOG_AUTH;
		return NULL;
	}
#endif
...
#ifdef LOG_AUTHPRIV
	if (crex_string_equals_literal_ci(val, "AUTHPRIV")) {
		*conf = LOG_AUTHPRIV;
		return NULL;
	}
#endif
...
#ifdef LOG_CRON
	if (crex_string_equals_literal_ci(val, "CRON")) {
		*conf = LOG_CRON;
		return NULL;
	}
#endif
...
#ifdef LOG_DAEMON
	if (crex_string_equals_literal_ci(val, "DAEMON")) {
		*conf = LOG_DAEMON;
		return NULL;
	}
#endif
...
#ifdef LOG_FTP
	if (crex_string_equals_literal_ci(val, "FTP")) {
		*conf = LOG_FTP;
		return NULL;
	}
#endif
...
#ifdef LOG_KERN
	if (crex_string_equals_literal_ci(val, "KERN")) {
		*conf = LOG_KERN;
		return NULL;
	}
#endif
...
#ifdef LOG_LPR
	if (crex_string_equals_literal_ci(val, "LPR")) {
		*conf = LOG_LPR;
		return NULL;
	}
#endif
...
#ifdef LOG_MAIL
	if (crex_string_equals_literal_ci(val, "MAIL")) {
		*conf = LOG_MAIL;
		return NULL;
	}
#endif
...
#ifdef LOG_NEWS
	if (crex_string_equals_literal_ci(val, "NEWS")) {
		*conf = LOG_NEWS;
		return NULL;
	}
#endif
...
#ifdef LOG_SYSLOG
	if (crex_string_equals_literal_ci(val, "SYSLOG")) {
		*conf = LOG_SYSLOG;
		return NULL;
	}
#endif
...
#ifdef LOG_USER
	if (crex_string_equals_literal_ci(val, "USER")) {
		*conf = LOG_USER;
		return NULL;
	}
#endif
...
#ifdef LOG_UUCP
	if (crex_string_equals_literal_ci(val, "UUCP")) {
		*conf = LOG_UUCP;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL0
	if (crex_string_equals_literal_ci(val, "LOCAL0")) {
		*conf = LOG_LOCAL0;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL1
	if (crex_string_equals_literal_ci(val, "LOCAL1")) {
		*conf = LOG_LOCAL1;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL2
	if (crex_string_equals_literal_ci(val, "LOCAL2")) {
		*conf = LOG_LOCAL2;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL3
	if (crex_string_equals_literal_ci(val, "LOCAL3")) {
		*conf = LOG_LOCAL3;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL4
	if (crex_string_equals_literal_ci(val, "LOCAL4")) {
		*conf = LOG_LOCAL4;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL5
	if (crex_string_equals_literal_ci(val, "LOCAL5")) {
		*conf = LOG_LOCAL5;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL6
	if (crex_string_equals_literal_ci(val, "LOCAL6")) {
		*conf = LOG_LOCAL6;
		return NULL;
	}
#endif
...
#ifdef LOG_LOCAL7
	if (crex_string_equals_literal_ci(val, "LOCAL7")) {
		*conf = LOG_LOCAL7;
		return NULL;
	}
#endif
...
static char *fpm_conf_set_rlimit_core(zval *value, void **config, intptr_t offset) /* {{{ */
{
	...
	if (crex_string_equals_literal_ci(val, "unlimited")) {
		*ptr = -1;
	} else {
		int int_value;
...
static char *fpm_conf_set_pm(zval *value, void **config, intptr_t offset) /* {{{ */
{
	...
	if (crex_string_equals_ci(val, ZSTR_KNOWN(CREX_STR_STATIC))) {
		c->pm = PM_STYLE_STATIC;
	} else if (crex_string_equals_literal_ci(val, "dynamic")) {
		c->pm = PM_STYLE_DYNAMIC;
	} else if (crex_string_equals_literal_ci(val, "ondemand")) {
...
static char *fpm_conf_set_array(zval *key, zval *value, void **config, int convert_to_bool) /* {{{ */
{
	...
	if (convert_to_bool) {
		...
		kv->value = strdup(C_STRVAL_P(value));
...
int fpm_worker_pool_config_free(struct fpm_worker_pool_config_s *wpc) /* {{{ */
{
	...
	for (kv = wpc->crx_values; kv; kv = kv_next) {
		kv_next = kv->next;
		free(kv->key);
		free(kv->value);
		free(kv);
	}
	for (kv = wpc->crx_admin_values; kv; kv = kv_next) {
		kv_next = kv->next;
		free(kv->key);
		free(kv->value);
		free(kv);
...
static int fpm_worker_pool_shared_status_alloc(struct fpm_worker_pool_s *shared_wp) { /* {{{ */
	...
	fpm_conf_apply_kv_array_to_kv_array(shared_config->crx_values, (char *)config + WPO(crx_values));
	fpm_conf_apply_kv_array_to_kv_array(shared_config->crx_admin_values, (char *)config + WPO(crx_admin_values));
...
static int fpm_evaluate_full_path(char **path, struct fpm_worker_pool_s *wp, char *default_prefix, int expand) /* {{{ */
{
	...
	if (prefix == NULL) {
		prefix = default_prefix ? default_prefix : CRX_PREFIX;
	}
...
static int fpm_conf_process_all_pools(void)
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		if (!wp->config->security_limit_extensions) {
			wp->config->security_limit_extensions = strdup(".crx .crxa");
		}
		...
		/* env[], crx_value[], crx_admin_values[] */
		if (!wp->config->chroot) {
			struct key_value_s *kv;
			static const char *const options[] = FPM_CRX_INI_TO_EXPAND;
...
static int fpm_conf_post_process(int force_daemon) /* {{{ */
{
	...
	if (fpm_global_config.pid_file) {
		fpm_evaluate_full_path(&fpm_global_config.pid_file, NULL, CRX_LOCALSTATEDIR, 0);
	}
	...
	if (!fpm_global_config.error_log) {
		fpm_global_config.error_log = strdup("log/crx-fpm.log");
	}
	...
	if (!fpm_global_config.syslog_ident) {
		fpm_global_config.syslog_ident = strdup("crx-fpm");
	}
...
#endif
	{
		fpm_evaluate_full_path(&fpm_global_config.error_log, NULL, CRX_LOCALSTATEDIR, 0);
	}
...
static void fpm_conf_ini_parser_section(zval *section, void *arg) /* {{{ */
{
	...
	if (crex_string_equals_literal_ci(C_STR_P(section), "global")) {
		current_wp = NULL;
		return;
	}
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		if (!strcasecmp(wp->config->name, C_STRVAL_P(section))) {
	...
	if (!current_wp || !config) {
		zlog(ZLOG_ERROR, "[%s:%d] Unable to alloc a new WorkerPool for worker '%s'", ini_filename, ini_lineno, C_STRVAL_P(section));
		*error = 1;
		return;
	}
	config->name = strdup(C_STRVAL_P(section));
	if (!config->name) {
		zlog(ZLOG_ERROR, "[%s:%d] Unable to alloc memory for configuration name for worker '%s'", ini_filename, ini_lineno, C_STRVAL_P(section));
		*error = 1;
		return;
	}
}
...
static void fpm_conf_ini_parser_entry(zval *name, zval *value, void *arg) /* {{{ */
{
	...
	int *error = (int *)arg;
	if (!value) {
		zlog(ZLOG_ERROR, "[%s:%d] value is NULL for a CREX_INI_PARSER_ENTRY", ini_filename, ini_lineno);
		*error = 1;
		return;
	}

	if (crex_string_equals_literal(C_STR_P(name), "include")) {
		if (ini_include) {
			zlog(ZLOG_ERROR, "[%s:%d] two includes at the same time !", ini_filename, ini_lineno);
			*error = 1;
			return;
		}
		ini_include = strdup(C_STRVAL_P(value));
		return;
	}
	...
	for (; parser->name; parser++) {
		if (!strcasecmp(parser->name, C_STRVAL_P(name))) {
			char *ret;
			if (!parser->parser) {
	...
	zlog(ZLOG_ERROR, "[%s:%d] unknown entry '%s'", ini_filename, ini_lineno, C_STRVAL_P(name));
	*error = 1;
}
/* }}} */
...
static void fpm_conf_ini_parser_array(zval *name, zval *key, zval *value, void *arg) /* {{{ */
{
	...
	if (crex_string_equals_literal(C_STR_P(name), "access.suppress_path")) {
		if (!(*C_STRVAL_P(key) == '\0')) {
			zlog(ZLOG_ERROR, "[%s:%d] Keys provided to field 'access.suppress_path' are ignored", ini_filename, ini_lineno);
			*error = 1;
		}
		if (!(*C_STRVAL_P(value)) || (*C_STRVAL_P(value) != '/')) {
	...
	} else if (!*C_STRVAL_P(key)) {
		zlog(ZLOG_ERROR, "[%s:%d] You must provide a key for field '%s'", ini_filename, ini_lineno, C_STRVAL_P(name));
		*error = 1;
		return;
	}
	...
	if (crex_string_equals_literal(C_STR_P(name), "env")) {
		if (!*C_STRVAL_P(value)) {
	...
	} else if (crex_string_equals_literal(C_STR_P(name), "crx_value")) {
		config = (char *)current_wp->config + WPO(crx_values);
		err = fpm_conf_set_array(key, value, &config, 0);

	} else if (crex_string_equals_literal(C_STR_P(name), "crx_admin_value")) {
		config = (char *)current_wp->config + WPO(crx_admin_values);
		err = fpm_conf_set_array(key, value, &config, 0);

	} else if (crex_string_equals_literal(C_STR_P(name), "crx_flag")) {
		config = (char *)current_wp->config + WPO(crx_values);
		err = fpm_conf_set_array(key, value, &config, 1);

	} else if (crex_string_equals_literal(C_STR_P(name), "crx_admin_flag")) {
		config = (char *)current_wp->config + WPO(crx_admin_values);
		err = fpm_conf_set_array(key, value, &config, 1);

	} else if (crex_string_equals_literal(C_STR_P(name), "access.suppress_path")) {
		config = (char *)current_wp->config + WPO(access_suppress_paths);
		err = fpm_conf_set_array(NULL, value, &config, 0);

	} else {
		zlog(ZLOG_ERROR, "[%s:%d] unknown directive '%s'", ini_filename, ini_lineno, C_STRVAL_P(name));
		*error = 1;
		return;
	}

	if (err) {
		zlog(ZLOG_ERROR, "[%s:%d] error while parsing '%s[%s]' : %s", ini_filename, ini_lineno, C_STRVAL_P(name), C_STRVAL_P(key), err);
		*error = 1;
		return;
	}
}
...
int fpm_conf_load_ini_file(char *filename) /* {{{ */
{
	...
	while (nb_read > 0) {
		...
		buf[n++] = '\n';
		buf[n] = '\0';
		tmp = crex_parse_ini_string(buf, 1, CREX_INI_SCANNER_NORMAL, (crex_ini_parser_cb_t)fpm_conf_ini_parser, &error);
		ini_filename = filename;
...
static void fpm_conf_dump(void)
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		...
		for (kv = wp->config->crx_values; kv; kv = kv->next) {
			zlog(ZLOG_NOTICE, "\tcrx_value[%s] = %s", kv->key, kv->value);
		}

		for (kv = wp->config->crx_admin_values; kv; kv = kv->next) {
			zlog(ZLOG_NOTICE, "\tcrx_admin_value[%s] = %s", kv->key, kv->value);
		}
		zlog(ZLOG_NOTICE, " ");
	}
}
...
int fpm_conf_init_main(int test_conf, int force_daemon) /* {{{ */
{
	...
	if (fpm_globals.config == NULL) {
		...
		if (fpm_globals.prefix == NULL) {
			spprintf(&tmp, 0, "%s/crx-fpm.conf", CRX_SYSCONFDIR);
		} else {
			spprintf(&tmp, 0, "%s/etc/crx-fpm.conf", fpm_globals.prefix);
		}
