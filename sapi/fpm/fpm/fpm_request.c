...
{
	struct fpm_scoreboard_proc_s *proc;
	char *request_uri = fpm_crx_request_uri();
	char *request_method = fpm_crx_request_method();
	char *script_filename = fpm_crx_script_filename();
	char *query_string = fpm_crx_query_string();
	char *auth_user = fpm_crx_auth_user();
	size_t content_length = fpm_crx_content_length();
	struct timeval now;
...
void fpm_request_end(void)
{
	...
	struct tms cpu;
#endif
	size_t memory = crex_memory_peak_usage(1);
