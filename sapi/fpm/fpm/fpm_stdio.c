...
static inline int fpm_use_error_log(void) {
	 ...
	 * - SysV init launch crx-fpm as a daemon
	 * - Systemd launch crx-fpm in foreground
	 */
#ifdef HAVE_UNISTD_H
...
int fpm_stdio_save_original_stderr(void)
{
	 ...
	 * It should be used only when CRX-FPM is not daemonized otherwise it might break some
...
#ifdef HAVE_SYSLOG_H
	if (fpm_globals.error_log_fd == ZLOG_SYSLOG) {
		crx_closelog(); /* ensure to close syslog not to interrupt with CRX syslog code */
	} else
#endif
...
stdio_read:
		...
		if (in_buf <= 0) { /* no data */
			if (in_buf == 0 || !CRX_IS_TRANSIENT_ERROR(errno)) {
...
int fpm_stdio_open_error_log(int reopen) /* {{{ */
{
	...
	if (!strcasecmp(fpm_global_config.error_log, "syslog")) {
		crx_openlog(fpm_global_config.syslog_ident, LOG_PID | LOG_CONS, fpm_global_config.syslog_facility);
