...
if test "$CRX_FPM" != "no"; then
  AC_MSG_RESULT($CRX_FPM)
  ...
  if test "$CRX_FPM_SYSTEMD" != "no" ; then
    ...
    if test $HAVE_SD_DAEMON_H = "no"; then
      ...
      CRX_FPM_SD_FILES="fpm/fpm_systemd.c"
      CRX_EVAL_LIBLINE($SYSTEMD_LIBS)
      CRX_EVAL_INCLINE($SYSTEMD_CFLAGS)
      crx_fpm_systemd=notify
    fi
  else
    crx_fpm_systemd=simple
  fi

  if test "$CRX_FPM_ACL" != "no" ; then
  ...
  if test "x$CRX_FPM_SELINUX" != "xno" ; then
    AC_CHECK_HEADERS([selinux/selinux.h])
    AC_CHECK_LIB(selinux, security_setenforce, [
      CRX_ADD_LIBRARY(selinux)
      AC_DEFINE(HAVE_SELINUX, 1, [ SElinux available ])
    ],[])
  fi

  CRX_SUBST_OLD(crx_fpm_systemd)
  AC_DEFINE_UNQUOTED(CRX_FPM_SYSTEMD, "$crx_fpm_systemd", [fpm systemd service type])

  if test -z "$CRX_FPM_USER" -o "$CRX_FPM_USER" = "yes" -o "$CRX_FPM_USER" = "no"; then
    crx_fpm_user="nobody"
  else
    crx_fpm_user="$CRX_FPM_USER"
  fi

  if test -z "$CRX_FPM_GROUP" -o "$CRX_FPM_GROUP" = "yes" -o "$CRX_FPM_GROUP" = "no"; then
    crx_fpm_group="nobody"
  else
    crx_fpm_group="$CRX_FPM_GROUP"
  fi

  CRX_SUBST_OLD(crx_fpm_user)
  CRX_SUBST_OLD(crx_fpm_group)
  crx_fpm_sysconfdir=`eval echo $sysconfdir`
  CRX_SUBST_OLD(crx_fpm_sysconfdir)
  crx_fpm_localstatedir=`eval echo $localstatedir`
  CRX_SUBST_OLD(crx_fpm_localstatedir)
  crx_fpm_prefix=`eval echo $prefix`
  CRX_SUBST_OLD(crx_fpm_prefix)

  AC_DEFINE_UNQUOTED(CRX_FPM_USER, "$crx_fpm_user", [fpm user name])
  AC_DEFINE_UNQUOTED(CRX_FPM_GROUP, "$crx_fpm_group", [fpm group name])

  CRX_ADD_BUILD_DIR(sapi/fpm/fpm)
  CRX_ADD_BUILD_DIR(sapi/fpm/fpm/events)
  CRX_OUTPUT(sapi/fpm/crx-fpm.conf sapi/fpm/www.conf sapi/fpm/init.d.crx-fpm sapi/fpm/crx-fpm.service sapi/fpm/crx-fpm.8 sapi/fpm/status.html)
  CRX_ADD_MAKEFILE_FRAGMENT([$abs_srcdir/sapi/fpm/Makefile.frag])

  SAPI_FPM_PATH=sapi/fpm/crx-fpm

  if test "$fpm_trace_type" && test -f "$abs_srcdir/sapi/fpm/fpm/fpm_trace_$fpm_trace_type.c"; then
    CRX_FPM_TRACE_FILES="fpm/fpm_trace.c fpm/fpm_trace_$fpm_trace_type.c"
  fi

  CRX_FPM_CFLAGS="-I$abs_srcdir/sapi/fpm -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1"

  CRX_FPM_FILES="fpm/fpm.c \
    fpm/fpm_children.c \
    fpm/fpm_cleanup.c \
    fpm/fpm_clock.c \
    fpm/fpm_conf.c \
  ...
  CRX_SELECT_SAPI(fpm, program, $CRX_FPM_FILES $CRX_FPM_TRACE_FILES $CRX_FPM_SD_FILES, $CRX_FPM_CFLAGS, '$(SAPI_FPM_PATH)')

  case $host_alias in
      *aix*)
        BUILD_FPM="echo '\#! .' > crx.sym && echo >>crx.sym && nm -BCpg \`echo \$(CRX_GLOBAL_OBJS) \$(CRX_BINARY_OBJS) \$(CRX_FPM_OBJS) | sed 's/\([A-Za-z0-9_]*\)\.lo/\1.o/g'\` | \$(AWK) '{ if (((\$\$2 == \"T\") || (\$\$2 == \"D\") || (\$\$2 == \"B\")) && (substr(\$\$3,1,1) != \".\")) { print \$\$3 } }' | sort -u >> crx.sym && \$(LIBTOOL) --mode=link \$(CC) -export-dynamic \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) -Wl,-brtl -Wl,-bE:crx.sym \$(CRX_RPATHS) \$(CRX_GLOBAL_OBJS) \$(CRX_BINARY_OBJS) \$(CRX_FASTCGI_OBJS) \$(CRX_FPM_OBJS) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(CREX_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
        ;;
      *darwin*)
        BUILD_FPM="\$(CC) \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) \$(NATIVE_RPATHS) \$(CRX_GLOBAL_OBJS:.lo=.o) \$(CRX_BINARY_OBJS:.lo=.o) \$(CRX_FASTCGI_OBJS:.lo=.o) \$(CRX_FPM_OBJS:.lo=.o) \$(CRX_FRAMEWORKS) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(CREX_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
      ;;
      *)
        BUILD_FPM="\$(LIBTOOL) --mode=link \$(CC) -export-dynamic \$(CFLAGS_CLEAN) \$(EXTRA_CFLAGS) \$(EXTRA_LDFLAGS_PROGRAM) \$(LDFLAGS) \$(CRX_RPATHS) \$(CRX_GLOBAL_OBJS:.lo=.o) \$(CRX_BINARY_OBJS:.lo=.o) \$(CRX_FASTCGI_OBJS:.lo=.o) \$(CRX_FPM_OBJS:.lo=.o) \$(EXTRA_LIBS) \$(FPM_EXTRA_LIBS) \$(CREX_EXTRA_LIBS) -o \$(SAPI_FPM_PATH)"
      ;;
  esac

  CRX_SUBST(SAPI_FPM_PATH)
  CRX_SUBST(BUILD_FPM)
