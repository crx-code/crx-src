...
set DEPS_DIR=%CRX_BUILD_CACHE_BASE_DIR%\deps-%BRANCH%-%CRX_SDK_VS%-%CRX_SDK_ARCH%
...
echo ^<?crx $conn_str = "host=127.0.0.1 dbname=test port=5432 user=%PGUSER% password=%PGPASSWORD%"; ?^> >> "./ext/pgsql/tests/config.inc"
...
set PDO_FIREBIRD_TEST_PASS=crxfi
...
rem work-around for failing to dl(mysqli) with OPcache (https://github.com/crx/crx-src/issues/8508)
...
powershell -Command wget http://windows.crx.net/downloads/qa/appveyor/ext/enchant/dict.zip -OutFile dict.zip
unzip dict.zip
del /q dict.zip
popd
...
set CRX_BUILD_DIR=%CRX_BUILD_OBJ_DIR%\Release
if "%THREAD_SAFE%" equ "1" set CRX_BUILD_DIR=%CRX_BUILD_DIR%_TS
...
%CRX_BUILD_DIR%\crx.exe -dextension_dir=%CRX_BUILD_DIR% -dextension=com_dotnet appveyor\setup_hmailserver.crx

mkdir %CRX_BUILD_DIR%\test_file_cache
rem generate crx.ini
echo extension_dir=%CRX_BUILD_DIR% > %CRX_BUILD_DIR%\crx.ini
echo opcache.file_cache=%CRX_BUILD_DIR%\test_file_cache >> %CRX_BUILD_DIR%\crx.ini
if "%OPCACHE%" equ "1" echo crex_extension=crx_opcache.dll >> %CRX_BUILD_DIR%\crx.ini
rem work-around for some spawned CRX processes requiring OpenSSL
echo extension=crx_openssl.dll >> %CRX_BUILD_DIR%\crx.ini

rem remove ext dlls for which tests are not supported
for %%i in (ldap oci8_12c pdo_oci) do (
	del %CRX_BUILD_DIR%\crx_%%i.dll
)

set TEST_CRXDBG_EXECUTABLE=%CRX_BUILD_DIR%\crxdbg.exe

mkdir c:\tests_tmp

set TEST_CRX_JUNIT=c:\junit.out.xml
