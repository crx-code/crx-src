/*
   ...
   +----------------------------------------------------------------------+
   | Authors: Daniel Beulshausen <daniel@crx4win.de>                      |
   +----------------------------------------------------------------------+
*/
...
{/*{{{*/
	...
	int ret = crx_win32_ioutil_access_w(pathw, mode);
	...
	if (CWDG(realpath_cache_size_limit)) {
		...
		if(bucket == NULL && !real_path[0]) {
			...
			if(tsrm_realpath(pathname, real_path) != NULL) {
				pathname = real_path;
				bucket = realpath_cache_lookup(pathname, strlen(pathname), t);
				CRX_WIN32_IOUTIL_REINIT_W(pathname);
			}
		}
	}
...
{/*{{{*/
	...
	cmdw = crx_win32_cp_any_to_w(cmd);
	if (!cmdw) {
		free(cmd);
		return NULL;
	}
	...
	if (cwd) {
		cwdw = crx_win32_ioutil_any_to_w(cwd);
		if (!cwdw) {
			free(cmd);
			free(cmdw);
			return NULL;
	...
	envw = crx_win32_cp_env_any_to_w(env);
	if (envw) {
...
static key_t tsrm_choose_random_shm_key(key_t prev_key) {
	unsigned char buf[4];
	if (crx_win32_get_random_bytes(buf, 4) != SUCCESS) {
		return prev_key + 2;
	}
	uint32_t n =
		((uint32_t)(buf[0]) << 24) |
...
static crex_always_inline void UnixTimeToFileTime(time_t t, LPFILETIME pft) /* {{{ */
{
...
TSRM_API int win32_utime(const char *filename, struct utimbuf *buf) /* {{{ */
{
	...
	HANDLE hFile;
	CRX_WIN32_IOUTIL_INIT_W(filename)
