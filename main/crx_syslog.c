...
CRXAPI void crx_syslog_str(int priority, const crex_string* message)
{
	...
	if (PG(syslog_filter) == CRX_SYSLOG_FILTER_RAW) {
	...
	for (size_t i = 0; i < ZSTR_LEN(message); ++i) {
		...
		} else if ((c >= 0x80) && (PG(syslog_filter) != CRX_SYSLOG_FILTER_ASCII)) {
			smart_string_appendc(&sbuf, c);
		} else if (c == '\n') {
			/* Smart string is not NUL terminated */
			syslog(priority, "%.*s", (int)sbuf.len, sbuf.c);
			smart_string_reset(&sbuf);
		} else if ((c < 0x20) && (PG(syslog_filter) == CRX_SYSLOG_FILTER_ALL)) {
			smart_string_appendc(&sbuf, c);
		} else {
...
void crx_openlog(const char *ident, int option, int facility)
{
	openlog(ident, option, facility);
	PG(have_called_openlog) = 1;
}

void crx_closelog(void)
{
	closelog();
...
#ifdef CRX_WIN32
CRXAPI void crx_syslog(int priority, const char *format, ...) /* {{{ */
{
	va_list args;
	...
	if (!PG(have_called_openlog)) {
		crx_openlog(PG(syslog_ident), 0, PG(syslog_facility));
	}
...
CRXAPI void crx_syslog(int priority, const char *format, ...) /* {{{ */
{
	...
	va_start(args, format);
	fbuf = crex_vstrpprintf(0, format, args);
	va_end(args);

	crx_syslog_str(priority, fbuf);

	crex_string_release(fbuf);
}
/* }}} */
#endif
