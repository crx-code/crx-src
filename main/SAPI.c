...
static void sapi_globals_ctor(sapi_globals_struct *sapi_globals)
{
	...
	crex_hash_init(&sapi_globals->known_post_content_types, 8, NULL, _type_dtor, 1);
	crx_setup_sapi_content_types();
}

static void sapi_globals_dtor(sapi_globals_struct *sapi_globals)
{
	crex_hash_destroy(&sapi_globals->known_post_content_types);
}
...
CRX_FUNCTION(header_register_callback)
{
	crex_fcall_info fci;
	...
	if (C_TYPE(SG(callback_func)) != IS_UNDEF) {
...
static void sapi_run_header_callback(zval *callback)
{
	...
	if (crex_fcall_info_init(callback, 0, &fci, &SG(fci_cache), NULL, &callback_error) == SUCCESS) {
		fci.retval = &retval;

		error = crex_call_function(&fci, &SG(fci_cache));
		if (error == FAILURE) {
...
callback_failed:
		crx_error_docref(NULL, E_WARNING, "Could not call the sapi_header_callback");
	}
...
static void sapi_read_post_data(void)
{
	...
	if ((post_entry = crex_hash_str_find_ptr(&SG(known_post_content_types), content_type,
...
SAPI_API SAPI_POST_READER_FUNC(sapi_read_standard_form_data)
{
	if ((SG(post_max_size) > 0) && (SG(request_info).content_length > SG(post_max_size))) {
		crx_error_docref(NULL, E_WARNING, "POST Content-Length of " CREX_LONG_FMT " bytes exceeds the limit of " CREX_LONG_FMT " bytes",
					SG(request_info).content_length, SG(post_max_size));
		return;
	}


	SG(request_info).request_body = crx_stream_temp_create_ex(TEMP_STREAM_DEFAULT, SAPI_POST_BLOCK_SIZE, PG(upload_tmp_dir));
	...
	if (sapi_module.read_post) {
		...
		for (;;) {
			...
			if (read_bytes > 0) {
				if (crx_stream_write(SG(request_info).request_body, buffer, read_bytes) != read_bytes) {
					/* if parts of the stream can't be written, purge it completely */
					crx_stream_truncate_set_size(SG(request_info).request_body, 0);
					crx_error_docref(NULL, E_WARNING, "POST data can't be buffered; all data discarded");
					break;
				}
			}

			if ((SG(post_max_size) > 0) && (SG(read_post_bytes) > SG(post_max_size))) {
				crx_error_docref(NULL, E_WARNING, "Actual POST length does not match Content-Length, and exceeds " CREX_LONG_FMT " bytes", SG(post_max_size));
				break;
			}
		...
		crx_stream_rewind(SG(request_info).request_body);
	}
}
	...
	if (*mimetype != NULL) {
		if (*charset && strncmp(*mimetype, "text/", 5) == 0 && strstr(*mimetype, "charset=") == NULL) {
			...
			newtype = emalloc(newlen + 1);
	 		CRX_STRLCPY(newtype, *mimetype, newlen + 1, len);
...
SAPI_API void sapi_activate_headers_only(void)
{
	...
	crex_llist_init(&SG(sapi_headers).headers, sizeof(sapi_header_struct),
...
/*
 * Called from crx_request_startup() for every request.
 */

SAPI_API void sapi_activate(void)
{
	crex_llist_init(&SG(sapi_headers).headers, sizeof(sapi_header_struct), (void (*)(void *)) sapi_free_header, 0);
	SG(sapi_headers).send_default_content_type = 1;
...
{
	crex_llist_destroy(&SG(sapi_headers).headers);
...
/*
 * since crex_llist_del_element only removes one matched item once,
 * we should remove them manually
 */
static void sapi_remove_header(crex_llist *l, char *name, size_t len) {
...
static void sapi_header_add_op(sapi_header_op_enum op, sapi_header_struct *sapi_header)
{
	if (!sapi_module.header_handler ||
		...
		crex_llist_add_element(&SG(sapi_headers).headers, (void *) sapi_header);
	} else {
...
SAPI_API int sapi_header_op(sapi_header_op_enum op, void *arg)
{
	...
	switch (op) {
		...
		case SAPI_HEADER_DELETE_ALL:
			...
			crex_llist_clean(&SG(sapi_headers).headers);
			return SUCCESS;
			...
			*colon_offset = 0;
			if (!strcasecmp(header_line, "Content-Type")) {
				...
				if (newlen != 0) {
					...
					CRX_STRLCPY(newheader, "Content-type: ", newlen, sizeof("Content-type: ")-1);
				...
				crex_string *key = ZSTR_INIT_LITERAL("zlib.output_compression", 0);
				crex_alter_ini_entry_chars(key,
					"0", sizeof("0") - 1, CRX_INI_USER, CRX_INI_STAGE_RUNTIME);
...
SAPI_API int sapi_send_headers(void)
{
	...
	switch (retval) {
		...
		case SAPI_HEADER_DO_SEND: {
			...
			crex_llist_apply_with_argument(&SG(sapi_headers).headers, (llist_apply_with_arg_func_t) sapi_module.send_header, SG(server_context));
...
SAPI_API int sapi_register_post_entry(const sapi_post_entry *post_entry)
{
	...
	key = crex_string_init(post_entry->content_type, post_entry->content_type_len, 1);
	GC_MAKE_PERSISTENT_LOCAL(key);
	ret = crex_hash_add_mem(&SG(known_post_content_types), key,
...
SAPI_API void sapi_unregister_post_entry(const sapi_post_entry *post_entry)
{
	...
	crex_hash_str_del(&SG(known_post_content_types), post_entry->content_type,
...
SAPI_API crex_stat_t *sapi_get_stat(void)
{
