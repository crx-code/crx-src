...
BEGIN_EXTERN_C()
CRXAPI int ap_crx_slprintf(char *buf, size_t len, const char *format,...) CREX_ATTRIBUTE_FORMAT(printf, 3, 4);
CRXAPI int ap_crx_vslprintf(char *buf, size_t len, const char *format, va_list ap);
CRXAPI int ap_crx_snprintf(char *, size_t, const char *, ...) CREX_ATTRIBUTE_FORMAT(printf, 3, 4);
CRXAPI int ap_crx_vsnprintf(char *, size_t, const char *, va_list ap);
CRXAPI int ap_crx_vasprintf(char **buf, const char *format, va_list ap);
CRXAPI int ap_crx_asprintf(char **buf, const char *format, ...) CREX_ATTRIBUTE_FORMAT(printf, 2, 3);
CRXAPI char * crx_0cvt(double value, int ndigit, char dec_point, char exponent, char *buf);
...
#define crx_gcvt crex_gcvt

#ifdef slprintf
#undef slprintf
#endif
#define slprintf ap_crx_slprintf

#ifdef vslprintf
#undef vslprintf
#endif
#define vslprintf ap_crx_vslprintf

#ifdef snprintf
#undef snprintf
#endif
#define snprintf ap_crx_snprintf

#ifdef vsnprintf
#undef vsnprintf
#endif
#define vsnprintf ap_crx_vsnprintf

#ifndef HAVE_VASPRINTF
#define vasprintf ap_crx_vasprintf
#endif

#ifndef HAVE_ASPRINTF
#define asprintf ap_crx_asprintf
#endif
...
CRXAPI char * ap_crx_conv_p2(uint64_t num, int nbits,
