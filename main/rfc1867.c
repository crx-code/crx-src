...
static char *crx_ap_getword(const crex_encoding *encoding, char **line, char stop);
static char *crx_ap_getword_conf(const crex_encoding *encoding, char *str);

static crx_rfc1867_encoding_translation_t crx_rfc1867_encoding_translation = dummy_encoding_translation;
static crx_rfc1867_get_detect_order_t crx_rfc1867_get_detect_order = NULL;
static crx_rfc1867_set_input_encoding_t crx_rfc1867_set_input_encoding = NULL;
static crx_rfc1867_getword_t crx_rfc1867_getword = crx_ap_getword;
static crx_rfc1867_getword_conf_t crx_rfc1867_getword_conf = crx_ap_getword_conf;
static crx_rfc1867_basename_t crx_rfc1867_basename = NULL;

CRXAPI crex_result (*crx_rfc1867_callback)(unsigned int event, void *event_data, void **extra) = NULL;

static void safe_crx_register_variable(char *var, char *strval, size_t val_len, zval *track_vars_array, bool override_protection);
...
static void add_protected_variable(char *varname) /* {{{ */
{
	...
	crex_hash_str_add_empty_element(&PG(rfc1867_protected_variables), varname, strlen(varname));
}
/* }}} */
...
{
	normalize_protected_variable(varname);
	return crex_hash_str_exists(&PG(rfc1867_protected_variables), varname, strlen(varname));
}
/* }}} */
...
static void safe_crx_register_variable(char *var, char *strval, size_t val_len, zval *track_vars_array, bool override_protection) /* {{{ */
{
	if (override_protection || !is_protected_variable(var)) {
		crx_register_variable_safe(var, strval, val_len, track_vars_array);
	}
}
/* }}} */

static void safe_crx_register_variable_ex(char *var, zval *val, zval *track_vars_array, bool override_protection) /* {{{ */
{
	if (override_protection || !is_protected_variable(var)) {
		crx_register_variable_ex(var, val, track_vars_array);
	}
}
/* }}} */
...
{
	safe_crx_register_variable(strvar, val, strlen(val), http_post_files, override_protection);
}
/* }}} */

static void register_http_post_files_variable_ex(char *var, zval *val, zval *http_post_files, bool override_protection) /* {{{ */
{
	safe_crx_register_variable_ex(var, val, http_post_files, override_protection);
}
/* }}} */

static void free_filename(zval *el) {
	crex_string *filename = C_STR_P(el);
	crex_string_release_ex(filename, 0);
}
...
CRXAPI void destroy_uploaded_files_hash(void) /* {{{ */
{
	...
	CREX_HASH_MAP_FOREACH_VAL(SG(rfc1867_uploaded_files), el) {
		crex_string *filename = C_STR_P(el);
		VCWD_UNLINK(ZSTR_VAL(filename));
	} CREX_HASH_FOREACH_END();
	crex_hash_destroy(SG(rfc1867_uploaded_files));
...
typedef struct {
	...
	const crex_encoding *input_encoding;
	const crex_encoding **detect_order;
...
static multipart_buffer *multipart_buffer_new(char *boundary, int boundary_len)
{
	...
	if (crx_rfc1867_encoding_translation()) {
		crx_rfc1867_get_detect_order(&self->detect_order, &self->detect_order_size);
	} else {
...
/* Free header entry */
static void crx_free_hdr_entry(mime_header_entry *h)
{
	if (h->key) {
		efree(h->key);
	}
...
/* parse headers */
static int multipart_buffer_headers(multipart_buffer *self, crex_llist *header)
{
	char *line;
	...
	while ((line = get_line(self)) && line[0] != '\0') {
		...
		if (crx_rfc1867_encoding_translation()) {
			self->input_encoding = crex_multibyte_encoding_detector((const unsigned char *) line, strlen(line), self->detect_order, self->detect_order_size);
		}
		...
		if (value) {
			if (buf_value.c && key) {
				...
				crex_llist_add_element(header, &entry);
				buf_value.c = NULL;
				key = NULL;
			}
...
static char *crx_mime_get_hdr_value(crex_llist header, char *key)
{
	...
	entry = crex_llist_get_first(&header);
	while (entry) {
		if (!strcasecmp(entry->key, key)) {
			return entry->value;
		}
		entry = crex_llist_get_next(&header);
	}
...
static char *crx_ap_basename(const crex_encoding *encoding, char *path)
{
	char *s = strrchr(path, '\\');
 ...
 */
static void *crx_ap_memstr(char *haystack, int haystacklen, char *needle, int needlen, int partial)
{
	int len = haystacklen;
	char *ptr = haystack;
...
static size_t multipart_buffer_read(multipart_buffer *self, char *buf, size_t bytes, int *end)
{
	...
	if ((bound = crx_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 1))) {
		max = bound - self->buf_begin;
		if (end && crx_ap_memstr(self->buf_begin, self->bytes_in_buffer, self->boundary_next, self->boundary_next_len, 0)) {
			*end = 1;
		}
	} else {
...
SAPI_API SAPI_POST_HANDLER_FUNC(rfc1867_post_handler) /* {{{ */
{
	...
	crex_string *temp_filename = NULL;
	...
	const crex_encoding *internal_encoding = crex_multibyte_get_internal_encoding();
	crx_rfc1867_getword_t getword;
	crx_rfc1867_getword_conf_t getword_conf;
	crx_rfc1867_basename_t _basename;
	crex_long count = 0;

	if (crx_rfc1867_encoding_translation() && internal_encoding) {
		getword = crx_rfc1867_getword;
		getword_conf = crx_rfc1867_getword_conf;
		_basename = crx_rfc1867_basename;
	} else {
		getword = crx_ap_getword;
		getword_conf = crx_ap_getword_conf;
		_basename = crx_ap_basename;
	}

	if (SG(post_max_size) > 0 && SG(request_info).content_length > SG(post_max_size)) {
		sapi_module.sapi_error(E_WARNING, "POST Content-Length of " CREX_LONG_FMT " bytes exceeds the limit of " CREX_LONG_FMT " bytes", SG(request_info).content_length, SG(post_max_size));
		return;
	}
	...
	if (!boundary) {
		...
		crex_str_tolower(content_type_lcase, content_type_len);
	...
	/* Initialize $_FILES[] */
	crex_hash_init(&PG(rfc1867_protected_variables), 8, NULL, NULL, 0);

	ALLOC_HASHTABLE(uploaded_files);
	crex_hash_init(uploaded_files, 8, NULL, free_filename, 0);
	SG(rfc1867_uploaded_files) = uploaded_files;

	if (C_TYPE(PG(http_globals)[TRACK_VARS_FILES]) != IS_ARRAY) {
		/* crx_auto_globals_create_files() might have already done that */
		array_init(&PG(http_globals)[TRACK_VARS_FILES]);
	}

	crex_llist_init(&header, sizeof(mime_header_entry), (llist_dtor_func_t) crx_free_hdr_entry, 0);

	if (crx_rfc1867_callback != NULL) {
		multipart_event_start event_start;

		event_start.content_length = SG(request_info).content_length;
		if (crx_rfc1867_callback(MULTIPART_EVENT_START, &event_start, &event_extra_data) == FAILURE) {
	...
	while (!multipart_buffer_eof(mbuff))
	{
		...
		crex_llist_clean(&header);

		if (!multipart_buffer_headers(mbuff, &header)) {
			goto fileupload_done;
		}

		if ((cd = crx_mime_get_hdr_value(header, "Content-Disposition"))) {
			char *pair = NULL;
			int end = 0;

			if (--body_parts_cnt < 0) {
				crx_error_docref(NULL, E_WARNING, "Multipart body parts limit exceeded %d. To increase the limit change max_multipart_body_parts in crx.ini.", body_parts_limit);
				goto fileupload_done;
			}
				...
				if (strchr(pair, '=')) {
					...
					if (!strcasecmp(key, "name")) {
						...
						if (mbuff->input_encoding && internal_encoding) {
							...
							if ((size_t)-1 != crex_multibyte_encoding_converter(&new_param, &new_param_len, (unsigned char *)param, strlen(param), internal_encoding, mbuff->input_encoding)) {
								efree(param);
						...
						if (mbuff->input_encoding && internal_encoding) {
							...
							if ((size_t)-1 != crex_multibyte_encoding_converter(&new_filename, &new_filename_len, (unsigned char *)filename, strlen(filename), internal_encoding, mbuff->input_encoding)) {
								efree(filename);
			...
			if (!filename && param) {
				...
				if (mbuff->input_encoding && internal_encoding) {
					...
					if ((size_t)-1 != crex_multibyte_encoding_converter(&new_value, &new_value_len, (unsigned char *)value, value_len, internal_encoding, mbuff->input_encoding)) {
						efree(value);
				...
				if (++count <= PG(max_input_vars) && sapi_module.input_filter(PARSE_POST, param, &value, value_len, &new_val_len)) {
					if (crx_rfc1867_callback != NULL) {
						...
						if (crx_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data) == FAILURE) {
							efree(param);
							efree(value);
							continue;
						}
					...
					safe_crx_register_variable(param, value, new_val_len, array_ptr, 0);
				} else {
					...
					if (crx_rfc1867_callback != NULL) {
						...
						crx_rfc1867_callback(MULTIPART_EVENT_FORMDATA, &event_formdata, &event_extra_data);
					}
				}
			...
			if (!skip_upload && crx_rfc1867_callback != NULL) {
				multipart_event_file_start event_file_start;

				event_file_start.post_bytes_processed = SG(read_post_bytes);
				event_file_start.name = param;
				event_file_start.filename = &filename;
				if (crx_rfc1867_callback(MULTIPART_EVENT_FILE_START, &event_file_start, &event_extra_data) == FAILURE) {
					temp_filename = NULL;
					efree(param);
					efree(filename);
					continue;
...
#endif
				cancel_upload = CRX_UPLOAD_ERROR_D;
			}
			...
			if (!cancel_upload) {
				...
				{
#endif
					fd = crx_open_temporary_fd_ex(PG(upload_tmp_dir), "crx", &temp_filename, CRX_TMP_FILE_OPEN_BASEDIR_CHECK_ON_FALLBACK);
					upload_cnt--;
					if (fd == -1) {
						sapi_module.sapi_error(E_WARNING, "File upload error - unable to create a temporary file");
						cancel_upload = CRX_UPLOAD_ERROR_E;
					}
				}
			}
			...
			while (!cancel_upload && (blen > 0))
			{
				if (crx_rfc1867_callback != NULL) {
					...
					if (crx_rfc1867_callback(MULTIPART_EVENT_FILE_DATA, &event_file_data, &event_extra_data) == FAILURE) {
						cancel_upload = CRX_UPLOAD_ERROR_X;
						continue;
					}
				}

				if (PG(upload_max_filesize) > 0 && (crex_long)(total_bytes+blen) > PG(upload_max_filesize)) {
#if DEBUG_FILE_UPLOAD
					sapi_module.sapi_error(E_NOTICE, "upload_max_filesize of " CREX_LONG_FMT " bytes exceeded - file [%s=%s] not saved", PG(upload_max_filesize), param, filename);
#endif
					cancel_upload = CRX_UPLOAD_ERROR_A;
				} else if (max_file_size && ((crex_long)(total_bytes+blen) > max_file_size)) {
#if DEBUG_FILE_UPLOAD
					sapi_module.sapi_error(E_NOTICE, "MAX_FILE_SIZE of %" PRId64 " bytes exceeded - file [%s=%s] not saved", max_file_size, param, filename);
#endif
					cancel_upload = CRX_UPLOAD_ERROR_B;
				} else if (blen > 0) {
#ifdef CRX_WIN32
...
#endif
						cancel_upload = CRX_UPLOAD_ERROR_F;
					} else if (wlen < blen) {
#if DEBUG_FILE_UPLOAD
...
#endif
				cancel_upload = CRX_UPLOAD_ERROR_C;
			}
#if DEBUG_FILE_UPLOAD
			...
			if (crx_rfc1867_callback != NULL) {
				...
				if (crx_rfc1867_callback(MULTIPART_EVENT_FILE_END, &event_file_end, &event_extra_data) == FAILURE) {
					cancel_upload = CRX_UPLOAD_ERROR_X;
				}
			}
			...
			if (cancel_upload) {
				if (temp_filename) {
					if (cancel_upload != CRX_UPLOAD_ERROR_E) { /* file creation failed */
						unlink(ZSTR_VAL(temp_filename));
					}
					crex_string_release_ex(temp_filename, 0);
				}
				temp_filename = NULL;
			} else {
				crex_hash_add_ptr(SG(rfc1867_uploaded_files), temp_filename, temp_filename);
			}
			...
			if (cancel_upload || !(cd = crx_mime_get_hdr_value(header, "Content-Type"))) {
				cd = "";
			} else {
				/* fix for Opera 6.01 */
				s = strchr(cd, ';');
...
fileupload_done:
	if (crx_rfc1867_callback != NULL) {
		...
		crx_rfc1867_callback(MULTIPART_EVENT_END, &event_end, &event_extra_data);
	}

	if (lbuf) efree(lbuf);
	if (abuf) efree(abuf);
	if (array_index) efree(array_index);
	crex_hash_destroy(&PG(rfc1867_protected_variables));
	crex_llist_destroy(&header);
...
{
	crx_rfc1867_encoding_translation = encoding_translation;
	crx_rfc1867_get_detect_order = get_detect_order;
	crx_rfc1867_set_input_encoding = set_input_encoding;
	crx_rfc1867_getword = getword;
	crx_rfc1867_getword_conf = getword_conf;
	crx_rfc1867_basename = basename;
}
/* }}} */
