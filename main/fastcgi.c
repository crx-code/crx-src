...
#endif
		crex_hash_init(&fcgi_mgmt_vars, 8, NULL, fcgi_free_mgmt_var_cb, 1);
...
# endif
		if ((GetStdHandle(STD_OUTPUT_HANDLE) == INVALID_HANDLE_VALUE) &&
			...
			if (str != NULL) {
				fcgi_accept_mutex = (HANDLE) CREX_ATOL(str);
			}
			return is_fastcgi = 1;
		} else {
			return is_fastcgi = 0;
...
{
	if (is_initialized) {
		crex_hash_destroy(&fcgi_mgmt_vars);
	}
	is_fastcgi = 0;
	if (allowed_clients) {
		...
		if (!*host || !strncmp(host, "*", sizeof("*")-1)) {
			...
			if (sa.sa_inet.sin_addr.s_addr == INADDR_NONE) {
				...
				if(strlen(host) > MAXFQDNLEN) {
					...
					hep = crx_network_gethostbyname(host);
				}
...
# endif
		...
		q = req->env.list;
		while (q != NULL) {
			if ((value = crex_hash_str_find(&fcgi_mgmt_vars, q->var, q->var_len)) == NULL) {
				q = q->list_next;
				continue;
			}
			zlen = (unsigned int)C_STRLEN_P(value);
			...
			p += q->var_len;
			memcpy(p, C_STRVAL_P(value), zlen);
			p += zlen;
			q = q->list_next;
		}
...
{
	zval zvalue;
	crex_string *key = crex_string_init(name, name_len, 1);
	ZVAL_NEW_STR(&zvalue, crex_string_init(value, value_len, 1));
	GC_MAKE_PERSISTENT_LOCAL(key);
	GC_MAKE_PERSISTENT_LOCAL(C_STR(zvalue));
	crex_hash_add(&fcgi_mgmt_vars, key, &zvalue);
