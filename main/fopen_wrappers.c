...
CRXAPI CREX_INI_MH(OnUpdateBaseDir)
{
	...
	if (stage == CRX_INI_STAGE_STARTUP || stage == CRX_INI_STAGE_SHUTDOWN || stage == CRX_INI_STAGE_ACTIVATE || stage == CRX_INI_STAGE_DEACTIVATE) {
		if (PG(open_basedir_modified)) {
			efree(*p);
		}
		/* We're in a CRX_INI_SYSTEM context, no restrictions */
	...
	while (ptr && *ptr) {
		...
		if (crx_check_open_basedir_ex(resolved_name, 0) != 0) {
	...
	crex_string *tmp = smart_str_extract(&buf);
...
/* {{{ crx_check_specific_open_basedir
...
CRXAPI int crx_check_specific_open_basedir(const char *basedir, const char *path)
{
	...
	while (VCWD_REALPATH(path_tmp, resolved_name) == NULL) {
		...
		if (nesting_level == 0) {
			...
			ret = crx_sys_readlink(path_tmp, buf, MAXPATHLEN - 1);
			if (ret == -1) {
	...
	if (expand_filepath(local_open_basedir, resolved_basedir) != NULL) {
...
#ifdef CRX_WIN32
		if (basedir[basedir_len - 1] == CRX_DIR_SEPARATOR || basedir[basedir_len - 1] == '/') {
#else
		if (basedir[basedir_len - 1] == CRX_DIR_SEPARATOR) {
#endif
			if (resolved_basedir[resolved_basedir_len - 1] != CRX_DIR_SEPARATOR) {
				resolved_basedir[resolved_basedir_len] = CRX_DIR_SEPARATOR;
		...
		if (path_tmp[path_len - 1] == CRX_DIR_SEPARATOR) {
			if (resolved_name[resolved_name_len - 1] != CRX_DIR_SEPARATOR) {
				resolved_name[resolved_name_len] = CRX_DIR_SEPARATOR;
		...
		if (strncmp(resolved_basedir, resolved_name, resolved_basedir_len) == 0) {
			...
			if (resolved_name_len > resolved_basedir_len &&
				resolved_name[resolved_basedir_len - 1] != CRX_DIR_SEPARATOR) {
				return -1;
			} else {
			...
			if (resolved_basedir_len == (resolved_name_len + 1) && resolved_basedir[resolved_basedir_len - 1] == CRX_DIR_SEPARATOR) {
#ifdef CRX_WIN32
...
{
	return crx_check_open_basedir_ex(path, 1);
}

/* {{{ crx_check_open_basedir */
	...
	if (PG(open_basedir) && *PG(open_basedir)) {
		...
		* message. */
		if (strlen(path) > (MAXPATHLEN - 1)) {
			crx_error_docref(NULL, E_WARNING, "File name is longer than the maximum allowed path length on this platform (%d): %s", MAXPATHLEN, path);
			errno = EINVAL;
			return -1;
		}
		...
		while (ptr && *ptr) {
			...
			if (crx_check_specific_open_basedir(ptr, path) == 0) {
				efree(pathbuf);
				return 0;
			}
		...
		if (warn) {
			crx_error_docref(NULL, E_WARNING, "open_basedir restriction in effect. File(%s) is not within the allowed path(s): (%s)", path, PG(open_basedir));
		}
		efree(pathbuf);
...
/* {{{ crx_fopen_and_set_opened_path */
static FILE *crx_fopen_and_set_opened_path(const char *path, const char *mode, crex_string **opened_path)
{
	FILE *fp;

	if (crx_check_open_basedir((char *)path)) {
		return NULL;
	}
	...
	if (fp && opened_path) {
		...
		if (tmp) {
			*opened_path = crex_string_init(tmp, strlen(tmp), 0);
			efree(tmp);
		}
	}
	return fp;
...
CRXAPI int crx_fopen_primary_script(crex_file_handle *file_handle)
{
	...
	memset(file_handle, 0, sizeof(crex_file_handle));
...
#endif
			if (pw && pw->pw_dir) {
				filename = crex_strpprintf(0, "%s%c%s%c%s", pw->pw_dir, CRX_DIR_SEPARATOR, PG(user_dir), CRX_DIR_SEPARATOR, s + 1); /* Safe */
			} else if (SG(request_info).path_translated) {
				filename = crex_string_init(SG(request_info).path_translated,
	...
	if (!resolved_path) {
		 ...
		 * crx_destroy_request_info assumes that it will get
...
static crex_string *tsrm_realpath_str(const char *path) {
	char *realpath = tsrm_realpath(path, NULL);
	if (!realpath) {
		return NULL;
	}
	crex_string *realpath_str = crex_string_init(realpath, strlen(realpath), 0);
	efree(realpath);
	return realpath_str;
}
 ...
 */
CRXAPI crex_string *crx_resolve_path(const char *filename, size_t filename_length, const char *path)
{
	...
	if ((*p == ':') && (p - filename > 1) && (p[1] == '/') && (p[2] == '/')) {
		wrapper = crx_stream_locate_url_wrapper(filename, &actual_path, STREAM_OPEN_FOR_INCLUDE);
	...
	while (ptr && *ptr) {
		...
		if (is_stream_wrapper) {
			wrapper = crx_stream_locate_url_wrapper(trypath, &actual_path, STREAM_OPEN_FOR_INCLUDE);
			if (!wrapper) {
				continue;
			} else if (wrapper != &crx_plain_files_wrapper) {
				if (wrapper->wops->url_stat) {
					crx_stream_statbuf ssb;

					if (SUCCESS == wrapper->wops->url_stat(wrapper, trypath, CRX_STREAM_URL_STAT_QUIET, &ssb, NULL)) {
						return crex_string_init(trypath, strlen(trypath), 0);
					}
					if (EG(exception)) {
						return NULL;
					}
...
CRXAPI FILE *crx_fopen_with_path(const char *filename, const char *mode, const char *path, crex_string **opened_path)
{
	...
	/* Absolute path open */
	 ...
	 || (!path || !*path)
		...
		return crx_fopen_and_set_opened_path(filename, mode, opened_path);
	}
	...
	while (ptr && *ptr) {
		...
		fp = crx_fopen_and_set_opened_path(trypath, mode, opened_path);
		if (fp) {
			efree(pathbuf);
			return fp;
		}
...
/* {{{ crx_strip_url_passwd */
CRXAPI char *crx_strip_url_passwd(char *url)
{
	char *p, *url_start;
...
CRXAPI char *expand_filepath_with_mode(const char *filepath, char *real_path, const char *relative_to, size_t relative_to_len, int realpath_mode)
{
	cwd_state new_state;
	char cwd[MAXPATHLEN];
	size_t copy_len;
