...
{
	crx_ini_builder_realloc(b, length);
	if (b->length > 0)
...
{
	crx_ini_builder_realloc(b, name_length + 1 + value_length + 1);
...
{
	crx_ini_builder_realloc(b, name_length + 2 + value_length + 2);
...
CRXAPI void crx_ini_builder_define(struct crx_ini_builder *b, const char *arg)
{
	...
	if (val != NULL) {
		...
		if (!isalnum(*val) && *val != '"' && *val != '\'' && *val != '\0') {
			crx_ini_builder_quoted(b, arg, val - arg - 1, val, arg + len - val);
		} else {
			crx_ini_builder_realloc(b, len + strlen("\n"));
			memcpy(b->value + b->length, arg, len);
			b->length += len;
			b->value[b->length++] = '\n';
		}
	} else {
		crx_ini_builder_unquoted(b, arg, len, "1", 1);
	}
}
