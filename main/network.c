...
# define CRX_TIMEOUT_ERROR_VALUE		WSAETIMEDOUT
...
# define CRX_TIMEOUT_ERROR_VALUE		ETIMEDOUT
#endif

#if HAVE_GETADDRINFO
#ifdef HAVE_GAI_STRERROR
#  define CRX_GAI_STRERROR(x) (gai_strerror(x))
#else
#  define CRX_GAI_STRERROR(x) (crx_gai_strerror(x))
/* {{{ crx_gai_strerror */
static const char *crx_gai_strerror(int code)
{
	static struct {
		int code;
		const char *msg;
...
# endif
	...
	if ((n = getaddrinfo(host, NULL, &hints, &res))) {
		if (error_string) {
			...
			if (*error_string) {
				crex_string_release_ex(*error_string, 0);
			}
			*error_string = strpprintf(0, "crx_network_getaddresses: getaddrinfo for %s failed: %s", host, CRX_GAI_STRERROR(n));
			crx_error_docref(NULL, E_WARNING, "%s", ZSTR_VAL(*error_string));
		} else {
			crx_error_docref(NULL, E_WARNING, "crx_network_getaddresses: getaddrinfo for %s failed: %s", host, CRX_GAI_STRERROR(n));
		}
		return 0;
	} else if (res == NULL) {
		if (error_string) {
			...
			*error_string = strpprintf(0, "crx_network_getaddresses: getaddrinfo for %s failed (null result pointer) errno=%d", host, errno);
			crx_error_docref(NULL, E_WARNING, "%s", ZSTR_VAL(*error_string));
		} else {
			crx_error_docref(NULL, E_WARNING, "crx_network_getaddresses: getaddrinfo for %s failed (null result pointer)", host);
		}
		return 0;
	}
	...
	*sal = safe_emalloc((n + 1), sizeof(*sal), 0);
	...
	if (!inet_aton(host, &in)) {
		...
		if(strlen(host) > MAXFQDNLEN) {
			...
			host_info = crx_network_gethostbyname(host);
		}
		if (host_info == NULL) {
			if (error_string) {
				/* free error string received during previous iteration (if any) */
				...
				*error_string = strpprintf(0, "crx_network_getaddresses: gethostbyname failed. errno=%d", errno);
				crx_error_docref(NULL, E_WARNING, "%s", ZSTR_VAL(*error_string));
			} else {
				crx_error_docref(NULL, E_WARNING, "crx_network_getaddresses: gethostbyname failed");
			}
			return 0;
		}
...
#ifdef CRX_WIN32
typedef u_long crx_non_blocking_flags_t;
...
#else
typedef int crx_non_blocking_flags_t;
#  define SET_SOCKET_BLOCKING_MODE(sock, save) \
...
CRXAPI int crx_network_connect_socket(crx_socket_t sockfd,
		...
		int *error_code)
{
	crx_non_blocking_flags_t orig_flags;
	int n;
	int error = 0;
	socklen_t len;
	int ret = 0;
	...
	if ((n = connect(sockfd, addr, addrlen)) != 0) {
		...
		if (error != EINPROGRESS) {
			if (error_string) {
				*error_string = crx_socket_error_str(error);
			}
...
# ifdef CRX_WIN32
	 ...
	 * the exceptfds set. Indeed, using CRX_POLLREADABLE results in select
	 * failing only due to the timeout and not immediately as would be
	 * expected when a connection is actively refused. This way,
	 * crx_pollfd_for will return a mask with POLLOUT if the connection
	 * is successful and with POLLPRI otherwise. */
	if ((n = crx_pollfd_for(sockfd, POLLOUT|POLLPRI, timeout)) == 0) {
#else
	if ((n = crx_pollfd_for(sockfd, CRX_POLLREADABLE|POLLOUT, timeout)) == 0) {
#endif
		error = CRX_TIMEOUT_ERROR_VALUE;
	}
 ...
 * */
/* {{{ crx_network_bind_socket_to_local_addr */
crx_socket_t crx_network_bind_socket_to_local_addr(const char *host, unsigned port,
		...
		)
{
	...
	num_addrs = crx_network_getaddresses(host, socktype, &psal, error_string);
	...
	if (error_string) {
		*error_string = crx_socket_error_str(err);
	}
...
CRXAPI int crx_network_parse_network_address_with_port(const char *addr, crex_long addrlen, struct sockaddr *sa, socklen_t *sl)
{
	char *colon;
	char *tmp;
	int ret = FAILURE;
	...
	n = crx_network_getaddresses(tmp, SOCK_DGRAM, &psal, &errstr);

	if (n == 0) {
		if (errstr) {
			crx_error_docref(NULL, E_WARNING, "Failed to resolve `%s': %s", tmp, ZSTR_VAL(errstr));
			...
			case AF_UNIX:
				{
					...
					if (ua->sun_path[0] == '\0') {
						...
						*textaddr = crex_string_init((char*)ua->sun_path, len, 0);
					} else {
...
{
	crx_socket_t clisock = -1;
	int error = 0, n;
	crx_sockaddr_storage sa;
	socklen_t sl;

	n = crx_pollfd_for(srvsock, CRX_POLLREADABLE, timeout);
...
/* {{{ crx_network_connect_socket_to_host */
crx_socket_t crx_network_connect_socket_to_host(const char *host, unsigned short port,
	...
	for (sal = psal; !fatal && *sal != NULL; sal++) {
		...
		if (bindto) {
			...
			if (local_address_len == 0) {
				crx_error_docref(NULL, E_WARNING, "Invalid IP Address: %s", bindto);
			} else if (bind(sock, &local_address.common, local_address_len)) {
				crx_error_docref(NULL, E_WARNING, "Failed to bind to '%s:%d', system said: %s", bindto, bindport, strerror(errno));
			}
		}
		/* free error string received during previous iteration (if any) */
...
#endif
		n = crx_network_connect_socket(sock, sa, socklen, asynchronous,
...
#else
		if (error_code && *error_code == CRX_TIMEOUT_ERROR_VALUE) {
...
/* {{{ crx_any_addr
 * Fills any (wildcard) address into crx_sockaddr_storage
 */
CRXAPI void crx_any_addr(int family, crx_sockaddr_storage *addr, unsigned short port)
{
	memset(addr, 0, sizeof(crx_sockaddr_storage));
	switch (family) {
#if HAVE_IPV6
	case AF_INET6: {
...
CRXAPI char *crx_socket_strerror(long err, char *buf, size_t bufsize)
{
	...
	return buf;
#else
	char *sysbuf = crx_win32_error_to_msg(err);
	if (!sysbuf[0]) {
	...
	crx_win32_error_msg_free(sysbuf);
...
CRXAPI crex_string *crx_socket_error_str(long err)
{
	...
	errstr = strerror(err);
	return crex_string_init(errstr, strlen(errstr), 0);
#else
	crex_string *ret;
	...
	ret = crex_string_init(sysbuf, strlen(sysbuf), 0);
...
CRXAPI crx_stream *_crx_stream_sock_open_from_socket(crx_socket_t socket, const char *persistent_id STREAMS_DC)
{
	...
	stream = crx_stream_alloc_rel(&crx_stream_generic_socket_ops, sock, persistent_id, "r+");

	if (stream == NULL) {
		pefree(sock, persistent_id ? 1 : 0);
	} else {
		stream->flags |= CRX_STREAM_FLAG_AVOID_BLOCKING;
	}
		...
		int socktype, struct timeval *timeout, const char *persistent_id STREAMS_DC)
{
	...
	stream = crx_stream_xport_create(res, reslen, REPORT_ERRORS,
...
CRXAPI void _crx_emit_fd_setsize_warning(int max_fd)
{
	...
	crx_error_docref(NULL, E_WARNING,
		"CRX needs to be recompiled with a larger value of FD_SETSIZE.\n"
		"If this binary is from an official www.crx.net package, file a bug report\n"
		"at https://github.com/crx/crx-src/issues, including the following information:\n"
...
#else
	crx_error_docref(NULL, E_WARNING,
		"You MUST recompile CRX with a larger value of FD_SETSIZE.\n"
...
#if defined(CRX_USE_POLL_2_EMULATION)
...
CRXAPI int crx_poll2(crx_pollfd *ufds, unsigned int nfds, int timeout)
{
	...
	crx_socket_t max_fd = SOCK_ERR; /* effectively unused on Windows */
	unsigned int i;
	int n;
	struct timeval tv;
	...
	if (!CRX_SAFE_MAX_FD(max_fd, nfds + 1)) {
#ifdef CRX_WIN32
	...
	for (i = 0; i < nfds; i++) {
		if (ufds[i].events & CRX_POLLREADABLE) {
			CRX_SAFE_FD_SET(ufds[i].fd, &rset);
		}
		if (ufds[i].events & POLLOUT) {
			CRX_SAFE_FD_SET(ufds[i].fd, &wset);
		}
		if (ufds[i].events & POLLPRI) {
			CRX_SAFE_FD_SET(ufds[i].fd, &eset);
		}
	}
...
/* Resetting/initializing */
	...
	if (n >= 0) {
		for (i = 0; i < nfds; i++) {
			...
			if (CRX_SAFE_FD_ISSET(ufds[i].fd, &rset)) {
				/* could be POLLERR or POLLHUP but can't tell without probing */
				ufds[i].revents |= POLLIN;
			}
			if (CRX_SAFE_FD_ISSET(ufds[i].fd, &wset)) {
				ufds[i].revents |= POLLOUT;
			}
			if (CRX_SAFE_FD_ISSET(ufds[i].fd, &eset)) {
