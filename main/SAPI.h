...
typedef struct {
	...
	char *query_string;
	char *cookie_data;
	crex_long content_length;

	char *path_translated;
	char *request_uri;

	/* Do not use request_body directly, but the crx://input stream wrapper instead */
	struct _crx_stream *request_body;
...
typedef struct _sapi_globals_struct {
	...
	crex_stat_t global_stat;
	char *default_mimetype;
	char *default_charset;
	HashTable *rfc1867_uploaded_files;
	crex_long post_max_size;
	int options;
	bool sapi_started;
...
BEGIN_EXTERN_C()
#ifdef ZTS
# define SG(v) CREX_TSRMG_FAST(sapi_globals_offset, sapi_globals_struct *, v)
...
typedef struct {
	...
	size_t line_len;
	crex_long response_code; /* long due to crex_parse_parameters compatibility */
} sapi_header_line;
...
struct _sapi_module_struct {
	...
	crex_stat_t *(*get_stat)(void);
	char *(*getenv)(const char *name, size_t name_len);

	void (*sapi_error)(int type, const char *error_msg, ...) CREX_ATTRIBUTE_FORMAT(printf, 2, 3);
	...
	crex_result (*get_request_time)(double *request_time);
	void (*terminate_process)(void);

	char *crx_ini_path_override;
	...
	int crx_ini_ignore;
	int crx_ini_ignore_cwd; /* don't look for crx.ini in the current directory */
	...
	const crex_function_entry *additional_functions;
...
#define SAPI_DEFAULT_CHARSET		CRX_DEFAULT_CHARSET
#define SAPI_CRX_VERSION_HEADER		"X-Powered-By: CRX/" CRX_VERSION
...
#define STANDARD_SAPI_MODULE_PROPERTIES \
	NULL, /* crx_ini_path_override   */ \
	NULL, /* default_post_reader     */ \
	NULL, /* treat_data              */ \
	NULL, /* executable_location     */ \
	0,    /* crx_ini_ignore          */ \
	0,    /* crx_ini_ignore_cwd      */ \
	NULL, /* get_fd                  */ \
	NULL, /* force_http_10           */ \
	NULL, /* get_target_uid          */ \
	NULL, /* get_target_gid          */ \
