...
var CRX_SRC_DIR=FSO.GetParentFolderName(WScript.ScriptFullName);
...
var CRX_OBJECT_OUT_DIR = "";
var CRX_CONFIG_PROFILE = "no";
var CRX_SANITIZER = "no";
var VERBOSITY = 0;
var CMD_MOD1 = "@";
var CMD_MOD2 = "@";

var CRX_TEST_INI_PATH = "";
var CRX_TEST_INI = "";
var CRX_TEST_INI_EXT_EXCLUDE = "";

var CRX_MAKEFILE_FRAGMENTS = CRX_SRC_DIR + "\\Makefile.fragments.w32";
...
if (MODE_CRXIZE) {
	...
	if (!FSO.FileExists("main\\crx_version.h")) {
		STDERR.WriteLine("Must be run from the root of the crx source");
		WScript.Quit(10);
	}
}
...
if (typeof(CWD) == "undefined") {
	CWD = FSO.GetParentFolderName(FSO.GetParentFolderName(FSO.GetAbsolutePathName("main\\crx_version.h")));
}

if (!MODE_CRXIZE) {
	/* defaults; we pick up the precise versions from configure.ac */
	var CRX_VERSION = 8;
	var CRX_MINOR_VERSION = 3;
	var CRX_RELEASE_VERSION = 0;
	var CRX_EXTRA_VERSION = "";
	var CRX_VERSION_STRING = "8.3.0";
}
...
function get_version_numbers()
{
	...
	if (cin.match(new RegExp(regex))) {
		CRX_VERSION = RegExp.$1;
		CRX_MINOR_VERSION = RegExp.$2;
		CRX_RELEASE_VERSION = RegExp.$3;
		CRX_EXTRA_VERSION = RegExp.$4;
	}

	CRX_VERSION_STRING = CRX_VERSION + "." + CRX_MINOR_VERSION + "." + CRX_RELEASE_VERSION + CRX_EXTRA_VERSION;

	DEFINE('CRX_VERSION_STRING', CRX_VERSION_STRING);
}
...
(function () {
	...
	AC_DEFINE("CRX_BUILD_SYSTEM", os.Caption + " [" + os.Version + "]", "Windows build system version");
	var build_provider = WshShell.Environment("Process").Item("CRX_BUILD_PROVIDER");
	if (build_provider) {
		AC_DEFINE("CRX_BUILD_PROVIDER", build_provider);
	}
}());
...
function conf_process_args()
{
	...
	for (i = 0; i < args.length; i++) {
		...
		for (j = 0; j < configure_args.length; j++) {
			if (argname == configure_args[j].imparg || argname == configure_args[j].arg) {
				...
				/* Force shared when called after crxize */
				if (MODE_CRXIZE) {
					shared = "shared";
				} else {
					shared = analyzed[0];
				...
				arg.argval = argval;
				eval("CRX_" + arg.symval + " = argval;");
				eval("CRX_" + arg.symval + "_SHARED = shared;");
				break;
			}
		}
		if (!found) {
	...
	if (CRX_SNAPSHOT_BUILD != 'no' && INVALID_CONFIG_ARGS.length) {
...
"
	...
	var snapshot_build_exclusions = new Array(
		'debug', 'lzf-better-compression', 'crx-build', 'snapshot-template', 'zts',
	...
	for (i = 0; i < configure_args.length; i++) {
		...
		// in a snapshot build
		if (CRX_SNAPSHOT_BUILD != "no" && argval == "yes" && !shared) {
		...
		if (CRX_SNAPSHOT_BUILD != "no" && argval == "no") {
			force = true;
			for (j = 0; j < snapshot_build_exclusions.length; j++) {
				if (snapshot_build_exclusions[j] == arg.optname) {
					force = false;
	...
	if (CRX_CONFIG_PROFILE != 'no') {
		if (CRX_CONFIG_PROFILE.toLowerCase() == 'nice') {
			WARNING('Config profile name cannot be named \'nice\'');
		} else {
			var config_profile = FSO.CreateTextFile('config.' + CRX_CONFIG_PROFILE + '.bat', true);
...
function verbalize_deps_path(path)
{
	...
	if (absolute_path.indexOf(CRX_CRX_BUILD) == 0) {
		absolute_path = absolute_path.substring(CRX_CRX_BUILD.length).split('\\');
...
function PATH_PROG(progname, additional_paths, symbol)
{
	var exe;
	var place;
	var cyg_path = CRX_CYGWIN + "\\bin;" + CRX_CYGWIN + "\\usr\\local\\bin";
	var crx_build_bin_path = CRX_CRX_BUILD + "\\bin"
	...
	additional_paths = additional_paths + ";" + crx_build_bin_path;
...
function CHECK_LIB(libnames, target, path_to_check, common_name)
{
	...
	// path to be searched
	if (common_name != null) {
		path_to_check += ";" + CRX_CRX_BUILD + "\\" + common_name + "*";
		path_to_check += ";" + CRX_CRX_BUILD + "\\lib\\" + common_name + "*";
	...
	path_to_check += ";" + crx_usual_lib_suspects;

	// It is common practice to put libs under one of these dir names
	var subdirs = new Array(CRX_DEBUG == "yes" ? "Debug" : (CRX_DEBUG_PACK == "yes"?"Release_Dbg":"Release"), "lib", "libs", "libexec");
	...
	for (i = 0; i < libnames.length; i++) {
		...
		location = find_pattern_in_path(libname, CRX_EXTRA_LIBS);
		if (location) {
			location = condense_path(location);
			libname = FSO.GetFileName(location);
			ADD_FLAG("LIBS" + target, libname);
...
function OLD_CHECK_LIB(libnames, target, path_to_check)
{
	...
	var subdir = CRX_DEBUG == "yes" ? "Debug" : (CRX_DEBUG_PACK == "yes"?"Release_Dbg":"Release");
	...
	for (i = 0; i < libnames.length; i++) {
		...
		if (typeof(p) == "string") {
			 ...
			 * to LIBS_XXX */
			if (false != search_paths(libname, CRX_EXTRA_LIBS, null)) {
				ADD_FLAG("LIBS" + target, libname);
				have = 1;
			}
		}
...
function GREP_HEADER(header_name, regex, path_to_check)
{
	...
	if (!c) {
		...
		if (typeof(p) == "string") {
			...
			p = search_paths(header_name, CRX_EXTRA_INCLUDES, null);
			if (typeof(p) == "string") {
				c = file_get_contents(p);
			}
		}
...
function CHECK_HEADER_ADD_INCLUDE(header_name, flag_name, path_to_check, use_env, add_dir_part, add_to_flag_only)
{
	...
	if (path_to_check == null) {
		path_to_check = crx_usual_include_suspects;
	} else {
		path_to_check += ";" + crx_usual_include_suspects;
	}
...
function generate_version_info_manifest(makefiletarget)
{
	...
	if (MODE_CRXIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + manifest_name + ": " + CRX_DIR + "\\build\\default.manifest");
		MFO.WriteLine("\t" + CMD_MOD2 + "copy " + CRX_DIR + "\\build\\default.manifest $(BUILD_DIR)\\" + makefiletarget + ".manifest >nul");
	} else {
		MFO.WriteLine("$(BUILD_DIR)\\" + manifest_name + ": win32\\build\\default.manifest");
		MFO.WriteLine("\t" + CMD_MOD2 + "copy $(CRX_SRC_DIR)\\win32\\build\\default.manifest $(BUILD_DIR)\\" + makefiletarget + ".manifest >nul");
	}
...
function generate_version_info_resource(makefiletarget, basename, creditspath, sapi)
{
	...
	var res_prod_name = "CRX " + makefiletarget;
	var credits;
	var thanks = "";
	var logo = "";
	var debug = "";
	var project_url = "https://www.crx.net";
	var project_header = creditspath + "/crx_" + basename + ".h";
	var versioning = "";
	...
	if (creditspath.match(new RegExp("pecl"))) {
		...
		project_url = "https://pecl.crx.net/" + basename;
		...
		if (FSO.FileExists(project_header)) {
			if (header = FSO.OpenTextFile(project_header, 1)) {
				...
				if (contents.match(new RegExp('CRX_' + basename.toUpperCase() + '_VERSION(\\s+)"((\\d+\.\\d+(\.\\d+)?)((a|b)(\\d)?|\-[a-z]{3,5})?(RC\\d+)?(\-dev)?)'))) {
	...
	if (MODE_CRXIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + resname + ": $(CRX_DIR)\\build\\template.rc");
		MFO.WriteLine("\t" + CMD_MOD1 + "$(RC) /nologo  $(BASE_INCLUDES) /I $(CRX_DIR)/include /n /fo $(BUILD_DIR)\\" + resname + logo + debug +
			' /d FILE_DESCRIPTION="\\"' + res_desc + '\\"" /d FILE_NAME="\\"'
			+ makefiletarget + '\\"" /d URL="\\"' + project_url +
			'\\"" /d INTERNAL_NAME="\\"' + internal_name + versioning +
			'\\"" /d THANKS_GUYS="\\"' + thanks + '\\"" $(CRX_DIR)\\build\\template.rc');
	} else {
...
var CRX_MYMODULE_PGO = false; */
function is_pgo_desired(mod)
{
	var varname = "CRX_" + mod.toUpperCase() + "_PGO";

	/* XXX enable PGO in crxize mode */
	if (MODE_CRXIZE) {
		return false;
	}
...
function SAPI(sapiname, file_list, makefiletarget, cflags, obj_dir)
{
	...
	if (MODE_CRXIZE) {
		MFO.WriteLine("$(BUILD_DIR)\\" + makefiletarget + ": $(DEPS_" + SAPI + ") $(" + SAPI + "_GLOBAL_OBJS) $(CRXLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
	} else {
		MFO.WriteLine("$(BUILD_DIR)\\" + makefiletarget + ": $(DEPS_" + SAPI + ") $(" + SAPI + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(CRXLIB) $(BUILD_DIR)\\" + resname  + " $(BUILD_DIR)\\" + manifest_name);
	}
	...
	if(is_pgo_desired(sapiname) && (CRX_PGI == "yes" || CRX_PGO != "no")) {
	...
	if (MODE_CRXIZE) {
		if (ld) {
			MFO.WriteLine("\t" + ld + " /nologo /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(" + SAPI + "_GLOBAL_OBJS_RESP) $(CRXLIB) $(ARFLAGS_" + SAPI + ") $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname);
		} else {
			ld = CMD_MOD1 + '"$(LINK)"';
			MFO.WriteLine("\t" + ld + " /nologo " + " $(" + SAPI + "_GLOBAL_OBJS_RESP) $(CRXLIB) $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname + " /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(LDFLAGS_" + SAPI + ")");
		}
	} else {
		if (ld) {
			MFO.WriteLine("\t" + ld + " /nologo /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(" + SAPI + "_GLOBAL_OBJS_RESP) $(BUILD_DIR)\\$(CRXLIB) $(ARFLAGS_" + SAPI + ") $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname);
		} else {
			ld = CMD_MOD1 + '"$(LINK)"';
			MFO.WriteLine("\t" + ld + " /nologo " + " $(" + SAPI + "_GLOBAL_OBJS_RESP) $(BUILD_DIR)\\$(CRXLIB) $(LIBS_" + SAPI + ") $(BUILD_DIR)\\" + resname + " /out:$(BUILD_DIR)\\" + makefiletarget + " " + ldflags + " $(LDFLAGS_" + SAPI + ")");
		}
	}
...
function ADD_DIST_FILE(filename)
{
	if (configure_module_dirname.match("pecl")) {
		...
		ADD_FLAG("CRX_EXTRA_DIST_FILES", filename);
	}
}
...
function ADD_EXTENSION_DEP(extname, dependson, optional)
{
	...
	try {
		dep_present = eval("CRX_" + DEP);

		if (dep_present != "no") {
			try {
				dep_shared = eval("CRX_" + DEP + "_SHARED");
			} catch (e) {
				dep_shared = false;
			}
		}
	...
	var ext_shared = eval("CRX_" + EXT + "_SHARED");
	...
	if (dep_shared) {
		...
		ADD_FLAG("LIBS_" + EXT, "crx_" + dependson + ".lib");
		if (MODE_CRXIZE) {
			ADD_FLAG("LDFLAGS_" + EXT, "/libpath:$(BUILD_DIR_DEV)\\lib");
			ADD_FLAG("DEPS_" + EXT, "$(BUILD_DIR_DEV)\\lib\\crx_" + dependson + ".lib");
		} else {
			ADD_FLAG("LDFLAGS_" + EXT, "/libpath:$(BUILD_DIR)");
			ADD_FLAG("DEPS_" + EXT, "$(BUILD_DIR)\\crx_" + dependson + ".lib");
		}
	...
	} // dependency is statically built-in to CRX
	return true;
}

var static_pgo_enabled = false;

function CREX_EXTENSION(extname, file_list, shared, cflags, dllname, obj_dir)
{
...
function EXTENSION(extname, file_list, shared, cflags, dllname, obj_dir)
{
	...
	if (shared == null) {
		if (force_all_shared()) {
			shared = true;
			eval("CRX_" + EXT + "_SHARED = true;");
		} else {
			eval("shared = CRX_" + EXT + "_SHARED;");
		}
	} else {
		eval("CRX_" + EXT + "_SHARED = shared;");
	}
	...
	if (shared) {
		...
		cflags = "/D CREX_COMPILE_DL_EXT=1 /D COMPILE_DL_" + EXT + " /D " + EXT + "_EXPORTS=1 " + cflags;
		ADD_FLAG("CFLAGS_CRX", "/D COMPILE_DL_" + EXT);
	} else {
	...
	if (shared) {
		...
		ldflags = "";
		if (is_pgo_desired(extname) && (CRX_PGI == "yes" || CRX_PGO != "no")) {
			// Add compiler and link flags if PGO options are selected
			if (CRX_DEBUG != "yes" && CRX_PGI == "yes") {
		...
		MFO.WriteBlankLines(1);
		if (MODE_CRXIZE) {
			MFO.WriteLine("$(BUILD_DIR)\\" + dllname + ": $(DEPS_" + EXT + ") $(" + EXT + "_GLOBAL_OBJS) $(CRXLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
			MFO.WriteLine("\t" + ld + " $(" + EXT + "_GLOBAL_OBJS_RESP) $(CRXLIB) $(LIBS_" + EXT + ") $(LIBS) $(BUILD_DIR)\\" + resname + " /out:$(BUILD_DIR)\\" + dllname + " $(DLL_LDFLAGS) $(LDFLAGS) $(LDFLAGS_" + EXT + ")");
		} else {
			MFO.WriteLine("$(BUILD_DIR)\\" + dllname + ": $(DEPS_" + EXT + ") $(" + EXT + "_GLOBAL_OBJS) $(BUILD_DIR)\\$(CRXLIB) $(BUILD_DIR)\\" + resname + " $(BUILD_DIR)\\" + manifest_name);
			MFO.WriteLine("\t" + ld + " $(" + EXT + "_GLOBAL_OBJS_RESP) $(BUILD_DIR)\\$(CRXLIB) $(LIBS_" + EXT + ") $(LIBS) $(BUILD_DIR)\\" + resname + " /out:$(BUILD_DIR)\\" + dllname + ldflags + " $(DLL_LDFLAGS) $(LDFLAGS) $(LDFLAGS_" + EXT + ")");
		}
		...
		extension_module_ptrs += '\tcrxext_' + extname + '_ptr,\r\n';

		DEFINE('CFLAGS_' + EXT + '_OBJ', '$(CFLAGS_CRX) $(CFLAGS_' + EXT + ')');
	}
	if (MODE_CRXIZE) {
		if (!FSO.FileExists(CRX_DIR + "/include/main/config.pickle.h")) {
			var _tmp = FSO.CreateTextFile(CRX_DIR + "/include/main/config.pickle.h", true);
			_tmp.Close();
		}
	}
...
function ADD_SOURCES(dir, file_list, target, obj_dir)
{
	...
	for (i in file_list) {
		...
		if (obj_dir == null) {
			if (MODE_CRXIZE) {
				/* In the crxize mode, the subdirs are always relative to BUID_DIR.
...
function REMOVE_TARGET(dllname, flag)
{
	...
	var EXT = dllname.replace(/crx_(\S+)\.dll/, "$1").toUpperCase();
	var crx_flags = configure_subst.Item("CFLAGS_CRX");
	...
	if (configure_subst.Exists(flag)) {
		...
		if (targets.match(dllname)) {
			...
			configure_subst.Item("CFLAGS_CRX") = crx_flags.replace(" /D COMPILE_DL_" + EXT, "");
...
function write_extensions_summary()
{
	...
	if(crex_exts.length)
	{
		...
		STDOUT.WriteLine('Enabled Crex extensions:');
		output_as_table(['Extension', 'Mode'], crex_exts.sort());
	}
}
...
function write_summary()
{
	...
	ar[k++] = ['Build type', CRX_DEBUG == "yes" ? "Debug" : "Release"];
	ar[k++] = ['Thread Safety', CRX_ZTS == "yes" ? "Yes" : "No"];
	...
	if (CRX_PGO == "yes") {
		...
		ar[k++] = ['Optimization', CRX_DEBUG == "yes" ? "disabled" : "PGO disabled"];
	}
	var simd = configure_subst.Item("CRX_SIMD_SCALE");
	if (!!simd) {
...
function generate_tmp_crx_ini()
{
	...
	var ini_dir = CRX_OBJECT_OUT_DIR + ("yes" == CRX_DEBUG ? "Debug" : "Release") + ("yes" == CRX_ZTS ? "_TS" : "");
	CRX_TEST_INI_PATH = ini_dir + "\\tmp-crx.ini";

	if (FSO.FileExists(CRX_TEST_INI_PATH)) {
		STDOUT.WriteLine("Generating " + CRX_TEST_INI_PATH + " ...");
		var INI = FSO.OpenTextFile(CRX_TEST_INI_PATH, 2);
	} else {
		STDOUT.WriteLine("Regenerating " + CRX_TEST_INI_PATH + " ...");
		var INI = FSO.CreateTextFile(CRX_TEST_INI_PATH, true);
	}

	var ext_list = CRX_TEST_INI_EXT_EXCLUDE.split(",");
	...
	for (var i in extensions_enabled) {
		...
		var directive = (extensions_enabled[i][2] ? 'crex_extension' : 'extension');

		var ext_name = extensions_enabled[i][0];

		if (!is_on_exclude_list_for_test_ini(ext_list, ext_name)) {
			INI.WriteLine(directive + "=crx_" + ext_name + ".dll");
...
function generate_files()
{
	...
	if (CRX_SNAPSHOT_BUILD != "no") {
		STDOUT.WriteLine("Type 'nmake snap' to build a CRX snapshot");
	} else {
		STDOUT.WriteLine("Type 'nmake' to build CRX");
	}
}
...
function generate_config_pickle_h()
{
	...
	dest = CRX_DIR + "/include/main";

	var ignore_key = function(key) {
		var ignores = [ "CONFIGURE_COMMAND", "CRX_COMPILER_ID", "COMPILER", "ARCHITECTURE", "HAVE_STRNLEN", "CRX_DIR" ];
...
function generate_config_h()
{
	...
	prefix = CRX_PREFIX.replace(new RegExp("\\\\", "g"), "\\\\");
...
/* Generate crxize */
function generate_crxize()
{
	STDOUT.WriteLine("Generating crxize");
	...
	var MF = FSO.CreateTextFile(dest + "/crxize.js", true);
	var DEPS = FSO.CreateTextFile(dest + "/ext_deps.js", true);

	prefix = get_define("CRX_PREFIX");
	prefix = prefix.replace(new RegExp("/", "g"), "\\");
	prefix = prefix.replace(new RegExp("\\\\", "g"), "\\\\");
	MF.WriteLine("var CRX_PREFIX=" + '"' + prefix + '"');
	MF.WriteLine("var CRX_ZTS=" + '"' + (CRX_ZTS.toLowerCase() == "yes" ? "Yes" : "No") + '"');
	MF.WriteLine("var VC_VERSION=" + VCVERS);
	MF.WriteLine("var CRX_VERSION=" + CRX_VERSION);
	MF.WriteLine("var CRX_MINOR_VERSION=" + CRX_MINOR_VERSION);
	MF.WriteLine("var CRX_RELEASE_VERSION=" + CRX_RELEASE_VERSION);
	MF.WriteLine("var CRX_EXTRA_VERSION=\"" + CRX_EXTRA_VERSION + "\"");
	MF.WriteLine("var CRX_VERSION_STRING=\"" + CRX_VERSION_STRING + "\"");
	...
	for (i in extensions_enabled) {
		out = "CRX_" + extensions_enabled[i][0].toUpperCase() + "_SHARED=" + (extensions_enabled[i][1] == 'shared' ? 'true' : 'false') + ";";
		DEPS.WriteLine("CRX_" + extensions_enabled[i][0].toUpperCase() + "=true;");
		DEPS.WriteLine(out);
		MF.WriteLine(out);
	}

	MF.WriteBlankLines(2);
	MF.WriteLine("/* Generated win32/build/crxize.js.in */");
	MF.WriteBlankLines(1);
	MF.Write(file_get_contents("win32/build/crxize.js.in"));
	MF.Close();
	DEPS.Close();

	/* Generate flags file */
	/* spit out variable definitions */
	CJ = FSO.CreateTextFile(dest + "/config.crxize.js");

	CJ.WriteLine("var CRX_ZTS =" + '"' + CRX_ZTS + '"');
	CJ.WriteLine("var CRX_DEBUG=" + '"' + CRX_DEBUG + '"');
	CJ.WriteLine("var CRX_DLL_LIB =" + '"' + get_define('CRXLIB') + '"');
	CJ.WriteLine("var CRX_DLL =" + '"' + get_define('CRXDLL') + '"');
	CJ.WriteLine("var CRX_SECURITY_FLAGS =" + '"' + CRX_SECURITY_FLAGS + '"');

	/* The corresponding configure options aren't enabled through crxize,
		thus these dummy declarations are required. */
	CJ.WriteLine("var CRX_ANALYZER =" + '"no"');
	CJ.WriteLine("var CRX_PGO =" + '"no"');
	CJ.WriteLine("var CRX_PGI =" + '"no"');
	CJ.WriteLine("var CRX_ALL_SHARED =" + '"no"');
...
function handle_analyzer_makefile_flags(fd, key, val)
{
	...
	if ("no" == CRX_ANALYZER || "vs" == CRX_ANALYZER) {
		return;
	}
...
function generate_makefile()
{
	...
	var i;
	MF.WriteLine("CRX_SRC_DIR =" + CRX_SRC_DIR);
	for (i in keys) {
	...
	var val = "yes" == CRX_TEST_INI ? CRX_TEST_INI_PATH : "";
	/* Be sure it's done after generate_tmp_crx_ini(). */
	MF.WriteLine("CRX_TEST_INI_PATH=\"" + val + "\"");

	MF.WriteBlankLines(1);
	if (MODE_CRXIZE) {
		var TF = FSO.OpenTextFile(CRX_DIR + "/script/Makefile.crxize", 1);
	} else {
	...
	if (MODE_CRXIZE) {
		...
		for (var i in extensions_enabled) {
			var lib = "crx_" + extensions_enabled[i][0] + ".lib";
			var dll = "crx_" + extensions_enabled[i][0] + ".dll";
			MF.WriteLine("	" + CMD_MOD2 + "copy $(BUILD_DIR)\\" + lib + " $(BUILD_DIR_DEV)\\lib");
			MF.WriteLine("  " + CMD_MOD2 + "if not exist $(CRX_PREFIX) mkdir $(CRX_PREFIX) >nul");
			MF.WriteLine("  " + CMD_MOD2 + "if not exist $(CRX_PREFIX)\\ext mkdir $(CRX_PREFIX)\\ext >nul");
			MF.WriteLine("	" + CMD_MOD2 + "copy $(BUILD_DIR)\\" + dll + " $(CRX_PREFIX)\\ext");
		}
	} else {
		MF.WriteBlankLines(1);
		...
		for (var i in extensions_enabled) {
			...
			lib = "crx_" + extensions_enabled[i][0] + "*.lib";
	...
	var extra_path = "$(CRX_BUILD)\\bin";
	if (CRX_EXTRA_LIBS.length) {
		path = CRX_EXTRA_LIBS.split(';');
	...
	if (FSO.FileExists(CRX_MAKEFILE_FRAGMENTS)) {
		TF = FSO.OpenTextFile(CRX_MAKEFILE_FRAGMENTS, 1);
		if (!TF.AtEndOfStream) {
			MF.Write(TF.ReadAll());
		}
		TF.Close();
		MF.WriteBlankLines(2);
		FSO.DeleteFile(CRX_MAKEFILE_FRAGMENTS, true);
	}
...
{
	if (!configure_subst.Exists("CRXDEF")) {
		DEFINE("CRXDEF", "$(BUILD_DIR)\\$(CRXDLL).def");
		ADD_FLAG("CRX_LDFLAGS", "/def:$(CRXDEF)");
	}
	ADD_FLAG("CRX_DLL_DEF_SOURCES", name);
}
...
/* Install Headers */
function CRX_INSTALL_HEADERS(dir, headers_list)
{
...
{
	if ("clang" == CRX_TOOLSET) {
		VS_TOOLSET = false;
		CLANG_TOOLSET = true;
		ICC_TOOLSET = false;
	} else if ("icc" == CRX_TOOLSET) {
		VS_TOOLSET = false;
		CLANG_TOOLSET = false;
		ICC_TOOLSET = true;
	} else {
		/* Visual Studio is the default toolset. */
		CRX_TOOLSET = "no" == CRX_TOOLSET ? "vs" : CRX_TOOLSET;
		if (!!CRX_TOOLSET && "vs" != CRX_TOOLSET) {
			ERROR("Unsupported toolset '" + CRX_TOOLSET + "'");
		}
		VS_TOOLSET = true;
		CLANG_TOOLSET = false;
		ICC_TOOLSET = false;
...
function toolset_setup_compiler()
{
	...
	if (VS_TOOLSET) {
		...
		if ("unknown" == COMPILER_NAME_LONG) {
			var tmp = probe_binary(CRX_CL);
			COMPILER_NAME_LONG = COMPILER_NAME_SHORT = "MSVC " + tmp + ", untested";

			WARNING("Using unknown MSVC version " + tmp);

			AC_DEFINE('CRX_BUILD_COMPILER', COMPILER_NAME_LONG, "Detected compiler version");
			DEFINE("CRX_COMPILER_SHORT", tmp);
			AC_DEFINE('CRX_COMPILER_ID', tmp, "Compiler compatibility ID");
		} else {
			AC_DEFINE('CRX_BUILD_COMPILER', COMPILER_NAME_LONG, "Detected compiler version");
			DEFINE("CRX_COMPILER_SHORT", COMPILER_NAME_SHORT.toLowerCase());
			AC_DEFINE('CRX_COMPILER_ID', COMPILER_NAME_SHORT.toUpperCase(), "Compiler compatibility ID");
		}
	} else if (CLANG_TOOLSET) {
		CLANGVERS = COMPILER_NUMERIC_VERSION;

		AC_DEFINE('CRX_BUILD_COMPILER', COMPILER_NAME_LONG, "Detected compiler version");
		DEFINE("CRX_COMPILER_SHORT", "clang");
		AC_DEFINE('CRX_COMPILER_ID', "clang"); /* XXX something better were to write here */

	} else if (ICC_TOOLSET) {
		INTELVERS = COMPILER_NUMERIC_VERSION;

		AC_DEFINE('CRX_BUILD_COMPILER', COMPILER_NAME_LONG, "Detected compiler version");
		DEFINE("CRX_COMPILER_SHORT", "icc");
		AC_DEFINE('CRX_COMPILER_ID', "icc"); /* XXX something better were to write here */
	}
...
{
	if (VS_TOOLSET) {
		return PATH_PROG('cl', null, 'CRX_CL')
	} else if (CLANG_TOOLSET) {
		return PATH_PROG('clang-cl', null, 'CRX_CL')
	} else if (ICC_TOOLSET) {
		return PATH_PROG('icl', null, 'CRX_CL')
	}
...
function toolset_get_compiler_version()
{
	...
	if (VS_TOOLSET) {
		version = probe_binary(CRX_CL).substr(0, 5).replace('.', '');

		return version;
	} else if (CLANG_TOOLSET) {
		var command = 'cmd /c ""' + CRX_CL + '" -v"';
	...
	if (VS_TOOLSET) {
		...
		if (version >= 1940) {
			...
			/* NOTE - VS is intentional. Due to changes in recent Visual Studio
						...
						crx_win32_image_compatible(), if needed. */
...
{
	if (VS_TOOLSET) {
		return probe_binary(CRX_CL, 'arch');
	} else if (CLANG_TOOLSET) {
		/*var command = 'cmd /c ""' + CRX_CL + '" -v"';
		var full = execute(command + '" 2>&1"');
...
function toolset_setup_arch()
{
	...
	AC_DEFINE('CRX_BUILD_ARCH', TARGET_ARCH, "Detected compiler architecture");
	DEFINE("CRX_ARCHITECTURE", TARGET_ARCH);
}

function toolset_setup_codegen_arch()
{
	if("no" == CRX_CODEGEN_ARCH || "yes" == CRX_CODEGEN_ARCH) {
		return;
	}

	if (VS_TOOLSET) {
		var arc = CRX_CODEGEN_ARCH.toUpperCase();
		...
		CRX_NATIVE_INTRINSICS = "disabled";
	}
}
...
function toolset_setup_linker()
{
	...
	AC_DEFINE('CRX_LINKER_MAJOR', major, "Linker major version", false);
	AC_DEFINE('CRX_LINKER_MINOR', minor, "Linker minor version", false);
...
function toolset_setup_common_cflags()
{
	...
	// CFLAGS for building the CRX dll
	DEFINE("CFLAGS_CRX", "/D _USRDLL /D CRX7DLLTS_EXPORTS /D CRX_EXPORTS \
	/D LIBCREX_EXPORTS /D TSRM_EXPORTS /D SAPI_EXPORTS /D WINVER=" + WINVER);

	DEFINE('CFLAGS_CRX_OBJ', '$(CFLAGS_CRX) $(STATIC_EXT_CFLAGS)');

	// General CFLAGS for building objects
	DEFINE("CFLAGS", "/nologo $(BASE_INCLUDES) /D _WINDOWS /D WINDOWS=1 \
		/D CREX_WIN32=1 /D CRX_WIN32=1 /D WIN32 /D _MBCS \
		/D _USE_MATH_DEFINES");
		...
		if (CRX_DEBUG == "yes") {
			...
			if (CRX_DEBUG == "no" && CRX_SECURITY_FLAGS == "yes") {
				...
				if (VCVERS >= 1912) {
					...
					var subver1900 = probe_binary(CRX_CL).substr(6);
					if (subver1900 >= 24241) {
						ADD_FLAG('CFLAGS', "/Qspectre");
					}
				}
...
function toolset_setup_intrinsic_cflags()
{
	...
	if (VS_TOOLSET) {
		if ("disabled" == CRX_NATIVE_INTRINSICS) {
			ERROR("Can't enable intrinsics, --with-codegen-arch passed with an incompatible option. ")
		}

		if (TARGET_ARCH == 'arm64') {
			/* arm64 supports neon */
			configure_subst.Add("CRX_SIMD_SCALE", 'NEON');
			/* all offically supported arm64 cpu supports crc32 (TODO: to be confirmed) */
			AC_DEFINE('HAVE_ARCH64_CRC32', 1);
			return;
		}

		if ("no" == CRX_NATIVE_INTRINSICS || "yes" == CRX_NATIVE_INTRINSICS) {
			CRX_NATIVE_INTRINSICS = default_enabled;
		}

		if ("all" == CRX_NATIVE_INTRINSICS) {
			...
			configure_subst.Add("CRX_SIMD_SCALE", "AVX2");
		} else {
			var list = CRX_NATIVE_INTRINSICS.split(",");
			var j = 0;
			...
			configure_subst.Add("CRX_SIMD_SCALE", scale[j].toUpperCase());
...
function toolset_setup_common_ldlags()
{
	...
	// CRX DLL link flags
	DEFINE("CRX_LDFLAGS", "$(DLL_LDFLAGS)");
	...
	// we want msvcrt in the CRX DLL
	ADD_FLAG("CRX_LDFLAGS", "/nodefaultlib:libcmt");
...
{
	if (CRX_DEBUG == "yes") {
		ADD_FLAG("CFLAGS", "/LDd /MDd /Od /D _DEBUG /D CREX_DEBUG=1 " +
			(TARGET_ARCH == 'x86'?"/ZI":"/Zi"));
		ADD_FLAG("LDFLAGS", "/debug");
		// Avoid problems when linking to release libraries that use the release
		// version of the libc
		ADD_FLAG("CRX_LDFLAGS", "/nodefaultlib:msvcrt");
	} else {
		...
		if (CRX_SANITIZER == "yes" && CLANG_TOOLSET) {
			ADD_FLAG("CFLAGS", "/Od /D NDebug /D NDEBUG /D CREX_WIN32_NEVER_INLINE /D CREX_DEBUG=0");
		} else {
			// Equivalent to Release_TSInline build -> best optimization
			ADD_FLAG("CFLAGS", "/Ox /D NDebug /D NDEBUG /GF /D CREX_DEBUG=0");
		}
...
{
	if (CRX_OBJECT_OUT_DIR.length) {
		CRX_OBJECT_OUT_DIR = FSO.GetAbsolutePathName(CRX_OBJECT_OUT_DIR);
		if (!FSO.FolderExists(CRX_OBJECT_OUT_DIR)) {
			ERROR('chosen output directory ' + CRX_OBJECT_OUT_DIR + ' does not exist');
		}
		CRX_OBJECT_OUT_DIR += '\\';
	} else {
		CRX_OBJECT_OUT_DIR = FSO.GetAbsolutePathName(".") + '\\';

		if (TARGET_ARCH == 'x64') {
			CRX_OBJECT_OUT_DIR += 'x64\\';
			if (!FSO.FolderExists(CRX_OBJECT_OUT_DIR)) {
				FSO.CreateFolder(CRX_OBJECT_OUT_DIR);
			}
		} else if (TARGET_ARCH == 'arm64') {
			CRX_OBJECT_OUT_DIR += 'arm64\\';
			if (!FSO.FolderExists(CRX_OBJECT_OUT_DIR)) {
				FSO.CreateFolder(CRX_OBJECT_OUT_DIR);
			}
		}
...
function setup_zts_stuff()
{
	...
	DEFINE("CRX_ZTS_ARCHIVE_POSTFIX", CRX_ZTS == "yes" ? '' : "-nts");

	// set up the build dir and DLL name
	if (CRX_DEBUG == "yes" && CRX_ZTS == "yes") {
		DEFINE("BUILD_DIR", CRX_OBJECT_OUT_DIR + "Debug_TS");
		if (!MODE_CRXIZE) {
			DEFINE("CRXDLL", "crx" + CRX_VERSION + "ts_debug.dll");
			DEFINE("CRXLIB", "crx" + CRX_VERSION + "ts_debug.lib");
		}
	} else if (CRX_DEBUG == "yes" && CRX_ZTS == "no") {
		DEFINE("BUILD_DIR", CRX_OBJECT_OUT_DIR + "Debug");
		if (!MODE_CRXIZE) {
			DEFINE("CRXDLL", "crx" + CRX_VERSION + "_debug.dll");
			DEFINE("CRXLIB", "crx" + CRX_VERSION + "_debug.lib");
		}
	} else if (CRX_DEBUG == "no" && CRX_ZTS == "yes") {
		DEFINE("BUILD_DIR", CRX_OBJECT_OUT_DIR + "Release_TS");
		if (!MODE_CRXIZE) {
			DEFINE("CRXDLL", "crx" + CRX_VERSION + "ts.dll");
			DEFINE("CRXLIB", "crx" + CRX_VERSION + "ts.lib");
		}
	} else if (CRX_DEBUG == "no" && CRX_ZTS == "no") {
		DEFINE("BUILD_DIR", CRX_OBJECT_OUT_DIR + "Release");
		if (!MODE_CRXIZE) {
			DEFINE("CRXDLL", "crx" + CRX_VERSION + ".dll");
			DEFINE("CRXLIB", "crx" + CRX_VERSION + ".lib");
		}
	}
...
function crx_build_option_handle()
{
	if (CRX_CRX_BUILD == 'no') {
		if (FSO.FolderExists("..\\deps")) {
			CRX_CRX_BUILD = "..\\deps";
		} else {
			if (FSO.FolderExists("..\\crx_build")) {
				CRX_CRX_BUILD = "..\\crx_build";
			} else {
				if (TARGET_ARCH != 'x86') {
					if (FSO.FolderExists("..\\win64build")) {
						CRX_CRX_BUILD = "..\\win64build";
					} else if (FSO.FolderExists("..\\crx-win64-dev\\crx_build")) {
						CRX_CRX_BUILD = "..\\crx-win64-dev\\crx_build";
					}
				} else {
					if (FSO.FolderExists("..\\win32build")) {
						CRX_CRX_BUILD = "..\\win32build";
					} else if (FSO.FolderExists("..\\crx-win32-dev\\crx_build")) {
						CRX_CRX_BUILD = "..\\crx-win32-dev\\crx_build";
					}
				}
			}
		}
		CRX_CRX_BUILD = FSO.GetAbsolutePathName(CRX_CRX_BUILD);
	}
...
function probe_basic_headers()
{
	...
	if (CRX_CRX_BUILD != "no") {
		crx_usual_include_suspects += ";" + CRX_CRX_BUILD + "\\include";
		crx_usual_lib_suspects += ";" + CRX_CRX_BUILD + "\\lib";
	}
}
...
function add_extra_dirs()
{
	...
	if (CRX_EXTRA_INCLUDES.length) {
		path = CRX_EXTRA_INCLUDES.split(';');
		for (i = 0; i < path.length; i++) {
...
{
	return !!CRX_ALL_SHARED && "yes" == CRX_ALL_SHARED;
}
...
function ADD_MAKEFILE_FRAGMENT(src_file)
{
	...
	if (FSO.FileExists(fn_in)) {
		var h_in, h_out;
		var create_out_fl = !FSO.FileExists(CRX_MAKEFILE_FRAGMENTS);
		var open_flags = create_out_fl ? 2 : 8;

		h_in = FSO.OpenTextFile(fn_in, 1);
		h_out = FSO.OpenTextFile(CRX_MAKEFILE_FRAGMENTS, open_flags, create_out_fl);
...
function check_binary_tools_sdk()
{
	...
	var out = execute("cmd /c crxsdk_version");

	if (out.match(/CRX SDK (\d+)\.(\d+)\.(\d+).*/)) {
	...
	if (BIN_TOOLS_SDK_VER_MAJOR < 2) {
		ERROR("Incompatible binary tools version. Please consult\r\nhttps://wiki.crx.net/internals/windows/stepbystepbuild_sdk_2");
	}
}
...
{
	if ("no" != CRX_VERBOSITY) {
		if ("yes" == CRX_VERBOSITY) {
			VERBOSITY = 1;
			if (1 == VERBOSITY) {
				CMD_MOD1 = "";
			}
		} else {
			var _tmp = parseInt(CRX_VERBOSITY);
			if (0 != _tmp && 1 != _tmp && 2 != _tmp) {
				ERROR("Unsupported verbosity level '" + CRX_VERBOSITY + "'");
			}
			VERBOSITY = _tmp;
