 ...
 * and have been modified to work with CRX.
 */

static crex_always_inline void usage_to_timeval(FILETIME *ft, struct timeval *tv)
{
	ULARGE_INTEGER time;
	...
	tv->tv_sec = (crex_long) (time.QuadPart / 10000000);
	tv->tv_usec = (crex_long) ((time.QuadPart % 10000000) / 10);
}
