/*
 *    CRX Sendmail for Windows.
 *
 *  This file is rewritten specifically for CRXFI.  Some functionality
...
/* '*error_message' has to be passed around from crx_mail() */
...
static char *ErrorMessages[] =
{
	...
	{"Failed to connect to mailserver, verify your \"SMTP\" setting in crx.ini"},
	{"Failed to Send"},
	{"Failed to Receive"},
	{"Server Error"},
	...
	{"Bad Mail Host"},
	{"Bad Message File"},
	{"\"sendmail_from\" not set in crx.ini or custom \"From:\" header missing"},
	{"Mailserver rejected our \"sendmail_from\" setting"}, /* 20 */
	{"Error while trimming mail header with PCRE, please file a bug report at https://github.com/crx/crx-src/issues"} /* 21 */
};
 ...
 */
#define CRX_WIN32_MAIL_UNIFY_PATTERN	"/(\r\n?)|\n/"
#define CRX_WIN32_MAIL_UNIFY_REPLACE	"\r\n"
...
#define CRX_WIN32_MAIL_RMVDBL_PATTERN	"/^\r\n|(\r\n)+$/m"
#define CRX_WIN32_MAIL_RMVDBL_REPLACE	""
 ...
 * message body. */
#define CRX_WIN32_MAIL_DOT_PATTERN	"\n."
#define CRX_WIN32_MAIL_DOT_REPLACE	"\n.."
 ...
 */
static crex_string *crx_win32_mail_trim_header(const char *header)
{
	crex_string *result, *result2;
	crex_string *replace;
	crex_string *regex;
	...
	replace = crex_string_init(CRX_WIN32_MAIL_UNIFY_REPLACE, strlen(CRX_WIN32_MAIL_UNIFY_REPLACE), 0);
	regex = crex_string_init(CRX_WIN32_MAIL_UNIFY_PATTERN, sizeof(CRX_WIN32_MAIL_UNIFY_PATTERN)-1, 0);

	result = crx_pcre_replace(regex,
				  NULL, header, strlen(header),
				  replace,
				  -1,
				  NULL);

	crex_string_release_ex(replace, 0);
	crex_string_release_ex(regex, 0);

	if (NULL == result) {
		return NULL;
	}

	replace = crex_string_init(CRX_WIN32_MAIL_RMVDBL_PATTERN, strlen(CRX_WIN32_MAIL_RMVDBL_PATTERN), 0);
	regex = crex_string_init(CRX_WIN32_MAIL_RMVDBL_PATTERN, sizeof(CRX_WIN32_MAIL_RMVDBL_PATTERN)-1, 0);

	result2 = crx_pcre_replace(regex,
...
//********************************************************************/
CRXAPI int TSendMail(const char *host, int *error, char **error_message,
			  const char *headers, const char *Subject, const char *mailTo, const char *data,
			  char *mailCc, char *mailBcc, char *mailRPath)
{
	int ret;
	char *RPath = NULL;
	crex_string *headers_lc = NULL, *headers_trim = NULL; /* headers_lc is only created if we've a header at all */
	...
	if (headers) {
		...
		if (NULL == (headers_trim = crx_win32_mail_trim_header(headers))) {
		...
		headers_lc = crex_string_tolower(headers_trim);
	}

	/* Fall back to sendmail_from crx.ini setting */
	if (mailRPath && *mailRPath) {
		...
		if (!found) {
			if (headers) {
				crex_string_release(headers_trim);
				crex_string_release(headers_lc);
			}
	...
	if (*error != 0) {
		...
		*error_message = ecalloc(1, HOST_NAME_LEN + 128);
		snprintf(*error_message, HOST_NAME_LEN + 128,
			"Failed to connect to mailserver at \"%s\" port " CREX_ULONG_FMT ", verify your \"SMTP\" "
			"and \"smtp_port\" setting in crx.ini or use ini_set()",
...
// History:
//*******************************************************************/
CRXAPI char *GetSMErrorText(int index)
{
			 ...
			 const char *headers, char *headers_lc, char **error_message)
{
	...
	*/
	...
	/* Escape \n. sequences
	 * We use crx_str_to_str() and not crx_str_replace_in_subject(), since the latter
	 * uses ZVAL as it's parameters */
	data_cln = crx_str_to_str(data, strlen(data), CRX_WIN32_MAIL_DOT_PATTERN, sizeof(CRX_WIN32_MAIL_DOT_PATTERN) - 1,
					CRX_WIN32_MAIL_DOT_REPLACE, sizeof(CRX_WIN32_MAIL_DOT_REPLACE) - 1);
	if (!data_cln) {
...
static int PostHeader(char *RPath, const char *Subject, const char *mailTo, char *xheaders)
{
	...
	if (!xheaders || !strstr(headers_lc, "date:")) {
		...
		crex_string *dt = crx_format_date("r", 1, tNow, 1);
...
{
	CREX_TLS char buf[MAIL_BUFFER_SIZE];
	int rlen;
	int Index = 0;
	int Received = 0;
