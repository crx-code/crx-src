...
void vsyslog(int priority, const char *message, va_list args)
{
	...
	if (INVALID_HANDLE_VALUE == PW32G(log_source))
		openlog("crx", LOG_PID, LOG_SYSLOG);

	switch (priority) {			/* translate UNIX type into NT type */
		case LOG_ALERT:
			etype = EVENTLOG_ERROR_TYPE;
			evid = CRX_SYSLOG_ERROR_TYPE;
			break;
		case LOG_INFO:
			etype = EVENTLOG_INFORMATION_TYPE;
			evid = CRX_SYSLOG_INFO_TYPE;
			break;
		default:
			etype = EVENTLOG_WARNING_TYPE;
			evid = CRX_SYSLOG_WARNING_TYPE;
	}

	vspprintf(&tmp, 0, message, args);	/* build message */

	strsw[0] = crx_win32_cp_any_to_w(PW32G(log_header));
	strsw[1] = crx_win32_cp_any_to_w(tmp);
...
void openlog(const char *ident, int logopt, int facility)
{
	...
	PW32G(log_source) = RegisterEventSource(NULL, "CRX-" CRX_VERSION);
