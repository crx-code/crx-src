...
{/*{{{*/
	...
	buf = crx_win32_cp_conv_w_to_any(bufw, ret, CRX_WIN32_CP_IGNORE_LEN_P);
...
* the BSD like license. Modified for CRX by ab@crx.net
*
...
{/*{{{*/
	...
	uname = crx_win32_cp_conv_w_to_any(unamew, unsize - 1, &uname_len);
	if (!uname) {
		return NULL;
	}
...
static crex_always_inline BOOL is_compatible(HMODULE handle, BOOL is_smaller, char *format, char **err)
{/*{{{*/
	...
	/* VS 2015, 2017, 2019 and 2022 are binary compatible, but only forward compatible.
		...
		Analogously, it should be fine, if a CRX build linked with an older version
	...
	DWORD core_minor = (DWORD)(CRX_LINKER_MINOR/10);
	DWORD comp_minor = (DWORD)(minor/10);
	if (14 == major && (is_smaller ? core_minor < comp_minor : core_minor > comp_minor) || CRX_LINKER_MAJOR != major)
#else
	if (CRX_LINKER_MAJOR != major)
#endif
	{
		char buf[MAX_PATH];
		if (GetModuleFileName(handle, buf, sizeof(buf)) != 0) {
			spprintf(err, 0, format, buf, major, minor, CRX_LINKER_MAJOR, CRX_LINKER_MINOR);
		} else {
...
# endif
	...
	return is_compatible(handle, FALSE, "'%s' %u.%u is not compatible with this CRX build linked with %d.%d", err);
#endif
	return TRUE;
}/*}}}*/
