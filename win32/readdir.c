...
{/*{{{*/
	...
	resolvedw = crx_win32_ioutil_conv_any_to_w(resolved_path_buff, CRX_WIN32_CP_IGNORE_LEN, &resolvedw_len);
	if (!resolvedw) {
		return NULL;
	}

	might_need_prefix = resolvedw_len >= 3 && CRX_WIN32_IOUTIL_IS_LETTERW(resolvedw[0]) && L':' == resolvedw[1] && CRX_WIN32_IOUTIL_IS_SLASHW(resolvedw[2]);

	filespecw_len = resolvedw_len + 2;
	if (filespecw_len >= _MAX_PATH && might_need_prefix) {
		filespecw_len += CRX_WIN32_IOUTIL_LONG_PATH_PREFIX_LENW;
	}
	...
	if (filespecw_len >= _MAX_PATH && might_need_prefix) {
		wcscpy(filespecw, CRX_WIN32_IOUTIL_LONG_PATH_PREFIXW);
		wcscpy(filespecw + CRX_WIN32_IOUTIL_LONG_PATH_PREFIX_LENW, resolvedw);
		index = resolvedw_len + CRX_WIN32_IOUTIL_LONG_PATH_PREFIX_LENW - 1;
	} else {
...
{/*{{{*/
	...
	_tmp = crx_win32_cp_conv_w_to_any(dp->fileinfo.cFileName, CRX_WIN32_CP_IGNORE_LEN, &reclen);
	if (!_tmp) {
...
{/*{{{*/
	...
	might_need_prefix = dirw_len >= 3 && CRX_WIN32_IOUTIL_IS_LETTERW(dp->dirw[0]) && L':' == dp->dirw[1] && CRX_WIN32_IOUTIL_IS_SLASHW(dp->dirw[2]);
	...
	if (filespecw_len >= _MAX_PATH && might_need_prefix) {
		wcscpy(filespecw, CRX_WIN32_IOUTIL_LONG_PATH_PREFIXW);
		wcscpy(filespecw + CRX_WIN32_IOUTIL_LONG_PATH_PREFIX_LENW, dp->dirw);
		index = dirw_len + CRX_WIN32_IOUTIL_LONG_PATH_PREFIX_LENW - 1;
	} else {
