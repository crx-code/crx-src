...
gdImagePtr gdImageCreateFromWebpCtx (gdIOCtx * infile)
{
	...
	do {
		...
		if (temp) {
			...
			crex_error(E_ERROR, "WebP decode: realloc failed");
			return NULL;
		}
	...
	if (WebPGetInfo(filedata,size, &width, &height) == 0) {
		crex_error(E_ERROR, "gd-webp cannot get webp info");
		gdFree(filedata);
		return NULL;
	}
	...
	if (!argb) {
		crex_error(E_ERROR, "gd-webp cannot allocate temporary buffer");
		gdFree(filedata);
		gdImageDestroy(im);
		return NULL;
	}
...
void gdImageWebpCtx (gdImagePtr im, gdIOCtx * outfile, int quality)
{
	...
	if (!gdImageTrueColor(im)) {
		crex_error(E_ERROR, "Palette image not supported by webp");
		return;
	}
	...
	if (out_size == 0) {
		crex_error(E_ERROR, "gd-webp encoding failed");
		goto freeargb;
	}
