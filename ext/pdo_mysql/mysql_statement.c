...
static int pdo_mysql_stmt_dtor(pdo_stmt_t *stmt) /* {{{ */
{
	...
	if (!S->done && !C_ISUNDEF(stmt->database_object_handle)
...
static void pdo_mysql_stmt_set_row_count(pdo_stmt_t *stmt) /* {{{ */
{
	...
	crex_long row_count = (crex_long) mysql_stmt_affected_rows(S->stmt);
	if (row_count != (crex_long)-1) {
...
static int pdo_mysql_fill_stmt_from_result(pdo_stmt_t *stmt) /* {{{ */
{
	...
	if (row_count == (my_ulonglong)-1) {
		...
		stmt->row_count = (crex_long) mysql_num_rows(S->result);
		crx_pdo_stmt_set_column_count(stmt, (int) mysql_num_fields(S->result));
		S->fields = mysql_fetch_fields(S->result);
	} else {
		/* this was a DML or DDL query (INSERT, UPDATE, DELETE, ... */
		stmt->row_count = (crex_long) row_count;
	}
...
static bool pdo_mysql_stmt_after_execute_prepared(pdo_stmt_t *stmt) {
	...
	crx_pdo_stmt_set_column_count(stmt, mysql_stmt_field_count(S->stmt));
	...
	if (S->result) {
		...
		S->out_length = ecalloc(stmt->column_count, sizeof(crex_ulong));
		...
		switch (event_type) {
			...
			case PDO_PARAM_EVT_EXEC_PRE:
...
#ifdef PDO_USE_MYSQLND
				if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_NULL || (C_TYPE_P(parameter) == IS_NULL)) {
				...
				*b->is_null = 0;
				...
				PDO_DBG_INF_FMT("param->parameter->type=%d", C_TYPE(param->parameter));
				if (!C_ISREF(param->parameter)) {
					parameter = &param->parameter;
				} else {
					parameter = C_REFVAL(param->parameter);
				...
				switch (C_TYPE_P(parameter)) {
					case IS_STRING:
						b->buffer_type = MYSQL_TYPE_STRING;
						b->buffer = C_STRVAL_P(parameter);
						b->buffer_length = C_STRLEN_P(parameter);
						*b->length = C_STRLEN_P(parameter);
						PDO_DBG_RETURN(1);
					...
					case IS_LONG:
						...
						b->buffer = &C_LVAL_P(parameter);
						PDO_DBG_RETURN(1);

					case IS_DOUBLE:
						b->buffer_type = MYSQL_TYPE_DOUBLE;
						b->buffer = &C_DVAL_P(parameter);
						PDO_DBG_RETURN(1);
...
static int pdo_mysql_stmt_fetch(pdo_stmt_t *stmt, enum pdo_fetch_orientation ori, crex_long offset) /* {{{ */
{
	pdo_mysql_stmt *S = (pdo_mysql_stmt*)stmt->driver_data;
...
#ifdef PDO_USE_MYSQLND
			cols[i].name = crex_string_copy(S->fields[i].sname);
#else
			cols[i].name = crex_string_init(S->fields[i].name, S->fields[i].name_length, 0);
#endif
		}
...
static int pdo_mysql_stmt_col_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value) /* {{{ */
{
	pdo_mysql_stmt *S = (pdo_mysql_stmt*)stmt->driver_data;
	const MYSQL_FIELD *F;
	zval flags;
