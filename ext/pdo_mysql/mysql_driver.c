...
static bool mysql_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options)
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
	...
	if (S->num_params) {
		...
		S->in_length = ecalloc(S->num_params, sizeof(crex_ulong));
#endif
	}
...
/* {{{ mysql_handle_doer */
static crex_long mysql_handle_doer(pdo_dbh_t *dbh, const crex_string *sql)
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
...
static crex_string *pdo_mysql_last_insert_id(pdo_dbh_t *dbh, const crex_string *name)
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
	PDO_DBG_ENTER("pdo_mysql_last_insert_id");
	PDO_DBG_RETURN(crex_u64_to_str(mysql_insert_id(H->server)));
}
/* }}} */
...
/* {{{ mysql_handle_quoter */
static crex_string* mysql_handle_quoter(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype )
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
...
static bool mysql_handle_begin(pdo_dbh_t *dbh)
{
	...
	crex_string_release_ex(command, 0);
...
static bool pdo_mysql_set_attribute(pdo_dbh_t *dbh, crex_long attr, zval *val)
{
	crex_long lval;
	bool bval;
	PDO_DBG_ENTER("pdo_mysql_set_attribute");
	PDO_DBG_INF_FMT("dbh=%p", dbh);
	PDO_DBG_INF_FMT("attr=" CREX_LONG_FMT, attr);
...
static int pdo_mysql_get_attribute(pdo_dbh_t *dbh, crex_long attr, zval *return_value)
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
...
static crex_result pdo_mysql_check_liveness(pdo_dbh_t *dbh)
{
	pdo_mysql_db_handle *H = (pdo_mysql_db_handle *)dbh->driver_data;
...
static int pdo_mysql_handle_factory(pdo_dbh_t *dbh, zval *driver_options)
{
	...
	crx_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, 7);
	...
	if (driver_options) {
		crex_long connect_timeout = pdo_attr_lval(driver_options, PDO_ATTR_TIMEOUT, 30);
		crex_string *init_cmd = NULL;
#ifndef PDO_USE_MYSQLND
		crex_string *default_file = NULL, *default_group = NULL;
#endif
		crex_long compress = 0;
		crex_string *ssl_key = NULL, *ssl_cert = NULL, *ssl_ca = NULL, *ssl_capath = NULL, *ssl_cipher = NULL;
...
#if (MYSQL_VERSION_ID >= 80021 && !defined(MARIADB_BASE_VERSION)) || defined(PDO_USE_MYSQLND)
		crex_string *local_infile_directory = pdo_attr_strval(driver_options, PDO_MYSQL_ATTR_LOCAL_INFILE_DIRECTORY, NULL);
		if (local_infile_directory && !crx_check_open_basedir(ZSTR_VAL(local_infile_directory))) {
			if (mysql_options(H->server, MYSQL_OPT_LOAD_DATA_LOCAL_DIR, (const char *)ZSTR_VAL(local_infile_directory))) {
				crex_string_release(local_infile_directory);
				pdo_mysql_error(dbh);
				goto cleanup;
			}
			crex_string_release(local_infile_directory);
		...
		*/
		...
		if (init_cmd) {
			if (mysql_options(H->server, MYSQL_INIT_COMMAND, (const char *)ZSTR_VAL(init_cmd))) {
				crex_string_release_ex(init_cmd, 0);
				pdo_mysql_error(dbh);
				goto cleanup;
			}
			crex_string_release_ex(init_cmd, 0);
		...
		if (default_file) {
			if (mysql_options(H->server, MYSQL_READ_DEFAULT_FILE, (const char *)ZSTR_VAL(default_file))) {
				crex_string_release_ex(default_file, 0);
				pdo_mysql_error(dbh);
				goto cleanup;
			}
			crex_string_release_ex(default_file, 0);
		...
		if (default_group) {
			if (mysql_options(H->server, MYSQL_READ_DEFAULT_GROUP, (const char *)ZSTR_VAL(default_group))) {
				crex_string_release_ex(default_group, 0);
				pdo_mysql_error(dbh);
				goto cleanup;
			}
			crex_string_release_ex(default_group, 0);
		...
		if (ssl_key || ssl_cert || ssl_ca || ssl_capath || ssl_cipher) {
			...
			if (ssl_key) {
				crex_string_release_ex(ssl_key, 0);
			}
			if (ssl_cert) {
				crex_string_release_ex(ssl_cert, 0);
			}
			if (ssl_ca) {
				crex_string_release_ex(ssl_ca, 0);
			}
			if (ssl_capath) {
				crex_string_release_ex(ssl_capath, 0);
			}
			if (ssl_cipher) {
				crex_string_release_ex(ssl_cipher, 0);
			}
		}

#if MYSQL_VERSION_ID > 50605 || defined(PDO_USE_MYSQLND)
		{
			crex_string *public_key = pdo_attr_strval(driver_options, PDO_MYSQL_ATTR_SERVER_PUBLIC_KEY, NULL);
			if (public_key) {
				if (mysql_options(H->server, MYSQL_SERVER_PUBLIC_KEY, ZSTR_VAL(public_key))) {
					pdo_mysql_error(dbh);
					crex_string_release_ex(public_key, 0);
					goto cleanup;
				}
				crex_string_release_ex(public_key, 0);
			}
...
#ifdef PDO_USE_MYSQLND
		{
			crex_long ssl_verify_cert = pdo_attr_lval(driver_options,
