...
#ifndef CRX_PDO_MYSQL_INT_H
#define CRX_PDO_MYSQL_INT_H
...
#if defined(PDO_USE_MYSQLND) && CRX_DEBUG && !defined(CRX_WIN32)
...
CREX_BEGIN_MODULE_GLOBALS(pdo_mysql)
#ifndef CRX_WIN32
	char          *default_socket;
#endif
#if PDO_DBG_ENABLED
...
#endif
#if defined(CRX_WIN32) && !PDO_DBG_ENABLED
	/* dummy member so we get at least one member in the struct
	 * and avoids build errors.
	 */
	void *dummymemmber;
#endif
CREX_END_MODULE_GLOBALS(pdo_mysql)

CREX_EXTERN_MODULE_GLOBALS(pdo_mysql)
#define PDO_MYSQL_G(v) CREX_MODULE_GLOBALS_ACCESSOR(pdo_mysql, v)
...
#ifndef PDO_USE_MYSQLND
	my_bool					*in_null;
	crex_ulong			*in_length;
	PDO_MYSQL_PARAM_BIND	*bound_result;
	my_bool					*out_null;
	crex_ulong				*out_length;
