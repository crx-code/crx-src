...
static HRESULT dotnet_bind_runtime(LPVOID FAR *ppv)
{
	...
	if (version == NULL || *version == '\0') {
		...
		oleversion = crx_com_string_to_olestring(version, strlen(version), COMG(code_page));
	}
...
CRX_METHOD(dotnet, __main)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "ss|l",
	...
	if (!cp_it) {
		crx_com_throw_exception(E_INVALIDARG, "Could not create .Net object - invalid codepage!");
		RETURN_THROWS();
	}
	obj->code_page = (int)cp_it->id;

	oletype = crx_com_string_to_olestring(datatype_name, datatype_name_len, obj->code_page);
	oleassembly = crx_com_string_to_olestring(assembly_name, assembly_name_len, obj->code_page);
