 ...
 * Implements: IStream* wrapper for CRX streams.
...
/* {{{ expose crx_stream as a COM IStream */
...
static void istream_destructor(crx_istream *stm);

static void istream_dtor(crex_resource *rsrc)
{
	crx_istream *stm = (crx_istream *)rsrc->ptr;
	istream_destructor(stm);
}

#define FETCH_STM()	\
	crx_istream *stm = (crx_istream*)This; \
...
static HRESULT STDMETHODCALLTYPE stm_read(IStream *This, void *pv, ULONG cb, ULONG *pcbRead)
{
	...
	nread = (ULONG)crx_stream_read(stm->stream, pv, cb);
...
static HRESULT STDMETHODCALLTYPE stm_write(IStream *This, void const *pv, ULONG cb, ULONG *pcbWritten)
{
	...
	nwrote = crx_stream_write(stm->stream, pv, cb);
		...
		DWORD dwOrigin, ULARGE_INTEGER *plibNewPosition)
{
	...
	ret = crx_stream_seek(stm->stream, offset, whence);
...
static HRESULT STDMETHODCALLTYPE stm_set_size(IStream *This, ULARGE_INTEGER libNewSize)
{
	...
	if (crx_stream_truncate_supported(stm->stream)) {
		int ret = crx_stream_truncate_set_size(stm->stream, (size_t)libNewSize.QuadPart);
...
static struct IStreamVtbl crx_istream_vtbl = {
	stm_queryinterface,
	stm_addref,
	stm_release,
	stm_read,
...
static void istream_destructor(crx_istream *stm)
{
	...
	crex_list_delete(stm->stream->res);

	CoTaskMemFree(stm);
}
/* }}} */

CRX_COM_DOTNET_API IStream *crx_com_wrapper_export_stream(crx_stream *stream)
{
	crx_istream *stm = (crx_istream*)CoTaskMemAlloc(sizeof(*stm));
	...
	stm->lpVtbl = &crx_istream_vtbl;
	stm->refcount = 1;
	stm->stream = stream;

	GC_ADDREF(stream->res);
	stm->res = crex_register_resource(stm, le_istream);

	return (IStream*)stm;
}

#define CPH_METHOD(fname)		CRX_METHOD(COMPersistHelper, fname)

#define CPH_FETCH()				crx_com_persist_helper *helper = (crx_com_persist_helper*)C_OBJ_P(getThis());

#define CPH_NO_OBJ()			if (helper->unk == NULL) { crx_com_throw_exception(E_INVALIDARG, "No COM object is associated with this helper instance"); RETURN_THROWS(); }
...
typedef struct {
	...
	IPersistFile		*ipf;
} crx_com_persist_helper;

static crex_object_handlers helper_handlers;
static crex_class_entry *helper_ce;

static inline HRESULT get_persist_stream(crx_com_persist_helper *helper)
{
...
static inline HRESULT get_persist_stream_init(crx_com_persist_helper *helper)
{
...
static inline HRESULT get_persist_file(crx_com_persist_helper *helper)
{
...
CPH_METHOD(GetCurFileName)
{
	...
	if (helper->ipf) {
		...
		if (res == S_OK) {
			crex_string *str = crx_com_olestring_to_string(olename, helper->codepage);
...
CPH_METHOD(SaveToFile)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "p!|b",
	...
	if (helper->ipf) {
		if (filename) {
			...
			olefilename = crx_com_string_to_olestring(fullpath, strlen(fullpath), helper->codepage);
			efree(fullpath);
		}
...
CPH_METHOD(LoadFromFile)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "p|l",
...
CPH_METHOD(GetMaxStreamSize)
{
	...
	if (res != S_OK) {
		...
		RETURN_LONG((crex_long)size.QuadPart);
	}
}
/* }}} */
...
CPH_METHOD(LoadFromStream)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "r", &zstm)) {
		RETURN_THROWS();
	}

	crx_stream_from_zval_no_verify(stream, zstm);

	if (stream == NULL) {
		crx_com_throw_exception(E_INVALIDARG, "expected a stream");
		RETURN_THROWS();
	}

	stm = crx_com_wrapper_export_stream(stream);
	if (stm == NULL) {
		crx_com_throw_exception(E_UNEXPECTED, "failed to wrap stream");
		RETURN_THROWS();
	}
	...
	if (helper->unk == NULL) {
		...
		if (SUCCEEDED(res)) {
			crx_com_wrap_dispatch(return_value, disp, COMG(code_page));
		}
	} else {
		res = get_persist_stream_init(helper);
		if (helper->ipsi) {
...
CPH_METHOD(__main)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "|O!",
				&zobj, crx_com_variant_class_entry)) {
		RETURN_THROWS();
	}
	...
	if (V_VT(&obj->v) != VT_DISPATCH || V_DISPATCH(&obj->v) == NULL) {
		crx_com_throw_exception(E_INVALIDARG, "parameter must represent an IDispatch COM object");
		RETURN_THROWS();
	}
...
static void helper_free_storage(crex_object *obj)
{
	crx_com_persist_helper *object = (crx_com_persist_helper*)obj;
...
static crex_object* helper_clone(crex_object *obj)
{
	crx_com_persist_helper *clone, *object = (crx_com_persist_helper*) obj;

	clone = emalloc(sizeof(*object));
	memcpy(clone, object, sizeof(*object));

	crex_object_std_init(&clone->std, object->std.ce);
	...
	return (crex_object*)clone;
}

static crex_object* helper_new(crex_class_entry *ce)
{
	crx_com_persist_helper *helper;

	helper = emalloc(sizeof(*helper));
	memset(helper, 0, sizeof(*helper));

	crex_object_std_init(&helper->std, helper_ce);
...
void crx_com_persist_minit(INIT_FUNC_ARGS)
{
	...
	le_istream = crex_register_list_destructors_ex(istream_dtor,
