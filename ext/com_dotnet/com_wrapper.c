...
/* This module exports a CRX object as a COM object by wrapping it
 * using IDispatchEx */
...
static void disp_destructor(crx_dispatchex *disp);

static void dispatch_dtor(crex_resource *rsrc)
{
	crx_dispatchex *disp = (crx_dispatchex *)rsrc->ptr;
	disp_destructor(disp);
}

int crx_com_wrapper_minit(INIT_FUNC_ARGS)
{
	le_dispatch = crex_register_list_destructors_ex(dispatch_dtor,
...
#define FETCH_DISP(methname)																			\
	crx_dispatchex *disp = (crx_dispatchex*)This; 														\
	if (COMG(rshutdown_started)) {																		\
		trace(" CRX Object:%p (name:unknown) %s\n", C_OBJ(disp->object),  methname); 							\
	} else {																							\
		trace(" CRX Object:%p (name:%s) %s\n", C_OBJ(disp->object), C_OBJCE(disp->object)->name->val, methname); 	\
	}																									\
	...
	/* [size_is][out] */ DISPID *rgDispId)
{
	...
	for (i = 0; i < cNames; i++) {
		...
		name = crx_com_olestring_to_string(rgszNames[i], COMG(code_page));

		/* Lookup the name in the hash */
		if ((tmp = crex_hash_find(disp->name_to_dispid, name)) == NULL) {
			ret = DISP_E_UNKNOWNNAME;
			rgDispId[i] = 0;
		} else {
			rgDispId[i] = (DISPID)C_LVAL_P(tmp);
		}

		crex_string_release_ex(name, /* persistent */ false);
	...
	/* [out] */ DISPID *pid)
{
	...
	name = crx_com_olestring_to_string(bstrName, COMG(code_page));
	...
	/* [unique][in] */ IServiceProvider *pspCaller)
{
	...
	if (NULL != (name = crex_hash_index_find(disp->dispid_to_name, id))) {
		/* TODO: add support for overloaded objects */

		trace("-- Invoke: %d %20s [%d] flags=%08x args=%d\n", id, C_STRVAL_P(name), C_STRLEN_P(name), wFlags, pdp->cArgs);
		...
		if (pdp->cArgs) {
			...
			for (i = 0; i < pdp->cArgs; i++) {
				...
				crx_com_wrap_variant(&params[i], arg, COMG(code_page));
			}
		}

		trace("arguments processed, prepare to do some work\n");

		/* TODO: if CRX raises an exception here, we should catch it
		 * and expose it as a COM exception */

		if (wFlags & DISPATCH_PROPERTYGET) {
			retval = crex_read_property(C_OBJCE(disp->object), C_OBJ(disp->object), C_STRVAL_P(name), C_STRLEN_P(name)+1, 1, &rv);
		} else if (wFlags & DISPATCH_PROPERTYPUT) {
			crex_update_property(C_OBJCE(disp->object), C_OBJ(disp->object), C_STRVAL_P(name), C_STRLEN_P(name), &params[0]);
			...
			crex_try {
				...
				if (SUCCESS == call_user_function(NULL, &disp->object, name,
					...
					for (i = 0; i < pdp->cArgs; i++) {
						crx_com_dotnet_object *obj = CDNO_FETCH(&params[i]);
						...
						if ((V_VT(dstvar) & VT_BYREF) && obj->modified ) {
							...
							crx_com_copy_variant(dstvar, srcvar);
						}
					}
				} else {
		...
		if (retval) {
			if (pvarRes) {
				VariantInit(pvarRes);
				crx_com_variant_from_zval(pvarRes, retval, COMG(code_page));
			}
	...
	/* [out] */ BSTR *pbstrName)
{
	...
	if (NULL != (name = crex_hash_index_find(disp->dispid_to_name, id))) {
		OLECHAR *olestr = crx_com_string_to_olestring(C_STRVAL_P(name), C_STRLEN_P(name), COMG(code_page));
...
static HRESULT STDMETHODCALLTYPE disp_getnextdispid(
	...
	/* [out] */ DISPID *pid)
{
	crex_ulong next = id+1;
	FETCH_DISP("GetNextDispID");

	while(!crex_hash_index_exists(disp->dispid_to_name, next))
		next++;

	if (crex_hash_index_exists(disp->dispid_to_name, next)) {
		*pid = next;
		return S_OK;
	}
	return S_FALSE;
...
static struct IDispatchExVtbl crx_dispatch_vtbl = {
	disp_queryinterface,
	disp_addref,
	disp_release,
 ...
 * dispatch ids */
static void generate_dispids(crx_dispatchex *disp)
{
	HashPosition pos;
	...
	if (disp->dispid_to_name == NULL) {
		...
		crex_hash_init(disp->name_to_dispid, 0, NULL, ZVAL_PTR_DTOR, 0);
		crex_hash_init(disp->dispid_to_name, 0, NULL, ZVAL_PTR_DTOR, 0);
	}

	/* properties */
	if (C_OBJPROP(disp->object)) {
		crex_hash_internal_pointer_reset_ex(C_OBJPROP(disp->object), &pos);
		while (HASH_KEY_NON_EXISTENT != (keytype =
				crex_hash_get_current_key_ex(C_OBJPROP(disp->object), &name,
				&pid, &pos))) {
			char namebuf[32];
			if (keytype == HASH_KEY_IS_LONG) {
				snprintf(namebuf, sizeof(namebuf), CREX_ULONG_FMT, pid);
				name = crex_string_init(namebuf, strlen(namebuf), 0);
			} else {
				crex_string_addref(name);
			}

			crex_hash_move_forward_ex(C_OBJPROP(disp->object), &pos);
			...
			pid = crex_hash_next_free_element(disp->dispid_to_name);
			crex_hash_index_update(disp->dispid_to_name, pid, &tmp2);

			ZVAL_LONG(&tmp2, pid);
			crex_hash_update(disp->name_to_dispid, name, &tmp2);
	...
	/* functions */
	if (C_OBJCE(disp->object)) {
		crex_hash_internal_pointer_reset_ex(&C_OBJCE(disp->object)->function_table, &pos);
		while (HASH_KEY_NON_EXISTENT != (keytype =
				crex_hash_get_current_key_ex(&C_OBJCE(disp->object)->function_table,
			 	&name, &pid, &pos))) {
			...
			crex_hash_move_forward_ex(&C_OBJCE(disp->object)->function_table, &pos);
...
static crx_dispatchex *disp_constructor(zval *object)
{
	crx_dispatchex *disp = (crx_dispatchex*)CoTaskMemAlloc(sizeof(crx_dispatchex));
	zval *tmp;

	trace("constructing a COM wrapper for CRX object %p (%s)\n", object, C_OBJCE_P(object)->name);

	if (disp == NULL)
		return NULL;

	memset(disp, 0, sizeof(crx_dispatchex));

	disp->engine_thread = GetCurrentThreadId();
	disp->lpVtbl = &crx_dispatch_vtbl;
	disp->refcount = 1;
	...
	tmp = crex_list_insert(disp, le_dispatch);
	disp->res = C_RES_P(tmp);
...
static void disp_destructor(crx_dispatchex *disp)
{
	...
	if (COMG(rshutdown_started)) {
		trace("destroying COM wrapper for CRX object %p (name:unknown)\n", C_OBJ(disp->object));
	} else {
		trace("destroying COM wrapper for CRX object %p (name:%s)\n", C_OBJ(disp->object), C_OBJCE(disp->object)->name->val);
	}
	...
	crex_hash_destroy(disp->dispid_to_name);
	crex_hash_destroy(disp->name_to_dispid);
...
CRX_COM_DOTNET_API IDispatch *crx_com_wrapper_export_as_sink(zval *val, GUID *sinkid,
	   HashTable *id_to_name)
{
	crx_dispatchex *disp = disp_constructor(val);
	HashPosition pos;
	crex_string *name = NULL;
	zval tmp, *ntmp;
	int keytype;
	...
	crex_hash_internal_pointer_reset_ex(id_to_name, &pos);
	while (HASH_KEY_NON_EXISTENT != (keytype =
				crex_hash_get_current_key_ex(id_to_name, &name, &pid, &pos))) {

		if (keytype == HASH_KEY_IS_LONG) {

			ntmp = crex_hash_get_current_data_ex(id_to_name, &pos);

			ZVAL_LONG(&tmp, pid);
			crex_hash_update(disp->name_to_dispid, C_STR_P(ntmp), &tmp);
		}

		crex_hash_move_forward_ex(id_to_name, &pos);
	}
...
{
	crx_dispatchex *disp = NULL;

	if (C_TYPE_P(val) != IS_OBJECT) {
		return NULL;
	}

	if (crx_com_is_valid_object(val)) {
		/* pass back its IDispatch directly */
		crx_com_dotnet_object *obj = CDNO_FETCH(val);
