...
void crx_com_throw_exception(HRESULT code, char *message)
{
	int free_msg = 0;
	if (message == NULL) {
		message = crx_win32_error_to_msg(code);
		free_msg = 1;
	}
#if SIZEOF_CREX_LONG == 8
	crex_throw_exception(crx_com_exception_class_entry, message, (crex_long)(uint32_t)code);
#else
	crex_throw_exception(crx_com_exception_class_entry, message, (crex_long)code);
#endif
	if (free_msg) {
		crx_win32_error_msg_free(message);
	}
}
		...
		int codepage)
{
	...
	obj->ce = crx_com_variant_class_entry;
	obj->zo.ce = crx_com_variant_class_entry;
	...
	crex_object_std_init(&obj->zo, crx_com_variant_class_entry);
	ZVAL_OBJ(z, &obj->zo);
}
