...
/* create an automation SafeArray from a CRX array.
 * Only creates a single-dimensional array of variants.
 * The keys of the CRX hash MUST be numeric.  If the array
...
static void safe_array_from_zval(VARIANT *v, zval *z, int codepage)
{
	...
	crex_ulong intindex = 0;
	VARIANT *va;
	zval *item;

	/* find the largest array index, and assert that all keys are integers */
	crex_hash_internal_pointer_reset_ex(C_ARRVAL_P(z), &pos);
	for (;; crex_hash_move_forward_ex(C_ARRVAL_P(z), &pos)) {

		keytype = crex_hash_get_current_key_ex(C_ARRVAL_P(z), &strindex, &intindex, &pos);
		...
		if (HASH_KEY_IS_STRING == keytype) {
			...
			crx_error_docref(NULL, E_WARNING, "COM: max number %u of elements in safe array exceeded", UINT_MAX);
			break;
		}
	}
	...
	bound.lLbound = 0;
	bound.cElements = crex_hash_num_elements(C_ARRVAL_P(z));
	...
	crex_hash_internal_pointer_reset_ex(C_ARRVAL_P(z), &pos);
	for (;; crex_hash_move_forward_ex(C_ARRVAL_P(z), &pos)) {
		if (NULL == (item = crex_hash_get_current_data_ex(C_ARRVAL_P(z), &pos))) {
			break;
		}
		crex_hash_get_current_key_ex(C_ARRVAL_P(z), &strindex, &intindex, &pos);
		crx_com_variant_from_zval(&va[intindex], item, codepage);
	}
...
bogus:
	crx_error_docref(NULL, E_WARNING, "COM: converting from CRX array to VARIANT array; only arrays with numeric keys are allowed");
...
static void crx_com_variant_from_zval_ex(VARIANT *v, zval *z, int codepage, VARTYPE vt)
{
	...
	switch (ztype) {
		...
		case IS_OBJECT:
			if (crx_com_is_valid_object(z)) {
				...
				/* export the CRX object using our COM wrapper */
				V_VT(v) = VT_DISPATCH;
				V_DISPATCH(v) = crx_com_wrapper_export(z);
			}
			break;
		...
		case IS_STRING:
			V_VT(v) = VT_BSTR;
			V_BSTR(v) = crx_com_string_to_bstr(C_STR_P(z), codepage);
			break;
...
{
	crx_com_variant_from_zval_ex(v, z, codepage, VT_EMPTY);
}
...
CRX_COM_DOTNET_API crex_result crx_com_zval_from_variant(zval *z, VARIANT *v, int codepage)
{
	...
	switch (V_VT(v)) {
		...
		case VT_VOID:
			...
			break;
		case VT_UI1:
			ZVAL_LONG(z, (crex_long)V_UI1(v));
			break;
		case VT_I1:
			ZVAL_LONG(z, (crex_long)V_I1(v));
			break;
		case VT_UI2:
			ZVAL_LONG(z, (crex_long)V_UI2(v));
			break;
		case VT_I2:
			ZVAL_LONG(z, (crex_long)V_I2(v));
			break;
		...
		case VT_UI8:
			ZVAL_LONG(z, (crex_long)V_UI8(v));
			break;
		case VT_I8:
			ZVAL_LONG(z, (crex_long)V_I8(v));
			break;
#endif
		case VT_INT:
			ZVAL_LONG(z, V_INT(v));
		...
		case VT_UINT: /* TODO: promote to double if large? */
			ZVAL_LONG(z, (crex_long)V_UINT(v));
			break;
		case VT_R4:
		...
		case VT_BSTR:
			olestring = V_BSTR(v);
			if (olestring) {
				crex_string *str = crx_com_bstr_to_string(olestring, codepage);
				ZVAL_STR(z, str);
				olestring = NULL;
			}
			break;
		case VT_UNKNOWN:
			if (V_UNKNOWN(v) != NULL) {
				...
				if (SUCCEEDED(IUnknown_QueryInterface(V_UNKNOWN(v), &IID_IDispatch, &disp))) {
					crx_com_wrap_dispatch(z, disp, codepage);
		...
		case VT_DISPATCH:
			if (V_DISPATCH(v) != NULL) {
				crx_com_wrap_dispatch(z, V_DISPATCH(v), codepage);
			}
			break;

		case VT_VARIANT:
			/* points to another variant */
			return crx_com_zval_from_variant(z, V_VARIANTREF(v), codepage);

		default:
			crx_com_wrap_variant(z, v, codepage);
	}
	...
	if (ret == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "variant->zval: conversion from 0x%x ret=%d", V_VT(v), ret);
	}
...
CRX_COM_DOTNET_API crex_result crx_com_copy_variant(VARIANT *dstvar, VARIANT *srcvar)
{
	crex_result ret = SUCCESS;
	...
	case VT_VARIANT:
		return crx_com_copy_variant(V_VARIANTREF(dstvar), srcvar);

	default:
		crx_error_docref(NULL, E_WARNING, "variant->__main: failed to copy from 0x%x to 0x%x", V_VT(dstvar), V_VT(srcvar));
		ret = FAILURE;
	}
	return ret;
}
...
{
	/* VARTYPE == unsigned short */ crex_long vt = VT_EMPTY;
	crex_long codepage = CP_ACP;
	zval *object = getThis();
	crx_com_dotnet_object *obj;
	zval *zvalue = NULL;
	HRESULT res;
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(),
	...
	if (zvalue) {
		crx_com_variant_from_zval_ex(&obj->v, zvalue, obj->code_page, vt);
	}

	/* Only perform conversion if variant not already of type passed */
	if ((CREX_NUM_ARGS() >= 2) && (vt != V_VT(&obj->v))) {
...
CRX_FUNCTION(variant_set)
{
	zval *zobj, *zvalue = NULL;
	crx_com_dotnet_object *obj;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(),
			"Oz!", &zobj, crx_com_variant_class_entry, &zvalue)) {
		RETURN_THROWS();
	}
	...
	if (obj->sink_dispatch) {
		crx_com_object_enable_event_sink(obj, /* enable */ false);
	...
	crx_com_variant_from_zval(&obj->v, zvalue, obj->code_page);
...
static void variant_binary_operation(enum variant_binary_opcode op, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	...
	if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "OO", &zleft, crx_com_variant_class_entry,
			&zright, crx_com_variant_class_entry)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
	} else if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "Oz!", &zleft, crx_com_variant_class_entry,
			&zright)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		vright = &right_val;
		crx_com_variant_from_zval(vright, zright, codepage);
	} else if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "z!O", &zleft, &zright, crx_com_variant_class_entry)) {
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
		vleft = &left_val;
		crx_com_variant_from_zval(vleft, zleft, codepage);
	} else if (SUCCESS == crex_parse_parameters(CREX_NUM_ARGS(),
			"z!z!", &zleft, &zright)) {
	...
	if (SUCCEEDED(result)) {
		crx_com_wrap_variant(return_value, &vres, codepage);
	} else {
		crx_com_throw_exception(result, NULL);
	}
...
static void variant_unary_operation(enum variant_unary_opcode op, INTERNAL_FUNCTION_PARAMETERS) /* {{{ */
{
	...
	if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "O", &zleft, crx_com_variant_class_entry)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
	} else if (SUCCESS == crex_parse_parameters(CREX_NUM_ARGS(),
			"z!", &zleft)) {
...
CRX_FUNCTION(variant_round)
{
	...
	crx_com_dotnet_object *obj;
	int codepage = CP_ACP;
	crex_long decimals = 0;

	VariantInit(&left_val);
	VariantInit(&vres);

	if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "Ol", &zleft, crx_com_variant_class_entry, &decimals)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
	} else if (SUCCESS == crex_parse_parameters(CREX_NUM_ARGS(),
			"z!l", &zleft, &decimals)) {
...
CRX_FUNCTION(variant_cmp)
{
	...
	crx_com_dotnet_object *obj;
	int codepage = CP_ACP;
	crex_long lcid = LOCALE_SYSTEM_DEFAULT;
	crex_long flags = 0;
	...
	if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "OO|ll", &zleft, crx_com_variant_class_entry,
			&zright, crx_com_variant_class_entry, &lcid, &flags)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
	} else if (SUCCESS == crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET,
			CREX_NUM_ARGS(), "Oz!|ll", &zleft, crx_com_variant_class_entry,
			&zright, &lcid, &flags)) {
		obj = CDNO_FETCH(zleft);
		vleft = &obj->v;
		vright = &right_val;
		crx_com_variant_from_zval(vright, zright, codepage);
			...
			CREX_NUM_ARGS(), "z!O|ll", &zleft, &zright, crx_com_variant_class_entry,
			&lcid, &flags)) {
		obj = CDNO_FETCH(zright);
		vright = &obj->v;
		vleft = &left_val;
...
CRX_FUNCTION(variant_date_to_timestamp)
{
	VARIANT vres;
	zval *zleft = NULL;
	crx_com_dotnet_object *obj;
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(),
		"O", &zleft, crx_com_variant_class_entry)) {
		RETURN_THROWS();
	}
	obj = CDNO_FETCH(zleft);
...
CRX_FUNCTION(variant_date_from_timestamp)
{
	crex_long timestamp;
	time_t ttstamp;
	SYSTEMTIME systime;
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "l",
			&timestamp)) {
		RETURN_THROWS();
	}
	...
	if (!tmv) {
		crex_argument_value_error(1, "must not go past 23:59:59, December 31, 3000, UTC");
		RETURN_THROWS();
	}
	...
	crx_com_wrap_variant(return_value, &res, CP_ACP);
...
CRX_FUNCTION(variant_get_type)
{
	zval *zobj;
	crx_com_dotnet_object *obj;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(),
		"O", &zobj, crx_com_variant_class_entry)) {
		RETURN_THROWS();
	}
	obj = CDNO_FETCH(zobj);
...
CRX_FUNCTION(variant_set_type)
{
	zval *zobj;
	crx_com_dotnet_object *obj;
	/* VARTYPE == unsigned short */ crex_long vt;
	VARIANT vtmp;
	HRESULT res;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(),
		"Ol", &zobj, crx_com_variant_class_entry, &vt)) {
		RETURN_THROWS();
	}
	obj = CDNO_FETCH(zobj);
...
CRX_FUNCTION(variant_cast)
{
	...
	if (SUCCEEDED(res)) {
		crx_com_wrap_variant(return_value, &vres, obj->code_page);
	} else {
		char *werr, *msg;
