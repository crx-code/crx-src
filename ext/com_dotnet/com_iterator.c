...
static void com_iter_dtor(crex_object_iterator *iter)
{
	struct crx_com_iterator *I = (struct crx_com_iterator*)C_PTR(iter->data);
...
static int com_iter_valid(crex_object_iterator *iter)
{
	struct crx_com_iterator *I = (struct crx_com_iterator*)C_PTR(iter->data);

	if (C_TYPE(I->zdata) != IS_UNDEF) {
		return SUCCESS;
	}
...
static zval* com_iter_get_data(crex_object_iterator *iter)
{
	struct crx_com_iterator *I = (struct crx_com_iterator*)C_PTR(iter->data);
...
static void com_iter_get_key(crex_object_iterator *iter, zval *key)
{
	struct crx_com_iterator *I = (struct crx_com_iterator*)C_PTR(iter->data);
...
static void com_iter_move_forwards(crex_object_iterator *iter)
{
	struct crx_com_iterator *I = (struct crx_com_iterator*)C_PTR(iter->data);
	...
	if (I->ev) {
		...
		I->key++;
		if (!crx_com_safearray_get_elem(&I->safe_array, &I->v, (LONG)I->key)) {
			I->key = (crex_ulong)-1;
			return;
		}
	}
	...
	ZVAL_NULL(&ptr);
	crx_com_zval_from_variant(&ptr, &I->v, I->code_page);
	/* crx_com_wrap_variant(ptr, &I->v, I->code_page); */
	ZVAL_COPY_VALUE(&I->zdata, &ptr);
}


static const crex_object_iterator_funcs com_iter_funcs = {
	com_iter_dtor,
	com_iter_valid,
	com_iter_get_data,
	com_iter_get_key,
...
crex_object_iterator *crx_com_iter_get(crex_class_entry *ce, zval *object, int by_ref)
{
	...
	if (V_VT(&obj->v) != VT_DISPATCH && !V_ISARRAY(&obj->v)) {
		...
		crx_error_docref(NULL, E_WARNING, "Variant is not an object or array VT=%d", V_VT(&obj->v));
		return NULL;
	}
	...
	I = (struct crx_com_iterator*)ecalloc(1, sizeof(*I));
	...
	if (V_ISARRAY(&obj->v)) {
		...
		/* same semantics as foreach on a CRX array;
		...
		if (I->sa_max >= bound && crx_com_safearray_get_elem(&I->safe_array, &I->v, bound)) {
			I->key = bound;
			ZVAL_NULL(&ptr);
			crx_com_zval_from_variant(&ptr, &I->v, I->code_page);
			ZVAL_COPY_VALUE(&I->zdata, &ptr);
