...
CRX_METHOD(com, __main)
{
	...
	crex_string *user_name = NULL, *password = NULL, *domain_name = NULL;
	OLECHAR *moniker;
	CLSID clsid;
	...
	COAUTHINFO		authinfo = {
		RPC_C_AUTHN_WINNT, RPC_C_AUTHC_NONE, NULL,
		RPC_C_AUTHN_LEVEL_DEFAULT, RPC_C_IMP_LEVEL_IMPERSONATE,
		&authid, EOAC_NONE
	};
	crex_long cp = GetACP();
	const struct crx_win32_cp *cp_it;

	CREX_PARSE_PARAMETERS_START(1, 4)
		C_PARAM_STRING(module_name, module_name_len)
		C_PARAM_OPTIONAL
		C_PARAM_ARRAY_HT_OR_STR_OR_NULL(server_params, server_name)
		C_PARAM_LONG(cp)
		C_PARAM_STRING(typelib_name, typelib_name_len)
	CREX_PARSE_PARAMETERS_END();

	crx_com_initialize();
	obj = CDNO_FETCH(object);

	cp_it = crx_win32_cp_get_by_id((DWORD)cp);
	if (!cp_it) {
		crx_com_throw_exception(E_INVALIDARG, "Could not create COM object - invalid codepage!");
		RETURN_THROWS();
	}
	...
	if (server_name) {
		...
		if (NULL != (tmp = crex_hash_str_find(server_params,
	...
	if (server_name && !COMG(allow_dcom)) {
		crx_com_throw_exception(E_ERROR, "DCOM has been disabled by your administrator [com.allow_dcom=0]");
		RETURN_THROWS();
	}

	moniker = crx_com_string_to_olestring(module_name, module_name_len, obj->code_page);
	...
	if (server_name) {
		info.dwReserved1 = 0;
		info.dwReserved2 = 0;
		info.pwszName = crx_com_string_to_olestring(ZSTR_VAL(server_name), ZSTR_LEN(server_name), obj->code_page);
	...
	if (server_name) {
		...
		if (server_params) crex_string_release(server_name);
	}
	if (user_name) crex_string_release(user_name);
	if (password) crex_string_release(password);
	if (domain_name) crex_string_release(domain_name);
	...
	if (FAILED(res)) {
		...
		werr = crx_win32_error_to_msg(res);
		spprintf(&msg, 0, "Failed to create COM object `%s': %s", module_name, werr);
		crx_win32_error_msg_free(werr);

		crx_com_throw_exception(res, msg);
		efree(msg);
		RETURN_THROWS();
	}
	...
	if (FAILED(IDispatch_GetTypeInfo(V_DISPATCH(&obj->v), 0, MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), &obj->typeinfo)) && typelib_name) {
		...
		TL = crx_com_load_typelib_via_cache(typelib_name, obj->code_page);

		if (TL) {
			if (COMG(autoreg_on)) {
				crx_com_import_typelib(TL, 0, obj->code_page);
			}
		...
		if (SUCCEEDED(ITypeInfo_GetContainingTypeLib(obj->typeinfo, &TL, &idx))) {
			...
			if (SUCCEEDED(ITypeLib_GetDocumentation(TL, -1, &name, NULL, NULL, NULL))) {
				crex_string *typelib_str = crx_com_olestring_to_string(name, obj->code_page);

				if (NULL != crx_com_cache_typelib(TL, ZSTR_VAL(typelib_str), ZSTR_LEN(typelib_str))) {
					crx_com_import_typelib(TL, 0, obj->code_page);

					/* add a reference for the hash */
					ITypeLib_AddRef(TL);
				}
				crex_string_release_ex(typelib_str, /* persistent */ false);
			} else {
				/* try it anyway */
				crx_com_import_typelib(TL, 0, obj->code_page);
			}
...
CRX_FUNCTION(com_get_active_object)
{
	CLSID clsid;
	char *module_name;
	...
	crx_com_initialize();
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "s|l!",
	...
	module = crx_com_string_to_olestring(module_name, module_name_len, (int)code_page);
	...
	if (FAILED(res)) {
		...
		if (FAILED(res)) {
			...
			if (FAILED(res)) {
				...
				crx_com_wrap_dispatch(return_value, obj, (int)code_page);
			}
		}
	}
	...
	if (!silent && FAILED(hr)) {
		...
		switch (hr) {
			case DISP_E_EXCEPTION: {
				crex_string *source = NULL, *desc_str = NULL;
				if (e.bstrSource) {
					source = crx_com_olestring_to_string(e.bstrSource, obj->code_page);
					SysFreeString(e.bstrSource);
				}
				if (e.bstrDescription) {
					desc_str = crx_com_olestring_to_string(e.bstrDescription, obj->code_page);
				...
				if (desc_str) {
					crex_string_release_ex(desc_str, /* persistent */ false);
				}
				if (source) {
					crex_string_release_ex(source, /* persistent */ false);
				}
				if (e.bstrHelpFile) {
			...
			case DISP_E_TYPEMISMATCH:
				desc = crx_win32_error_to_msg(hr);
				spprintf(&msg, 0, "Parameter %d: %s", arg_err, desc);
				crx_win32_error_msg_free(desc);
				break;
		...
		if (msg) {
			crx_com_throw_exception(hr, msg);
			efree(msg);
		}
	}
		...
		DISPID *dispid)
{
	...
	if (obj->id_of_name_cache && NULL != (tmp = crex_hash_find(obj->id_of_name_cache, name))) {
		*dispid = (DISPID)C_LVAL_P(tmp);
		return S_OK;
	}

	olename = crx_com_string_to_olestring(ZSTR_VAL(name), ZSTR_LEN(name), obj->code_page);
	...
	if (SUCCEEDED(hr)) {
		...
		if (!obj->id_of_name_cache) {
			...
			crex_hash_init(obj->id_of_name_cache, 2, NULL, NULL, 0);
		}
		ZVAL_LONG(&tmp, *dispid);
		crex_hash_update(obj->id_of_name_cache, name, &tmp);
	}
		...
		WORD flags,	VARIANT *v, int nargs, zval *args)
{
	...
	hr = crx_com_get_id_of_name(obj, f->function_name, &dispid);

	if (FAILED(hr)) {
		char *msg = NULL;
		char *winerr = crx_win32_error_to_msg(hr);
		spprintf(&msg, 0, "Unable to lookup `%s': %s", f->function_name->val, winerr);
		crx_win32_error_msg_free(winerr);
		crx_com_throw_exception(hr, msg);
		efree(msg);
		return FAILURE;
	}
	...
	if (f->arg_info) {
		for (i = 0; i < nargs; i++) {
			if (CREX_ARG_SEND_MODE(&f->arg_info[nargs - i - 1])) {
				byref_count++;
			}
		}
	}
	...
	if (byref_count) {
		...
		for (j = 0, i = 0; i < nargs; i++) {
			if (CREX_ARG_SEND_MODE(&f->arg_info[nargs - i - 1])) {
				...
				crx_com_variant_from_zval(&byref_vals[j], &args[nargs - i - 1], obj->code_page);
				...
				crx_com_variant_from_zval(&vargs[i], &args[nargs - i - 1], obj->code_page);
			}
		}
	...
	hr = crx_com_invoke_helper(obj, dispid, flags, &disp_params, v, 0, 0);
		...
		WORD flags,	VARIANT *v, int nargs, zval *args, bool silent, bool allow_noarg)
{
	...
	/* this will create an exception if needed */
	hr = crx_com_invoke_helper(obj, dispid, flags, &disp_params, v, silent, allow_noarg);
		...
		WORD flags,	VARIANT *v, int nargs, zval *args, bool allow_noarg)
{
	...
	hr = crx_com_get_id_of_name(obj, name, &dispid);
	...
	return crx_com_do_invoke_by_id(obj, dispid, flags, v, nargs, args, 0, allow_noarg);
}

/* {{{ Generate a globally unique identifier (GUID) */
CRX_FUNCTION(com_create_guid)
{
	GUID retval;
	OLECHAR *guid_string;
	...
	if (CoCreateGuid(&retval) == S_OK && StringFromCLSID(&retval, &guid_string) == S_OK) {
		RETVAL_STR(crx_com_olestring_to_string(guid_string, CP_ACP));
...
/* {{{ Connect events from a COM object to a CRX object */
CRX_FUNCTION(com_event_sink)
{
	zval *object, *sinkobject;
	crex_string *sink_str = NULL;
	HashTable *sink_ht = NULL;
	crex_string *type_lib_name = NULL;
	crex_string *dispatch_name = NULL;
	crx_com_dotnet_object *obj;
	ITypeInfo *typeinfo = NULL;

	CREX_PARSE_PARAMETERS_START(2, 3)
		C_PARAM_OBJECT_OF_CLASS(object, crx_com_variant_class_entry)
		C_PARAM_OBJECT(sinkobject)
		C_PARAM_OPTIONAL
		C_PARAM_ARRAY_HT_OR_STR_OR_NULL(sink_ht, sink_str)
	CREX_PARSE_PARAMETERS_END();
	...
	if (sink_ht) {
		...
		if ((tmp = crex_hash_index_find(sink_ht, 0)) != NULL && C_TYPE_P(tmp) == IS_STRING)
			type_lib_name = C_STR_P(tmp);
		if ((tmp = crex_hash_index_find(sink_ht, 1)) != NULL && C_TYPE_P(tmp) == IS_STRING)
			dispatch_name = C_STR_P(tmp);
	} else if (sink_str) {
		dispatch_name = sink_str;
	}

	typeinfo = crx_com_locate_typeinfo(type_lib_name, obj, dispatch_name, /* sink */ true);
	...
	if (typeinfo) {
		...
		if (crx_com_process_typeinfo(typeinfo, id_to_name, 0, &obj->sink_id, obj->code_page)) {

			/* Create the COM wrapper for this sink */
			obj->sink_dispatch = crx_com_wrapper_export_as_sink(sinkobject, &obj->sink_id, id_to_name);

			/* Now hook it up to the source */
			crx_com_object_enable_event_sink(obj, /* enable */ true);
			RETVAL_TRUE;
...
/* {{{ Print out a CRX class definition for a dispatchable interface */
CRX_FUNCTION(com_print_typeinfo)
{
	crex_object *object_zpp;
	crex_string *type_lib_name = NULL;
	crex_string *interface_name = NULL;
	bool want_sink = false;
	crx_com_dotnet_object *obj = NULL;
	ITypeInfo *typeinfo;

	CREX_PARSE_PARAMETERS_START(1, 3)
		C_PARAM_OBJ_OF_CLASS_OR_STR(object_zpp, crx_com_variant_class_entry, type_lib_name)
		C_PARAM_OPTIONAL
		C_PARAM_STR_OR_NULL(interface_name)
		C_PARAM_BOOL(want_sink)
	CREX_PARSE_PARAMETERS_END();

	crx_com_initialize();
	if (object_zpp) {
		obj = (crx_com_dotnet_object*)object_zpp;
	}

	typeinfo = crx_com_locate_typeinfo(type_lib_name, obj, interface_name, want_sink);
	if (typeinfo) {
		crx_com_process_typeinfo(typeinfo, NULL, 1, NULL, obj ? obj->code_page : COMG(code_page));
		ITypeInfo_Release(typeinfo);
		RETURN_TRUE;
	}

	crx_error_docref(NULL, E_WARNING, "Unable to find typeinfo using the parameters supplied");
	RETURN_FALSE;
}
/* }}} */

/* {{{ Process COM messages, sleeping for up to timeoutms milliseconds */
CRX_FUNCTION(com_message_pump)
{
	crex_long timeoutms = 0;
	MSG msg;
	DWORD result;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|l", &timeoutms) == FAILURE)
		RETURN_THROWS();
...
CRX_FUNCTION(com_load_typelib)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "s|b", &name, &namelen, &cs)) {
		RETURN_THROWS();
	}

	if (!cs) {
		crx_error_docref(NULL, E_WARNING, "com_load_typelib(): Argument #2 ($case_insensitive) is ignored since declaration of case-insensitive constants is no longer supported");
	}

	RETVAL_FALSE;

	crx_com_initialize();
	pTL = crx_com_load_typelib_via_cache(name, codepage);
	if (pTL) {
		if (crx_com_import_typelib(pTL, 0, codepage) == SUCCESS) {
			RETVAL_TRUE;
		}
