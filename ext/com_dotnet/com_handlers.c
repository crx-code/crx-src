...
static zval *com_property_read(crex_object *object, crex_string *member, int type, void **cache_slot, zval *rv)
{
	...
	obj = (crx_com_dotnet_object*) object;

	if (V_VT(&obj->v) == VT_DISPATCH) {
		VariantInit(&v);

		res = crx_com_do_invoke(obj, member, DISPATCH_METHOD|DISPATCH_PROPERTYGET,
			&v, 0, NULL, 1);

		if (res == SUCCESS) {
			crx_com_zval_from_variant(rv, &v, obj->code_page);
			VariantClear(&v);
		} else if (res == DISP_E_BADPARAMCOUNT) {
			zval zv;

			ZVAL_STR(&zv, member);
			crx_com_saproxy_create(object, rv, &zv);
		}
	} else {
		crx_com_throw_exception(E_INVALIDARG, "this variant has no properties");
	}

	return rv;
}

static zval *com_property_write(crex_object *object, crex_string *member, zval *value, void **cache_slot)
{
	crx_com_dotnet_object *obj;
	VARIANT v;
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		if (SUCCESS == crx_com_do_invoke(obj, member,
...
static zval *com_read_dimension(crex_object *object, zval *offset, int type, zval *rv)
{
	crx_com_dotnet_object *obj;
	VARIANT v;
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		if (SUCCESS == crx_com_do_invoke_by_id(obj, DISPID_VALUE,
		...
		if (SafeArrayGetDim(V_ARRAY(&obj->v)) == 1) {
			if (crx_com_safearray_get_elem(&obj->v, &v, (LONG)C_LVAL_P(offset))) {
				crx_com_wrap_variant(rv, &v, obj->code_page);
				VariantClear(&v);
			}
		} else {
		...
		crx_com_throw_exception(E_INVALIDARG, "this variant is not an array type");
	}

	return rv;
}

static void com_write_dimension(crex_object *object, zval *offset, zval *value)
{
	crx_com_dotnet_object *obj;
	zval args[2];
	VARIANT v;
	...
	if (offset == NULL) {
		crx_com_throw_exception(DISP_E_BADINDEX, "appending to variants is not supported");
		return;
	}
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		...
		if (SafeArrayGetDim(V_ARRAY(&obj->v)) == 1) {
			...
			convert_to_long(offset);
			indices = (LONG)C_LVAL_P(offset);

			VariantInit(&v);
			crx_com_variant_from_zval(&v, value, obj->code_page);
			...
			crx_com_throw_exception(DISP_E_BADINDEX, "this variant has multiple dimensions; you can't set a new value without specifying *all* dimensions");
		}
...
static zval *com_get_property_ptr_ptr(crex_object *object, crex_string *member, int type, void **cache_slot)
{
	return NULL;
}

static int com_property_exists(crex_object *object, crex_string *member, int check_empty, void **cache_slot)
{
	DISPID dispid;
	crx_com_dotnet_object *obj;
	...
	if (V_VT(&obj->v) == VT_DISPATCH) {
		if (SUCCEEDED(crx_com_get_id_of_name(obj, member, &dispid))) {
...
static int com_dimension_exists(crex_object *object, zval *member, int check_empty)
{
	/* TODO Add support */
...
static void com_property_delete(crex_object *object, crex_string *member, void **cache_slot)
{
	crex_throw_error(NULL, "Cannot delete properties from a COM object");
}

static void com_dimension_delete(crex_object *object, zval *offset)
{
	crex_throw_error(NULL, "Cannot delete dimension from a COM object");
}

static HashTable *com_properties_get(crex_object *object)
{
...
static HashTable *com_get_gc(crex_object *object, zval **table, int *n)
{
	*table = NULL;
	*n = 0;
	return NULL;
...
{
	crex_internal_function *f = (crex_internal_function*)C_PTR_P(zv);

	crex_string_release_ex(f->function_name, 0);
	if (f->arg_info) {
		efree(f->arg_info);
	}
	efree(f);
...
static CRX_FUNCTION(com_method_handler)
{
	zval *object = getThis();
	crex_string *method = EX(func)->common.function_name;
	zval *args = NULL;
	crx_com_dotnet_object *obj = CDNO_FETCH(object);
	int nargs;
	VARIANT v;
	...
	if (nargs) {
		...
		crex_get_parameters_array_ex(nargs, args);
	}

	VariantInit(&v);

	if (SUCCESS == crx_com_do_invoke_byref(obj, (crex_internal_function*)EX(func), DISPATCH_METHOD|DISPATCH_PROPERTYGET, &v, nargs, args)) {
		ret = crx_com_zval_from_variant(return_value, &v, obj->code_page);
		VariantClear(&v);
	}
...
static crex_function *com_method_get(crex_object **object_ptr, crex_string *name, const zval *key)
{
	crex_internal_function f, *fptr = NULL;
	crex_function *func;
	DISPID dummy;
	crx_com_dotnet_object *obj = (crx_com_dotnet_object*)*object_ptr;

	if (V_VT(&obj->v) != VT_DISPATCH) {
		return NULL;
	}

	if (FAILED(crx_com_get_id_of_name(obj, name, &dummy))) {
		return NULL;
	}

	/* check cache */
	if (obj->method_cache == NULL || NULL == (fptr = crex_hash_find_ptr(obj->method_cache, name))) {
		memset(&f, 0, sizeof(crex_internal_function));
		f.type = CREX_INTERNAL_FUNCTION;
		f.num_args = 0;
		f.arg_info = NULL;
		f.scope = obj->ce;
		f.fn_flags = CREX_ACC_CALL_VIA_HANDLER;
		f.function_name = crex_string_copy(name);
		f.handler = CRX_FN(com_method_handler);
		...
		if (obj->typeinfo) {
			...
			if (SUCCEEDED(ITypeInfo_GetTypeComp(obj->typeinfo, &comp))) {
				olename = crx_com_string_to_olestring(name->val, name->len, obj->code_page);
				...
				if (SUCCEEDED(ITypeComp_Bind(comp, olename, lhash, INVOKE_FUNC, &TI, &kind, &bindptr))) {
					switch (kind) {
						case DESCKIND_FUNCDESC:
							f.arg_info = ecalloc(bindptr.lpfuncdesc->cParams, sizeof(crex_arg_info));

							for (i = 0; i < bindptr.lpfuncdesc->cParams; i++) {
								bool by_ref = (bindptr.lpfuncdesc->lprgelemdescParam[i].paramdesc.wParamFlags & PARAMFLAG_FOUT) != 0;
								f.arg_info[i].type = (crex_type) CREX_TYPE_INIT_NONE(_CREX_ARG_INFO_FLAGS(by_ref, 0, 0));
							}
		...
		crex_set_function_arg_flags((crex_function*)&f);
		/* save this method in the cache */
		if (!obj->method_cache) {
			ALLOC_HASHTABLE(obj->method_cache);
			crex_hash_init(obj->method_cache, 2, NULL, function_dtor, 0);
		}

		crex_hash_update_mem(obj->method_cache, name, &f, sizeof(f));
	}

	if (fptr) {
		/* duplicate this into a new chunk of emalloc'd memory,
		 * since the engine will efree it */
		crex_string_addref(fptr->function_name);
...
static crex_string* com_class_name_get(const crex_object *object)
{
	crx_com_dotnet_object *obj = (crx_com_dotnet_object *)object;

	return crex_string_copy(obj->ce->name);
}

/* This compares two variants for equality */
static int com_objects_compare(zval *object1, zval *object2)
{
	crx_com_dotnet_object *obja, *objb;
	int ret;
...
static crex_result com_object_cast(crex_object *readobj, zval *writeobj, int type)
{
	crx_com_dotnet_object *obj;
	VARIANT v;
	VARTYPE vt = VT_EMPTY;
	...
	obj = (crx_com_dotnet_object*) readobj;
	ZVAL_NULL(writeobj);
	VariantInit(&v);
	...
	if (SUCCEEDED(res)) {
		crx_com_zval_from_variant(writeobj, &v, obj->code_page);
	}
	...
	return crex_std_cast_object_tostring(readobj, writeobj, type);
}

static crex_result com_object_count(crex_object *object, crex_long *count)
{
	crx_com_dotnet_object *obj;
...
crex_object_handlers crx_com_object_handlers = {
	0,
	crx_com_object_free_storage,
	crex_objects_destroy_object,
	crx_com_object_clone,
	com_property_read,
	com_property_write,
	com_read_dimension,
	com_write_dimension,
...
void crx_com_object_free_storage(crex_object *object)
{
	...
	if (obj->method_cache) {
		crex_hash_destroy(obj->method_cache);
		FREE_HASHTABLE(obj->method_cache);
	}
	if (obj->id_of_name_cache) {
		crex_hash_destroy(obj->id_of_name_cache);
...
{
	crx_com_dotnet_object *cloneobj, *origobject;

	origobject = (crx_com_dotnet_object*) object;
	cloneobj = (crx_com_dotnet_object*)emalloc(sizeof(crx_com_dotnet_object));
...
crex_object* crx_com_object_new(crex_class_entry *ce)
{
	...
	crex_object_std_init(&obj->zo, ce);

	obj->typeinfo = NULL;

	return (crex_object*)obj;
}
