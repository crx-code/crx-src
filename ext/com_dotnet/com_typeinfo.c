...
static HashTable crx_com_typelibraries;

#ifdef ZTS
static MUTEX_T crx_com_typelibraries_mutex;
#endif

CRX_MINIT_FUNCTION(com_typeinfo)
{
	crex_hash_init(&crx_com_typelibraries, 0, NULL, crx_com_typelibrary_dtor, 1);

#ifdef ZTS
	crx_com_typelibraries_mutex = tsrm_mutex_alloc();
#endif
...
{
	crex_hash_destroy(&crx_com_typelibraries);

#ifdef ZTS
	tsrm_mutex_free(crx_com_typelibraries_mutex);
#endif
...
CRX_COM_DOTNET_API ITypeLib *crx_com_load_typelib(char *search_string, int codepage)
{
	...
	search_string = crx_strtok_r(search_string, ",", &strtok_buf);

	if (search_string == NULL) {
		return NULL;
	}

	major = crx_strtok_r(NULL, ",", &strtok_buf);
	minor = crx_strtok_r(NULL, ",", &strtok_buf);

	p = crx_com_string_to_olestring(search_string, strlen(search_string), codepage);
							...
							for (j = 0; j < VersionCount; j++) {
								...
								if (ERROR_SUCCESS == RegQueryValue(hsubkey, version, libname, &libnamelen)) {
									if (0 == stricmp(libname, search_string)) {
										...
										/* recurse */
										TL = crx_com_load_typelib(str, codepage);
	...
	for (i = 0; i < interfaces; i++) {
		...
		if (pTKind == TKIND_ENUM) {
			...
			for (j = 0; ; j++) {
				...
				const_name = crx_com_olestring_to_string(bstr_ids, codepage);
				SysFreeString(bstr_ids);

				/* sanity check for the case where the constant is already defined */
				crx_com_zval_from_variant(&value, pVarDesc->lpvarValue, codepage);
				if ((exists = crex_get_constant(const_name)) != NULL) {
					if (COMG(autoreg_verbose) && !compare_function(&results, &value, exists)) {
						crx_error_docref(NULL, E_WARNING, "Type library constant %s is already defined", ZSTR_VAL(const_name));
					}
					crex_string_release_ex(const_name, /* persistent */ false);
					ITypeInfo_ReleaseVarDesc(TypeInfo, pVarDesc);
					continue;
				}

				/* register the constant */
				if (C_TYPE(value) == IS_LONG) {
					CREX_CONSTANT_SET_FLAGS(&c, mode, 0);
					ZVAL_LONG(&c.value, C_LVAL(value));
					if (mode & CONST_PERSISTENT) {
						/* duplicate string in a persistent manner */
						c.name = crex_string_dup(const_name, /* persistent */ true);
						crex_string_release_ex(const_name, /* persistent */ false);
					} else {
						c.name = const_name;
					}
...
{
	ITypeLib *Lib = (ITypeLib*)C_PTR_P(pDest);
...
ITypeLib *crx_com_cache_typelib(ITypeLib* TL, char *cache_key, crex_long cache_key_len) {
	ITypeLib* result;
#ifdef ZTS
	tsrm_mutex_lock(crx_com_typelibraries_mutex);
#endif

	result = crex_hash_str_add_ptr(&crx_com_typelibraries, cache_key, cache_key_len, TL);

#ifdef ZTS
	tsrm_mutex_unlock(crx_com_typelibraries_mutex);
#endif
...
CRX_COM_DOTNET_API ITypeLib *crx_com_load_typelib_via_cache(const char *search_string, int codepage)
{
	ITypeLib *TL;
	char *name_dup;
	crex_string *key = crex_string_init(search_string, strlen(search_string), 1);

#ifdef ZTS
	tsrm_mutex_lock(crx_com_typelibraries_mutex);
#endif

	if ((TL = crex_hash_find_ptr(&crx_com_typelibraries, key)) != NULL) {
		/* add a reference for the caller */
		ITypeLib_AddRef(TL);

		goto crx_com_load_typelib_via_cache_return;
	}

	name_dup = estrndup(ZSTR_VAL(key), ZSTR_LEN(key));
	TL = crx_com_load_typelib(name_dup, codepage);
	efree(name_dup);

	if (TL) {
		if (NULL != crex_hash_add_ptr(&crx_com_typelibraries, key, TL)) {
			/* add a reference for the hash table */
			ITypeLib_AddRef(TL);
		}
	}

crx_com_load_typelib_via_cache_return:
#ifdef ZTS
	tsrm_mutex_unlock(crx_com_typelibraries_mutex);
#endif
	...
	crex_string *dispatch_name, bool sink)
{
	...
	if (obj) {
		if (dispatch_name == NULL && sink) {
			if (V_VT(&obj->v) == VT_DISPATCH) {
				...
				if (!gotguid && SUCCEEDED(IDispatch_QueryInterface(V_DISPATCH(&obj->v), &IID_IProvideClassInfo, (void**)&pci))) {
					...
					crx_error_docref(NULL, E_WARNING, "IProvideClassInfo: this code not yet written!");
		...
		typelib = crx_com_load_typelib(ZSTR_VAL(type_lib_name), CP_THREAD_ACP);
	}

	if (!gotguid && dispatch_name && typelib) {
		unsigned short cfound;
		MEMBERID memid;
		OLECHAR *olename = crx_com_string_to_olestring(ZSTR_VAL(dispatch_name), ZSTR_LEN(dispatch_name), CP_ACP);
...
static crex_string *crx_com_string_from_clsid(const CLSID *clsid, int codepage)
{
	LPOLESTR ole_clsid;
	crex_string *clsid_str;

	StringFromCLSID(clsid, &ole_clsid);
	clsid_str = crx_com_olestring_to_string(ole_clsid, codepage);
	LocalFree(ole_clsid);
...
bool crx_com_process_typeinfo(ITypeInfo *typeinfo, HashTable *id_to_name, bool printdef, GUID *guid, int codepage)
{
	...
	if (id_to_name == NULL || attr->typekind == TKIND_DISPATCH) {
		...
		if (printdef) {
			crex_string *guid_str;

			ITypeInfo_GetDocumentation(typeinfo, MEMBERID_NIL, &olename, NULL, NULL, NULL);
			ansi_name = crx_com_olestring_to_string(olename, codepage);
			SysFreeString(olename);

			guid_str = crx_com_string_from_clsid(&attr->guid, codepage);
			crx_printf("class %s { /* GUID=%s */\n", ZSTR_VAL(ansi_name), ZSTR_VAL(guid_str));
			crex_string_release_ex(guid_str, /* persistent */ false);
			crex_string_release_ex(ansi_name, /* persistent */ false);
		}

		if (id_to_name) {
			crex_hash_init(id_to_name, 0, NULL, ZVAL_PTR_DTOR, 0);
		}
			...
			if (!isprop || lastid != func->memid) {
				...
				if (printdef) {
					...
					if (isprop) {
						...
						if (olename) {
							func_desc = crx_com_olestring_to_string(olename, codepage);
							SysFreeString(olename);
							crx_printf("\t/* %s */\n", ZSTR_VAL(func_desc));
							crex_string_release_ex(func_desc, /* persistent */ false);
						}
						...
						crx_printf("\tfunction %s(\n", ZSTR_VAL(ansi_name));

						for (j = 0; j < func->cParams; j++) {
							ELEMDESC *elem = &func->lprgelemdescParam[j];

							crx_printf("\t\t/* %s [%d] ", vt_to_string(elem->tdesc.vt), elem->tdesc.vt);
							...
							if (j+1 < (int)cnames) {
								func_desc = crx_com_olestring_to_string(names[j+1], codepage);
				...
				if (id_to_name) {
					crex_string *lc_ansi_name = crex_string_tolower(ansi_name);
					ZVAL_STR(&tmp, lc_ansi_name);
					crex_hash_index_update(id_to_name, func->memid, &tmp);
				}
				crex_string_release_ex(ansi_name, /* persistent */ false);
			}
		...
		crex_throw_error(NULL, "Type kind must be dispatchable, %08x given", attr->typekind);
	}
