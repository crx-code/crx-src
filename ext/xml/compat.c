...
XML_ParserCreate_MM(const XML_Char *encoding, const XML_Memory_Handling_Suite *memsuite, const XML_Char *sep)
{
	...
	parser->parser = xmlCreatePushParserCtxt((xmlSAXHandlerPtr) &crx_xml_compat_handlers, (void *) parser, NULL, 0, NULL);
	if (parser->parser == NULL) {
		efree(parser);
		return NULL;
	}

	crx_libxml_sanitize_parse_ctxt_options(parser->parser);
