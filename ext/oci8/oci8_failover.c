...
sb4 callback_fn(void *svchp, void *envhp, void *fo_ctx, ub4 fo_type, ub4 fo_event)
{
    ...
    crx_oci_connection *connection = (crx_oci_connection*)fo_ctx;

	/* Default return value */
	sb4 returnValue = 0;

	/* Check if userspace callback function was unregistered */
	if (C_ISUNDEF(connection->taf_callback) || C_ISNULL(connection->taf_callback)) {
		return 0;
	}
	...
	if (call_user_function(NULL, NULL, &connection->taf_callback, &retval, 3, params) == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "Unable to call Oracle TAF callback function");
	}

	/* Set return value */
	if (C_TYPE(retval) == IS_LONG) {
		returnValue = (sb4) C_LVAL(retval);
	}
...
/* {{{ crx_oci_unregister_taf_callback()
...
{
	return crx_oci_register_taf_callback(connection, NULL);
}
/* }}} */

/* {{{ crx_oci_register_taf_callback()
...
int crx_oci_register_taf_callback(crx_oci_connection *connection, zval *callback)
{
	...
	/* do the registration */
	CRX_OCI_CALL_RETURN(errstatus, OCIAttrSet, (connection->server, (ub4) OCI_HTYPE_SERVER, (void *) &failover, (ub4) 0, (ub4) OCI_ATTR_FOCBK, connection->err));
