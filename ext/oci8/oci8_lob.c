...
crx_oci_descriptor *crx_oci_lob_create (crx_oci_connection *connection, crex_long type)
{
	...
	switch (type) {
		...
		case OCI_DTYPE_ROWID:
			...
			break;
		default:
			crx_error_docref(NULL, E_WARNING, "Unknown descriptor type " CREX_LONG_FMT, type);
			return NULL;
			break;
	}

	descriptor = ecalloc(1, sizeof(crx_oci_descriptor));
	descriptor->type = (ub4) type;
	descriptor->connection = connection;
	GC_ADDREF(descriptor->connection->id);

	CRX_OCI_CALL_RETURN(errstatus, OCIDescriptorAlloc, (connection->env, (dvoid*)&(descriptor->descriptor), descriptor->type, (size_t) 0, (dvoid **) 0));

	if (errstatus != OCI_SUCCESS) {
		OCI_G(errcode) = crx_oci_error(OCI_G(err), errstatus);
		CRX_OCI_HANDLE_ERROR(connection, OCI_G(errcode));
		efree(descriptor);
		return NULL;
	} else {
	...
	CRX_OCI_REGISTER_RESOURCE(descriptor, le_descriptor);

	descriptor->lob_current_position = 0;
	descriptor->lob_size = -1;				/* we should set it to -1 to know, that it's just not initialized */
	descriptor->buffering = CRX_OCI_LOB_BUFFER_DISABLED;				/* buffering is off by default */
	...
	if (descriptor->type == OCI_DTYPE_LOB || descriptor->type == OCI_DTYPE_FILE) {
		...
		if (!connection->descriptors) {
			...
			crex_hash_init(connection->descriptors, 0, NULL, crx_oci_descriptor_flush_hash_dtor, 0);
			connection->descriptor_count = 0;
		}

		descriptor->index = (connection->descriptor_count)++;
		if (connection->descriptor_count == LONG_MAX) {
			crx_error_docref(NULL, E_WARNING, "Internal descriptor counter has reached limit");
			crx_oci_connection_descriptors_free(connection);
			return NULL;
		}

		crex_hash_index_update_ptr(connection->descriptors, descriptor->index, descriptor);
	}
	return descriptor;
...
{
	crx_oci_connection *connection = descriptor->connection;
	sword errstatus;
	...
	if (descriptor->lob_size >= 0) {
		*length = descriptor->lob_size;
		...
		if (descriptor->type == OCI_DTYPE_FILE) {
			CRX_OCI_CALL_RETURN(errstatus, OCILobFileOpen, (connection->svc, connection->err, descriptor->descriptor, OCI_FILE_READONLY));
			if (errstatus != OCI_SUCCESS) {
		...
		CRX_OCI_CALL_RETURN(errstatus, OCILobGetLength, (connection->svc, connection->err, descriptor->descriptor, (ub4 *)length));
		...
		if (descriptor->type == OCI_DTYPE_FILE) {
			CRX_OCI_CALL_RETURN(errstatus, OCILobFileClose, (connection->svc, connection->err, descriptor->descriptor));
...
{
	ub4 lenp = (ub4) len;
	crx_oci_lob_ctx *ctx = (crx_oci_lob_ctx *)ctxp;
	...
	switch (piece)
	{
		...
		default: {
					crx_error_docref(NULL, E_WARNING, "Unexpected LOB piece id received (value:%d)", piece);
			*(ctx->lob_data) = NULL;
			*(ctx->lob_len) = 0;
			return OCI_ERROR;
		}
...
/* {{{ crx_oci_lob_calculate_buffer()
   Work out the size for LOB buffering */
static inline int crx_oci_lob_calculate_buffer(crx_oci_descriptor *descriptor, crex_long read_length)
{
	crx_oci_connection *connection = descriptor->connection;
	ub4 chunk_size;
	sword errstatus;
	...
	if (!descriptor->chunk_size) {
		CRX_OCI_CALL_RETURN(errstatus, OCILobGetChunkSize, (connection->svc, connection->err, descriptor->descriptor, &chunk_size));
...
int crx_oci_lob_read (crx_oci_descriptor *descriptor, crex_long read_length, crex_long initial_offset, char **data, ub4 *data_len)
{
	crx_oci_connection *connection = descriptor->connection;
	ub4 length = 0;
	int buffer_size = CRX_OCI_LOB_BUFFER_SIZE;
	crx_oci_lob_ctx ctx;
	ub1 *bufp;
	...
	*data = NULL;
	...
	if (crx_oci_lob_get_length(descriptor, &length)) {
		return 1;
	}
	...
	if (initial_offset > length) {
		crx_error_docref(NULL, E_WARNING, "Offset must be less than size of the LOB");
		return 1;
	}
	...
	if (descriptor->type == OCI_DTYPE_FILE) {
		...
		CRX_OCI_CALL_RETURN(errstatus, OCILobCharSetId, (connection->env, connection->err, descriptor->descriptor, &charset_id));
	...
	if (is_clob) {
		CRX_OCI_CALL_RETURN(errstatus, OCINlsNumericInfoGet, (connection->env, connection->err, &bytes_per_char, OCI_NLS_CHARSET_MAXBYTESZ));
	...
	*data = ecalloc(bytes_per_char, requested_len + 1);
	...
	buffer_size = crx_oci_lob_calculate_buffer(descriptor, buffer_size);	/* use chunk size */

	bufp = (ub1 *) ecalloc(1, buffer_size);
	CRX_OCI_CALL_RETURN(errstatus, OCILobRead2,
		(
			connection->svc,
			connection->err,
			...
			OCI_FIRST_PIECE,
			(dvoid *)&ctx,
			(OCICallbackLobRead2) crx_oci_lob_callback,				/* callback... */
...
int crx_oci_lob_write (crx_oci_descriptor *descriptor, ub4 offset, char *data, int data_len, ub4 *bytes_written)
{
	...
	crx_oci_connection *connection = (crx_oci_connection *) descriptor->connection;
	ub4 lob_length;
	sword errstatus;
	...
	*bytes_written = 0;
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobWrite,
			(
				connection->svc,
				connection->err,
				lob,
	...
	*bytes_written = data_len;
	...
	if (descriptor->buffering == CRX_OCI_LOB_BUFFER_ENABLED) {
		descriptor->buffering = CRX_OCI_LOB_BUFFER_USED;
	}
...
int crx_oci_lob_set_buffering (crx_oci_descriptor *descriptor, int on_off)
{
	...
	if (!on_off && descriptor->buffering == CRX_OCI_LOB_BUFFER_DISABLED) {
	...
	if (on_off) {
		CRX_OCI_CALL_RETURN(errstatus, OCILobEnableBuffering, (connection->svc, connection->err, descriptor->descriptor));
	} else {
		CRX_OCI_CALL_RETURN(errstatus, OCILobDisableBuffering, (connection->svc, connection->err, descriptor->descriptor));
	}
	...
	descriptor->buffering = on_off ? CRX_OCI_LOB_BUFFER_ENABLED : CRX_OCI_LOB_BUFFER_DISABLED;
	connection->errcode = 0; /* retain backwards compat with OCI8 1.4 */
	return 0;
}
/* }}} */

/* {{{ crx_oci_lob_get_buffering()
...
int crx_oci_lob_copy (crx_oci_descriptor *descriptor_dest, crx_oci_descriptor *descriptor_from, crex_long length)
{
	crx_oci_connection *connection = descriptor_dest->connection;
	ub4 length_dest, length_from, copy_len;
	sword errstatus;

	if (crx_oci_lob_get_length(descriptor_dest, &length_dest)) {
		return 1;
	}

	if (crx_oci_lob_get_length(descriptor_from, &length_from)) {
		return 1;
	}
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobCopy,
			(
			 connection->svc,
			 connection->err,
...
int crx_oci_lob_close (crx_oci_descriptor *descriptor)
{
	...
	if (descriptor->is_open) {
		CRX_OCI_CALL_RETURN(errstatus, OCILobClose, (connection->svc, connection->err, descriptor->descriptor));
	...
	if (crx_oci_temp_lob_close(descriptor)) {
		return 1;
	}
...
int crx_oci_temp_lob_close (crx_oci_descriptor *descriptor)
{
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobIsTemporary, (connection->env,connection->err, descriptor->descriptor, &is_temporary));
	...
	if (is_temporary) {
		CRX_OCI_CALL_RETURN(errstatus, OCILobFreeTemporary, (connection->svc, connection->err, descriptor->descriptor));
...
int crx_oci_lob_flush(crx_oci_descriptor *descriptor, crex_long flush_flag)
{
	...
	switch (flush_flag) {
		...
		case OCI_LOB_BUFFER_FREE:
			...
			break;
		default:
			crx_error_docref(NULL, E_WARNING, "Invalid flag value: " CREX_LONG_FMT, flush_flag);
			return 1;
			break;
	}
	 ...
	 * */
	if (descriptor->buffering != CRX_OCI_LOB_BUFFER_USED) {
		return 0;
	}

	CRX_OCI_CALL_RETURN(errstatus, OCILobFlushBuffer, (connection->svc, connection->err, lob, (ub4) flush_flag));
...
void crx_oci_lob_free (crx_oci_descriptor *descriptor)
{
	...
	if (descriptor->connection->descriptors) {
		if (crex_hash_num_elements(descriptor->connection->descriptors) == 0) {
			descriptor->connection->descriptor_count = 0;
		} else {
			/* delete descriptor from the hash */
			crex_hash_index_del(descriptor->connection->descriptors, descriptor->index);
			if (descriptor->index + 1 == descriptor->connection->descriptor_count) {
				 ...
				 * crex_hash_num_elements() returns 0.
				 */
	...
	if ((descriptor->type == OCI_DTYPE_FILE || descriptor->type == OCI_DTYPE_LOB) && descriptor->buffering == CRX_OCI_LOB_BUFFER_USED) {
	...
	CRX_OCI_CALL(OCIDescriptorFree, (descriptor->descriptor, descriptor->type));

	crex_list_delete(descriptor->connection->id);
	efree(descriptor);
}
/* }}} */
...
int crx_oci_lob_append (crx_oci_descriptor *descriptor_dest, crx_oci_descriptor *descriptor_from)
{
	...
	if (crx_oci_lob_get_length(descriptor_dest, &dest_len)) {
		return 1;
	}

	if (crx_oci_lob_get_length(descriptor_from, &from_len)) {
		return 1;
	}
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobAppend, (connection->svc, connection->err, lob_dest, lob_from));
...
int crx_oci_lob_truncate (crx_oci_descriptor *descriptor, crex_long new_lob_length)
{
	...
	if (new_lob_length < 0) {
		crx_error_docref(NULL, E_WARNING, "Size must be greater than or equal to 0");
		return 1;
	}

	if (new_lob_length > lob_length) {
		crx_error_docref(NULL, E_WARNING, "Size must be less than or equal to the current LOB size");
		return 1;
	}

	CRX_OCI_CALL_RETURN(errstatus, OCILobTrim, (connection->svc, connection->err, lob, (ub4) new_lob_length));
...
int crx_oci_lob_erase (crx_oci_descriptor *descriptor, crex_long offset, ub4 length, ub4 *bytes_erased)
{
	...
	*bytes_erased = 0;
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobErase, (connection->svc, connection->err, lob, (ub4 *)&length, (ub4) offset+1));
...
{
	crx_oci_connection *connection = descriptor_first->connection;
	OCILobLocator *first_lob   = descriptor_first->descriptor;
	OCILobLocator *second_lob  = descriptor_second->descriptor;
	sword errstatus;

	CRX_OCI_CALL_RETURN(errstatus, OCILobIsEqual, (connection->env, first_lob, second_lob, result));
...
int crx_oci_lob_write_tmp (crx_oci_descriptor *descriptor, crex_long type, char *data, int data_len)
{
	...
	switch (type) {
		...
		case OCI_TEMP_CLOB:
			...
			break;
		default:
			crx_error_docref(NULL, E_WARNING, "Invalid temporary lob type: " CREX_LONG_FMT, type);
			return 1;
			break;
	}
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobCreateTemporary,
			(
			 connection->svc,
			 connection->err,
			 lob,
	...
	CRX_OCI_CALL_RETURN(errstatus, OCILobOpen, (connection->svc, connection->err, lob, OCI_LOB_READWRITE));
	...
	return crx_oci_lob_write(descriptor, 0, data, data_len, &bytes_written);
}
/* }}} */
