...
#define LONG_CHECK_VALID_INT(l, arg_pos) \
	do { \
		if ((l) < INT_MIN || (l) > INT_MAX) { \
			crex_argument_value_error((arg_pos), "must be between %d and %d", INT_MIN, INT_MAX); \
			RETURN_THROWS(); \
		} \
	} while (0)
...
static void init_ancillary_registry(void)
{
	...
	crex_hash_init(&ancillary_registry.ht, 32, NULL, ancillary_registery_free_elem, 1);
...
#define PUT_ENTRY(sizev, var_size, calc, from, to, level, type) \
	...
	key.cmsg_type		= type; \
	crex_hash_str_update_mem(&ancillary_registry.ht, (char*)&key, sizeof(key), (void*)&entry, sizeof(entry))
...
static void destroy_ancillary_registry(void)
{
	if (ancillary_registry.initialized) {
		crex_hash_destroy(&ancillary_registry.ht);
...
ancillary_reg_entry *get_ancillary_reg_entry(int cmsg_level, int msg_type)
{
	...
	if ((entry = crex_hash_str_find_ptr(&ancillary_registry.ht, (char*)&key, sizeof(key))) != NULL) {
		return entry;
	} else {
		return NULL;
	}
...
CRX_FUNCTION(socket_sendmsg)
{
	...
	crex_llist		*allocations;
	struct err_s	err = {0};
	ssize_t			res;

	/* zmsg should be passed by ref */
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Oa|l", &zsocket, socket_ce, &zmsg, &flags) == FAILURE) {
		RETURN_THROWS();
	}

	LONG_CHECK_VALID_INT(flags, 3);

	crx_sock = C_SOCKET_P(zsocket);
	ENSURE_SOCKET_VALID(crx_sock);

	msghdr = from_zval_run_conversions(zmsg, crx_sock, from_zval_write_msghdr_send,
	...
	res = sendmsg(crx_sock->bsd_socket, msghdr, (int)flags);

	if (res != -1) {
		RETVAL_LONG((crex_long)res);
	} else {
		CRX_SOCKET_ERROR(crx_sock, "Error in sendmsg", errno);
		RETVAL_FALSE;
	}
...
CRX_FUNCTION(socket_recvmsg)
{
	...
	msghdr = from_zval_run_conversions(zmsg, crx_sock, from_zval_write_msghdr_recv,
			sizeof(*msghdr), "msghdr", &allocations, &err);
	...
	res = recvmsg(crx_sock->bsd_socket, msghdr, (int)flags);
	...
	if (res != -1) {
		...
		crx_error_docref(NULL, E_WARNING, "Error in recvmsg [%d]: %s",
...
CRX_FUNCTION(socket_cmsg_space)
{
	crex_long				level,
						type,
						n = 0;
	...
	if (entry == NULL) {
		crex_value_error("Pair level " CREX_LONG_FMT " and/or type " CREX_LONG_FMT " is not supported",
			level, type);
		RETURN_THROWS();
	}
	...
	RETURN_LONG((crex_long)CMSG_SPACE(entry->size + n * entry->var_el_size));
}
...
int crx_do_setsockopt_ipv6_rfc3542(crx_socket *crx_sock, int level, int optname, zval *arg4)
{
	...
	case IPV6_PKTINFO:
#ifdef CRX_WIN32
		if (C_TYPE_P(arg4) == IS_ARRAY) {
			crx_error_docref(NULL, E_WARNING, "Windows does not "
...
#endif
		opt_ptr = from_zval_run_conversions(arg4, crx_sock, from_zval_write_in6_pktinfo,
...
int crx_do_getsockopt_ipv6_rfc3542(crx_socket *crx_sock, int level, int optname, zval *result)
{
	...
	res = getsockopt(crx_sock->bsd_socket, level, optname, buffer, &size);
	if (res != 0) {
		CRX_SOCKET_ERROR(crx_sock, "unable to get socket option", errno);
	} else {
		zval tmp;
