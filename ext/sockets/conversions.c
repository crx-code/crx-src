...
{
	int *elem;
	if ((elem = crex_hash_str_find_ptr(ctx, key, strlen(key))) != NULL) {
		return *elem;
	} else {
		return def;
	}
...
static inline void *accounted_emalloc(size_t alloc_size, ser_context *ctx)
{
	...
	crex_llist_add_element(&ctx->allocations, &ret);
	return ret;
}
								...
								va_list ap)
{
	...
	for (node = crex_llist_get_first_ex(keys, &pos);
			node != NULL;
			node = crex_llist_get_next_ex(keys, &pos)) {
...
CREX_ATTRIBUTE_FORMAT(printf, 2 ,3)
...
{
	if (err->msg != NULL) {
		crx_error_docref(NULL, err->level, "%s", err->msg);
		if (err->should_free) {
			efree(err->msg);
		}
	}
}
void allocations_dispose(crex_llist **allocations)
{
	crex_llist_destroy(*allocations);
	efree(*allocations);
	*allocations = NULL;
}
										...
										ser_context *ctx)
{
	...
	for (descr = descriptors; descr->name != NULL && !ctx->err.has_error; descr++) {
		if ((elem = crex_hash_str_find(C_ARRVAL_P(container),
			...
			crex_llist_add_element(&ctx->keys, (void*)&descr->name);
									 ...
									 res_context *ctx)
{
	...
	assert(C_TYPE_P(zarr) == IS_ARRAY);
	assert(C_ARRVAL_P(zarr) != NULL);
	...
	for (descr = descriptors; descr->name != NULL && !ctx->err.has_error; descr++) {
		...
		ZVAL_NULL(&tmp);
		new_zv = crex_symtable_str_update(C_ARRVAL_P(zarr), descr->name, descr->name_size - 1, &tmp);
...
static crex_long from_zval_integer_common(const zval *arr_value, ser_context *ctx)
{
	crex_long ret = 0;
	zval lzval;

	ZVAL_NULL(&lzval);
	if (C_TYPE_P(arr_value) != IS_LONG) {
		ZVAL_COPY(&lzval, (zval *)arr_value);
		arr_value = &lzval;
	}

	switch (C_TYPE_P(arr_value)) {
	case IS_LONG:
long_case:
		ret = C_LVAL_P(arr_value);
		break;
...
double_case:
	...
	case IS_STRING: {
		...
		switch (is_numeric_string(C_STRVAL(lzval), C_STRLEN(lzval), &lval, &dval, 0)) {
		case IS_DOUBLE:
		...
		do_from_zval_err(ctx, "expected an integer, but got a non numeric "
				"string (possibly from a converted object): '%s'", C_STRVAL_P(arr_value));
		break;
	}

	default:
		do_from_zval_err(ctx, "%s", "expected an integer, either of a CRX "
...
void from_zval_write_int(const zval *arr_value, char *field, ser_context *ctx)
{
	...
	if (lval > INT_MAX || lval < INT_MIN) {
		do_from_zval_err(ctx, "%s", "given CRX integer is out of bounds "
				"for a native int");
		return;
	}
...
static void from_zval_write_uint32(const zval *arr_value, char *field, ser_context *ctx)
{
	...
	if (sizeof(crex_long) > sizeof(uint32_t) && (lval < 0 || lval > 0xFFFFFFFF)) {
		do_from_zval_err(ctx, "%s", "given CRX integer is out of bounds "
...
static void from_zval_write_uid_t(const zval *arr_value, char *field, ser_context *ctx)
{
	...
	if ((uid_t)-1 > (uid_t)0) {
		if (sizeof(crex_long) > sizeof(uid_t) && (lval < 0 || (uid_t)lval != lval)) {
			do_from_zval_err(ctx, "%s", "given CRX integer is out of bounds "
					"for a uid_t value");
			return;
		}
	} else {
		if (sizeof(crex_long) > sizeof(uid_t) && (uid_t)lval != lval) {
			do_from_zval_err(ctx, "%s", "given CRX integer is out of bounds "
					"for a uid_t value");
			return;
		}
...
static void to_zval_read_net_uint16(const char *data, zval *zv, res_context *ctx)
{
	...
	ZVAL_LONG(zv, (crex_long)ntohs(ival));
}
...
static void from_zval_write_sin_addr(const zval *zaddr_str, char *inaddr, ser_context *ctx)
{
	...
	crex_string			*addr_str, *tmp_addr_str;

	addr_str = zval_get_tmp_string((zval *) zaddr_str, &tmp_addr_str);
	res = crx_set_inet_addr(&saddr, ZSTR_VAL(addr_str), ctx->sock);
	if (res) {
	...
	crex_tmp_string_release(tmp_addr_str);
}
static void to_zval_read_sin_addr(const char *data, zval *zv, res_context *ctx)
{
	...
	crex_string *str = crex_string_alloc(size - 1, 0);
	memset(ZSTR_VAL(str), '\0', size);

	ZVAL_NEW_STR(zv, str);

	if (inet_ntop(AF_INET, addr, C_STRVAL_P(zv), size) == NULL) {
		do_to_zval_err(ctx, "could not convert IPv4 address to string "
				"(errno %d)", errno);
		return;
	}

	C_STRLEN_P(zv) = strlen(C_STRVAL_P(zv));
}
...
static void to_zval_read_sin6_addr(const char *data, zval *zv, res_context *ctx)
{
	...
	if (inet_ntop(AF_INET6, addr, C_STRVAL_P(zv), size) == NULL) {
...
{
	crex_string			*path_str, *tmp_path_str;
	...
	if (ZSTR_LEN(path_str) == 0) {
		...
		crex_tmp_string_release(tmp_path_str);
		return;
	}
										 ...
										 ser_context *ctx)
{
	...
	*sockaddr_len = 0;
	...
	if ((elem = crex_hash_str_find(C_ARRVAL_P(container), "family", sizeof("family") - 1)) != NULL
			&& C_TYPE_P(elem) != IS_NULL) {
		const char *node = "family";
		crex_llist_add_element(&ctx->keys, &node);
									...
									ser_context			*ctx)
{
	...
	if (entry->calc_space) {
		...
		if ((data_elem = crex_hash_str_find(C_ARRVAL_P(arr), "data", sizeof("data") - 1)) == NULL) {
...
static void from_zval_write_control_array(const zval *arr, char *msghdr_c, ser_context *ctx)
{
	...
	if (C_TYPE_P(arr) != IS_ARRAY) {
		do_from_zval_err(ctx, "%s", "expected an array here");
		return;
	}

	num_elems = crex_hash_num_elements(C_ARRVAL_P(arr));
	if (num_elems == 0) {
		return;
	}
...
static void to_zval_read_cmsg_data(const char *cmsghdr_c, zval *zv, res_context *ctx)
{
	...
	if (CMSG_LEN(entry->size) > cmsg->cmsg_len) {
		do_to_zval_err(ctx, "the cmsghdr structure is unexpectedly small; "
				"expected a length of at least " CREX_LONG_FMT ", but got " CREX_LONG_FMT,
				(crex_long)CMSG_LEN(entry->size), (crex_long)cmsg->cmsg_len);
		return;
	}

	len = (size_t)cmsg->cmsg_len; /* use another var because type of cmsg_len varies */

	if (crex_hash_str_add_ptr(&ctx->params, KEY_CMSG_LEN, sizeof(KEY_CMSG_LEN) - 1, len_p) == NULL) {
	...
	crex_hash_str_del(&ctx->params, KEY_CMSG_LEN, sizeof(KEY_CMSG_LEN) - 1);
}
...
static void to_zval_read_control_array(const char *msghdr_c, zval *zv, res_context *ctx)
{
	...
	for (cmsg = CMSG_FIRSTHDR(msg);
		...
		ZVAL_NULL(&tmp);
		elem = crex_hash_next_index_insert(C_ARRVAL_P(zv), &tmp);
...
static void from_zval_write_msghdr_buffer_size(const zval *elem, char *msghdr_c, ser_context *ctx)
{
	...
	if (lval < 0 || (crex_ulong)lval > MAX_USER_BUFF_SIZE) {
		do_from_zval_err(ctx, "the buffer size must be between 1 and " CREX_LONG_FMT "; "
				"given " CREX_LONG_FMT, (crex_long)MAX_USER_BUFF_SIZE, lval);
		return;
	}
...
static void from_zval_write_iov_array(const zval *arr, char *msghdr_c, ser_context *ctx)
{
	...
	num_elem = crex_hash_num_elements(C_ARRVAL_P(arr));
	if (num_elem == 0) {
		return;
	}
...
void from_zval_write_msghdr_recv(const zval *container, char *msghdr_c, ser_context *ctx)
{
	...
	if (crex_hash_str_add_ptr(&ctx->params, KEY_FILL_SOCKADDR, sizeof(KEY_FILL_SOCKADDR) - 1, (void *)falsevp) == NULL) {
	...
	crex_hash_str_del(&ctx->params, KEY_FILL_SOCKADDR, sizeof(KEY_FILL_SOCKADDR) - 1);
	if (ctx->err.has_error) {
		return;
	}
...
static void to_zval_read_iov(const char *msghdr_c, zval *zv, res_context *ctx)
{
	...
	if ((recvmsg_ret = crex_hash_str_find_ptr(&ctx->params, KEY_RECVMSG_RET, sizeof(KEY_RECVMSG_RET) - 1)) == NULL) {
	...
	for (i = 0; bytes_left > 0 && i < (uint32_t)iovlen; i++) {
		...
		crex_string	*buf = crex_string_alloc(len, 0);
...
static void from_zval_write_ifindex(const zval *zv, char *uinteger, ser_context *ctx)
{
	...
	if (C_TYPE_P(zv) == IS_LONG) {
		if (C_LVAL_P(zv) < 0 || (crex_ulong)C_LVAL_P(zv) > UINT_MAX) { /* allow 0 (unspecified interface) */
			do_from_zval_err(ctx, "the interface index cannot be negative or "
					"larger than %u; given " CREX_LONG_FMT, UINT_MAX, C_LVAL_P(zv));
		} else {
			ret = (unsigned)C_LVAL_P(zv);
		}
	} else {
		crex_string *str, *tmp_str;
...
size_t calculate_scm_rights_space(const zval *arr, ser_context *ctx)
{
	...
	return crex_hash_num_elements(C_ARRVAL_P(arr)) * sizeof(int);
}
static void from_zval_write_fd_array_aux(zval *elem, unsigned i, void **args, ser_context *ctx)
{
	int *iarr = args[0];

	if (C_TYPE_P(elem) == IS_OBJECT && C_OBJCE_P(elem) == socket_ce) {
		crx_socket *sock = C_SOCKET_P(elem);
	...
	if (C_TYPE_P(elem) == IS_RESOURCE) {
		crx_stream *stream;

		stream = (crx_stream *)crex_fetch_resource2_ex(elem, NULL, crx_file_le_stream(), crx_file_le_pstream());
		if (stream == NULL) {
			do_from_zval_err(ctx, "resource is not a stream");
			return;
		}

		if (crx_stream_cast(stream, CRX_STREAM_AS_FD, (void **)&iarr[i - 1], REPORT_ERRORS) == FAILURE) {
...
void to_zval_read_fd_array(const char *data, zval *zv, res_context *ctx)
{
	...
	if ((cmsg_len = crex_hash_str_find_ptr(&ctx->params, KEY_CMSG_LEN, sizeof(KEY_CMSG_LEN) - 1)) == NULL) {
	...
	if (*cmsg_len < data_offset) {
		do_to_zval_err(ctx, "length of cmsg is smaller than its data member "
				"offset (" CREX_LONG_FMT " vs " CREX_LONG_FMT ")", (crex_long)*cmsg_len, (crex_long)data_offset);
		return;
	}
	...
	for (i = 0; i < num_elems; i++) {
		...
		if (S_ISSOCK(statbuf.st_mode)) {
			...
			crx_stream *stream = crx_stream_fopen_from_fd(fd, "rw", NULL);
			crx_stream_to_zval(stream, &elem);
		}
								...
								struct err_s			*err /* in/out */)
{
	...
	*allocations = NULL;
	...
	crex_hash_init(&ctx.params, 8, NULL, NULL, 0);
	crex_llist_init(&ctx.keys, sizeof(const char *), NULL, 0);
	crex_llist_init(&ctx.allocations, sizeof(void *), &free_from_zval_allocation, 0);
	ctx.sock = sock;

	structure = ecalloc(1, struct_size);

	crex_llist_add_element(&ctx.keys, &top_name);
	crex_llist_add_element(&ctx.allocations, &structure);

	/* main call */
	writer(container, structure, &ctx);

	if (ctx.err.has_error) {
		crex_llist_destroy(&ctx.allocations); /* deallocates structure as well */
		structure = NULL;
		*err = ctx.err;
	} else {
	...
	crex_llist_destroy(&ctx.keys);
	crex_hash_destroy(&ctx.params);
							  ...
							  struct err_s *err, zval *zv)
{
	...
	for (kv = key_value_pairs; kv->key != NULL; kv++) {
		crex_hash_str_update_ptr(&ctx.params, kv->key, kv->key_size - 1, kv->value);
	}
