...
#include <sys/socket.h>
  ...
  CRX_SOCKETS_CFLAGS=-DCREX_ENABLE_STATIC_TSRMLS_CACHE=1
  case $host_alias in
  *darwin*) CRX_SOCKETS_CFLAGS="$CRX_SOCKETS_CFLAGS -D__APPLE_USE_RFC_3542"
  esac
  CRX_NEW_EXTENSION(
    [sockets],
    ...
    [$ext_shared],,
    $CRX_SOCKETS_CFLAGS)
  CRX_INSTALL_HEADERS([ext/sockets/], [crx_sockets.h])
fi
