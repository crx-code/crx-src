...
static void permute(CRX_SHA3_CTX* ctx) {
...
static void CRX_SHA3_Init(CRX_SHA3_CTX* ctx,
                          int bits) {
	memset(ctx, 0, sizeof(CRX_SHA3_CTX));
}

static void CRX_SHA3_Update(CRX_SHA3_CTX* ctx,
...
static void CRX_SHA3_Final(unsigned char* digest,
                           CRX_SHA3_CTX* ctx,
                           size_t block_size,
                           size_t digest_size) {
	...
	// Zero out context
	CREX_SECURE_ZERO(ctx, sizeof(CRX_SHA3_CTX));
}

static int crx_sha3_unserialize(crx_hashcontext_object *hash,
				crex_long magic,
				const zval *zv,
				size_t block_size)
{
	CRX_SHA3_CTX *ctx = (CRX_SHA3_CTX *) hash->context;
	int r = FAILURE;
	if (magic == CRX_HASH_SERIALIZE_MAGIC_SPEC
		&& (r = crx_hash_unserialize_spec(hash, zv, CRX_SHA3_SPEC)) == SUCCESS
		&& ctx->pos < block_size) {
		return SUCCESS;
	} else {
...
void CRX_SHA3##bits##Init(CRX_SHA3_##bits##_CTX* ctx, CREX_ATTRIBUTE_UNUSED HashTable *args) { \
	CRX_SHA3_Init(ctx, bits); \
} \
void CRX_SHA3##bits##Update(CRX_SHA3_##bits##_CTX* ctx, \
                            const unsigned char* input, \
                            size_t inputLen) { \
	CRX_SHA3_Update(ctx, input, inputLen, \
                    (1600 - (2 * bits)) >> 3); \
} \
void CRX_SHA3##bits##Final(unsigned char* digest, \
                           CRX_SHA3_##bits##_CTX* ctx) { \
	CRX_SHA3_Final(digest, ctx, \
                   (1600 - (2 * bits)) >> 3, \
                   bits >> 3); \
} \
static int crx_sha3##bits##_unserialize(crx_hashcontext_object *hash, \
					crex_long magic, \
					const zval *zv) { \
	return crx_sha3_unserialize(hash, magic, zv, (1600 - (2 * bits)) >> 3); \
} \
const crx_hash_ops crx_hash_sha3_##bits##_ops = { \
	"sha3-" #bits, \
	(crx_hash_init_func_t) CRX_SHA3##bits##Init, \
	(crx_hash_update_func_t) CRX_SHA3##bits##Update, \
	(crx_hash_final_func_t) CRX_SHA3##bits##Final, \
	crx_hash_copy, \
	crx_hash_serialize, \
	crx_sha3##bits##_unserialize, \
	CRX_SHA3_SPEC, \
	bits >> 3, \
	(1600 - (2 * bits)) >> 3, \
	sizeof(CRX_SHA3_##bits##_CTX), \
	1 \
}
...
#define SUCCESS SHA3_SUCCESS /* Avoid conflict between KeccacHash.h and crex_types.h */
...
# define CRX_HASH_SERIALIZE_MAGIC_KECCAK 100
#elif Keccak_HashInstance_ImplType == 32
/* corresponds to sha3/generic32lc */
# define CRX_HASH_SERIALIZE_MAGIC_KECCAK 101
#else
# error "Unknown Keccak_HashInstance_ImplType"
#endif
#define CRX_KECCAK_SPEC "b200IiIIB"

static int crx_keccak_serialize(const crx_hashcontext_object *hash, crex_long *magic, zval *zv)
{
	*magic = CRX_HASH_SERIALIZE_MAGIC_KECCAK;
	return crx_hash_serialize_spec(hash, zv, CRX_KECCAK_SPEC);
}

static int crx_keccak_unserialize(crx_hashcontext_object *hash, crex_long magic, const zval *zv)
{
	Keccak_HashInstance *ctx = (Keccak_HashInstance *) hash->context;
	int r = FAILURE;
	if (magic == CRX_HASH_SERIALIZE_MAGIC_KECCAK
		&& (r = crx_hash_unserialize_spec(hash, zv, CRX_KECCAK_SPEC)) == SUCCESS
...
#define DECLARE_SHA3_OPS(bits) \
void CRX_SHA3##bits##Init(CRX_SHA3_##bits##_CTX* ctx, CREX_ATTRIBUTE_UNUSED HashTable *args) { \
	CREX_ASSERT(sizeof(Keccak_HashInstance) <= sizeof(CRX_SHA3_##bits##_CTX)); \
...
const crx_hash_ops crx_hash_sha3_##bits##_ops = { \
	...
	(crx_hash_final_func_t) CRX_SHA3##bits##Final, \
	crx_hash_copy, \
	crx_keccak_serialize, \
	crx_keccak_unserialize, \
	CRX_KECCAK_SPEC, \
	bits >> 3, \
	(1600 - (2 * bits)) >> 3, \
	sizeof(CRX_SHA3_CTX), \
