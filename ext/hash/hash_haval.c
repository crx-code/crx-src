...
/* {{{ CRX_3HAVALTransform */
static void CRX_3HAVALTransform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
/* {{{ CRX_4HAVALTransform */
static void CRX_4HAVALTransform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
/* {{{ CRX_5HAVALTransform */
static void CRX_5HAVALTransform(uint32_t state[8], const unsigned char block[128])
{
	uint32_t E[8];
	uint32_t x[32];
	int i;
...
#define CRX_HASH_HAVAL_INIT(p,b) \
const crx_hash_ops crx_hash_##p##haval##b##_ops = { \
	"haval" #b "," #p, \
	(crx_hash_init_func_t) CRX_##p##HAVAL##b##Init, \
	(crx_hash_update_func_t) CRX_HAVALUpdate, \
	(crx_hash_final_func_t) CRX_HAVAL##b##Final, \
	crx_hash_copy, \
	crx_hash_serialize, \
	crx_hash_unserialize, \
	CRX_HAVAL_SPEC, \
	((b) / 8), 128, sizeof(CRX_HAVAL_CTX), 1 }; \
CRX_HASH_API void CRX_##p##HAVAL##b##Init(CRX_HAVAL_CTX *context, CREX_ATTRIBUTE_UNUSED HashTable *args) \
{	int i; context->count[0] = 	context->count[1] = 	0; \
	for(i = 0; i < 8; i++) context->state[i] = D0[i]; \
	context->passes = p;	context->output = b; \
	context->Transform = CRX_##p##HAVALTransform; }

CRX_HASH_HAVAL_INIT(3,128)
CRX_HASH_HAVAL_INIT(3,160)
CRX_HASH_HAVAL_INIT(3,192)
CRX_HASH_HAVAL_INIT(3,224)
CRX_HASH_HAVAL_INIT(3,256)

CRX_HASH_HAVAL_INIT(4,128)
CRX_HASH_HAVAL_INIT(4,160)
CRX_HASH_HAVAL_INIT(4,192)
CRX_HASH_HAVAL_INIT(4,224)
CRX_HASH_HAVAL_INIT(4,256)

CRX_HASH_HAVAL_INIT(5,128)
CRX_HASH_HAVAL_INIT(5,160)
CRX_HASH_HAVAL_INIT(5,192)
CRX_HASH_HAVAL_INIT(5,224)
CRX_HASH_HAVAL_INIT(5,256)

/* {{{ CRX_HAVALUpdate */
CRX_HASH_API void CRX_HAVALUpdate(CRX_HAVAL_CTX *context, const unsigned char *input, size_t inputLen)
{
...
#define CRX_HASH_HAVAL_VERSION	0x01

/* {{{ CRX_HAVAL128Final */
CRX_HASH_API void CRX_HAVAL128Final(unsigned char *digest, CRX_HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;

	/* Version, Passes, and Digest Length */
	bits[0] =	(CRX_HASH_HAVAL_VERSION & 0x07) |
	...
	CRX_HAVALUpdate(context, PADDING, padLen);

	/* Append version, passes, digest length, and message length */
	CRX_HAVALUpdate(context, bits, 10);
...
/* {{{ CRX_HAVAL160Final */
CRX_HASH_API void CRX_HAVAL160Final(unsigned char *digest, CRX_HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ CRX_HAVAL192Final */
CRX_HASH_API void CRX_HAVAL192Final(unsigned char *digest, CRX_HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ CRX_HAVAL224Final */
CRX_HASH_API void CRX_HAVAL224Final(unsigned char *digest, CRX_HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
...
/* {{{ CRX_HAVAL256Final */
CRX_HASH_API void CRX_HAVAL256Final(unsigned char *digest, CRX_HAVAL_CTX * context)
{
	unsigned char bits[10];
	unsigned int index, padLen;
