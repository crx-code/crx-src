/*
  ...
  +----------------------------------------------------------------------+
  | Authors: Steffan Esser <sesser@crx.net>                              |
  |          Sara Golemon <pollita@crx.net>                              |
  +----------------------------------------------------------------------+
*/

#include "crx_hash.h"
#include "crx_hash_sha.h"
...
const crx_hash_ops crx_hash_sha1_ops = {
	"sha1",
	(crx_hash_init_func_t) CRX_SHA1InitArgs,
	(crx_hash_update_func_t) CRX_SHA1Update,
	(crx_hash_final_func_t) CRX_SHA1Final,
	crx_hash_copy,
	crx_hash_serialize,
...
const crx_hash_ops crx_hash_sha256_ops = {
	"sha256",
	(crx_hash_init_func_t) CRX_SHA256InitArgs,
	(crx_hash_update_func_t) CRX_SHA256Update,
	(crx_hash_final_func_t) CRX_SHA256Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA256_SPEC,
	32,
	64,
	sizeof(CRX_SHA256_CTX),
	1
};

const crx_hash_ops crx_hash_sha224_ops = {
	"sha224",
	(crx_hash_init_func_t) CRX_SHA224InitArgs,
	(crx_hash_update_func_t) CRX_SHA224Update,
	(crx_hash_final_func_t) CRX_SHA224Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA224_SPEC,
 ...
 */
CRX_HASH_API void CRX_SHA256InitArgs(CRX_SHA256_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
...
static void SHA256Transform(uint32_t state[8], const unsigned char block[64])
{
	...
	CREX_SECURE_ZERO((unsigned char*) x, sizeof(x));
}
/* }}} */
 ...
 */
CRX_HASH_API void CRX_SHA224InitArgs(CRX_SHA224_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
...
/* {{{ CRX_SHA224Update
   ...
   context.
 */
CRX_HASH_API void CRX_SHA224Update(CRX_SHA224_CTX * context, const unsigned char *input, size_t inputLen)
{
 ...
 */
CRX_HASH_API void CRX_SHA224Final(unsigned char digest[28], CRX_SHA224_CTX * context)
{
	...
	CRX_SHA224Update(context, PADDING, padLen);

	/* Append length (before padding) */
	CRX_SHA224Update(context, bits, 8);
...
/* {{{ CRX_SHA256Update
   ...
   context.
 */
CRX_HASH_API void CRX_SHA256Update(CRX_SHA256_CTX * context, const unsigned char *input, size_t inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ CRX_SHA256Final
   ...
   the message digest and zeroizing the context.
 */
CRX_HASH_API void CRX_SHA256Final(unsigned char digest[32], CRX_SHA256_CTX * context)
{
	unsigned char bits[8];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x3f);
	padLen = (index < 56) ? (56 - index) : (120 - index);
	CRX_SHA256Update(context, PADDING, padLen);

	/* Append length (before padding) */
	CRX_SHA256Update(context, bits, 8);
 ...
 */
CRX_HASH_API void CRX_SHA384InitArgs(CRX_SHA384_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
...
/* {{{ CRX_SHA384Update
   ...
   context.
 */
CRX_HASH_API void CRX_SHA384Update(CRX_SHA384_CTX * context, const unsigned char *input, size_t inputLen)
{
...
/* {{{ CRX_SHA384Final
   ...
   the message digest and zeroizing the context.
 */
CRX_HASH_API void CRX_SHA384Final(unsigned char digest[48], CRX_SHA384_CTX * context)
{
	...
	CRX_SHA384Update(context, PADDING, padLen);

	/* Append length (before padding) */
	CRX_SHA384Update(context, bits, 16);
...
const crx_hash_ops crx_hash_sha384_ops = {
	"sha384",
	(crx_hash_init_func_t) CRX_SHA384InitArgs,
	(crx_hash_update_func_t) CRX_SHA384Update,
	(crx_hash_final_func_t) CRX_SHA384Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA384_SPEC,
 ...
 */
CRX_HASH_API void CRX_SHA512InitArgs(CRX_SHA512_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	context->count[0] = context->count[1] = 0;
	/* Load magic initialization constants.
	 */
...
/* {{{ CRX_SHA512_256InitArgs
 * SHA512/245 initialization. Identical algorithm to SHA512, using alternate initval and truncation
 */
CRX_HASH_API void CRX_SHA512_256InitArgs(CRX_SHA512_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	context->count[0] = context->count[1] = 0;
...
/* {{{ CRX_SHA512_224InitArgs
 * SHA512/224 initialization. Identical algorithm to SHA512, using alternate initval and truncation
 */
CRX_HASH_API void CRX_SHA512_224InitArgs(CRX_SHA512_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
        context->count[0] = context->count[1] = 0;
...
/* {{{ CRX_SHA512Update
   ...
   context.
 */
CRX_HASH_API void CRX_SHA512Update(CRX_SHA512_CTX * context, const unsigned char *input, size_t inputLen)
{
	unsigned int i, index, partLen;
...
/* {{{ CRX_SHA512Final
   ...
   the message digest and zeroizing the context.
 */
CRX_HASH_API void CRX_SHA512Final(unsigned char digest[64], CRX_SHA512_CTX * context)
{
	unsigned char bits[16];
	unsigned int index, padLen;
	...
	index = (unsigned int) ((context->count[0] >> 3) & 0x7f);
	padLen = (index < 112) ? (112 - index) : (240 - index);
	CRX_SHA512Update(context, PADDING, padLen);

	/* Append length (before padding) */
	CRX_SHA512Update(context, bits, 16);
 ...
 */
CRX_HASH_API void CRX_SHA512_256Final(unsigned char digest[32], CRX_SHA512_CTX * context)
{
	unsigned char full_digest[64];
	CRX_SHA512Final(full_digest, context);
 ...
 */
CRX_HASH_API void CRX_SHA512_224Final(unsigned char digest[28], CRX_SHA512_CTX * context)
{
	unsigned char full_digest[64];
	CRX_SHA512Final(full_digest, context);
...
const crx_hash_ops crx_hash_sha512_ops = {
	"sha512",
	(crx_hash_init_func_t) CRX_SHA512InitArgs,
	(crx_hash_update_func_t) CRX_SHA512Update,
	(crx_hash_final_func_t) CRX_SHA512Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA512_SPEC,
	64,
	128,
	sizeof(CRX_SHA512_CTX),
	1
};

const crx_hash_ops crx_hash_sha512_256_ops = {
	"sha512/256",
	(crx_hash_init_func_t) CRX_SHA512_256InitArgs,
	(crx_hash_update_func_t) CRX_SHA512_256Update,
	(crx_hash_final_func_t) CRX_SHA512_256Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA512_SPEC,
...
const crx_hash_ops crx_hash_sha512_224_ops = {
	"sha512/224",
	(crx_hash_init_func_t) CRX_SHA512_224InitArgs,
	(crx_hash_update_func_t) CRX_SHA512_224Update,
	(crx_hash_final_func_t) CRX_SHA512_224Final,
	crx_hash_copy,
	crx_hash_serialize,
	crx_hash_unserialize,
	CRX_SHA512_SPEC,
