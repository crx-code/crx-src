...
#include "crx_hash_whirlpool.h"
#include "crx_hash_whirlpool_tables.h"
...
static void WhirlpoolTransform(CRX_WHIRLPOOL_CTX *context)
{
    int i, r;
	...
	CREX_SECURE_ZERO(state, sizeof(state));
}

CRX_HASH_API void CRX_WHIRLPOOLInit(CRX_WHIRLPOOL_CTX *context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	memset(context, 0, sizeof(*context));
}

CRX_HASH_API void CRX_WHIRLPOOLUpdate(CRX_WHIRLPOOL_CTX *context, const unsigned char *input, size_t len)
{
...
CRX_HASH_API void CRX_WHIRLPOOLFinal(unsigned char digest[64], CRX_WHIRLPOOL_CTX *context)
{
    int i;
    unsigned char *buffer      = context->buffer.data;
    unsigned char *bitLength   = context->bitlength;
...
static int crx_whirlpool_unserialize(crx_hashcontext_object *hash, crex_long magic, const zval *zv)
{
    CRX_WHIRLPOOL_CTX *ctx = (CRX_WHIRLPOOL_CTX *) hash->context;
    int r = FAILURE;
    if (magic == CRX_HASH_SERIALIZE_MAGIC_SPEC
        && (r = crx_hash_unserialize_spec(hash, zv, CRX_WHIRLPOOL_SPEC)) == SUCCESS
        && ctx->buffer.pos >= 0
...
const crx_hash_ops crx_hash_whirlpool_ops = {
	"whirlpool",
	(crx_hash_init_func_t) CRX_WHIRLPOOLInit,
	(crx_hash_update_func_t) CRX_WHIRLPOOLUpdate,
	(crx_hash_final_func_t) CRX_WHIRLPOOLFinal,
	crx_hash_copy,
	crx_hash_serialize,
	crx_whirlpool_unserialize,
	CRX_WHIRLPOOL_SPEC,
	64,
	64,
	sizeof(CRX_WHIRLPOOL_CTX),
	1
};
