...
#include "crx_hash.h"
#include "crx_hash_snefru.h"
#include "crx_hash_snefru_tables.h"
...
static inline void SnefruTransform(CRX_SNEFRU_CTX *context, const unsigned char input[32])
{
	int i, j;
	...
	CREX_SECURE_ZERO(&context->state[8], sizeof(uint32_t) * 8);
}

CRX_HASH_API void CRX_SNEFRUInit(CRX_SNEFRU_CTX *context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
...
CRX_HASH_API void CRX_SNEFRUUpdate(CRX_SNEFRU_CTX *context, const unsigned char *input, size_t len)
{
...
CRX_HASH_API void CRX_SNEFRUFinal(unsigned char digest[32], CRX_SNEFRU_CTX *context)
{
	uint32_t i, j;
...
static int crx_snefru_unserialize(crx_hashcontext_object *hash, crex_long magic, const zval *zv)
{
	CRX_SNEFRU_CTX *ctx = (CRX_SNEFRU_CTX *) hash->context;
	int r = FAILURE;
	if (magic == CRX_HASH_SERIALIZE_MAGIC_SPEC
		&& (r = crx_hash_unserialize_spec(hash, zv, CRX_SNEFRU_SPEC)) == SUCCESS
...
const crx_hash_ops crx_hash_snefru_ops = {
	"snefru",
	(crx_hash_init_func_t) CRX_SNEFRUInit,
	(crx_hash_update_func_t) CRX_SNEFRUUpdate,
	(crx_hash_final_func_t) CRX_SNEFRUFinal,
	crx_hash_copy,
	crx_hash_serialize,
	crx_snefru_unserialize,
	CRX_SNEFRU_SPEC,
	32,
	32,
	sizeof(CRX_SNEFRU_CTX),
	1
};
