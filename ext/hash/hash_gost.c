...
#include "crx_hash.h"
#include "crx_hash_gost.h"
#include "crx_hash_gost_tables.h"
...
static inline void Gost(CRX_GOST_CTX *context, uint32_t data[8])
{
	int i;
...
static inline void GostTransform(CRX_GOST_CTX *context, const unsigned char input[32])
{
	int i, j;
...
CRX_HASH_API void CRX_GOSTInit(CRX_GOST_CTX *context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	memset(context, 0, sizeof(*context));
	context->tables = &tables_test;
}

CRX_HASH_API void CRX_GOSTInitCrypto(CRX_GOST_CTX *context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
	CRX_GOSTInit(context, NULL);
	context->tables = &tables_crypto;
}

static const uint32_t MAX32 = 0xffffffffLU;

CRX_HASH_API void CRX_GOSTUpdate(CRX_GOST_CTX *context, const unsigned char *input, size_t len)
{
	...
	if (context->length + len < 32) {
		...
		CREX_SECURE_ZERO(&context->buffer[r], 32 - r);
		context->length = (unsigned char)r;
	}
}

CRX_HASH_API void CRX_GOSTFinal(unsigned char digest[32], CRX_GOST_CTX *context)
{
	uint32_t i, j, l[8] = {0};
...
static int crx_gost_unserialize(crx_hashcontext_object *hash, crex_long magic, const zval *zv)
{
	CRX_GOST_CTX *ctx = (CRX_GOST_CTX *) hash->context;
	int r = FAILURE;
	if (magic == CRX_HASH_SERIALIZE_MAGIC_SPEC
		&& (r = crx_hash_unserialize_spec(hash, zv, CRX_GOST_SPEC)) == SUCCESS
...
const crx_hash_ops crx_hash_gost_ops = {
	"gost",
	(crx_hash_init_func_t) CRX_GOSTInit,
	(crx_hash_update_func_t) CRX_GOSTUpdate,
	(crx_hash_final_func_t) CRX_GOSTFinal,
	crx_hash_copy,
	crx_hash_serialize,
	crx_gost_unserialize,
	CRX_GOST_SPEC,
	32,
	32,
	sizeof(CRX_GOST_CTX),
...
const crx_hash_ops crx_hash_gost_crypto_ops = {
	"gost-crypto",
	(crx_hash_init_func_t) CRX_GOSTInitCrypto,
	(crx_hash_update_func_t) CRX_GOSTUpdate,
	(crx_hash_final_func_t) CRX_GOSTFinal,
	crx_hash_copy,
	crx_hash_serialize,
