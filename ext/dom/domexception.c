...
extern crex_class_entry *dom_domexception_class_entry;

void crx_dom_throw_error_with_message(int error_code, char *error_message, int strict_error) /* {{{ */
{
	if (strict_error == 1) {
		crex_throw_exception(dom_domexception_class_entry, error_message, error_code);
	} else {
		crx_libxml_issue_error(E_WARNING, error_message);
	}
}
/* }}} */
...
void crx_dom_throw_error(int error_code, int strict_error)
{
	...
	crx_dom_throw_error_with_message(error_code, error_message, strict_error);
}
/* }}} end crx_dom_throw_error */
