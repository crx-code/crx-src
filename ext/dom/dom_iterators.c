...
static void crx_dom_iterator_dtor(crex_object_iterator *iter) /* {{{ */
{
	crx_dom_iterator *iterator = (crx_dom_iterator *)iter;
...
static int crx_dom_iterator_valid(crex_object_iterator *iter) /* {{{ */
{

	crx_dom_iterator *iterator = (crx_dom_iterator *)iter;

	if (C_TYPE(iterator->curobj) != IS_UNDEF) {
		return SUCCESS;
	} else {
		return FAILURE;
	}
...
zval *crx_dom_iterator_current_data(crex_object_iterator *iter) /* {{{ */
{
	crx_dom_iterator *iterator = (crx_dom_iterator *)iter;
...
static void crx_dom_iterator_current_key(crex_object_iterator *iter, zval *key) /* {{{ */
{
	crx_dom_iterator *iterator = (crx_dom_iterator *)iter;
	zval *object = &iterator->intern.data;

	if (instanceof_function(C_OBJCE_P(object), dom_nodelist_class_entry)) {
		ZVAL_LONG(key, iter->index);
	} else {
		dom_object *intern = C_DOMOBJ_P(&iterator->curobj);

		if (intern != NULL && intern->ptr != NULL) {
			xmlNodePtr curnode = (xmlNodePtr)((crx_libxml_node_ptr *)intern->ptr)->node;
...
static void crx_dom_iterator_move_forward(crex_object_iterator *iter) /* {{{ */
{
	zval *object;
	...
	nnmap = C_DOMOBJ_P(object);
	objmap = (dom_nnodemap_object *)nnmap->ptr;

	intern = C_DOMOBJ_P(&iterator->curobj);
	...
	if (intern != NULL && intern->ptr != NULL) {
		if (objmap->nodetype != XML_ENTITY_NODE &&
			...
			if (objmap->nodetype == DOM_NODESET) {
				...
				crex_hash_move_forward_ex(nodeht, &iterator->pos);
				if ((entry = crex_hash_get_current_data_ex(nodeht, &iterator->pos))) {
					zval_ptr_dtor(&iterator->curobj);
				...
				if (objmap->nodetype == XML_ATTRIBUTE_NODE ||
					...
					curnode = (xmlNodePtr)((crx_libxml_node_ptr *)intern->ptr)->node;
					...
					if (crx_dom_is_cache_tag_stale_from_node(&iterator->cache_tag, basenode)) {
						crx_dom_mark_cache_tag_up_to_date_from_node(&iterator->cache_tag, basenode);
						previndex = 0;
			...
			if (objmap->nodetype == XML_ENTITY_NODE) {
				curnode = crx_dom_libxml_hash_iter(objmap->ht, iter->index);
			} else {
				curnode = crx_dom_libxml_notation_iter(objmap->ht, iter->index);
			}
		}
	}
err:
	...
	if (curnode) {
		crx_dom_create_object(curnode, &iterator->curobj, objmap->baseobj);
	}
}
/* }}} */

static const crex_object_iterator_funcs crx_dom_iterator_funcs = {
	crx_dom_iterator_dtor,
	crx_dom_iterator_valid,
	crx_dom_iterator_current_data,
	crx_dom_iterator_current_key,
	crx_dom_iterator_move_forward,
	NULL,
	NULL,
	NULL, /* get_gc */
};
...
crex_object_iterator *crx_dom_get_iterator(crex_class_entry *ce, zval *object, int by_ref) /* {{{ */
{
	...
	if (objmap != NULL) {
		if (objmap->nodetype != XML_ENTITY_NODE &&
			...
			if (objmap->nodetype == DOM_NODESET) {
				nodeht = HASH_OF(&objmap->baseobj_zv);
				crex_hash_internal_pointer_reset_ex(nodeht, &iterator->pos);
				if ((entry = crex_hash_get_current_data_ex(nodeht, &iterator->pos))) {
					ZVAL_COPY(&iterator->curobj, entry);
				}
			} else {
			...
			if (objmap->nodetype == XML_ENTITY_NODE) {
				curnode = crx_dom_libxml_hash_iter(objmap->ht, 0);
			} else {
				curnode = crx_dom_libxml_notation_iter(objmap->ht, 0);
			}
		}
	}
err:
