...
extern crex_module_entry dom_module_entry;
#define crxext_dom_ptr &dom_module_entry
...
/* DOM API_VERSION, please bump it up, if you change anything in the API
   ...
   Can be checked with crxversion("dom");
*/
...
static inline dom_xpath_object *crx_xpath_obj_from_obj(crex_object *obj) {
	return (dom_xpath_object*)((char*)(obj)
		- XtOffsetOf(dom_xpath_object, dom) - XtOffsetOf(dom_object, std));
}

#define C_XPATHOBJ_P(zv)  crx_xpath_obj_from_obj(C_OBJ_P((zv)))
...
static inline dom_object_namespace_node *crx_dom_namespace_node_obj_from_obj(crex_object *obj) {
...
void crx_dom_throw_error(int error_code, int strict_error);
void crx_dom_throw_error_with_message(int error_code, char *error_message, int strict_error);
...
xmlNode *crx_dom_libxml_hash_iter(xmlHashTable *ht, int index);
xmlNode *crx_dom_libxml_notation_iter(xmlHashTable *ht, int index);
crex_object_iterator *crx_dom_get_iterator(crex_class_entry *ce, zval *object, int by_ref);
...
void crx_dom_get_content_into_zval(const xmlNode *nodep, zval *target, bool default_is_null);
...
#define DOM_GET_INTERN(__id, __intern) { \
	__intern = C_DOMOBJ_P(__id); \
	if (UNEXPECTED(__intern->ptr == NULL)) { \
		crex_throw_error(NULL, "Couldn't fetch %s", ZSTR_VAL(__intern->std.ce->name));\
		RETURN_THROWS();\
  	} \
}

#define DOM_GET_THIS_INTERN(__intern) DOM_GET_INTERN(CREX_THIS, __intern)

#define DOM_GET_OBJ(__ptr, __id, __prtype, __intern) { \
	DOM_GET_INTERN(__id, __intern); \
	__ptr = (__prtype)((crx_libxml_node_ptr *)__intern->ptr)->node; \
}
...
#define DOM_NOT_IMPLEMENTED() \
	crex_throw_error(NULL, "Not yet implemented"); \
	RETURN_THROWS();

#define DOM_NODELIST 0
#define DOM_NAMEDNODEMAP 1

static crex_always_inline bool crx_dom_is_cache_tag_stale_from_doc_ptr(const crx_libxml_cache_tag *cache_tag, const crx_libxml_ref_obj *doc_ptr)
{
	CREX_ASSERT(cache_tag != NULL);
	CREX_ASSERT(doc_ptr != NULL);
	/* See overflow comment in crx_libxml_invalidate_node_list_cache(). */
#if SIZEOF_SIZE_T == 8
...
static crex_always_inline bool crx_dom_is_cache_tag_stale_from_node(const crx_libxml_cache_tag *cache_tag, const xmlNodePtr node)
{
	CREX_ASSERT(node != NULL);
	crx_libxml_node_ptr *private = node->_private;
	if (!private) {
		return true;
	}
	crx_libxml_node_object *object_private = private->_private;
	if (!object_private || !object_private->document) {
		return true;
	}
	return crx_dom_is_cache_tag_stale_from_doc_ptr(cache_tag, object_private->document);
}

static crex_always_inline void crx_dom_mark_cache_tag_up_to_date_from_node(crx_libxml_cache_tag *cache_tag, const xmlNodePtr node)
{
	CREX_ASSERT(cache_tag != NULL);
	crx_libxml_node_ptr *private = node->_private;
	if (private) {
