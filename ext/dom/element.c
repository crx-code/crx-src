...
CRX_METHOD(DOMElement, __main)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|s!s", &name, &name_len, &value, &value_len, &uri, &uri_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (uri_len > 0) {
		...
		if (errorcode != 0) {
			...
			crx_dom_throw_error(errorcode, 1);
			RETURN_THROWS();
		}
	} else {
	    ...
	    if (prefix != NULL) {
			xmlFree(localname);
			xmlFree(prefix);
	        crx_dom_throw_error(NAMESPACE_ERR, 1);
	        RETURN_THROWS();
	    }
...
static xmlAttrPtr dom_element_reflected_attribute_write(dom_object *obj, zval *newval, const char *name)
{
	...
	return xmlSetNsProp(nodep, NULL, (const xmlChar *) name, (const xmlChar *) C_STRVAL_P(newval));
}
...
static void crx_set_attribute_id(xmlAttrPtr attrp, bool is_id);
...
int dom_element_id_write(dom_object *obj, zval *newval)
{
	...
	crx_set_attribute_id(attr, true);
	return SUCCESS;
}
/* }}} */
...
Since:
*/
CRX_METHOD(DOMElement, getAttribute)
{
	zval *id;
	xmlNode *nodep;
	char *name;
...
Since:
*/
CRX_METHOD(DOMElement, getAttributeNames)
{
	zval *id;
	xmlNode *nodep;
	...
	HashTable *ht = C_ARRVAL_P(return_value);
	...
	for (xmlNsPtr nsptr = nodep->nsDef; nsptr; nsptr = nsptr->next) {
		...
		crex_hash_next_index_insert(ht, &tmp);
	}
...
Since:
*/
CRX_METHOD(DOMElement, setAttribute)
{
	zval *id;
	xmlNode *nodep;
	...
	if (!attr) {
		crex_argument_value_error(1, "must be a valid XML attribute");
		RETURN_THROWS();
	}
...
static void dom_deep_ns_redef(xmlNodePtr node, xmlNsPtr ns_to_redefine)
{
	...
	while (worklist_size > 0) {
		...
		CREX_ASSERT(current_worklist_item->current_node->type == XML_ELEMENT_NODE);
...
{
	CREX_ASSERT(thisp != NULL);
	CREX_ASSERT(attrp != NULL);

	switch (attrp->type) {
		case XML_ATTRIBUTE_NODE:
			if (crx_dom_object_get_data(attrp) == NULL) {
		...
		case XML_NAMESPACE_DECL: {
			...
			ns->next = NULL;
			crx_libxml_set_old_ns(thisp->doc, ns); /* note: can't deallocate as it might be referenced by a "fake namespace node" */
...
Since:
*/
CRX_METHOD(DOMElement, removeAttribute)
{
	zval *id;
...
Since:
*/
CRX_METHOD(DOMElement, getAttributeNode)
{
	zval *id;
	xmlNodePtr nodep, attrp;
	size_t name_len;
	...
	if (attrp->type == XML_NAMESPACE_DECL) {
		...
		(void) crx_dom_create_fake_namespace_decl(nodep, original, return_value, intern);
	} else {
...
Since:
*/
CRX_METHOD(DOMElement, setAttributeNode)
{
	zval *id, *node;
	xmlNode *nodep;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &node, dom_attr_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (attrp->type != XML_ATTRIBUTE_NODE) {
		crex_argument_value_error(1, "must have the node attribute");
		RETURN_THROWS();
	}
	...
	if (existattrp != NULL && existattrp->type != XML_ATTRIBUTE_DECL) {
		if ((oldobj = crx_dom_object_get_data((xmlNodePtr) existattrp)) != NULL &&
			((crx_libxml_node_ptr *)oldobj->ptr)->node == (xmlNodePtr) attrp)
		{
			RETURN_NULL();
		}
	...
	if (attrp->doc == NULL && nodep->doc != NULL) {
		...
		crx_libxml_increment_doc_ref((crx_libxml_node_object *)attrobj, NULL);
	}
...
Since:
*/
CRX_METHOD(DOMElement, removeAttributeNode)
{
	zval *id, *node;
	xmlNode *nodep;
	xmlAttr *attrp;
	...
	if (attrp->type != XML_ATTRIBUTE_NODE || attrp->parent != nodep) {
		crx_dom_throw_error(NOT_FOUND_ERR, dom_get_strict_error(intern->document));
		RETURN_FALSE;
	}
...
Since:
*/
CRX_METHOD(DOMElement, getElementsByTagName)
{
	size_t name_len;
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, getAttributeNS)
{
	zval *id;
	xmlNodePtr elemp;
	xmlNsPtr nsptr;
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, setAttributeNS)
{
	zval *id;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s!ss", &uri, &uri_len, &name, &name_len, &value, &value_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (errorcode != 0) {
		crx_dom_throw_error(errorcode, stricterror);
	}
...
{
	CREX_ASSERT(node->type == XML_ELEMENT_NODE);
...
static void dom_eliminate_ns(xmlNodePtr nodep, xmlNsPtr nsptr)
{
	...
	nsptr->next = NULL;
	crx_libxml_set_old_ns(nodep->doc, nsptr);
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, removeAttributeNS)
{
	zval *id;
	xmlNode *nodep;
	xmlAttr *attrp;
	...
	if (attrp && attrp->type != XML_ATTRIBUTE_DECL) {
		if (crx_dom_object_get_data((xmlNodePtr) attrp) == NULL) {
			node_list_unlink(attrp->children);
			xmlUnlinkNode((xmlNodePtr) attrp);
			xmlFreeProp(attrp);
		} else {
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, getAttributeNodeNS)
{
	zval *id;
	xmlNodePtr elemp;
	xmlAttrPtr attrp;
	...
	if (attrp == NULL) {
		if (xmlStrEqual((xmlChar *) uri, (xmlChar *)DOM_XMLNS_NAMESPACE)) {
			...
			if (nsptr != NULL) {
				/* Keep parent alive, because we're a fake child. */
				GC_ADDREF(&intern->std);
				(void) crx_dom_create_fake_namespace_decl(elemp, nsptr, return_value, intern);
			} else {
				RETURN_NULL();
			}
		} else {
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, setAttributeNodeNS)
{
	zval *id, *node;
	xmlNode *nodep;
	xmlNs *nsp;
	...
	CREX_ASSERT(attrp->type == XML_ATTRIBUTE_NODE);
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, getElementsByTagNameNS)
{
	size_t uri_len, name_len;
	dom_object *intern, *namednode;
	char *uri, *name;
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, hasAttribute)
{
	zval *id;
	xmlNode *nodep;
	dom_object *intern;
...
Since: DOM Level 2
*/
CRX_METHOD(DOMElement, hasAttributeNS)
{
	zval *id;
	xmlNodePtr elemp;
	xmlNs *nsp;
...
Since: DOM Level 3
*/
CRX_METHOD(DOMElement, setIdAttribute)
{
	zval *id;
	xmlNode *nodep;
	xmlAttrPtr attrp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sb", &name, &name_len, &is_id) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (attrp == NULL || attrp->type == XML_ATTRIBUTE_DECL) {
		...
		crx_set_attribute_id(attrp, is_id);
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMElement, setIdAttributeNS)
{
	zval *id;
	xmlNodePtr elemp;
	xmlAttrPtr attrp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ssb", &uri, &uri_len, &name, &name_len, &is_id) == FAILURE) {
		RETURN_THROWS();
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMElement, setIdAttributeNode)
{
	zval *id, *node;
	xmlNode *nodep;
	xmlAttrPtr attrp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ob", &node, dom_attr_class_entry, &is_id) == FAILURE) {
		RETURN_THROWS();
	}
...
Since: DOM Living Standard (DOM4)
*/
CRX_METHOD(DOMElement, replaceWith)
{
	uint32_t argc = 0;
	zval *args;
	dom_object *intern;
...
Since:
*/
CRX_METHOD(DOMElement, replaceChildren)
{
	uint32_t argc = 0;
	zval *args;
	dom_object *intern;
...
static xmlNodePtr dom_insert_adjacent(const crex_string *where, xmlNodePtr thisp, dom_object *this_intern, xmlNodePtr otherp)
{
	if (crex_string_equals_literal_ci(where, "beforebegin")) {
		if (thisp->parent == NULL) {
			return NULL;
		}
		if (dom_hierarchy(thisp->parent, otherp) == FAILURE) {
			crx_dom_throw_error(HIERARCHY_REQUEST_ERR, dom_get_strict_error(this_intern->document));
			return INSERT_ADJACENT_RES_FAILED;
		}
		if (!crx_dom_adopt_node(otherp, this_intern, thisp->doc)) {
			return INSERT_ADJACENT_RES_FAILED;
		}
		otherp = xmlAddPrevSibling(thisp, otherp);
	} else if (crex_string_equals_literal_ci(where, "afterbegin")) {
	...
	} else if (crex_string_equals_literal_ci(where, "beforeend")) {
		if (dom_hierarchy(thisp, otherp) == FAILURE) {
			crx_dom_throw_error(HIERARCHY_REQUEST_ERR, dom_get_strict_error(this_intern->document));
			return INSERT_ADJACENT_RES_FAILED;
		}
		...
		otherp = xmlAddChild(thisp, otherp);
	} else if (crex_string_equals_literal_ci(where, "afterend")) {
		if (thisp->parent == NULL) {
			return NULL;
		}
		if (dom_hierarchy(thisp->parent, otherp) == FAILURE) {
		...
		crx_dom_throw_error(SYNTAX_ERR, dom_get_strict_error(this_intern->document));
		return INSERT_ADJACENT_RES_FAILED;
	}
...
Since:
*/
CRX_METHOD(DOMElement, insertAdjacentElement)
{
	crex_string *where;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "SO", &where, &element_zval, dom_element_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
...
Since:
*/
CRX_METHOD(DOMElement, insertAdjacentText)
{
	crex_string *where, *data;
	dom_object *this_intern;
	zval *id;
	xmlNodePtr thisp;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "SS", &where, &data) == FAILURE) {
		RETURN_THROWS();
	}

	DOM_GET_THIS_OBJ(thisp, id, xmlNodePtr, this_intern);

	if (UNEXPECTED(CREX_SIZE_T_INT_OVFL(ZSTR_LEN(data)))) {
		crex_argument_value_error(2, "is too long");
		RETURN_THROWS();
	}
...
Since:
*/
CRX_METHOD(DOMElement, toggleAttribute)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|b!", &qname, &qname_length, &force, &force_is_null) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (thisp->doc->type == XML_HTML_DOCUMENT_NODE && (thisp->ns == NULL || xmlStrEqual(thisp->ns->href, (const xmlChar *) "http://www.w3.org/1999/xhtml"))) {
		qname_tmp = crex_str_tolower_dup_ex(qname, qname_length);
		if (qname_tmp != NULL) {
			qname = qname_tmp;
		}
	}
