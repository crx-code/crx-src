...
/* {{{ class entries */
CRX_DOM_EXPORT crex_class_entry *dom_node_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_domexception_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_parentnode_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_childnode_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_domimplementation_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_documentfragment_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_document_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_nodelist_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_namednodemap_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_characterdata_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_attr_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_element_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_text_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_comment_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_cdatasection_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_documenttype_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_notation_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_entity_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_entityreference_class_entry;
CRX_DOM_EXPORT crex_class_entry *dom_processinginstruction_class_entry;
#ifdef LIBXML_XPATH_ENABLED
CRX_DOM_EXPORT crex_class_entry *dom_xpath_class_entry;
#endif
CRX_DOM_EXPORT crex_class_entry *dom_namespace_node_class_entry;
/* }}} */

crex_object_handlers dom_object_handlers;
crex_object_handlers dom_nnodemap_object_handlers;
crex_object_handlers dom_nodelist_object_handlers;
crex_object_handlers dom_object_namespace_node_handlers;
#ifdef LIBXML_XPATH_ENABLED
crex_object_handlers dom_xpath_object_handlers;
#endif
...
static crex_object *dom_objects_namespace_node_new(crex_class_entry *class_type);
static void dom_object_namespace_node_free_storage(crex_object *object);
static xmlNodePtr crx_dom_create_fake_namespace_decl_node_ptr(xmlNodePtr nodep, xmlNsPtr original);
...
static crex_object_handlers* dom_get_obj_handlers(void) {
...
/* {{{ dom_get_doc_props() */
dom_doc_propsptr dom_get_doc_props(crx_libxml_ref_obj *document)
{
...
libxml_doc_props const* dom_get_doc_props_read_only(const crx_libxml_ref_obj *document)
{
	if (document && document->doc_props) {
		return document->doc_props;
	} else {
...
static void dom_copy_doc_props(crx_libxml_ref_obj *source_doc, crx_libxml_ref_obj *dest_doc)
{
	...
	if (source_doc && dest_doc) {
		...
		if (source->classmap) {
			...
			crex_hash_init(dest->classmap, 0, NULL, NULL, 0);
			crex_hash_copy(dest->classmap, source->classmap, NULL);
		}

	}
}

void dom_set_doc_classmap(crx_libxml_ref_obj *document, crex_class_entry *basece, crex_class_entry *ce)
{
	dom_doc_propsptr doc_props;
	...
	if (document) {
		...
		if (doc_props->classmap == NULL) {
			...
			crex_hash_init(doc_props->classmap, 0, NULL, NULL, 0);
		}
		if (ce) {
			crex_hash_update_ptr(doc_props->classmap, basece->name, ce);
		} else {
			crex_hash_del(doc_props->classmap, basece->name);
		}
	}
}

crex_class_entry *dom_get_doc_classmap(crx_libxml_ref_obj *document, crex_class_entry *basece)
{
	if (document) {
		libxml_doc_props const* doc_props = dom_get_doc_props_read_only(document);
		if (doc_props->classmap) {
			crex_class_entry *ce = crex_hash_find_ptr(doc_props->classmap, basece->name);
			if (ce) {
				return ce;
			}
		}
...
int dom_get_strict_error(crx_libxml_ref_obj *document) {
...
{
	if (obj && obj->ptr != NULL) {
		return ((crx_libxml_node_ptr *)obj->ptr)->node;
	} else {
		return NULL;
	}
}
...
/* {{{ dom_object *crx_dom_object_get_data(xmlNodePtr obj) */
CRX_DOM_EXPORT dom_object *crx_dom_object_get_data(xmlNodePtr obj)
{
	if (obj && obj->_private != NULL) {
		return (dom_object *) ((crx_libxml_node_ptr *) obj->_private)->_private;
	} else {
		return NULL;
	}
}
/* }}} end crx_dom_object_get_data */
...
static void dom_register_prop_handler(HashTable *prop_handler, char *name, size_t name_len, dom_read_t read_func, dom_write_t write_func)
{
	...
	crex_hash_add_mem(prop_handler, str, &hnd, sizeof(dom_prop_handler));
	crex_string_release_ex(str, 1);
}

static zval *dom_get_property_ptr_ptr(crex_object *object, crex_string *name, int type, void **cache_slot)
{
	dom_object *obj = crx_dom_obj_from_obj(object);

	if (!obj->prop_handler || !crex_hash_exists(obj->prop_handler, name)) {
...
/* {{{ dom_read_property */
zval *dom_read_property(crex_object *object, crex_string *name, int type, void **cache_slot, zval *rv)
{
	dom_object *obj = crx_dom_obj_from_obj(object);
	zval *retval;
...
zval *dom_write_property(crex_object *object, crex_string *name, zval *value, void **cache_slot)
{
	dom_object *obj = crx_dom_obj_from_obj(object);
	dom_prop_handler *hnd = NULL;
	...
	if (hnd) {
		...
		if (prop && CREX_TYPE_IS_SET(prop->type)) {
			zval tmp;
			ZVAL_COPY(&tmp, value);
			if (!crex_verify_property_type(prop, &tmp, CREX_CALL_USES_STRICT_TYPES(EG(current_execute_data)))) {
				zval_ptr_dtor(&tmp);
				return &EG(error_zval);
			}
...
/* {{{ dom_property_exists */
static int dom_property_exists(crex_object *object, crex_string *name, int check_empty, void **cache_slot)
{
	dom_object *obj = crx_dom_obj_from_obj(object);
	dom_prop_handler *hnd = NULL;
	int retval = 0;
	...
	if (hnd) {
		...
		retval = crex_std_has_property(object, name, check_empty, cache_slot);
	}
...
static HashTable* dom_get_debug_info_helper(crex_object *object, int *is_temp) /* {{{ */
{
	dom_object			*obj = crx_dom_obj_from_obj(object);
	...
	crex_string         *object_str;

	*is_temp = 1;

	std_props = crex_std_get_properties(object);
	debug_info = crex_array_dup(std_props);
	...
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(prop_handlers, string_key, entry) {
		zval value;
		...
		if (C_TYPE(value) == IS_OBJECT) {
			zval_ptr_dtor(&value);
			ZVAL_NEW_STR(&value, object_str);
			crex_string_addref(object_str);
		}

		crex_hash_update(debug_info, string_key, &value);
	} CREX_HASH_FOREACH_END();

	crex_string_release_ex(object_str, 0);

	return debug_info;
}
/* }}} */

static HashTable* dom_get_debug_info(crex_object *object, int *is_temp) /* {{{ */
{
	return dom_get_debug_info_helper(object, is_temp);
}
/* }}} */

void *crx_dom_export_node(zval *object) /* {{{ */
{
	crx_libxml_node_object *intern;
	xmlNodePtr nodep = NULL;

	intern = (crx_libxml_node_object *) C_DOMOBJ_P(object);
	if (intern->node) {
...
CRX_FUNCTION(dom_import_simplexml)
{
	zval *node;
	xmlNodePtr nodep = NULL;
	crx_libxml_node_object *nodeobj;
	int ret;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "o", &node) == FAILURE) {
		RETURN_THROWS();
	}

	nodeobj = (crx_libxml_node_object *) ((char *) C_OBJ_P(node) - C_OBJ_HT_P(node)->offset);
	nodep = crx_libxml_import_node(node);
	...
	if (nodep && nodeobj && (nodep->type == XML_ELEMENT_NODE || nodep->type == XML_ATTRIBUTE_NODE)) {
		...
		crex_argument_value_error(1, "is not a valid node type");
		RETURN_THROWS();
	}
}
/* }}} */

static dom_object* dom_objects_set_class(crex_class_entry *class_type);
...
static void dom_update_refcount_after_clone(dom_object *original, xmlNodePtr original_node, dom_object *clone, xmlNodePtr cloned_node)
{
	...
	crx_libxml_increment_doc_ref((crx_libxml_node_object *)clone, cloned_node->doc);
	crx_libxml_increment_node_ptr((crx_libxml_node_object *)clone, cloned_node, (void *)clone);
	if (original->document != clone->document) {
		dom_copy_doc_props(original->document, clone->document);
	}
}

static crex_object *dom_objects_store_clone_obj(crex_object *zobject) /* {{{ */
{
	dom_object *intern = crx_dom_obj_from_obj(zobject);
	...
	crex_objects_clone_members(&clone->std, &intern->std);

	return &clone->std;
}
/* }}} */

static crex_object *dom_object_namespace_node_clone_obj(crex_object *zobject)
{
	dom_object_namespace_node *intern = crx_dom_namespace_node_obj_from_obj(zobject);
	crex_object *clone = dom_objects_namespace_node_new(intern->dom.std.ce);
	dom_object_namespace_node *clone_intern = crx_dom_namespace_node_obj_from_obj(clone);

	xmlNodePtr original_node = dom_object_get_node(&intern->dom);
	CREX_ASSERT(original_node->type == XML_NAMESPACE_DECL);
	xmlNodePtr cloned_node = crx_dom_create_fake_namespace_decl_node_ptr(original_node->parent, original_node->ns);
	...
	crex_objects_clone_members(clone, &intern->dom.std);
	return clone;
}

static void dom_copy_prop_handler(zval *zv) /* {{{ */
{
	dom_prop_handler *hnd = C_PTR_P(zv);
	C_PTR_P(zv) = malloc(sizeof(dom_prop_handler));
	memcpy(C_PTR_P(zv), hnd, sizeof(dom_prop_handler));
}
/* }}} */
...
static const crex_module_dep dom_deps[] = {
	CREX_MOD_REQUIRED("libxml")
	CREX_MOD_CONFLICTS("domxml")
	CREX_MOD_END
};

crex_module_entry dom_module_entry = { /* {{{ */
...
void dom_objects_free_storage(crex_object *object);
void dom_nnodemap_objects_free_storage(crex_object *object);
static zval *dom_nodelist_read_dimension(crex_object *object, zval *offset, int type, zval *rv);
static int dom_nodelist_has_dimension(crex_object *object, zval *member, int check_empty);
static zval *dom_nodemap_read_dimension(crex_object *object, zval *offset, int type, zval *rv);
static int dom_nodemap_has_dimension(crex_object *object, zval *member, int check_empty);
static crex_object *dom_objects_store_clone_obj(crex_object *zobject);
#ifdef LIBXML_XPATH_ENABLED
void dom_xpath_objects_free_storage(crex_object *object);
#endif

/* {{{ CRX_MINIT_FUNCTION(dom) */
CRX_MINIT_FUNCTION(dom)
{
	memcpy(&dom_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	...
	memcpy(&dom_nnodemap_object_handlers, &dom_object_handlers, sizeof(crex_object_handlers));
	...
	memcpy(&dom_nodelist_object_handlers, &dom_nnodemap_object_handlers, sizeof(crex_object_handlers));
	...
	memcpy(&dom_object_namespace_node_handlers, &dom_object_handlers, sizeof(crex_object_handlers));
	...
	dom_domexception_class_entry = register_class_DOMException(crex_ce_exception);
	...
	crex_hash_init(&dom_node_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_add_ptr(&classes, dom_node_class_entry->name, &dom_node_prop_handlers);
	...
	crex_hash_init(&dom_namespace_node_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_add_ptr(&classes, dom_namespace_node_class_entry->name, &dom_namespace_node_prop_handlers);
	...
	crex_hash_init(&dom_documentfragment_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_documentfragment_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_documentfragment_class_entry->name, &dom_documentfragment_prop_handlers);
	...
	crex_hash_init(&dom_document_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_document_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_document_class_entry->name, &dom_document_prop_handlers);

	dom_nodelist_class_entry = register_class_DOMNodeList(crex_ce_aggregate, crex_ce_countable);
	...
	dom_nodelist_class_entry->get_iterator = crx_dom_get_iterator;

	crex_hash_init(&dom_nodelist_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	dom_register_prop_handler(&dom_nodelist_prop_handlers, "length", sizeof("length")-1, dom_nodelist_length_read, NULL);
	crex_hash_add_ptr(&classes, dom_nodelist_class_entry->name, &dom_nodelist_prop_handlers);

	dom_namednodemap_class_entry = register_class_DOMNamedNodeMap(crex_ce_aggregate, crex_ce_countable);
	...
	dom_namednodemap_class_entry->get_iterator = crx_dom_get_iterator;

	crex_hash_init(&dom_namednodemap_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	dom_register_prop_handler(&dom_namednodemap_prop_handlers, "length", sizeof("length")-1, dom_namednodemap_length_read, NULL);
	crex_hash_add_ptr(&classes, dom_namednodemap_class_entry->name, &dom_namednodemap_prop_handlers);
	...
	crex_hash_init(&dom_characterdata_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_characterdata_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_characterdata_class_entry->name, &dom_characterdata_prop_handlers);

	dom_attr_class_entry = register_class_DOMAttr(dom_node_class_entry);
	dom_attr_class_entry->create_object = dom_objects_new;

	crex_hash_init(&dom_attr_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_attr_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_attr_class_entry->name, &dom_attr_prop_handlers);
	...
	crex_hash_init(&dom_element_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_element_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_element_class_entry->name, &dom_element_prop_handlers);
	...
	crex_hash_init(&dom_text_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	dom_register_prop_handler(&dom_text_prop_handlers, "wholeText", sizeof("wholeText")-1, dom_text_whole_text_read, NULL);
	crex_hash_merge(&dom_text_prop_handlers, &dom_characterdata_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_text_class_entry->name, &dom_text_prop_handlers);
	...
	crex_hash_add_ptr(&classes, dom_comment_class_entry->name, &dom_characterdata_prop_handlers);
	...
	crex_hash_add_ptr(&classes, dom_cdatasection_class_entry->name, &dom_text_prop_handlers);
	...
	crex_hash_init(&dom_documenttype_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_documenttype_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_documenttype_class_entry->name, &dom_documenttype_prop_handlers);
	...
	crex_hash_init(&dom_notation_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_notation_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_notation_class_entry->name, &dom_notation_prop_handlers);

	dom_entity_class_entry = register_class_DOMEntity(dom_node_class_entry);
	dom_entity_class_entry->create_object = dom_objects_new;

	crex_hash_init(&dom_entity_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_entity_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_entity_class_entry->name, &dom_entity_prop_handlers);
	...
	crex_hash_add_ptr(&classes, dom_entityreference_class_entry->name, &dom_node_prop_handlers);
	...
	crex_hash_init(&dom_processinginstruction_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_merge(&dom_processinginstruction_prop_handlers, &dom_node_prop_handlers, dom_copy_prop_handler, 0);
	crex_hash_add_ptr(&classes, dom_processinginstruction_class_entry->name, &dom_processinginstruction_prop_handlers);

#ifdef LIBXML_XPATH_ENABLED
	memcpy(&dom_xpath_object_handlers, &dom_object_handlers, sizeof(crex_object_handlers));
	...
	crex_hash_init(&dom_xpath_prop_handlers, 0, NULL, dom_dtor_prop_handler, 1);
	...
	crex_hash_add_ptr(&classes, dom_xpath_class_entry->name, &dom_xpath_prop_handlers);
#endif

	register_crx_dom_symbols(module_number);

	crx_libxml_register_export(dom_node_class_entry, crx_dom_export_node);
...
/* {{{ */
CRX_MINFO_FUNCTION(dom)
{
	crx_info_print_table_start();
	crx_info_print_table_row(2, "DOM/XML", "enabled");
	crx_info_print_table_row(2, "DOM/XML API Version", DOM_API_VERSION);
	crx_info_print_table_row(2, "libxml Version", LIBXML_DOTTED_VERSION);
#ifdef LIBXML_HTML_ENABLED
	crx_info_print_table_row(2, "HTML Support", "enabled");
#endif
#ifdef LIBXML_XPATH_ENABLED
	crx_info_print_table_row(2, "XPath Support", "enabled");
#endif
#ifdef LIBXML_XPTR_ENABLED
	crx_info_print_table_row(2, "XPointer Support", "enabled");
#endif
#ifdef LIBXML_SCHEMAS_ENABLED
	crx_info_print_table_row(2, "Schema Support", "enabled");
	crx_info_print_table_row(2, "RelaxNG Support", "enabled");
#endif
	crx_info_print_table_end();
}
/* }}} */

CRX_MSHUTDOWN_FUNCTION(dom) /* {{{ */
{
	crex_hash_destroy(&dom_document_prop_handlers);
	crex_hash_destroy(&dom_documentfragment_prop_handlers);
	crex_hash_destroy(&dom_node_prop_handlers);
	crex_hash_destroy(&dom_namespace_node_prop_handlers);
	crex_hash_destroy(&dom_nodelist_prop_handlers);
	crex_hash_destroy(&dom_namednodemap_prop_handlers);
	crex_hash_destroy(&dom_characterdata_prop_handlers);
	crex_hash_destroy(&dom_attr_prop_handlers);
	crex_hash_destroy(&dom_element_prop_handlers);
	crex_hash_destroy(&dom_text_prop_handlers);
	crex_hash_destroy(&dom_documenttype_prop_handlers);
	crex_hash_destroy(&dom_notation_prop_handlers);
	crex_hash_destroy(&dom_entity_prop_handlers);
	crex_hash_destroy(&dom_processinginstruction_prop_handlers);
#ifdef LIBXML_XPATH_ENABLED
	crex_hash_destroy(&dom_xpath_prop_handlers);
#endif
...
void node_list_unlink(xmlNodePtr node)
{
	...
	while (node != NULL) {
		...
		wrapper = crx_dom_object_get_data(node);
...
void dom_xpath_objects_free_storage(crex_object *object)
{
	dom_xpath_object *intern = crx_xpath_obj_from_obj(object);

	crex_object_std_dtor(&intern->dom.std);
...
void dom_objects_free_storage(crex_object *object)
{
	dom_object *intern = crx_dom_obj_from_obj(object);
	...
	if (intern->ptr != NULL && ((crx_libxml_node_ptr *)intern->ptr)->node != NULL) {
		if (((xmlNodePtr) ((crx_libxml_node_ptr *)intern->ptr)->node)->type != XML_DOCUMENT_NODE && ((xmlNodePtr) ((crx_libxml_node_ptr *)intern->ptr)->node)->type != XML_HTML_DOCUMENT_NODE) {
			crx_libxml_node_decrement_resource((crx_libxml_node_object *) intern);
		} else {
			crx_libxml_decrement_node_ptr((crx_libxml_node_object *) intern);
			retcount = crx_libxml_decrement_doc_ref((crx_libxml_node_object *)intern);
		}
		intern->ptr = NULL;
	}
}
...
static void dom_objects_set_class_ex(crex_class_entry *class_type, dom_object *intern)
{
	crex_class_entry *base_class = class_type;
	while ((base_class->type != CREX_INTERNAL_CLASS || base_class->info.internal.module->module_number != dom_module_entry.module_number) && base_class->parent != NULL) {
		base_class = base_class->parent;
	}

	intern->prop_handler = crex_hash_find_ptr(&classes, base_class->name);
...
static dom_object* dom_objects_set_class(crex_class_entry *class_type)
{
	dom_object *intern = crex_object_alloc(sizeof(dom_object), class_type);
	dom_objects_set_class_ex(class_type, intern);
	return intern;
}

/* {{{ dom_objects_new */
crex_object *dom_objects_new(crex_class_entry *class_type)
{
...
static crex_object *dom_objects_namespace_node_new(crex_class_entry *class_type)
{
	dom_object_namespace_node *intern = crex_object_alloc(sizeof(dom_object_namespace_node), class_type);
...
static void dom_object_namespace_node_free_storage(crex_object *object)
{
	dom_object_namespace_node *intern = crx_dom_namespace_node_obj_from_obj(object);
...
#ifdef LIBXML_XPATH_ENABLED
/* {{{ crex_object dom_xpath_objects_new(crex_class_entry *class_type) */
crex_object *dom_xpath_objects_new(crex_class_entry *class_type)
{
	dom_xpath_object *intern = crex_object_alloc(sizeof(dom_xpath_object), class_type);
	...
	crex_object_std_init(&intern->dom.std, class_type);
...
void dom_nnodemap_objects_free_storage(crex_object *object) /* {{{ */
{
	...
	if (objmap) {
		...
		if (!C_ISUNDEF(objmap->baseobj_zv)) {
	...
	crx_libxml_decrement_doc_ref((crx_libxml_node_object *)intern);

	crex_object_std_dtor(&intern->std);
}
/* }}} */

crex_object *dom_nnodemap_objects_new(crex_class_entry *class_type)
{
	dom_object *intern;
...
void crx_dom_create_iterator(zval *return_value, int ce_type) /* {{{ */
{
	crex_class_entry *ce;
...
CRX_DOM_EXPORT bool crx_dom_create_object(xmlNodePtr obj, zval *return_value, dom_object *domobj)
{
	...
	if ((intern = (dom_object *) crx_dom_object_get_data((void *) obj))) {
	...
	switch (obj->type) {
		...
		default:
			/* TODO Convert to a CREX assertion? */
			crex_throw_error(NULL, "Unsupported node type: %d", obj->type);
			ZVAL_NULL(return_value);
			return 0;
	}
	...
	intern = C_DOMOBJ_P(return_value);
	if (obj->doc != NULL) {
		if (domobj != NULL) {
			intern->document = domobj->document;
		}
		crx_libxml_increment_doc_ref((crx_libxml_node_object *)intern, obj->doc);
	}

	crx_libxml_increment_node_ptr((crx_libxml_node_object *)intern, obj, (void *)intern);
	return 0;
}
/* }}} end crx_domobject_new */

void crx_dom_create_implementation(zval *retval) {
...
/* {{{ */
bool dom_has_feature(crex_string *feature, crex_string *version)
{
	if (crex_string_equals_literal(version, "1.0")
		|| crex_string_equals_literal(version, "2.0")
		|| crex_string_equals_literal(version, "")
	) {
		if (crex_string_equals_literal_ci(feature, "XML")
			|| (crex_string_equals_literal_ci(feature, "Core") && crex_string_equals_literal(version, "1.0"))
		) {
			return true;
		}
	}
...
xmlNode *dom_get_elements_by_tag_name_ns_raw(xmlNodePtr basep, xmlNodePtr nodep, char *ns, char *local, int *cur, int index) /* {{{ */
{
	 ...
	 *       CRX however does not do this and internally uses the empty string everywhere when the user provides ns == NULL.
	 *       This is because for CRX ns == NULL has another meaning: "match every namespace" instead of "match the empty namespace". */
	...
	while (*cur <= index) {
		...
		if (nodep->next) {
			...
			do {
				...
				if (UNEXPECTED(nodep == NULL)) {
					crex_throw_error(NULL, "Current node in traversal is not in the document. Please report this as a bug in crx-src.");
					return NULL;
				}
			...
			case XML_TEXT_NODE:
				...
				while (nextp != NULL) {
					if (nextp->type == XML_TEXT_NODE) {
						...
						xmlFree(strContent);
						xmlUnlinkNode(nextp);
						crx_libxml_node_free_resource(nextp);
						nextp = newnextp;
					} else {
						break;
					}
				...
				if (is_empty_node(child)) {
					nextp = child->next;
					xmlUnlinkNode(child);
					crx_libxml_node_free_resource(child);
					child = nextp;
					continue;
				}
				break;
		...
		while (curns) {
			...
			if (curns->href != NULL) {
				if((nsptr = xmlSearchNsByHref(doc, search_parent, curns->href)) &&
					...
					crx_libxml_set_old_ns(doc, curns);
					curns = prevns;
				}
			}
			prevns = curns;
...
{
	/* Ideally we'd use the DOM-wrapped version, but we cannot: https://github.com/crx/crx-src/pull/12308. */
#if 0
...
xmlNsPtr dom_get_ns(xmlNodePtr nodep, char *uri, int *errorcode, char *prefix) {
	...
	if (! ((prefix && !strcmp (prefix, "xml") && strcmp(uri, (char *)XML_XML_NAMESPACE)) ||
		...
		if (UNEXPECTED(nsptr == NULL)) {
			 ...
			 * This isn't needed for every caller (such as createElementNS & DOMElement::__main), but isn't harmful and simplifies the mental model "when do I use which function?".
...
xmlNodePtr crx_dom_create_fake_namespace_decl(xmlNodePtr nodep, xmlNsPtr original, zval *return_value, dom_object *parent_intern)
{
	xmlNodePtr attrp = crx_dom_create_fake_namespace_decl_node_ptr(nodep, original);
	crx_dom_create_object(attrp, return_value, parent_intern);
	/* This object must exist, because we just created an object for it via crx_dom_create_object(). */
	crx_dom_namespace_node_obj_from_obj(C_OBJ_P(return_value))->parent_intern = parent_intern;
	return attrp;
}

static bool dom_nodemap_or_nodelist_process_offset_as_named(zval *offset, crex_long *lval)
{
	if (C_TYPE_P(offset) == IS_STRING) {
		/* See zval_get_long_func() */
		double dval;
		crex_uchar is_numeric_string_type;
		if (0 == (is_numeric_string_type = is_numeric_string(C_STRVAL_P(offset), C_STRLEN_P(offset), lval, &dval, true))) {
			return true;
...
static zval *dom_nodelist_read_dimension(crex_object *object, zval *offset, int type, zval *rv) /* {{{ */
{
	if (UNEXPECTED(!offset)) {
		crex_throw_error(NULL, "Cannot access DOMNodeList without offset");
		return NULL;
	}
	...
	crx_dom_nodelist_get_item_into_zval(crx_dom_obj_from_obj(object)->ptr, lval, rv);
	return rv;
...
static int dom_nodelist_has_dimension(crex_object *object, zval *member, int check_empty)
{
	...
	return offset >= 0 && offset < crx_dom_get_nodelist_length(crx_dom_obj_from_obj(object));
...
void dom_remove_all_children(xmlNodePtr nodep)
{
	if (nodep->children) {
		...
		crx_libxml_node_free_list((xmlNodePtr) nodep->children);
		nodep->children = NULL;
		nodep->last = NULL;
	}
}

void crx_dom_get_content_into_zval(const xmlNode *nodep, zval *retval, bool default_is_null)
{
	CREX_ASSERT(nodep != NULL);
...
{
	if (UNEXPECTED(!offset)) {
		crex_throw_error(NULL, "Cannot access DOMNamedNodeMap without offset");
		return NULL;
	}
	...
	if (dom_nodemap_or_nodelist_process_offset_as_named(offset, &lval)) {
		...
		crx_dom_named_node_map_get_named_item_into_zval(crx_dom_obj_from_obj(object)->ptr, C_STRVAL_P(offset), rv);
		return rv;
	}

	/* see CRX_METHOD(DOMNamedNodeMap, item) */
	if (UNEXPECTED(lval < 0 || CREX_LONG_INT_OVFL(lval))) {
		crex_value_error("must be between 0 and %d", INT_MAX);
		return NULL;
	}

	crx_dom_named_node_map_get_item_into_zval(crx_dom_obj_from_obj(object)->ptr, lval, rv);
	return rv;
...
static int dom_nodemap_has_dimension(crex_object *object, zval *member, int check_empty)
{
	...
	if (dom_nodemap_or_nodelist_process_offset_as_named(member, &offset)) {
		/* exceptional case, switch to named lookup */
		return crx_dom_named_node_map_get_named_item(crx_dom_obj_from_obj(object)->ptr, C_STRVAL_P(member), false) != NULL;
	}

	return offset >= 0 && offset < crx_dom_get_namednodemap_length(crx_dom_obj_from_obj(object));
