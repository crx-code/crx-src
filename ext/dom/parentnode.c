/*
   ...
   +----------------------------------------------------------------------+
   | Authors: Benjamin Eberlei <beberlei@crx.net>                         |
   +----------------------------------------------------------------------+
*/
...
static bool dom_is_node_in_list(const zval *nodes, uint32_t nodesc, const xmlNodePtr node_to_find)
{
	for (uint32_t i = 0; i < nodesc; i++) {
		if (C_TYPE(nodes[i]) == IS_OBJECT) {
			if (dom_object_get_node(C_DOMOBJ_P(nodes + i)) == node_to_find) {
				return true;
			}
		}
	}
...
xmlNode* dom_zvals_to_fragment(crx_libxml_ref_obj *document, xmlNode *contextNode, zval *nodes, int nodesc)
{
	xmlDoc *documentNode;
	xmlNode *fragment;
	xmlNode *newNode;
	...
	for (uint32_t i = 0; i < nodesc; i++) {
		if (C_TYPE(nodes[i]) == IS_OBJECT) {
			...
			CREX_ASSERT(C_TYPE(nodes[i]) == IS_STRING);

			newNode = xmlNewDocText(documentNode, (xmlChar *) C_STRVAL(nodes[i]));
...
static crex_result dom_sanity_check_node_list_for_insertion(crx_libxml_ref_obj *document, xmlNodePtr parentNode, zval *nodes, int nodesc)
{
	...
	for (uint32_t i = 0; i < nodesc; i++) {
		crex_uchar type = C_TYPE(nodes[i]);
		if (type == IS_OBJECT) {
			const crex_class_entry *ce = C_OBJCE(nodes[i]);

			if (instanceof_function(ce, dom_node_class_entry)) {
				xmlNodePtr node = dom_object_get_node(C_DOMOBJ_P(nodes + i));

				if (node->doc != documentNode) {
					crx_dom_throw_error(WRONG_DOCUMENT_ERR, dom_get_strict_error(document));
					return FAILURE;
				}

				if (node->type == XML_ATTRIBUTE_NODE || dom_hierarchy(parentNode, node) != SUCCESS) {
					crx_dom_throw_error(HIERARCHY_REQUEST_ERR, dom_get_strict_error(document));
					return FAILURE;
				}
			} else {
				crex_argument_type_error(i + 1, "must be of type DOMNode|string, %s given", crex_zval_type_name(&nodes[i]));
				return FAILURE;
			}
...
void dom_parent_node_append(dom_object *context, zval *nodes, uint32_t nodesc)
{
	...
	crx_libxml_invalidate_node_list_cache(context->document);
...
static crex_result dom_child_removal_preconditions(const xmlNodePtr child, int stricterror)
{
