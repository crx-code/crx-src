...
crex_string *dom_node_concatenated_name_helper(size_t name_len, const char *name, size_t prefix_len, const char *prefix)
{
	if (UNEXPECTED(prefix_len > ZSTR_MAX_LEN / 2 - 1 || name_len > ZSTR_MAX_LEN / 2 - 1)) {
		return crex_empty_string;
	}
	crex_string *str = crex_string_alloc(prefix_len + 1 + name_len, false);
...
crex_string *dom_node_get_node_name_attribute_or_element(const xmlNode *nodep)
{
	...
	if (nodep->ns != NULL && nodep->ns->prefix != NULL) {
		...
		return crex_string_init((const char *) nodep->name, name_len, false);
	}
}

bool crx_dom_is_node_connected(const xmlNode *node)
{
	CREX_ASSERT(node != NULL);
	do {
...
int dom_node_node_value_read(dom_object *obj, zval *retval)
{
	...
	switch (nodep->type) {
		...
		case XML_CDATA_SECTION_NODE:
		case XML_PI_NODE:
			crx_dom_get_content_into_zval(nodep, retval, true);
			break;
		case XML_NAMESPACE_DECL: {
...
int dom_node_node_value_write(dom_object *obj, zval *newval)
{
	...
	crx_libxml_invalidate_node_list_cache(obj->document);
...
static crex_result dom_node_parent_get(dom_object *obj, zval *retval, bool only_element)
{
	...
	crx_dom_create_object(nodeparent, retval, obj);
	return SUCCESS;
}
...
int dom_node_child_nodes_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_iterator(retval, DOM_NODELIST);
	intern = C_DOMOBJ_P(retval);
...
int dom_node_first_child_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object(first, retval, obj);
	return SUCCESS;
}
...
int dom_node_last_child_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object(last, retval, obj);
	return SUCCESS;
}
...
int dom_node_previous_sibling_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object(prevsib, retval, obj);
	return SUCCESS;
}
...
int dom_node_next_sibling_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object(nextsib, retval, obj);
	return SUCCESS;
}
...
int dom_node_attributes_read(dom_object *obj, zval *retval)
{
	...
	if (nodep->type == XML_ELEMENT_NODE) {
		crx_dom_create_iterator(retval, DOM_NAMEDNODEMAP);
		intern = C_DOMOBJ_P(retval);
...
int dom_node_is_connected_read(dom_object *obj, zval *retval)
{
	...
	ZVAL_BOOL(retval, crx_dom_is_node_connected(nodep));
	return SUCCESS;
}
/* }}} */
...
int dom_node_prefix_write(dom_object *obj, zval *newval)
{
	...
	switch (nodep->type) {
		...
		case XML_ATTRIBUTE_NODE:
			...
			prefix_str = C_STR_P(newval);

			prefix = ZSTR_VAL(prefix_str);
			if (nsnode && nodep->ns != NULL && !xmlStrEqual(nodep->ns->prefix, (xmlChar *)prefix)) {
				strURI = (char *) nodep->ns->href;
				if (strURI == NULL ||
					(crex_string_equals_literal(prefix_str, "xml") && strcmp(strURI, (char *) XML_XML_NAMESPACE)) ||
					(nodep->type == XML_ATTRIBUTE_NODE && crex_string_equals_literal(prefix_str, "xmlns") &&
				...
				if (ns == NULL) {
					crx_dom_throw_error(NAMESPACE_ERR, dom_get_strict_error(obj->document));
					return FAILURE;
				}
...
int dom_node_text_content_write(dom_object *obj, zval *newval)
{
	...
	/* Typed property, this is already a string */
	CREX_ASSERT(C_TYPE_P(newval) == IS_STRING);
	const xmlChar *xmlChars = (const xmlChar *) C_STRVAL_P(newval);
	int type = nodep->type;
...
static xmlNodePtr _crx_dom_insert_fragment(xmlNodePtr nodep, xmlNodePtr prevsib, xmlNodePtr nextsib, xmlNodePtr fragment, dom_object *intern, dom_object *childobj) /* {{{ */
{
		...
		while (node != NULL) {
			...
			if (node->doc != nodep->doc) {
				...
				if (node->_private != NULL) {
					...
					crx_libxml_increment_doc_ref((crx_libxml_node_object *)childobj, NULL);
				}
			}
...
Since:
*/
CRX_METHOD(DOMNode, insertBefore)
{
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O|O!", &node, dom_node_class_entry, &ref, dom_node_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (dom_node_is_read_only(parentp) == SUCCESS ||
		...
		crx_dom_throw_error(NO_MODIFICATION_ALLOWED_ERR, stricterror);
		RETURN_FALSE;
	}

	if (dom_hierarchy(parentp, child) == FAILURE) {
		crx_dom_throw_error(HIERARCHY_REQUEST_ERR, stricterror);
		RETURN_FALSE;
	}

	if (child->doc != parentp->doc && child->doc != NULL) {
		crx_dom_throw_error(WRONG_DOCUMENT_ERR, stricterror);
		RETURN_FALSE;
	}

	if (child->type == XML_DOCUMENT_FRAG_NODE && child->children == NULL) {
		/* TODO Drop Warning? */
		crx_error_docref(NULL, E_WARNING, "Document Fragment is empty");
		RETURN_FALSE;
	}
	...
	if (ref != NULL) {
		...
		if (refp->parent != parentp) {
			crx_dom_throw_error(NOT_FOUND_ERR, stricterror);
			RETURN_FALSE;
		}
		...
		if (child->type == XML_TEXT_NODE && (refp->type == XML_TEXT_NODE ||
			...
			if (lastattr != NULL && lastattr->type != XML_ATTRIBUTE_DECL) {
				if (lastattr != (xmlAttrPtr) child) {
					...
					crx_libxml_node_free_resource((xmlNodePtr) lastattr);
				} else {
			...
			new_child = _crx_dom_insert_fragment(parentp, refp->prev, refp, child, intern, childobj);
		...
		if (child->type == XML_TEXT_NODE && parentp->last != NULL && parentp->last->type == XML_TEXT_NODE) {
			...
			xmlNodePtr last = child->last;
			new_child = _crx_dom_insert_fragment(parentp, parentp->last, NULL, child, intern, childobj);
			dom_reconcile_ns_list(parentp->doc, new_child, last);
		} else {
			new_child = xmlAddChild(parentp, child);
			if (UNEXPECTED(NULL == new_child)) {
	...
	return;
cannot_add:
	crex_throw_error(NULL, "Cannot add newnode as the previous sibling of refnode");
	RETURN_THROWS();
}
...
Since:
*/
CRX_METHOD(DOMNode, replaceChild)
{
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "OO", &newnode, dom_node_class_entry, &oldnode, dom_node_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (newchild->type == XML_DOCUMENT_FRAG_NODE) {
		...
		newchild = _crx_dom_insert_fragment(nodep, prevsib, nextsib, newchild, intern, newchildobj);
		if (newchild) {
		...
		if (newchild->doc == NULL && nodep->doc != NULL) {
			...
			crx_libxml_increment_doc_ref((crx_libxml_node_object *)newchildobj, NULL);
		}
...
Since:
*/
CRX_METHOD(DOMNode, removeChild)
{
	zval *id, *node;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &node, dom_node_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
...
Since:
*/
CRX_METHOD(DOMNode, appendChild)
{
	zval *id, *node;
	...
	if (child->type == XML_TEXT_NODE && nodep->last != NULL && nodep->last->type == XML_TEXT_NODE) {
		...
		xmlNodePtr last = child->last;
		new_child = _crx_dom_insert_fragment(nodep, nodep->last, NULL, child, intern, childobj);
...
cannot_add:
	...
	crx_error_docref(NULL, E_WARNING, "Couldn't append node");
	RETURN_FALSE;
}
...
Since:
*/
CRX_METHOD(DOMNode, hasChildNodes)
{
	zval *id;
	xmlNode *nodep;
	dom_object *intern;
...
Since: DOM Level 2
*/
CRX_METHOD(DOMNode, isSupported)
{
	crex_string *feature, *version;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "SS", &feature, &version) == FAILURE) {
		RETURN_THROWS();
	}
...
Since: DOM Level 2
*/
CRX_METHOD(DOMNode, hasAttributes)
{
	zval *id;
	xmlNode *nodep;
	dom_object *intern;
...
static bool crx_dom_node_is_content_equal(const xmlNode *this, const xmlNode *other)
{
...
static bool crx_dom_node_is_ns_uri_equal(const xmlNode *this, const xmlNode *other)
{
...
static bool crx_dom_node_is_ns_prefix_equal(const xmlNode *this, const xmlNode *other)
{
...
static bool crx_dom_node_is_equal_node(const xmlNode *this, const xmlNode *other);

#define CRX_DOM_FUNC_CAT(prefix, suffix) prefix##_##suffix
/* xmlNode and xmlNs have incompatible struct layouts, i.e. the next field is in a different offset */
#define CRX_DOM_DEFINE_LIST_EQUALITY_HELPER(type)																\
	static size_t CRX_DOM_FUNC_CAT(crx_dom_node_count_list_size, type)(const type *node)						\
	{																											\
		size_t counter = 0;																						\
		while (node) {																							\
			counter++;																							\
	...
	static bool CRX_DOM_FUNC_CAT(crx_dom_node_list_equality_check, type)(const type *list1, const type *list2)	\
	{																											\
		size_t count = CRX_DOM_FUNC_CAT(crx_dom_node_count_list_size, type)(list1);								\
		if (count != CRX_DOM_FUNC_CAT(crx_dom_node_count_list_size, type)(list2)) {								\
			return false;																						\
		}																										\
		for (size_t i = 0; i < count; i++) {																	\
			if (!crx_dom_node_is_equal_node((const xmlNode *) list1, (const xmlNode *) list2)) {				\
				return false;																					\
			}																									\
			list1 = list1->next;																				\
			list2 = list2->next;																				\
...
CRX_DOM_DEFINE_LIST_EQUALITY_HELPER(xmlNode)
CRX_DOM_DEFINE_LIST_EQUALITY_HELPER(xmlNs)

static bool crx_dom_node_is_equal_node(const xmlNode *this, const xmlNode *other)
{
	CREX_ASSERT(this != NULL);
	CREX_ASSERT(other != NULL);
	...
	if (this->type == XML_ELEMENT_NODE) {
		return xmlStrEqual(this->name, other->name)
			&& crx_dom_node_is_ns_prefix_equal(this, other)
			&& crx_dom_node_is_ns_uri_equal(this, other)
			/* Check attributes first, then namespace declarations, then children */
			&& crx_dom_node_list_equality_check_xmlNode((const xmlNode *) this->properties, (const xmlNode *) other->properties)
			&& crx_dom_node_list_equality_check_xmlNs(this->nsDef, other->nsDef)
			&& crx_dom_node_list_equality_check_xmlNode(this->children, other->children);
		...
		return xmlStrEqual(this_attr->name, other_attr->name)
			&& crx_dom_node_is_ns_uri_equal(this, other)
			&& crx_dom_node_is_content_equal(this, other);
		...
		return crx_dom_node_list_equality_check_xmlNode(this->children, other->children);
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMNode, isEqualNode)
{
	zval *id, *node;
	...
	RETURN_BOOL(crx_dom_node_is_equal_node(nodep, otherp));
}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMNode, lookupPrefix)
{
	zval *id;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &uri, &uri_len) == FAILURE) {
		RETURN_THROWS();
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMNode, isDefaultNamespace)
{
	zval *id;
	xmlNodePtr nodep;
	dom_object *intern;
...
Since: DOM Level 3
*/
CRX_METHOD(DOMNode, lookupNamespaceURI)
{
	zval *id;
	xmlNodePtr nodep;
	dom_object *intern;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s!", &prefix, &prefix_len) == FAILURE) {
		RETURN_THROWS();
	}
...
static void dom_canonicalization(INTERNAL_FUNCTION_PARAMETERS, int mode) /* {{{ */
{
	...
	if (! docp) {
		crex_throw_error(NULL, "Node must be associated with a document");
		RETURN_THROWS();
	}

	crx_libxml_invalidate_node_list_cache_from_doc(docp);
	...
	if (xpath_array == NULL) {
		if (nodep->type != XML_DOCUMENT_NODE) {
			...
			if (xpathobjp && xpathobjp->type == XPATH_NODESET) {
				...
				crex_throw_error(NULL, "XPath query did not return a nodeset");
				RETURN_THROWS();
			}
		}
	} else {
		/*xpath query from xpath_array */
		HashTable *ht = C_ARRVAL_P(xpath_array);
		zval *tmp;
		char *xquery;

		/* Find "query" key */
		tmp = crex_hash_find(ht, ZSTR_KNOWN(CREX_STR_QUERY));
		if (!tmp) {
			/* if mode == 0 then $xpath arg is 3, if mode == 1 then $xpath is 4 */
			crex_argument_value_error(3 + mode, "must have a \"query\" key");
			RETURN_THROWS();
		}
		if (C_TYPE_P(tmp) != IS_STRING) {
			/* if mode == 0 then $xpath arg is 3, if mode == 1 then $xpath is 4 */
			crex_argument_type_error(3 + mode, "\"query\" option must be a string, %s given", crex_zval_value_name(tmp));
			RETURN_THROWS();
		}
		xquery = C_STRVAL_P(tmp);

		ctxp = xmlXPathNewContext(docp);
		ctxp->node = nodep;

		tmp = crex_hash_str_find(ht, "namespaces", sizeof("namespaces")-1);
		if (tmp && C_TYPE_P(tmp) == IS_ARRAY && !HT_IS_PACKED(C_ARRVAL_P(tmp))) {
			zval *tmpns;
			crex_string *prefix;

			CREX_HASH_MAP_FOREACH_STR_KEY_VAL(C_ARRVAL_P(tmp), prefix, tmpns) {
				if (C_TYPE_P(tmpns) == IS_STRING) {
					if (prefix) {
						xmlXPathRegisterNs(ctxp, (xmlChar *) ZSTR_VAL(prefix), (xmlChar *) C_STRVAL_P(tmpns));
					}
				}
	...
	if (ns_prefixes != NULL) {
		if (exclusive) {
			...
			inclusive_ns_prefixes = safe_emalloc(crex_hash_num_elements(C_ARRVAL_P(ns_prefixes)) + 1,
				sizeof(xmlChar *), 0);
			CREX_HASH_FOREACH_VAL(C_ARRVAL_P(ns_prefixes), tmpns) {
				if (C_TYPE_P(tmpns) == IS_STRING) {
					inclusive_ns_prefixes[nscount++] = (xmlChar *) C_STRVAL_P(tmpns);
				}
			} CREX_HASH_FOREACH_END();
...
CRX_METHOD(DOMNode, getNodePath)
{
	zval *id;
	xmlNode *nodep;
	dom_object *intern;
...
CRX_METHOD(DOMNode, contains)
{
	...
	if (UNEXPECTED(!instanceof_function(C_OBJCE_P(other), dom_node_class_entry) && !instanceof_function(C_OBJCE_P(other), dom_namespace_node_class_entry))) {
		crex_argument_type_error(1, "must be of type DOMNode|DOMNameSpaceNode|null, %s given", crex_zval_value_name(other));
		RETURN_THROWS();
	}
...
Since:
*/
CRX_METHOD(DOMNode, getRootNode)
{
	zval *id;
	xmlNodePtr thisp;
	dom_object *intern;
...
CRX_METHOD(DOMNode, __sleep)
{
	crex_throw_exception_ex(NULL, 0, "Serialization of '%s' is not allowed, unless serialization methods are implemented in a subclass", ZSTR_VAL(C_OBJCE_P(CREX_THIS)->name));
	RETURN_THROWS();
}

CRX_METHOD(DOMNode, __wakeup)
{
	crex_throw_exception_ex(NULL, 0, "Unserialization of '%s' is not allowed, unless unserialization methods are implemented in a subclass", ZSTR_VAL(C_OBJCE_P(CREX_THIS)->name));
	RETURN_THROWS();
}
