...
Since:
*/
CRX_METHOD(DOMImplementation, hasFeature)
{
...
Since: DOM Level 2
*/
CRX_METHOD(DOMImplementation, createDocumentType)
{
	xmlDtd *doctype;
	int ret;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|ss", &name, &name_len, &publicid, &publicid_len, &systemid, &systemid_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	crx_dom_throw_error(INVALID_CHARACTER_ERR,);
	*/
	...
	if (doctype == NULL) {
		crx_error_docref(NULL, E_WARNING, "Unable to create DocumentType");
		RETURN_FALSE;
	}
...
Since: DOM Level 2
*/
CRX_METHOD(DOMImplementation, createDocument)
{
	zval *node = NULL;
	xmlDoc *docp;
	xmlNode *nodep;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|s!sO!", &uri, &uri_len, &name, &name_len, &node, dom_documenttype_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (node != NULL) {
		...
		if (doctype->type == XML_DOCUMENT_TYPE_NODE) {
			crex_argument_value_error(3, "is an invalid DocumentType object");
			RETURN_THROWS();
		}
		if (doctype->doc != NULL) {
			crx_dom_throw_error(WRONG_DOCUMENT_ERR, 1);
			RETURN_THROWS();
		}
	} else {
		doctobj = NULL;
	...
	if (doctobj != NULL) {
		doctobj->document = ((dom_object *)((crx_libxml_node_ptr *)docp->_private)->_private)->document;
		crx_libxml_increment_doc_ref((crx_libxml_node_object *)doctobj, docp);
	}
}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMImplementation, getFeature)
{
	size_t feature_len, version_len;
	char *feature, *version;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss", &feature, &feature_len, &version, &version_len) == FAILURE) {
		RETURN_THROWS();
	}
