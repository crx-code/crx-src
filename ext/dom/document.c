...
int dom_document_doctype_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object((xmlNodePtr) dtdptr, retval, obj);
	return SUCCESS;
}
...
{
	crx_dom_create_implementation(retval);
	return SUCCESS;
}
...
int dom_document_document_element_read(dom_object *obj, zval *retval)
{
	...
	crx_dom_create_object(root, retval, obj);
	return SUCCESS;
}
...
crex_result dom_document_encoding_write(dom_object *obj, zval *newval)
{
	xmlDoc *docp = (xmlDocPtr) dom_object_get_node(obj);
	...
	CREX_ASSERT(C_TYPE_P(newval) == IS_STRING || C_TYPE_P(newval) == IS_NULL);
...
invalid_encoding:
	crex_value_error("Invalid document encoding");
	return FAILURE;
}
...
int dom_document_standalone_write(dom_object *obj, zval *newval)
{
	...
	docp->standalone = CREX_NORMALIZE_BOOL(standalone);
...
int dom_document_strict_error_checking_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		...
		doc_prop->stricterror = crex_is_true(newval);
	}
...
int dom_document_format_output_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->formatoutput = crex_is_true(newval);
	}
...
int dom_document_validate_on_parse_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->validateonparse = crex_is_true(newval);
	}
...
int dom_document_resolve_externals_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->resolveexternals = crex_is_true(newval);
	}
...
int dom_document_preserve_whitespace_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->preservewhitespace = crex_is_true(newval);
	}
...
int dom_document_recover_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->recover = crex_is_true(newval);
	}
...
int dom_document_substitue_entities_write(dom_object *obj, zval *newval)
{
	if (obj->document) {
		dom_doc_propsptr doc_prop = dom_get_doc_props(obj->document);
		doc_prop->substituteentities = crex_is_true(newval);
	}
...
Since:
*/
CRX_METHOD(DOMDocument, createElement)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
	...
	if (xmlValidateName((xmlChar *) name, 0) != 0) {
		crx_dom_throw_error(INVALID_CHARACTER_ERR, dom_get_strict_error(intern->document));
		RETURN_FALSE;
	}

	node = xmlNewDocNode(docp, NULL, (xmlChar *) name, (xmlChar *) value);
	if (!node) {
		crx_dom_throw_error(INVALID_STATE_ERR, /* strict */ true);
		RETURN_THROWS();
	}
...
Since:
*/
CRX_METHOD(DOMDocument, createDocumentFragment)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createTextNode)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createComment)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createCDATASection)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createProcessingInstruction)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createAttribute)
{
	zval *id;
	xmlAttrPtr node;
	xmlDocPtr docp;
...
Since:
*/
CRX_METHOD(DOMDocument, createEntityReference)
{
	zval *id;
	xmlNode *node;
	xmlDocPtr docp = NULL;
...
Since:
*/
CRX_METHOD(DOMDocument, getElementsByTagName)
{
	size_t name_len;
	...
	namednode = C_DOMOBJ_P(return_value);
...
Since: DOM Level 2
*/
CRX_METHOD(DOMDocument, importNode)
{
	zval *node;
	xmlDocPtr docp;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O|b", &node, dom_node_class_entry, &recursive) == FAILURE) {
		RETURN_THROWS();
	}

	DOM_GET_OBJ(docp, CREX_THIS, xmlDocPtr, intern);
	...
	if (nodep->type == XML_HTML_DOCUMENT_NODE || nodep->type == XML_DOCUMENT_NODE
		...
		crx_error_docref(NULL, E_WARNING, "Cannot import: Node Type Not Supported");
		RETURN_FALSE;
	}
	...
	crx_libxml_invalidate_node_list_cache(intern->document);
...
Since: DOM Level 2
*/
CRX_METHOD(DOMDocument, createElementNS)
{
	zval *id;
	xmlDocPtr docp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s!s|s", &uri, &uri_len, &name, &name_len, &value, &value_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (errorcode != 0) {
		...
		crx_dom_throw_error(errorcode, dom_get_strict_error(intern->document));
		RETURN_FALSE;
	}
...
Since: DOM Level 2
*/
CRX_METHOD(DOMDocument, createAttributeNS)
{
	zval *id;
	xmlDocPtr docp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s!s", &uri, &uri_len, &name, &name_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (root != NULL) {
		...
		crx_error_docref(NULL, E_WARNING, "Document Missing Root Element");
		RETURN_FALSE;
	}
...
Since: DOM Level 2
*/
CRX_METHOD(DOMDocument, getElementsByTagNameNS)
{
...
Since: DOM Level 2
*/
CRX_METHOD(DOMDocument, getElementById)
{
	zval *id;
	xmlDocPtr docp;
	xmlAttrPtr  attrp;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &idname, &idname_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (attrp && attrp->parent && crx_dom_is_node_connected(attrp->parent)) {
...
static void crx_dom_transfer_document_ref(xmlNodePtr node, dom_object *dom_object_document, xmlDocPtr document)
{
	if (node->children) {
		crx_dom_transfer_document_ref(node->children, dom_object_document, document);
	}
	while (node) {
		crx_libxml_node_ptr *iteration_object_ptr = node->_private;
		if (iteration_object_ptr) {
			crx_libxml_node_object *iteration_object = iteration_object_ptr->_private;
			CREX_ASSERT(iteration_object != NULL);
			...
			crx_libxml_decrement_doc_ref(iteration_object);
...
bool crx_dom_adopt_node(xmlNodePtr nodep, dom_object *dom_object_new_document, xmlDocPtr new_document)
{
	crx_libxml_invalidate_node_list_cache_from_doc(nodep->doc);
	if (nodep->doc != new_document) {
		crx_libxml_invalidate_node_list_cache(dom_object_new_document->document);
		...
		crx_dom_transfer_document_ref(nodep, dom_object_new_document, new_document);
	} else {
		xmlUnlinkNode(nodep);
	}
	return true;
...
*/
CRX_METHOD(DOMDocument, adoptNode)
{
	zval *node_zval;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &node_zval, dom_node_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (UNEXPECTED(nodep->type == XML_DOCUMENT_NODE
		...
		crx_dom_throw_error(NOT_SUPPORTED_ERR, dom_get_strict_error(dom_object_nodep->document));
		RETURN_FALSE;
	}

	xmlDocPtr new_document;
	dom_object *dom_object_new_document;
	zval *new_document_zval = CREX_THIS;
	DOM_GET_OBJ(new_document, new_document_zval, xmlDocPtr, dom_object_new_document);

	if (!crx_dom_adopt_node(nodep, dom_object_new_document, new_document)) {
		RETURN_FALSE;
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMDocument, normalizeDocument)
{
	zval *id;
	xmlDocPtr docp;
	dom_object *intern;
...
CRX_METHOD(DOMDocument, __main)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|ss", &version, &version_len, &encoding, &encoding_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (olddoc != NULL) {
		...
		refcount = crx_libxml_decrement_doc_ref((crx_libxml_node_object *)intern);
		if (refcount != 0) {
			olddoc->_private = NULL;
		}
	}
	intern->document = NULL;
	if (crx_libxml_increment_doc_ref((crx_libxml_node_object *)intern, docp) == -1) {
		/* docp is always non-null so crx_libxml_increment_doc_ref() never returns -1 */
		CREX_UNREACHABLE();
	}
	crx_libxml_increment_node_ptr((crx_libxml_node_object *)intern, (xmlNodePtr)docp, (void *)intern);
}
/* }}} end DOMDocument::__main */
...
static xmlDocPtr dom_document_parser(zval *id, int mode, char *source, size_t source_len, size_t options) /* {{{ */
{
	...
	crx_libxml_ref_obj *document = intern->document;
	...
	if (mode == DOM_LOAD_FILE) {
		...
		if (CHECK_NULL_PATH(source, source_len)) {
			crex_value_error("Path to document must not contain any null bytes");
			return NULL;
		}
	...
	ctxt->vctxt.error = crx_libxml_ctx_error;
	ctxt->vctxt.warning = crx_libxml_ctx_warning;

	if (ctxt->sax != NULL) {
		ctxt->sax->error = crx_libxml_ctx_error;
		ctxt->sax->warning = crx_libxml_ctx_warning;
	}
	...
	crx_libxml_sanitize_parse_ctxt_options(ctxt);
...
static void dom_finish_loading_document(zval *this, zval *return_value, xmlDocPtr newdoc)
{
	...
	dom_object *intern = C_DOMOBJ_P(this);
	size_t old_modification_nr = 0;
	if (intern != NULL) {
		xmlDocPtr docp = (xmlDocPtr) dom_object_get_node(intern);
		dom_doc_propsptr doc_prop = NULL;
		if (docp != NULL) {
			const crx_libxml_ref_obj *doc_ptr = intern->document;
			CREX_ASSERT(doc_ptr != NULL); /* Must exist, we have a document */
			old_modification_nr = doc_ptr->cache_tag.modification_nr;
			crx_libxml_decrement_node_ptr((crx_libxml_node_object *) intern);
			doc_prop = intern->document->doc_props;
			intern->document->doc_props = NULL;
			int refcount = crx_libxml_decrement_doc_ref((crx_libxml_node_object *)intern);
			if (refcount != 0) {
				docp->_private = NULL;
			}
		}
		intern->document = NULL;
		if (crx_libxml_increment_doc_ref((crx_libxml_node_object *)intern, newdoc) == -1) {
			RETURN_FALSE;
		}
		intern->document->doc_props = doc_prop;
	}

	crx_libxml_increment_node_ptr((crx_libxml_node_object *)intern, (xmlNodePtr)newdoc, (void *)intern);
...
static void dom_parse_document(INTERNAL_FUNCTION_PARAMETERS, int mode) {
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|l", &source, &source_len, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (CREX_SIZE_T_INT_OVFL(source_len)) {
		crx_error_docref(NULL, E_WARNING, "Input string is too long");
		RETURN_FALSE;
	}
	if (CREX_LONG_EXCEEDS_INT(options)) {
		crx_error_docref(NULL, E_WARNING, "Invalid options");
		RETURN_FALSE;
	}

	xmlDocPtr newdoc = dom_document_parser(CREX_THIS, mode, source, source_len, options);

	dom_finish_loading_document(CREX_THIS, return_value, newdoc);
}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMDocument, loadXML)
{
...
CRX_METHOD(DOMDocument, save)
{
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "p|l", &file, &file_len, &options) == FAILURE) {
		RETURN_THROWS();
	}
...
Since: DOM Level 3
*/
CRX_METHOD(DOMDocument, saveXML)
{
	zval *id, *nodep = NULL;
	xmlDoc *docp;
	xmlNode *node;
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|O!l", &nodep, dom_node_class_entry, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (nodep != NULL) {
		...
		if (node->doc != docp) {
			crx_dom_throw_error(WRONG_DOCUMENT_ERR, dom_get_strict_error(intern->document));
			RETURN_FALSE;
		}

		buf = xmlBufferCreate();
		if (!buf) {
			crx_error_docref(NULL, E_WARNING, "Could not fetch buffer");
			RETURN_FALSE;
		}
		...
		if (UNEXPECTED(!ctxt)) {
			xmlBufferFree(buf);
			crx_error_docref(NULL, E_WARNING, "Could not create save context");
			RETURN_FALSE;
		}
		if (UNEXPECTED(xmlSaveDoc(ctxt, docp) < 0)) {
			(void) xmlSaveClose(ctxt);
			xmlBufferFree(buf);
			crx_error_docref(NULL, E_WARNING, "Could not save document");
			RETURN_FALSE;
		}
...
static xmlNodePtr crx_dom_free_xinclude_node(xmlNodePtr cur) /* {{{ */
{
	xmlNodePtr xincnode;
	...
	crx_libxml_node_free_resource(xincnode);

	return cur;
}
/* }}} */

static void crx_dom_remove_xinclude_nodes(xmlNodePtr cur) /* {{{ */
{
	while(cur) {
		if (cur->type == XML_XINCLUDE_START) {
			cur = crx_dom_free_xinclude_node(cur);
			...
			while(cur && cur->type != XML_XINCLUDE_END) {
				...
				if (cur->type == XML_ELEMENT_NODE) {
					   crx_dom_remove_xinclude_nodes(cur->children);
				}
				cur = cur->next;
			}
...
CRX_METHOD(DOMDocument, xinclude)
{
	zval *id;
	xmlDoc *docp;
	xmlNodePtr root;
	...
	if (CREX_LONG_EXCEEDS_INT(flags)) {
		crx_error_docref(NULL, E_WARNING, "Invalid flags");
		RETURN_FALSE;
	}

	DOM_GET_OBJ(docp, id, xmlDocPtr, intern);

	CRX_LIBXML_SANITIZE_GLOBALS(xinclude);
	err = xmlXIncludeProcessFlags(docp, (int)flags);
	CRX_LIBXML_RESTORE_GLOBALS(xinclude);
	...
	if (root) {
		crx_dom_remove_xinclude_nodes(root);
	}
...
/* {{{ Since: DOM extended */
CRX_METHOD(DOMDocument, validate)
{
	zval *id;
	xmlDoc *docp;
	dom_object *intern;
	...
	CRX_LIBXML_SANITIZE_GLOBALS(validate);
	cvp = xmlNewValidCtxt();

	cvp->userData = NULL;
	cvp->error    = (xmlValidityErrorFunc) crx_libxml_error_handler;
	cvp->warning  = (xmlValidityErrorFunc) crx_libxml_error_handler;
	...
	CRX_LIBXML_RESTORE_GLOBALS(validate);
...
static void _dom_document_schema_validate(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|l", &source, &source_len, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	CRX_LIBXML_SANITIZE_GLOBALS(new_parser_ctxt);

	switch (type) {
	case DOM_LOAD_FILE:
		if (CHECK_NULL_PATH(source, source_len)) {
			CRX_LIBXML_RESTORE_GLOBALS(new_parser_ctxt);
		...
		if (!valid_file) {
			CRX_LIBXML_RESTORE_GLOBALS(new_parser_ctxt);
			crx_error_docref(NULL, E_WARNING, "Invalid Schema file source");
			RETURN_FALSE;
		}
	...
	xmlSchemaSetParserErrors(parser,
		(xmlSchemaValidityErrorFunc) crx_libxml_error_handler,
		(xmlSchemaValidityWarningFunc) crx_libxml_error_handler,
		parser);
	...
	if (!sptr) {
		if (!EG(exception)) {
			crx_error_docref(NULL, E_WARNING, "Invalid Schema");
		}
		RETURN_FALSE;
	}
	...
	if (!vptr) {
		xmlSchemaFree(sptr);
		crex_throw_error(NULL, "Invalid Schema Validation Context");
		RETURN_THROWS();
	}
	...
	xmlSchemaSetValidErrors(vptr, crx_libxml_error_handler, crx_libxml_error_handler, vptr);
...
/* {{{ */
CRX_METHOD(DOMDocument, schemaValidate)
{
...
/* {{{ */
CRX_METHOD(DOMDocument, schemaValidateSource)
{
...
static void _dom_document_relaxNG_validate(INTERNAL_FUNCTION_PARAMETERS, int type) /* {{{ */
{
	...
	id = CREX_THIS;
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &source, &source_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	case DOM_LOAD_FILE:
		...
		valid_file = _dom_get_valid_file_path(source, resolved_path, MAXPATHLEN);
		if (!valid_file) {
			crx_error_docref(NULL, E_WARNING, "Invalid RelaxNG file source");
			RETURN_FALSE;
		}
	...
	xmlRelaxNGSetParserErrors(parser,
		(xmlRelaxNGValidityErrorFunc) crx_libxml_error_handler,
		(xmlRelaxNGValidityWarningFunc) crx_libxml_error_handler,
		parser);
	sptr = xmlRelaxNGParse(parser);
	xmlRelaxNGFreeParserCtxt(parser);
	CRX_LIBXML_RESTORE_GLOBALS(parse);
	if (!sptr) {
		crx_error_docref(NULL, E_WARNING, "Invalid RelaxNG");
		RETURN_FALSE;
	}
	...
	if (!vptr) {
		xmlRelaxNGFree(sptr);
		crex_throw_error(NULL, "Invalid RelaxNG Validation Context");
		RETURN_THROWS();
	}

	xmlRelaxNGSetValidErrors(vptr, crx_libxml_error_handler, crx_libxml_error_handler, vptr);
...
/* {{{ */
CRX_METHOD(DOMDocument, relaxNGValidate)
{
...
/* {{{ */
CRX_METHOD(DOMDocument, relaxNGValidateSource)
{
...
/* {{{ Since: DOM extended */
CRX_METHOD(DOMDocument, loadHTMLFile)
{
...
/* {{{ Since: DOM extended */
CRX_METHOD(DOMDocument, loadHTML)
{
...
CRX_METHOD(DOMDocument, saveHTMLFile)
{
	zval *id;
	xmlDoc *docp;
	size_t file_len;
...
CRX_METHOD(DOMDocument, saveHTML)
{
	zval *id, *nodep = NULL;
	xmlDoc *docp;
	xmlNode *node;
	...
	if (nodep != NULL) {
		...
		if (!outBuf) {
			xmlBufferFree(buf);
			crx_error_docref(NULL, E_WARNING, "Could not fetch output buffer");
			RETURN_FALSE;
		}
		...
		if (!outBuf->error) {
			...
			crx_error_docref(NULL, E_WARNING, "Error dumping HTML node");
			RETVAL_FALSE;
		}
...
CRX_METHOD(DOMDocument, registerNodeClass)
{
	crex_class_entry *basece = dom_node_class_entry, *ce = NULL;
	dom_object *intern;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "CC!", &basece, &ce) == FAILURE) {
		RETURN_THROWS();
	}

	if (ce == NULL || instanceof_function(ce, basece)) {
		if (UNEXPECTED(ce != NULL && (ce->ce_flags & CREX_ACC_ABSTRACT))) {
			crex_argument_value_error(2, "must not be an abstract class");
			RETURN_THROWS();
		}
		DOM_GET_THIS_INTERN(intern);
	...
	crex_argument_error(NULL, 2, "must be a class name derived from %s or null, %s given", ZSTR_VAL(basece->name), ZSTR_VAL(ce->name));
}
/* }}} */
...
Since: DOM Living Standard (DOM4)
*/
CRX_METHOD(DOMDocument, prepend)
{
	uint32_t argc = 0;
	zval *args;
	dom_object *intern;
...
Since:
*/
CRX_METHOD(DOMDocument, replaceChildren)
{
	uint32_t argc = 0;
	zval *args;
	dom_object *intern;
