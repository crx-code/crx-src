...
#include "Crex/crex_extensions.h"
...
static crx_stream *oci_create_lob_stream(zval *dbh, pdo_stmt_t *stmt, OCILobLocator *lob);
...
static int oci_stmt_dtor(pdo_stmt_t *stmt) /* {{{ */
{
	...
	if (BC) {
		crex_hash_destroy(BC);
		FREE_HASHTABLE(stmt->bound_columns);
		stmt->bound_columns = NULL;
	}

	if (BP) {
		crex_hash_destroy(BP);
...
static sb4 oci_bind_input_cb(dvoid *ctx, OCIBind *bindp, ub4 iter, ub4 index, dvoid **bufpp, ub4 *alenp, ub1 *piecep, dvoid **indpp) /* {{{ */
{
	...
	*indpp = &P->indicator;
	...
	if (P->thing) {
		...
		*bufpp = C_STRVAL_P(parameter);
		*alenp = (ub4) C_STRLEN_P(parameter);
	}
...
static sb4 oci_bind_output_cb(dvoid *ctx, OCIBind *bindp, ub4 iter, ub4 index, dvoid **bufpp, ub4 **alenpp, ub1 *piecep, dvoid **indpp, ub2 **rcodepp) /* {{{ */
{
	...
	if (C_TYPE_P(parameter) == IS_OBJECT || C_TYPE_P(parameter) == IS_RESOURCE) {
		return OCI_CONTINUE;
	}

	zval_ptr_dtor(parameter);

	C_STR_P(parameter) = crex_string_alloc(param->max_value_len, 1);
	P->used_for_output = 1;

	P->actual_len = (ub4) C_STRLEN_P(parameter);
	*alenpp = &P->actual_len;
	*bufpp = (C_STR_P(parameter))->val;
	*piecep = OCI_ONE_PIECE;
	*rcodepp = &P->retcode;
	*indpp = &P->indicator;
		...
		switch (event_type) {
			...
			case PDO_PARAM_EVT_EXEC_POST:
				...
				if (P->used_for_output) {
					if (P->indicator == -1) {
						/* set up a NULL value */
						if (C_TYPE_P(parameter) == IS_STRING) {
							/* OCI likes to stick non-terminated strings in things */
							*C_STRVAL_P(parameter) = '\0';
						}
						zval_ptr_dtor_str(parameter);
						ZVAL_UNDEF(parameter);
					} else if (C_TYPE_P(parameter) == IS_STRING) {
						C_STR_P(parameter) = crex_string_init(C_STRVAL_P(parameter), P->actual_len, 1);
					}
					...
					if (C_TYPE_P(parameter) == IS_NULL) {
						...
						if (stm) {
							...
							crx_stream_to_zval(stm, parameter);
						}
					} else {
						...
						if (stm) {
							...
							do {
								char buf[8192];
								n = crx_stream_read(stm, buf, sizeof(buf));
								if ((int)n <= 0) {
									break;
								}
								consume = buf;
							...
							consume = C_STRVAL_P(parameter);
							n = C_STRLEN_P(parameter);
							if (n) {
								OCILobOpen(S->H->svc, S->err, (OCILobLocator*)P->thing, OCI_LOB_READWRITE);
								while (n) {
									amt = (ub4) n;
...
static int oci_stmt_fetch(pdo_stmt_t *stmt, enum pdo_fetch_orientation ori,	crex_long offset) /* {{{ */
{
...
static int oci_stmt_describe(pdo_stmt_t *stmt, int colno) /* {{{ */
{
	...
	col->name = crex_string_init((char *)colname, namelen, 0);
...
static ssize_t oci_blob_write(crx_stream *stream, const char *buf, size_t count)
{
...
static ssize_t oci_blob_read(crx_stream *stream, char *buf, size_t count)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
#if HAVE_OCILOBREAD2
...
static int oci_blob_close(crx_stream *stream, int close_handle)
{
	struct oci_lob_self *self = (struct oci_lob_self *)stream->abstract;
	pdo_stmt_t *stmt = self->stmt;

	if (close_handle) {
		crex_object *obj = &stmt->std;
...
static int oci_blob_flush(crx_stream *stream)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
...
static int oci_blob_seek(crx_stream *stream, crex_off_t offset, int whence, crex_off_t *newoffset)
{
	struct oci_lob_self *self = (struct oci_lob_self*)stream->abstract;
	...
	if (offset >= PDO_OCI_LOBMAXSIZE) {
		...
		self->offset = (ub4) offset + 1;  /* Oracle LOBS are 1-based, but CRX is 0-based */
		return 0;
	}
}

static const crx_stream_ops oci_blob_stream_ops = {
	oci_blob_write,
	oci_blob_read,
	oci_blob_close,
	oci_blob_flush,
...
static crx_stream *oci_create_lob_stream(zval *dbh, pdo_stmt_t *stmt, OCILobLocator *lob)
{
	...
	stm = crx_stream_alloc(&oci_blob_stream_ops, self, 0, "r+b");
...
static int oci_stmt_get_col(pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type) /* {{{ */
{
	...
	if (C->indicator == -1) {
		...
		if (C->dtype == SQLT_BLOB || C->dtype == SQLT_CLOB) {
			if (C->data) {
				crx_stream *stream = oci_create_lob_stream(&stmt->database_object_handle, stmt, (OCILobLocator*)C->data);
		...
		/* it was truncated */
		crx_error_docref(NULL, E_WARNING, "Column %d data was too large for buffer and was truncated to fit it", colno);
...
static int oci_stmt_col_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value) /* {{{ */
{
	pdo_oci_stmt *S = (pdo_oci_stmt*)stmt->driver_data;
	OCIParam *param = NULL;
	ub2 dtype, precis;
