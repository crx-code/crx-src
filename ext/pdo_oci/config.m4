...
CRX_PDO_OCI_TAIL1=`echo a | tail -n1 2>/dev/null`
if test "$CRX_PDO_OCI_TAIL1" = "a"; then
    CRX_PDO_OCI_TAIL1="tail -n1"
else
    CRX_PDO_OCI_TAIL1="tail -1"
fi
...
AC_DEFUN([AC_PDO_OCI_VERSION],[
  ...
  PDO_OCI_LCS=`ls $PDO_OCI_LCS_BASE.*.1 2> /dev/null | $CRX_PDO_OCI_TAIL1`
  if test -f "$PDO_OCI_LCS"; then
    ...
    PDO_OCI_VERSION=`echo $PDO_OCI_LCS | $CRX_PDO_OCI_SED -e 's/.*\.\(.*\)\.1$/\1.1/'`
...
if test "$CRX_PDO_OCI" != "no"; then
  ...
  if test "$CRX_PDO_OCI" = "yes" || test -z "$CRX_PDO_OCI"; then
    PDO_OCI_DIR=$ORACLE_HOME
  else
    PDO_OCI_DIR=$CRX_PDO_OCI
  fi
  AC_MSG_RESULT($CRX_PDO_OCI)
  ...
  if test "instantclient" = "`echo $PDO_OCI_DIR | cut -d, -f1`" ; then
    ...
    OCISDKRPMINC=`echo "$PDO_OCI_LIB_DIR" | $CRX_PDO_OCI_SED -e 's!^\(.*\)/lib/oracle/\(.*\)/\('${PDO_OCI_CLIENT_DIR}'\)/lib[/]*$!\1/include/oracle/\2/\3!'`

    dnl Header directory for manual installation
    OCISDKMANINC=`echo "$PDO_OCI_LIB_DIR" | $CRX_PDO_OCI_SED -e 's!^\(.*\)/lib[/]*$!\1/include!'`
  ...
  CRX_ADD_LIBRARY(clntsh, 1, PDO_OCI_SHARED_LIBADD)
  CRX_ADD_LIBPATH($PDO_OCI_LIB_DIR, PDO_OCI_SHARED_LIBADD)

  CRX_CHECK_LIBRARY(clntsh, OCIEnvCreate,
  [
  ...
  CRX_CHECK_LIBRARY(clntsh, OCIEnvNlsCreate,
  [
  ...
  CRX_CHECK_LIBRARY(clntsh, OCIStmtFetch2,
  [
  ...
  CRX_CHECK_LIBRARY(clntsh, OCILobRead2,
  [
  ...
  CRX_NEW_EXTENSION(pdo_oci, pdo_oci.c oci_driver.c oci_statement.c, $ext_shared,,-I$pdo_cv_inc_path)

  CRX_SUBST_OLD(PDO_OCI_SHARED_LIBADD)
  CRX_SUBST_OLD(PDO_OCI_DIR)
  CRX_SUBST_OLD(PDO_OCI_VERSION)

  CRX_ADD_EXTENSION_DEP(pdo_oci, pdo)

  AC_DEFINE_UNQUOTED(CRX_PDO_OCI_CLIENT_VERSION, "$PDO_OCI_VERSION", [ ])
fi
