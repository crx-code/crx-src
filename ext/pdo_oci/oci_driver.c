...
ub4 _oci_error(OCIError *err, pdo_dbh_t *dbh, pdo_stmt_t *stmt, char *what, sword status, int isinit, const char *file, int line) /* {{{ */
{
	...
	if (!dbh->methods && status != OCI_SUCCESS_WITH_INFO) {
		crex_throw_exception_ex(crx_pdo_get_exception(), einfo->errcode, "SQLSTATE[%s]: %s", *pdo_err, einfo->errmsg);
	}
...
static bool oci_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
...
static crex_long oci_handle_doer(pdo_dbh_t *dbh, const crex_string *sql) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	OCIStmt		*stmt;
	ub2 stmt_type;
	...
	if (stmt_type == OCI_STMT_SELECT) {
		...
		OCIHandleFree(stmt, OCI_HTYPE_STMT);
		crx_error_docref(NULL, E_WARNING, "issuing a SELECT query here is invalid");
		return -1;
	}
...
static crex_string* oci_handle_quoter(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype ) /* {{{ */
{
	int qcount = 0;
	char const *cu, *l, *r;
	char *c, *quoted;
...
static bool oci_handle_set_attribute(pdo_dbh_t *dbh, crex_long attr, zval *val) /* {{{ */
{
	crex_long lval;
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	...
	switch (attr) {
		...
		case PDO_OCI_ATTR_ACTION:
		{
			...
			crex_string *action = zval_try_get_string(val);
		...
		{
#if (OCI_MAJOR_VERSION >= 10)
			crex_string *client_info = zval_try_get_string(val);
		...
		{
#if (OCI_MAJOR_VERSION >= 10)
			crex_string *identifier = zval_try_get_string(val);
		...
		{
#if (OCI_MAJOR_VERSION >= 10)
			crex_string *module = zval_try_get_string(val);
...
static int oci_handle_get_attribute(pdo_dbh_t *dbh, crex_long attr, zval *return_value)  /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	...
	switch (attr) {
		...
		case PDO_ATTR_CLIENT_VERSION:
		{
			...
			ZVAL_STRING(return_value, verstr);
#elif defined(CRX_PDO_OCI_CLIENT_VERSION)
			/* Compile time client version */
			ZVAL_STRING(return_value, CRX_PDO_OCI_CLIENT_VERSION);
#else
			return FALSE;
		...
		case PDO_OCI_ATTR_CALL_TIMEOUT:
		{
			...
			ZVAL_LONG(return_value, (crex_long) timeout);
			return TRUE;
#else
			oci_drv_error("Unsupported attribute type");
			return FALSE;
...
static crex_result pdo_oci_check_liveness(pdo_dbh_t *dbh) /* {{{ */
{
	pdo_oci_db_handle *H = (pdo_oci_db_handle *)dbh->driver_data;
	sb4 error_code = 0;
#if (!((OCI_MAJOR_VERSION > 10) || ((OCI_MAJOR_VERSION == 10) && (OCI_MINOR_VERSION >= 2))))
...
static int pdo_oci_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	...
	crx_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, 4);
	...
	if (vars[0].optval) {
		...
		if (!H->charset) {
			...
			if (OCIEnvNlsCreate(&H->env, PDO_OCI_INIT_MODE, 0, NULL, NULL, NULL, 0, NULL, H->charset, H->charset) != OCI_SUCCESS) {
				oci_init_error("OCIEnvNlsCreate: Check the character set is valid and that CRX has access to Oracle libraries and NLS data");
				goto cleanup;
			}
		}
	}
