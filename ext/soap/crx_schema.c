...
static encodePtr create_encoder(sdlPtr sdl, sdlTypePtr cur_type, const xmlChar *ns, const xmlChar *type)
{
	...
	if (sdl->encoders == NULL) {
		...
		crex_hash_init(sdl->encoders, 0, NULL, delete_encoder, 0);
	}
	smart_str_appends(&nscat, (char*)ns);
	smart_str_appendc(&nscat, ':');
	smart_str_appends(&nscat, (char*)type);
	smart_str_0(&nscat);
	if ((enc_ptr = crex_hash_find_ptr(sdl->encoders, nscat.s)) != NULL) {
		enc = enc_ptr;
		if (enc->details.ns) {
	...
	if (enc_ptr == NULL) {
		crex_hash_update_ptr(sdl->encoders, nscat.s, enc);
	}
...
static void schema_load_file(sdlCtx *ctx, xmlAttrPtr ns, xmlChar *location, xmlAttrPtr tns, int import) {
	if (location != NULL &&
	    !crex_hash_str_exists(&ctx->docs, (char*)location, xmlStrlen(location))) {
		xmlDocPtr doc;
		xmlNodePtr schema;
		xmlAttrPtr new_tns;
		...
		crex_hash_str_add_ptr(&ctx->docs, (char*)location, xmlStrlen(location), doc);
...
int load_schema(sdlCtx *ctx, xmlNodePtr schema)
{
	...
	if (!ctx->sdl->types) {
		...
		crex_hash_init(ctx->sdl->types, 0, NULL, delete_type, 0);
	}
	if (!ctx->attributes) {
		ctx->attributes = emalloc(sizeof(HashTable));
		crex_hash_init(ctx->attributes, 0, NULL, delete_attribute, 0);
	}
	if (!ctx->attributeGroups) {
		ctx->attributeGroups = emalloc(sizeof(HashTable));
		crex_hash_init(ctx->attributeGroups, 0, NULL, delete_type, 0);
	}
...
static int schema_simpleType(sdlPtr sdl, xmlAttrPtr tns, xmlNodePtr simpleType, sdlTypePtr cur_type)
{
	...
	if (cur_type != NULL) {
		...
		ptr = crex_hash_next_index_insert_ptr(sdl->types,  newType);
		...
		crex_hash_next_index_insert_ptr(sdl->encoders,  cur_type->encode);
		...
		if (cur_type == NULL) {
			...
			if (cur_type->elements == NULL) {
				...
				crex_hash_init(cur_type->elements, 0, NULL, delete_type, 0);
			}
			ptr = crex_hash_str_update_ptr(cur_type->elements, newType->name, strlen(newType->name), newType);
		}
		cur_type = ptr;
...
static int schema_list(sdlPtr sdl, xmlAttrPtr tns, xmlNodePtr listType, sdlTypePtr cur_type)
{
	...
	if (itemType != NULL) {
		...
		if (nsptr != NULL) {
			...
			crex_hash_next_index_insert_ptr(cur_type->elements, newType);
		}
		if (type) {efree(type);}
		if (ns) {efree(ns);}
	}
	...
	if (trav != NULL && node_is_equal(trav,"simpleType")) {
...

		{
			...
			char *res = crex_print_long_to_buf(buf + sizeof(buf) - 1, crex_hash_num_elements(sdl->types));
	...
	while (trav != NULL) {
		if (node_is_equal(trav, "minExclusive")) {
			...
			if (cur_type->restrictions->enumeration == NULL) {
				...
				crex_hash_init(cur_type->restrictions->enumeration, 0, NULL, delete_restriction_var_char, 0);
			}
			if (crex_hash_str_add_ptr(cur_type->restrictions->enumeration, enumval->value, strlen(enumval->value), enumval) == NULL) {
...
static int schema_all(sdlPtr sdl, xmlAttrPtr tns, xmlNodePtr all, sdlTypePtr cur_type, sdlContentModelPtr model)
{
	...
	crex_hash_init(newModel->u.content, 0, NULL, delete_model, 0);
	if (model == NULL) {
		cur_type->model = newModel;
	} else {
		crex_hash_next_index_insert_ptr(model->u.content, newModel);
	}
	...
	if (name) {
		...
		if (cur_type == NULL) {
			...
			if (sdl->groups == NULL) {
				...
				crex_hash_init(sdl->groups, 0, NULL, delete_type, 0);
			}
			if (crex_hash_add_ptr(sdl->groups, key.s, newType) == NULL) {
	...
	if (name) {
		...
		if (cur_type == NULL) {
			if (sdl->elements == NULL) {
				...
				crex_hash_init(sdl->elements, 0, NULL, delete_type, 0);
			}
		...
		smart_str_0(&key);
		if (crex_hash_add_ptr(addHash, key.s, newType) == NULL) {
			if (cur_type == NULL) {
				soap_error1(E_ERROR, "Parsing Schema: element '%s' already defined", ZSTR_VAL(key.s));
			} else {
				crex_hash_next_index_insert_ptr(addHash, newType);
			}
		}
		smart_str_free(&key);
	...
	if (name) {
		...
		if (cur_type == NULL) {
			...
			if (cur_type->attributes == NULL) {
				...
				crex_hash_init(cur_type->attributes, 0, NULL, delete_attribute, 0);
			}
			addHash = cur_type->attributes;
		}

		if (crex_hash_add_ptr(addHash, key.s, newAttr) == NULL) {
		...
		if (attr_is_equal_ex(attr, "default", SCHEMA_NAMESPACE)) {
			...
			if (strncmp((char*)nsPtr->href, SCHEMA_NAMESPACE, sizeof(SCHEMA_NAMESPACE))) {
				...
				if (!newAttr->extraAttributes) {
					...
					crex_hash_init(newAttr->extraAttributes, 0, NULL, delete_extra_attribute, 0);
				}
				...
				smart_str_0(&key2);
				crex_hash_add_ptr(newAttr->extraAttributes, key2.s, ext);
				smart_str_free(&key2);
			}
		}
		attr = attr->next;
...
static int schema_attributeGroup(sdlPtr sdl, xmlAttrPtr tns, xmlNodePtr attrGroup, sdlTypePtr cur_type, sdlCtx *ctx)
{
	...
	if (name) {
		if (cur_type == NULL) {
			...
			if (crex_hash_add_ptr(ctx->attributeGroups, key.s, newType) == NULL) {
			...
			crex_hash_next_index_insert_ptr(cur_type->attributes, newAttr);
			cur_type = NULL;
		}
	} else{
...
static void copy_extra_attribute(zval *zv)
{
	...
	memcpy(new_attr, C_PTR_P(zv), sizeof(sdlExtraAttribute));
	C_PTR_P(zv) = new_attr;
	if (new_attr->ns) {
...
static void* schema_find_by_ref(HashTable *ht, char *ref)
{
	...
	if ((tmp = crex_hash_str_find_ptr(ht, ref, strlen(ref))) != NULL) {
		return tmp;
	} else {
		ref = strrchr(ref, ':');
		if (ref) {
		...
		if (ctx->attributes != NULL) {
			...
			if (tmp) {
				...
				if (tmp->extraAttributes != NULL) {
					...
					crex_hash_init(attr->extraAttributes, crex_hash_num_elements(tmp->extraAttributes), NULL, delete_extra_attribute, 0);
					crex_hash_copy(attr->extraAttributes, tmp->extraAttributes, copy_extra_attribute);
				}
...
static void schema_attributegroup_fixup(sdlCtx *ctx, sdlAttributePtr attr, HashTable *ht)
{
	...
	if (attr->ref != NULL) {
		if (ctx->attributeGroups != NULL) {
			...
			if (tmp) {
				if (tmp->attributes) {
					crex_hash_internal_pointer_reset(tmp->attributes);
					while ((tmp_attr = crex_hash_get_current_data_ptr(tmp->attributes)) != NULL) {
						if (crex_hash_get_current_key_type(tmp->attributes) == HASH_KEY_IS_STRING) {
							crex_string* _key;
							sdlAttributePtr newAttr;
							...
							if (newAttr->extraAttributes) {
								...
								crex_hash_init(ht, crex_hash_num_elements(newAttr->extraAttributes), NULL, delete_extra_attribute, 0);
								crex_hash_copy(ht, newAttr->extraAttributes, copy_extra_attribute);
								newAttr->extraAttributes = ht;
							}

							crex_hash_get_current_key(tmp->attributes, &_key, NULL);
							crex_hash_add_ptr(ht, _key, newAttr);

							crex_hash_move_forward(tmp->attributes);
						} else {
							crex_ulong index;

							schema_attributegroup_fixup(ctx, tmp_attr, ht);
							crex_hash_get_current_key(tmp->attributes, NULL, &index);
							crex_hash_index_del(tmp->attributes, index);
						}
					}
				}
			}
...
static void schema_content_model_fixup(sdlCtx *ctx, sdlContentModelPtr model)
{
	switch (model->kind) {
		case XSD_CONTENT_GROUP_REF: {
			...
			if (ctx->sdl->groups && (tmp = crex_hash_str_find_ptr(ctx->sdl->groups, model->u.group_ref, strlen(model->u.group_ref))) != NULL) {
...
static void schema_type_fixup(sdlCtx *ctx, sdlTypePtr type)
{
	...
	if (type->elements) {
		CREX_HASH_FOREACH_PTR(type->elements, tmp) {
			schema_type_fixup(ctx, tmp);
		} CREX_HASH_FOREACH_END();
	}
	if (type->model) {
	...
	if (type->attributes) {
		HashPosition pos;
		crex_hash_internal_pointer_reset_ex(type->attributes, &pos);

		while ((attr = crex_hash_get_current_data_ptr_ex(type->attributes, &pos)) != NULL) {
			crex_string *str_key;
			crex_ulong index;

			if (crex_hash_get_current_key_ex(type->attributes, &str_key, &index, &pos) == HASH_KEY_IS_STRING) {
				schema_attribute_fixup(ctx, attr);
				crex_result result = crex_hash_move_forward_ex(type->attributes, &pos);
				CREX_ASSERT(result == SUCCESS);
			} else {
				schema_attributegroup_fixup(ctx, attr, type->attributes);
				crex_result result = crex_hash_index_del(type->attributes, index);
				CREX_ASSERT(result == SUCCESS);
			}
		}
	}
...
void schema_pass2(sdlCtx *ctx)
{
	...
	if (ctx->attributes) {
		CREX_HASH_FOREACH_PTR(ctx->attributes, attr) {
			schema_attribute_fixup(ctx, attr);
		} CREX_HASH_FOREACH_END();
	}
	if (ctx->attributeGroups) {
		CREX_HASH_MAP_FOREACH_PTR(ctx->attributeGroups, type) {
			schema_type_fixup(ctx, type);
		} CREX_HASH_FOREACH_END();
	}
	if (sdl->elements) {
		CREX_HASH_MAP_FOREACH_PTR(sdl->elements, type) {
			schema_type_fixup(ctx, type);
		} CREX_HASH_FOREACH_END();
	}
	if (sdl->groups) {
		CREX_HASH_MAP_FOREACH_PTR(sdl->groups, type) {
			schema_type_fixup(ctx, type);
		} CREX_HASH_FOREACH_END();
	}
	if (sdl->types) {
	...
	if (ctx->attributes) {
		crex_hash_destroy(ctx->attributes);
		efree(ctx->attributes);
	}
	if (ctx->attributeGroups) {
		crex_hash_destroy(ctx->attributeGroups);
...
{
	sdlContentModelPtr tmp = C_PTR_P(zv);
	switch (tmp->kind) {
		...
		case XSD_CONTENT_ALL:
		case XSD_CONTENT_CHOICE:
			crex_hash_destroy(tmp->u.content);
			efree(tmp->u.content);
			break;
		case XSD_CONTENT_GROUP_REF:
...
{
	delete_model_persistent_int(C_PTR_P(zv));
}

void delete_type(zval *zv)
{
	sdlTypePtr type = C_PTR_P(zv);
	...
	if (type->elements) {
		crex_hash_destroy(type->elements);
		efree(type->elements);
	}
	if (type->attributes) {
		crex_hash_destroy(type->attributes);
	...
	if (type->restrictions) {
		...
		if (type->restrictions->enumeration) {
			crex_hash_destroy(type->restrictions->enumeration);
...
{
	sdlExtraAttributePtr attr = C_PTR_P(zv);
...
{
	sdlAttributePtr attr = C_PTR_P(zv);
	...
	if (attr->extraAttributes) {
		crex_hash_destroy(attr->extraAttributes);
...
{
	delete_restriction_var_char_int(C_PTR_P(zv));
}
...
{
	delete_restriction_var_char_persistent_int(C_PTR_P(zv));
}
