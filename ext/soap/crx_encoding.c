...
static encodePtr find_encoder_by_type_name(sdlPtr sdl, const char *type)
{
	if (sdl && sdl->encoders) {
		...
		CREX_HASH_FOREACH_PTR(sdl->encoders, enc)  {
...
static bool soap_check_zval_ref(zval *data, xmlNodePtr node) {
	...
	if (SOAP_GLOBAL(ref_map)) {
		if (C_TYPE_P(data) == IS_OBJECT) {
			data = (zval*)C_OBJ_P(data);
		}
		if ((node_ptr = crex_hash_index_find_ptr(SOAP_GLOBAL(ref_map), (crex_ulong)data)) != NULL) {
			...
			crex_hash_index_update_ptr(SOAP_GLOBAL(ref_map), (crex_ulong)data, node);
		}
	}
	return 0;
}
...
static bool soap_check_xml_ref(zval *data, xmlNodePtr node)
{
	...
	if (SOAP_GLOBAL(ref_map)) {
		if ((data_ptr = crex_hash_index_find(SOAP_GLOBAL(ref_map), (crex_ulong)node)) != NULL) {
			if (!C_REFCOUNTED_P(data) ||
			    !C_REFCOUNTED_P(data_ptr) ||
			    C_COUNTED_P(data) != C_COUNTED_P(data_ptr)) {
				zval_ptr_dtor(data);
...
{
	if (SOAP_GLOBAL(ref_map)) {
		crex_hash_index_update(SOAP_GLOBAL(ref_map), (crex_ulong)node, data);
	}
}
...
static xmlNodePtr master_to_xml_int(encodePtr encode, zval *data, int style, xmlNodePtr parent, int check_class_map)
{
	...
	if (data && C_TYPE_P(data) == IS_OBJECT && C_OBJCE_P(data) == soap_var_class_entry) {
		encodePtr enc = NULL;

		zval *ztype = C_VAR_ENC_TYPE_P(data);
		if (C_TYPE_P(ztype) != IS_LONG) {
			soap_error0(E_ERROR, "Encoding: SoapVar has no 'enc_type' property");
		}

		zval *zstype = C_VAR_ENC_STYPE_P(data);
		if (C_TYPE_P(zstype) == IS_STRING) {
			zval *zns = C_VAR_ENC_NS_P(data);
			if (C_TYPE_P(zns) == IS_STRING) {
				enc = get_encoder(SOAP_GLOBAL(sdl), C_STRVAL_P(zns), C_STRVAL_P(zstype));
			} else {
				zns = NULL;
				enc = get_encoder_ex(SOAP_GLOBAL(sdl), C_STRVAL_P(zstype), C_STRLEN_P(zstype));
			}
			if (enc == NULL && SOAP_GLOBAL(typemap)) {
				smart_str nscat = {0};

				if (zns != NULL) {
					smart_str_append(&nscat, C_STR_P(zns));
					smart_str_appendc(&nscat, ':');
				}
				smart_str_append(&nscat, C_STR_P(zstype));
				smart_str_0(&nscat);
				enc = crex_hash_find_ptr(SOAP_GLOBAL(typemap), nscat.s);
				smart_str_free(&nscat);
			}
		}
		if (enc == NULL) {
			enc = get_conversion(C_LVAL_P(ztype));
		}
		if (enc == NULL) {
			enc = encode;
		}

		node = master_to_xml(enc, C_VAR_ENC_VALUE_P(data), style, parent);
		...
		if (style == SOAP_ENCODED || (SOAP_GLOBAL(sdl) && encode != enc)) {
			...
			if (C_TYPE_P(zstype) == IS_STRING) {
				zval *zns = C_VAR_ENC_NS_P(data);
				if (C_TYPE_P(zns) == IS_STRING) {
					set_ns_and_type_ex(node, C_STRVAL_P(zns), C_STRVAL_P(zstype));
				} else {
					set_ns_and_type_ex(node, NULL, C_STRVAL_P(zstype));
				}
			}
		}

		zval *zname = C_VAR_ENC_NAME_P(data);
		if (C_TYPE_P(zname) == IS_STRING) {
			xmlNodeSetName(node, BAD_CAST(C_STRVAL_P(zname)));
		}

		zval *znamens = C_VAR_ENC_NAMENS_P(data);
		if (C_TYPE_P(znamens) == IS_STRING) {
			xmlNsPtr nsp = encode_add_ns(node, C_STRVAL_P(znamens));
			xmlSetNs(node, nsp);
		}
	} else {
		if (check_class_map && SOAP_GLOBAL(class_map) && data &&
		    C_TYPE_P(data) == IS_OBJECT &&
		    !GC_IS_RECURSIVE(C_OBJPROP_P(data))) {
			crex_class_entry *ce = C_OBJCE_P(data);
			zval *tmp;
			crex_string *type_name;

			CREX_HASH_MAP_FOREACH_STR_KEY_VAL(SOAP_GLOBAL(class_map), type_name, tmp) {
				ZVAL_DEREF(tmp);
				if (C_TYPE_P(tmp) == IS_STRING &&
				    ZSTR_LEN(ce->name) == C_STRLEN_P(tmp) &&
				    crex_binary_strncasecmp(ZSTR_VAL(ce->name), ZSTR_LEN(ce->name), C_STRVAL_P(tmp), ZSTR_LEN(ce->name), ZSTR_LEN(ce->name)) == 0 &&
				    type_name) {
		...
		if (SOAP_GLOBAL(typemap) && encode->details.type_str) {
			...
			smart_str_0(&nscat);
			if ((new_enc = crex_hash_find_ptr(SOAP_GLOBAL(typemap), nscat.s)) != NULL) {
				encode = new_enc;
			}
			smart_str_free(&nscat);
		}
...
xmlNodePtr to_xml_user(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (type && type->map && C_TYPE(type->map->to_xml) != IS_UNDEF) {
		...
		if (C_TYPE(return_value) == IS_STRING) {
			xmlDocPtr doc = soap_xmlParseMemory(C_STRVAL(return_value), C_STRLEN(return_value));
			if (doc && doc->children) {
...
{
	if (type && type->map && C_TYPE(type->map->to_zval) != IS_UNDEF) {
		xmlBufferPtr buf;
		zval data;
		xmlNodePtr copy;
...
static zval *to_zval_base64(zval *ret, encodeTypePtr type, xmlNodePtr data)
{
	...
	if (data && data->children) {
		if (data->children->type == XML_TEXT_NODE && data->children->next == NULL) {
			whiteSpace_collapse(data->children->content);
			str = crx_base64_decode(data->children->content, strlen((char*)data->children->content));
			if (!str) {
				soap_error0(E_ERROR, "Encoding: Violation of encoding rules");
			}
			ZVAL_STR(ret, str);
...
static zval *to_zval_hexbin(zval *ret, encodeTypePtr type, xmlNodePtr data)
{
	...
	if (data && data->children) {
		...
		str = crex_string_alloc(strlen((char*)data->children->content) / 2, 0);
...
static xmlNodePtr to_xml_string(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_STRING) {
		str = estrndup(C_STRVAL_P(data), C_STRLEN_P(data));
		new_len = C_STRLEN_P(data);
	} else {
		crex_string *tmp = zval_get_string_func(data);
	...
	if (!crx_libxml_xmlCheckUTF8(BAD_CAST(str))) {
...
static xmlNodePtr to_xml_base64(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_STRING) {
		str = crx_base64_encode((unsigned char*)C_STRVAL_P(data), C_STRLEN_P(data));
	} else {
		crex_string *tmp = zval_get_string_func(data);
		str = crx_base64_encode((unsigned char*) ZSTR_VAL(tmp), ZSTR_LEN(tmp));
		crex_string_release_ex(tmp, 0);
	}
...
static xmlNodePtr to_xml_hexbin(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	str = (unsigned char *) safe_emalloc(C_STRLEN_P(data) * 2, sizeof(char), 1);

	for (i = j = 0; i < C_STRLEN_P(data); i++) {
		str[j++] = hexconvtab[((unsigned char)C_STRVAL_P(data)[i]) >> 4];
		str[j++] = hexconvtab[((unsigned char)C_STRVAL_P(data)[i]) & 15];
	}
	str[j] = '\0';

	text = xmlNewTextLen(str, C_STRLEN_P(data) * 2 * sizeof(char));
	xmlAddChild(ret, text);
	efree(str);
	if (data == &tmp) {
...
static xmlNodePtr to_xml_long(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_DOUBLE) {
		char s[256];

		snprintf(s, sizeof(s), "%0.0F",floor(C_DVAL_P(data)));
		xmlNodeSetContent(ret, BAD_CAST(s));
	} else {
		crex_string *str = crex_long_to_str(zval_get_long(data));
...
static xmlNodePtr to_xml_double(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	crex_gcvt(C_DVAL(tmp), EG(precision), '.', 'E', str);
...
{
	crex_update_property(C_OBJCE_P(object), C_OBJ_P(object), name, strlen(name), val);
	C_TRY_DELREF_P(val);
}
...
static zval* get_zval_property(zval* object, char* name, zval *rv)
{
	if (C_TYPE_P(object) == IS_OBJECT) {
		zval *data = crex_read_property(C_OBJCE_P(object), C_OBJ_P(object), name, strlen(name), 1, rv);
		if (data == &EG(uninitialized_zval)) {
			return NULL;
		}
		ZVAL_DEREF(data);
		return data;
	} else if (C_TYPE_P(object) == IS_ARRAY) {
		return crex_hash_str_find_deref(C_ARRVAL_P(object), name, strlen(name));
	}
	return NULL;
}
...
{
	if (C_TYPE_P(object) == IS_OBJECT) {
		crex_unset_property(C_OBJCE_P(object), C_OBJ_P(object), name, strlen(name));
	} else if (C_TYPE_P(object) == IS_ARRAY) {
		crex_hash_str_del(C_ARRVAL_P(object), name, strlen(name));
	}
}
...
static void model_to_zval_any(zval *ret, xmlNodePtr node)
{
	...
	while (node != NULL) {
		if (get_zval_property(ret, (char*)node->name, &rv) == NULL) {
			...
			if (C_TYPE(val) == IS_STRING && *C_STRVAL(val) == '<') {
				name = NULL;
				while (node->next != NULL) {
					...
					if (C_TYPE(val2) != IS_STRING ||  *C_STRVAL(val) != '<') {
						C_TRY_DELREF(val2);
						break;
					}
			...
			if (any == NULL) {
				...
				if (name) {
					zval *el;
					if ((el = crex_hash_str_find(C_ARRVAL_P(any), name, strlen(name))) != NULL) {
						if (C_TYPE_P(el) != IS_ARRAY) {
							/* Convert into array */
							array_init(&arr);
		...
		case XSD_CONTENT_ELEMENT:
			if (model->u.element->name) {
				...
				if (node) {
					...
					} else if ((C_TYPE(val) != IS_NULL || !model->u.element->nillable) &&
...
static zval *to_zval_object_ex(zval *ret, encodeTypePtr type, xmlNodePtr data, crex_class_entry *pce)
{
	xmlNodePtr trav;
	sdlPtr sdl;
	sdlTypePtr sdlType = type->sdl_type;
	crex_class_entry *ce = CREX_STANDARD_CLASS_DEF_PTR;
	...
	if (pce) {
		...
		if ((classname = crex_hash_str_find_deref(SOAP_GLOBAL(class_map), type->type_str, strlen(type->type_str))) != NULL &&
		    C_TYPE_P(classname) == IS_STRING &&
		    (tmp = crex_fetch_class(C_STR_P(classname), CREX_FETCH_CLASS_AUTO)) != NULL) {
			ce = tmp;
		}
	}
	sdl = SOAP_GLOBAL(sdl);
	if (sdlType) {
		if (sdlType->kind == XSD_TYPEKIND_RESTRICTION &&
			...
			if (sdlType->encode->details.sdl_type &&
			    ...
			    if (ce != CREX_STANDARD_CLASS_DEF_PTR &&
				...
				if (C_TYPE_P(ret) == IS_OBJECT && ce != CREX_STANDARD_CLASS_DEF_PTR) {
					crex_object *zobj = C_OBJ_P(ret);
					zobj->ce = ce;
				}
			} else {
				zval base;
		...
		if (sdlType->attributes) {
			...
			CREX_HASH_FOREACH_PTR(sdlType->attributes, attr) {
				if (attr->name) {
		...
		case XSD_CONTENT_ELEMENT: {
			...
			if (data) {
				...
				if ((model->max_occurs == -1 || model->max_occurs > 1) &&
				    ...
				    !is_map(data)) {
					HashTable *ht = C_ARRVAL_P(data);
					zval *val;

					CREX_HASH_FOREACH_VAL(ht, val) {
						ZVAL_DEREF(val);
						if (C_TYPE_P(val) == IS_NULL && model->u.element->nillable) {
					...
					if (C_TYPE_P(data) == IS_NULL && model->u.element->nillable) {
						property = xmlNewNode(NULL, BAD_CAST("BOGUS"));
						xmlAddChild(node, property);
						set_xsi_nil(property);
					} else if (C_TYPE_P(data) == IS_NULL && model->min_occurs == 0) {
						return 1;
					} else {
...
static sdlTypePtr model_array_element(sdlContentModelPtr model)
{
	switch (model->kind) {
		...
		case XSD_CONTENT_CHOICE: {
			...
			if (crex_hash_num_elements(model->u.content) != 1) {
			  return NULL;
			}
			CREX_HASH_FOREACH_PTR(model->u.content, tmp) {
...
static xmlNodePtr to_xml_object(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_OBJECT) {
		prop = C_OBJPROP_P(data);
	} else if (C_TYPE_P(data) == IS_ARRAY) {
		prop = C_ARRVAL_P(data);
	}
	...
	if (sdlType) {
		...
		if (prop != NULL) {
			...
			if (C_TYPE_P(data) == IS_ARRAY &&
		      ...
		      (array_el = model_array_element(sdlType->model)) != NULL) {
				...
				CREX_HASH_FOREACH_VAL(prop, val) {
					xmlNodePtr property;
					ZVAL_DEREF(val);
					if (C_TYPE_P(val) == IS_NULL && array_el->nillable) {
						property = xmlNewNode(NULL, BAD_CAST("BOGUS"));
		...
		if (prop != NULL) {
			...
			CREX_HASH_FOREACH_STR_KEY_VAL_IND(prop, str_key, zprop) {
				ZVAL_DEREF(zprop);
				property = master_to_xml(get_conversion(C_TYPE_P(zprop)), zprop, style, xmlParam);
				...
				if (str_key) {
					...
					if (C_TYPE_P(data) == IS_OBJECT) {
						...
						crex_unmangle_property_name(str_key, &class_name, &prop_name);
					} else {
                                  ...
                                  )
{
	...
	if (data && C_TYPE_P(data) == IS_ARRAY) {
		CREX_HASH_FOREACH_VAL_IND(C_ARRVAL_P(data), zdata) {
	 		if (j >= dims[0]) {
	 			break;
	 		}
			ZVAL_DEREF(zdata);
			if (dimension == 1) {
				if (enc == NULL) {
					xparam = master_to_xml(get_conversion(C_TYPE_P(zdata)), zdata, style, xmlParam);
				} else {
...
static xmlNodePtr to_xml_array(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_OBJECT && instanceof_function(C_OBJCE_P(data), crex_ce_traversable)) {
		...
		while (iter->funcs->valid(iter) == SUCCESS) {
			...
			if (iter->funcs->get_current_key) {
				...
				array_set_zval_key(C_ARRVAL(array_copy), &key, val);
				zval_ptr_dtor(val);
				zval_ptr_dtor(&key);
			} else {
...
iterator_done:
	...
	if (C_TYPE_P(data) == IS_ARRAY) {
		...
		i = crex_hash_num_elements(C_ARRVAL_P(data));

		if (sdl_type &&
		    sdl_type->attributes &&
		    (arrayType = crex_hash_str_find_ptr(sdl_type->attributes, SOAP_1_1_ENC_NAMESPACE":arrayType",
		      sizeof(SOAP_1_1_ENC_NAMESPACE":arrayType")-1)) != NULL &&
		    arrayType->extraAttributes &&
		    (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arrayType", sizeof(WSDL_NAMESPACE":arrayType")-1)) != NULL) {
			...
			for (i = 1; i < dimension; i++) {
				if (el != NULL && C_TYPE_P(el) == IS_ARRAY &&
				    crex_hash_num_elements(C_ARRVAL_P(el)) > 0) {
				    CREX_HASH_FOREACH_VAL_IND(C_ARRVAL_P(el), el) {
				    	break;
				    } CREX_HASH_FOREACH_END();
					ZVAL_DEREF(el);
					if (C_TYPE_P(el) == IS_ARRAY) {
						dims[i] = crex_hash_num_elements(C_ARRVAL_P(el));
					} else {
						dims[i] = 0;
					}
				}
			...
			efree(value);
		           ...
		           sdl_type->attributes &&
		           (arrayType = crex_hash_str_find_ptr(sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":itemType",
		             sizeof(SOAP_1_2_ENC_NAMESPACE":itemType")-1)) != NULL &&
		           arrayType->extraAttributes &&
		           (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":itemType", sizeof(WSDL_NAMESPACE":itemType")-1)) != NULL) {
			if (ext->ns != NULL) {
			...
			if ((arrayType = crex_hash_str_find_ptr(sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":arraySize",
			                   sizeof(SOAP_1_2_ENC_NAMESPACE":arraySize")-1)) != NULL &&
			    arrayType->extraAttributes &&
			    (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arraySize", sizeof(WSDL_NAMESPACE":arraysize")-1)) != NULL) {
				...
				*dims = 0;
				smart_str_append_long(&array_size, i);
		           ...
		           sdl_type->attributes &&
		           (arrayType = crex_hash_str_find_ptr(sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":arraySize",
		             sizeof(SOAP_1_2_ENC_NAMESPACE":arraySize")-1)) != NULL &&
		           arrayType->extraAttributes &&
		           (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arraySize", sizeof(WSDL_NAMESPACE":arraySize")-1)) != NULL) {
			dimension = calc_dimension_12(ext->val);
			...
			if (sdl_type && sdl_type->elements &&
			    crex_hash_num_elements(sdl_type->elements) == 1 &&
			    (crex_hash_internal_pointer_reset(sdl_type->elements),
			     (elementType = crex_hash_get_current_data_ptr(sdl_type->elements)) != NULL) &&
		...
		if (style == SOAP_ENCODED) {
			if (soap_version == SOAP_1_1) {
				...
				CREX_DIAGNOSTIC_IGNORED_START("-Wstringop-overread")
#endif
...
static zval *to_zval_array(zval *ret, encodeTypePtr type, xmlNodePtr data)
{
	...
	if (data &&
		...
		dims = get_position_12(dimension, (char*)attr->children->content);
	           ...
	           (arrayType = crex_hash_str_find_ptr(type->sdl_type->attributes, SOAP_1_1_ENC_NAMESPACE":arrayType",
	                          sizeof(SOAP_1_1_ENC_NAMESPACE":arrayType")-1)) != NULL &&
	           arrayType->extraAttributes &&
	           (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arrayType", sizeof(WSDL_NAMESPACE":arrayType")-1)) != NULL) {
		char *type, *end;
		...
		*dims = 0;
	           ...
	           type->sdl_type->attributes != NULL &&
	           (arrayType = crex_hash_str_find_ptr(type->sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":itemType",
	                          sizeof(SOAP_1_2_ENC_NAMESPACE":itemType")-1)) != NULL &&
	           arrayType->extraAttributes &&
	           (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":itemType", sizeof(WSDL_NAMESPACE":itemType")-1)) != NULL) {
		...
		if ((arrayType = crex_hash_str_find_ptr(type->sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":arraySize",
		                   sizeof(SOAP_1_2_ENC_NAMESPACE":arraySize")-1)) != NULL &&
		    arrayType->extraAttributes &&
		    (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arraySize", sizeof(WSDL_NAMESPACE":arraysize")-1)) != NULL) {
			dimension = calc_dimension_12(ext->val);
			...
			*dims = 0;
	           ...
	           type->sdl_type->attributes != NULL &&
	           (arrayType = crex_hash_str_find_ptr(type->sdl_type->attributes, SOAP_1_2_ENC_NAMESPACE":arraySize",
	                          sizeof(SOAP_1_2_ENC_NAMESPACE":arraySize")-1)) != NULL &&
	           arrayType->extraAttributes &&
	           (ext = crex_hash_str_find_ptr(arrayType->extraAttributes, WSDL_NAMESPACE":arraySize", sizeof(WSDL_NAMESPACE":arraysize")-1)) != NULL) {
		...
		if (type->sdl_type && type->sdl_type->elements &&
		    crex_hash_num_elements(type->sdl_type->elements) == 1 &&
		    (crex_hash_internal_pointer_reset(type->sdl_type->elements),
		     (elementType = crex_hash_get_current_data_ptr(type->sdl_type->elements)) != NULL) &&
		    elementType->encode) {
			enc = elementType->encode;
		}
	...
	while (trav) {
		if (trav->type == XML_ELEMENT_NODE) {
			...
			while (i < dimension-1) {
				zval* ar2;
				if ((ar2 = crex_hash_index_find(C_ARRVAL_P(ar), pos[i])) != NULL) {
					ar = ar2;
				} else {
					zval tmpAr;
					array_init(&tmpAr);
					ar = crex_hash_index_update(C_ARRVAL_P(ar), pos[i], &tmpAr);
				}
				i++;
			}
			crex_hash_index_update(C_ARRVAL_P(ar), pos[i], &tmpVal);
...
static xmlNodePtr to_xml_map(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_ARRAY) {
		CREX_HASH_FOREACH_KEY_VAL_IND(C_ARRVAL_P(data), int_val, key_val, temp_data) {
			...
			xparam = master_to_xml(get_conversion(C_TYPE_P(temp_data)), temp_data, style, item);
	...
	if (data && data->children) {
		...
		FOREACHNODE(trav, "item", item) {
			...
			if (C_TYPE(key) == IS_STRING) {
				crex_symtable_update(C_ARRVAL_P(ret), C_STR(key), &value);
			} else if (C_TYPE(key) == IS_LONG) {
				crex_hash_index_update(C_ARRVAL_P(ret), C_LVAL(key), &value);
			} else {
...
static xmlNodePtr guess_xml_convert(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (data) {
		enc = get_conversion(C_TYPE_P(data));
	} else {
		enc = get_conversion(IS_NULL);
	}
...
static zval *guess_zval_convert(zval *ret, encodeTypePtr type, xmlNodePtr data)
{
	...
	if (SOAP_GLOBAL(sdl) && type_name && enc->details.sdl_type) {
		...
		ZVAL_LONG(C_VAR_ENC_TYPE_P(&soapvar), enc->details.type);
		ZVAL_COPY_VALUE(C_VAR_ENC_VALUE_P(&soapvar), ret);
		parse_namespace(type_name, &cptype, &ns);
		nsptr = xmlSearchNs(data->doc, data, BAD_CAST(ns));
		ZVAL_STRING(C_VAR_ENC_STYPE_P(&soapvar), cptype);
		if (nsptr) {
			ZVAL_STRING(C_VAR_ENC_NS_P(&soapvar), (char*)nsptr->href);
		}
		efree(cptype);
		if (ns) {efree(ns);}
...
static xmlNodePtr to_xml_datetime_ex(encodeTypePtr type, zval *data, char *format, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_LONG) {
		timestamp = C_LVAL_P(data);
		ta = crx_localtime_r(&timestamp, &tmbuf);
		/*ta = crx_gmtime_r(&timestamp, &tmbuf);*/
		if (!ta) {
			soap_error1(E_ERROR, "Encoding: Invalid timestamp " CREX_LONG_FMT, C_LVAL_P(data));
		}
...
#else
# if defined(__CYGWIN__) || (defined(CRX_WIN32) && defined(_MSC_VER) && _MSC_VER >= 1900)
...
# endif
		...
		xmlNodeSetContentLen(xmlParam, BAD_CAST(C_STRVAL_P(data)), C_STRLEN_P(data));
	}
...
static xmlNodePtr to_xml_list(encodeTypePtr enc, zval *data, int style, xmlNodePtr parent) {
	...
	if (enc->sdl_type && enc->sdl_type->kind == XSD_TYPEKIND_LIST && enc->sdl_type->elements) {
		...
		CREX_HASH_FOREACH_PTR(enc->sdl_type->elements, type) {
...
static zval *to_zval_any(zval *ret, encodeTypePtr type, xmlNodePtr data)
{
	...
	if (SOAP_GLOBAL(sdl) && SOAP_GLOBAL(sdl)->elements && data->name) {
		...
		if ((sdl_type = crex_hash_find_ptr(SOAP_GLOBAL(sdl)->elements, nscat.s)) != NULL &&
		    sdl_type->encode) {
			smart_str_free(&nscat);
...
static xmlNodePtr to_xml_any(encodeTypePtr type, zval *data, int style, xmlNodePtr parent)
{
	...
	if (C_TYPE_P(data) == IS_ARRAY) {
		...
		CREX_HASH_FOREACH_STR_KEY_VAL(C_ARRVAL_P(data), name, el) {
	...
	if (C_TYPE_P(data) == IS_STRING) {
		ret = xmlNewTextLen(BAD_CAST(C_STRVAL_P(data)), C_STRLEN_P(data));
	} else {
		crex_string *tmp = zval_get_string_func(data);
...
/*FIXME: restriction support
	if (type && type->restrictions &&
		...
		if (type->restrictions->enumeration) {
			if (!crex_hash_exists(type->restrictions->enumeration,data->children->content,strlen(data->children->content)+1)) {
...
/*FIXME: restriction support
	if (type) {
		if (type->restrictions && C_TYPE_P(data) == IS_STRING) {
			if (type->restrictions->enumeration) {
				if (!crex_hash_exists(type->restrictions->enumeration,C_STRVAL_P(data),C_STRLEN_P(data)+1)) {
					soap_error1(E_WARNING, "Encoding: Restriction: invalid enumeration value \"%s\".", C_STRVAL_P(data));
				}
			}
			if (type->restrictions->minLength &&
			    C_STRLEN_P(data) < type->restrictions->minLength->value) {
		  	soap_error0(E_WARNING, "Encoding: Restriction: length less than 'minLength'");
			}
			if (type->restrictions->maxLength &&
			    C_STRLEN_P(data) > type->restrictions->maxLength->value) {
		  	soap_error0(E_WARNING, "Encoding: Restriction: length greater than 'maxLength'");
			}
			if (type->restrictions->length &&
			    C_STRLEN_P(data) != type->restrictions->length->value) {
		  	soap_error0(E_WARNING, "Encoding: Restriction: length is not equal to 'length'");
			}
		}
	}
...
xmlNsPtr encode_add_ns(xmlNodePtr node, const char* ns)
{
	...
	if (xmlns == NULL) {
		...
		if ((prefix = crex_hash_str_find_ptr(&SOAP_GLOBAL(defEncNs), (char*)ns, strlen(ns))) != NULL) {
...
void encode_reset_ns(void)
{
	...
	if (SOAP_GLOBAL(ref_map)) {
		crex_hash_destroy(SOAP_GLOBAL(ref_map));
	} else {
		SOAP_GLOBAL(ref_map) = emalloc(sizeof(HashTable));
	}
	crex_hash_init(SOAP_GLOBAL(ref_map), 0, NULL, NULL, 0);
}
...
encodePtr get_conversion(int encode)
{
	...
	if ((enc = crex_hash_index_find_ptr(&SOAP_GLOBAL(defEncIndex), encode)) == NULL) {
...
static int is_map(zval *array)
{
	...
	if (HT_IS_PACKED(C_ARRVAL_P(array)) && HT_IS_WITHOUT_HOLES(C_ARRVAL_P(array))) {
		return FALSE;
	}

	CREX_HASH_FOREACH_KEY(C_ARRVAL_P(array), index, key) {
		if (key || index != i) {
			return TRUE;
		}
		i++;
...
static encodePtr get_array_type(xmlNodePtr node, zval *array, smart_str *type)
{
	...
	if (!array || C_TYPE_P(array) != IS_ARRAY) {
	...
	CREX_HASH_FOREACH_VAL_IND(ht, tmp) {
		ZVAL_DEREF(tmp);
		if (C_TYPE_P(tmp) == IS_OBJECT &&
		    C_OBJCE_P(tmp) == soap_var_class_entry) {
			zval *ztype = C_VAR_ENC_TYPE_P(tmp);
			if (C_TYPE_P(ztype) != IS_LONG) {
				soap_error0(E_ERROR,  "Encoding: SoapVar has no 'enc_type' property");
			}
			cur_type = C_LVAL_P(ztype);

			zval *zstype = C_VAR_ENC_STYPE_P(tmp);
			if (C_TYPE_P(zstype) == IS_STRING) {
				cur_stype = C_STRVAL_P(zstype);
			} else {
				cur_stype = NULL;
			}

			zval *zns = C_VAR_ENC_NS_P(tmp);
			if (C_TYPE_P(zns) == IS_STRING) {
				cur_ns = C_STRVAL_P(zns);
			} else {
				cur_ns = NULL;
			}

		} else if (C_TYPE_P(tmp) == IS_ARRAY && is_map(tmp)) {
			cur_type = APACHE_MAP;
			cur_stype = NULL;
			cur_ns = NULL;
		} else {
			cur_type = C_TYPE_P(tmp);
			cur_stype = NULL;
			cur_ns = NULL;
		}
