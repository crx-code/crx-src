...
static sdlTypePtr get_element(sdlPtr sdl, xmlNodePtr node, const xmlChar *type)
{
	...
	if (sdl->elements) {
		...
		if (nsptr != NULL) {
			...
			if ((sdl_type = crex_hash_str_find_ptr(sdl->elements, nscat, len)) != NULL) {
				ret = sdl_type;
			} else if ((sdl_type = crex_hash_str_find_ptr(sdl->elements, (char*)type, type_len)) != NULL) {
				ret = sdl_type;
			}
			efree(nscat);
		} else {
			if ((sdl_type = crex_hash_str_find_ptr(sdl->elements, (char*)type, xmlStrlen(type))) != NULL) {
				ret = sdl_type;
			}
		}
	...
	if (enc == NULL &&
		...
		if (enc && sdl) {
			...
			if (sdl->is_persistent) {
				new_enc->details.ns = crex_strndup(ns, ns_len);
			...
			if (sdl->encoders == NULL) {
				...
				crex_hash_init(sdl->encoders, 0, NULL, sdl->is_persistent ? delete_encoder_persistent : delete_encoder, sdl->is_persistent);
			}
			crex_hash_str_update_ptr(sdl->encoders, nscat, len, new_enc);
			enc = new_enc;
		}
	}
	efree(nscat);
...
encodePtr get_encoder_ex(sdlPtr sdl, const char *nscat, int len)
{
	...
	if ((enc = crex_hash_str_find_ptr(&SOAP_GLOBAL(defEnc), (char*)nscat, len)) != NULL) {
		return enc;
	} else if (sdl && sdl->encoders && (enc = crex_hash_str_find_ptr(sdl->encoders, (char*)nscat, len)) != NULL) {
		return enc;
	}
	return NULL;
}
...
sdlBindingPtr get_binding_from_type(sdlPtr sdl, sdlBindingType type)
{
	...
	CREX_HASH_MAP_FOREACH_PTR(sdl->bindings, binding) {
...
sdlBindingPtr get_binding_from_name(sdlPtr sdl, char *name, char *ns)
{
	...
	binding = crex_hash_find_ptr(sdl->bindings, key.s);
...
void sdl_set_uri_credentials(sdlCtx *ctx, char *uri)
{
	...
	if (l1 != l2 || memcmp(ctx->sdl->source, uri, l1) != 0) {
		...
		crx_libxml_switch_context(NULL, &context);
		crx_libxml_switch_context(&context, NULL);
		if (C_TYPE(context) != IS_UNDEF) {
			zval *context_ptr = &context;
			ctx->context = crx_stream_context_from_zval(context_ptr, 1);

			if (ctx->context &&
			    (header = crx_stream_context_get_option(ctx->context, "http", "header")) != NULL) {
				s = strstr(C_STRVAL_P(header), "Authorization: Basic");
				if (s && (s == C_STRVAL_P(header) || *(s-1) == '\n' || *(s-1) == '\r')) {
					char *rest = strstr(s, "\r\n");
					if (rest) {
						zval new_header;

						rest += 2;
						ZVAL_NEW_STR(&new_header, crex_string_alloc(C_STRLEN_P(header) - (rest - s), 0));
						memcpy(C_STRVAL(new_header), C_STRVAL_P(header), s - C_STRVAL_P(header));
						memcpy(C_STRVAL(new_header) + (s - C_STRVAL_P(header)), rest, C_STRLEN_P(header) - (rest - C_STRVAL_P(header)) + 1);
						ZVAL_COPY(&ctx->old_header, header);
						crx_stream_context_set_option(ctx->context, "http", "header", &new_header);
...
{
	if (C_TYPE(ctx->old_header) != IS_UNDEF) {
	    crx_stream_context_set_option(ctx->context, "http", "header", &ctx->old_header);
...
static void load_wsdl_ex(zval *this_ptr, char *struri, sdlCtx *ctx, int include)
{
	...
	if (crex_hash_str_exists(&ctx->docs, struri, strlen(struri))) {
		return;
	}
	...
	crex_hash_str_add_ptr(&ctx->docs, struri, strlen(struri), wsdl);
	...
	while (trav != NULL) {
		...
		if (node_is_equal(trav,"types")) {
			...
			if (name && name->children && name->children->content) {
				if (crex_hash_str_add_ptr(&ctx->messages, (char*)name->children->content, xmlStrlen(name->children->content), trav) == NULL) {
			...
			xmlAttrPtr name = get_attribute(trav->properties, "name");
			if (name && name->children && name->children->content) {
				if (crex_hash_str_add_ptr(&ctx->portTypes, (char*)name->children->content, xmlStrlen(name->children->content), trav) == NULL) {
			...
			xmlAttrPtr name = get_attribute(trav->properties, "name");
			if (name && name->children && name->children->content) {
				if (crex_hash_str_add_ptr(&ctx->bindings, (char*)name->children->content, xmlStrlen(name->children->content), trav) == NULL) {
			...
			xmlAttrPtr name = get_attribute(trav->properties, "name");
			if (name && name->children && name->children->content) {
				if (crex_hash_str_add_ptr(&ctx->services, (char*)name->children->content, xmlStrlen(name->children->content), trav) == NULL) {
...
static sdlSoapBindingFunctionHeaderPtr wsdl_soap_binding_header(sdlCtx* ctx, xmlNodePtr header, char* wsdl_soap_namespace, int fault)
{
	...
	if ((message = crex_hash_str_find_ptr(&ctx->messages, ctype, strlen(ctype))) == NULL) {
		...
		while (trav != NULL) {
			if (node_is_equal_ex(trav, "headerfault", wsdl_soap_namespace)) {
				...
				if (h->headerfaults == NULL) {
					...
					crex_hash_init(h->headerfaults, 0, NULL, delete_header, 0);
				}
				...
				smart_str_0(&key);
				if (crex_hash_add_ptr(h->headerfaults, key.s, hf) == NULL) {
	...
	while (trav != NULL) {
		if (node_is_equal_ex(trav, "body", wsdl_soap_namespace)) {
			...
			if (tmp) {
				...
				crex_hash_init(&ht, 0, NULL, delete_parameter, 0);
				while (*parts) {
					sdlParamPtr param;
					int found = 0;
					char *end;
					...
					if (end) *end = '\0';
					CREX_HASH_FOREACH_PTR(params, param) {
						if (param->paramName &&
					  	...
					  	*x_param = *param;
					  	...
					  	crex_hash_next_index_insert_ptr(&ht, x_param);
					  	found = 1;
					  	break;
						}
					} CREX_HASH_FOREACH_END();
			...
			if (binding->headers == NULL) {
				...
				crex_hash_init(binding->headers, 0, NULL, delete_header, 0);
			}
			...
			smart_str_0(&key);
			if (crex_hash_add_ptr(binding->headers, key.s, h) == NULL) {
				delete_header_int(h);
			}
			smart_str_free(&key);
		} else if (is_wsdl_element(trav) && !node_is_equal(trav,"documentation")) {
...
static HashTable* wsdl_message(sdlCtx *ctx, xmlChar* message_name)
{
	...
	if ((tmp = crex_hash_str_find_ptr(&ctx->messages, ctype, strlen(ctype))) == NULL) {
		soap_error1(E_ERROR, "Parsing WSDL: Missing <message> with name '%s'", message_name);
	}
	message = tmp;

	parameters = emalloc(sizeof(HashTable));
	crex_hash_init(parameters, 0, NULL, delete_parameter, 0);
	...
	while (trav != NULL) {
		...
		crex_hash_next_index_insert_ptr(parameters, param);
...
static sdlPtr load_wsdl(zval *this_ptr, char *struri)
{
	...
	crex_hash_init(&ctx.sdl->functions, 0, NULL, delete_function, 0);

	crex_hash_init(&ctx.docs, 0, NULL, delete_document, 0);
	crex_hash_init(&ctx.messages, 0, NULL, NULL, 0);
	crex_hash_init(&ctx.bindings, 0, NULL, NULL, 0);
	crex_hash_init(&ctx.portTypes, 0, NULL, NULL, 0);
	crex_hash_init(&ctx.services,  0, NULL, NULL, 0);

	crex_try {
	load_wsdl_ex(this_ptr, struri, &ctx, 0);
	schema_pass2(&ctx);

	n = crex_hash_num_elements(&ctx.services);
	if (n > 0) {
		crex_hash_internal_pointer_reset(&ctx.services);
		for (i = 0; i < n; i++) {
			xmlNodePtr service, tmp;
			xmlNodePtr trav, port;
			int has_soap_port = 0;

			service = tmp = crex_hash_get_current_data_ptr(&ctx.services);
			...
			while (trav != NULL) {
				...
				if ((tmp = crex_hash_str_find_ptr(&ctx.bindings, ctype, strlen(ctype))) == NULL) {
				...
				if ((tmp = crex_hash_str_find_ptr(&ctx.portTypes, ctype, strlen(ctype))) == NULL) {
...
*/
					...
					while (fault != NULL) {
						if (node_is_equal_ex(fault, "fault", WSDL_NAMESPACE)) {
							...
							if (f->details == NULL || crex_hash_num_elements(f->details) > 1) {
							...
							if (function->faults == NULL) {
								...
								crex_hash_init(function->faults, 0, NULL, delete_fault, 0);
							}
							if (crex_hash_str_add_ptr(function->faults, f->name, strlen(f->name), f) == NULL) {
...

					{
						...
						crex_str_tolower(tmp, len);
						if (crex_hash_str_add_ptr(&ctx.sdl->functions, tmp, len, function) == NULL) {
							crex_hash_next_index_insert_ptr(&ctx.sdl->functions, function);
						}
						efree(tmp);
						if (function->requestName != NULL && strcmp(function->requestName,function->functionName) != 0) {
							if (ctx.sdl->requests == NULL) {
								...
								crex_hash_init(ctx.sdl->requests, 0, NULL, NULL, 0);
							}
							tmp = estrdup(function->requestName);
							len = strlen(tmp);
							crex_str_tolower(tmp, len);
							crex_hash_str_add_ptr(ctx.sdl->requests, tmp, len, function);
							efree(tmp);
						}
					}
					trav2 = trav2->next;
				...
				if (!ctx.sdl->bindings) {
					...
					crex_hash_init(ctx.sdl->bindings, 0, NULL, delete_binding, 0);
				}

				if (!crex_hash_str_add_ptr(ctx.sdl->bindings, tmpbinding->name, strlen(tmpbinding->name), tmpbinding)) {
					crex_hash_next_index_insert_ptr(ctx.sdl->bindings, tmpbinding);
				}
				trav= trav->next;
			}

			crex_hash_move_forward(&ctx.services);
		}
	} else {
	...
	if (ctx.sdl->bindings == NULL || ctx.sdl->bindings->nNumOfElements == 0) {
		...
		crex_hash_destroy(&ctx.docs);
		crex_bailout();
	} crex_end_try();

	crex_hash_destroy(&ctx.messages);
	crex_hash_destroy(&ctx.bindings);
	crex_hash_destroy(&ctx.portTypes);
	crex_hash_destroy(&ctx.services);
	crex_hash_destroy(&ctx.docs);
...
static void sdl_deserialize_key(HashTable* ht, void* data, char **in)
{
	...
	WSDL_CACHE_GET_INT(len, in);
	if (len == WSDL_NO_STRING_MARKER) {
		crex_hash_next_index_insert_ptr(ht, data);
	} else {
		crex_hash_str_add_ptr(ht, *in, len, data);
...
static void sdl_deserialize_attribute(sdlAttributePtr attr, encodePtr *encoders, char **in)
{
	...
	if (i > 0) {
		...
		crex_hash_init(attr->extraAttributes, i, NULL, delete_extra_attribute, 0);
		while (i > 0) {
...
static sdlContentModelPtr sdl_deserialize_model(sdlTypePtr *types, sdlTypePtr *elements, char **in)
{
	...
	switch (model->kind) {
		...
		case XSD_CONTENT_CHOICE:
			...
			crex_hash_init(model->u.content, i, NULL, delete_model, 0);
			while (i > 0) {
				sdlContentModelPtr x = sdl_deserialize_model(types, elements, in);
				crex_hash_next_index_insert_ptr(model->u.content, x);
				i--;
			}
			break;
...
static void sdl_deserialize_type(sdlTypePtr type, sdlTypePtr *types, encodePtr *encoders, char **in)
{
	...
	if (**in == 1) {
		...
		if (i > 0) {
			...
			crex_hash_init(type->restrictions->enumeration, i, NULL, delete_restriction_var_char, 0);
			while (i > 0) {
	...
	if (i > 0) {
		...
		crex_hash_init(type->elements, i, NULL, delete_type, 0);
		while (i > 0) {
	...
	if (i > 0) {
		...
		crex_hash_init(type->attributes, i, NULL, delete_attribute, 0);
		while (i > 0) {
...
static void sdl_deserialize_soap_body(sdlSoapBindingFunctionBodyPtr body, encodePtr *encoders, sdlTypePtr *types, char **in)
{
	...
	if (i > 0) {
		...
		crex_hash_init(body->headers, i, NULL, delete_header, 0);
		while (i > 0) {
			...
			if (j > 0) {
				...
				crex_hash_init(tmp->headerfaults, i, NULL, delete_header, 0);
				while (j > 0) {
...
static HashTable* sdl_deserialize_parameters(encodePtr *encoders, sdlTypePtr *types, char **in)
{
	...
	crex_hash_init(ht, i, NULL, delete_parameter, 0);
	while (i > 0) {
...
static sdlPtr get_sdl_from_cache(const char *fn, const char *uri, size_t uri_len, time_t t, time_t *cached)
{
	...
	*cached = old_t;
	...
	if (num_groups > 0) {
		...
		crex_hash_init(sdl->groups, num_groups, NULL, delete_type, 0);
	...
	if (num_types > 0) {
		...
		crex_hash_init(sdl->types, num_types, NULL, delete_type, 0);
	...
	if (num_elements > 0) {
		...
		crex_hash_init(sdl->elements, num_elements, NULL, delete_type, 0);
	...
	if (num_encoders > 0) {
		...
		crex_hash_init(sdl->encoders, num_encoders, NULL, delete_encoder, 0);
	...
	if (num_bindings > 0) {
		...
		crex_hash_init(sdl->bindings, num_bindings, NULL, delete_binding, 0);
	...
	crex_hash_init(&sdl->functions, num_func, NULL, delete_function, 0);
	if (num_func > 0) {
		...
		for (i = 0; i < num_func; i++) {
			...
			if (num_faults > 0) {
			  int j;
				...
				crex_hash_init(func->faults, num_faults, NULL, delete_fault, 0);
	...
	if (i > 0) {
		...
		crex_hash_init(sdl->requests, i, NULL, NULL, 0);
		while (i > 0) {
			int function_num;
...
// TODO: refactor it
static void sdl_serialize_key(crex_string *key, smart_str *out)
{
	if (key) {
...
static void sdl_serialize_encoder_ref(encodePtr enc, HashTable *tmp_encoders, smart_str *out) {
	if (enc) {
		zval *encoder_num;
		if ((encoder_num = crex_hash_str_find(tmp_encoders, (char*)&enc, sizeof(enc))) != 0) {
			WSDL_CACHE_PUT_INT(C_LVAL_P(encoder_num), out);
		} else {
...
static void sdl_serialize_type_ref(sdlTypePtr type, HashTable *tmp_types, smart_str *out) {
	if (type) {
		zval *type_num;
		if ((type_num = crex_hash_str_find(tmp_types, (char*)&type, sizeof(type))) != NULL) {
			WSDL_CACHE_PUT_INT(C_LVAL_P(type_num), out);
		} else {
			WSDL_CACHE_PUT_INT(0, out);
		}
	} else {
...
static void sdl_serialize_attribute(sdlAttributePtr attr, HashTable *tmp_encoders, smart_str *out)
{
	...
	if (attr->extraAttributes) {
		i = crex_hash_num_elements(attr->extraAttributes);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(attr->extraAttributes, key, tmp) {
...
static void sdl_serialize_model(sdlContentModelPtr model, HashTable *tmp_types, HashTable *tmp_elements, smart_str *out)
{
	...
	switch (model->kind) {
		...
		case XSD_CONTENT_CHOICE: {
				...
				int i = crex_hash_num_elements(model->u.content);
...
static void sdl_serialize_type(sdlTypePtr type, HashTable *tmp_encoders, HashTable *tmp_types, smart_str *out)
{
	...
	if (type->restrictions) {
		...
		if (type->restrictions->enumeration) {
			i = crex_hash_num_elements(type->restrictions->enumeration);
		} else {
			i = 0;
		}
		WSDL_CACHE_PUT_INT(i, out);
		if (i > 0) {
			...
			CREX_HASH_MAP_FOREACH_STR_KEY_PTR(type->restrictions->enumeration, key, tmp) {
	...
	if (type->elements) {
		i = crex_hash_num_elements(type->elements);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		crex_hash_init(tmp_elements, i, NULL, NULL, 0);

		CREX_HASH_FOREACH_STR_KEY_PTR(type->elements, key, tmp) {
			sdl_serialize_key(key, out);
			sdl_serialize_type(tmp, tmp_encoders, tmp_types, out);
			ZVAL_LONG(&zv, i);
			crex_hash_str_add(tmp_elements, (char*)&tmp, sizeof(tmp), &zv);
			i--;
		} CREX_HASH_FOREACH_END();
	}

	if (type->attributes) {
		i = crex_hash_num_elements(type->attributes);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_FOREACH_STR_KEY_PTR(type->attributes, key, tmp) {
			sdl_serialize_key(key, out);
	...
	if (tmp_elements != NULL) {
		crex_hash_destroy(tmp_elements);
		efree(tmp_elements);
	}
}
...
static void sdl_serialize_parameters(HashTable *ht, HashTable *tmp_encoders, HashTable *tmp_types, smart_str *out)
{
	...
	if (i > 0) {
		...
		CREX_HASH_FOREACH_STR_KEY_PTR(ht, key, tmp) {
			sdl_serialize_key(key, out);
...
static void sdl_serialize_soap_body(sdlSoapBindingFunctionBodyPtr body, HashTable *tmp_encoders, HashTable *tmp_types, smart_str *out)
{
	...
	if (body->headers) {
		i = crex_hash_num_elements(body->headers);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(body->headers, key, tmp) {
			sdl_serialize_key(key, out);
			...
			sdl_serialize_type_ref(tmp->element, tmp_types, out);
			if (tmp->headerfaults) {
				j = crex_hash_num_elements(tmp->headerfaults);
			} else {
				j = 0;
			}
			...
			if (j > 0) {
				...
				CREX_HASH_MAP_FOREACH_STR_KEY_PTR(body->headers, key, tmp2) {
					sdl_serialize_key(key, out);
...
static void add_sdl_to_cache(const char *fn, const char *uri, time_t t, sdlPtr sdl)
{
	...
	crex_string *temp_file_path;
	f = crx_open_temporary_fd_ex(SOAP_GLOBAL(cache_dir), "tmp.wsdl.", &temp_file_path, CRX_TMP_FILE_SILENT);

	if (f < 0) {return;}

	crex_hash_init(&tmp_types, 0, NULL, NULL, 0);
	crex_hash_init(&tmp_encoders, 0, NULL, NULL, 0);
	crex_hash_init(&tmp_bindings, 0, NULL, NULL, 0);
	crex_hash_init(&tmp_functions, 0, NULL, NULL, 0);
	...
	if (sdl->groups) {
		i = crex_hash_num_elements(sdl->groups);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_PTR(sdl->groups, tmp) {
			ZVAL_LONG(&zv, type_num);
			crex_hash_str_add(&tmp_types, (char*)&tmp, sizeof(tmp), &zv);
			++type_num;
		} CREX_HASH_FOREACH_END();
	}

	if (sdl->types) {
		i = crex_hash_num_elements(sdl->types);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_FOREACH_PTR(sdl->types, tmp) {
			ZVAL_LONG(&zv,  type_num);
			crex_hash_str_add(&tmp_types, (char*)&tmp, sizeof(tmp), &zv);
			++type_num;
		} CREX_HASH_FOREACH_END();
	...
	if (sdl->elements) {
		i = crex_hash_num_elements(sdl->elements);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_PTR(sdl->elements, tmp) {
			ZVAL_LONG(&zv, type_num);
			crex_hash_str_add(&tmp_types, (char*)&tmp, sizeof(tmp), &zv);
			++type_num;
		} CREX_HASH_FOREACH_END();
	...
	if (sdl->encoders) {
		i = crex_hash_num_elements(sdl->encoders);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_FOREACH_PTR(sdl->encoders, tmp) {
			ZVAL_LONG(&zv, encoder_num);
			crex_hash_str_add(&tmp_encoders, (char*)&tmp, sizeof(tmp), &zv);
			++encoder_num;
		} CREX_HASH_FOREACH_END();
	}
	enc = defaultEncoding;
	while (enc->details.type != END_KNOWN_TYPES) {
		...
		ZVAL_LONG(&zv, encoder_num);
		crex_hash_str_add(&tmp_encoders, (char*)&enc, sizeof(encodePtr), &zv);
		enc++;
		++encoder_num;
	}
	...
	if (sdl->groups) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->groups, key, tmp) {
			sdl_serialize_key(key, out);
			sdl_serialize_type(tmp, &tmp_encoders, &tmp_types, out);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->types) {
		...
		CREX_HASH_FOREACH_STR_KEY_PTR(sdl->types, key, tmp) {
			sdl_serialize_key(key, out);
			sdl_serialize_type(tmp, &tmp_encoders, &tmp_types, out);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->elements) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->elements, key, tmp) {
			sdl_serialize_key(key, out);
			sdl_serialize_type(tmp, &tmp_encoders, &tmp_types, out);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->encoders) {
		...
		CREX_HASH_FOREACH_STR_KEY_PTR(sdl->encoders, key, tmp) {
			sdl_serialize_key(key, out);
	...
	/* serialize bindings */
	if (sdl->bindings) {
		i = crex_hash_num_elements(sdl->bindings);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->bindings, key, tmp) {
			sdl_serialize_key(key, out);
			sdl_serialize_string(tmp->name, out);
			...
			crex_hash_str_add(&tmp_bindings, (char*)&tmp, sizeof(tmp), &zv);
			binding_num++;
		} CREX_HASH_FOREACH_END();
	}

	/* serialize functions */
	i = crex_hash_num_elements(&sdl->functions);
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		sdlFunctionPtr tmp;
		zval *binding_num, zv;
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&sdl->functions, key, tmp) {
			sdl_serialize_key(key, out);
			...
			if (tmp->binding) {
				binding_num = crex_hash_str_find(&tmp_bindings,(char*)&tmp->binding, sizeof(tmp->binding));
				if (binding_num) {
					WSDL_CACHE_PUT_INT(C_LVAL_P(binding_num), out);
					if (C_LVAL_P(binding_num) >= 0) {
			...
			if (tmp->faults) {
				...
				WSDL_CACHE_PUT_INT(crex_hash_num_elements(tmp->faults), out);

				CREX_HASH_MAP_FOREACH_STR_KEY_PTR(tmp->faults, key, fault) {
					sdl_serialize_key(key, out);
			...
			crex_hash_str_add(&tmp_functions, (char*)&tmp, sizeof(tmp), &zv);
			function_num++;
		} CREX_HASH_FOREACH_END();
	}
	...
	/* serialize requests */
	if (sdl->requests) {
		i = crex_hash_num_elements(sdl->requests);
	} else {
		i = 0;
	}
	WSDL_CACHE_PUT_INT(i, out);
	if (i > 0) {
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->requests, key, tmp) {
			function_num = crex_hash_str_find(&tmp_functions, (char*)&tmp, sizeof(tmp));
			WSDL_CACHE_PUT_INT(C_LVAL_P(function_num), out);
			sdl_serialize_key(key, out);
		} CREX_HASH_FOREACH_END();
	}
	...
	smart_str_free(&buf);
	crex_hash_destroy(&tmp_functions);
	crex_hash_destroy(&tmp_bindings);
	crex_hash_destroy(&tmp_encoders);
	crex_hash_destroy(&tmp_types);
	crex_string_release_ex(temp_file_path, false);
}
...
static void make_persistent_sdl_type_ref(sdlTypePtr *type, HashTable *ptr_map, HashTable *bp_types)
{
	...
	if ((tmp = crex_hash_str_find_ptr(ptr_map, (char *)type, sizeof(sdlTypePtr))) != NULL) {
		*type = tmp;
	} else {
		crex_hash_next_index_insert_ptr(bp_types, *type);
	}
}
...
static void make_persistent_sdl_encoder_ref(encodePtr *enc, HashTable *ptr_map, HashTable *bp_encoders)
{
	...
	if ((tmp = crex_hash_str_find_ptr(ptr_map, (char *)enc, sizeof(encodePtr))) != NULL) {
		*enc = tmp;
	} else {
		crex_hash_next_index_insert_ptr(bp_encoders, enc);
	}
}
...
static HashTable* make_persistent_sdl_function_headers(HashTable *headers, HashTable *ptr_map)
{
	...
	crex_hash_init(pheaders, crex_hash_num_elements(headers), NULL, delete_header_persistent, 1);

	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(headers, key, tmp) {
		...
		*pheader = *tmp;
		...
		if (pheader->encode && pheader->encode->details.sdl_type) {
			if ((penc = crex_hash_str_find_ptr(ptr_map, (char*)&pheader->encode, sizeof(encodePtr))) == NULL) {
				assert(0);
			}
			pheader->encode = penc;
		}
		if (pheader->element) {
			if ((ptype = crex_hash_str_find_ptr(ptr_map, (char*)&pheader->element, sizeof(sdlTypePtr))) == NULL) {
				assert(0);
			}
		...
		if (key) {
			...
			crex_hash_str_add_ptr(pheaders, ZSTR_VAL(key), ZSTR_LEN(key), pheader);
		} else {
			crex_hash_next_index_insert_ptr(pheaders, pheader);
		}
	} CREX_HASH_FOREACH_END();
...
static HashTable* make_persistent_sdl_parameters(HashTable *params, HashTable *ptr_map)
{
	...
	crex_hash_init(pparams, crex_hash_num_elements(params), NULL, delete_parameter_persistent, 1);

	CREX_HASH_FOREACH_STR_KEY_PTR(params, key, tmp) {
		...
		*pparam = *tmp;
		...
		if (pparam->encode && pparam->encode->details.sdl_type) {
			if ((penc = crex_hash_str_find_ptr(ptr_map, (char*)&pparam->encode, sizeof(encodePtr))) == NULL) {
				assert(0);
			}
			pparam->encode = penc;
		}
		if (pparam->element) {
			if ((ptype = crex_hash_str_find_ptr(ptr_map, (char*)&pparam->element, sizeof(sdlTypePtr))) == NULL) {
				assert(0);
			}
		...
		if (key) {
			/* We have to duplicate key emalloc->malloc */
			crex_hash_str_add_ptr(pparams, ZSTR_VAL(key), ZSTR_LEN(key), pparam);
		} else {
			crex_hash_next_index_insert_ptr(pparams, pparam);
		}
	} CREX_HASH_FOREACH_END();
...
static HashTable* make_persistent_sdl_function_faults(sdlFunctionPtr func, HashTable *faults, HashTable *ptr_map)
{
	...
	crex_hash_init(pfaults, crex_hash_num_elements(faults), NULL, delete_fault_persistent, 1);

	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(faults, key, tmp) {
		...
		*pfault = *tmp;
		...
		if (key) {
			/* We have to duplicate key emalloc->malloc */
			crex_hash_str_add_ptr(pfaults, ZSTR_VAL(key), ZSTR_LEN(key), pfault);
		} else {
			crex_hash_next_index_insert_ptr(pfaults, pfault);
		}
...
static sdlAttributePtr make_persistent_sdl_attribute(sdlAttributePtr attr, HashTable *ptr_map, HashTable *bp_types, HashTable *bp_encoders)
{
	...
	*pattr = *attr;
	...
	if (pattr->extraAttributes) {
		...
		crex_hash_init(pattr->extraAttributes, crex_hash_num_elements(attr->extraAttributes), NULL, delete_extra_attribute_persistent, 1);
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(attr->extraAttributes, key, tmp) {
			if (key) {
				...
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(pattr->extraAttributes, ZSTR_VAL(key), ZSTR_LEN(key), pextra);
			}
		} CREX_HASH_FOREACH_END();
	}
	...
	*pmodel = *model;
	...
	switch (pmodel->kind) {
		...
		case XSD_CONTENT_CHOICE:
			...
			crex_hash_init(pmodel->u.content, crex_hash_num_elements(model->u.content), NULL, delete_model_persistent, 1);

			CREX_HASH_FOREACH_PTR(model->u.content, tmp) {
				pcontent = make_persistent_sdl_model(tmp, ptr_map, bp_types, bp_encoders);
				crex_hash_next_index_insert_ptr(pmodel->u.content, pcontent);
			} CREX_HASH_FOREACH_END();
			break;
...
static sdlTypePtr make_persistent_sdl_type(sdlTypePtr type, HashTable *ptr_map, HashTable *bp_types, HashTable *bp_encoders)
{
	...
	*ptype = *type;
	...
	if (ptype->restrictions) {
		...
		*ptype->restrictions = *type->restrictions;
		...
		if (type->restrictions->enumeration) {
			...
			crex_hash_init(ptype->restrictions->enumeration, crex_hash_num_elements(type->restrictions->enumeration), NULL, delete_restriction_var_char_persistent, 1);
			CREX_HASH_MAP_FOREACH_STR_KEY_PTR(type->restrictions->enumeration, key, tmp) {
				penum = tmp;
				make_persistent_restriction_char_int(&penum);
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(ptype->restrictions->enumeration, ZSTR_VAL(key), ZSTR_LEN(key), penum);
			} CREX_HASH_FOREACH_END();
		}
	}
	...
	if (ptype->elements) {
		...
		crex_hash_init(ptype->elements, crex_hash_num_elements(type->elements), NULL, delete_type_persistent, 1);

		CREX_HASH_FOREACH_STR_KEY_PTR(type->elements, key, tmp) {
			pelem = make_persistent_sdl_type(tmp, ptr_map, bp_types, bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(ptype->elements, ZSTR_VAL(key), ZSTR_LEN(key), pelem);
			} else {
				crex_hash_next_index_insert_ptr(ptype->elements, pelem);
			}
			crex_hash_str_add_ptr(ptr_map, (char*)&tmp, sizeof(tmp), pelem);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (ptype->attributes) {
		...
		crex_hash_init(ptype->attributes, crex_hash_num_elements(type->attributes), NULL, delete_attribute_persistent, 1);

		CREX_HASH_FOREACH_STR_KEY_PTR(type->attributes, key, tmp) {
			pattr = make_persistent_sdl_attribute(tmp, ptr_map, bp_types, bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(ptype->attributes, ZSTR_VAL(key), ZSTR_LEN(key), pattr);
			} else {
				crex_hash_next_index_insert_ptr(ptype->attributes, pattr);
			}
		} CREX_HASH_FOREACH_END();
	}
...
static sdlFunctionPtr make_persistent_sdl_function(sdlFunctionPtr func, HashTable *ptr_map)
{
	...
	*pfunc = *func;
	...
	if (pfunc->binding) {
		...
		if ((tmp = crex_hash_str_find_ptr(ptr_map, (char*)&pfunc->binding, sizeof(pfunc->binding))) == NULL) {
			assert(0);
		}
		pfunc->binding = tmp;
...
static sdlPtr make_persistent_sdl(sdlPtr sdl)
{
	...
	crex_hash_init(&bp_types, 0, NULL, NULL, 0);
	crex_hash_init(&bp_encoders, 0, NULL, NULL, 0);
	crex_hash_init(&ptr_map, 0, NULL, NULL, 0);
	...
	if (sdl->groups) {
		...
		crex_hash_init(psdl->groups, crex_hash_num_elements(sdl->groups), NULL, delete_type_persistent, 1);

		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->groups, key, tmp) {
			ptype = make_persistent_sdl_type(tmp, &ptr_map, &bp_types, &bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->groups, ZSTR_VAL(key), ZSTR_LEN(key), ptype);
			} else {
				crex_hash_next_index_insert_ptr(psdl->groups, ptype);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), ptype);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->types) {
		...
		crex_hash_init(psdl->types, crex_hash_num_elements(sdl->types), NULL, delete_type_persistent, 1);

		CREX_HASH_FOREACH_STR_KEY_PTR(sdl->types, key, tmp) {
			ptype = make_persistent_sdl_type(tmp, &ptr_map, &bp_types, &bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->types, ZSTR_VAL(key), ZSTR_LEN(key), ptype);
			} else {
				crex_hash_next_index_insert_ptr(psdl->types, ptype);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), ptype);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->elements) {
		...
		crex_hash_init(psdl->elements, crex_hash_num_elements(sdl->elements), NULL, delete_type_persistent, 1);

		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->elements, key, tmp) {
			ptype = make_persistent_sdl_type(tmp, &ptr_map, &bp_types, &bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->elements, ZSTR_VAL(key), ZSTR_LEN(key), ptype);
			} else {
				crex_hash_next_index_insert_ptr(psdl->elements, ptype);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), ptype);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->encoders) {
		...
		crex_hash_init(psdl->encoders, crex_hash_num_elements(sdl->encoders), NULL, delete_encoder_persistent, 1);

		CREX_HASH_FOREACH_STR_KEY_PTR(sdl->encoders, key, tmp) {
			penc = make_persistent_sdl_encoder(tmp, &ptr_map, &bp_types, &bp_encoders);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->encoders, ZSTR_VAL(key), ZSTR_LEN(key), penc);
			} else {
				crex_hash_next_index_insert_ptr(psdl->encoders, penc);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), penc);
		} CREX_HASH_FOREACH_END();
	}

	/* do backpatching here */
	if (crex_hash_num_elements(&bp_types)) {
		sdlTypePtr *tmp, ptype = NULL;

		CREX_HASH_FOREACH_PTR(&bp_types, tmp) {
			if ((ptype = crex_hash_str_find_ptr(&ptr_map, (char*)tmp, sizeof(*tmp))) == NULL) {
				assert(0);
			}
			*tmp = ptype;
		} CREX_HASH_FOREACH_END();
	}
	if (crex_hash_num_elements(&bp_encoders)) {
		encodePtr *tmp, penc = NULL;

		CREX_HASH_FOREACH_PTR(&bp_encoders, tmp) {
			if ((penc = crex_hash_str_find_ptr(&ptr_map, (char*)tmp, sizeof(*tmp))) == NULL) {
				assert(0);
			}
			*tmp = penc;
		} CREX_HASH_FOREACH_END();
	...
	if (sdl->bindings) {
		...
		crex_hash_init(psdl->bindings, crex_hash_num_elements(sdl->bindings), NULL, delete_binding_persistent, 1);

		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(sdl->bindings, key, tmp) {
			pbind = make_persistent_sdl_binding(tmp, &ptr_map);
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->bindings, ZSTR_VAL(key), ZSTR_LEN(key), pbind);
			} else {
				crex_hash_next_index_insert_ptr(psdl->bindings, pbind);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), pbind);
		} CREX_HASH_FOREACH_END();
	}

	crex_hash_init(&psdl->functions, crex_hash_num_elements(&sdl->functions), NULL, delete_function_persistent, 1);
	if (crex_hash_num_elements(&sdl->functions)) {
		sdlFunctionPtr tmp;
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&sdl->functions, key, tmp) {
			...
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(&psdl->functions, ZSTR_VAL(key), ZSTR_LEN(key), pfunc);
			} else {
				crex_hash_next_index_insert_ptr(&psdl->functions, pfunc);
			}
			crex_hash_str_add_ptr(&ptr_map, (char*)&tmp, sizeof(tmp), pfunc);
		} CREX_HASH_FOREACH_END();
	}
	...
	if (sdl->requests) {
		...
		crex_hash_init(psdl->requests, crex_hash_num_elements(sdl->requests), NULL, NULL, 1);

		CREX_HASH_MAP_FOREACH_STR_KEY_VAL(sdl->requests, key, zv) {
			tmp = C_PTR_P(zv);
			if ((preq = crex_hash_str_find_ptr(&ptr_map, (char*)&tmp, sizeof(tmp))) == NULL) {
				assert(0);
			}
			C_PTR_P(zv) = preq;
			if (key) {
				/* We have to duplicate key emalloc->malloc */
				crex_hash_str_add_ptr(psdl->requests, ZSTR_VAL(key), ZSTR_LEN(key), preq);
			}
		} CREX_HASH_FOREACH_END();
	}

	crex_hash_destroy(&ptr_map);
	crex_hash_destroy(&bp_encoders);
	crex_hash_destroy(&bp_types);
...
static void delete_psdl_int(sdl_cache_bucket *p)
{
	...
	crex_hash_destroy(&tmp->functions);
	if (tmp->source) {
		free(tmp->source);
	}
	if (tmp->target_ns) {
	...
	if (tmp->elements) {
		crex_hash_destroy(tmp->elements);
		free(tmp->elements);
	}
	if (tmp->encoders) {
		crex_hash_destroy(tmp->encoders);
		free(tmp->encoders);
	}
	if (tmp->types) {
		crex_hash_destroy(tmp->types);
		free(tmp->types);
	}
	if (tmp->groups) {
		crex_hash_destroy(tmp->groups);
		free(tmp->groups);
	}
	if (tmp->bindings) {
		crex_hash_destroy(tmp->bindings);
		free(tmp->bindings);
	}
	if (tmp->requests) {
		crex_hash_destroy(tmp->requests);
		free(tmp->requests);
	}
	free(tmp);
}
...
{
	delete_psdl_int(C_PTR_P(zv));
	free(C_PTR_P(zv));
}

sdlPtr get_sdl(zval *this_ptr, char *uri, crex_long cache_wsdl)
{
	char  fn[MAXPATHLEN];
	sdlPtr sdl = NULL;
	...
	if ((cache_wsdl & WSDL_CACHE_MEMORY) && SOAP_GLOBAL(mem_cache)) {
		...
		if (NULL != (p = crex_hash_str_find_ptr(SOAP_GLOBAL(mem_cache), uri, uri_len))) {
			if (p->time < t - SOAP_GLOBAL(cache_ttl)) {
				/* in-memory cache entry is expired */
				crex_hash_str_del(&EG(persistent_list), uri, uri_len);
			} else {
				return p->sdl;
			}
		}
	...
	if ((cache_wsdl & WSDL_CACHE_DISK) && (uri_len < MAXPATHLEN)) {
		...
		time_t cached;
		char *user = crx_get_current_user();
		int user_len = user ? strlen(user) + 1 : 0;

		md5str[0] = '\0';
		CRX_MD5Init(&context);
		CRX_MD5Update(&context, (unsigned char*)uri, uri_len);
	...
	if (instanceof_function(C_OBJCE_P(this_ptr), soap_class_entry)) {
		...
		if (C_TYPE_P(tmp) == IS_STRING && C_STRLEN_P(tmp) > 0) {
			smart_str_appends(&headers, "User-Agent: ");
			smart_str_appends(&headers, C_STRVAL_P(tmp));
		...
		if (C_TYPE_P(proxy_host) == IS_STRING && C_TYPE_P(proxy_port) == IS_LONG) {
			...
			smart_str_appends(&proxy,C_STRVAL_P(proxy_host));
			smart_str_appends(&proxy,":");
			smart_str_append_long(&proxy,C_LVAL_P(proxy_port));
			ZVAL_STR(&str_proxy, smart_str_extract(&proxy));

			if (!context) {
				context = crx_stream_context_alloc();
			}
			crx_stream_context_set_option(context, "http", "proxy", &str_proxy);
			zval_ptr_dtor(&str_proxy);

			if (uri_len < sizeof("https://")-1 ||
				strncasecmp(uri, "https://", sizeof("https://")-1) != 0) {
				ZVAL_TRUE(&str_proxy);
				crx_stream_context_set_option(context, "http", "request_fulluri", &str_proxy);
			}
	...
	if ((tmp = crx_stream_context_get_option(context, "http", "protocol_version")) == NULL) {
		zval http_version;

		ZVAL_DOUBLE(&http_version, 1.1);
		crx_stream_context_set_option(context, "http", "protocol_version", &http_version);
	...
	if (headers.s && ZSTR_LEN(headers.s) > 0) {
		...
		crx_stream_context_set_option(context, "http", "header", &str_headers);
		zval_ptr_dtor(&str_headers);
	}

	if (context) {
		ZVAL_RES(&new_context, context->res);
		crx_libxml_switch_context(&new_context, &orig_context);
	}
	...
	if (context) {
		crx_libxml_switch_context(&orig_context, NULL);
	...
	if (cache_wsdl & WSDL_CACHE_MEMORY) {
		if (sdl) {
			...
			if (SOAP_GLOBAL(mem_cache) == NULL) {
				...
				crex_hash_init(SOAP_GLOBAL(mem_cache), 0, NULL, delete_psdl, 1);
			} else if (SOAP_GLOBAL(cache_limit) > 0 &&
			           SOAP_GLOBAL(cache_limit) <= (crex_long)crex_hash_num_elements(SOAP_GLOBAL(mem_cache))) {
				/* in-memory cache overflow */
				sdl_cache_bucket *q;
				time_t latest = t;
				crex_string *latest_key = NULL, *key;

				CREX_HASH_MAP_FOREACH_STR_KEY_PTR(SOAP_GLOBAL(mem_cache), key, q) {
					if (q->time < latest) {
						latest = q->time;
						latest_key = key;
					}
				...
				if (latest_key) {
					crex_hash_del(SOAP_GLOBAL(mem_cache), latest_key);
				} else {
					return sdl;
				}
			}
			...
			crex_hash_str_update_mem(SOAP_GLOBAL(mem_cache), uri,
...
static void delete_binding(zval *zv)
{
	sdlBindingPtr binding = C_PTR_P(zv);
...
static void delete_sdl_soap_binding_function_body(sdlSoapBindingFunctionBody body)
{
	...
	if (body.headers) {
		crex_hash_destroy(body.headers);
		efree(body.headers);
	}
}
...
static void delete_function(zval *zv)
{
	sdlFunctionPtr function = C_PTR_P(zv);
	...
	if (function->requestParameters) {
		crex_hash_destroy(function->requestParameters);
		efree(function->requestParameters);
	}
	if (function->responseParameters) {
		crex_hash_destroy(function->responseParameters);
		efree(function->responseParameters);
	}
	if (function->faults) {
		crex_hash_destroy(function->faults);
...
static void delete_header_int(sdlSoapBindingFunctionHeaderPtr hdr)
{
	...
	if (hdr->headerfaults) {
		crex_hash_destroy(hdr->headerfaults);
...
{
	delete_header_int(C_PTR_P(zv));
}

static void delete_header_persistent(zval *zv)
{
	sdlSoapBindingFunctionHeaderPtr hdr = C_PTR_P(zv);
	if (hdr->name) {
		free(hdr->name);
	}
	if (hdr->ns) {
...
static void delete_fault(zval *zv)
{
	sdlFaultPtr fault = C_PTR_P(zv);
	if (fault->name) {
		efree(fault->name);
	}
	if (fault->details) {
		crex_hash_destroy(fault->details);
		efree(fault->details);
	}
