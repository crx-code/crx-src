...
static CREX_NORETURN void soap_server_fault(char* code, char* string, char *actor, zval* details, char *name);
...
#define SOAP_CLIENT_BEGIN_CODE() \
	...
	crex_execute_data *_old_current_execute_data = EG(current_execute_data); \
...
#define SOAP_CLIENT_END_CODE() \
		...
		if (_old_stack_top != EG(vm_stack_top)) { \
			while (EG(vm_stack)->prev != NULL && \
				...
				crex_vm_stack tmp = EG(vm_stack)->prev; \
				efree(EG(vm_stack)); \
				EG(vm_stack) = tmp; \
...
#define FETCH_THIS_SDL(ss) \
	{ \
		zval *__tmp = C_CLIENT_SDL_P(CREX_THIS); \
		if (C_TYPE_P(__tmp) == IS_RESOURCE) { \
...
#define FETCH_SDL_RES(ss,tmp) ss = (sdlPtr) crex_fetch_resource_ex(tmp, "sdl", le_sdl)

#define FETCH_TYPEMAP_RES(ss,tmp) ss = (HashTable*) crex_fetch_resource_ex(tmp, "typemap", le_typemap)

#define C_PARAM_NAME_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 0))
#define C_PARAM_DATA_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 1))

#define C_HEADER_NAMESPACE_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 0))
#define C_HEADER_NAME_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 1))
#define C_HEADER_DATA_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 2))
#define C_HEADER_MUST_UNDERSTAND_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 3))
#define C_HEADER_ACTOR_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 4))

#define C_SERVER_SOAP_FAULT_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), 0))

/* SoapFault extends Exception, so take those properties into account. */
#define FAULT_PROP_START_OFFSET crex_ce_exception->default_properties_count
#define C_FAULT_STRING_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 0))
#define C_FAULT_CODE_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 1))
#define C_FAULT_CODENS_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 2))
#define C_FAULT_ACTOR_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 3))
#define C_FAULT_DETAIL_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 4))
#define C_FAULT_NAME_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 5))
#define C_FAULT_HEADERFAULT_P(zv) crx_soap_deref(OBJ_PROP_NUM(C_OBJ_P(zv), FAULT_PROP_START_OFFSET + 6))

#define FETCH_THIS_SERVICE(ss) \
	{ \
		ss = soap_server_object_fetch(C_OBJ_P(CREX_THIS))->service; \
		if (!ss) { \
			crex_throw_error(NULL, "Cannot fetch SoapServer object"); \
			SOAP_SERVER_END_CODE(); \
			RETURN_THROWS(); \
		} \
	}

crex_class_entry* soap_class_entry;
static crex_class_entry* soap_server_class_entry;
static crex_class_entry* soap_fault_class_entry;
static crex_class_entry* soap_header_class_entry;
static crex_class_entry* soap_param_class_entry;
crex_class_entry* soap_var_class_entry;

static crex_object_handlers soap_server_object_handlers;
...
static inline soap_server_object *soap_server_object_fetch(crex_object *obj) {
	return (soap_server_object *) ((char *) obj - XtOffsetOf(soap_server_object, std));
}

static crex_object *soap_server_object_create(crex_class_entry *ce)
{
	soap_server_object *obj = crex_object_alloc(sizeof(soap_server_object), ce);
	crex_object_std_init(&obj->std, ce);
	object_properties_init(&obj->std, ce);
	return &obj->std;
}

static void soap_server_object_free(crex_object *obj) {
...
CREX_DECLARE_MODULE_GLOBALS(soap)

static void (*old_error_handler)(int, crex_string *, const uint32_t, crex_string *);

CRX_RINIT_FUNCTION(soap);
CRX_MINIT_FUNCTION(soap);
CRX_MSHUTDOWN_FUNCTION(soap);
CRX_MINFO_FUNCTION(soap);

crex_module_entry soap_module_entry = {
...
#endif
CREX_GET_MODULE(soap)
#endif

CREX_INI_MH(OnUpdateCacheMode)
{
...
static CRX_INI_MH(OnUpdateCacheDir)
{
...
CRX_INI_BEGIN()
STD_CRX_INI_BOOLEAN("soap.wsdl_cache_enabled",     "1", CRX_INI_ALL, OnUpdateBool,
                  cache_enabled, crex_soap_globals, soap_globals)
STD_CRX_INI_ENTRY("soap.wsdl_cache_dir",         "/tmp", CRX_INI_ALL, OnUpdateCacheDir,
                  cache_dir, crex_soap_globals, soap_globals)
STD_CRX_INI_ENTRY("soap.wsdl_cache_ttl",         "86400", CRX_INI_ALL, OnUpdateLong,
                  cache_ttl, crex_soap_globals, soap_globals)
STD_CRX_INI_ENTRY("soap.wsdl_cache",             "1", CRX_INI_ALL, OnUpdateCacheMode,
                  cache_mode, crex_soap_globals, soap_globals)
STD_CRX_INI_ENTRY("soap.wsdl_cache_limit",       "5", CRX_INI_ALL, OnUpdateLong,
                  cache_limit, crex_soap_globals, soap_globals)
CRX_INI_END()

static HashTable defEnc, defEncIndex, defEncNs;

static void crx_soap_prepare_globals(void)
{
	int i;
	const encode* enc;

	crex_hash_init(&defEnc, 0, NULL, NULL, 1);
	crex_hash_init(&defEncIndex, 0, NULL, NULL, 1);
	crex_hash_init(&defEncNs, 0, NULL, NULL, 1);
	...
	do {
		...
		if (defaultEncoding[i].details.type_str) {
			if (defaultEncoding[i].details.ns != NULL) {
				...
				crex_hash_str_add_ptr(&defEnc, ns_type, strlen(ns_type), (void*)enc);
				efree(ns_type);
			} else {
				crex_hash_str_add_ptr(&defEnc, defaultEncoding[i].details.type_str, strlen(defaultEncoding[i].details.type_str), (void*)enc);
			}
		}
		/* Index everything by number */
		crex_hash_index_add_ptr(&defEncIndex, defaultEncoding[i].details.type, (void*)enc);
		i++;
	} while (defaultEncoding[i].details.type != END_KNOWN_TYPES);

	/* hash by namespace */
	crex_hash_str_add_ptr(&defEncNs, XSD_1999_NAMESPACE, sizeof(XSD_1999_NAMESPACE)-1, XSD_NS_PREFIX);
	crex_hash_str_add_ptr(&defEncNs, XSD_NAMESPACE, sizeof(XSD_NAMESPACE)-1, XSD_NS_PREFIX);
	crex_hash_str_add_ptr(&defEncNs, XSI_NAMESPACE, sizeof(XSI_NAMESPACE)-1, XSI_NS_PREFIX);
	crex_hash_str_add_ptr(&defEncNs, XML_NAMESPACE, sizeof(XML_NAMESPACE)-1, XML_NS_PREFIX);
	crex_hash_str_add_ptr(&defEncNs, SOAP_1_1_ENC_NAMESPACE, sizeof(SOAP_1_1_ENC_NAMESPACE)-1, SOAP_1_1_ENC_NS_PREFIX);
	crex_hash_str_add_ptr(&defEncNs, SOAP_1_2_ENC_NAMESPACE, sizeof(SOAP_1_2_ENC_NAMESPACE)-1, SOAP_1_2_ENC_NS_PREFIX);
}

static void crx_soap_init_globals(crex_soap_globals *soap_globals)
{
...
CRX_MSHUTDOWN_FUNCTION(soap)
{
	crex_error_cb = old_error_handler;
	crex_hash_destroy(&SOAP_GLOBAL(defEnc));
	crex_hash_destroy(&SOAP_GLOBAL(defEncIndex));
	crex_hash_destroy(&SOAP_GLOBAL(defEncNs));
	if (SOAP_GLOBAL(mem_cache)) {
		crex_hash_destroy(SOAP_GLOBAL(mem_cache));
...
static void delete_sdl_res(crex_resource *res)
{
	delete_sdl(res->ptr);
}

static void delete_url_res(crex_resource *res)
{
	delete_url(res->ptr);
}

static void delete_hashtable_res(crex_resource *res)
{
...
CRX_MINIT_FUNCTION(soap)
{
	...
	crx_soap_prepare_globals();
	CREX_INIT_MODULE_GLOBALS(soap, crx_soap_init_globals, NULL);
	...
	memcpy(&soap_server_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	...
	soap_fault_class_entry = register_class_SoapFault(crex_ce_exception);
	...
	le_sdl = crex_register_list_destructors_ex(delete_sdl_res, NULL, "SOAP SDL", module_number);
	le_url = crex_register_list_destructors_ex(delete_url_res, NULL, "SOAP URL", module_number);
	le_typemap = crex_register_list_destructors_ex(delete_hashtable_res, NULL, "SOAP table", module_number);

	register_soap_symbols(module_number);

	old_error_handler = crex_error_cb;
	crex_error_cb = soap_error_handler;
...
CRX_MINFO_FUNCTION(soap)
{
	...
	crx_info_print_table_row(2, "Soap Client", "enabled");
	crx_info_print_table_row(2, "Soap Server", "enabled");
...
CRX_METHOD(SoapParam, __main)
{
	zval *data;
	crex_string *name;
	zval *this_ptr;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "zS", &data, &name) == FAILURE) {
		RETURN_THROWS();
	}
	...
	this_ptr = CREX_THIS;
	ZVAL_STR_COPY(C_PARAM_NAME_P(this_ptr), name);
	ZVAL_COPY(C_PARAM_DATA_P(this_ptr), data);
}
/* }}} */


/* {{{ SoapHeader constructor */
CRX_METHOD(SoapHeader, __main)
{
	zval *data = NULL;
	crex_string *ns, *name, *actor_str = NULL;
	crex_long actor_long;
	...
	CREX_PARSE_PARAMETERS_START(2, 5)
		...
		C_PARAM_OPTIONAL
		C_PARAM_ZVAL(data)
		C_PARAM_BOOL(must_understand)
		C_PARAM_STR_OR_LONG_OR_NULL(actor_str, actor_long, actor_is_null)
	...
	this_ptr = CREX_THIS;
	ZVAL_STR_COPY(C_HEADER_NAMESPACE_P(this_ptr), ns);
	ZVAL_STR_COPY(C_HEADER_NAME_P(this_ptr), name);
	if (data) {
		ZVAL_COPY(C_HEADER_DATA_P(this_ptr), data);
	}
	ZVAL_BOOL(C_HEADER_MUST_UNDERSTAND_P(this_ptr), must_understand);

	if (actor_str) {
		if (ZSTR_LEN(actor_str) > 2) {
			ZVAL_STR_COPY(C_HEADER_ACTOR_P(this_ptr), actor_str);
		} else {
			crex_argument_value_error(5, "must be longer than 2 characters");
			RETURN_THROWS();
		}
	} else if (!actor_is_null) {
		if ((actor_long == SOAP_ACTOR_NEXT || actor_long == SOAP_ACTOR_NONE || actor_long == SOAP_ACTOR_UNLIMATERECEIVER)) {
			ZVAL_LONG(C_HEADER_ACTOR_P(this_ptr), actor_long);
		} else {
			crex_argument_value_error(5, "must be one of SOAP_ACTOR_NEXT, SOAP_ACTOR_NONE, or SOAP_ACTOR_UNLIMATERECEIVER");
			RETURN_THROWS();
		}
	}
}
...
CRX_METHOD(SoapFault, __main)
{
	...
	CREX_PARSE_PARAMETERS_START(2, 6)
		C_PARAM_ARRAY_HT_OR_STR_OR_NULL(code_ht, code_str)
		C_PARAM_STRING(fault_string, fault_string_len)
		C_PARAM_OPTIONAL
		C_PARAM_STRING_OR_NULL(fault_actor, fault_actor_len)
		C_PARAM_ZVAL_OR_NULL(details)
		C_PARAM_STRING_OR_NULL(name, name_len)
		C_PARAM_ZVAL_OR_NULL(headerfault)
	CREX_PARSE_PARAMETERS_END();

	if (code_str) {
		fault_code = ZSTR_VAL(code_str);
		fault_code_len = ZSTR_LEN(code_str);
	} else if (code_ht && crex_hash_num_elements(code_ht) == 2) {
		zval *t_ns = crex_hash_index_find(code_ht, 0);
		zval *t_code = crex_hash_index_find(code_ht, 1);
		if (t_ns && t_code && C_TYPE_P(t_ns) == IS_STRING && C_TYPE_P(t_code) == IS_STRING) {
			fault_code_ns = C_STRVAL_P(t_ns);
			fault_code = C_STRVAL_P(t_code);
			fault_code_len = C_STRLEN_P(t_code);
		}
	}

	if ((code_str || code_ht) && (fault_code == NULL || fault_code_len == 0)) {
		crex_argument_value_error(1, "is not a valid fault code");
		RETURN_THROWS();
	}
	...
	if (headerfault != NULL) {
		ZVAL_COPY(C_FAULT_HEADERFAULT_P(this_ptr), headerfault);
	}
}
/* }}} */

/* {{{ SoapFault constructor */
CRX_METHOD(SoapFault, __toString)
{
	zval *faultcode, *faultstring, *file, *line, trace, rv1, rv2, rv3, rv4;
	crex_string *str;
	zval *this_ptr;
	crex_string *faultcode_val, *faultstring_val, *file_val;
	crex_long line_val;
	...
	this_ptr = CREX_THIS;
	faultcode = crex_read_property(soap_fault_class_entry, C_OBJ_P(this_ptr), "faultcode", sizeof("faultcode")-1, 1, &rv1);
	faultstring = crex_read_property(soap_fault_class_entry, C_OBJ_P(this_ptr), "faultstring", sizeof("faultstring")-1, 1, &rv2);
	file = crex_read_property_ex(soap_fault_class_entry, C_OBJ_P(this_ptr), ZSTR_KNOWN(CREX_STR_FILE), /* silent */ true, &rv3);
	line = crex_read_property_ex(soap_fault_class_entry, C_OBJ_P(this_ptr), ZSTR_KNOWN(CREX_STR_LINE), /* silent */ true, &rv4);

	crex_call_method_with_0_params(
		C_OBJ_P(CREX_THIS), C_OBJCE_P(CREX_THIS), NULL, "gettraceasstring", &trace);
	...
	str = strpprintf(0, "SoapFault exception: [%s] %s in %s:" CREX_LONG_FMT "\nStack trace:\n%s",
	               ZSTR_VAL(faultcode_val), ZSTR_VAL(faultstring_val), ZSTR_VAL(file_val), line_val,
	               C_STRLEN(trace) ? C_STRVAL(trace) : "#0 {main}\n");

	crex_string_release_ex(file_val, 0);
	crex_string_release_ex(faultstring_val, 0);
	crex_string_release_ex(faultcode_val, 0);
	zval_ptr_dtor(&trace);
...
/* {{{ SoapVar constructor */
CRX_METHOD(SoapVar, __main)
{
	zval *data, *this_ptr;
	crex_long type;
	bool type_is_null = 1;
	crex_string *stype = NULL, *ns = NULL, *name = NULL, *namens = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "z!l!|S!S!S!S!", &data, &type, &type_is_null, &stype, &ns, &name, &namens) == FAILURE) {
		RETURN_THROWS();
	}

	this_ptr = CREX_THIS;
	if (type_is_null) {
		ZVAL_LONG(C_VAR_ENC_TYPE_P(this_ptr), UNKNOWN_TYPE);
	} else {
		if (crex_hash_index_exists(&SOAP_GLOBAL(defEncIndex), type)) {
			ZVAL_LONG(C_VAR_ENC_TYPE_P(this_ptr), type);
		} else {
			crex_argument_value_error(2, "is not a valid encoding");
			RETURN_THROWS();
		}
	}

	if (data) {
		ZVAL_COPY(C_VAR_ENC_VALUE_P(this_ptr), data);
	}

	if (stype && ZSTR_LEN(stype) != 0) {
		ZVAL_STR_COPY(C_VAR_ENC_STYPE_P(this_ptr), stype);
	}
	if (ns && ZSTR_LEN(ns) != 0) {
		ZVAL_STR_COPY(C_VAR_ENC_NS_P(this_ptr), ns);
	}
	if (name && ZSTR_LEN(name) != 0) {
		ZVAL_STR_COPY(C_VAR_ENC_NAME_P(this_ptr), name);
	}
	if (namens && ZSTR_LEN(namens) != 0) {
		ZVAL_STR_COPY(C_VAR_ENC_NAMENS_P(this_ptr), namens);
	}
}
/* }}} */
...
static HashTable* soap_create_typemap(sdlPtr sdl, HashTable *ht) /* {{{ */
{
	...
	CREX_HASH_FOREACH_VAL(ht, tmp) {
		...
		if (C_TYPE_P(tmp) != IS_ARRAY) {
			crex_type_error("SoapHeader::__main(): \"typemap\" option must be of type array, %s given", crex_zval_value_name(tmp));
			return NULL;
		}
		ht2 = C_ARRVAL_P(tmp);

		if (!HT_IS_PACKED(ht2)) {
			CREX_HASH_MAP_FOREACH_STR_KEY_VAL(ht2, name, tmp) {
				if (name) {
					if (crex_string_equals_literal(name, "type_name")) {
						if (C_TYPE_P(tmp) == IS_STRING) {
							type_name = C_STRVAL_P(tmp);
						} else if (C_TYPE_P(tmp) != IS_NULL) {
						}
					} else if (crex_string_equals_literal(name, "type_ns")) {
						if (C_TYPE_P(tmp) == IS_STRING) {
							type_ns = C_STRVAL_P(tmp);
						} else if (C_TYPE_P(tmp) != IS_NULL) {
						}
					} else if (crex_string_equals_literal(name, "to_xml")) {
						to_xml = tmp;
					} else if (crex_string_equals_literal(name, "from_xml")) {
						to_zval = tmp;
					}
				}
		...
		if (type_name) {
			...
			} else if (enc->details.map && C_TYPE(enc->details.map->to_xml) != IS_UNDEF) {
			...
			} else if (enc->details.map && C_TYPE(enc->details.map->to_zval) != IS_UNDEF) {
				ZVAL_COPY(&new_enc->details.map->to_zval, &enc->details.map->to_zval);
			}
			if (!typemap) {
				typemap = emalloc(sizeof(HashTable));
				crex_hash_init(typemap, 0, NULL, delete_encoder, 0);
			}
			...
			smart_str_0(&nscat);
			crex_hash_update_ptr(typemap, nscat.s, new_enc);
...
CRX_METHOD(SoapServer, __main)
{
	soapServicePtr service;
	zval *options = NULL;
	crex_string *wsdl;
	int version = SOAP_1_1;
	crex_long cache_wsdl;
	HashTable *typemap_ht = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S!|a", &wsdl, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (options != NULL) {
		HashTable *ht = C_ARRVAL_P(options);
		zval *tmp;

		if ((tmp = crex_hash_str_find(ht, "soap_version", sizeof("soap_version")-1)) != NULL) {
			if (C_TYPE_P(tmp) == IS_LONG &&
			    (C_LVAL_P(tmp) == SOAP_1_1 || C_LVAL_P(tmp) == SOAP_1_2)) {
				version = C_LVAL_P(tmp);
			} else {
				crx_error_docref(NULL, E_ERROR, "'soap_version' option must be SOAP_1_1 or SOAP_1_2");
			}
		}

		if ((tmp = crex_hash_str_find(ht, "uri", sizeof("uri")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			service->uri = estrndup(C_STRVAL_P(tmp), C_STRLEN_P(tmp));
		} else if (!wsdl) {
			crx_error_docref(NULL, E_ERROR, "'uri' option is required in nonWSDL mode");
		}

		if ((tmp = crex_hash_str_find(ht, "actor", sizeof("actor")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			service->actor = estrndup(C_STRVAL_P(tmp), C_STRLEN_P(tmp));
		}

		if ((tmp = crex_hash_str_find(ht, "encoding", sizeof("encoding")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			xmlCharEncodingHandlerPtr encoding;

			encoding = xmlFindCharEncodingHandler(C_STRVAL_P(tmp));
			if (encoding == NULL) {
				crx_error_docref(NULL, E_ERROR, "Invalid 'encoding' option - '%s'", C_STRVAL_P(tmp));
			} else {
			  service->encoding = encoding;
			}
		}

		if ((tmp = crex_hash_str_find(ht, "classmap", sizeof("classmap")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_ARRAY) {
			service->class_map = crex_array_dup(C_ARRVAL_P(tmp));
		}

		if ((tmp = crex_hash_str_find(ht, "typemap", sizeof("typemap")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_ARRAY &&
			crex_hash_num_elements(C_ARRVAL_P(tmp)) > 0) {
			typemap_ht = C_ARRVAL_P(tmp);
		}

		if ((tmp = crex_hash_str_find(ht, "features", sizeof("features")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_LONG) {
			service->features = C_LVAL_P(tmp);
		}

		if ((tmp = crex_hash_str_find(ht, "cache_wsdl", sizeof("cache_wsdl")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_LONG) {
			cache_wsdl = C_LVAL_P(tmp);
		}

		if ((tmp = crex_hash_str_find(ht, "send_errors", sizeof("send_errors")-1)) != NULL) {
			if (C_TYPE_P(tmp) == IS_FALSE) {
				service->send_errors = 0;
			} else if (C_TYPE_P(tmp) == IS_TRUE) {
				service->send_errors = 1;
			} else if (C_TYPE_P(tmp) == IS_LONG) {
				service->send_errors = C_LVAL_P(tmp);
			}
		}
	...
	service->soap_functions.ft = crex_new_array(0);

	if (wsdl) {
		service->sdl = get_sdl(CREX_THIS, ZSTR_VAL(wsdl), cache_wsdl);
		if (service->uri == NULL) {
	...
	soap_server_object *server_obj = soap_server_object_fetch(C_OBJ_P(CREX_THIS));
...
CRX_METHOD(SoapServer, setPersistence)
{
	soapServicePtr service;
	crex_long value;
	...
	if (service->type == SOAP_CLASS) {
		if (value == SOAP_PERSISTENCE_SESSION ||
			...
			crex_argument_value_error(
				1, "must be either SOAP_PERSISTENCE_SESSION or SOAP_PERSISTENCE_REQUEST when the SOAP server is used in class mode"
			);
			RETURN_THROWS();
		}
	} else {
		crex_throw_error(NULL, "SoapServer::setPersistence(): Persistence cannot be set when the SOAP server is used in function mode");
		RETURN_THROWS();
	}
...
CRX_METHOD(SoapServer, setClass)
{
	soapServicePtr service;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "C*", &ce, &argv, &num_args) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_METHOD(SoapServer, setObject)
{
	soapServicePtr service;
	zval *obj;
	...
	ZVAL_OBJ_COPY(&service->soap_object, C_OBJ_P(obj));
...
CRX_METHOD(SoapServer, getFunctions)
{
	soapServicePtr  service;
	HashTable      *ft = NULL;
	...
	if (service->type == SOAP_OBJECT) {
		ft = &(C_OBJCE(service->soap_object)->function_table);
		...
		CREX_HASH_MAP_FOREACH_VAL(service->soap_functions.ft, name) {
			add_next_index_str(return_value, crex_string_copy(C_STR_P(name)));
		} CREX_HASH_FOREACH_END();
	}
	if (ft != NULL) {
		crex_function *f;

		CREX_HASH_MAP_FOREACH_PTR(ft, f) {
			if ((service->type != SOAP_OBJECT && service->type != SOAP_CLASS) || (f->common.fn_flags & CREX_ACC_PUBLIC)) {
				add_next_index_str(return_value, crex_string_copy(f->common.function_name));
			}
		} CREX_HASH_FOREACH_END();
	}
...
CRX_METHOD(SoapServer, addFunction)
{
	soapServicePtr service;
	zval *function_name, function_copy;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "z", &function_name) == FAILURE) {
		RETURN_THROWS();
	}
	...
	/* TODO: could use crex_is_callable here */

	if (C_TYPE_P(function_name) == IS_ARRAY) {
		if (service->type == SOAP_FUNCTIONS) {
			zval *tmp_function;

			if (service->soap_functions.ft == NULL) {
				service->soap_functions.functions_all = FALSE;
				service->soap_functions.ft = crex_new_array(crex_hash_num_elements(C_ARRVAL_P(function_name)));
			}

			CREX_HASH_FOREACH_VAL(C_ARRVAL_P(function_name), tmp_function) {
				crex_string *key;
				crex_function *f;

				if (C_TYPE_P(tmp_function) != IS_STRING) {
					crex_argument_type_error(1, "must contain only strings");
					RETURN_THROWS();
				}

				key = crex_string_tolower(C_STR_P(tmp_function));

				if ((f = crex_hash_find_ptr(EG(function_table), key)) == NULL) {
					crex_type_error("SoapServer::addFunction(): Function \"%s\" not found", C_STRVAL_P(tmp_function));
					RETURN_THROWS();
				}

				ZVAL_STR_COPY(&function_copy, f->common.function_name);
				crex_hash_update(service->soap_functions.ft, key, &function_copy);

				crex_string_release_ex(key, 0);
			} CREX_HASH_FOREACH_END();
		}
	} else if (C_TYPE_P(function_name) == IS_STRING) {
		crex_string *key;
		crex_function *f;

		key = crex_string_tolower(C_STR_P(function_name));

		if ((f = crex_hash_find_ptr(EG(function_table), key)) == NULL) {
			crex_argument_type_error(1, "must be a valid function name, function \"%s\" not found", C_STRVAL_P(function_name));
			RETURN_THROWS();
		}
		if (service->soap_functions.ft == NULL) {
			service->soap_functions.functions_all = FALSE;
		...
		crex_hash_update(service->soap_functions.ft, key, &function_copy);
		crex_string_release_ex(key, 0);
	} else if (C_TYPE_P(function_name) == IS_LONG) {
		if (C_LVAL_P(function_name) == SOAP_FUNCTIONS_ALL) {
			if (service->soap_functions.ft != NULL) {
				crex_hash_destroy(service->soap_functions.ft);
				efree(service->soap_functions.ft);
				service->soap_functions.ft = NULL;
			}
			service->soap_functions.functions_all = TRUE;
		} else {
			crex_argument_value_error(1, "must be SOAP_FUNCTIONS_ALL when an integer is passed");
			RETURN_THROWS();
		}
	} else {
		crex_argument_type_error(1, "must be of type array|string|int, %s given", crex_zval_value_name(function_name));
		RETURN_THROWS();
	}
...
static void _soap_server_exception(soapServicePtr service, sdlFunctionPtr function, zval *this_ptr) /* {{{ */
{
	...
	if (instanceof_function(C_OBJCE(exception_object), soap_fault_class_entry)) {
		soap_server_fault_ex(function, &exception_object, NULL);
	} else if (instanceof_function(C_OBJCE(exception_object), crex_ce_error)) {
		if (service->send_errors) {
			zval rv;
			crex_string *msg = zval_get_string(crex_read_property_ex(crex_ce_error, C_OBJ(exception_object), ZSTR_KNOWN(CREX_STR_MESSAGE), /* silent */ false, &rv));
			add_soap_fault_ex(&exception_object, this_ptr, "Server", ZSTR_VAL(msg), NULL, NULL);
			crex_string_release_ex(msg, 0);
		} else {
...
CRX_METHOD(SoapServer, handle)
{
	...
	if (arg && CREX_SIZE_T_INT_OVFL(arg_len)) {
...
*/
	...
	if (crx_output_start_default() != SUCCESS) {
		crx_error_docref(NULL, E_ERROR,"ob_start failed");
	}

	if (!arg) {
		if (SG(request_info).request_body && 0 == crx_stream_rewind(SG(request_info).request_body)) {
			zval *server_vars, *encoding;
			crx_stream_filter *zf = NULL;
			crex_string *server = ZSTR_KNOWN(CREX_STR_AUTOGLOBAL_SERVER);

			crex_is_auto_global(server);
			if ((server_vars = crex_hash_find(&EG(symbol_table), server)) != NULL &&
			    C_TYPE_P(server_vars) == IS_ARRAY &&
			    (encoding = crex_hash_str_find(C_ARRVAL_P(server_vars), "HTTP_CONTENT_ENCODING", sizeof("HTTP_CONTENT_ENCODING")-1)) != NULL &&
			    C_TYPE_P(encoding) == IS_STRING) {

				if (crex_string_equals_literal(C_STR_P(encoding), "gzip")
					|| crex_string_equals_literal(C_STR_P(encoding), "x-gzip")
					|| crex_string_equals_literal(C_STR_P(encoding), "deflate")
				) {
					zval filter_params;
					...
					zf = crx_stream_filter_create("zlib.inflate", &filter_params, 0);
					crex_array_destroy(C_ARR(filter_params));

					if (zf) {
						crx_stream_filter_append(&SG(request_info).request_body->readfilters, zf);
					} else {
						crx_error_docref(NULL, E_WARNING,"Can't uncompress compressed request");
						return;
					}
				} else {
					crx_error_docref(NULL, E_WARNING,"Request is compressed with unknown compression '%s'",C_STRVAL_P(encoding));
					return;
				}
			}

			doc_request = soap_xmlParseFile("crx://input");

			if (zf) {
				crx_stream_filter_remove(zf, 1);
			}
		} else {
	...
	if (EG(exception)) {
		if (!crex_is_unwind_exit(EG(exception))) {
			crx_output_discard();
			_soap_server_exception(service, function, CREX_THIS);
		}
		goto fail;
	}
	...
	if (service->type == SOAP_OBJECT) {
		...
		function_table = &((C_OBJCE_P(soap_obj))->function_table);
	} else if (service->type == SOAP_CLASS) {
#if defined(HAVE_CRX_SESSION) && !defined(COMPILE_DL_SESSION)
		...
		if (service->soap_class.persistence == SOAP_PERSISTENCE_SESSION) {
			...
			if (C_TYPE_P(session_vars) == IS_ARRAY &&
			    (tmp_soap_p = crex_hash_str_find(C_ARRVAL_P(session_vars), "_bogus_session_name", sizeof("_bogus_session_name")-1)) != NULL &&
			    C_TYPE_P(tmp_soap_p) == IS_OBJECT &&
			    C_OBJCE_P(tmp_soap_p) == service->soap_class.ce) {
				soap_obj = tmp_soap_p;
			}
		}
#endif
		...
		if (soap_obj == NULL) {
			...
			if (service->soap_class.ce->constructor) {
				crex_call_known_instance_method(
					service->soap_class.ce->constructor, C_OBJ(tmp_soap), NULL,
			...
			if (service->soap_class.persistence == SOAP_PERSISTENCE_SESSION) {
				...
				ZVAL_DEREF(session_vars);
				if (C_TYPE_P(session_vars) == IS_ARRAY &&
				    (tmp_soap_p = crex_hash_str_update(C_ARRVAL_P(session_vars), "_bogus_session_name", sizeof("_bogus_session_name")-1, &tmp_soap)) != NULL) {
					soap_obj = tmp_soap_p;
				} else {
					soap_obj = &tmp_soap;
				}
...
#endif
			crex_string *fn_name = crex_string_tolower(C_STR(h->function_name));
			if (crex_hash_exists(function_table, fn_name) ||
			    ((service->type == SOAP_CLASS || service->type == SOAP_OBJECT) &&
			     crex_hash_str_exists(function_table, CREX_CALL_FUNC_NAME, sizeof(CREX_CALL_FUNC_NAME)-1))) {
				...
				if (call_status != SUCCESS) {
					crx_error_docref(NULL, E_WARNING, "Function '%s' call failed", C_STRVAL(h->function_name));
					return;
				}
				if (C_TYPE(h->retval) == IS_OBJECT &&
				    instanceof_function(C_OBJCE(h->retval), soap_fault_class_entry)) {
					crx_output_discard();
	...
	crex_string *fn_name = crex_string_tolower(C_STR(function_name));
	if (crex_hash_exists(function_table, fn_name) ||
	    ((service->type == SOAP_CLASS || service->type == SOAP_OBJECT) &&
	     crex_hash_str_exists(function_table, CREX_CALL_FUNC_NAME, sizeof(CREX_CALL_FUNC_NAME)-1))) {
		if (service->type == SOAP_CLASS || service->type == SOAP_OBJECT) {
		...
		crx_error(E_ERROR, "Function '%s' doesn't exist", C_STRVAL(function_name));
	}
	crex_string_release(fn_name);
	...
	if (call_status == SUCCESS) {
		...
		if (C_TYPE(retval) == IS_OBJECT &&
		    instanceof_function(C_OBJCE(retval), soap_fault_class_entry)) {
			crx_output_discard();
		...
		if (function && function->responseName) {
			...
			response_name = emalloc(C_STRLEN(function_name) + sizeof("Response"));
			memcpy(response_name,C_STRVAL(function_name),C_STRLEN(function_name));
			memcpy(response_name+C_STRLEN(function_name),"Response",sizeof("Response"));
		}
		doc_return = serialize_response_call(function, response_name, service->uri, &retval, soap_headers, soap_version);
		efree(response_name);
	} else {
		crx_error_docref(NULL, E_WARNING, "Function '%s' call failed", C_STRVAL(function_name));
		return;
	}
	...
	if (doc_return) {
		...
		if (size == 0) {
			crx_error_docref(NULL, E_ERROR, "Dump memory failed");
		}
		...
		if (crex_ini_long("zlib.output_compression", sizeof("zlib.output_compression"), 0)) {
...
CRX_METHOD(SoapServer, fault)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss|szs",
...
/* {{{ */
CRX_METHOD(SoapServer, addSoapHeader)
{
	soapServicePtr service;
	zval *fault;
	soapHeader **p;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &fault, soap_header_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!service || !service->soap_headers_ptr) {
		crex_throw_error(NULL, "SoapServer::addSoapHeader() may be called only during SOAP request processing");
		RETURN_THROWS();
	}
	...
	*p = emalloc(sizeof(soapHeader));
	...
	ZVAL_OBJ_COPY(&(*p)->retval, C_OBJ_P(fault));
...
static void soap_server_fault_ex(sdlFunctionPtr function, zval* fault, soapHeader *hdr) /* {{{ */
{
	...
	if ((C_TYPE(PG(http_globals)[TRACK_VARS_SERVER]) == IS_ARRAY || crex_is_auto_global(ZSTR_KNOWN(CREX_STR_AUTOGLOBAL_SERVER))) &&
		(agent_name = crex_hash_str_find(C_ARRVAL(PG(http_globals)[TRACK_VARS_SERVER]), "HTTP_USER_AGENT", sizeof("HTTP_USER_AGENT")-1)) != NULL &&
		C_TYPE_P(agent_name) == IS_STRING) {
		if (crex_string_equals_literal(C_STR_P(agent_name), "Shockwave Flash")) {
...
static crex_never_inline CREX_COLD void soap_real_error_handler(int error_num, crex_string *error_filename, const uint32_t error_lineno, crex_string *message) /* {{{ */
{
	bool _old_in_compilation;
	crex_execute_data *_old_current_execute_data;
	...
	if (C_OBJ(SOAP_GLOBAL(error_object)) &&
	    instanceof_function(C_OBJCE(SOAP_GLOBAL(error_object)), soap_class_entry)) {
		bool use_exceptions =
			C_TYPE_P(C_CLIENT_EXCEPTIONS_P(&SOAP_GLOBAL(error_object))) != IS_FALSE;
		if ((error_num & E_FATAL_ERRORS) && use_exceptions) {
			...
			C_ADDREF(fault);
			crex_throw_exception_object(&fault);
			crex_bailout();
		...
		if (error_num & E_FATAL_ERRORS) {
			...
			if (C_OBJ_P(error_object) &&
			    instanceof_function(C_OBJCE_P(error_object), soap_server_class_entry) &&
				(service = soap_server_object_fetch(C_OBJ_P(error_object))->service) &&
				!service->send_errors) {
				buffer = ZSTR_INIT_LITERAL("Internal Error", 0);
			} else {
				buffer = crex_string_copy(message);

				/* Get output buffer and send as fault detials */
				zval outbuflen;
				if (crx_output_get_length(&outbuflen) != FAILURE && C_LVAL(outbuflen) != 0) {
					crx_output_get_contents(&outbuf);
				}
				crx_output_discard();
...
/* {{{ */
CRX_FUNCTION(use_soap_error_handler)
{
	bool handler = 1;

	ZVAL_BOOL(return_value, SOAP_GLOBAL(use_soap_error_handler));
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|b", &handler) == SUCCESS) {
...
/* {{{ */
CRX_FUNCTION(is_soap_fault)
{
	zval *fault;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "z", &fault) == SUCCESS &&
	    C_TYPE_P(fault) == IS_OBJECT &&
	    instanceof_function(C_OBJCE_P(fault), soap_fault_class_entry)) {
		RETURN_TRUE;
	}
	RETURN_FALSE;
}
...
CRX_METHOD(SoapClient, __main)
{
	...
	if (options != NULL) {
		...
		if (!wsdl) {
			...
			if ((tmp = crex_hash_str_find(ht, "uri", sizeof("uri")-1)) != NULL &&
			    C_TYPE_P(tmp) == IS_STRING) {
				ZVAL_STR_COPY(C_CLIENT_URI_P(this_ptr), C_STR_P(tmp));
			} else {
				crx_error_docref(NULL, E_ERROR, "'uri' option is required in nonWSDL mode");
			}

			if ((tmp = crex_hash_str_find(ht, "style", sizeof("style")-1)) != NULL &&
					C_TYPE_P(tmp) == IS_LONG &&
					(C_LVAL_P(tmp) == SOAP_RPC || C_LVAL_P(tmp) == SOAP_DOCUMENT)) {
				ZVAL_LONG(C_CLIENT_STYLE_P(this_ptr), C_LVAL_P(tmp));
			}

			if ((tmp = crex_hash_str_find(ht, "use", sizeof("use")-1)) != NULL &&
					C_TYPE_P(tmp) == IS_LONG &&
					(C_LVAL_P(tmp) == SOAP_LITERAL || C_LVAL_P(tmp) == SOAP_ENCODED)) {
				ZVAL_LONG(C_CLIENT_USE_P(this_ptr), C_LVAL_P(tmp));
			}
		}

		if ((tmp = crex_hash_str_find(ht, "stream_context", sizeof("stream_context")-1)) != NULL &&
				C_TYPE_P(tmp) == IS_RESOURCE) {
			context = crx_stream_context_from_zval(tmp, 1);
			C_ADDREF_P(tmp);
		} else {
			context = crx_stream_context_alloc();
		}

		if ((tmp = crex_hash_str_find(ht, "location", sizeof("location")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			ZVAL_STR_COPY(C_CLIENT_LOCATION_P(this_ptr), C_STR_P(tmp));
		} else if (!wsdl) {
			crx_error_docref(NULL, E_ERROR, "'location' option is required in nonWSDL mode");
		}

		if ((tmp = crex_hash_str_find(ht, "soap_version", sizeof("soap_version")-1)) != NULL) {
			if (C_TYPE_P(tmp) == IS_LONG &&
			    (C_LVAL_P(tmp) == SOAP_1_1 || C_LVAL_P(tmp) == SOAP_1_2)) {
				soap_version = C_LVAL_P(tmp);
			}
		}
		if ((tmp = crex_hash_str_find(ht, "login", sizeof("login")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			ZVAL_STR_COPY(C_CLIENT_LOGIN_P(this_ptr), C_STR_P(tmp));
			if ((tmp = crex_hash_str_find(ht, "password", sizeof("password")-1)) != NULL &&
			    C_TYPE_P(tmp) == IS_STRING) {
				ZVAL_STR_COPY(C_CLIENT_PASSWORD_P(this_ptr), C_STR_P(tmp));
			}
			if ((tmp = crex_hash_str_find(ht, "authentication", sizeof("authentication")-1)) != NULL &&
			    C_TYPE_P(tmp) == IS_LONG &&
			    C_LVAL_P(tmp) == SOAP_AUTHENTICATION_DIGEST) {
				ZVAL_TRUE(C_CLIENT_USE_DIGEST_P(this_ptr));
			}
		}
		if ((tmp = crex_hash_str_find(ht, "proxy_host", sizeof("proxy_host")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			ZVAL_STR_COPY(C_CLIENT_PROXY_HOST_P(this_ptr), C_STR_P(tmp));
			if ((tmp = crex_hash_str_find(ht, "proxy_port", sizeof("proxy_port")-1)) != NULL) {
				if (C_TYPE_P(tmp) != IS_LONG) {
					ZVAL_LONG(&tmp2, zval_get_long(tmp));
					tmp = &tmp2;
				}
				ZVAL_LONG(C_CLIENT_PROXY_PORT_P(this_ptr), C_LVAL_P(tmp));
			}
			if ((tmp = crex_hash_str_find(ht, "proxy_login", sizeof("proxy_login")-1)) != NULL &&
			    C_TYPE_P(tmp) == IS_STRING) {
				ZVAL_STR_COPY(C_CLIENT_PROXY_LOGIN_P(this_ptr), C_STR_P(tmp));
				if ((tmp = crex_hash_str_find(ht, "proxy_password", sizeof("proxy_password")-1)) != NULL &&
				    C_TYPE_P(tmp) == IS_STRING) {
					ZVAL_STR_COPY(C_CLIENT_PROXY_PASSWORD_P(this_ptr), C_STR_P(tmp));
				}
			}
		}
		if ((tmp = crex_hash_str_find(ht, "local_cert", sizeof("local_cert")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			if (!context) {
				context = crx_stream_context_alloc();
			}
			crx_stream_context_set_option(context, "ssl", "local_cert", tmp);
			if ((tmp = crex_hash_str_find(ht, "passphrase", sizeof("passphrase")-1)) != NULL &&
			    C_TYPE_P(tmp) == IS_STRING) {
				crx_stream_context_set_option(context, "ssl", "passphrase", tmp);
			}
		}
		if ((tmp = crex_hash_find(ht, ZSTR_KNOWN(CREX_STR_TRACE))) != NULL &&
		    (C_TYPE_P(tmp) == IS_TRUE ||
		     (C_TYPE_P(tmp) == IS_LONG && C_LVAL_P(tmp) == 1))) {
			ZVAL_TRUE(C_CLIENT_TRACE_P(this_ptr));
		}

		if ((tmp = crex_hash_str_find(ht, "exceptions", sizeof("exceptions")-1)) != NULL &&
		    (C_TYPE_P(tmp) == IS_FALSE ||
		     (C_TYPE_P(tmp) == IS_LONG && C_LVAL_P(tmp) == 0))) {
			ZVAL_FALSE(C_CLIENT_EXCEPTIONS_P(this_ptr));
		}

		if ((tmp = crex_hash_str_find(ht, "compression", sizeof("compression")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_LONG &&
	      crex_hash_str_exists(EG(function_table), "gzinflate", sizeof("gzinflate")-1) &&
	      crex_hash_str_exists(EG(function_table), "gzdeflate", sizeof("gzdeflate")-1) &&
	      crex_hash_str_exists(EG(function_table), "gzuncompress", sizeof("gzuncompress")-1) &&
	      crex_hash_str_exists(EG(function_table), "gzcompress", sizeof("gzcompress")-1) &&
	      crex_hash_str_exists(EG(function_table), "gzencode", sizeof("gzencode")-1)) {
			ZVAL_LONG(C_CLIENT_COMPRESSION_P(this_ptr), C_LVAL_P(tmp));
		}
		if ((tmp = crex_hash_str_find(ht, "encoding", sizeof("encoding")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			xmlCharEncodingHandlerPtr encoding;
			...
			if (encoding == NULL) {
				...
				ZVAL_STR_COPY(C_CLIENT_ENCODING_P(this_ptr), C_STR_P(tmp));
			}
		}
		if ((tmp = crex_hash_str_find(ht, "classmap", sizeof("classmap")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_ARRAY) {
			ZVAL_COPY(C_CLIENT_CLASSMAP_P(this_ptr), tmp);
		}
		...
		if ((tmp = crex_hash_str_find(ht, "features", sizeof("features")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_LONG) {
			ZVAL_LONG(C_CLIENT_FEATURES_P(this_ptr), C_LVAL_P(tmp));
		}

		if ((tmp = crex_hash_str_find(ht, "connection_timeout", sizeof("connection_timeout")-1)) != NULL) {
			crex_long lval = zval_get_long(tmp);
			if (lval > 0) {
				ZVAL_LONG(C_CLIENT_CONNECTION_TIMEOUT_P(this_ptr), lval);
			}
		}

		if (context) {
			ZVAL_RES(C_CLIENT_STREAM_CONTEXT_P(this_ptr), context->res);
		}
		...
		if ((tmp = crex_hash_str_find(ht, "user_agent", sizeof("user_agent")-1)) != NULL &&
		    C_TYPE_P(tmp) == IS_STRING) {
			ZVAL_STR_COPY(C_CLIENT_USER_AGENT_P(this_ptr), C_STR_P(tmp));
		}

		if ((tmp = crex_hash_str_find(ht, "keep_alive", sizeof("keep_alive")-1)) != NULL &&
				(C_TYPE_P(tmp) == IS_FALSE ||
				 (C_TYPE_P(tmp) == IS_LONG && C_LVAL_P(tmp) == 0))) {
			ZVAL_FALSE(C_CLIENT_KEEP_ALIVE_P(this_ptr));
		}

		if ((tmp = crex_hash_str_find(ht, "ssl_method", sizeof("ssl_method")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_LONG) {
			ZVAL_LONG(C_CLIENT_SSL_METHOD_P(this_ptr), C_LVAL_P(tmp));
			crx_error_docref(NULL, E_DEPRECATED,
				"The \"ssl_method\" option is deprecated. "
				"Use \"ssl\" stream context options instead");
		}
	} else if (!wsdl) {
		crx_error_docref(NULL, E_ERROR, "'location' and 'uri' options are required in nonWSDL mode");
	}

	ZVAL_LONG(C_CLIENT_SOAP_VERSION_P(this_ptr), soap_version);
	...
	if (wsdl) {
		...
		res = crex_register_resource(sdl, le_sdl);

		ZVAL_RES(C_CLIENT_SDL_P(this_ptr), res);
	...
	if (typemap_ht) {
		...
		if (typemap) {
			crex_resource *res = crex_register_resource(typemap, le_typemap);
			ZVAL_RES(C_CLIENT_TYPEMAP_P(this_ptr), res);
		}
	}
...
static int do_request(zval *this_ptr, xmlDoc *request, char *location, char *action, int version, bool one_way, zval *response) /* {{{ */
{
	...
	crex_try {
		zval *trace = C_CLIENT_TRACE_P(this_ptr);
		if (C_TYPE_P(trace) == IS_TRUE) {
			zval_ptr_dtor(C_CLIENT_LAST_REQUEST_P(this_ptr));
			ZVAL_STRINGL(C_CLIENT_LAST_REQUEST_P(this_ptr), buf, buf_size);
		}
		...
		if (call_user_function(NULL, this_ptr, &func, response, 5, params) != SUCCESS) {
			...
			ret = FALSE;
		} else if (C_TYPE_P(response) != IS_STRING) {
			if (EG(exception) && instanceof_function(EG(exception)->ce, crex_ce_error)) {
				/* Programmer error in __doRequest() implementation, let it bubble up. */
			} else if (C_TYPE_P(C_CLIENT_SOAP_FAULT_P(this_ptr)) != IS_OBJECT) {
				add_soap_fault(this_ptr, "Client", "SoapClient::__doRequest() returned non string value", NULL, NULL);
			}
			ret = FALSE;
		} else if (C_TYPE_P(trace) == IS_TRUE) {
			zval_ptr_dtor(C_CLIENT_LAST_RESPONSE_P(this_ptr));
			ZVAL_STR_COPY(C_CLIENT_LAST_RESPONSE_P(this_ptr), C_STR_P(response));
		}
	} crex_catch {
		_bailout = 1;
	} crex_end_try();
	...
	if (ret && C_TYPE_P(C_CLIENT_SOAP_FAULT_P(this_ptr)) == IS_OBJECT) {
		ret = FALSE;
	}
	return ret;
}
...
static void do_soap_call(crex_execute_data *execute_data,
                         zval* this_ptr,
                         char* function,
                         size_t function_len,
                         int arg_count,
                        ...
                        ) /* {{{ */
{
	...
	if (C_TYPE_P(C_CLIENT_TRACE_P(this_ptr)) == IS_TRUE) {
		convert_to_null(C_CLIENT_LAST_REQUEST_P(this_ptr));
		convert_to_null(C_CLIENT_LAST_RESPONSE_P(this_ptr));
	}

	zval *tmp = C_CLIENT_SOAP_VERSION_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_LONG && C_LVAL_P(tmp) == SOAP_1_2) {
	...
	if (location == NULL) {
		tmp = C_CLIENT_LOCATION_P(this_ptr);
		if (C_TYPE_P(tmp) == IS_STRING) {
			location = C_STRVAL_P(tmp);
		}
	}

	tmp = C_CLIENT_SDL_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_RESOURCE) {
		FETCH_SDL_RES(sdl,tmp);
	}

	tmp = C_CLIENT_TYPEMAP_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_RESOURCE) {
	...
	SOAP_GLOBAL(sdl) = sdl;
	old_encoding = SOAP_GLOBAL(encoding);
	tmp = C_CLIENT_ENCODING_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_STRING) {
		SOAP_GLOBAL(encoding) = xmlFindCharEncodingHandler(C_STRVAL_P(tmp));
	} else {
		SOAP_GLOBAL(encoding) = NULL;
	}
	old_class_map = SOAP_GLOBAL(class_map);
	tmp = C_CLIENT_CLASSMAP_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_ARRAY) {
		SOAP_GLOBAL(class_map) = C_ARRVAL_P(tmp);
	} else {
		SOAP_GLOBAL(class_map) = NULL;
	}
	old_typemap = SOAP_GLOBAL(typemap);
	...
	old_features = SOAP_GLOBAL(features);
	tmp = C_CLIENT_FEATURES_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_LONG) {
		SOAP_GLOBAL(features) = C_LVAL_P(tmp);
	} else {
		SOAP_GLOBAL(features) = 0;
	}
	...
	crex_try {
	 	if (sdl != NULL) {
			...
			if (fn != NULL) {
				...
				if (ret && C_TYPE(response) == IS_STRING) {
					encode_reset_ns();
					ret = parse_packet_soap(this_ptr, C_STRVAL(response), C_STRLEN(response), fn, NULL, return_value, output_headers);
					encode_finish();
				}
			...
			zval *uri = C_CLIENT_URI_P(this_ptr);
			if (C_TYPE_P(uri) != IS_STRING) {
				...
				if (ret && C_TYPE(response) == IS_STRING) {
					encode_reset_ns();
					ret = parse_packet_soap(this_ptr, C_STRVAL(response), C_STRLEN(response), NULL, function, return_value, output_headers);
					encode_finish();
				}
		...
		zval *fault = C_CLIENT_SOAP_FAULT_P(this_ptr);
		if (!ret) {
			if (C_TYPE_P(fault) == IS_OBJECT) {
		...
		if (!EG(exception) &&
		    C_TYPE_P(return_value) == IS_OBJECT &&
		    instanceof_function(C_OBJCE_P(return_value), soap_fault_class_entry) &&
		    C_TYPE_P(C_CLIENT_EXCEPTIONS_P(this_ptr)) != IS_FALSE) {
			C_ADDREF_P(return_value);
			crex_throw_exception_object(return_value);
		}
...
static void verify_soap_headers_array(HashTable *ht) /* {{{ */
{
	...
	CREX_HASH_FOREACH_VAL(ht, tmp) {
		if (C_TYPE_P(tmp) != IS_OBJECT ||
		    !instanceof_function(C_OBJCE_P(tmp), soap_header_class_entry)) {
			crx_error_docref(NULL, E_ERROR, "Invalid SOAP header");
		}
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
void soap_client_call_impl(INTERNAL_FUNCTION_PARAMETERS, bool is_soap_call)
{
	...
	if (is_soap_call) {
		if (crex_parse_parameters(CREX_NUM_ARGS(), "sa|a!zz",
			&function, &function_len, &args, &options, &headers, &output_headers) == FAILURE) {
			RETURN_THROWS();
		}
	} else {
		if (crex_parse_parameters(CREX_NUM_ARGS(), "sa", &function, &function_len, &args) == FAILURE) {
			RETURN_THROWS();
		}
	}

	if (options) {
		HashTable *hto = C_ARRVAL_P(options);
		if ((tmp = crex_hash_str_find(hto, "location", sizeof("location")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_STRING) {
			location = C_STRVAL_P(tmp);
		}

		if ((tmp = crex_hash_str_find(hto, "soapaction", sizeof("soapaction")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_STRING) {
			soap_action = C_STRVAL_P(tmp);
		}

		if ((tmp = crex_hash_str_find(hto, "uri", sizeof("uri")-1)) != NULL &&
			C_TYPE_P(tmp) == IS_STRING) {
			uri = C_STRVAL_P(tmp);
		}
	}

	if (headers == NULL || C_TYPE_P(headers) == IS_NULL) {
	} else if (C_TYPE_P(headers) == IS_ARRAY) {
		soap_headers = C_ARRVAL_P(headers);
		verify_soap_headers_array(soap_headers);
		free_soap_headers = 0;
	} else if (C_TYPE_P(headers) == IS_OBJECT &&
	           instanceof_function(C_OBJCE_P(headers), soap_header_class_entry)) {
	    soap_headers = crex_new_array(0);
		crex_hash_next_index_insert(soap_headers, headers);
		C_ADDREF_P(headers);
		free_soap_headers = 1;
	} else {
		crex_argument_type_error(4, "must be of type SoapHeader|array|null, %s given", crex_zval_value_name(headers));
		RETURN_THROWS();
	}

	/* Add default headers */
	this_ptr = CREX_THIS;
	tmp = C_CLIENT_DEFAULT_HEADERS_P(this_ptr);
	if (C_TYPE_P(tmp) == IS_ARRAY) {
		HashTable *default_headers = C_ARRVAL_P(tmp);
		if (soap_headers) {
			if (!free_soap_headers) {
				soap_headers = crex_array_dup(soap_headers);
				free_soap_headers = 1;
			}
			CREX_HASH_FOREACH_VAL(default_headers, tmp) {
				if(C_TYPE_P(tmp) == IS_OBJECT) {
					C_ADDREF_P(tmp);
					crex_hash_next_index_insert(soap_headers, tmp);
				}
			} CREX_HASH_FOREACH_END();
		} else {
			soap_headers = C_ARRVAL_P(tmp);
			free_soap_headers = 0;
		}
	}

	arg_count = crex_hash_num_elements(C_ARRVAL_P(args));

	if (arg_count > 0) {
		real_args = safe_emalloc(sizeof(zval), arg_count, 0);
		CREX_HASH_FOREACH_VAL(C_ARRVAL_P(args), param) {
			/*zval_add_ref(param);*/
			ZVAL_DEREF(param);
	...
	if (output_headers) {
		output_headers = crex_try_array_init(output_headers);
		if (!output_headers) {
			goto cleanup;
		}
	}
...
cleanup:
	...
	if (soap_headers && free_soap_headers) {
		crex_hash_destroy(soap_headers);
		efree(soap_headers);
	}
}
/* }}} */

CRX_METHOD(SoapClient, __call)
{
	soap_client_call_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}

CRX_METHOD(SoapClient, __soapCall)
{
	soap_client_call_impl(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}

/* {{{ Returns list of SOAP functions */
CRX_METHOD(SoapClient, __getFunctions)
{
	sdlPtr sdl;
	...
	if (sdl) {
		...
		array_init(return_value);
		CREX_HASH_MAP_FOREACH_PTR(&sdl->functions, function) {
...
CRX_METHOD(SoapClient, __getTypes)
{
	sdlPtr sdl;
	...
	if (sdl) {
		...
		array_init(return_value);
		if (sdl->types) {
			CREX_HASH_FOREACH_PTR(sdl->types, type) {
...
CRX_METHOD(SoapClient, __getLastRequest)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}

	RETURN_COPY(C_CLIENT_LAST_REQUEST_P(CREX_THIS));
}
/* }}} */


/* {{{ Returns last SOAP response */
CRX_METHOD(SoapClient, __getLastResponse)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}

	RETURN_COPY(C_CLIENT_LAST_RESPONSE_P(CREX_THIS));
}
/* }}} */


/* {{{ Returns last SOAP request headers */
CRX_METHOD(SoapClient, __getLastRequestHeaders)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}

	RETURN_COPY(C_CLIENT_LAST_REQUEST_HEADERS_P(CREX_THIS));
}
/* }}} */


/* {{{ Returns last SOAP response headers */
CRX_METHOD(SoapClient, __getLastResponseHeaders)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}

	RETURN_COPY(C_CLIENT_LAST_RESPONSE_HEADERS_P(CREX_THIS));
}
/* }}} */


/* {{{ SoapClient::__doRequest() */
CRX_METHOD(SoapClient, __doRequest)
{
	crex_string *buf;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Sssl|b",
	    &buf,
...
CRX_METHOD(SoapClient, __setCookie)
{
	crex_string *name, *val = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S|S!", &name, &val) == FAILURE) {
		RETURN_THROWS();
	}

	zval *cookies = C_CLIENT_COOKIES_P(CREX_THIS);
	SEPARATE_ARRAY(cookies);
	if (val == NULL) {
		crex_hash_del(C_ARRVAL_P(cookies), name);
	} else {
		zval zcookie;
		array_init(&zcookie);
		add_index_str(&zcookie, 0, crex_string_copy(val));
		crex_hash_update(C_ARRVAL_P(cookies), name, &zcookie);
	}
}
/* }}} */

/* {{{ Returns list of cookies */
CRX_METHOD(SoapClient, __getCookies)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}

	RETURN_COPY(C_CLIENT_COOKIES_P(CREX_THIS));
}
/* }}} */
...
CRX_METHOD(SoapClient, __setSoapHeaders)
{
	zval *headers = NULL;
	zval *this_ptr = CREX_THIS;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|z", &headers) == FAILURE) {
		RETURN_THROWS();
	}

	if (headers == NULL || C_TYPE_P(headers) == IS_NULL) {
		convert_to_null(C_CLIENT_DEFAULT_HEADERS_P(this_ptr));
	} else if (C_TYPE_P(headers) == IS_ARRAY) {
		verify_soap_headers_array(C_ARRVAL_P(headers));
		zval_ptr_dtor(C_CLIENT_DEFAULT_HEADERS_P(this_ptr));
		ZVAL_COPY(C_CLIENT_DEFAULT_HEADERS_P(this_ptr), headers);
	} else if (C_TYPE_P(headers) == IS_OBJECT &&
	           instanceof_function(C_OBJCE_P(headers), soap_header_class_entry)) {
		...
		zval_ptr_dtor(C_CLIENT_DEFAULT_HEADERS_P(this_ptr));
		ZVAL_COPY_VALUE(C_CLIENT_DEFAULT_HEADERS_P(this_ptr), &default_headers);
	} else {
		crex_argument_type_error(1, "must be of type SoapHeader|array|null, %s given", crex_zval_value_name(headers));
		RETURN_THROWS();
	}
	RETURN_TRUE;
}
...
CRX_METHOD(SoapClient, __setLocation)
{
	crex_string *location = NULL;
	zval *this_ptr = CREX_THIS;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|S!", &location) == FAILURE) {
		RETURN_THROWS();
	}

	RETVAL_COPY_VALUE(C_CLIENT_LOCATION_P(this_ptr));

	if (location && ZSTR_LEN(location) != 0) {
		ZVAL_STR_COPY(C_CLIENT_LOCATION_P(this_ptr), location);
	} else {
		ZVAL_NULL(C_CLIENT_LOCATION_P(this_ptr));
	}
}
/* }}} */
...
static void clear_soap_fault(zval *obj) /* {{{ */
{
	CREX_ASSERT(instanceof_function(C_OBJCE_P(obj), soap_class_entry));
	convert_to_null(C_CLIENT_SOAP_FAULT_P(obj));
}
/* }}} */
...
static void add_soap_fault_ex(zval *fault, zval *obj, char *fault_code, char *fault_string, char *fault_actor, zval *fault_detail) /* {{{ */
{
	...
	zval *target;
	if (instanceof_function(C_OBJCE_P(obj), soap_class_entry)) {
		target = C_CLIENT_SOAP_FAULT_P(obj);
	} else if (instanceof_function(C_OBJCE_P(obj), soap_server_class_entry)) {
		target = C_SERVER_SOAP_FAULT_P(obj);
	} else {
		CREX_UNREACHABLE();
	}
...
static void set_soap_fault(zval *obj, char *fault_code_ns, char *fault_code, char *fault_string, char *fault_actor, zval *fault_detail, char *name) /* {{{ */
{
	...
	ZVAL_STRING(C_FAULT_STRING_P(obj), fault_string ? fault_string : "");
	crex_update_property_ex(crex_ce_exception, C_OBJ_P(obj), ZSTR_KNOWN(CREX_STR_MESSAGE), C_FAULT_STRING_P(obj));

	if (fault_code != NULL) {
		int soap_version = SOAP_GLOBAL(soap_version);

		if (fault_code_ns) {
			ZVAL_STRING(C_FAULT_CODE_P(obj), fault_code);
			ZVAL_STRING(C_FAULT_CODENS_P(obj), fault_code_ns);
		} else {
			if (soap_version == SOAP_1_1) {
				...
				if (strcmp(fault_code,"Client") == 0 ||
					...
					ZVAL_STRING(C_FAULT_CODENS_P(obj), SOAP_1_1_ENV_NAMESPACE);
				}
			} else if (soap_version == SOAP_1_2) {
				if (strcmp(fault_code,"Client") == 0) {
					ZVAL_STRING(C_FAULT_CODE_P(obj), "Sender");
					ZVAL_STRING(C_FAULT_CODENS_P(obj), SOAP_1_2_ENV_NAMESPACE);
				} else if (strcmp(fault_code,"Server") == 0) {
					ZVAL_STRING(C_FAULT_CODE_P(obj), "Receiver");
					ZVAL_STRING(C_FAULT_CODENS_P(obj), SOAP_1_2_ENV_NAMESPACE);
	...
	if (fault_actor != NULL) {
		ZVAL_STRING(C_FAULT_ACTOR_P(obj), fault_actor);
	}
	if (fault_detail != NULL && C_TYPE_P(fault_detail) != IS_UNDEF) {
		ZVAL_COPY(C_FAULT_DETAIL_P(obj), fault_detail);
	}
	if (name != NULL) {
		ZVAL_STRING(C_FAULT_NAME_P(obj), name);
	}
}
/* }}} */
...
static void deserialize_parameters(xmlNodePtr params, sdlFunctionPtr function, int *num_params, zval **parameters) /* {{{ */
{
	...
	if (function != NULL) {
		...
		num_of_params = crex_hash_num_elements(function->requestParameters);
		CREX_HASH_FOREACH_PTR(function->requestParameters, param) {
	...
	if (params) {
		...
		if (num_of_params == 1 &&
		    ...
		    (function->requestParameters == NULL ||
		     crex_hash_num_elements(function->requestParameters) == 0) &&
			...
			while (trav != 0 && cur_param < num_of_params) {
				if (trav->type == XML_ELEMENT_NODE) {
					...
					if (function != NULL &&
					    (param = crex_hash_index_find_ptr(function->requestParameters, cur_param)) == NULL) {
	...
	if (func == NULL) {
		...
		if (sdl != NULL && function == NULL) {
			if (*version == SOAP_1_2) {
				...
				crx_error(E_ERROR, "Procedure '%s' not present", func->name);
			}
		}
	}
			...
			if (trav->type == XML_ELEMENT_NODE) {
				...
				if (!h->function && sdl && function && function->binding && function->binding->bindingType == BINDING_SOAP) {
					...
					if (fnb->input.headers) {
						...
						smart_str_appendl(&key, C_STRVAL(h->function_name), C_STRLEN(h->function_name));
						smart_str_0(&key);
						if ((hdr = crex_hash_find_ptr(fnb->input.headers, key.s)) != NULL) {
							h->hdr = hdr;
						}
						smart_str_free(&key);
					}
...
{
	if (C_TYPE_P(C_HEADER_MUST_UNDERSTAND_P(header)) == IS_TRUE) {
		if (version == SOAP_1_1) {
	...
	zval *tmp = C_HEADER_ACTOR_P(header);
	if (C_TYPE_P(tmp) == IS_STRING) {
		if (version == SOAP_1_1) {
			xmlSetProp(h, BAD_CAST(SOAP_1_1_ENV_NS_PREFIX":actor"), BAD_CAST(C_STRVAL_P(tmp)));
		} else {
			xmlSetProp(h, BAD_CAST(SOAP_1_2_ENV_NS_PREFIX":role"), BAD_CAST(C_STRVAL_P(tmp)));
		}
	} else if (C_TYPE_P(tmp) == IS_LONG) {
		if (version == SOAP_1_1) {
			if (C_LVAL_P(tmp) == SOAP_ACTOR_NEXT) {
			...
			} else if (C_LVAL_P(tmp) == SOAP_ACTOR_NONE) {
				xmlSetProp(h, BAD_CAST(SOAP_1_2_ENV_NS_PREFIX":role"), BAD_CAST(SOAP_1_2_ACTOR_NONE));
			} else if (C_LVAL_P(tmp) == SOAP_ACTOR_UNLIMATERECEIVER) {
...
static int serialize_response_call2(xmlNodePtr body, sdlFunctionPtr function, char *function_name, char *uri, zval *ret, int version, int main, xmlNodePtr *node) /* {{{ */
{
	...
	if (function != NULL) {
		if (function->responseParameters) {
			param_count = crex_hash_num_elements(function->responseParameters);
		} else {
		  param_count = 0;
		}
	} else {
	...
	} else if (param_count > 1 && C_TYPE_P(ret) == IS_ARRAY) {
		zval *data;
		int i = 0;
		crex_string *param_name;
		crex_ulong param_index = i;

		CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(ret), param_index, param_name, data) {
...
static xmlDocPtr serialize_response_call(sdlFunctionPtr function, char *function_name, char *uri, zval *ret, soapHeader* headers, int version) /* {{{ */
{
	...
	if (C_TYPE_P(ret) == IS_OBJECT &&
	    instanceof_function(C_OBJCE_P(ret), soap_fault_class_entry)) {
		char *detail_name;
		sdlFaultPtr fault = NULL;
		char *fault_ns = NULL;
		zval *tmp = C_FAULT_HEADERFAULT_P(ret);
		if (headers && C_TYPE_P(tmp) > IS_NULL) {
			encodePtr hdr_enc = NULL;
			int hdr_use = SOAP_LITERAL;
			zval *hdr_ret  = tmp;
			char *hdr_ns   = headers->hdr?headers->hdr->ns:NULL;
			char *hdr_name = C_STRVAL(headers->function_name);

			head = xmlNewChild(envelope, ns, BAD_CAST("Header"), NULL);
			if (C_TYPE_P(hdr_ret) == IS_OBJECT &&
			    instanceof_function(C_OBJCE_P(hdr_ret), soap_header_class_entry)) {
				sdlSoapBindingFunctionHeaderPtr hdr;
				smart_str key = {0};

				tmp = C_HEADER_NAMESPACE_P(hdr_ret);
				if (C_TYPE_P(tmp) == IS_STRING) {
					smart_str_appendl(&key, C_STRVAL_P(tmp), C_STRLEN_P(tmp));
					smart_str_appendc(&key, ':');
					hdr_ns = C_STRVAL_P(tmp);
				}

				tmp = C_HEADER_NAME_P(hdr_ret);
				if (C_TYPE_P(tmp) == IS_STRING) {
					smart_str_appendl(&key, C_STRVAL_P(tmp), C_STRLEN_P(tmp));
					hdr_name = C_STRVAL_P(tmp);
				}
				smart_str_0(&key);
				if (headers->hdr && headers->hdr->headerfaults &&
				    (hdr = crex_hash_find_ptr(headers->hdr->headerfaults, key.s)) != NULL) {
					hdr_enc = hdr->encode;
					hdr_use = hdr->use;
				}
				smart_str_free(&key);
				tmp = C_HEADER_DATA_P(hdr_ret);
			...
			if (headers->function) {
				if (serialize_response_call2(head, headers->function, C_STRVAL(headers->function_name), uri, hdr_ret, version, 0, NULL) == SOAP_ENCODED) {
					use = SOAP_ENCODED;
				}
			} else {
		...
		tmp = C_FAULT_CODENS_P(ret);
		if (C_TYPE_P(tmp) == IS_STRING) {
			fault_ns = C_STRVAL_P(tmp);
		}
		use = SOAP_LITERAL;

		tmp = C_FAULT_NAME_P(ret);
		if (C_TYPE_P(tmp) == IS_STRING) {
			sdlFaultPtr tmp_fault;
			if (function && function->faults &&
			    (tmp_fault = crex_hash_find_ptr(function->faults, C_STR_P(tmp))) != NULL) {
				fault = tmp_fault;
				if (function->binding &&
				    function->binding->bindingType == BINDING_SOAP &&
		           ...
		           crex_hash_num_elements(function->faults) == 1) {

			crex_hash_internal_pointer_reset(function->faults);
			fault = crex_hash_get_current_data_ptr(function->faults);
			if (function->binding &&
			    function->binding->bindingType == BINDING_SOAP &&
			    fault->bindingAttributes) {
				sdlSoapBindingFunctionFaultPtr fb = (sdlSoapBindingFunctionFaultPtr)fault->bindingAttributes;
		...
		if (fault_ns == NULL &&
		    fault &&
		    fault->details &&
		    crex_hash_num_elements(fault->details) == 1) {
			sdlParamPtr sparam;

			crex_hash_internal_pointer_reset(fault->details);
			sparam = crex_hash_get_current_data_ptr(fault->details);
			if (sparam->element) {
		...
		if (version == SOAP_1_1) {
			tmp = C_FAULT_CODE_P(ret);
			if (C_TYPE_P(tmp) == IS_STRING) {
				xmlNodePtr node = xmlNewNode(NULL, BAD_CAST("faultcode"));
				crex_string *str = crx_escape_html_entities((unsigned char*)C_STRVAL_P(tmp), C_STRLEN_P(tmp), 0, 0, NULL);
		...
		if (fault && fault->details && crex_hash_num_elements(fault->details) == 1) {
			xmlNodePtr node;
			zval *detail = NULL;
			sdlParamPtr sparam;
			xmlNodePtr x;

			tmp = C_FAULT_DETAIL_P(ret);
			if (C_TYPE_P(tmp) > IS_NULL) {
				detail = tmp;
			}
			...
			if (detail &&
			    C_TYPE_P(detail) == IS_OBJECT &&
			    sparam->element &&
			    crex_hash_num_elements(C_OBJPROP_P(detail)) == 1 &&
			    (tmp = crex_hash_str_find(C_OBJPROP_P(detail), sparam->element->name, strlen(sparam->element->name))) != NULL) {
				detail = tmp;
			}
		...
		if (headers) {
			...
			h = headers;
			while (h != NULL) {
				if (C_TYPE(h->retval) != IS_NULL) {
					encodePtr hdr_enc = NULL;
					int hdr_use = SOAP_LITERAL;
					zval *hdr_ret = &h->retval;
					char *hdr_ns   = h->hdr?h->hdr->ns:NULL;
					char *hdr_name = C_TYPE(h->function_name) == IS_STRING
						? C_STRVAL(h->function_name) : NULL;
					bool is_header_object = C_TYPE(h->retval) == IS_OBJECT &&
					    instanceof_function(C_OBJCE(h->retval), soap_header_class_entry);
					...
					if (is_header_object) {
						...
						tmp = C_HEADER_NAMESPACE_P(&h->retval);
						if (C_TYPE_P(tmp) == IS_STRING) {
							smart_str_appendl(&key, C_STRVAL_P(tmp), C_STRLEN_P(tmp));
							smart_str_appendc(&key, ':');
							hdr_ns = C_STRVAL_P(tmp);
						}
						tmp = C_HEADER_NAME_P(&h->retval);
						if (C_TYPE_P(tmp) == IS_STRING) {
							smart_str_appendl(&key, C_STRVAL_P(tmp), C_STRLEN_P(tmp));
							hdr_name = C_STRVAL_P(tmp);
						}
						...
						if (function && function->binding && function->binding->bindingType == BINDING_SOAP) {
							...
							if (fnb->output.headers &&
							    (hdr = crex_hash_find_ptr(fnb->output.headers, key.s)) != NULL) {
								hdr_enc = hdr->encode;
								hdr_use = hdr->use;
							}
						}
						smart_str_free(&key);
						tmp = C_HEADER_DATA_P(&h->retval);
						if (C_TYPE_P(tmp) > IS_NULL) {
							hdr_ret = tmp;
						} else {
							hdr_ret = NULL;
					...
					if (h->function) {
						...
						if (serialize_response_call2(head, h->function, C_STRVAL(h->function_name), uri, hdr_ret, version, 0, &xmlHdr) == SOAP_ENCODED) {
							use = SOAP_ENCODED;
						}
						if (is_header_object) {
...
static xmlDocPtr serialize_function_call(zval *this_ptr, sdlFunctionPtr function, char *function_name, char *uri, zval *arguments, int arg_count, int version, HashTable *soap_headers) /* {{{ */
{
	...
	if (function && function->binding->bindingType == BINDING_SOAP) {
		...
		zstyle = C_CLIENT_STYLE_P(this_ptr);
		if (C_TYPE_P(zstyle) == IS_LONG) {
			style = C_LVAL_P(zstyle);
		} else {
			style = SOAP_RPC;
		}
		/*FIXME: how to pass method name if style is SOAP_DOCUMENT */
		...
		zuse = C_CLIENT_USE_P(this_ptr);
		if (C_TYPE_P(zuse) == IS_LONG && C_LVAL_P(zuse) == SOAP_LITERAL) {
			use = SOAP_LITERAL;
		} else {
			use = SOAP_ENCODED;
		}
	...
	if (function && function->requestParameters) {
		int n = crex_hash_num_elements(function->requestParameters);
	...
	if (head) {
		...
		CREX_HASH_FOREACH_VAL(soap_headers, header) {
			if (C_TYPE_P(header) != IS_OBJECT
					|| !instanceof_function(C_OBJCE_P(header), soap_header_class_entry)) {
				continue;
			}

			zval *name = C_HEADER_NAME_P(header);
			zval *ns = C_HEADER_NAMESPACE_P(header);
			if (C_TYPE_P(name) == IS_STRING && C_TYPE_P(ns) == IS_STRING) {
				xmlNodePtr h;
				xmlNsPtr nsptr;
				int hdr_use = SOAP_LITERAL;
				encodePtr enc = NULL;
				...
				if (hdrs) {
					...
					smart_str_appendl(&key, C_STRVAL_P(ns), C_STRLEN_P(ns));
					smart_str_appendc(&key, ':');
					smart_str_appendl(&key, C_STRVAL_P(name), C_STRLEN_P(name));
					smart_str_0(&key);
					if ((hdr = crex_hash_find_ptr(hdrs, key.s)) != NULL) {
						hdr_use = hdr->use;
						enc = hdr->encode;
				...
				zval *data = C_HEADER_DATA_P(header);
				if (C_TYPE_P(data) > IS_NULL) {
					h = master_to_xml(enc, data, hdr_use, head);
					xmlNodeSetName(h, BAD_CAST(C_STRVAL_P(name)));
				} else {
					h = xmlNewNode(NULL, BAD_CAST(C_STRVAL_P(name)));
					xmlAddChild(head, h);
				}
				nsptr = encode_add_ns(h, C_STRVAL_P(ns));
				xmlSetNs(h, nsptr);
...
static xmlNodePtr serialize_parameter(sdlParamPtr param, zval *param_val, int index, char *name, int style, xmlNodePtr parent) /* {{{ */
{
	...
	if (param_val && C_TYPE_P(param_val) == IS_OBJECT
			&& C_OBJCE_P(param_val) == soap_param_class_entry) {
		zval *param_name = C_PARAM_NAME_P(param_val);
		zval *param_data = C_PARAM_DATA_P(param_val);
		if (C_TYPE_P(param_name) == IS_STRING && C_TYPE_P(param_data) != IS_UNDEF) {
			param_val = param_data;
			name = C_STRVAL_P(param_name);
		}
	}
...
static sdlParamPtr get_param(sdlFunctionPtr function, char *param_name, int index, int response) /* {{{ */
{
	...
	if (param_name != NULL) {
		if ((tmp = crex_hash_str_find_ptr(ht, param_name, strlen(param_name))) != NULL) {
			return tmp;
		} else {
			CREX_HASH_FOREACH_PTR(ht, tmp) {
		...
		if ((tmp = crex_hash_index_find_ptr(ht, index)) != NULL) {
			return tmp;
		}
	}
	return NULL;
...
static sdlFunctionPtr get_function(sdlPtr sdl, const char *function_name) /* {{{ */
{
	...
	crex_str_tolower(str,len);
	if (sdl != NULL) {
		if ((tmp = crex_hash_str_find_ptr(&sdl->functions, str, len)) != NULL) {
			efree(str);
			return tmp;
		} else if (sdl->requests != NULL && (tmp = crex_hash_str_find_ptr(sdl->requests, str, len)) != NULL) {
			efree(str);
			return tmp;
		}
	}
...
static sdlFunctionPtr get_doc_function(sdlPtr sdl, xmlNodePtr params) /* {{{ */
{
	if (sdl) {
		...
		CREX_HASH_MAP_FOREACH_PTR(&sdl->functions, tmp) {
				...
				if (fnb->style == SOAP_DOCUMENT) {
					if (params == NULL) {
						if (tmp->requestParameters == NULL ||
						    crex_hash_num_elements(tmp->requestParameters) == 0) {
						  return tmp;
						}
						...
						CREX_HASH_FOREACH_PTR(tmp->requestParameters, param) {
							if (param->element) {
...
static void function_to_string(sdlFunctionPtr function, smart_str *buf) /* {{{ */
{
	...
	if (function->responseParameters &&
	    crex_hash_num_elements(function->responseParameters) > 0) {
		if (crex_hash_num_elements(function->responseParameters) == 1) {
			crex_hash_internal_pointer_reset(function->responseParameters);
			param = crex_hash_get_current_data_ptr(function->responseParameters);
			...
			CREX_HASH_FOREACH_PTR(function->responseParameters, param) {
				if (i > 0) {
...
static void model_to_string(sdlContentModelPtr model, smart_str *buf, int level) /* {{{ */
{
	...
	switch (model->kind) {
		...
		case XSD_CONTENT_CHOICE: {
			...
			CREX_HASH_FOREACH_PTR(model->u.content, tmp) {
	...
	switch (type->kind) {
		...
		case XSD_TYPEKIND_LIST:
			...
			if (type->elements) {
				...
				smart_str_appendl(buf, " {", 2);
				CREX_HASH_FOREACH_PTR(type->elements, item_type) {
		...
		case XSD_TYPEKIND_EXTENSION:
			if (type->encode &&
			  ...
			  sdlExtraAttributePtr ext;
				...
				if (type->attributes &&
				    (attr = crex_hash_str_find_ptr(type->attributes, SOAP_1_1_ENC_NAMESPACE":arrayType",
				      sizeof(SOAP_1_1_ENC_NAMESPACE":arrayType")-1)) != NULL &&
					attr->extraAttributes &&
				    (ext = crex_hash_str_find_ptr(attr->extraAttributes, WSDL_NAMESPACE":arrayType", sizeof(WSDL_NAMESPACE":arrayType")-1)) != NULL) {
					...
					if (type->attributes &&
					    (attr = crex_hash_str_find_ptr(type->attributes, SOAP_1_2_ENC_NAMESPACE":itemType",
					      sizeof(SOAP_1_2_ENC_NAMESPACE":itemType")-1)) != NULL &&
						attr->extraAttributes &&
				    (ext = crex_hash_str_find_ptr(attr->extraAttributes, WSDL_NAMESPACE":itemType", sizeof(WSDL_NAMESPACE":arrayType")-1)) != NULL) {
						smart_str_appends(buf, ext->val);
						smart_str_appendc(buf, ' ');
					} else if (type->elements &&
					           crex_hash_num_elements(type->elements) == 1 &&
					           (crex_hash_internal_pointer_reset(type->elements),
					            (elementType = crex_hash_get_current_data_ptr(type->elements)) != NULL) &&
					...
					smart_str_appendl(buf, type->name, strlen(type->name));
					if (type->attributes &&
					    (attr = crex_hash_str_find_ptr(type->attributes, SOAP_1_2_ENC_NAMESPACE":arraySize",
					      sizeof(SOAP_1_2_ENC_NAMESPACE":arraySize")-1)) != NULL &&
						attr->extraAttributes &&
					    (ext = crex_hash_str_find_ptr(attr->extraAttributes, WSDL_NAMESPACE":itemType", sizeof(WSDL_NAMESPACE":arraySize")-1)) != NULL) {
						smart_str_appendc(buf, '[');
						smart_str_appends(buf, ext->val);
						smart_str_appendc(buf, ']');
					} else {
				...
				if (type->attributes) {
					...
					CREX_HASH_FOREACH_PTR(type->attributes, attr) {
						if (spaces.s) {
							smart_str_appendl(buf, ZSTR_VAL(spaces.s), ZSTR_LEN(spaces.s));
						}
						smart_str_appendc(buf, ' ');
...
static void delete_service(void *data) /* {{{ */
{
	...
	if (service->typemap) {
		crex_hash_destroy(service->typemap);
	...
	if (service->class_map) {
		crex_hash_destroy(service->class_map);
