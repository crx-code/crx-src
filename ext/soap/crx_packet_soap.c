...
int parse_packet_soap(zval *this_ptr, char *buffer, int buffer_size, sdlFunctionPtr fn, char *fn_name, zval *return_value, zval *soap_headers)
{
	...
	if (fault != NULL) {
		char *faultcode = NULL;
		crex_string *faultstring = NULL, *faultactor = NULL;
		zval details;
		xmlNodePtr tmp;
		...
		if (faultstring) {
			crex_string_release_ex(faultstring, 0);
		}
		if (faultactor) {
			crex_string_release_ex(faultactor, 0);
		}
		if (C_REFCOUNTED(details)) {
			C_DELREF(details);
		}
		xmlFreeDoc(response);
		return FALSE;
		...
		if (fn != NULL && fn->binding && fn->binding->bindingType == BINDING_SOAP) {
		  /* Function has WSDL description */
			...
			if (fn->responseParameters) {
				res_count = crex_hash_num_elements(fn->responseParameters);
				CREX_HASH_FOREACH_PTR(fn->responseParameters, param) {
...
*/
			...
			while (val != NULL) {
				...
				if (val != NULL) {
					if (!node_is_equal_ex(val,"result",RPC_SOAP12_NAMESPACE)) {
						...
						master_to_zval(&tmp, NULL, val);
						if (val->name) {
							if ((arr = crex_hash_str_find(C_ARRVAL_P(return_value), (char*)val->name, strlen((char*)val->name))) != NULL) {
	...
	if (C_TYPE_P(return_value) == IS_ARRAY) {
		if (param_count == 0) {
			...
			crex_hash_internal_pointer_reset(C_ARRVAL_P(return_value));
			tmp = crex_hash_get_current_data(C_ARRVAL_P(return_value));
			if (!C_REFCOUNTED_P(return_value)) {
				ZVAL_COPY(return_value, tmp);
			} else {
				crex_refcounted *garbage = C_COUNTED_P(return_value);
				ZVAL_COPY(return_value, tmp);
