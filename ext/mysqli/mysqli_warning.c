...
/* Define these in the CRX7 tree to make merging easy process */
...
/* {{{ void crx_clear_warnings() */
void crx_clear_warnings(MYSQLI_WARNING *w)
{
	MYSQLI_WARNING *n;
...
/* {{{ MYSQLI_WARNING *crx_new_warning */
static
MYSQLI_WARNING *crx_new_warning(zval * reason, int errorno)
{
	MYSQLI_WARNING *w;
...
/* {{{ MYSQLI_WARNING *crx_get_warnings(MYSQL *mysql) */
MYSQLI_WARNING * crx_get_warnings(MYSQLND_CONN_DATA * mysql)
{
	...
	for (;;) {
		...
		crex_hash_internal_pointer_reset(C_ARRVAL(row));
		/* 0. we don't care about the first */
		crex_hash_move_forward(C_ARRVAL(row));

		/* 1. Here comes the error no */
		entry = crex_hash_get_current_data(C_ARRVAL(row));
		...
		w = crx_new_warning(entry, mysqli_errno);
		/*
...
CRX_METHOD(mysqli_warning, next)
{
	MYSQLI_WARNING 	*w;
	mysqli_object *obj = C_MYSQLI_P(CREX_THIS);
	...
	if (obj->ptr) {
		MYSQLI_FETCH_RESOURCE(w, MYSQLI_WARNING *, CREX_THIS, "mysqli_warning", MYSQLI_STATUS_VALID);
...
CRX_METHOD(mysqli_warning, __main)
{
	CREX_PARSE_PARAMETERS_NONE();

	crex_throw_error(NULL, "Cannot directly construct mysqli_warning");
}
