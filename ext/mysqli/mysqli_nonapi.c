...
/* {{{ crx_mysqli_set_error */
static void crx_mysqli_set_error(crex_long mysql_errno, char *mysql_err)
{
...
void mysqli_common_connect(INTERNAL_FUNCTION_PARAMETERS, bool is_real_connect, bool in_ctor) /* {{{ */
{
	...
	crex_long			port = 0, flags = 0;
	bool           port_is_null = 1;
	crex_string			*hash_key = NULL;
	...
	if (getThis() && !CREX_NUM_ARGS() && in_ctor) {
		crx_mysqli_init(INTERNAL_FUNCTION_PARAM_PASSTHRU, in_ctor);
		return;
	}
	hostname = username = dbname = passwd = socket = NULL;

	if (!is_real_connect) {
		if (crex_parse_parameters(CREX_NUM_ARGS(), "|s!s!s!s!l!s!", &hostname, &hostname_len, &username, &username_len,
				&passwd, &passwd_len, &dbname, &dbname_len, &port, &port_is_null, &socket, &socket_len) == FAILURE) {
			RETURN_THROWS();
		}

		if (object) {
			CREX_ASSERT(instanceof_function(C_OBJCE_P(object), mysqli_link_class_entry));
			mysqli_resource = (C_MYSQLI_P(object))->ptr;
		...
		/* We have flags too */
		if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "O|s!s!s!s!l!s!l", &object, mysqli_link_class_entry,
		...
		(mysqli_resource->status > MYSQLI_STATUS_INITIALIZED))
	{
		...
		crx_mysqli_close(mysql, MYSQLI_CLOSE_IMPLICIT, mysqli_resource->status);
	}

	if (strlen(SAFE_STR(hostname)) > 2 && !strncasecmp(hostname, "p:", 2)) {
		hostname += 2;
		if (!MyG(allow_persistent)) {
			crx_error_docref(NULL, E_WARNING, "Persistent connections are disabled. Downgrading to normal");
		} else {
			mysql->persistent = persistent = true;

			hash_key = strpprintf(0, "mysqli_%s_%s" CREX_LONG_FMT "%s%s%s", SAFE_STR(hostname), SAFE_STR(socket),
			...
			if ((le = crex_hash_find_ptr(&EG(persistent_list), hash_key)) != NULL) {
				if (le->type == crx_le_pmysqli()) {
					plist = (mysqli_plist_entry *) le->ptr;

					do {
						if (crex_ptr_stack_num_elements(&plist->free_links)) {
							...
							mysql->mysql = crex_ptr_stack_pop(&plist->free_links);
							...
							if (!mysql_ping(mysql->mysql)) {
								...
								/* clear error */
								crx_mysqli_set_error(mysql_errno(mysql->mysql), (char *) mysql_error(mysql->mysql));
				...
				crex_ptr_stack_init_ex(&plist->free_links, 1);
				crex_register_persistent_resource(ZSTR_VAL(hash_key), ZSTR_LEN(hash_key), plist, crx_le_pmysqli());
			}
		}
	}
	if (MyG(max_links) != -1 && MyG(num_links) >= MyG(max_links)) {
		crx_error_docref(NULL, E_WARNING, "Too many open links (" CREX_LONG_FMT ")", MyG(num_links));
		goto err;
	}
	...
	{
		crx_error_docref(NULL, E_WARNING, "Too many open persistent links (" CREX_LONG_FMT ")",
						...
						port, socket, flags, MYSQLND_CLIENT_NO_FLAG) == NULL)
	{
		...
		crx_mysqli_set_error(mysql_errno(mysql->mysql), (char *) mysql_error(mysql->mysql));
		crx_mysqli_throw_sql_exception((char *)mysql_sqlstate(mysql->mysql), mysql_errno(mysql->mysql),
	...
	if (MyG(local_infile_directory) && !crx_check_open_basedir(MyG(local_infile_directory))) {
	...
	if (object) {
		CREX_ASSERT(instanceof_function(C_OBJCE_P(object), mysqli_link_class_entry));
		(C_MYSQLI_P(object))->ptr = mysqli_resource;
		RETURN_TRUE;
	} else {
...
CRX_FUNCTION(mysqli_connect)
{
	mysqli_common_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, false, false);
}
/* }}} */

CRX_METHOD(mysqli, __main)
{
	mysqli_common_connect(INTERNAL_FUNCTION_PARAM_PASSTHRU, false, true);
}

/* {{{ Initialize mysqli and return a resource for use with mysql_real_connect */
CRX_METHOD(mysqli, init)
{
	crx_mysqli_init(INTERNAL_FUNCTION_PARAM_PASSTHRU, true);
}
/* }}} */

/* {{{ Returns the numerical value of the error message from last connect command */
CRX_FUNCTION(mysqli_connect_errno)
{
...
CRX_FUNCTION(mysqli_connect_error)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(mysqli_fetch_array)
{
	crx_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */

/* {{{ Fetch a result row as an associative array */
CRX_FUNCTION(mysqli_fetch_assoc)
{
	crx_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MYSQLI_ASSOC, 0);
}
/* }}} */

/* {{{ Fetch a column from the result row  */
CRX_FUNCTION(mysqli_fetch_column)
{
	MYSQL_RES		*result;
	zval			*mysql_result;
	zval 			row_array;
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "O|l", &mysql_result, mysqli_result_class_entry, &col_no) == FAILURE) {
		RETURN_THROWS();
	}
	...
	crx_mysqli_fetch_into_hash_aux(&row_array, result, MYSQLI_NUM);
	if (C_TYPE(row_array) != IS_ARRAY) {
		zval_ptr_dtor_nogc(&row_array);
		RETURN_FALSE;
	}

	ZVAL_COPY(return_value, crex_hash_index_find(C_ARR(row_array), col_no));
	zval_ptr_dtor_nogc(&row_array);
}
/* }}} */

/* {{{ Fetches all result rows as an associative array, a numeric array, or both */
CRX_FUNCTION(mysqli_fetch_all)
{
	MYSQL_RES	*result;
	zval		*mysql_result;
	crex_long		mode = MYSQLI_NUM;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "O|l", &mysql_result, mysqli_result_class_entry, &mode) == FAILURE) {
		RETURN_THROWS();
	}
	MYSQLI_FETCH_RESOURCE(result, MYSQL_RES *, mysql_result, "mysqli_result", MYSQLI_STATUS_VALID);
	...
	do {
		zval row;
		crx_mysqli_fetch_into_hash_aux(&row, result, mode);
		if (C_TYPE(row) != IS_ARRAY) {
...
CRX_FUNCTION(mysqli_get_client_stats)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}
...
/* {{{ Returns statistics about the zval cache */
CRX_FUNCTION(mysqli_get_connection_stats)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
...
CRX_FUNCTION(mysqli_error_list)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
	...
	for (message = (MYSQLND_ERROR_LIST_ELEMENT *) crex_llist_get_first_ex(&mysql->mysql->data->error_info->error_list, &pos);
			message;
			message = (MYSQLND_ERROR_LIST_ELEMENT *) crex_llist_get_next_ex(&mysql->mysql->data->error_info->error_list, &pos))
	{
		zval single_error;
...
/* {{{ */
CRX_FUNCTION(mysqli_stmt_error_list)
{
	MY_STMT	*stmt;
	zval 	*mysql_stmt;
	...
	if (stmt->stmt && stmt->stmt->data && stmt->stmt->data->error_info) {
		...
		crex_llist_position pos;
		array_init(return_value);
		for (message = (MYSQLND_ERROR_LIST_ELEMENT *) crex_llist_get_first_ex(&stmt->stmt->data->error_info->error_list, &pos);
			 message;
			 message = (MYSQLND_ERROR_LIST_ELEMENT *) crex_llist_get_next_ex(&stmt->stmt->data->error_info->error_list, &pos))
		{
			zval single_error;
			array_init(&single_error);
			add_assoc_long_ex(&single_error, "errno", sizeof("errno") - 1, message->error_no);
...
CRX_FUNCTION(mysqli_fetch_object)
{
	crx_mysqli_fetch_into_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, MYSQLI_ASSOC, 1);
}
/* }}} */

/* {{{ allows to execute multiple queries  */
CRX_FUNCTION(mysqli_multi_query)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
	char		*query = NULL;
...
CRX_FUNCTION(mysqli_query)
{
	...
	char				*query = NULL;
	size_t 				query_len;
	crex_long 				resultmode = MYSQLI_STORE_RESULT;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Os|l", &mysql_link, mysqli_link_class_entry, &query, &query_len, &resultmode) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if ((resultmode & ~MYSQLI_ASYNC) != MYSQLI_USE_RESULT &&
		...
		crex_argument_value_error(ERROR_ARG_POS(3), "must be either MYSQLI_USE_RESULT or MYSQLI_STORE_RESULT with MYSQLI_ASYNC as an optional bitmask flag");
		RETURN_THROWS();
	}
...
static int mysqlnd_zval_array_to_mysqlnd_array(zval *in_array, MYSQLND ***out_array)
{
	...
	if (C_TYPE_P(in_array) != IS_ARRAY) {
		return SUCCESS;
	}
	*out_array = ecalloc(crex_hash_num_elements(C_ARRVAL_P(in_array)) + 1, sizeof(MYSQLND *));
	CREX_HASH_FOREACH_VAL(C_ARRVAL_P(in_array), elem) {
		i++;
		if (C_TYPE_P(elem) != IS_OBJECT ||
			!instanceof_function(C_OBJCE_P(elem), mysqli_link_class_entry)) {
			crex_argument_type_error(i, "must be an instance of mysqli, %s given", crex_zval_value_name(elem));
			return FAILURE;
		} else {
			MY_MYSQL *mysql;
			MYSQLI_RESOURCE *my_res;
			mysqli_object *intern = C_MYSQLI_P(elem);
...
static crex_result mysqlnd_zval_array_from_mysqlnd_array(MYSQLND **in_array, zval *out_array)
{
	MYSQLND **p = in_array;
	zval dest_array;
	zval *elem, *dest_elem;

	array_init_size(&dest_array, crex_hash_num_elements(C_ARRVAL_P(out_array)));

	CREX_HASH_FOREACH_VAL(C_ARRVAL_P(out_array), elem) {
		if (C_TYPE_P(elem) != IS_OBJECT ||
				!instanceof_function(C_OBJCE_P(elem), mysqli_link_class_entry)) {
			continue;
		}
		{
			...
			mysql = (MY_MYSQL *) my_res->ptr;
			if (mysql->mysql == *p) {
				dest_elem = crex_hash_next_index_insert(C_ARRVAL(dest_array), elem);
				if (dest_elem) {
...
static void mysqlnd_dont_poll_zval_array_from_mysqlnd_array(MYSQLND **in_array, zval *in_zval_array, zval *out_array)
{
	...
	array_init(&proxy);
	if (in_array) {
		CREX_HASH_FOREACH_VAL(C_ARRVAL_P(in_zval_array), elem) {
			MY_MYSQL *mysql;
			mysqli_object *intern = C_MYSQLI_P(elem);
			mysql = (MY_MYSQL *)((MYSQLI_RESOURCE *)intern->ptr)->ptr;
			if (mysql->mysql == *p) {
				dest_elem = crex_hash_next_index_insert(C_ARRVAL(proxy), elem);
				if (dest_elem) {
					zval_add_ref(dest_elem);
				}
				p++;
...
/* {{{ Poll connections */
CRX_FUNCTION(mysqli_poll)
{
	zval			*r_array, *e_array, *dont_poll_array;
	MYSQLND			**new_r_array = NULL, **new_e_array = NULL, **new_dont_poll_array = NULL;
	crex_long			sec = 0, usec = 0;
	enum_func_status ret;
	int 			desc_num;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "a!a!al|l", &r_array, &e_array, &dont_poll_array, &sec, &usec) == FAILURE) {
		RETURN_THROWS();
	}
	if (sec < 0) {
...
/* {{{ Poll connections */
CRX_FUNCTION(mysqli_reap_async_query)
{
	MY_MYSQL		*mysql;
	zval			*mysql_link;
	MYSQLI_RESOURCE		*mysqli_resource;
	...
	if (!mysql_field_count(mysql->mysql)) {
		/* no result set - not a SELECT */
		if (MyG(report_mode) & MYSQLI_REPORT_INDEX) {
/*			crx_mysqli_report_index("n/a", mysqli_server_status(mysql->mysql)); */
		}
		RETURN_TRUE;
	}
...
CRX_FUNCTION(mysqli_stmt_get_result)
{
	MYSQL_RES 		*result;
	MYSQLI_RESOURCE	*mysqli_resource;
	MY_STMT			*stmt;
...
/* {{{ */
CRX_FUNCTION(mysqli_get_warnings)
{
	MY_MYSQL			*mysql;
	zval				*mysql_link;
	MYSQLI_RESOURCE		*mysqli_resource;
	...
	if (mysql_warning_count(mysql->mysql)) {
		w = crx_get_warnings(mysql->mysql->data);
	}
	if (!w) {
		RETURN_FALSE;
	}
...
/* {{{ */
CRX_FUNCTION(mysqli_stmt_get_warnings)
{
	MY_STMT				*stmt;
	zval				*stmt_link;
	MYSQLI_RESOURCE		*mysqli_resource;
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "O", &stmt_link, mysqli_stmt_class_entry) == FAILURE) {
		RETURN_THROWS();
	}
	MYSQLI_FETCH_RESOURCE_STMT(stmt, stmt_link, MYSQLI_STATUS_VALID);

	if (mysqli_stmt_warning_count(stmt->stmt)) {
		w = crx_get_warnings(mysqli_stmt_get_connection(stmt->stmt));
	}
	if (!w) {
		RETURN_FALSE;
	}
...
CRX_FUNCTION(mysqli_set_charset)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
	char		*cs_name;
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Os", &mysql_link, mysqli_link_class_entry, &cs_name, &csname_len) == FAILURE) {
		RETURN_THROWS();
	}
	MYSQLI_FETCH_RESOURCE_CONN(mysql, mysql_link, MYSQLI_STATUS_VALID);
...
CRX_FUNCTION(mysqli_get_charset)
{
	MY_MYSQL				*mysql;
	zval					*mysql_link;
	...
	if (!cs) {
		crx_error_docref(NULL, E_WARNING, "The connection has no charset associated");
		RETURN_NULL();
	}
	name = cs->name;
...
CRX_FUNCTION(mysqli_begin_transaction)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
	crex_long		flags = TRANS_START_NO_OPT;
	char *		name = NULL;
	size_t			name_len = 0;
	...
	MYSQLI_FETCH_RESOURCE_CONN(mysql, mysql_link, MYSQLI_STATUS_VALID);
	if (flags < 0) {
		crex_argument_value_error(ERROR_ARG_POS(2), "must be one of the MYSQLI_TRANS_* constants");
		RETURN_THROWS();
	}
	if (name && !name_len) {
		crex_argument_value_error(ERROR_ARG_POS(3), "cannot be empty");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(mysqli_savepoint)
{
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Os", &mysql_link, mysqli_link_class_entry, &name, &name_len) == FAILURE) {
		RETURN_THROWS();
	}
	MYSQLI_FETCH_RESOURCE_CONN(mysql, mysql_link, MYSQLI_STATUS_VALID);
	if (name_len == 0) {
...
/* {{{ Starts a transaction */
CRX_FUNCTION(mysqli_release_savepoint)
{
	MY_MYSQL	*mysql;
	zval		*mysql_link;
	char *		name = NULL;
...
CRX_FUNCTION(mysqli_get_links_stats)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}
