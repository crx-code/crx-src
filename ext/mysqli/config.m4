...
AC_DEFUN([CRX_MYSQL_SOCKET_SEARCH], [
  for i in  \
...
if test "$CRX_MYSQLI" != "no"; then
  ...
  CRX_NEW_EXTENSION(mysqli, $mysqli_sources, $ext_shared,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)
  CRX_SUBST(MYSQLI_SHARED_LIBADD)
  CRX_INSTALL_HEADERS([ext/mysqli/crx_mysqli_structs.h])

  if test "$CRX_MYSQLI" = "yes" || test "$CRX_MYSQLI" = "mysqlnd"; then
    CRX_ADD_EXTENSION_DEP(mysqli, mysqlnd)
    CRX_INSTALL_HEADERS([ext/mysqli/mysqli_mysqlnd.h])
  fi
fi
