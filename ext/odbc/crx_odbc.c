...
#define CRX_ODBC_BINMODE_PASSTHRU 0
#define CRX_ODBC_BINMODE_RETURN 1
#define CRX_ODBC_BINMODE_CONVERT 2
...
CRX_ODBC_API CREX_DECLARE_MODULE_GLOBALS(odbc)
static CRX_GINIT_FUNCTION(odbc);

/* {{{ odbc_module_entry */
crex_module_entry odbc_module_entry = {
...
#endif
CREX_GET_MODULE(odbc)
#endif

/* {{{ _free_odbc_result */
static void _free_odbc_result(crex_resource *rsrc)
{
	...
	if (res) {
		...
		if (res->stmt && !(PG(connection_status) & CRX_CONNECTION_TIMEOUT)) {
			 ...
			 * crex_list_delete(res->conn_ptr->id);
			 */
		}
		if (res->param_info) {
...
/* {{{ _close_odbc_conn */
static void _close_odbc_conn(crex_resource *rsrc)
{
	crex_resource *p;
	odbc_result *res;
	...
	CREX_HASH_FOREACH_PTR(&EG(regular_list), p) {
	...
	/* If aborted via timer expiration, don't try to call any unixODBC function */
	if (!(PG(connection_status) & CRX_CONNECTION_TIMEOUT)) {
...
static void _close_odbc_pconn(crex_resource *rsrc)
{
	crex_resource *p;
	odbc_result *res;
	odbc_connection *conn = (odbc_connection *)rsrc->ptr;
...
/* {{{ CRX_INI_DISP(display_link_nums) */
static CRX_INI_DISP(display_link_nums)
{
	char *value;

	if (type == CRX_INI_DISPLAY_ORIG && ini_entry->modified) {
...
/* {{{ CRX_INI_DISP(display_defPW) */
static CRX_INI_DISP(display_defPW)
{
	char *value;
...
/* {{{ CRX_INI_DISP(display_binmode) */
static CRX_INI_DISP(display_binmode)
{
	char *value;
...
/* {{{ CRX_INI_DISP(display_lrl) */
static CRX_INI_DISP(display_lrl)
{
	char *value;
	...
	if (value) {
		if (atoi(value) <= 0) {
			...
			crx_printf("return up to %s bytes", value);
		}
	}
}
/* }}} */
...
/* {{{ CRX_INI_DISP(display_cursortype) */
static CRX_INI_DISP(display_cursortype)
{
	char *value;
		...
		switch (atoi (value))
		  {
			...
			default:
				crx_printf("Unknown cursor model %s", value);
				break;
		  }
	}
}
...
/* {{{ CRX_INI_BEGIN */
CRX_INI_BEGIN()
	STD_CRX_INI_BOOLEAN("odbc.allow_persistent", "1", CRX_INI_SYSTEM, OnUpdateBool,
			allow_persistent, crex_odbc_globals, odbc_globals)
	STD_CRX_INI_ENTRY_EX("odbc.max_persistent",  "-1", CRX_INI_SYSTEM, OnUpdateLong,
			max_persistent, crex_odbc_globals, odbc_globals, display_link_nums)
	STD_CRX_INI_ENTRY_EX("odbc.max_links", "-1", CRX_INI_SYSTEM, OnUpdateLong,
			max_links, crex_odbc_globals, odbc_globals, display_link_nums)
	STD_CRX_INI_ENTRY("odbc.default_db", NULL, CRX_INI_ALL, OnUpdateString,
			defDB, crex_odbc_globals, odbc_globals)
	STD_CRX_INI_ENTRY("odbc.default_user", NULL, CRX_INI_ALL, OnUpdateString,
			defUser, crex_odbc_globals, odbc_globals)
	STD_CRX_INI_ENTRY_EX("odbc.default_pw", NULL, CRX_INI_ALL, OnUpdateString,
			defPW, crex_odbc_globals, odbc_globals, display_defPW)
	STD_CRX_INI_ENTRY_EX("odbc.defaultlrl", "4096", CRX_INI_ALL, OnUpdateLong,
			defaultlrl, crex_odbc_globals, odbc_globals, display_lrl)
	STD_CRX_INI_ENTRY_EX("odbc.defaultbinmode", "1", CRX_INI_ALL, OnUpdateLong,
			defaultbinmode, crex_odbc_globals, odbc_globals, display_binmode)
	STD_CRX_INI_BOOLEAN("odbc.check_persistent", "1", CRX_INI_SYSTEM, OnUpdateBool,
			check_persistent, crex_odbc_globals, odbc_globals)
	STD_CRX_INI_ENTRY_EX("odbc.default_cursortype", "3", CRX_INI_ALL, OnUpdateLong,
			default_cursortype, crex_odbc_globals, odbc_globals, display_cursortype)
CRX_INI_END()
/* }}} */
...
CRX_MINIT_FUNCTION(odbc)
{
	...
	le_result = crex_register_list_destructors_ex(_free_odbc_result, NULL, "odbc result", module_number);
	le_conn = crex_register_list_destructors_ex(_close_odbc_conn, NULL, "odbc link", module_number);
	le_pconn = crex_register_list_destructors_ex(NULL, _close_odbc_pconn, "odbc link persistent", module_number);
	odbc_module_entry.type = type;

#if defined(HAVE_IBMDB2) && defined(_AIX)
	/* atexit() handler in the DB2/AIX library segfaults in CRX CLI */
...
CRX_MINFO_FUNCTION(odbc)
{
	...
	crx_info_print_table_row(2, "ODBC Support", "enabled");
	snprintf(buf, sizeof(buf), CREX_LONG_FMT, ODBCG(num_persistent));
	crx_info_print_table_row(2, "Active Persistent Links", buf);
	snprintf(buf, sizeof(buf), CREX_LONG_FMT, ODBCG(num_links));
	crx_info_print_table_row(2, "Active Links", buf);
	crx_info_print_table_row(2, "ODBC library", CRX_ODBC_TYPE);
#ifdef ODBCVER
	snprintf(buf, sizeof(buf), "0x%.4x", ODBCVER);
	crx_info_print_table_row(2, "ODBCVER", buf);
#endif
#ifndef CRX_WIN32
	crx_info_print_table_row(2, "ODBC_CFLAGS", CRX_ODBC_CFLAGS);
	crx_info_print_table_row(2, "ODBC_LFLAGS", CRX_ODBC_LFLAGS);
	crx_info_print_table_row(2, "ODBC_LIBS", CRX_ODBC_LIBS);
#endif
...
void odbc_sql_error(ODBC_SQL_ERROR_PARAMS)
{
	...
	if (func) {
		crx_error_docref(NULL, E_WARNING, "SQL error: %s, SQL state %s in %s", ODBCG(lasterrormsg), ODBCG(laststate), func);
	} else {
		crx_error_docref(NULL, E_WARNING, "SQL error: %s, SQL state %s", ODBCG(lasterrormsg), ODBCG(laststate));
	}
	/*
...
/* {{{ crx_odbc_fetch_attribs */
void crx_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	odbc_result *result;
	zval *pv_res;
	crex_long flag;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "rl", &pv_res, &flag) == FAILURE) {
		RETURN_THROWS();
	}

	if ((result = (odbc_result *)crex_fetch_resource(C_RES_P(pv_res), "ODBC result", le_result)) == NULL) {
		RETURN_THROWS();
	}
...
int odbc_bindcols(odbc_result *result)
{
	...
	for(i = 0; i < result->numcols; i++) {
		...
		rc = CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), CRX_ODBC_SQL_DESC_NAME,
				result->values[i].name, sizeof(result->values[i].name), &colnamelen, 0);
		result->values[i].coltype = 0;
		rc = CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), SQL_COLUMN_TYPE,
		...
		switch(result->values[i].coltype) {
			...
			case SQL_WVARCHAR:
				...
				CREX_FALLTHROUGH;
			default:
				rc = CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(i+1), colfieldid,
				...
				if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO && colfieldid == SQL_DESC_OCTET_LENGTH) {
					...
					if (SQL_SUCCESS == SQLGetDiagRec(SQL_HANDLE_STMT, result->stmt, 1, state, &err, errtxt, 128, NULL)) {
						errtxt[127] = '\0';
						state[5] = '\0';
						crx_error_docref(NULL, E_WARNING, "SQLColAttribute can't handle SQL_DESC_OCTET_LENGTH: [%s] %s", state, errtxt);
					}
...
void odbc_transact(INTERNAL_FUNCTION_PARAMETERS, int type)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r", &pv_conn) == FAILURE) {
		RETURN_THROWS();
	}

	if (!(conn = (odbc_connection *)crex_fetch_resource2(C_RES_P(pv_conn), "ODBC-Link", le_conn, le_pconn))) {
		RETURN_THROWS();
	}
...
static int _close_pconn_with_res(zval *zv, void *p)
{
	...
	crex_resource *res = (crex_resource*)p;
...
void odbc_column_lengths(INTERNAL_FUNCTION_PARAMETERS, int type)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rl", &pv_res, &pv_num) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (result->numcols == 0) {
		crx_error_docref(NULL, E_WARNING, "No tuples available at this result index");
		RETURN_FALSE;
	}

	if (pv_num > result->numcols) {
		crx_error_docref(NULL, E_WARNING, "Field index larger than number of fields");
		RETURN_FALSE;
	}

	CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)pv_num, (SQLUSMALLINT) (type?SQL_COLUMN_SCALE:SQL_COLUMN_PRECISION), NULL, 0, NULL, &len);
	...
	CREX_HASH_FOREACH_PTR(&EG(regular_list), p) {
		if (p->ptr) {
			if (p->type == le_conn){
				...
				crex_hash_apply_with_argument(&EG(persistent_list),
...
CRX_FUNCTION(odbc_binmode)
{
	crx_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Handle LONG columns */
CRX_FUNCTION(odbc_longreadlen)
{
	crx_odbc_fetch_attribs(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ Prepares a statement for execution */
CRX_FUNCTION(odbc_prepare)
{
	zval *pv_conn;
	char *query;
	size_t query_len;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs", &pv_conn, &query, &query_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	rc = CRX_ODBC_SQLALLOCSTMT(conn->hdbc, &(result->stmt));
	if (rc == SQL_INVALID_HANDLE) {
		efree(result);
		crx_error_docref(NULL, E_WARNING, "SQLAllocStmt error 'Invalid Handle'");
		RETURN_FALSE;
	}
	...
	RETURN_RES(crex_register_resource(result, le_result));
}
/* }}} */
...
static void odbc_release_params(odbc_result *result, odbc_params_t *params) {
	...
	for (int i = 0; i < result->numparams; i++) {
		...
		if (params[i].zstr) {
			crex_string_release(params[i].zstr);
		}
	}
	efree(params);
}
...
CRX_FUNCTION(odbc_execute)
{
	zval *pv_res, *tmp;
	HashTable *pv_param_ht = (HashTable *) &crex_empty_array;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|h", &pv_res, &pv_param_ht) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (result->numparams > 0) {
		if ((ne = crex_hash_num_elements(pv_param_ht)) < result->numparams) {
			crx_error_docref(NULL, E_WARNING, "Not enough parameters (%d should be %d) given", ne, result->numparams);
			RETURN_FALSE;
		}
		...
		i = 1;
		CREX_HASH_FOREACH_VAL(pv_param_ht, tmp) {
			unsigned char otype = C_TYPE_P(tmp);
			crex_string *tmpstr = zval_try_get_string(tmp);
			if (!tmpstr) {
			...
			if (ZSTR_LEN(tmpstr) > 2 &&
				...
				if ((params[i-1].fp = open(filename,O_RDONLY)) == -1) {
					crx_error_docref(NULL, E_WARNING,"Can't open file %s", filename);
					odbc_release_params(result, params);
					efree(filename);
					RETURN_FALSE;
				}
...
CRX_FUNCTION(odbc_cursor)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r", &pv_res) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (max_len > 0) {
		...
		if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
			...
			if (!strncmp(state,"S1015",5)) {
				snprintf(cursorname, max_len+1, "crx_curs_" CREX_ULONG_FMT, (crex_ulong)result->stmt);
				...
				crx_error_docref(NULL, E_WARNING, "SQL error: %s, SQL state %s", errormsg, state);
				RETVAL_FALSE;
			}
		} else {
			RETVAL_STRING(cursorname);
...
CRX_FUNCTION(odbc_data_source)
{
	zval *zv_conn;
	crex_long zv_fetch_type;
	RETCODE rc = 0; /* assume all is good */
	odbc_connection *conn;
	UCHAR server_name[100], desc[200];
	SQLSMALLINT len1=0, len2=0, fetch_type;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "rl", &zv_conn, &zv_fetch_type) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!(fetch_type == SQL_FETCH_FIRST || fetch_type == SQL_FETCH_NEXT)) {
		crex_argument_value_error(2, "must be either SQL_FETCH_FIRST or SQL_FETCH_NEXT");
		RETURN_THROWS();
	}

	if (!(conn = (odbc_connection *)crex_fetch_resource2(C_RES_P(zv_conn), "ODBC-Link", le_conn, le_pconn))) {
		RETURN_THROWS();
	}
...
#ifdef CRX_ODBC_HAVE_FETCH_HASH
#define ODBC_NUM  1
#define ODBC_OBJECT  2

/* {{{ crx_odbc_fetch_hash */
static void crx_odbc_fetch_hash(INTERNAL_FUNCTION_PARAMETERS, int result_type)
{
	int i;
	odbc_result *result;
	RETCODE rc;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|l", &pv_res, &pv_row) == FAILURE) {
		RETURN_THROWS();
	}
	...
	for(i = 0; i < result->numcols; i++) {
		...
		switch(result->values[i].coltype) {
			...
			case SQL_WLONGVARCHAR:
				...
				if (rc == SQL_SUCCESS_WITH_INFO) {
					...
					crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%d (retcode %u)", i + 1, rc);
					ZVAL_FALSE(&tmp);
				} else if (result->values[i].vallen == SQL_NULL_DATA) {
					ZVAL_NULL(&tmp);
					break;
				} else if (result->values[i].vallen == SQL_NO_TOTAL) {
					crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%d (driver cannot determine length)", i + 1);
					ZVAL_FALSE(&tmp);
				} else {
		...
		if (result_type & ODBC_NUM) {
			crex_hash_index_update(C_ARRVAL_P(return_value), i, &tmp);
		} else {
			if (!*(result->values[i].name) && C_TYPE(tmp) == IS_STRING) {
				crex_hash_update(C_ARRVAL_P(return_value), C_STR(tmp), &tmp);
			} else {
				crex_hash_str_update(C_ARRVAL_P(return_value), result->values[i].name, strlen(result->values[i].name), &tmp);
			}
		}
	}
	if (buf) {
...
CRX_FUNCTION(odbc_fetch_object)
{
	crx_odbc_fetch_hash(INTERNAL_FUNCTION_PARAM_PASSTHRU, ODBC_OBJECT);
...
CRX_FUNCTION(odbc_fetch_into)
{
...
#ifdef HAVE_SQL_EXTENDED_FETCH
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rz|l", &pv_res, &pv_res_arr, &pv_row) == FAILURE) {
		RETURN_THROWS();
	}

	rownum = pv_row;
#else
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rz", &pv_res, &pv_res_arr) == FAILURE) {
		RETURN_THROWS();
	}
#endif /* HAVE_SQL_EXTENDED_FETCH */
	...
	pv_res_arr = crex_try_array_init(pv_res_arr);
	if (!pv_res_arr) {
		RETURN_THROWS();
	}
	...
	for(i = 0; i < result->numcols; i++) {
		...
		crex_hash_index_update(C_ARRVAL_P(pv_res_arr), i, &tmp);
	}
	if (buf) efree(buf);
...
CRX_FUNCTION(odbc_fetch_row)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|l!", &pv_res, &pv_row, &pv_row_is_null) == FAILURE) {
		RETURN_THROWS();
	}
...
/* {{{ Get result data */
CRX_FUNCTION(odbc_result)
{
	char *field;
	crex_string *field_str, *pv_field_str;
	crex_long pv_field_long;
	int field_ind;
	...
	CREX_PARSE_PARAMETERS_START(2, 2)
		C_PARAM_RESOURCE(pv_res)
		C_PARAM_STR_OR_LONG(pv_field_str, pv_field_long)
	...
	if (field != NULL) {
		if (result->values == NULL) {
			crx_error_docref(NULL, E_WARNING, "Result set contains no data");
			RETURN_FALSE;
		}
		...
		if (field_ind < 0) {
			crx_error_docref(NULL, E_WARNING, "Field %s not found", field);
			RETURN_FALSE;
		}
	} else {
		/* check for limits of field_ind if the field parameter was an int */
		if (field_ind >= result->numcols || field_ind < 0) {
			crx_error_docref(NULL, E_WARNING, "Field index is larger than the number of fields");
			RETURN_FALSE;
		}
	}
	...
	switch(result->values[field_ind].coltype) {
		...
		case SQL_WLONGVARCHAR:
			...
			if (IS_SQL_LONG(result->values[field_ind].coltype)) {
			   ...
			   CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)(field_ind + 1),
			...
			field_str = crex_string_alloc(fieldsize, 0);
		 ...
		 */
			...
			if (rc == SQL_ERROR) {
				odbc_sql_error(result->conn_ptr, result->stmt, "SQLGetData");
				crex_string_efree(field_str);
				RETURN_FALSE;
			}

			if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
				crex_string_efree(field_str);
				crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%d (retcode %u)", field_ind + 1, rc);
				RETURN_FALSE;
				...
				crex_string_efree(field_str);
				crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%d (driver cannot determine length)", field_ind + 1);
				RETURN_FALSE;
			}
			 ...
			 */
			if (rc != SQL_SUCCESS_WITH_INFO) {
				field_str = crex_string_truncate(field_str, result->values[field_ind].vallen, 0);
			}
...
/* If we come here, output unbound LONG and/or BINARY column data to the client */
	...
	while(1) {
		...
		CRXWRITE(field,(rc == SQL_SUCCESS_WITH_INFO) ? 4095 : result->values[field_ind].vallen);
...
CRX_FUNCTION(odbc_result_all)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|s", &pv_res, &pv_format, &pv_format_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (rc != SQL_SUCCESS && rc != SQL_SUCCESS_WITH_INFO) {
		crx_printf("<h2>No rows found</h2>\n");
		RETURN_LONG(0);
	}
	...
	if (CREX_NUM_ARGS() == 1) {
		...
		crx_printf("<table %s ><tr>", pv_format);
	}

	for (i = 0; i < result->numcols; i++) {
		crx_printf("<th>%s</th>", result->values[i].name);
	}
	...
	while(rc == SQL_SUCCESS || rc == SQL_SUCCESS_WITH_INFO) {
		...
		for(i = 0; i < result->numcols; i++) {
			...
			switch(result->values[i].coltype) {
				...
				case SQL_LONGVARBINARY:
					if (result->binmode <= 0) {
						crx_printf("<td>Not printable</td>");
						break;
					}
					if (result->binmode <= 1) sql_c_type = SQL_C_BINARY;
				...
				case SQL_WLONGVARCHAR:
					...
					if (rc == SQL_SUCCESS_WITH_INFO) {
						if (result->values[i].vallen == SQL_NO_TOTAL) {
							crx_printf("</td></tr></table>");
							crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%zu (driver cannot determine length)", i + 1);
							efree(buf);
							RETURN_FALSE;
						} else {
							CRXWRITE(buf, result->longreadlen);
						}
					} else if (rc != SQL_SUCCESS) {
						crx_printf("</td></tr></table>");
						crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%zu (retcode %u)", i + 1, rc);
						efree(buf);
						RETURN_FALSE;
					} else if (result->values[i].vallen == SQL_NULL_DATA) {
						crx_printf("<td>NULL</td>");
						break;
					} else {
						CRXWRITE(buf, result->values[i].vallen);
					}
					crx_printf("</td>");
					break;
				default:
					if (result->values[i].vallen == SQL_NULL_DATA) {
						...
						crx_error_docref(NULL, E_WARNING, "Cannot get data of column #%zu (driver cannot determine length)", i + 1);
						crx_printf("<td>FALSE</td>");
					} else {
						crx_printf("<td>%s</td>", result->values[i].value);
					}
					break;
			}
		}
...
CRX_FUNCTION(odbc_free_result)
{
	...
	crex_list_close(C_RES_P(pv_res));
...
#if defined(HAVE_EMPRESS) || defined(HAVE_UNIXODBC) || defined(CRX_WIN32) || defined (HAVE_IODBC)
 ...
 * #ifdef HAVE_EMPRESS */
	{
		...
		/* a connection string may have = but not ; - i.e. "DSN=CRX" */
		if (strstr((char*)db, "=")) {
			direct = 1;
			...
			if (is_uid_set && is_pwd_set) {
				...
				bool should_quote_uid = !crx_odbc_connstr_is_quoted(uid) && crx_odbc_connstr_should_quote(uid);
				bool should_quote_pwd = !crx_odbc_connstr_is_quoted(pwd) && crx_odbc_connstr_should_quote(pwd);
				if (should_quote_uid) {
					size_t estimated_length = crx_odbc_connstr_estimate_quote_length(uid);
					uid_quoted = emalloc(estimated_length);
					crx_odbc_connstr_quote(uid_quoted, uid, estimated_length);
				} else {
					uid_quoted = uid;
				}
				if (should_quote_pwd) {
					size_t estimated_length = crx_odbc_connstr_estimate_quote_length(pwd);
					pwd_quoted = emalloc(estimated_length);
					crx_odbc_connstr_quote(pwd_quoted, pwd, estimated_length);
				} else {
					pwd_quoted = pwd;
				}
...
void odbc_do_connect(INTERNAL_FUNCTION_PARAMETERS, int persistent)
{
	...
	crex_long pv_opt = SQL_CUR_DEFAULT;
	 ...
	 */
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sss|l", &db, &db_len, &uid, &uid_len, &pwd, &pwd_len, &pv_opt) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (CREX_NUM_ARGS() > 3) {
		...
		if (! (cur_opt == SQL_CUR_USE_IF_NEEDED ||
			...
			crex_argument_value_error(4, "must be one of SQL_CUR_USE_IF_NEEDED, "
...
try_and_get_another_connection:
	...
	if (persistent) {
		...
		if ((le = crex_hash_str_find_ptr(&EG(persistent_list), hashed_details, hashed_len)) == NULL) {
			if (ODBCG(max_links) != -1 && ODBCG(num_links) >= ODBCG(max_links)) {
				crx_error_docref(NULL, E_WARNING, "Too many open links (" CREX_LONG_FMT ")", ODBCG(num_links));
				efree(hashed_details);
				RETURN_FALSE;
			}
			if (ODBCG(max_persistent) != -1 && ODBCG(num_persistent) >= ODBCG(max_persistent)) {
				crx_error_docref(NULL, E_WARNING,"Too many open persistent links (" CREX_LONG_FMT ")", ODBCG(num_persistent));
				efree(hashed_details);
				RETURN_FALSE;
			}
			...
			if (crex_register_persistent_resource(hashed_details, hashed_len, db_conn, le_pconn) == NULL) {
				free(db_conn);
				efree(hashed_details);
				RETURN_FALSE;
			}
			ODBCG(num_persistent)++;
			ODBCG(num_links)++;
			db_conn->res = crex_register_resource(db_conn, le_pconn);
			RETVAL_RES(db_conn->res);
		} else { /* found connection */
			CREX_ASSERT(le->type == le_pconn);
			...
			if(ODBCG(check_persistent)){
				...
				if (ret == SQL_SUCCESS && dead == SQL_CD_TRUE) {
					...
					crex_hash_str_del(&EG(persistent_list), hashed_details, hashed_len);
		...
		if (ODBCG(max_links) != -1 && ODBCG(num_links) >= ODBCG(max_links)) {
			crx_error_docref(NULL, E_WARNING,"Too many open connections (" CREX_LONG_FMT ")",ODBCG(num_links));
			RETURN_FALSE;
		}
		...
		db_conn->res = crex_register_resource(db_conn, le_conn);
		RETVAL_RES(db_conn->res);
		ODBCG(num_links)++;
	}
}
...
CRX_FUNCTION(odbc_close)
{
	...
	if (C_RES_P(pv_conn)->type == le_pconn) {
		is_pconn = 1;
	}
	...
	crex_list_close(C_RES_P(pv_conn));

	if(is_pconn){
		crex_hash_apply_with_argument(&EG(persistent_list), _close_pconn_with_res, (void *) C_RES_P(pv_conn));
	}
}
/* }}} */

/* {{{ Get number of rows in a result */
CRX_FUNCTION(odbc_num_rows)
{
	odbc_result *result;
	SQLLEN rows;
	zval *pv_res;
...
CRX_FUNCTION(odbc_field_type)
{
	...
	CRX_ODBC_SQLCOLATTRIBUTE(result->stmt, (SQLUSMALLINT)pv_num, SQL_COLUMN_TYPE_NAME, tmp, 31, &tmplen, NULL);
	RETURN_STRING(tmp);
}
/* }}} */

/* {{{ Get the length (precision) of a column */
CRX_FUNCTION(odbc_field_len)
{
...
CRX_FUNCTION(odbc_field_num)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs", &pv_res, &fname, &fname_len) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_autocommit)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|b!", &pv_conn, &pv_onoff, &pv_onoff_is_null) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!pv_onoff_is_null) {
		...
		RETURN_LONG((crex_long)status);
	}
}
/* }}} */

/* {{{ Commit an ODBC transaction */
CRX_FUNCTION(odbc_commit)
{
...
/* {{{ crx_odbc_lasterror */
static void crx_odbc_lasterror(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	odbc_connection *conn;
	zval *pv_handle = NULL;
	char *ret;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|r!", &pv_handle) == FAILURE) {
		RETURN_THROWS();
	}

	if (pv_handle) {
		if (!(conn = (odbc_connection *)crex_fetch_resource2(C_RES_P(pv_handle), "ODBC-Link", le_conn, le_pconn))) {
			RETURN_THROWS();
		}
		if (mode == 0) {
			ret = conn->laststate;
...
CRX_FUNCTION(odbc_error)
{
	crx_odbc_lasterror(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Get the last error message */
CRX_FUNCTION(odbc_errormsg)
{
	crx_odbc_lasterror(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
*/
CRX_FUNCTION(odbc_setoption)
{
	odbc_connection *conn;
	odbc_result	*result;
	RETCODE rc;
	zval *pv_handle;
	crex_long pv_which, pv_opt, pv_val;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "rlll", &pv_handle, &pv_which, &pv_opt, &pv_val) == FAILURE) {
		RETURN_THROWS();
	}
	...
	switch (pv_which) {
		case 1:		/* SQLSetConnectOption */
			...
			if (conn->persistent) {
				crx_error_docref(NULL, E_WARNING, "Unable to set option for persistent connection");
				RETURN_FALSE;
			}
		...
		case 2:		/* SQLSetStmtOption */
			if ((result = (odbc_result *)crex_fetch_resource(C_RES_P(pv_handle), "ODBC result", le_result)) == NULL) {
				RETURN_THROWS();
			}
			...
			break;
		default:
			crex_argument_value_error(2, "must be 1 for SQLSetConnectOption(), or 2 for SQLSetStmtOption()");
			RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_tables)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|s!s!s!s!", &pv_conn, &cat, &cat_len, &schema, &schema_len,
...
CRX_FUNCTION(odbc_columnprivileges)
{
	zval *pv_conn;
	odbc_result *result = NULL;
	odbc_connection *conn;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs!sss", &pv_conn, &cat, &cat_len, &schema, &schema_len,
		&table, &table_len, &column, &column_len) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_foreignkeys)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs!sssss", &pv_conn, &pcat, &pcat_len, &pschema, &pschema_len,
...
CRX_FUNCTION(odbc_gettypeinfo)
{
	zval *pv_conn;
	crex_long pv_data_type = SQL_ALL_TYPES;
	odbc_result *result = NULL;
	odbc_connection *conn;
	RETCODE rc;
	SQLSMALLINT data_type;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|l", &pv_conn, &pv_data_type) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_primarykeys)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs!ss", &pv_conn, &cat, &cat_len, &schema, &schema_len, &table, &table_len) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_procedurecolumns)
{
	zval *pv_conn;
	odbc_result *result = NULL;
	odbc_connection *conn;
...
CRX_FUNCTION(odbc_procedures)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "r|s!s!s!", &pv_conn, &cat, &cat_len, &schema, &schema_len, &proc, &proc_len) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(odbc_specialcolumns)
{
	zval *pv_conn;
	crex_long vtype, vscope, vnullable;
	odbc_result *result = NULL;
	odbc_connection *conn;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rls!ssll", &pv_conn, &vtype, &cat, &cat_len, &schema, &schema_len,
...
CRX_FUNCTION(odbc_statistics)
{
	zval *pv_conn;
	crex_long vunique, vreserved;
	odbc_result *result = NULL;
	odbc_connection *conn;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "rs!ssll", &pv_conn, &cat, &cat_len, &schema, &schema_len,
...
CRX_FUNCTION(odbc_tableprivileges)
{
	zval *pv_conn;
	odbc_result   *result = NULL;
	odbc_connection *conn;
