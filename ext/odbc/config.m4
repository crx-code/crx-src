AC_DEFUN([CRX_ODBC_CHECK_HEADER],[
...
dnl
AC_DEFUN([CRX_ODBC_FIND_SOLID_LIBS],[
...
AC_DEFUN([CRX_ODBC_FIND_EMPRESS_LIBS],[
...
AC_DEFUN([CRX_ODBC_FIND_EMPRESS_BCS_LIBS],[
...
CRX_ARG_WITH([adabas],,
  ...
  if test "$CRX_ADABAS" != "no"; then
    if test "$CRX_ADABAS" = "yes"; then
      CRX_ADABAS=/usr/local
    fi
    CRX_ADD_INCLUDE($CRX_ADABAS/incl)
    CRX_ADD_LIBPATH($CRX_ADABAS/$CRX_LIBDIR)
    ODBC_OBJS="$CRX_ADABAS/$CRX_LIBDIR/odbclib.a"
    ODBC_LIB="$abs_builddir/ext/odbc/libodbc_adabas.a"
    $srcdir/build/shtool mkdir -f -p ext/odbc
    rm -f "$ODBC_LIB"
    cp "$ODBC_OBJS" "$ODBC_LIB"
    CRX_ADD_LIBRARY(sqlptc)
    CRX_ADD_LIBRARY(sqlrte)
    CRX_ADD_LIBRARY_WITH_PATH(odbc_adabas, $abs_builddir/ext/odbc)
    ODBC_TYPE=adabas
    ODBC_INCDIR=$CRX_ADABAS/incl
    CRX_ODBC_CHECK_HEADER(sqlext.h)
...
CRX_ARG_WITH([sapdb],,
  ...
  if test "$CRX_SAPDB" != "no"; then
    if test "$CRX_SAPDB" = "yes"; then
      CRX_SAPDB=/usr/local
    fi
    CRX_ADD_INCLUDE($CRX_SAPDB/incl)
    CRX_ADD_LIBPATH($CRX_SAPDB/$CRX_LIBDIR)
    CRX_ADD_LIBRARY(sqlod)
    ODBC_TYPE=sapdb
...
CRX_ARG_WITH([solid],,
  ...
  if test "$CRX_SOLID" != "no"; then
    if test "$CRX_SOLID" = "yes"; then
      CRX_SOLID=/usr/local/solid
    fi
    ODBC_INCDIR=$CRX_SOLID/include
    ODBC_LIBDIR=$CRX_SOLID/$CRX_LIBDIR
    ...
    AC_MSG_RESULT([$ext_output])
    CRX_ODBC_FIND_SOLID_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
if test -z "$ODBC_TYPE"; then
CRX_ARG_WITH([ibm-db2],,
  [AS_HELP_STRING([[--with-ibm-db2[=DIR]]],
    [Include IBM DB2 support [/home/db2inst1/sqllib]])])

  AC_MSG_CHECKING(for IBM DB2 support)
  if test "$CRX_IBM_DB2" != "no"; then
    if test "$CRX_IBM_DB2" = "yes"; then
      ODBC_INCDIR=/home/db2inst1/sqllib/include
      ODBC_LIBDIR=/home/db2inst1/sqllib/lib
    else
      ODBC_INCDIR=$CRX_IBM_DB2/include
      ODBC_LIBDIR=$CRX_IBM_DB2/$CRX_LIBDIR
    fi

    CRX_ODBC_CHECK_HEADER(sqlcli1.h)
...
You need to source your DB2 environment before running CRX configure:
# . \$IBM_DB2/db2profile
])
    ], [
...
CRX_ARG_WITH([empress],,
  ...
  if test "$CRX_EMPRESS" != "no"; then
    ...
    AC_MSG_RESULT([$ext_output])
    CRX_ODBC_FIND_EMPRESS_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
if test -z "$ODBC_TYPE"; then
CRX_ARG_WITH([empress-bcs],,
  [AS_HELP_STRING([[--with-empress-bcs[=DIR]]],
    [Include Empress Local Access support $EMPRESSPATH (Empress Version >=
    8.60 required)])])

  AC_MSG_CHECKING(for Empress local access support)
  if test "$CRX_EMPRESS_BCS" != "no"; then
    if test "$CRX_EMPRESS_BCS" = "yes"; then
      ODBC_INCDIR=$EMPRESSPATH/include/odbc
      ODBC_LIBDIR=$EMPRESSPATH/shlib
    else
      ODBC_INCDIR=$CRX_EMPRESS_BCS/include/odbc
      ODBC_LIBDIR=$CRX_EMPRESS_BCS/shlib
    fi
    ...
    ODBC_LIBS="-lempcrxbcs -lms -lmscfg -lbasic -lbasic_os -lnlscstab -lnlsmsgtab -lm -ldl -lcrypt"
    ODBC_TYPE=empress-bcs
    AC_DEFINE(HAVE_EMPRESS,1,[ ])
    AC_MSG_RESULT([$ext_output])
    CRX_ODBC_FIND_EMPRESS_BCS_LIBS($ODBC_LIBDIR)
  else
    AC_MSG_RESULT(no)
  fi
fi
...
CRX_ARG_WITH([custom-odbc],,
  ...
  if test "$CRX_CUSTOM_ODBC" != "no"; then
    if test "$CRX_CUSTOM_ODBC" = "yes"; then
      CRX_CUSTOM_ODBC=/usr/local
    fi
    ODBC_INCDIR=$CRX_CUSTOM_ODBC/include
    ODBC_LIBDIR=$CRX_CUSTOM_ODBC/$CRX_LIBDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_CFLAGS=-I$ODBC_INCDIR
...
CRX_ARG_WITH([iodbc],,
  ...
  if test "$CRX_IODBC" != "no"; then
    AC_MSG_RESULT(yes)
    PKG_CHECK_MODULES([ODBC], [libiodbc])
    CRX_EVAL_INCLINE($ODBC_CFLAGS)
    ODBC_TYPE=iodbc
...
CRX_ARG_WITH([esoob],,
  ...
  if test "$CRX_ESOOB" != "no"; then
    if test "$CRX_ESOOB" = "yes"; then
      CRX_ESOOB=/usr/local/easysoft/oob/client
    fi
    ODBC_INCDIR=$CRX_ESOOB/include
    ODBC_LIBDIR=$CRX_ESOOB/$CRX_LIBDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ODBC_CFLAGS=-I$ODBC_INCDIR
...
if test -z "$ODBC_TYPE"; then
CRX_ARG_WITH([dbmaker],,
  [AS_HELP_STRING([[--with-dbmaker[=DIR]]],
    [Include DBMaker support])])

  AC_MSG_CHECKING(for DBMaker support)
  if test "$CRX_DBMAKER" != "no"; then
    if test "$CRX_DBMAKER" = "yes"; then
      ...
      if test "$DBMAKER_VERSION" = "2.9"; then
        CRX_DBMAKER=$DBMAKER_HOME
      else
        CRX_DBMAKER=$DBMAKER_HOME/$DBMAKER_VERSION
      fi
    fi

    ODBC_INCDIR=$CRX_DBMAKER/include
    ODBC_LIBDIR=$CRX_DBMAKER/$CRX_LIBDIR
    ODBC_CFLAGS=-I$ODBC_INCDIR
    ODBC_LFLAGS=-L$ODBC_LIBDIR
    ...
    if test "$ext_shared" = "yes"; then
      ...
      CRX_ADD_LIBRARY_WITH_PATH(dmapic, $ODBC_LIBDIR)
      CRX_ADD_INCLUDE($ODBC_INCDIR)
      ODBC_STATIC="libcrxext_odbc.la"
    fi
  else
    AC_MSG_RESULT(no)
  fi
...
if test "no" != "$CRX_ODBCVER"; then
  if test "$CRX_ODBCVER" != "0"; then
    AC_DEFINE_UNQUOTED(ODBCVER, $CRX_ODBCVER, [ The highest supported ODBC version ])
  fi
else
...
if test -n "$ODBC_TYPE"; then
  if test "$ODBC_TYPE" != "dbmaker"; then
    CRX_EVAL_LIBLINE([$ODBC_LFLAGS $ODBC_LIBS], ODBC_SHARED_LIBADD)
  ...
  CRX_SUBST(ODBC_SHARED_LIBADD)
  CRX_SUBST(ODBC_INCDIR)
  CRX_SUBST(ODBC_LIBDIR)
  CRX_SUBST_OLD(ODBC_CFLAGS)
  CRX_SUBST_OLD(ODBC_LIBS)
  CRX_SUBST_OLD(ODBC_LFLAGS)
  CRX_SUBST_OLD(ODBC_TYPE)

  CRX_NEW_EXTENSION(odbc, crx_odbc.c odbc_utils.c, $ext_shared,, [$ODBC_CFLAGS -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1])
else
