...
typedef crex_long (*cal_to_jd_func_t) (int month, int day, int year);
typedef void (*cal_from_jd_func_t) (crex_long jd, int *year, int *month, int *day);
...
crex_module_entry calendar_module_entry = {
	...
	NULL,
	CRX_MINFO(calendar),
	CRX_CALENDAR_VERSION,
	STANDARD_MODULE_PROPERTIES,
};

#ifdef COMPILE_DL_CALENDAR
CREX_GET_MODULE(calendar)
#endif
...
CRX_MINFO_FUNCTION(calendar)
{
	...
	crx_info_print_table_row(2, "Calendar support", "enabled");
	crx_info_print_table_end();
}

static void _crx_cal_info(int cal, zval *ret)
{
	zval months, smonths;
	int i;
...
CRX_FUNCTION(cal_info)
{
	crex_long cal = -1;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|l", &cal) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (cal < 0 || cal >= CAL_NUM_CALS) {
		crex_argument_value_error(1, "must be a valid calendar ID");
		RETURN_THROWS();
	}

	_crx_cal_info(cal, return_value);
}
/* }}} */

/* {{{ Returns the number of days in a month for a given year and calendar */
CRX_FUNCTION(cal_days_in_month)
{
	crex_long cal, month, year;
	const struct cal_entry_t *calendar;
	crex_long sdn_start, sdn_next;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "lll", &cal, &month, &year) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(cal_to_jd)
{
	crex_long cal, month, day, year;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "llll", &cal, &month, &day, &year) != SUCCESS) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(cal_from_jd)
{
	crex_long jd, cal;
	int month, day, year, dow;
	const struct cal_entry_t *calendar;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "ll", &jd, &cal) == FAILURE) {
		RETURN_THROWS();
	}

	if (cal < 0 || cal >= CAL_NUM_CALS) {
		crex_argument_value_error(2, "must be a valid calendar ID");
		RETURN_THROWS();
	}
	calendar = &cal_conversion_table[cal];
	...
	add_assoc_str(return_value, "date",
		crex_strpprintf(0, "%i/%i/%i", month, day, year));
...
CRX_FUNCTION(jdtogregorian)
{
	crex_long julday;
	int year, month, day;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l", &julday) == FAILURE) {
		RETURN_THROWS();
	}

	SdnToGregorian(julday, &year, &month, &day);

	RETURN_NEW_STR(crex_strpprintf(0, "%i/%i/%i", month, day, year));
}
/* }}} */

/* {{{ Converts a gregorian calendar date to julian day count */
CRX_FUNCTION(gregoriantojd)
{
	crex_long year, month, day;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "lll", &month, &day, &year) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(jdtojewish)
{
	crex_long julday, fl = 0;
	bool heb   = 0;
	int year, month, day;
	char *dayp, *yearp;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|bl", &julday, &heb, &fl) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!heb) {
		...
		if (year <= 0 || year > 9999) {
			crex_value_error("Year out of range (0-9999)");
			RETURN_THROWS();
		}

		RETVAL_NEW_STR(crex_strpprintf(0, "%s %s %s", heb_number_to_chars(day, fl, &dayp), JEWISH_HEB_MONTH_NAME(year)[month], heb_number_to_chars(year, fl, &yearp)));
...
CRX_FUNCTION(jddayofweek)
{
	crex_long julday, mode = CAL_DOW_DAYNO;
	int day;
	const char *daynamel, *daynames;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|l", &julday, &mode) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(jdmonthname)
{
	crex_long julday, mode;
	const char *monthname = NULL;
	int month, day, year;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "ll", &julday, &mode) == FAILURE) {
		RETURN_THROWS();
	}
