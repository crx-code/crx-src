...
static void _cal_easter(INTERNAL_FUNCTION_PARAMETERS, bool gm)
{
	...
	struct tm te;
	crex_long year, golden, solar, lunar, pfm, dom, tmp, easter, result;
	crex_long method = CAL_EASTER_DEFAULT;
	bool year_is_null = 1;
	...
	if (gm && year < 1970) {
		...
		crex_argument_value_error(1, "must be a year after 1970 (inclusive)");
		RETURN_THROWS();
	}

	if (gm && year > 2000000000) {
		/* timestamps only go up to the year 2.000.000.000 */
		crex_argument_value_error(1, "must be a year before 2.000.000.000 (inclusive)");
		RETURN_THROWS();
	}
	#else
	/* Compiling for 32bit, allow years between 1970 and 2037 */	
	if (gm && (year < 1970 || year > 2037)) {
		crex_argument_value_error(1, "must be between 1970 and 2037 (inclusive)");
		RETURN_THROWS();
	}
	#endif
