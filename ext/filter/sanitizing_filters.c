...
/* {{{ HELPER FUNCTIONS */
static void crx_filter_encode_html(zval *value, const unsigned char *chars)
{
	smart_str str = {0};
	size_t len = C_STRLEN_P(value);
	unsigned char *s = (unsigned char *)C_STRVAL_P(value);
	unsigned char *e = s + len;

	if (C_STRLEN_P(value) == 0) {
		return;
	}
	...
	while (s < e) {
		if (chars[*s]) {
			...
			smart_str_append_unsigned(&str, (crex_ulong)*s);
...
static void crx_filter_encode_url(zval *value, const unsigned char* chars, const int char_len, int high, int low, int encode_nul)
{
	unsigned char *p;
	...
	str = crex_string_safe_alloc(C_STRLEN_P(value), 3, 0, 0);
	p = (unsigned char *) ZSTR_VAL(str);
	s = (unsigned char *) C_STRVAL_P(value);
	e = s + C_STRLEN_P(value);
...
static void crx_filter_strip(zval *value, crex_long flags)
{
	unsigned char *str;
	size_t i;
	size_t c;
	...
	str = (unsigned char *)C_STRVAL_P(value);
	buf = crex_string_alloc(C_STRLEN_P(value), 0);
	c = 0;
	for (i = 0; i < C_STRLEN_P(value); i++) {
...
void crx_filter_string(CRX_INPUT_FILTER_PARAM_DECL)
{
	...
	if (!C_REFCOUNTED_P(value)) {
		ZVAL_STRINGL(value, C_STRVAL_P(value), C_STRLEN_P(value));
	}

	/* strip high/strip low ( see flags )*/
	crx_filter_strip(value, flags);
	...
	crx_filter_encode_html(value, enc);

	/* strip tags, implicitly also removes \0 chars */
	new_len = crx_strip_tags_ex(C_STRVAL_P(value), C_STRLEN_P(value), NULL, 0, 1);
	C_STRLEN_P(value) = new_len;
...
void crx_filter_encoded(CRX_INPUT_FILTER_PARAM_DECL)
{
	...
	crx_filter_strip(value, flags);
	/* urlencode */
	crx_filter_encode_url(value, (unsigned char *)DEFAULT_URL_ENCODE, sizeof(DEFAULT_URL_ENCODE)-1, flags & FILTER_FLAG_ENCODE_HIGH, flags & FILTER_FLAG_ENCODE_LOW, 1);
}
/* }}} */

/* {{{ crx_filter_special_chars */
...
void crx_filter_full_special_chars(CRX_INPUT_FILTER_PARAM_DECL)
{
	...
	buf = crx_escape_html_entities_ex(
		(unsigned char *) C_STRVAL_P(value), C_STRLEN_P(value), /* all */ 1, quotes,
...
void crx_filter_unsafe_raw(CRX_INPUT_FILTER_PARAM_DECL)
{
	...
	if (flags != 0 && C_STRLEN_P(value) > 0) {
		...
		crx_filter_encode_html(value, enc);
	} else if (flags & FILTER_FLAG_EMPTY_STRING_NULL && C_STRLEN_P(value) == 0) {
		zval_ptr_dtor(value);
		ZVAL_NULL(value);
	}
}
...
/* {{{ crx_filter_add_slashes */
void crx_filter_add_slashes(CRX_INPUT_FILTER_PARAM_DECL)
{
	crex_string *buf = crx_addslashes(C_STR_P(value));
