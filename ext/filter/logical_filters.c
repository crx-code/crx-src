...
#define FETCH_DOUBLE_OPTION(var_name, option_name) \
	...
	var_name##_set = 0; \
	if (option_array) { \
		if ((option_val = crex_hash_str_find(C_ARRVAL_P(option_array), option_name, sizeof(option_name) - 1)) != NULL) {	\
...
#define FETCH_STRING_OPTION(var_name, option_name) \
	...
	var_name##_len = 0; \
	if (option_array) { \
		if ((option_val = crex_hash_str_find_deref(C_ARRVAL_P(option_array), option_name, sizeof(option_name) - 1)) != NULL) { \
			if (C_TYPE_P(option_val) == IS_STRING) { \
				var_name = C_STRVAL_P(option_val); \
				var_name##_len = C_STRLEN_P(option_val); \
				var_name##_set = 1; \
			} \
		} \
	}
...
static int _crx_filter_validate_ipv6(char *str, size_t str_len, int ip[8]);

static int crx_filter_parse_int(const char *str, size_t str_len, crex_long *ret) { /* {{{ */
	crex_long ctx_value;
	int sign = 0, digit = 0;
	...
	while (str < end) {
		if (*str >= '0' && *str <= '9') {
			digit = (*(str++) - '0');
			if ( (!sign) && ctx_value <= (CREX_LONG_MAX-digit)/10 ) {
				ctx_value = (ctx_value * 10) + digit;
			} else if ( sign && ctx_value >= (CREX_LONG_MIN+digit)/10) {
				ctx_value = (ctx_value * 10) - digit;
			} else {
				return -1;
			}
...
static int crx_filter_parse_octal(const char *str, size_t str_len, crex_long *ret) { /* {{{ */
	crex_ulong ctx_value = 0;
	const char *end = str + str_len;
	...
	while (str < end) {
		if (*str >= '0' && *str <= '7') {
			...
			if ((ctx_value > ((crex_ulong)(~(crex_long)0)) / 8) ||
				((ctx_value = ctx_value * 8) > ((crex_ulong)(~(crex_long)0)) - n)) {
				return -1;
			}
			ctx_value += n;
		} else {
	...
	*ret = (crex_long)ctx_value;
	return 1;
}
/* }}} */

static int crx_filter_parse_hex(const char *str, size_t str_len, crex_long *ret) { /* {{{ */
	crex_ulong ctx_value = 0;
	const char *end = str + str_len;
	crex_ulong n;
	...
	while (str < end) {
		...
		if ((ctx_value > ((crex_ulong)(~(crex_long)0)) / 16) ||
			((ctx_value = ctx_value * 16) > ((crex_ulong)(~(crex_long)0)) - n)) {
			return -1;
		}
		ctx_value += n;
	}
...
{
	zval *option_val;
	crex_long  min_range, max_range, option_flags;
	...
	if (*p == '0') {
		...
		if (allow_hex && (*p == 'x' || *p == 'X')) {
			...
			if (crx_filter_parse_hex(p, len, &ctx_value) < 0) {
				error = 1;
			}
		} else if (allow_octal) {
			...
			if (crx_filter_parse_octal(p, len, &ctx_value) < 0) {
				error = 1;
			}
		} else if (len != 0) {
			error = 1;
		...
		if (crx_filter_parse_int(p, len, &ctx_value) < 0) {
			error = 1;
		}
	}
...
{
	char *str = C_STRVAL_P(value);
	size_t len = C_STRLEN_P(value);
	int ret;

	CRX_FILTER_TRIM_DEFAULT_EX(str, len, 0);
...
void crx_filter_float(CRX_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	len = C_STRLEN_P(value);
	str = C_STRVAL_P(value);

	CRX_FILTER_TRIM_DEFAULT(str, len);
	end = str + len;
	...
	if (decimal_set) {
		if (decimal_len != 1) {
			crex_value_error("%s(): \"decimal\" option must be one character long", get_active_function_name());
			RETURN_VALIDATION_FAILED
		} else {
			dec_sep = *decimal;
		}
	...
	if (thousand_set) {
		if (thousand_len < 1) {
			crex_value_error("%s(): \"thousand\" option cannot be empty", get_active_function_name());
			RETURN_VALIDATION_FAILED
		} else {
			tsd_sep = thousand;
		}
	...
	*p = 0;
	...
	switch (is_numeric_string(num, p - num, &lval, &dval, 0)) {
		case IS_LONG:
			...
			break;
		case IS_DOUBLE:
			if ((!dval && p - num > 1 && strpbrk(num, "123456789")) || !crex_finite(dval)) {
				goto error;
			}
...
void crx_filter_validate_regexp(CRX_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	if (!regexp_set) {
		crex_value_error("%s(): \"regexp\" option is missing", get_active_function_name());
		RETURN_VALIDATION_FAILED
	}
	...
	rc = pcre2_match(re, (PCRE2_SPTR)C_STRVAL_P(value), C_STRLEN_P(value), 0, 0, match_data, crx_pcre_mctx());
...
static int _crx_filter_validate_domain(char * domain, size_t len, crex_long flags) /* {{{ */
{
	char *e, *s, *t;
	size_t l;
...
{
	if (!_crx_filter_validate_domain(C_STRVAL_P(value), C_STRLEN_P(value), flags)) {
		RETURN_VALIDATION_FAILED
	}
}
/* }}} */

static int is_userinfo_valid(crex_string *str)
{
	const char *valid = "-._~!$&'()*+,;=:";
...
{
	crx_url *url;
	size_t old_len = C_STRLEN_P(value);

	crx_filter_url(value, flags, option_array, charset);

	if (C_TYPE_P(value) != IS_STRING || old_len != C_STRLEN_P(value)) {
		RETURN_VALIDATION_FAILED
	}

	/* Use parse_url - if it returns false, we return NULL */
	url = crx_url_parse_ex(C_STRVAL_P(value), C_STRLEN_P(value));
	...
	if (url->scheme != NULL &&
		(crex_string_equals_literal_ci(url->scheme, "http") || crex_string_equals_literal_ci(url->scheme, "https"))) {
		char *e, *s, *t;
		size_t l;
		...
		if (*s == '[' && *t == ']' && _crx_filter_validate_ipv6((s + 1), l - 2, NULL)) {
			crx_url_free(url);
			return;
		}

		// Validate domain
		if (!_crx_filter_validate_domain(ZSTR_VAL(url->host), l, FILTER_FLAG_HOSTNAME)) {
			crx_url_free(url);
			RETURN_VALIDATION_FAILED
		}
	}
	...
	if (
		...
		(url->host == NULL && (!crex_string_equals_literal(url->scheme, "mailto") && !crex_string_equals_literal(url->scheme, "news") && !crex_string_equals_literal(url->scheme, "file"))) ||
...
void crx_filter_validate_email(CRX_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	if (C_STRLEN_P(value) > 320) {
		RETURN_VALIDATION_FAILED
	}

	sregexp = crex_string_init(regexp, regexp_len, 0);
	re = pcre_get_compiled_regex(sregexp, &capture_count);
	crex_string_release_ex(sregexp, 0);
	if (!re) {
		RETURN_VALIDATION_FAILED
	}
	match_data = crx_pcre_create_match_data(capture_count, re);
...
static int _crx_filter_validate_ipv4(char *str, size_t str_len, int *ip) /* {{{ */
{
	const char *end = str + str_len;
	int num, m;
	int n = 0;
...
static int _crx_filter_validate_ipv6(char *str, size_t str_len, int ip[8]) /* {{{ */
{
	...
	if (ipv4) {
		...
		if (!_crx_filter_validate_ipv4(ipv4, (str_len - (ipv4 - str)), ip4elm)) {
			return 0;
		}
...
void crx_filter_validate_ip(CRX_INPUT_FILTER_PARAM_DECL) /* {{{ */
{
	...
	if (memchr(C_STRVAL_P(value), ':', C_STRLEN_P(value))) {
		mode = FORMAT_IPV6;
	} else if (memchr(C_STRVAL_P(value), '.', C_STRLEN_P(value))) {
		mode = FORMAT_IPV4;
	} else {
		RETURN_VALIDATION_FAILED
	}
	...
	switch (mode) {
		case FORMAT_IPV4:
			if (!_crx_filter_validate_ipv4(C_STRVAL_P(value), C_STRLEN_P(value), ip)) {
				RETURN_VALIDATION_FAILED
			}
		...
		case FORMAT_IPV6:
			{
				int res = 0;
				res = _crx_filter_validate_ipv6(C_STRVAL_P(value), C_STRLEN_P(value), ip);
				if (res < 1) {
					RETURN_VALIDATION_FAILED
				}
				/* Check flags */
...
{
	char *input = C_STRVAL_P(value);
	size_t input_len = C_STRLEN_P(value);
	...
	if (exp_separator_set && exp_separator_len != 1) {
		crex_value_error("%s(): \"separator\" option must be one character long", get_active_function_name());
		RETURN_VALIDATION_FAILED;
	}
	...
	for (i = 0; i < tokens; i++) {
		...
		if (crx_filter_parse_hex(input + offset, length, &ret) < 0) {
