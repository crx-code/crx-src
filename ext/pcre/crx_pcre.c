...
#define CRX_PCRE_JIT_SUPPORT 1
#else
#define CRX_PCRE_JIT_SUPPORT 0
#endif

char *crx_pcre_version;

#include "crx_pcre_arginfo.h"
...
CREX_TLS pcre2_compile_context *cctx = NULL;
CREX_TLS pcre2_match_context   *mctx = NULL;
CREX_TLS pcre2_match_data      *mdata = NULL;
CREX_TLS bool              mdata_used = 0;
CREX_TLS uint8_t pcre2_init_ok = 0;
#if defined(ZTS) && defined(HAVE_PCRE_JIT_SUPPORT)
static MUTEX_T pcre_mt = NULL;
#define crx_pcre_mutex_alloc() \
	if (tsrm_is_main_thread() && !pcre_mt) pcre_mt = tsrm_mutex_alloc();
#define crx_pcre_mutex_free() \
	if (tsrm_is_main_thread() && pcre_mt) { tsrm_mutex_free(pcre_mt); pcre_mt = NULL; }
#define crx_pcre_mutex_lock() tsrm_mutex_lock(pcre_mt);
#define crx_pcre_mutex_unlock() tsrm_mutex_unlock(pcre_mt);
#else
#define crx_pcre_mutex_alloc()
#define crx_pcre_mutex_free()
#define crx_pcre_mutex_lock()
#define crx_pcre_mutex_unlock()
#endif

CREX_TLS HashTable char_tables;

static void crx_pcre_free_char_table(zval *data)
{/*{{{*/
	void *ptr = C_PTR_P(data);
	pefree(ptr, 1);
}/*}}}*/
...
static void pcre_handle_exec_error(int pcre_code) /* {{{ */
{
	...
	switch (pcre_code) {
		case PCRE2_ERROR_MATCHLIMIT:
			preg_code = CRX_PCRE_BACKTRACK_LIMIT_ERROR;
			break;

		case PCRE2_ERROR_RECURSIONLIMIT:
			preg_code = CRX_PCRE_RECURSION_LIMIT_ERROR;
			break;

		case PCRE2_ERROR_BADUTFOFFSET:
			preg_code = CRX_PCRE_BAD_UTF8_OFFSET_ERROR;
			break;

#ifdef HAVE_PCRE_JIT_SUPPORT
		case PCRE2_ERROR_JIT_STACKLIMIT:
			preg_code = CRX_PCRE_JIT_STACKLIMIT_ERROR;
			break;
#endif

		default:
			if (pcre_code <= PCRE2_ERROR_UTF8_ERR1 && pcre_code >= PCRE2_ERROR_UTF8_ERR21) {
				preg_code = CRX_PCRE_BAD_UTF8_ERROR;
			} else  {
				preg_code = CRX_PCRE_INTERNAL_ERROR;
			}
			break;
	}
...
static const char *crx_pcre_get_error_msg(crx_pcre_error_code error_code) /* {{{ */
{
	switch (error_code) {
		case CRX_PCRE_NO_ERROR:
			return "No error";
		case CRX_PCRE_INTERNAL_ERROR:
			return "Internal error";
		case CRX_PCRE_BAD_UTF8_ERROR:
			return "Malformed UTF-8 characters, possibly incorrectly encoded";
		case CRX_PCRE_BAD_UTF8_OFFSET_ERROR:
			return "The offset did not correspond to the beginning of a valid UTF-8 code point";
		case CRX_PCRE_BACKTRACK_LIMIT_ERROR:
			return "Backtrack limit exhausted";
		case CRX_PCRE_RECURSION_LIMIT_ERROR:
			return "Recursion limit exhausted";

#ifdef HAVE_PCRE_JIT_SUPPORT
		case CRX_PCRE_JIT_STACKLIMIT_ERROR:
...
static void crx_free_pcre_cache(zval *data) /* {{{ */
{
	pcre_cache_entry *pce = (pcre_cache_entry *) C_PTR_P(data);
	if (!pce) return;
...
static void crx_efree_pcre_cache(zval *data) /* {{{ */
{
	pcre_cache_entry *pce = (pcre_cache_entry *) C_PTR_P(data);
	if (!pce) return;
	pcre2_code_free(pce->re);
...
static void *crx_pcre_malloc(PCRE2_SIZE size, void *data)
{
	return pemalloc(size, 1);
}

static void crx_pcre_free(void *block, void *data)
{
	pefree(block, 1);
}

static void *crx_pcre_emalloc(PCRE2_SIZE size, void *data)
{
	return emalloc(size);
}

static void crx_pcre_efree(void *block, void *data)
{
	efree(block);
}
...
#define CRX_PCRE_DEFAULT_EXTRA_COPTIONS PCRE2_EXTRA_ALLOW_LOOKAROUND_BSK
#else
#define CRX_PCRE_DEFAULT_EXTRA_COPTIONS 0
#endif

#define CRX_PCRE_PREALLOC_MDATA_SIZE 32

static void crx_pcre_init_pcre2(uint8_t jit)
{/*{{{*/
	if (!gctx) {
		gctx = pcre2_general_context_create(crx_pcre_malloc, crx_pcre_free, NULL);
		if (!gctx) {
			pcre2_init_ok = 0;
			return;
		}
	...
	pcre2_set_compile_extra_options(cctx, CRX_PCRE_DEFAULT_EXTRA_COPTIONS);
	...
	if (!mdata) {
		mdata = pcre2_match_data_create(CRX_PCRE_PREALLOC_MDATA_SIZE, gctx);
		if (!mdata) {
			pcre2_init_ok = 0;
			return;
		}
...
static void crx_pcre_shutdown_pcre2(void)
{/*{{{*/
	if (gctx) {
...
static CRX_GINIT_FUNCTION(pcre) /* {{{ */
{
	crx_pcre_mutex_alloc();
	...
	if (!pcre_globals->per_request_cache) {
		crex_hash_init(&pcre_globals->pcre_cache, 0, NULL, crx_free_pcre_cache, 1);
	}

	pcre_globals->backtrack_limit = 0;
	pcre_globals->recursion_limit = 0;
	pcre_globals->error_code      = CRX_PCRE_NO_ERROR;
	...
	crx_pcre_init_pcre2(1);
	crex_hash_init(&char_tables, 1, NULL, crx_pcre_free_char_table, 1);
}
/* }}} */

static CRX_GSHUTDOWN_FUNCTION(pcre) /* {{{ */
{
	if (!pcre_globals->per_request_cache) {
		crex_hash_destroy(&pcre_globals->pcre_cache);
	}

	crx_pcre_shutdown_pcre2();
	crex_hash_destroy(&char_tables);
	crx_pcre_mutex_free();
}
/* }}} */

static CRX_INI_MH(OnUpdateBacktrackLimit)
{/*{{{*/
...
static CRX_INI_MH(OnUpdateRecursionLimit)
{/*{{{*/
	OnUpdateLong(entry, new_value, mh_arg1, mh_arg2, mh_arg3, stage);
	if (mctx) {
...
CRX_INI_BEGIN()
	STD_CRX_INI_ENTRY("pcre.backtrack_limit", "1000000", CRX_INI_ALL, OnUpdateBacktrackLimit, backtrack_limit, crex_pcre_globals, pcre_globals)
	STD_CRX_INI_ENTRY("pcre.recursion_limit", "100000",  CRX_INI_ALL, OnUpdateRecursionLimit, recursion_limit, crex_pcre_globals, pcre_globals)
#ifdef HAVE_PCRE_JIT_SUPPORT
	STD_CRX_INI_BOOLEAN("pcre.jit",           "1",       CRX_INI_ALL, OnUpdateJit,            jit,             crex_pcre_globals, pcre_globals)
#endif
CRX_INI_END()
...
/* {{{ CRX_MINFO_FUNCTION(pcre) */
static CRX_MINFO_FUNCTION(pcre)
{
#ifdef HAVE_PCRE_JIT_SUPPORT
	uint32_t flag = 0;
	...
	crx_info_print_table_row(2, "PCRE (Perl Compatible Regular Expressions) Support", "enabled" );
	crx_info_print_table_row(2, "PCRE Library Version", version);
	free(version);
	crx_info_print_table_row(2, "PCRE Unicode Version", unicode);
	free(unicode);

#ifdef HAVE_PCRE_JIT_SUPPORT
	if (!pcre2_config(PCRE2_CONFIG_JIT, &flag)) {
		crx_info_print_table_row(2, "PCRE JIT Support", flag ? "enabled" : "disabled");
	} else {
		crx_info_print_table_row(2, "PCRE JIT Support", "unknown" );
	}
	if (jit_target) {
		crx_info_print_table_row(2, "PCRE JIT Target", jit_target);
	}
	free(jit_target);
#else
	crx_info_print_table_row(2, "PCRE JIT Support", "not compiled in" );
#endif

#ifdef HAVE_PCRE_VALGRIND_SUPPORT
	crx_info_print_table_row(2, "PCRE Valgrind Support", "enabled" );
#endif
...
/* {{{ CRX_MINIT_FUNCTION(pcre) */
static CRX_MINIT_FUNCTION(pcre)
{
#ifdef HAVE_PCRE_JIT_SUPPORT
	if (UNEXPECTED(!pcre2_init_ok)) {
		/* Retry. */
		crx_pcre_init_pcre2(PCRE_G(jit));
		if (!pcre2_init_ok) {
			return FAILURE;
		}
	}
	...
	crx_pcre_version = _pcre2_config_str(PCRE2_CONFIG_VERSION);

	register_crx_pcre_symbols(module_number);

	return SUCCESS;
}
/* }}} */

/* {{{ CRX_MSHUTDOWN_FUNCTION(pcre) */
static CRX_MSHUTDOWN_FUNCTION(pcre)
{
	UNREGISTER_INI_ENTRIES();

	free(crx_pcre_version);

	return SUCCESS;
}
/* }}} */

/* {{{ CRX_RINIT_FUNCTION(pcre) */
static CRX_RINIT_FUNCTION(pcre)
{
#ifdef HAVE_PCRE_JIT_SUPPORT
	if (UNEXPECTED(!pcre2_init_ok)) {
		/* Retry. */
	...
	PCRE_G(error_code) = CRX_PCRE_NO_ERROR;
	PCRE_G(gctx_zmm) = pcre2_general_context_create(crx_pcre_emalloc, crx_pcre_efree, NULL);
	if (!PCRE_G(gctx_zmm)) {
		return FAILURE;
	}

	if (PCRE_G(per_request_cache)) {
		crex_hash_init(&PCRE_G(pcre_cache), 0, NULL, crx_efree_pcre_cache, 0);
	}
...
static CRX_RSHUTDOWN_FUNCTION(pcre)
{
	pcre2_general_context_free(PCRE_G(gctx_zmm));
	PCRE_G(gctx_zmm) = NULL;

	if (PCRE_G(per_request_cache)) {
		crex_hash_destroy(&PCRE_G(pcre_cache));
	}
...
static void free_subpats_table(crex_string **subpat_names, uint32_t num_subpats) {
	uint32_t i;
	for (i = 0; i < num_subpats; i++) {
		if (subpat_names[i]) {
			crex_string_release(subpat_names[i]);
		}
	}
	efree(subpat_names);
}
...
static crex_string **make_subpats_table(uint32_t num_subpats, pcre_cache_entry *pce)
{
	uint32_t name_cnt = pce->name_count, name_size, ni = 0;
	char *name_table;
	crex_string **subpat_names;
	int rc1, rc2;
	...
	if (rc1 < 0 || rc2 < 0) {
		crx_error_docref(NULL, E_WARNING, "Internal pcre2_pattern_info() error %d", rc1 < 0 ? rc1 : rc2);
		return NULL;
	}

	subpat_names = ecalloc(num_subpats, sizeof(crex_string *));
	while (ni++ < name_cnt) {
		unsigned short name_idx = 0x100 * (unsigned char)name_table[0] + (unsigned char)name_table[1];
		const char *name = name_table + 2;
		subpat_names[name_idx] = crex_string_init(name, strlen(name), 0);
		if (is_numeric_string(ZSTR_VAL(subpat_names[name_idx]), ZSTR_LEN(subpat_names[name_idx]), NULL, NULL, 0) > 0) {
			crx_error_docref(NULL, E_WARNING, "Numeric named subpatterns are not allowed");
...
static crex_always_inline size_t calculate_unit_length(pcre_cache_entry *pce, const char *start)
{
	size_t unit_len;
...
CRXAPI pcre_cache_entry* pcre_get_compiled_regex_cache_ex(crex_string *regex, int locale_aware)
{
	...
	zv = crex_hash_find(&PCRE_G(pcre_cache), key);
	if (zv) {
		if (key != regex) {
			crex_string_release_ex(key, 0);
		}
		return (pcre_cache_entry*)C_PTR_P(zv);
	}
	...
	if (p >= end_p) {
		...
		crx_error_docref(NULL, E_WARNING, "Empty regular expression");
	...
	if (isalnum((int)*(unsigned char *)&delimiter) || delimiter == '\\' || delimiter == '\0') {
		...
		crx_error_docref(NULL, E_WARNING, "Delimiter must not be alphanumeric, backslash, or NUL");
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		return NULL;
	}
	...
	if (pp >= end_p) {
		...
		if (start_delimiter == end_delimiter) {
			crx_error_docref(NULL,E_WARNING, "No ending delimiter '%c' found", delimiter);
		} else {
			crx_error_docref(NULL,E_WARNING, "No ending matching delimiter '%c' found", delimiter);
		}
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		return NULL;
	}
	...
	/* In  PCRE,  by  default, \d, \D, \s, \S, \w, and \W recognize only ASCII
	   ...
	   the PCRE2_UCP option. */
			...
			default:
				if (pp[-1]) {
					crx_error_docref(NULL, E_WARNING, "Unknown modifier '%c'", pp[-1]);
				} else {
					crx_error_docref(NULL, E_WARNING, "NUL is not a valid modifier");
				}
				pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
				efree(pattern);
				if (key != regex) {
	...
	if (poptions & PREG_REPLACE_EVAL) {
		crx_error_docref(NULL, E_WARNING, "The /e modifier is no longer supported, use preg_replace_callback instead");
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		efree(pattern);
		if (key != regex) {
			crex_string_release_ex(key, 0);
	...
	if (key != regex) {
		tables = (uint8_t *)crex_hash_find_ptr(&char_tables, BG(ctype_string));
		if (!tables) {
			crex_string *_k;
			tables = pcre2_maketables(gctx);
			if (UNEXPECTED(!tables)) {
				crx_error_docref(NULL,E_WARNING, "Failed to generate locale character tables");
				pcre_handle_exec_error(PCRE2_ERROR_NOMEMORY);
				crex_string_release_ex(key, 0);
				efree(pattern);
				return NULL;
			}
			_k = crex_string_init(ZSTR_VAL(BG(ctype_string)), ZSTR_LEN(BG(ctype_string)), 1);
			GC_MAKE_PERSISTENT_LOCAL(_k);
			crex_hash_add_ptr(&char_tables, _k, (void *)tables);
			crex_string_release(_k);
		}
	}
	...
	if (re == NULL) {
		...
		crx_error_docref(NULL,E_WARNING, "Compilation failed: %s at offset %zu", error, erroffset);
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		efree(pattern);
		return NULL;
	}
	...
	if (PCRE_G(jit)) {
		...
		if (EXPECTED(rc >= 0)) {
			...
			crx_error_docref(NULL, E_WARNING,
				...
				"Either grant CRX permission to allocate executable memory, or set pcre.jit=0");
			PCRE_G(jit) = 0;
		} else {
			pcre2_get_error_message(rc, error, sizeof(error));
			crx_error_docref(NULL, E_WARNING, "JIT compilation failed: %s", error);
			pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		}
	}
#endif
	 ...
	 * ones).
	 */
	if (crex_hash_num_elements(&PCRE_G(pcre_cache)) == PCRE_CACHE_SIZE) {
		int num_clean = PCRE_CACHE_SIZE / 8;
		crex_hash_apply_with_argument(&PCRE_G(pcre_cache), pcre_clean_cache, &num_clean);
	}
	...
	if (rc < 0) {
		...
		crx_error_docref(NULL, E_WARNING, "Internal pcre2_pattern_info() error %d", rc);
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		return NULL;
	}
	...
	if (rc < 0) {
		...
		crx_error_docref(NULL, E_WARNING, "Internal pcre_pattern_info() error %d", rc);
		pcre_handle_exec_error(PCRE2_ERROR_INTERNAL);
		return NULL;
	}
	...
	if (!(GC_FLAGS(key) & IS_STR_PERMANENT) && !PCRE_G(per_request_cache)) {
		crex_string *str = crex_string_init(ZSTR_VAL(key), ZSTR_LEN(key), 1);
		GC_MAKE_PERSISTENT_LOCAL(str);

		ret = crex_hash_add_new_mem(&PCRE_G(pcre_cache), str, &new_entry, sizeof(pcre_cache_entry));
		crex_string_release(str);
	} else {
		ret = crex_hash_add_new_mem(&PCRE_G(pcre_cache), key, &new_entry, sizeof(pcre_cache_entry));
	}
...
CRXAPI pcre2_match_data *crx_pcre_create_match_data(uint32_t capture_count, pcre2_code *re)
{/*{{{*/
	...
	if (EXPECTED(!mdata_used)) {
		...
		if (rc >= 0 && capture_count + 1 <= CRX_PCRE_PREALLOC_MDATA_SIZE) {
			mdata_used = 1;
			return mdata;
		}
	}
...
CRXAPI void crx_pcre_free_match_data(pcre2_match_data *match_data)
{/*{{{*/
...
static void init_unmatched_null_pair(void) {
	...
	ZVAL_NULL(&val1);
	ZVAL_LONG(&val2, -1);
	ZVAL_ARR(&PCRE_G(unmatched_null_pair), crex_new_pair(&val1, &val2));
}
...
static void init_unmatched_empty_pair(void) {
	...
	ZVAL_LONG(&val2, -1);
	ZVAL_ARR(&PCRE_G(unmatched_empty_pair), crex_new_pair(&val1, &val2));
}

static crex_always_inline void populate_match_value_str(
...
static inline void add_named(
		zval *subpats, crex_string *name, zval *val, bool unmatched) {
	...
	if (!unmatched) {
		crex_hash_update(C_ARRVAL_P(subpats), name, val);
	} else {
		if (!crex_hash_add(C_ARRVAL_P(subpats), name, val)) {
			return;
		}
	}
	C_TRY_ADDREF_P(val);
...
static inline void add_offset_pair(
		...
		crex_string *name, uint32_t unmatched_as_null)
{
	zval match_pair;
	...
	if (PCRE2_UNSET == start_offset) {
		if (unmatched_as_null) {
			if (C_ISUNDEF(PCRE_G(unmatched_null_pair))) {
				init_unmatched_null_pair();
			}
			ZVAL_COPY(&match_pair, &PCRE_G(unmatched_null_pair));
		} else {
			if (C_ISUNDEF(PCRE_G(unmatched_empty_pair))) {
		...
		ZVAL_ARR(&match_pair, crex_new_pair(&val1, &val2));
	}

	if (name) {
		add_named(result, name, &match_pair, start_offset == PCRE2_UNSET);
	}
	crex_hash_next_index_insert(C_ARRVAL_P(result), &match_pair);
}
/* }}} */

static void populate_subpat_array(
		zval *subpats, const char *subject, PCRE2_SIZE *offsets, crex_string **subpat_names,
		uint32_t num_subpats, int count, const PCRE2_SPTR mark, crex_long flags) {
	...
	if (subpat_names) {
		if (offset_capture) {
			...
			for (i = 0; i < count; i++) {
				...
				crex_hash_next_index_insert(C_ARRVAL_P(subpats), &val);
			}
			if (unmatched_as_null) {
				for (i = count; i < num_subpats; i++) {
					ZVAL_NULL(&val);
					if (subpat_names[i]) {
						crex_hash_add(C_ARRVAL_P(subpats), subpat_names[i], &val);
					}
					crex_hash_next_index_insert(C_ARRVAL_P(subpats), &val);
				}
			}
...
static void crx_do_pcre_match(INTERNAL_FUNCTION_PARAMETERS, int global) /* {{{ */
{
	/* parameters */
	crex_string		 *regex;			/* Regular expression */
	crex_string		 *subject;			/* String to match against */
	pcre_cache_entry *pce;				/* Compiled regular expression */
	zval			 *subpats = NULL;	/* Array for subpatterns */
	crex_long		  flags = 0;		/* Match control flags */
	crex_long		  start_offset = 0;	/* Where the new search starts */

	CREX_PARSE_PARAMETERS_START(2, 5)
		C_PARAM_STR(regex)
		C_PARAM_STR(subject)
		C_PARAM_OPTIONAL
		C_PARAM_ZVAL(subpats)
		C_PARAM_LONG(flags)
		C_PARAM_LONG(start_offset)
	...
	pce->refcount++;
	crx_pcre_match_impl(pce, subject, return_value, subpats,
		global, CREX_NUM_ARGS() >= 4, flags, start_offset);
	pce->refcount--;
}
/* }}} */

static crex_always_inline bool is_known_valid_utf8(
		crex_string *subject_str, PCRE2_SIZE start_offset) {
	...
	zval *subpats, int global, int use_flags, crex_long flags, crex_off_t start_offset)
{
	...
	crex_string	   **subpat_names;		/* Array for named subpatterns */
	size_t			 i;
	...
	if (subpats != NULL) {
		subpats = crex_try_array_init(subpats);
		if (!subpats) {
			RETURN_THROWS();
		}
	}
	...
	if (use_flags) {
		...
		if ((global && (subpats_order < PREG_PATTERN_ORDER || subpats_order > PREG_SET_ORDER)) ||
			...
			crex_argument_value_error(4, "must be a PREG_* constant");
			RETURN_THROWS();
		}
	} else {
		offset_capture = 0;
	...
	if (!mdata_used && num_subpats <= CRX_PCRE_PREALLOC_MDATA_SIZE) {
		match_data = mdata;
	} else {
		match_data = pcre2_match_data_create_from_pattern(pce->re, PCRE_G(gctx_zmm));
		if (!match_data) {
			PCRE_G(error_code) = CRX_PCRE_INTERNAL_ERROR;
			if (subpat_names) {
				free_subpats_table(subpat_names, num_subpats);
			}
			if (match_sets) {
	...
	while (1) {
		...
		if (count >= 0) {
			...
			if (UNEXPECTED(count == 0)) {
				crx_error_docref(NULL, E_NOTICE, "Matched, but too many substrings");
				count = num_subpats;
			}
...
matched:
			...
			if (subpats != NULL) {
				...
				if (offsets[1] < offsets[0]) {
					...
					crx_error_docref(NULL, E_WARNING, "Get subpatterns list failed");
					RETURN_FALSE;
				}
					...
					if (subpats && subpats_order == PREG_PATTERN_ORDER) {
						...
						if (offset_capture) {
							...
							for (i = 0; i < count; i++) {
								...
								populate_match_value(
									&val, subject, offsets[2*i], offsets[2*i+1], unmatched_as_null);
								crex_hash_next_index_insert_new(C_ARRVAL(match_sets[i]), &val);
							}
						}
						mark = pcre2_get_mark(match_data);
						/* Add MARK, if available */
						if (mark) {
							if (C_TYPE(marks) == IS_UNDEF) {
								array_init(&marks);
							}
						...
						crex_hash_next_index_insert(C_ARRVAL_P(subpats), &result_set);
					}
...
error:
	...
	if (global && subpats && subpats_order == PREG_PATTERN_ORDER) {
		if (subpat_names) {
			for (i = 0; i < num_subpats; i++) {
				if (subpat_names[i]) {
					crex_hash_update(C_ARRVAL_P(subpats), subpat_names[i], &match_sets[i]);
					C_ADDREF(match_sets[i]);
				}
				crex_hash_next_index_insert(C_ARRVAL_P(subpats), &match_sets[i]);
			}
		} else {
			for (i = 0; i < num_subpats; i++) {
				crex_hash_next_index_insert(C_ARRVAL_P(subpats), &match_sets[i]);
	...
	if (PCRE_G(error_code) == CRX_PCRE_NO_ERROR) {
...
CRX_FUNCTION(preg_match)
{
	crx_do_pcre_match(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Perform a Perl-style global regular expression match */
CRX_FUNCTION(preg_match_all)
{
	crx_do_pcre_match(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
/* {{{ preg_do_repl_func */
static crex_string *preg_do_repl_func(crex_fcall_info *fci, crex_fcall_info_cache *fcc, const char *subject, PCRE2_SIZE *offsets, crex_string **subpat_names, uint32_t num_subpats, int count, const PCRE2_SPTR mark, crex_long flags)
{
	...
	if (crex_call_function(fci, fcc) == SUCCESS && C_TYPE(retval) != IS_UNDEF) {
		if (EXPECTED(C_TYPE(retval) == IS_STRING)) {
			result_str = C_STR(retval);
		} else {
		...
		if (!EG(exception)) {
			crx_error_docref(NULL, E_WARNING, "Unable to call custom replacement function");
		}

		result_str = crex_string_init(&subject[offsets[0]], offsets[1] - offsets[0], 0);
	}
...
/* {{{ crx_pcre_replace */
CRXAPI crex_string *crx_pcre_replace(crex_string *regex,
...
{
	pcre_cache_entry	*pce;			    /* Compiled regular expression */
	crex_string	 		*result;			/* Function result */
	...
	pce->refcount++;
	result = crx_pcre_replace_impl(pce, subject_str, subject, subject_len, replace_str,
		limit, replace_count);
	pce->refcount--;
...
/* {{{ crx_pcre_replace_impl() */
CRXAPI crex_string *crx_pcre_replace_impl(pcre_cache_entry *pce, crex_string *subject_str, const char *subject, size_t subject_len, crex_string *replace_str, size_t limit, size_t *replace_count)
{
	uint32_t		 options;			/* Execution options */
	int				 count;				/* Count of matched subpatterns */
	PCRE2_SIZE		*offsets;			/* Array of subpattern offsets */
	...
	crex_string		*result;			/* Result of replacement */
	pcre2_match_data *match_data;
...
matched:
			...
			if (new_len >= alloc_len) {
				alloc_len = crex_safe_address_guarded(2, new_len, ZSTR_MAX_OVERHEAD) - ZSTR_MAX_OVERHEAD;
				if (result == NULL) {
					result = crex_string_alloc(alloc_len, 0);
				} else {
					result = crex_string_extend(result, alloc_len, 0);
				}
			}
...
/* {{{ crx_pcre_replace_func_impl() */
static crex_string *crx_pcre_replace_func_impl(pcre_cache_entry *pce, crex_string *subject_str, const char *subject, size_t subject_len, crex_fcall_info *fci, crex_fcall_info_cache *fcc, size_t limit, size_t *replace_count, crex_long flags)
{
	uint32_t		 options;			/* Execution options */
	int				 count;				/* Count of matched subpatterns */
	PCRE2_SIZE		*offsets;			/* Array of subpattern offsets */
	...
	size_t			result_len; 		/* Length of result */
	crex_string		*result;			/* Result of replacement */
	crex_string     *eval_result;		/* Result of custom function */
	pcre2_match_data *match_data;
	bool old_mdata_used;
	...
	if (!old_mdata_used && num_subpats <= CRX_PCRE_PREALLOC_MDATA_SIZE) {
		mdata_used = 1;
		match_data = mdata;
	} else {
		match_data = pcre2_match_data_create_from_pattern(pce->re, PCRE_G(gctx_zmm));
...
matched:
			...
			CREX_ASSERT(eval_result);
			new_len = crex_safe_address_guarded(1, ZSTR_LEN(eval_result) + ZSTR_MAX_OVERHEAD, new_len) -ZSTR_MAX_OVERHEAD;
			if (new_len >= alloc_len) {
				alloc_len = crex_safe_address_guarded(2, new_len, ZSTR_MAX_OVERHEAD) - ZSTR_MAX_OVERHEAD;
				if (result == NULL) {
					result = crex_string_alloc(alloc_len, 0);
			...
			crex_string_release_ex(eval_result, 0);
...
/* {{{ crx_pcre_replace_func */
static crex_always_inline crex_string *crx_pcre_replace_func(crex_string *regex,
							  crex_string *subject_str,
							  crex_fcall_info *fci, crex_fcall_info_cache *fcc,
							  size_t limit, size_t *replace_count, crex_long flags)
{
	pcre_cache_entry	*pce;			    /* Compiled regular expression */
	crex_string	 		*result;			/* Function result */
	...
	pce->refcount++;
	result = crx_pcre_replace_func_impl(
...
/* {{{ crx_pcre_replace_array */
static crex_string *crx_pcre_replace_array(HashTable *regex,
	crex_string *replace_str, HashTable *replace_ht,
	crex_string *subject_str, size_t limit, size_t *replace_count)
{
	zval		*regex_entry;
	crex_string *result;
	...
	if (replace_ht) {
		...
		CREX_HASH_FOREACH_VAL(regex, regex_entry) {
			/* Make sure we're dealing with strings. */
			crex_string *tmp_regex_str;
			crex_string *regex_str = zval_get_tmp_string(regex_entry, &tmp_regex_str);
			crex_string *replace_entry_str, *tmp_replace_entry_str;
			zval *zv;
			...
			while (1) {
				...
				zv = CREX_HASH_ELEMENT(replace_ht, replace_idx);
				replace_idx++;
			...
			result = crx_pcre_replace(regex_str, subject_str, ZSTR_VAL(subject_str),
				ZSTR_LEN(subject_str), replace_entry_str, limit, replace_count);
			crex_tmp_string_release(tmp_replace_entry_str);
			crex_tmp_string_release(tmp_regex_str);
...
/* {{{ crx_replace_in_subject */
static crex_always_inline crex_string *crx_replace_in_subject(
	crex_string *regex_str, HashTable *regex_ht,
	crex_string *replace_str, HashTable *replace_ht,
	crex_string *subject, size_t limit, size_t *replace_count)
{
	crex_string *result;

	if (regex_str) {
		CREX_ASSERT(replace_str != NULL);
		result = crx_pcre_replace(regex_str, subject, ZSTR_VAL(subject), ZSTR_LEN(subject),
			replace_str, limit, replace_count);
	} else {
		CREX_ASSERT(regex_ht != NULL);
		result = crx_pcre_replace_array(regex_ht, replace_str, replace_ht, subject,
			limit, replace_count);
	}
	return result;
}
...
/* {{{ crx_replace_in_subject_func */
static crex_string *crx_replace_in_subject_func(crex_string *regex_str, HashTable *regex_ht,
	crex_fcall_info *fci, crex_fcall_info_cache *fcc,
	crex_string *subject, size_t limit, size_t *replace_count, crex_long flags)
{
	crex_string *result;

	if (regex_str) {
		result = crx_pcre_replace_func(
		...
		crex_string_addref(subject);

		/* For each entry in the regex array, get the entry */
		CREX_HASH_FOREACH_VAL(regex_ht, regex_entry) {
			/* Make sure we're dealing with strings. */
			crex_string *tmp_regex_entry_str;
			crex_string *regex_entry_str = zval_get_tmp_string(regex_entry, &tmp_regex_entry_str);

			/* Do the actual replacement and put the result back into subject
			   for further replacements. */
			result = crx_pcre_replace_func(
				regex_entry_str, subject, fci, fcc, limit, replace_count, flags);
			crex_tmp_string_release(tmp_regex_entry_str);
...
static size_t preg_replace_func_impl(zval *return_value,
	crex_string *regex_str, HashTable *regex_ht,
	crex_fcall_info *fci, crex_fcall_info_cache *fcc,
	crex_string *subject_str, HashTable *subject_ht, crex_long limit_val, crex_long flags)
{
	crex_string	*result;
	size_t replace_count = 0;

	if (subject_str) {
		result = crx_replace_in_subject_func(
		...
		CREX_ASSERT(subject_ht != NULL);

		array_init_size(return_value, crex_hash_num_elements(subject_ht));

		/* For each subject entry, convert it to string, then perform replacement
		   and add the result to the return_value array. */
		CREX_HASH_FOREACH_KEY_VAL(subject_ht, num_key, string_key, subject_entry) {
			crex_string *tmp_subject_entry_str;
			crex_string *subject_entry_str = zval_get_tmp_string(subject_entry, &tmp_subject_entry_str);
			...
			if (result != NULL) {
				...
				ZVAL_STR(&zv, result);
				if (string_key) {
					crex_hash_add_new(C_ARRVAL_P(return_value), string_key, &zv);
				} else {
					crex_hash_index_add_new(C_ARRVAL_P(return_value), num_key, &zv);
				}
			}
			crex_tmp_string_release(tmp_subject_entry_str);
		} CREX_HASH_FOREACH_END();
	}
...
static void preg_replace_common(INTERNAL_FUNCTION_PARAMETERS, bool is_filter)
{
	...
	CREX_PARSE_PARAMETERS_START(3, 5)
		C_PARAM_ARRAY_HT_OR_STR(regex_ht, regex_str)
	...
	if (replace_ht && !regex_ht) {
		crex_argument_type_error(1, "must be of type array when argument #2 ($replacement) is an array, string given");
		RETURN_THROWS();
	}

	if (subject_str) {
		old_replace_count = replace_count;
		result = crx_replace_in_subject(regex_str, regex_ht, replace_str, replace_ht,
	...
	if (zcount) {
		CREX_TRY_ASSIGN_REF_LONG(zcount, replace_count);
	}
}
/* }}} */

/* {{{ Perform Perl-style regular expression replacement. */
CRX_FUNCTION(preg_replace)
{
...
CRX_FUNCTION(preg_replace_callback)
{
	zval *zcount = NULL;
	crex_string *regex_str;
	HashTable *regex_ht;
	crex_string *subject_str;
	HashTable *subject_ht;
	crex_long limit = -1, flags = 0;
	size_t replace_count;
	crex_fcall_info fci;
	crex_fcall_info_cache fcc;

	/* Get function parameters and do error-checking. */
	CREX_PARSE_PARAMETERS_START(3, 6)
		C_PARAM_ARRAY_HT_OR_STR(regex_ht, regex_str)
...
/* {{{ Perform Perl-style regular expression replacement using replacement callback. */
CRX_FUNCTION(preg_replace_callback_array)
{
	zval zv, *replace, *zcount = NULL;
	HashTable *pattern, *subject_ht;
	crex_string *subject_str, *str_idx_regex;
	crex_long limit = -1, flags = 0;
	size_t replace_count = 0;
	crex_fcall_info fci;
	crex_fcall_info_cache fcc;
	...
	CREX_HASH_FOREACH_STR_KEY_VAL(pattern, str_idx_regex, replace) {
		if (!crex_is_callable_ex(replace, NULL, 0, NULL, &fcc, NULL)) {
			crex_argument_type_error(1, "must contain only valid callbacks");
			goto error;
		}
		if (!str_idx_regex) {
			crex_argument_type_error(1, "must contain only string patterns as keys");
			goto error;
		}
		...
		switch (C_TYPE(zv)) {
			case IS_ARRAY:
				CREX_ASSERT(subject_ht);
				crex_array_release(subject_ht);
				subject_ht = C_ARR(zv);
				break;
			case IS_STRING:
				CREX_ASSERT(subject_str);
				crex_string_release(subject_str);
				subject_str = C_STR(zv);
				break;
			case IS_NULL:
				RETVAL_NULL();
				goto error;
...
CRX_FUNCTION(preg_split)
{
	crex_string			*regex;			/* Regular expression */
	crex_string			*subject;		/* String to match against */
	crex_long			 limit_val = -1;/* Integer value of limit */
	crex_long			 flags = 0;		/* Match control flags */
	pcre_cache_entry	*pce;			/* Compiled regular expression */
	...
	pce->refcount++;
	crx_pcre_split_impl(pce, subject, return_value, limit_val, flags);
	pce->refcount--;
}
/* }}} */
...
CRX_FUNCTION(preg_quote)
{
	crex_string *str;       		/* Input string argument */
	crex_string	*delim = NULL;		/* Additional delimiter argument */
	char		*in_str;			/* Input string */
	char		*in_str_end;    	/* End of the input string */
	crex_string	*out_str;			/* Output string with quoted characters */
	...
	out_str = crex_string_safe_alloc(1, ZSTR_LEN(str), extra_len, 0);
	q = ZSTR_VAL(out_str);
	p = in_str;
...
CRX_FUNCTION(preg_grep)
{
	...
	pce->refcount++;
	crx_pcre_grep_impl(pce, input, return_value, flags);
	pce->refcount--;
}
/* }}} */

CRXAPI void  crx_pcre_grep_impl(pcre_cache_entry *pce, zval *input, zval *return_value, crex_long flags) /* {{{ */
{
	...
	CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(input), num_key, string_key, entry) {
		crex_string *tmp_subject_str;
		crex_string *subject_str = zval_get_tmp_string(entry, &tmp_subject_str);
		...
		if (count >= 0) {
			...
			if (!invert) {
				...
				/* Add to return array */
				if (string_key) {
					crex_hash_update(C_ARRVAL_P(return_value), string_key, entry);
				} else {
					crex_hash_index_update(C_ARRVAL_P(return_value), num_key, entry);
				}
			}
		} else if (count == PCRE2_ERROR_NOMATCH) {
			if (invert) {
...
CRX_FUNCTION(preg_last_error_msg)
{
	CREX_PARSE_PARAMETERS_NONE();

	RETURN_STRING(crx_pcre_get_error_msg(PCRE_G(error_code)));
}
/* }}} */

/* {{{ module definition structures */

crex_module_entry pcre_module_entry = {
...
CRXAPI void crx_pcre_pce_incref(pcre_cache_entry *pce)
{/*{{{*/
	assert(NULL != pce);
	pce->refcount++;
}/*}}}*/

CRXAPI void crx_pcre_pce_decref(pcre_cache_entry *pce)
{/*{{{*/
	assert(NULL != pce);
...
CRXAPI pcre2_code *crx_pcre_pce_re(pcre_cache_entry *pce)
{/*{{{*/
	assert(NULL != pce);
	return pce->re;
}/*}}}*/
