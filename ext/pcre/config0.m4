...
if test "$CRX_EXTERNAL_PCRE" != "no"; then
  ...
  CRX_NEW_EXTENSION(pcre, crx_pcre.c, no,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)
  CRX_INSTALL_HEADERS([ext/pcre], [crx_pcre.h])
else
  ...
  CRX_PCRE_CFLAGS="-Wno-implicit-fallthrough -DHAVE_CONFIG_H -I@ext_srcdir@/pcre2lib -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1"
  ...
  if test "$CRX_PCRE_JIT" != "no"; then
    ...
    AC_CACHE_CHECK([whether Intel CET is enabled], ac_cv_have_pcre2_intel_cet, [
      ...
      if test "$ac_cv_have_pcre2_intel_cet" = yes; then
        CRX_PCRE_CFLAGS="-mshstk $CRX_PCRE_CFLAGS"
      fi
    ])
  ...
  CRX_NEW_EXTENSION(pcre, $pcrelib_sources crx_pcre.c, no,,$CRX_PCRE_CFLAGS)
  CRX_ADD_BUILD_DIR($ext_builddir/pcre2lib)
  CRX_INSTALL_HEADERS([ext/pcre], [crx_pcre.h pcre2lib/])

  if test "$CRX_VALGRIND" != "no" && test "$have_valgrind" = "yes"; then
