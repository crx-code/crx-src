...
MBSTRING_API SAPI_TREAT_DATA_FUNC(mbstr_treat_data)
{
	...
	if (!MBSTRG(encoding_translation)) {
		crx_default_treat_data(arg, str, destArray);
		return;
	}
	...
	detected = _crx_mb_encoding_handler_ex(&info, &v_array, res);
...
/* {{{ mbfl_no_encoding _crx_mb_encoding_handler_ex() */
const mbfl_encoding *_crx_mb_encoding_handler_ex(const crx_mb_encoding_handler_info_t *info, zval *array_ptr, char *res)
{
	char *var, *val;
	...
	n = 0;
	var = crx_strtok_r(res, info->separator, &strtok_buf);
	while (var)  {
		val = strchr(var, '=');
		if (val) { /* have a value */
			len_list[n] = crx_url_decode(var, val-var);
			val_list[n] = var;
			n++;

			*val++ = '\0';
			val_list[n] = val;
			len_list[n] = crx_url_decode(val, strlen(val));
		} else {
			len_list[n] = crx_url_decode(var, strlen(var));
			val_list[n] = var;
			n++;
		...
		n++;
		var = crx_strtok_r(NULL, info->separator, &strtok_buf);
	}

	if (CREX_SIZE_T_GT_CREX_LONG(n, (PG(max_input_vars) * 2))) {
	...
	while (n < num) {
		if (from_encoding != &mbfl_encoding_pass && info->to_encoding != &mbfl_encoding_pass) {
			...
			crex_string *converted_var = mb_fast_convert((unsigned char*)val_list[n], len_list[n], from_encoding, info->to_encoding, MBSTRG(current_filter_illegal_substchar), MBSTRG(current_filter_illegal_mode), &num_errors);
			MBSTRG(illegalchars) += num_errors;
			n++;

			num_errors = 0;
			crex_string *converted_val = mb_fast_convert((unsigned char*)val_list[n], len_list[n], from_encoding, info->to_encoding, MBSTRG(current_filter_illegal_substchar), MBSTRG(current_filter_illegal_mode), &num_errors);
			MBSTRG(illegalchars) += num_errors;
			n++;
			...
			if (sapi_module.input_filter(info->data_type, ZSTR_VAL(converted_var), &val, ZSTR_LEN(converted_val), &new_val_len)) {
				...
				crx_register_variable_safe(ZSTR_VAL(converted_var), val, new_val_len, array_ptr);
			}
			crex_string_free(converted_var);
			crex_string_free(converted_val);
		} else {
			var = val_list[n++];
...
SAPI_POST_HANDLER_FUNC(crx_mb_post_handler)
{
	const mbfl_encoding *detected;
	crx_mb_encoding_handler_info_t info;
	crex_string *post_data_str = NULL;
	...
	post_data_str = crx_stream_copy_to_mem(SG(request_info).request_body, CRX_STREAM_COPY_ALL, 0);
	detected = _crx_mb_encoding_handler_ex(&info, arg, post_data_str ? ZSTR_VAL(post_data_str) : NULL);
	if (post_data_str) {
		crex_string_release_ex(post_data_str, 0);
	}
