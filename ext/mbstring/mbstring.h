...
#define CRX_MBSTRING_VERSION CRX_VERSION
...
#define CRX_MBSTRING_API 20021024

extern crex_module_entry mbstring_module_entry;
#define crxext_mbstring_ptr &mbstring_module_entry
...
MBSTRING_API crex_string* crx_mb_convert_encoding_ex(
		const char *input, size_t length,
		const mbfl_encoding *to_encoding, const mbfl_encoding *from_encoding);
MBSTRING_API crex_string* crx_mb_convert_encoding(
...
CREX_BEGIN_MODULE_GLOBALS(mbstring)
...
#ifdef HAVE_MBREGEX
    struct _crex_mb_regex_globals *mb_regex_globals;
    crex_long regex_stack_limit;
#endif
	crex_string *last_used_encoding_name;
	...
	bool http_input_set;
#ifdef HAVE_MBREGEX
    crex_long regex_retry_limit;
#endif
CREX_END_MODULE_GLOBALS(mbstring)

#define MBSTRG(v) CREX_MODULE_GLOBALS_ACCESSOR(mbstring, v)
