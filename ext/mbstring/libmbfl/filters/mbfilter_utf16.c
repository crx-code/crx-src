 ...
 * by moriyoshi koizumi <moriyoshi@crx.net> on 4 dec 2002.
 *
 */
...
CREX_INTRIN_AVX2_FUNC_DECL(size_t mb_utf16be_to_wchar_avx2(unsigned char **in, size_t *in_len, uint32_t *buf, size_t bufsize, unsigned int *state));
CREX_INTRIN_AVX2_FUNC_DECL(void mb_wchar_to_utf16be_avx2(uint32_t *in, size_t len, mb_convert_buf *buf, bool end));
CREX_INTRIN_AVX2_FUNC_DECL(size_t mb_utf16le_to_wchar_avx2(unsigned char **in, size_t *in_len, uint32_t *buf, size_t bufsize, unsigned int *state));
CREX_INTRIN_AVX2_FUNC_DECL(void mb_wchar_to_utf16le_avx2(uint32_t *in, size_t len, mb_convert_buf *buf, bool end));
...
{
	return crex_cpu_supports_avx2() ? mb_utf16be_to_wchar_avx2 : mb_utf16be_to_wchar_default;
}
...
{
	return crex_cpu_supports_avx2() ? mb_wchar_to_utf16be_avx2 : mb_wchar_to_utf16be_default;
}
...
{
	return crex_cpu_supports_avx2() ? mb_utf16le_to_wchar_avx2 : mb_utf16le_to_wchar_default;
}
...
{
	return crex_cpu_supports_avx2() ? mb_wchar_to_utf16le_avx2 : mb_wchar_to_utf16le_default;
}
	...
	if (len >= 32 && bufsize >= 16) {
		...
		do {
			...
			if (surrogate_bitvec == 0) {
				...
				uint8_t n_chars = crex_ulong_ntz(surrogate_bitvec) >> 1;
				operand = _mm256_shuffle_epi8(operand, swap_bytes);
				...
				unsigned int n_chars = surrogate_bitvec ? crex_ulong_ntz(surrogate_bitvec) >> 1 : 16;
				do {
					unsigned char c1 = *p++;
					unsigned char c2 = *p++;
					...
					if ((c3 & 0xFC) == 0xDC) {
						...
						*out++ = (((n & 0x3FF) << 10) | (n2 & 0x3FF)) + 0x10000;
						bufsize--;
						len -= 4;
#if defined(CRX_HAVE_BUILTIN_USUB_OVERFLOW) && CRX_HAVE_BUILTIN_USUB_OVERFLOW
	...
	if (len >= 8) {
		...
		do {
			...
			if (bmp_bitvec == 0xFFFFFFFF) {
				...
				unsigned int n_bytes = crex_ulong_ntz(~bmp_bitvec);
				operand = _mm256_shuffle_epi8(operand, pack_8x16);
				__m256i operand2 = _mm256_permute2x128_si256(operand, operand, 1);
				operand = _mm256_alignr_epi8(operand2, operand, 8);
				...
				unsigned int n_words = bmp_bitvec ? crex_ulong_ntz(bmp_bitvec) >> 2 : 8;
				do {
					uint32_t w = *in++;
					n_words--;
					len--;
