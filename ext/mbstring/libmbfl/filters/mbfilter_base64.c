 ...
 * by Moriyoshi Koizumi <moriyoshi@crx.net> on 4 Dec 2002. The file
...
static void mb_wchar_to_base64(uint32_t *in, size_t len, mb_convert_buf *buf, bool end)
{
	 ...
	 * Use `crex_safe_address_guarded` to check that the multiplication doesn't overflow
	 *
	...
	MB_CONVERT_BUF_ENSURE(buf, out, limit, (crex_safe_address_guarded(len + (bits / 8), 26, 52) / 19) + 2);
