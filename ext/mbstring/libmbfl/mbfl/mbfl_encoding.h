 ...
 * by Moriyoshi Koizumi <moriyoshi@crx.net> on 20 Dec 2002. The file
...
static inline crex_string* mb_convert_buf_result_raw(mb_convert_buf *buf)
{
	CREX_ASSERT(buf->out <= buf->limit);
	crex_string *ret = buf->str;
	/* See `crex_string_alloc` in crex_string.h */
...
static inline crex_string* mb_convert_buf_result(mb_convert_buf *buf, const mbfl_encoding *enc)
{
	crex_string *ret = mb_convert_buf_result_raw(buf);
