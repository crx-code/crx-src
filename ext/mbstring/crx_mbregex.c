...
#include "crx_onig_compat.h" /* must come prior to the oniguruma header */
#include <oniguruma.h>
#undef UChar
...
CREX_EXTERN_MODULE_GLOBALS(mbstring)

char crx_mb_oniguruma_version[256];

struct _crex_mb_regex_globals {
...
/* {{{ static void crx_mb_regex_free_cache() */
static void crx_mb_regex_free_cache(zval *el) {
	onig_free((crx_mb_regex_t *)C_PTR_P(el));
}
/* }}} */

/* {{{ _crx_mb_regex_globals_ctor */
static int _crx_mb_regex_globals_ctor(crex_mb_regex_globals *pglobals)
{
	...
	pglobals->search_re = (crx_mb_regex_t*)NULL;
...
/* {{{ crx_mb_regex_globals_alloc */
crex_mb_regex_globals *crx_mb_regex_globals_alloc(void)
{
	crex_mb_regex_globals *pglobals = pemalloc(
			sizeof(crex_mb_regex_globals), 1);
	if (SUCCESS != _crx_mb_regex_globals_ctor(pglobals)) {
		pefree(pglobals, 1);
		return NULL;
	}
	return pglobals;
...
CRX_MINIT_FUNCTION(mb_regex)
{
	...
	snprintf(crx_mb_oniguruma_version, sizeof(crx_mb_oniguruma_version), "%d.%d.%d",
...
CRX_RINIT_FUNCTION(mb_regex)
{
	...
	crex_hash_init(&MBREX(ht_rc), 0, NULL, crx_mb_regex_free_cache, 0);
	return SUCCESS;
}
/* }}} */
...
CRX_RSHUTDOWN_FUNCTION(mb_regex)
{
	...
	MBREX(current_mbctype_mbfl_encoding) = mbfl_name2encoding(crx_mb_regex_get_default_mbctype());

	if (!C_ISUNDEF(MBREX(search_str))) {
...
CRX_MINFO_FUNCTION(mb_regex)
{
	...
	crx_info_print_table_row(2, "Multibyte (japanese) regex support", "enabled");
	snprintf(buf, sizeof(buf), "%d.%d.%d",
			ONIGURUMA_VERSION_MAJOR,
			ONIGURUMA_VERSION_MINOR,
			ONIGURUMA_VERSION_TEENY);
	crx_info_print_table_row(2, "Multibyte regex (oniguruma) version", buf);
...
/* {{{ encoding name map */
typedef struct _crx_mb_regex_enc_name_map_t {
	const char *names;
	OnigEncoding code;
} crx_mb_regex_enc_name_map_t;

static const crx_mb_regex_enc_name_map_t enc_name_map[] = {
...
#endif
#if defined(ONIG_ENCODING_KOI8) && !CRX_ONIG_BAD_KOI8_ENTRY
	{
		"KOI8\0KOI-8\0",
		ONIG_ENCODING_KOI8
	},
...
/* {{{ crx_mb_regex_name2mbctype */
static OnigEncoding _crx_mb_regex_name2mbctype(const char *pname)
{
	const char *p;
	const crx_mb_regex_enc_name_map_t *mapping;
...
/* {{{ crx_mb_regex_mbctype2name */
static const char *_crx_mb_regex_mbctype2name(OnigEncoding mbctype)
{
	const crx_mb_regex_enc_name_map_t *mapping;
...
/* {{{ crx_mb_regex_set_mbctype */
int crx_mb_regex_set_mbctype(const char *encname)
{
	OnigEncoding mbctype = _crx_mb_regex_name2mbctype(encname);
...
/* {{{ crx_mb_regex_set_default_mbctype */
...
/* {{{ crx_mb_regex_get_mbctype */
const char *crx_mb_regex_get_mbctype(void)
{
	return _crx_mb_regex_mbctype2name(MBREX(current_mbctype));
}
/* }}} */

/* {{{ crx_mb_regex_get_mbctype_encoding */
const mbfl_encoding *crx_mb_regex_get_mbctype_encoding(void)
{
	return MBREX(current_mbctype_mbfl_encoding);
}
/* }}} */

/* {{{ crx_mb_regex_get_default_mbctype */
const char *crx_mb_regex_get_default_mbctype(void)
{
	return _crx_mb_regex_mbctype2name(MBREX(default_mbctype));
}
/* }}} */
 ...
 * regex cache
 */
/* {{{ crx_mbregex_compile_pattern */
static crx_mb_regex_t *crx_mbregex_compile_pattern(const char *pattern, size_t patlen, OnigOptionType options, OnigSyntaxType *syntax)
{
	int err_code = 0;
	crx_mb_regex_t *retval = NULL, *rc = NULL;
	OnigErrorInfo err_info;
	OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
	OnigEncoding enc = MBREX(current_mbctype);

	if (!crx_mb_check_encoding(pattern, patlen, crx_mb_regex_get_mbctype_encoding())) {
		crx_error_docref(NULL, E_WARNING,
			"Pattern is not valid under %s encoding", _crx_mb_regex_mbctype2name(enc));
		return NULL;
	}

	rc = crex_hash_str_find_ptr(&MBREX(ht_rc), (char *)pattern, patlen);
	if (!rc || onig_get_options(rc) != options || onig_get_encoding(rc) != enc || onig_get_syntax(rc) != syntax) {
		if ((err_code = onig_new(&retval, (OnigUChar *)pattern, (OnigUChar *)(pattern + patlen), options, enc, syntax, &err_info)) != ONIG_NORMAL) {
			onig_error_code_to_str(err_str, err_code, &err_info);
			crx_error_docref(NULL, E_WARNING, "mbregex compile err: %s", err_str);
			return NULL;
		}
		...
		crex_hash_str_update_ptr(&MBREX(ht_rc), (char *)pattern, patlen, retval);
	} else {
		retval = rc;
	}
	return retval;
...
/* {{{ _crx_mb_regex_get_option_string */
static size_t _crx_mb_regex_get_option_string(char *str, size_t len, OnigOptionType option, OnigSyntaxType *syntax)
{
	size_t len_left = len;
	size_t len_req = 0;
	char *p = str;
...
/* {{{ _crx_mb_regex_init_options */
static bool _crx_mb_regex_init_options(const char *parg, size_t narg, OnigOptionType *option,
			...
			switch (c) {
				...
				case 'd':
					*syntax = ONIG_SYNTAX_POSIX_EXTENDED;
					break;
				default:
					crex_value_error("Option \"%c\" is not supported", c);
					return false;
			}
		}
...
/*
 * Helper for _crx_mb_regex_ereg_replace_exec
 */
/* {{{ mb_regex_substitute */
static inline void mb_regex_substitute(
	...
	while (p < eos) {
		clen = (int) crx_mb_mbchar_bytes(p, enc);
		if (clen != 1 || p == eos || p[0] != '\\') {
		...
		switch (p[0]) {
			...
			case 'k':
			{
				...
				name_end = name = p + 1;
				while (name_end < eos) {
					clen = (int) crx_mb_mbchar_bytes(name_end, enc);
					if (clen != 1) {
						name_end += clen;
						maybe_num = 0;
						continue;
...
CRX_FUNCTION(mb_regex_encoding)
{
	char *encoding = NULL;
	size_t encoding_len;
	...
	if (!encoding) {
		const char *retval = crx_mb_regex_get_mbctype();
		CREX_ASSERT(retval != NULL);

		RETURN_STRING(retval);
	} else {
		if (crx_mb_regex_set_mbctype(encoding) == FAILURE) {
			crex_argument_value_error(1, "must be a valid encoding, \"%s\" given", encoding);
			RETURN_THROWS();
		}
...
/* {{{ _crx_mb_onig_search */
static int _crx_mb_onig_search(regex_t* reg, const OnigUChar* str, const OnigUChar* end, const OnigUChar* start,
                   const OnigUChar* range, OnigRegion* region, OnigOptionType option) {
	OnigMatchParam *mp = onig_new_match_param();
	int err;
	onig_initialize_match_param(mp);
	if (!CREX_LONG_UINT_OVFL(MBSTRG(regex_stack_limit))) {
		onig_set_match_stack_limit_size_of_match_param(mp, (unsigned int)MBSTRG(regex_stack_limit));
	}
	if (!CREX_LONG_UINT_OVFL(MBSTRG(regex_retry_limit))) {
...
/* {{{ _crx_mb_regex_ereg_exec */
static void _crx_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAMETERS, int icase)
{
	zval *array = NULL;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss|z", &arg_pattern, &arg_pattern_len, &string, &string_len, &array) == FAILURE) {
		RETURN_THROWS();
	}
	...
	re = crx_mbregex_compile_pattern(arg_pattern, arg_pattern_len, options, MBREX(regex_default_syntax));
	if (re == NULL) {
		RETVAL_FALSE;
		goto out;
	}
	...
	if (_crx_mb_onig_search(re, (OnigUChar *)string, (OnigUChar *)(string + string_len), (OnigUChar *)string, (OnigUChar *)(string + string_len), regs, 0) < 0) {
		RETVAL_FALSE;
		goto out;
	}
...
CRX_FUNCTION(mb_ereg)
{
	_crx_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Case-insensitive regular expression match for multibyte string */
CRX_FUNCTION(mb_eregi)
{
	_crx_mb_regex_ereg_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ _crx_mb_regex_ereg_replace_exec */
static void _crx_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAMETERS, OnigOptionType options, int is_callable)
{
	char *arg_pattern;
	...
	crex_fcall_info arg_replace_fci;
	crex_fcall_info_cache arg_replace_fci_cache;
	...
	const mbfl_encoding *enc = crx_mb_regex_get_mbctype_encoding();
	CREX_ASSERT(enc != NULL);

	{
		...
		if (!is_callable) {
			if (crex_parse_parameters(CREX_NUM_ARGS(), "sss|s!",
			...
			if (crex_parse_parameters(CREX_NUM_ARGS(), "sfs|s!",
						&arg_pattern, &arg_pattern_len,
		...
		if (!crx_mb_check_encoding(string, string_len, enc)) {
			RETURN_NULL();
		}

		if (option_str != NULL) {
			/* Initialize option and in case of failure it means there is a value error */
			if (!_crx_mb_regex_init_options(option_str, option_str_len, &options, &syntax)) {
				RETURN_THROWS();
			}
		} else {
			options |= MBREX(regex_default_options);
	...
	re = crx_mbregex_compile_pattern(arg_pattern, arg_pattern_len, options, syntax);
	if (re == NULL) {
		RETURN_FALSE;
	}
	...
	if (is_callable) {
		pbuf = &eval_buf;
		description = crex_make_compiled_string_description("mbregex replace");
	} else {
		pbuf = &out_buf;
		description = NULL;
	}
	...
	regs = onig_region_new();
	while (err >= 0) {
		err = _crx_mb_onig_search(re, (OnigUChar *)string, (OnigUChar *)string_lim, pos, (OnigUChar *)string_lim, regs, 0);
		if (err <= -2) {
			OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
			onig_error_code_to_str(err_str, err);
			crx_error_docref(NULL, E_WARNING, "mbregex search failure in crx_mbereg_replace_exec(): %s", err_str);
			break;
		}
		if (err >= 0) {
			...
			if (is_callable) {
				...
				if (crex_call_function(&arg_replace_fci, &arg_replace_fci_cache) == SUCCESS &&
						!C_ISUNDEF(retval)) {
					convert_to_string(&retval);
					smart_str_appendl(&out_buf, C_STRVAL(retval), C_STRLEN(retval));
					smart_str_free(&eval_buf);
					zval_ptr_dtor(&retval);
				} else {
					if (!EG(exception)) {
						crex_throw_error(NULL, "Unable to call custom replacement function");
...
CRX_FUNCTION(mb_ereg_replace)
{
	_crx_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 0);
}
/* }}} */

/* {{{ Case insensitive replace regular expression for multibyte string */
CRX_FUNCTION(mb_eregi_replace)
{
	_crx_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, ONIG_OPTION_IGNORECASE, 0);
}
/* }}} */

/* {{{ regular expression for multibyte string using replacement callback */
CRX_FUNCTION(mb_ereg_replace_callback)
{
	_crx_mb_regex_ereg_replace_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0, 1);
}
/* }}} */

/* {{{ split multibyte string into array by regular expression */
CRX_FUNCTION(mb_split)
{
	char *arg_pattern;
	size_t arg_pattern_len;
	crx_mb_regex_t *re;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss|l", &arg_pattern, &arg_pattern_len, &string, &string_len, &count) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!crx_mb_check_encoding(string, string_len, crx_mb_regex_get_mbctype_encoding())) {
		RETURN_FALSE;
	}

	/* create regex pattern buffer */
	if ((re = crx_mbregex_compile_pattern(arg_pattern, arg_pattern_len, MBREX(regex_default_options), MBREX(regex_default_syntax))) == NULL) {
		RETURN_FALSE;
	}
	...
	while (count != 0 && (size_t)(pos - (OnigUChar *)string) < string_len) {
		size_t beg, end;
		err = _crx_mb_onig_search(re, (OnigUChar *)string, (OnigUChar *)(string + string_len), pos, (OnigUChar *)(string + string_len), regs, 0);
		if (err < 0) {
			break;
		}
	...
	if (err <= -2) {
		OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
		onig_error_code_to_str(err_str, err);
		crx_error_docref(NULL, E_WARNING, "mbregex search failure in mbsplit(): %s", err_str);
...
CRX_FUNCTION(mb_ereg_match)
{
...

	{
		...
		if (option_str != NULL) {
			if(!_crx_mb_regex_init_options(option_str, option_str_len, &option, &syntax)) {
				RETURN_THROWS();
			}
		} else {
	...
	if ((re = crx_mbregex_compile_pattern(arg_pattern, arg_pattern_len, option, syntax)) == NULL) {
		RETURN_FALSE;
	}
...
/* regex search */
/* {{{ _crx_mb_regex_ereg_search_exec */
static void _crx_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAMETERS, int mode)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|s!s!", &arg_pattern, &arg_pattern_len, &arg_options, &arg_options_len) == FAILURE) {
		RETURN_THROWS();
	}

	if (arg_options) {
		_crx_mb_regex_init_options(arg_options, arg_options_len, &option, &syntax);
	} else {
		option |= MBREX(regex_default_options);
		syntax = MBREX(regex_default_syntax);
	}
	...
	if (arg_pattern) {
		/* create regex pattern buffer */
		if ((MBREX(search_re) = crx_mbregex_compile_pattern(arg_pattern, arg_pattern_len, option, syntax)) == NULL) {
			RETURN_FALSE;
		}
	}
	...
	str = NULL;
	len = 0;
	if (!C_ISUNDEF(MBREX(search_str)) && C_TYPE(MBREX(search_str)) == IS_STRING){
		str = (OnigUChar *)C_STRVAL(MBREX(search_str));
		len = C_STRLEN(MBREX(search_str));
	}

	if (MBREX(search_re) == NULL) {
		crex_throw_error(NULL, "No pattern was provided");
		RETURN_THROWS();
	}

	if (str == NULL) {
		crex_throw_error(NULL, "No string was provided");
		RETURN_THROWS();
	}

	MBREX(search_regs) = onig_region_new();

	err = _crx_mb_onig_search(MBREX(search_re), str, str + len, str + pos, str  + len, MBREX(search_regs), 0);
	if (err == ONIG_MISMATCH) {
		...
		OnigUChar err_str[ONIG_MAX_ERROR_MESSAGE_LEN];
		onig_error_code_to_str(err_str, err);
		crx_error_docref(NULL, E_WARNING, "mbregex search failure in mbregex_search(): %s", err_str);
		RETVAL_FALSE;
	} else {
		switch (mode) {
		case 1:
		...
		case 2:
			...
			if (onig_number_of_names(MBREX(search_re)) > 0) {
				mb_regex_groups_iter_args args = {
					return_value,
					C_STRVAL(MBREX(search_str)),
					C_STRLEN(MBREX(search_str)),
					MBREX(search_regs)
				};
...
CRX_FUNCTION(mb_ereg_search)
{
	_crx_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Regular expression search for multibyte string */
CRX_FUNCTION(mb_ereg_search_pos)
{
	_crx_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */

/* {{{ Regular expression search for multibyte string */
CRX_FUNCTION(mb_ereg_search_regs)
{
	_crx_mb_regex_ereg_search_exec(INTERNAL_FUNCTION_PARAM_PASSTHRU, 2);
}
/* }}} */

/* {{{ Initialize string and regular expression for search. */
CRX_FUNCTION(mb_ereg_search_init)
{
	crex_string *arg_str;
	char *arg_pattern = NULL, *arg_options = NULL;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "S|s!s!", &arg_str, &arg_pattern, &arg_pattern_len, &arg_options, &arg_options_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!C_ISNULL(MBREX(search_str))) {
		zval_ptr_dtor(&MBREX(search_str));
	}

	ZVAL_STR_COPY(&MBREX(search_str), arg_str);

	if (crx_mb_check_encoding(ZSTR_VAL(arg_str), ZSTR_LEN(arg_str), crx_mb_regex_get_mbctype_encoding())) {
		MBREX(search_pos) = 0;
		RETVAL_TRUE;
	} else {
...
CRX_FUNCTION(mb_ereg_search_getregs)
{
	size_t n, i, len;
	/* Stored as int* in the OnigRegion struct */
	int beg, end;
	...
	if (MBREX(search_regs) != NULL && C_TYPE(MBREX(search_str)) == IS_STRING) {
		array_init(return_value);
...
CRX_FUNCTION(mb_ereg_search_getpos)
{
	if (crex_parse_parameters_none() == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(mb_ereg_search_setpos)
{
	crex_long position;
	...
	if ((position < 0) && (!C_ISUNDEF(MBREX(search_str))) && (C_TYPE(MBREX(search_str)) == IS_STRING)) {
		position += C_STRLEN(MBREX(search_str));
	}

	if (position < 0 || (!C_ISUNDEF(MBREX(search_str)) && C_TYPE(MBREX(search_str)) == IS_STRING && (size_t)position > C_STRLEN(MBREX(search_str)))) {
		crex_argument_value_error(1, "is out of range");
		RETURN_THROWS();
	}
...
/* {{{ crx_mb_regex_set_options */
static void _crx_mb_regex_set_options(OnigOptionType options, OnigSyntaxType *syntax, OnigOptionType *prev_options, OnigSyntaxType **prev_syntax)
{
...
CRX_FUNCTION(mb_regex_set_options)
{
	...
	if (string != NULL) {
		opt = 0;
		syntax = NULL;
		if(!_crx_mb_regex_init_options(string, string_len, &opt, &syntax)) {
			RETURN_THROWS();
		}
		_crx_mb_regex_set_options(opt, syntax, &prev_opt, &prev_syntax);
		opt = prev_opt;
		syntax = prev_syntax;
	} else {
	...
	_crx_mb_regex_get_option_string(buf, sizeof(buf), opt, syntax);
