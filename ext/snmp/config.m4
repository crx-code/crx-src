...
if test "$CRX_SNMP" != "no"; then

  if test "$CRX_SNMP" = "yes"; then
    AC_PATH_PROG(SNMP_CONFIG,net-snmp-config,,[/usr/local/bin:$PATH])
  else
    SNMP_CONFIG="$CRX_SNMP/bin/net-snmp-config"
  fi
  ...
  if test -x "$SNMP_CONFIG"; then
    ...
    if test "$SNMP_VERSION" -ge "5003"; then
      if test -n "$SNMP_LIBS" && test -n "$SNMP_PREFIX"; then
        CRX_ADD_INCLUDE(${SNMP_PREFIX}/include)
        CRX_EVAL_LIBLINE($SNMP_LIBS, SNMP_SHARED_LIBADD)
  ...
  dnl Test build.
  CRX_CHECK_LIBRARY($SNMP_LIBNAME, init_snmp,
  [
  ...
  CRX_CHECK_LIBRARY($SNMP_LIBNAME, shutdown_snmp_logging,
  [
  ...
  CRX_CHECK_LIBRARY($SNMP_LIBNAME, usmHMAC192SHA256AuthProtocol,
  [
  ...
  CRX_CHECK_LIBRARY($SNMP_LIBNAME, usmHMAC384SHA512AuthProtocol,
  [
  ...
  CRX_NEW_EXTENSION(snmp, snmp.c, $ext_shared)
  CRX_SUBST(SNMP_SHARED_LIBADD)
fi
