/*
  ...
  |          Daniel Lowrey <rdlowrey@crx.net>                            |
  |          Chris Wright <daverandom@crx.net>                           |
...
#define HAVE_SSL3 1
#define CRX_OPENSSL_MIN_PROTO_VERSION STREAM_CRYPTO_METHOD_SSLv3
#else
#define CRX_OPENSSL_MIN_PROTO_VERSION STREAM_CRYPTO_METHOD_TLSv1_0
#endif
#ifdef HAVE_TLS13
#define CRX_OPENSSL_MAX_PROTO_VERSION STREAM_CRYPTO_METHOD_TLSv1_3
#else
#define CRX_OPENSSL_MAX_PROTO_VERSION STREAM_CRYPTO_METHOD_TLSv1_2
#endif

/* Simplify ssl context option retrieval */
#define GET_VER_OPT(_name) \
	(CRX_STREAM_CONTEXT(stream) && (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", _name)) != NULL)
#define GET_VER_OPT_STRING(_name, _str) \
	do { \
		if (GET_VER_OPT(_name)) { \
			if (try_convert_to_string(val)) _str = C_STRVAL_P(val); \
		} \
	} while (0)
...
#define CRX_X509_NAME_ENTRY_TO_UTF8(ne, i, out) \
...
static RSA *crx_openssl_tmp_rsa_cb(SSL *s, int is_export, int keylength);
#endif

extern crx_stream* crx_openssl_get_stream_from_ssl_handle(const SSL *ssl);
extern crex_string* crx_openssl_x509_fingerprint(X509 *peer, const char *method, bool raw);
extern int crx_openssl_get_ssl_stream_data_index(void);
static struct timeval crx_openssl_subtract_timeval(struct timeval a, struct timeval b);
static int crx_openssl_compare_timeval(struct timeval a, struct timeval b);
static ssize_t crx_openssl_sockop_io(int read, crx_stream *stream, char *buf, size_t count);

const crx_stream_ops crx_openssl_socket_ops;

/* Certificate contexts used for server-side SNI selection */
typedef struct _crx_openssl_sni_cert_t {
	char *name;
	SSL_CTX *ctx;
} crx_openssl_sni_cert_t;

/* Provides leaky bucket handhsake renegotiation rate-limiting  */
typedef struct _crx_openssl_handshake_bucket_t {
	crex_long prev_handshake;
	crex_long limit;
	crex_long window;
	float tokens;
	unsigned should_close;
} crx_openssl_handshake_bucket_t;

#ifdef HAVE_TLS_ALPN
/* Holds the available server ALPN protocols for negotiation */
typedef struct _crx_openssl_alpn_ctx_t {
	unsigned char *data;
	unsigned short len;
} crx_openssl_alpn_ctx;
#endif
 ...
 * */
typedef struct _crx_openssl_netstream_data_t {
	crx_netstream_data_t s;
	SSL *ssl_handle;
	SSL_CTX *ctx;
	...
	crx_openssl_handshake_bucket_t *reneg;
	crx_openssl_sni_cert_t *sni_certs;
	unsigned sni_cert_count;
#ifdef HAVE_TLS_ALPN
	crx_openssl_alpn_ctx alpn_ctx;
#endif
	char *url_name;
	unsigned state_set:1;
	unsigned _spare:31;
} crx_openssl_netstream_data_t;
...
static int crx_openssl_is_http_stream_talking_to_iis(crx_stream *stream) /* {{{ */
{
...
#define SERVER_GOOGLE "Server: GFE/"
		...
		CREX_HASH_FOREACH_VAL(C_ARRVAL(stream->wrapperdata), tmp) {
			if (crex_string_equals_literal_ci(C_STR_P(tmp), SERVER_MICROSOFT_IIS)) {
				return 1;
			} else if (crex_string_equals_literal_ci(C_STR_P(tmp), SERVER_GOOGLE)) {
				return 1;
			}
...
static int crx_openssl_handle_ssl_error(crx_stream *stream, int nr_bytes, bool is_init) /* {{{ */
{
	crx_openssl_netstream_data_t *sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
	...
	switch(err) {
		...
		case SSL_ERROR_SYSCALL:
			if (ERR_peek_error() == 0) {
				if (nr_bytes == 0) {
					if (!crx_openssl_is_http_stream_talking_to_iis(stream) && ERR_get_error() != 0) {
						crx_error_docref(NULL, E_WARNING, "SSL: fatal protocol error");
					}
					SSL_set_shutdown(sslsock->ssl_handle, SSL_SENT_SHUTDOWN|SSL_RECEIVED_SHUTDOWN);
					stream->eof = 1;
					retry = 0;
				} else {
					char *estr = crx_socket_strerror(crx_socket_errno(), NULL, 0);
...
static int verify_callback(int preverify_ok, X509_STORE_CTX *ctx) /* {{{ */
{
	...
	int err, depth, ret;
	zval *val;
	crex_ulong allowed_depth = OPENSSL_DEFAULT_STREAM_VERIFY_DEPTH;
	...
	stream = (crx_stream*)SSL_get_ex_data(ssl, crx_openssl_get_ssl_stream_data_index());
	...
	if ((crex_ulong)depth > allowed_depth) {
		ret = 0;
...
static int crx_openssl_x509_fingerprint_cmp(X509 *peer, const char *method, const char *expected)
{
	crex_string *fingerprint;
	int result = -1;

	fingerprint = crx_openssl_x509_fingerprint(peer, method, 0);
	if (fingerprint) {
		result = strcasecmp(expected, ZSTR_VAL(fingerprint));
		crex_string_release_ex(fingerprint, 0);
	}

	return result;
}

static bool crx_openssl_x509_fingerprint_match(X509 *peer, zval *val)
{
	if (C_TYPE_P(val) == IS_STRING) {
		const char *method = NULL;

		switch (C_STRLEN_P(val)) {
			case 32:
				method = "md5";
				break;
		...
		return method && crx_openssl_x509_fingerprint_cmp(peer, method, C_STRVAL_P(val)) == 0;
	} else if (C_TYPE_P(val) == IS_ARRAY) {
		zval *current;
		crex_string *key;

		if (!crex_hash_num_elements(C_ARRVAL_P(val))) {
			crx_error_docref(NULL, E_WARNING, "Invalid peer_fingerprint array; [algo => fingerprint] form required");
			return 0;
		}

		CREX_HASH_FOREACH_STR_KEY_VAL(C_ARRVAL_P(val), key, current) {
			if (key == NULL || C_TYPE_P(current) != IS_STRING) {
				crx_error_docref(NULL, E_WARNING, "Invalid peer_fingerprint array; [algo => fingerprint] form required");
				return 0;
			}
			if (crx_openssl_x509_fingerprint_cmp(peer, ZSTR_VAL(key), C_STRVAL_P(current)) != 0) {
				return 0;
			}
		} CREX_HASH_FOREACH_END();
...
static bool crx_openssl_matches_wildcard_name(const char *subjectname, const char *certname) /* {{{ */
{
	char *wildcard = NULL;
...
static bool crx_openssl_matches_san_list(X509 *peer, const char *subject_name) /* {{{ */
{
	int i, len;
	...
	for (i = 0; i < alt_name_count; i++) {
		...
		if (san->type == GEN_DNS) {
			...
			if (crx_openssl_matches_wildcard_name(subject_name, (const char *)cert_name)) {
				OPENSSL_free(cert_name);
...
static bool crx_openssl_matches_common_name(X509 *peer, const char *subject_name) /* {{{ */
{
	char buf[1024];
	X509_NAME *cert_name;
	bool is_match = 0;
	...
	if (cert_name_len == -1) {
		crx_error_docref(NULL, E_WARNING, "Unable to locate peer certificate CN");
	} else if ((size_t)cert_name_len != strlen(buf)) {
		crx_error_docref(NULL, E_WARNING, "Peer certificate CN=`%.*s' is malformed", cert_name_len, buf);
	} else if (crx_openssl_matches_wildcard_name(subject_name, buf)) {
		is_match = 1;
	} else {
		crx_error_docref(NULL, E_WARNING,
...
static crex_result crx_openssl_apply_peer_verification_policy(SSL *ssl, X509 *peer, crx_stream *stream) /* {{{ */
{
	zval *val = NULL;
	...
	if ((must_verify_peer || must_verify_peer_name || must_verify_fingerprint) && peer == NULL) {
		crx_error_docref(NULL, E_WARNING, "Could not get peer certificate");
		return FAILURE;
	}
	...
	if (must_verify_peer) {
		...
		switch (err) {
			...
			case X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT:
				if (GET_VER_OPT("allow_self_signed") && crex_is_true(val)) {
					/* allowed */
					break;
				}
	...
	if (must_verify_fingerprint) {
		if (C_TYPE_P(peer_fingerprint) == IS_STRING || C_TYPE_P(peer_fingerprint) == IS_ARRAY) {
			if (!crx_openssl_x509_fingerprint_match(peer, peer_fingerprint)) {
				crx_error_docref(NULL, E_WARNING,
	...
	if (must_verify_peer_name) {
		...
		if (peer_name) {
			if (crx_openssl_matches_san_list(peer, peer_name)) {
				return SUCCESS;
			} else if (crx_openssl_matches_common_name(peer, peer_name)) {
				return SUCCESS;
			} else {
				return FAILURE;
			}
...
static int crx_openssl_passwd_callback(char *buf, int num, int verify, void *data) /* {{{ */
{
	crx_stream *stream = (crx_stream *)data;
	zval *val = NULL;
	char *passphrase = NULL;
	/* TODO: could expand this to make a callback into CRX user-space */

	GET_VER_OPT_STRING("passphrase", passphrase);

	if (passphrase) {
		if (C_STRLEN_P(val) < (size_t)num - 1) {
			memcpy(buf, C_STRVAL_P(val), C_STRLEN_P(val)+1);
			return (int)C_STRLEN_P(val);
		}
	}
	return 0;
}
...
static int crx_openssl_win_cert_verify_callback(X509_STORE_CTX *x509_store_ctx, void *arg) /* {{{ */
{
	...
	crx_stream *stream;
	crx_openssl_netstream_data_t *sslsock;
	zval *val;
	bool is_self_signed = 0;


	stream = (crx_stream*)arg;
	sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
	...
	{ /* First convert the x509 struct back to a DER encoded buffer and let Windows decode it into a form it can work with */
		...
		if (der_len < 0) {
			...
			crx_error_docref(NULL, E_WARNING, "Error encoding X509 certificate: %d: %s", err_code, ERR_error_string(err_code, err_buf));
		...
		if (cert_ctx == NULL) {
			...
			crx_error_docref(NULL, E_WARNING, "Error creating certificate context: %s", err);
	...
	{ /* Next fetch the relevant cert chain from the store */
		...
		if (!CertGetCertificateChain(NULL, cert_ctx, NULL, NULL, &chain_params, chain_flags, NULL, &cert_chain_ctx)) {
			char *err = crx_win_err();
			crx_error_docref(NULL, E_WARNING, "Error getting certificate chain: %s", err);
			crx_win_err_free(err);
	...
	{ /* Then verify it against a policy */
		...
		if (!verify_result) {
			char *err = crx_win_err();
			crx_error_docref(NULL, E_WARNING, "Error verifying certificate chain policy: %s", err);
			crx_win_err_free(err);
			RETURN_CERT_VERIFY_FAILURE(SSL_R_CERTIFICATE_VERIFY_FAILED);
		}
		...
		if (chain_policy_status.dwError != 0) {
			...
			if (is_self_signed && chain_policy_status.dwError == CERT_E_UNTRUSTEDROOT
				&& GET_VER_OPT("allow_self_signed") && crex_is_true(val)) {
...
static long crx_openssl_load_stream_cafile(X509_STORE *cert_store, const char *cafile) /* {{{ */
{
	crx_stream *stream;
	X509 *cert;
	BIO *buffer;
	...
	stream = crx_stream_open_wrapper(cafile, "rb", 0, NULL);

	if (stream == NULL) {
		crx_error(E_WARNING, "failed loading cafile stream: `%s'", cafile);
		return 0;
	} else if (stream->wrapper->is_url) {
		crx_stream_close(stream);
		crx_error(E_WARNING, "remote cafile streams are disabled for security purposes");
		return 0;
	}

	cert_start: {
		line = crx_stream_get_line(stream, NULL, 0, &line_len);
		if (line == NULL) {
	...
	if (certs_added == 0) {
		crx_error(E_WARNING, "no valid certs found cafile stream: `%s'", cafile);
	}
...
static crex_result crx_openssl_enable_peer_verification(SSL_CTX *ctx, crx_stream *stream) /* {{{ */
{
	zval *val = NULL;
	char *cafile = NULL;
	char *capath = NULL;
	...
	if (cafile == NULL) {
		cafile = crex_ini_string("openssl.cafile", sizeof("openssl.cafile")-1, 0);
		...
		if (cert_names != NULL) {
			...
			crx_error(E_WARNING, "SSL: failed loading CA names from cafile");
			return FAILURE;
		}
	}

	if (capath == NULL) {
		capath = crex_ini_string("openssl.capath", sizeof("openssl.capath")-1, 0);
	...
	if (cafile || capath) {
		if (!SSL_CTX_load_verify_locations(ctx, cafile, capath)) {
			ERR_clear_error();
			if (cafile && !crx_openssl_load_stream_cafile(SSL_CTX_get_cert_store(ctx), cafile)) {
				return FAILURE;
			}
		}
	} else {
#ifdef CRX_WIN32
		SSL_CTX_set_cert_verify_callback(ctx, crx_openssl_win_cert_verify_callback, (void *)stream);
#else
...
static void crx_openssl_disable_peer_verification(SSL_CTX *ctx, crx_stream *stream) /* {{{ */
{
	SSL_CTX_set_verify(ctx, SSL_VERIFY_NONE, NULL);
}
/* }}} */

static crex_result crx_openssl_set_local_cert(SSL_CTX *ctx, crx_stream *stream) /* {{{ */
{
	zval *val = NULL;
	char *certfile = NULL;
	size_t certfile_len;
	...
	if (certfile) {
		...
		if (!crx_openssl_check_path_ex(
				certfile, certfile_len, resolved_path_buff, 0, false, false,
				"local_cert in ssl stream context")) {
			crx_error_docref(NULL, E_WARNING, "Unable to get real path of certificate file `%s'", certfile);
			return FAILURE;
		}
		...
		if (private_key && !crx_openssl_check_path_ex(
				private_key, private_key_len, resolved_path_buff, 0, false, false,
				"local_pk in ssl stream context")) {
			crx_error_docref(NULL, E_WARNING, "Unable to get real path of private key file `%s'", private_key);
			return FAILURE;
		}
		if (SSL_CTX_use_PrivateKey_file(ctx, resolved_path_buff, SSL_FILETYPE_PEM) != 1) {
			crx_error_docref(NULL, E_WARNING, "Unable to set private key file `%s'", resolved_path_buff);
			return FAILURE;
		}
		if (!SSL_CTX_check_private_key(ctx)) {
			crx_error_docref(NULL, E_WARNING, "Private key does not match certificate!");
		}
	}
...
#if CRX_OPENSSL_API_VERSION < 0x10100
static int crx_openssl_get_crypto_method_ctx_flags(int method_flags) /* {{{ */
{
...
static inline int crx_openssl_get_min_proto_version_flag(int flags) /* {{{ */
{
	int ver;
	for (ver = CRX_OPENSSL_MIN_PROTO_VERSION; ver <= CRX_OPENSSL_MAX_PROTO_VERSION; ver <<= 1) {
		if (flags & ver) {
			return ver;
		}
	}
	return CRX_OPENSSL_MAX_PROTO_VERSION;
}
/* }}} */

static inline int crx_openssl_get_max_proto_version_flag(int flags) /* {{{ */
{
	int ver;
	for (ver = CRX_OPENSSL_MAX_PROTO_VERSION; ver >= CRX_OPENSSL_MIN_PROTO_VERSION; ver >>= 1) {
		if (flags & ver) {
			return ver;
		}
	}
...
#if CRX_OPENSSL_API_VERSION >= 0x10100
static inline int crx_openssl_map_proto_version(int flag) /* {{{ */
{
	switch (flag) {
#ifdef HAVE_TLS13
...
static int crx_openssl_get_min_proto_version(int flags) /* {{{ */
{
	return crx_openssl_map_proto_version(crx_openssl_get_min_proto_version_flag(flags));
}
/* }}} */

static int crx_openssl_get_max_proto_version(int flags) /* {{{ */
{
	return crx_openssl_map_proto_version(crx_openssl_get_max_proto_version_flag(flags));
}
/* }}} */
#endif

static int crx_openssl_get_proto_version_flags(int flags, int min, int max) /* {{{ */
{
	int ver;

	if (!min) {
		min = crx_openssl_get_min_proto_version_flag(flags);
	}
	if (!max) {
		max = crx_openssl_get_max_proto_version_flag(flags);
	}
...
static void crx_openssl_limit_handshake_reneg(const SSL *ssl) /* {{{ */
{
	crx_stream *stream;
	crx_openssl_netstream_data_t *sslsock;
	struct timeval now;
	crex_long elapsed_time;

	stream = crx_openssl_get_stream_from_ssl_handle(ssl);
	sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
	...
	if (sslsock->reneg->tokens > sslsock->reneg->limit) {
		...
		if (CRX_STREAM_CONTEXT(stream) && (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream),
				"ssl", "reneg_limit_callback")) != NULL
		) {
			zval param, retval;

			crx_stream_to_zval(stream, &param);
			...
			if (FAILURE == call_user_function(NULL, NULL, val, &retval, 1, &param)) {
				crx_error(E_WARNING, "SSL: failed invoking reneg limit notification callback");
			}
			stream->flags ^= CRX_STREAM_FLAG_NO_FCLOSE;

			/* If the reneg_limit_callback returned true don't auto-close */
			if (C_TYPE(retval) == IS_TRUE) {
...
static void crx_openssl_info_callback(const SSL *ssl, int where, int ret) /* {{{ */
{
	/* Rate-limit client-initiated handshake renegotiation to prevent DoS */
	if (where & SSL_CB_HANDSHAKE_START) {
		crx_openssl_limit_handshake_reneg(ssl);
	}
}
/* }}} */

static void crx_openssl_init_server_reneg_limit(crx_stream *stream, crx_openssl_netstream_data_t *sslsock) /* {{{ */
{
	zval *val;
	crex_long limit = OPENSSL_DEFAULT_RENEG_LIMIT;
	crex_long window = OPENSSL_DEFAULT_RENEG_WINDOW;

	if (CRX_STREAM_CONTEXT(stream) &&
		NULL != (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "reneg_limit"))
	) {
	...
	if (CRX_STREAM_CONTEXT(stream) &&
		NULL != (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "reneg_window"))
	) {
		window = zval_get_long(val);
	}

	sslsock->reneg = (void*)pemalloc(sizeof(crx_openssl_handshake_bucket_t),
		crx_stream_is_persistent(stream)
	);
	...
	SSL_set_info_callback(sslsock->ssl_handle, crx_openssl_info_callback);
}
/* }}} */
...
static RSA *crx_openssl_tmp_rsa_cb(SSL *s, int is_export, int keylength)
{
	...
	if (!rsa_tmp && ((bn = BN_new()) == NULL)) {
		crx_error_docref(NULL, E_WARNING, "allocation error generating RSA key");
	}
	if (!rsa_tmp && bn) {
...
static crex_result crx_openssl_set_server_dh_param(crx_stream * stream, SSL_CTX *ctx) /* {{{ */
{
	zval *zdhpath = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "dh_param");
	if (zdhpath == NULL) {
#if 0
	...
	BIO *bio = BIO_new_file(C_STRVAL_P(zdhpath), CRX_OPENSSL_BIO_MODE_R(PKCS7_BINARY));

	if (bio == NULL) {
		crx_error_docref(NULL, E_WARNING, "Invalid dh_param");
		return FAILURE;
	}
	...
	if (pkey == NULL) {
		crx_error_docref(NULL, E_WARNING, "Failed reading DH params");
		return FAILURE;
	}

	if (SSL_CTX_set0_tmp_dh_pkey(ctx, pkey) == 0) {
		crx_error_docref(NULL, E_WARNING, "Failed assigning DH params");
		EVP_PKEY_free(pkey);
		return FAILURE;
	}
#else
...
#if defined(HAVE_ECDH) && CRX_OPENSSL_API_VERSION < 0x10100
static crex_result crx_openssl_set_server_ecdh_curve(crx_stream *stream, SSL_CTX *ctx) /* {{{ */
{
	zval *zvcurve;
	int curve_nid;
	EC_KEY *ecdh;

	zvcurve = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "ecdh_curve");
	if (zvcurve == NULL) {
		...
		curve_nid = OBJ_sn2nid(C_STRVAL_P(zvcurve));
		if (curve_nid == NID_undef) {
			crx_error_docref(NULL, E_WARNING, "Invalid ecdh_curve specified");
			return FAILURE;
		}
	}
	...
	if (ecdh == NULL) {
		crx_error_docref(NULL, E_WARNING, "Failed generating ECDH curve");
		return FAILURE;
	}
...
static crex_result crx_openssl_set_server_specific_opts(crx_stream *stream, SSL_CTX *ctx) /* {{{ */
{
	zval *zv;
	long ssl_ctx_options = SSL_CTX_get_options(ctx);

#if defined(HAVE_ECDH) && CRX_OPENSSL_API_VERSION < 0x10100
	if (crx_openssl_set_server_ecdh_curve(stream, ctx) == FAILURE) {
		return FAILURE;
	}
#endif

#if CRX_OPENSSL_API_VERSION < 0x10100
	SSL_CTX_set_tmp_rsa_callback(ctx, crx_openssl_tmp_rsa_cb);
#endif
	/* We now use crx_openssl_tmp_rsa_cb to generate a key of appropriate size whenever necessary */
	if (crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "rsa_key_size") != NULL) {
		crx_error_docref(NULL, E_WARNING, "rsa_key_size context option has been removed");
	}

	if (crx_openssl_set_server_dh_param(stream, ctx) == FAILURE) {
		return FAILURE;
	}

	zv = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "single_dh_use");
	if (zv == NULL || crex_is_true(zv)) {
		ssl_ctx_options |= SSL_OP_SINGLE_DH_USE;
	}

	zv = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream), "ssl", "honor_cipher_order");
	if (zv == NULL || crex_is_true(zv)) {
...
#ifdef HAVE_TLS_SNI
static int crx_openssl_server_sni_callback(SSL *ssl_handle, int *al, void *arg) /* {{{ */
{
	crx_stream *stream;
	crx_openssl_netstream_data_t *sslsock;
	unsigned i;
	...
	stream = (crx_stream*)SSL_get_ex_data(ssl_handle, crx_openssl_get_ssl_stream_data_index());
	sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
	...
	for (i=0; i < sslsock->sni_cert_count; i++) {
		if (crx_openssl_matches_wildcard_name(server_name, sslsock->sni_certs[i].name)) {
...
static SSL_CTX *crx_openssl_create_sni_server_ctx(char *cert_path, char *key_path)  /* {{{ */
{
...
static crex_result crx_openssl_enable_server_sni(crx_stream *stream, crx_openssl_netstream_data_t *sslsock)  /* {{{ */
{
	zval *val;
	zval *current;
	crex_string *key;
	crex_ulong key_index;
	int i = 0;
	char resolved_path_buff[MAXPATHLEN];
	SSL_CTX *ctx;

	/* If the stream ctx disables SNI we're finished here */
	if (GET_VER_OPT("SNI_enabled") && !crex_is_true(val)) {
		return SUCCESS;
	}
	...
	sslsock->sni_cert_count = crex_hash_num_elements(C_ARRVAL_P(val));
	...
	sslsock->sni_certs = (crx_openssl_sni_cert_t*)safe_pemalloc(sslsock->sni_cert_count,
		sizeof(crx_openssl_sni_cert_t), 0, crx_stream_is_persistent(stream)
	);
	memset(sslsock->sni_certs, 0, sslsock->sni_cert_count * sizeof(crx_openssl_sni_cert_t));

	CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(val), key_index, key, current) {
		(void) key_index;
		...
		if (C_TYPE_P(current) == IS_ARRAY) {
			zval *local_pk, *local_cert;
			crex_string *local_pk_str, *local_cert_str;
			char resolved_cert_path_buff[MAXPATHLEN], resolved_pk_path_buff[MAXPATHLEN];

			local_cert = crex_hash_str_find(C_ARRVAL_P(current), "local_cert", sizeof("local_cert")-1);
			if (local_cert == NULL) {
				crx_error_docref(NULL, E_WARNING,
			...
			if (!crx_openssl_check_path_str_ex(
				...
				crex_string_release(local_cert_str);
				return FAILURE;
			}
			crex_string_release(local_cert_str);

			local_pk = crex_hash_str_find(C_ARRVAL_P(current), "local_pk", sizeof("local_pk")-1);
			if (local_pk == NULL) {
				crx_error_docref(NULL, E_WARNING,
			...
			if (!crx_openssl_check_path_str_ex(
				...
				crex_string_release(local_pk_str);
				return FAILURE;
			}
			crex_string_release(local_pk_str);

			ctx = crx_openssl_create_sni_server_ctx(resolved_cert_path_buff, resolved_pk_path_buff);

		} else if (crx_openssl_check_path_str_ex(
				C_STR_P(current), resolved_path_buff, 0, false, false,
				"SNI_server_certs in ssl stream context")) {
			ctx = crx_openssl_create_sni_server_ctx(resolved_path_buff, resolved_path_buff);
		} else {
			crx_error_docref(NULL, E_WARNING,
		...
		sslsock->sni_certs[i].name = pestrdup(ZSTR_VAL(key), crx_stream_is_persistent(stream));
		sslsock->sni_certs[i].ctx = ctx;
		++i;

	} CREX_HASH_FOREACH_END();

	SSL_CTX_set_tlsext_servername_callback(sslsock->ctx, crx_openssl_server_sni_callback);

	return SUCCESS;
}
/* }}} */

static void crx_openssl_enable_client_sni(crx_stream *stream, crx_openssl_netstream_data_t *sslsock) /* {{{ */
{
	zval *val;
 ...
 */
static unsigned char *crx_openssl_alpn_protos_parse(unsigned short *outlen, const char *in) /* {{{ */
{
	size_t len;
	unsigned char *out;
	size_t i, start = 0;
...
static int crx_openssl_server_alpn_callback(SSL *ssl_handle,
		const unsigned char **out, unsigned char *outlen,
		const unsigned char *in, unsigned int inlen, void *arg) /* {{{ */
{
	crx_openssl_netstream_data_t *sslsock = arg;
...
crex_result crx_openssl_setup_crypto(crx_stream *stream,
		crx_openssl_netstream_data_t *sslsock,
		crx_stream_xport_crypto_param *cparam) /* {{{ */
{
	const SSL_METHOD *method;
	int ssl_ctx_options;
	int method_flags;
	crex_long min_version = 0;
	crex_long max_version = 0;
	...
	if (sslsock->ssl_handle) {
		if (sslsock->s.is_blocked) {
			crx_error_docref(NULL, E_WARNING, "SSL/TLS already set-up for this stream");
			return FAILURE;
		} else {
			return SUCCESS;
		}
	...
	if (sslsock->ctx == NULL) {
		crx_error_docref(NULL, E_WARNING, "SSL context creation failure");
		return FAILURE;
	}

	GET_VER_OPT_LONG("min_proto_version", min_version);
	GET_VER_OPT_LONG("max_proto_version", max_version);
	method_flags = crx_openssl_get_proto_version_flags(method_flags, min_version, max_version);
#if CRX_OPENSSL_API_VERSION < 0x10100
	ssl_ctx_options = crx_openssl_get_crypto_method_ctx_flags(method_flags);
#else
	ssl_ctx_options = SSL_OP_ALL;
#endif

	if (GET_VER_OPT("no_ticket") && crex_is_true(val)) {
	...
	if (!GET_VER_OPT("disable_compression") || crex_is_true(val)) {
		ssl_ctx_options |= SSL_OP_NO_COMPRESSION;
	}

	if (GET_VER_OPT("verify_peer") && !crex_is_true(val)) {
		crx_openssl_disable_peer_verification(sslsock->ctx, stream);
	} else if (FAILURE == crx_openssl_enable_peer_verification(sslsock->ctx, stream)) {
		return FAILURE;
	}
	...
	if (GET_VER_OPT("passphrase")) {
		...
		SSL_CTX_set_default_passwd_cb(sslsock->ctx, crx_openssl_passwd_callback);
	}
	...
	if (GET_VER_OPT("security_level")) {
		crex_long lval = zval_get_long(val);
		if (lval < 0 || lval > 5) {
			crx_error_docref(NULL, E_WARNING, "Security level must be between 0 and 5");
		}
	...
	if (alpn_protocols) {
#ifdef HAVE_TLS_ALPN
		{
			...
			unsigned char *alpn = crx_openssl_alpn_protos_parse(&alpn_len, alpn_protocols);

			if (alpn == NULL) {
				crx_error_docref(NULL, E_WARNING, "Failed parsing comma-separated TLS ALPN protocol string");
			...
			if (sslsock->is_client) {
				...
				sslsock->alpn_ctx.data = (unsigned char *) pestrndup((const char*)alpn, alpn_len, crx_stream_is_persistent(stream));
				sslsock->alpn_ctx.len = alpn_len;
				SSL_CTX_set_alpn_select_cb(sslsock->ctx, crx_openssl_server_alpn_callback, sslsock);
			}
...
#else
		crx_error_docref(NULL, E_WARNING,
			"alpn_protocols support is not compiled into the OpenSSL library against which CRX is linked");
#endif
	}

	if (FAILURE == crx_openssl_set_local_cert(sslsock->ctx, stream)) {
		return FAILURE;
	}
...
#if CRX_OPENSSL_API_VERSION >= 0x10100
	SSL_CTX_set_min_proto_version(sslsock->ctx, crx_openssl_get_min_proto_version(method_flags));
	SSL_CTX_set_max_proto_version(sslsock->ctx, crx_openssl_get_max_proto_version(method_flags));
#endif

	if (sslsock->is_client == 0 &&
		CRX_STREAM_CONTEXT(stream) &&
		FAILURE == crx_openssl_set_server_specific_opts(stream, sslsock->ctx)
	) {
		return FAILURE;
	}
	...
	if (sslsock->ssl_handle == NULL) {
		crx_error_docref(NULL, E_WARNING, "SSL handle creation failure");
		SSL_CTX_free(sslsock->ctx);
		sslsock->ctx = NULL;
#ifdef HAVE_TLS_ALPN
		if (sslsock->alpn_ctx.data) {
			pefree(sslsock->alpn_ctx.data, crx_stream_is_persistent(stream));
			sslsock->alpn_ctx.data = NULL;
		}
#endif
		return FAILURE;
	} else {
		SSL_set_ex_data(sslsock->ssl_handle, crx_openssl_get_ssl_stream_data_index(), stream);
	}

	if (!SSL_set_fd(sslsock->ssl_handle, sslsock->s.socket)) {
		crx_openssl_handle_ssl_error(stream, 0, 1);
	}

#ifdef HAVE_TLS_SNI
	/* Enable server-side SNI */
	if (!sslsock->is_client && crx_openssl_enable_server_sni(stream, sslsock) == FAILURE) {
		return FAILURE;
	}
#endif
	...
	if (!sslsock->is_client) {
		crx_openssl_init_server_reneg_limit(stream, sslsock);
	}
	...
	if (cparam->inputs.session) {
		if (cparam->inputs.session->ops != &crx_openssl_socket_ops) {
			crx_error_docref(NULL, E_WARNING, "Supplied session stream must be an SSL enabled stream");
		} else if (((crx_openssl_netstream_data_t*)cparam->inputs.session->abstract)->ssl_handle == NULL) {
			crx_error_docref(NULL, E_WARNING, "Supplied SSL session stream is not initialized");
		} else {
			SSL_copy_session_id(sslsock->ssl_handle, ((crx_openssl_netstream_data_t*)cparam->inputs.session->abstract)->ssl_handle);
		}
	}
...
static int crx_openssl_capture_peer_certs(crx_stream *stream,
		crx_openssl_netstream_data_t *sslsock, X509 *peer_cert) /* {{{ */
{
	zval *val, zcert;
	crx_openssl_certificate_object *cert_object;
	int cert_captured = 0;

	if (NULL != (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream),
			"ssl", "capture_peer_cert")) &&
		crex_is_true(val)
	) {
		object_init_ex(&zcert, crx_openssl_certificate_ce);
		cert_object = C_OPENSSL_CERTIFICATE_P(&zcert);
		cert_object->x509 = peer_cert;

		crx_stream_context_set_option(CRX_STREAM_CONTEXT(stream), "ssl", "peer_certificate", &zcert);
		zval_ptr_dtor(&zcert);
		cert_captured = 1;
	}
	...
	if (NULL != (val = crx_stream_context_get_option(CRX_STREAM_CONTEXT(stream),
		...
		crx_stream_context_set_option(CRX_STREAM_CONTEXT(stream), "ssl", "peer_certificate_chain", &arr);
		zval_ptr_dtor(&arr);
	}
...
static int crx_openssl_enable_crypto(crx_stream *stream,
		crx_openssl_netstream_data_t *sslsock,
		crx_stream_xport_crypto_param *cparam) /* {{{ */
{
	int n;
	...
	if (cparam->inputs.activate && !sslsock->ssl_active) {
...
#ifdef HAVE_TLS_SNI
		if (sslsock->is_client) {
			crx_openssl_enable_client_sni(stream, sslsock);
		}
#endif
		...
		if (SUCCESS == crx_set_sock_blocking(sslsock->s.socket, 0)) {
		...
		do {
			...
			if (has_timeout) {
				...
				elapsed_time = crx_openssl_subtract_timeval(cur_time, start_time);

				if (crx_openssl_compare_timeval( elapsed_time, *timeout) > 0) {
					crx_error_docref(NULL, E_WARNING, "SSL: Handshake timed out");
					return -1;
				}
			}
			...
			if (n <= 0) {
				...
				retry = crx_openssl_handle_ssl_error(stream, n, blocked);
				if (retry) {
					...
					if (has_timeout) {
						left_time = crx_openssl_subtract_timeval(*timeout, elapsed_time);
					}
					crx_pollfd_for(sslsock->s.socket, (err == SSL_ERROR_WANT_READ) ?
		...
		if (sslsock->s.is_blocked != blocked && SUCCESS == crx_set_sock_blocking(sslsock->s.socket, blocked)) {
			sslsock->s.is_blocked = blocked;
		}

		if (n == 1) {
			peer_cert = SSL_get_peer_certificate(sslsock->ssl_handle);
			if (peer_cert && CRX_STREAM_CONTEXT(stream)) {
				cert_captured = crx_openssl_capture_peer_certs(stream, sslsock, peer_cert);
			}

			if (FAILURE == crx_openssl_apply_peer_verification_policy(sslsock->ssl_handle, peer_cert, stream)) {
...
static ssize_t crx_openssl_sockop_read(crx_stream *stream, char *buf, size_t count) /* {{{ */
{
	return crx_openssl_sockop_io( 1, stream, buf, count );
}
/* }}} */

static ssize_t crx_openssl_sockop_write(crx_stream *stream, const char *buf, size_t count) /* {{{ */
{
	return crx_openssl_sockop_io( 0, stream, (char*)buf, count );
}
/* }}} */
...
static ssize_t crx_openssl_sockop_io(int read, crx_stream *stream, char *buf, size_t count) /* {{{ */
{
	...
	if (sslsock->ssl_active) {
		...
		if (timeout && crx_set_sock_blocking(sslsock->s.socket, 0) == SUCCESS) {
			sslsock->s.is_blocked = 0;
		}
		...
		do {
			...
			if (has_timeout) {
				...
				if (crx_openssl_compare_timeval(elapsed_time, *timeout) > 0 ) {
					...
					if (began_blocked) {
						crx_set_sock_blocking(sslsock->s.socket, 1);
			...
			if (read) {
				...
				if (sslsock->reneg && sslsock->reneg->should_close) {
					...
					crx_stream_xport_shutdown(stream, (stream_shutdown_t)SHUT_RDWR);
					nr_bytes = 0;
					stream->eof = 1;
					 break;
				}
			...
			if (nr_bytes <= 0) {
				...
				int err = SSL_get_error(sslsock->ssl_handle, nr_bytes );
				retry = crx_openssl_handle_ssl_error(stream, nr_bytes, 0);
				 ...
				 * available. Now, crx_pollfd_for uses select to wait up to our time_left value only...
				 */
				if (retry) {
					if (read) {
						crx_pollfd_for(sslsock->s.socket, (err == SSL_ERROR_WANT_WRITE) ?
				...
				/* If we didn't get any error, then let's return it to CRX. */
		...
		/* Tell CRX if we read / wrote bytes. */
		if (nr_bytes > 0) {
			crx_stream_notify_progress_increment(CRX_STREAM_CONTEXT(stream), nr_bytes, 0);
		}

		/* And if we were originally supposed to be blocking, let's reset the socket to that. */
		if (began_blocked && crx_set_sock_blocking(sslsock->s.socket, 1) == SUCCESS) {
			sslsock->s.is_blocked = 1;
		}
		...
		if (read) {
			nr_bytes = crx_stream_socket_ops.read(stream, buf, count);
		} else {
			nr_bytes = crx_stream_socket_ops.write(stream, buf, count);
		}
...
static int crx_openssl_sockop_close(crx_stream *stream, int close_handle) /* {{{ */
{
	crx_openssl_netstream_data_t *sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
#ifdef CRX_WIN32
	int n;
	...
	if (close_handle) {
		...
		if (sslsock->s.socket != SOCK_ERR) {
			 ...
			 * */
			do {
				n = crx_pollfd_for_ms(sslsock->s.socket, POLLOUT, 500);
	...
	if (sslsock->sni_certs) {
		for (i = 0; i < sslsock->sni_cert_count; i++) {
			if (sslsock->sni_certs[i].ctx) {
				...
				pefree(sslsock->sni_certs[i].name, crx_stream_is_persistent(stream));
			}
		}
		pefree(sslsock->sni_certs, crx_stream_is_persistent(stream));
		sslsock->sni_certs = NULL;
	}

	if (sslsock->url_name) {
		pefree(sslsock->url_name, crx_stream_is_persistent(stream));
	}

	if (sslsock->reneg) {
		pefree(sslsock->reneg, crx_stream_is_persistent(stream));
	}

	pefree(sslsock, crx_stream_is_persistent(stream));

	return 0;
}
/* }}} */

static int crx_openssl_sockop_flush(crx_stream *stream) /* {{{ */
{
	return crx_stream_socket_ops.flush(stream);
}
/* }}} */

static int crx_openssl_sockop_stat(crx_stream *stream, crx_stream_statbuf *ssb) /* {{{ */
{
	return crx_stream_socket_ops.stat(stream, ssb);
}
/* }}} */

static inline int crx_openssl_tcp_sockop_accept(crx_stream *stream, crx_openssl_netstream_data_t *sock,
		crx_stream_xport_param *xparam STREAMS_DC)  /* {{{ */
{
	...
	clisock = crx_network_accept_incoming(sock->s.socket,
	...
	if (clisock >= 0) {
		crx_openssl_netstream_data_t *clisockdata = (crx_openssl_netstream_data_t*) emalloc(sizeof(*clisockdata));
		...
		if (xparam->outputs.client && sock->enable_on_connect) {
			...
			if (crx_stream_xport_crypto_setup(xparam->outputs.client, clisockdata->method,
					NULL) < 0 || crx_stream_xport_crypto_enable(
					xparam->outputs.client, 1) < 0) {
				crx_error_docref(NULL, E_WARNING, "Failed to enable crypto");

				crx_stream_close(xparam->outputs.client);
				xparam->outputs.client = NULL;
...
static int crx_openssl_sockop_set_option(crx_stream *stream, int option, int value, void *ptrparam)  /* {{{ */
{
	crx_openssl_netstream_data_t *sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
	crx_stream_xport_crypto_param *cparam = (crx_stream_xport_crypto_param *)ptrparam;
	crx_stream_xport_param *xparam = (crx_stream_xport_param *)ptrparam;
	...
	switch (option) {
		...
		case CRX_STREAM_OPTION_CHECK_LIVENESS:
			{
				...
				if (sslsock->s.socket == -1) {
					...
					crx_pollfd_for(sslsock->s.socket, CRX_POLLREADABLE|POLLPRI, &tv) > 0
				) {
					...
					} else if (0 == recv(sslsock->s.socket, &buf, sizeof(buf), MSG_PEEK|MSG_DONTWAIT) && crx_socket_errno() != EAGAIN) {
						alive = 0;
					}
				}
		...
		case CRX_STREAM_OPTION_CRYPTO_API:

			switch(cparam->op) {

				case STREAM_XPORT_CRYPTO_OP_SETUP:
					cparam->outputs.returncode = crx_openssl_setup_crypto(stream, sslsock, cparam);
					return CRX_STREAM_OPTION_RETURN_OK;
					break;
				case STREAM_XPORT_CRYPTO_OP_ENABLE:
					cparam->outputs.returncode = crx_openssl_enable_crypto(stream, sslsock, cparam);
					return CRX_STREAM_OPTION_RETURN_OK;
					break;
				default:
					/* fall through */
		...
		case CRX_STREAM_OPTION_XPORT_API:
			switch(xparam->op) {
				...
				case STREAM_XPORT_OP_CONNECT_ASYNC:
					...
					crx_stream_socket_ops.set_option(stream, option, value, ptrparam);
					...
					{
						if (crx_stream_xport_crypto_setup(stream, sslsock->method, NULL) < 0 ||
				...
				case STREAM_XPORT_OP_ACCEPT:
					 ...
					 * doesn't know about */
					xparam->outputs.returncode = crx_openssl_tcp_sockop_accept(stream, sslsock, xparam STREAMS_CC);
	...
	return crx_stream_socket_ops.set_option(stream, option, value, ptrparam);
}
/* }}} */

static int crx_openssl_sockop_cast(crx_stream *stream, int castas, void **ret)  /* {{{ */
{
	crx_openssl_netstream_data_t *sslsock = (crx_openssl_netstream_data_t*)stream->abstract;
			...
			if (ret) {
				...
				if (stream->writepos == stream->readpos
					...
					&& (pending = (size_t)SSL_pending(sslsock->ssl_handle)) > 0) {
						crx_stream_fill_read_buffer(stream, pending < stream->chunk_size
							? pending
							: stream->chunk_size);
				}

				*(crx_socket_t *)ret = sslsock->s.socket;
			}
			return SUCCESS;
...
const crx_stream_ops crx_openssl_socket_ops = {
	crx_openssl_sockop_write, crx_openssl_sockop_read,
	crx_openssl_sockop_close, crx_openssl_sockop_flush,
	"tcp_socket/ssl",
	NULL, /* seek */
	crx_openssl_sockop_cast,
	crx_openssl_sockop_stat,
	crx_openssl_sockop_set_option,
};

static crex_long crx_openssl_get_crypto_method(
		crx_stream_context *ctx, crex_long crypto_method)  /* {{{ */
{
	zval *val;

	if (ctx && (val = crx_stream_context_get_option(ctx, "ssl", "crypto_method")) != NULL) {
...
static char *crx_openssl_get_url_name(const char *resourcename,
		size_t resourcenamelen, int is_persistent)  /* {{{ */
{
	...
	url = crx_url_parse_ex(resourcename, resourcenamelen);
	if (!url) {
		return NULL;
	}
...
crx_stream *crx_openssl_ssl_socket_factory(const char *proto, size_t protolen,
		...
		crx_stream_context *context STREAMS_DC) /* {{{ */
{
	...
	crx_openssl_netstream_data_t *sslsock = NULL;

	sslsock = pemalloc(sizeof(crx_openssl_netstream_data_t), persistent_id ? 1 : 0);
	...
	stream = crx_stream_alloc_rel(&crx_openssl_socket_ops, sslsock, persistent_id, "r+");
	...
	if (strncmp(proto, "ssl", protolen) == 0) {
		...
		sslsock->method = crx_openssl_get_crypto_method(context, STREAM_CRYPTO_METHOD_TLS_ANY_CLIENT);
	} else if (strncmp(proto, "sslv2", protolen) == 0) {
		crx_error_docref(NULL, E_WARNING, "SSLv2 unavailable in this CRX version");
		crx_stream_close(stream);
		return NULL;
...
#else
		crx_error_docref(NULL, E_WARNING,
			"SSLv3 support is not compiled into the OpenSSL library against which CRX is linked");
		crx_stream_close(stream);
		return NULL;
#endif
...
#else
		crx_error_docref(NULL, E_WARNING,
			"TLSv1.1 support is not compiled into the OpenSSL library against which CRX is linked");
		crx_stream_close(stream);
		return NULL;
#endif
...
#else
		crx_error_docref(NULL, E_WARNING,
			"TLSv1.2 support is not compiled into the OpenSSL library against which CRX is linked");
		crx_stream_close(stream);
		return NULL;
#endif
...
#else
		crx_error_docref(NULL, E_WARNING,
			"TLSv1.3 support is not compiled into the OpenSSL library against which CRX is linked");
		crx_stream_close(stream);
		return NULL;
#endif
	}

	sslsock->url_name = crx_openssl_get_url_name(resourcename, resourcenamelen, !!persistent_id);
