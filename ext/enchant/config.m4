...
if test "$CRX_ENCHANT" != "no"; then
  ...
  if test "$found_enchant_2" = "yes"; then
    ...
    CRX_EVAL_INCLINE($ENCHANT_CFLAGS)
    CRX_EVAL_LIBLINE($ENCHANT_LIBS, ENCHANT_SHARED_LIBADD)

    CRX_CHECK_LIBRARY(enchant, enchant_get_version,
    [
      AC_DEFINE(HAVE_ENCHANT_GET_VERSION, 1, [ enchant_get_version since 1.6.0 ])
    ], [ ], [
      $ENCHANT_LIBS
    ...
    CRX_CHECK_LIBRARY(enchant, enchant_broker_set_param,
    [
  ...
  CRX_NEW_EXTENSION(enchant, enchant.c, $ext_shared)
  CRX_SUBST(ENCHANT_SHARED_LIBADD)
fi
