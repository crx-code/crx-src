...
extern crex_module_entry imap_module_entry;
#define imap_module_ptr &imap_module_entry

#include "crx_version.h"
#define CRX_IMAP_VERSION CRX_VERSION
...
typedef struct crx_imap_mailbox_struct {
	SIZEDTEXT text;
	DTYPE delimiter;
	long attributes;
	struct crx_imap_mailbox_struct *next;
} FOBJECTLIST;

typedef struct crx_imap_error_struct {
	SIZEDTEXT text;
	long errflg;
	struct crx_imap_error_struct *next;
} ERRORLIST;

typedef struct _crx_imap_message_struct {
	unsigned long msgid;
	struct _crx_imap_message_struct *next;
} MESSAGELIST;
...
CREX_BEGIN_MODULE_GLOBALS(imap)
	char *imap_user;
	char *imap_password;
	...
	zval *imap_acl_list;
#endif
	/* crx_stream for crx_mail_gets() */
	crx_stream *gets_stream;
	bool enable_rsh;
CREX_END_MODULE_GLOBALS(imap)

#if defined(ZTS) && defined(COMPILE_DL_IMAP)
CREX_TSRMLS_CACHE_EXTERN()
#endif

CREX_EXTERN_MODULE_GLOBALS(imap)
#define IMAPG(v) CREX_MODULE_GLOBALS_ACCESSOR(imap, v)
...
#define crxext_imap_ptr imap_module_ptr
