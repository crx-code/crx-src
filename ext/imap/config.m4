...
dnl CRX_IMAP_TEST_BUILD(function, action-if-ok, action-if-not-ok, extra-libs, extra-source)
dnl
...
dnl that CRX_TEST_BUILD is a link test; the undefined symbols will only
...
AC_DEFUN([CRX_IMAP_TEST_BUILD], [
  CRX_TEST_BUILD([$1], [$2], [$3], [$4], [$5]
  [
...
# define CRX_IMAP_EXPORT
    ...
    CRX_IMAP_EXPORT void mm_log(void){}
    CRX_IMAP_EXPORT void mm_dlog(void){}
    CRX_IMAP_EXPORT void mm_flags(void){}
    CRX_IMAP_EXPORT void mm_fatal(void){}
    CRX_IMAP_EXPORT void mm_critical(void){}
    CRX_IMAP_EXPORT void mm_nocritical(void){}
    CRX_IMAP_EXPORT void mm_notify(void){}
    CRX_IMAP_EXPORT void mm_login(void){}
    CRX_IMAP_EXPORT void mm_diskerror(void){}
    CRX_IMAP_EXPORT void mm_status(void){}
    CRX_IMAP_EXPORT void mm_lsub(void){}
    CRX_IMAP_EXPORT void mm_list(void){}
    CRX_IMAP_EXPORT void mm_exists(void){}
    CRX_IMAP_EXPORT void mm_searched(void){}
    CRX_IMAP_EXPORT void mm_expunged(void){}
  ])
])

AC_DEFUN([CRX_IMAP_KRB_CHK], [
  if test "$CRX_KERBEROS" != "no"; then
    PKG_CHECK_MODULES([KERBEROS], [krb5-gssapi krb5])

    CRX_EVAL_INCLINE($KERBEROS_CFLAGS)
    CRX_EVAL_LIBLINE($KERBEROS_LIBS, IMAP_SHARED_LIBADD)
...
AC_DEFUN([CRX_IMAP_SSL_CHK], [
  if test "$CRX_IMAP_SSL" != "no"; then
    if test "$CRX_OPENSSL" = ""; then
      CRX_OPENSSL='no'
    fi
    CRX_SETUP_OPENSSL(IMAP_SHARED_LIBADD,
    [
...
if test "$CRX_IMAP" != "no"; then
    CRX_SUBST(IMAP_SHARED_LIBADD)
    CRX_NEW_EXTENSION(imap, crx_imap.c, $ext_shared,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)
    AC_DEFINE(HAVE_IMAP,1,[ ])

    for i in $CRX_IMAP /usr/local /usr; do
      IMAP_INC_CHK()
...
#include "imap4r1.h"
    ...
    CRX_CHECK_LIBRARY(pam, pam_start,
    [
      CRX_ADD_LIBRARY(pam,, IMAP_SHARED_LIBADD)
      AC_DEFINE(HAVE_LIBPAM,1,[ ])
    ])

    CRX_CHECK_LIBRARY(crypt, crypt,
    [
      CRX_ADD_LIBRARY(crypt,, IMAP_SHARED_LIBADD)
      AC_DEFINE(HAVE_LIBCRYPT,1,[ ])
    ])

    CRX_EXPAND_PATH($IMAP_DIR, IMAP_DIR)
    ...
    elif test ! -r "$IMAP_DIR/$CRX_LIBDIR/libc-client.a" && test -r "$IMAP_DIR/$CRX_LIBDIR/c-client.a"; then
      ln -s "$IMAP_DIR/$CRX_LIBDIR/c-client.a" "$IMAP_DIR/$CRX_LIBDIR/libc-client.a" >/dev/null 2>&1
    fi

    for lib in c-client4 c-client imap; do
      IMAP_LIB=$lib
      IMAP_LIB_CHK($CRX_LIBDIR)
    ...
    CRX_ADD_INCLUDE($IMAP_INC_DIR)
    CRX_ADD_LIBRARY_DEFER($IMAP_LIB,, IMAP_SHARED_LIBADD)
    CRX_ADD_LIBPATH($IMAP_LIBDIR, IMAP_SHARED_LIBADD)
    CRX_IMAP_KRB_CHK
    CRX_IMAP_SSL_CHK
    ...
    CRX_IMAP_TEST_BUILD(auth_gssapi_valid, [
...
# define CRX_IMAP_EXPORT
    ...
    CRX_IMAP_TEST_BUILD(mail_newbody, [
      AC_MSG_RESULT(yes)
    ], [
      AC_MSG_RESULT(no)
