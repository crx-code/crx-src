/*
   ...
   |          Daniel R. Kalowsky  <kalowsky@crx.net>                      |
...
#define CRX_EXPUNGE 32768
#define CRX_IMAP_ADDRESS_SIZE_BUF 10
#ifndef SENDBUFLEN
#define SENDBUFLEN 16385
#endif

#define CRX_IMAP_OPENTIMEOUT 1
#define CRX_IMAP_READTIMEOUT 2
#define CRX_IMAP_WRITETIMEOUT 3
#define CRX_IMAP_CLOSETIMEOUT 4

#define SA_ALL SA_MESSAGES | SA_RECENT | SA_UNSEEN | SA_UIDNEXT | SA_UIDVALIDITY

#include "crx_imap_arginfo.h"

#if defined(__GNUC__) && __GNUC__ >= 4
# define CRX_IMAP_EXPORT __attribute__ ((visibility("default")))
#else
# define CRX_IMAP_EXPORT
#endif

static void _crx_make_header_object(zval *myzvalue, ENVELOPE *en);
static void _crx_imap_add_body(zval *arg, const BODY *body);
static crex_string* _crx_imap_parse_address(ADDRESS *addresslist, zval *paddress);
static crex_string* _crx_rfc822_write_address(ADDRESS *addresslist);

/* the gets we use */
static char *crx_mail_gets(readfn_t f, void *stream, unsigned long size, GETS_DATA *md);
...
CREX_DECLARE_MODULE_GLOBALS(imap)
static CRX_GINIT_FUNCTION(imap);

/* {{{ imap dependencies */
static const crex_module_dep imap_deps[] = {
...
/* {{{ CRX_INI */
CRX_INI_BEGIN()
STD_CRX_INI_BOOLEAN("imap.enable_insecure_rsh", "0", CRX_INI_SYSTEM, OnUpdateBool, enable_rsh, crex_imap_globals, imap_globals)
CRX_INI_END()
/* }}} */


/* {{{ imap_module_entry */
crex_module_entry imap_module_entry = {
...
#endif
CREX_GET_MODULE(imap)
#endif

/* Imap class entry definition */
static crex_class_entry *crx_imap_ce = NULL;
static crex_object_handlers imap_object_handlers;

typedef struct _crx_imap_object {
	MAILSTREAM *imap_stream;
	long flags;
	crex_object std;
} crx_imap_object;

static inline crex_object *imap_object_to_crex_object(crx_imap_object *obj) {
	return ((crex_object*)(obj + 1)) - 1;
}

static inline crx_imap_object *imap_object_from_crex_object(crex_object *zobj) {
	return ((crx_imap_object*)(zobj + 1)) - 1;
}

static crex_object* imap_object_create(crex_class_entry* ce) {
	crx_imap_object *obj = crex_object_alloc(sizeof(crx_imap_object), ce);
	crex_object *zobj = imap_object_to_crex_object(obj);
...
static crex_function *imap_object_get_constructor(crex_object *zobj) {
	crex_throw_error(NULL, "Cannot directly construct IMAP\\Connection, use imap_open() instead");
	return NULL;
}

static void imap_object_destroy(crex_object *zobj) {
	crx_imap_object *obj = imap_object_from_crex_object(zobj);
...
#define GET_IMAP_STREAM(imap_conn_struct, zval_imap_obj) \
	imap_conn_struct = imap_object_from_crex_object(C_OBJ_P(zval_imap_obj)); \
	if (imap_conn_struct->imap_stream == NULL) { \
		crex_throw_exception(crex_ce_value_error, "IMAP\\Connection is already closed", 0); \
		RETURN_THROWS(); \
	}

#define CRX_IMAP_CHECK_MSGNO_MAYBE_UID_PRE_FLAG_CHECKS(msgindex, arg_pos) \
	if (msgindex < 1) { \
		crex_argument_value_error(arg_pos, "must be greater than 0"); \
		RETURN_THROWS(); \
	} \

#define CRX_IMAP_CHECK_MSGNO_MAYBE_UID_POST_FLAG_CHECKS(msgindex, arg_pos, func_flags, uid_flag) \
	if (func_flags & uid_flag) { \
		...
		if (msg_no_from_uid == 0) { \
			crx_error_docref(NULL, E_WARNING, "UID does not exist"); \
			RETURN_FALSE; \
		} \
	} else { \
		if (((unsigned) msgindex) > imap_conn_struct->imap_stream->nmsgs) { \
			crx_error_docref(NULL, E_WARNING, "Bad message number"); \
			RETURN_FALSE; \
		} \
	} \

// TODO Promote to ValueError?
#define CRX_IMAP_CHECK_MSGNO(msgindex, arg_pos)	\
	if (msgindex < 1) { \
		crex_argument_value_error(arg_pos, "must be greater than 0"); \
		RETURN_THROWS(); \
	} \
...
CRX_MINIT_FUNCTION(imap)
{
	...
	crx_imap_ce = register_class_IMAP_Connection();
	crx_imap_ce->create_object = imap_object_create;

	memcpy(&imap_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	imap_object_handlers.offset = XtOffsetOf(crx_imap_object, std);
	...
	register_crx_imap_symbols(module_number);
	...
	if (!IMAPG(enable_rsh)) {
		/* disable SSH and RSH, see https://bugs.crx.net/bug.crx?id=77153 */
	...
	if (IMAPG(imap_errorstack) != NIL) {
		...
		if (EG(error_reporting) & E_NOTICE) {
			crex_try {
				...
				while (ecur != NIL) {
					crx_error_docref(NULL, E_NOTICE, "%s (errflg=%ld)", ecur->LTEXT, ecur->errflg);
					ecur = ecur->next;
				}
			} crex_end_try();
		}
	...
	if (IMAPG(imap_alertstack) != NIL) {
		...
		if (EG(error_reporting) & E_NOTICE) {
			crex_try {
				...
				while (acur != NIL) {
					crx_error_docref(NULL, E_NOTICE, "%s", acur->LTEXT);
					acur = acur->next;
				}
			} crex_end_try();
		}
...
/* {{{ CRX_MINFO_FUNCTION */
CRX_MINFO_FUNCTION(imap)
{
	crx_info_print_table_start();
	crx_info_print_table_row(2, "IMAP c-Client Version", CCLIENTVERSION);
#ifdef HAVE_IMAP_SSL
	crx_info_print_table_row(2, "SSL Support", "enabled");
#endif
#if defined(HAVE_IMAP_KRB) && defined(HAVE_IMAP_AUTH_GSS)
	crx_info_print_table_row(2, "Kerberos Support", "enabled");
#endif
...
CRX_FUNCTION(imap_open)
{
	crex_string *mailbox, *user, *passwd;
	crex_long retries = 0, flags = 0, cl_flags = 0;
	MAILSTREAM *imap_stream;
	HashTable *params = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "PSS|llh", &mailbox, &user, &passwd, &flags, &retries, &params) == FAILURE) {
		RETURN_THROWS();
	}

	/* Check for CRX_EXPUNGE and not CL_EXPUNGE as the user land facing CL_EXPUNGE constant is defined
	 ...
	 * IMAP\Connection object being destroyed naturally at the end of the CRX script */
	if (flags && ((flags & ~(OP_READONLY | OP_ANONYMOUS | OP_HALFOPEN | CRX_EXPUNGE | OP_DEBUG | OP_SHORTCACHE
	 		| OP_SILENT | OP_PROTOTYPE | OP_SECURE)) != 0)) {
		crex_argument_value_error(4, "must be a bitmask of the OP_* constants, and CL_EXPUNGE");
		RETURN_THROWS();
	}
	...
	if (params) {
		...
		if ((disabled_auth_method = crex_hash_str_find(params, "DISABLE_AUTHENTICATOR", sizeof("DISABLE_AUTHENTICATOR") - 1)) != NULL) {
			switch (C_TYPE_P(disabled_auth_method)) {
				case IS_STRING:
					if (C_STRLEN_P(disabled_auth_method) > 1) {
						mail_parameters(NIL, DISABLE_AUTHENTICATOR, (void *)C_STRVAL_P(disabled_auth_method));
					}
					break;
				case IS_ARRAY:
					{
						zval *z_auth_method;
						int i;
						int nelems = crex_hash_num_elements(C_ARRVAL_P(disabled_auth_method));

						if (nelems == 0 ) {
							break;
						}
						for (i = 0; i < nelems; i++) {
							if ((z_auth_method = crex_hash_index_find(C_ARRVAL_P(disabled_auth_method), i)) != NULL) {
								if (C_TYPE_P(z_auth_method) == IS_STRING) {
									if (C_STRLEN_P(z_auth_method) > 1) {
										mail_parameters(NIL, DISABLE_AUTHENTICATOR, (void *)C_STRVAL_P(z_auth_method));
									}
								} else {
									crex_argument_type_error(6, "option \"DISABLE_AUTHENTICATOR\" must be a string or an array of strings");
									RETURN_THROWS();
								}
							}
						}
	...
	if (ZSTR_VAL(mailbox)[0] != '{' && crx_check_open_basedir(ZSTR_VAL(mailbox))) {
		RETURN_FALSE;
	}
	...
	CREX_IGNORE_LEAKS_BEGIN();
	imap_stream = mail_open(NIL, ZSTR_VAL(mailbox), flags);
	CREX_IGNORE_LEAKS_END();

	if (imap_stream == NIL) {
		crx_error_docref(NULL, E_WARNING, "Couldn't open stream %s", ZSTR_VAL(mailbox));
		efree(IMAPG(imap_user)); IMAPG(imap_user) = 0;
		efree(IMAPG(imap_password)); IMAPG(imap_password) = 0;
		RETURN_FALSE;
	}

	object_init_ex(return_value, crx_imap_ce);
	imap_object_from_crex_object(C_OBJ_P(return_value))->imap_stream = imap_stream;
	imap_object_from_crex_object(C_OBJ_P(return_value))->flags = cl_flags;
}
/* }}} */

/* {{{ Reopen an IMAP stream to a new mailbox */
CRX_FUNCTION(imap_reopen)
{
	zval *imap_conn_obj;
	crex_string *mailbox;
	crex_long options = 0, retries = 0;
	crx_imap_object *imap_conn_struct;
	long flags = 0;
	long cl_flags = 0;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS|ll", &imap_conn_obj, crx_imap_ce, &mailbox, &options, &retries) == FAILURE) {
		RETURN_THROWS();
	}

	GET_IMAP_STREAM(imap_conn_struct, imap_conn_obj);

	/* TODO Verify these are the only options available as they are pulled from the crx.net documentation */
	/* Check for CRX_EXPUNGE and not CL_EXPUNGE as the user land facing CL_EXPUNGE constant is defined
     * to something different to prevent clashes between CL_EXPUNGE and an OP_* constant allowing setting
     * the CL_EXPUNGE flag which will expunge when the mailbox is closed (be that manually, or via the
     * IMAP\Connection object being destroyed naturally at the end of the CRX script */
	if (options && ((options & ~(OP_READONLY | OP_ANONYMOUS | OP_HALFOPEN | OP_EXPUNGE | CRX_EXPUNGE)) != 0)) {
		crex_argument_value_error(3, "must be a bitmask of OP_READONLY, OP_ANONYMOUS, OP_HALFOPEN, "
	...
	if (imap_conn_struct->imap_stream == NIL) {
		...
		crx_error_docref(NULL, E_WARNING, "Couldn't re-open stream");
		RETURN_FALSE;
	}
	RETURN_TRUE;
}
...
CRX_FUNCTION(imap_is_open)
{
	zval *imap_conn_obj;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &imap_conn_obj, crx_imap_ce) == FAILURE) {
		RETURN_THROWS();
	}

	/* Manual reimplementation of the GET_IMAP_STREAM() macro that doesn't throw */
	imap_conn_struct = imap_object_from_crex_object(C_OBJ_P(imap_conn_obj));
	/* Stream was closed */
	if (imap_conn_struct->imap_stream == NULL) {
		RETURN_FALSE;
	}
...
CRX_FUNCTION(imap_append)
{
	zval *imap_conn_obj;
	crex_string *folder, *message, *internal_date = NULL, *flags = NULL;
	crx_imap_object *imap_conn_struct;
	STRING st;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSS|S!S!", &imap_conn_obj, crx_imap_ce, &folder, &message, &flags, &internal_date) == FAILURE) {
		RETURN_THROWS();
	}

	if (internal_date) {
		crex_string *regex  = ZSTR_INIT_LITERAL("/[0-3][0-9]-((Jan)|(Feb)|(Mar)|(Apr)|(May)|(Jun)|(Jul)|(Aug)|(Sep)|(Oct)|(Nov)|(Dec))-[0-9]{4} [0-2][0-9]:[0-5][0-9]:[0-5][0-9] [+-][0-9]{4}/", 0);
		...
		crex_string_release(regex);
		crx_pcre_match_impl(pce, internal_date, return_value, subpats, global,
			0, C_L(0), C_L(0));

		if (!C_LVAL_P(return_value)) {
			// TODO Promoto to error?
			crx_error_docref(NULL, E_WARNING, "Internal date not correctly formatted");
			internal_date = NULL;
		}
	}

	/* TODO Check if flags are valid (documentation is not present on crx.net so need to check this first) */
...
CRX_FUNCTION(imap_get_quota)
{
	zval *imap_conn_obj;
	crex_string *qroot;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS", &imap_conn_obj, crx_imap_ce, &qroot) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!imap_getquota(imap_conn_struct->imap_stream, ZSTR_VAL(qroot))) {
		crx_error_docref(NULL, E_WARNING, "C-client imap_getquota failed");
...
CRX_FUNCTION(imap_get_quotaroot)
{
	zval *imap_conn_obj;
	crex_string *mbox;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS", &imap_conn_obj, crx_imap_ce, &mbox) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!imap_getquotaroot(imap_conn_struct->imap_stream, ZSTR_VAL(mbox))) {
		crx_error_docref(NULL, E_WARNING, "C-client imap_getquotaroot failed");
		crex_array_destroy(C_ARR_P(return_value));
		RETURN_FALSE;
	}
}
...
CRX_FUNCTION(imap_set_quota)
{
	zval *imap_conn_obj;
	crex_string *qroot;
	crex_long mailbox_size;
	crx_imap_object *imap_conn_struct;
	STRINGLIST	limits;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSl", &imap_conn_obj, crx_imap_ce, &qroot, &mailbox_size) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_setacl)
{
	zval *imap_conn_obj;
	crex_string *mailbox, *id, *rights;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSSS", &imap_conn_obj, crx_imap_ce, &mailbox, &id, &rights) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_getacl)
{
	zval *imap_conn_obj;
	crex_string *mailbox;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS", &imap_conn_obj, crx_imap_ce, &mailbox) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!imap_getacl(imap_conn_struct->imap_stream, ZSTR_VAL(mailbox))) {
		crx_error(E_WARNING, "c-client imap_getacl failed");
		crex_array_destroy(C_ARR_P(return_value));
		RETURN_FALSE;
	}
...
CRX_FUNCTION(imap_gc)
{
	zval *imap_conn_obj;
	crx_imap_object *imap_conn_struct;
	crex_long flags;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol", &imap_conn_obj, crx_imap_ce, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~(GC_TEXTS | GC_ELT | GC_ENV)) != 0)) {
		crex_argument_value_error(2, "must be a bitmask of IMAP_GC_TEXTS, IMAP_GC_ELT, and IMAP_GC_ENV");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_close)
{
	zval *imap_conn_obj;
	crx_imap_object *imap_conn_struct = NULL;
	crex_long options = 0;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "O|l", &imap_conn_obj, crx_imap_ce, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (options) {
		/* Check that flags is exactly equal to CRX_EXPUNGE or zero */
		if (options && ((options & ~CRX_EXPUNGE) != 0)) {
			crex_argument_value_error(2, "must be CL_EXPUNGE or 0");
			RETURN_THROWS();
		}

		/* Do the translation from CRX's internal CRX_EXPUNGE define to c-client's CL_EXPUNGE */
		if (options & CRX_EXPUNGE) {
			options ^= CRX_EXPUNGE;
			options |= CL_EXPUNGE;
		}
...
CRX_FUNCTION(imap_body)
{
	zval *imap_conn_obj;
	crex_long msgno, flags = 0;
	crx_imap_object *imap_conn_struct;
	char *body;
	unsigned long body_len = 0;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol|l", &imap_conn_obj, crx_imap_ce, &msgno, &flags) == FAILURE) {
		RETURN_THROWS();
	}

	GET_IMAP_STREAM(imap_conn_struct, imap_conn_obj);

	CRX_IMAP_CHECK_MSGNO_MAYBE_UID_PRE_FLAG_CHECKS(msgno, 2);

	if (flags && ((flags & ~(FT_UID|FT_PEEK|FT_INTERNAL)) != 0)) {
		crex_argument_value_error(3, "must be a bitmask of FT_UID, FT_PEEK, and FT_INTERNAL");
		RETURN_THROWS();
	}

	CRX_IMAP_CHECK_MSGNO_MAYBE_UID_POST_FLAG_CHECKS(msgno, 2, flags, FT_UID);
...
CRX_FUNCTION(imap_mail_copy)
{
	zval *imap_conn_obj;
	crex_long options = 0;
	crex_string *seq, *folder;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSS|l", &imap_conn_obj, crx_imap_ce, &seq, &folder, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (options && ((options & ~(CP_UID | CP_MOVE)) != 0)) {
		crex_argument_value_error(4, "must be a bitmask of CP_UID, and CP_MOVE");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_mail_move)
{
	...
	if (options && ((options & ~CP_UID) != 0)) {
		crex_argument_value_error(4, "must be CP_UID or 0");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_createmailbox)
{
	zval *imap_conn_obj;
	crex_string *folder;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS", &imap_conn_obj, crx_imap_ce, &folder) == FAILURE) {
		RETURN_THROWS();
	}
...
/* {{{ Rename a mailbox */
CRX_FUNCTION(imap_renamemailbox)
{
	zval *imap_conn_obj;
	crex_string *old_mailbox, *new_mailbox;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSS", &imap_conn_obj, crx_imap_ce, &old_mailbox, &new_mailbox) == FAILURE) {
		RETURN_THROWS();
	}
...
/* {{{ Delete a mailbox */
CRX_FUNCTION(imap_deletemailbox)
{
	zval *imap_conn_obj;
	crex_string *folder;
	crx_imap_object *imap_conn_struct;
...
CRX_FUNCTION(imap_list)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSS", &imap_conn_obj, crx_imap_ce, &ref, &pat) == FAILURE) {
		RETURN_THROWS();
	}
...
static void crx_imap_populate_mailbox_object(zval *z_object, const FOBJECTLIST *mailbox)
{
	crex_update_property_string(
		C_OBJCE_P(z_object), C_OBJ_P(z_object),
		"name", strlen("name"),
		(char*)mailbox->LTEXT
	);
...
/* Author: CJH */
CRX_FUNCTION(imap_getmailboxes)
{
	zval *imap_conn_obj;
	crex_string *ref, *pat;
	crx_imap_object *imap_conn_struct;
	...
	while (cur != NIL) {
		zval mboxob;
		object_init(&mboxob);
		crx_imap_populate_mailbox_object(&mboxob, cur);
		crex_hash_next_index_insert_new(C_ARR_P(return_value), &mboxob);
		cur=cur->next;
	}
...
CRX_FUNCTION(imap_listscan)
{
	zval *imap_conn_obj;
	crex_string *ref, *pat, *content;
	crx_imap_object *imap_conn_struct;
	STRINGLIST *cur=NIL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSSS", &imap_conn_obj, crx_imap_ce, &ref, &pat, &content) == FAILURE) {
		RETURN_THROWS();
	}
...
static void crx_imap_populate_mailbox_properties_object(zval *z_object, const MAILSTREAM *imap_stream)
{
	char date[100];
	rfc822_date(date);
	crex_update_property_string(
...
CRX_FUNCTION(imap_check)
{
	...
	if (imap_conn_struct->imap_stream->mailbox) {
		...
		crx_imap_populate_mailbox_properties_object(return_value, imap_conn_struct->imap_stream);
	} else {
		RETURN_FALSE;
	}
}
...
CRX_FUNCTION(imap_delete)
{
	zval *imap_conn_obj;
	crx_imap_object *imap_conn_struct;
	crex_string *sequence;
	crex_long flags = 0;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS|l", &imap_conn_obj, crx_imap_ce, &sequence, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~FT_UID) != 0)) {
		crex_argument_value_error(3, "must be FT_UID or 0");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_headerinfo)
{
	zval *imap_conn_obj;
	crex_long msgno, fromlength = 0, subjectlength = 0;
	crx_imap_object *imap_conn_struct;
	MESSAGECACHE *cache;
	ENVELOPE *en;
	char dummy[2000], fulladdress[MAILTMPLEN + 1];

	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol|ll", &imap_conn_obj, crx_imap_ce, &msgno, &fromlength, &subjectlength) == FAILURE) {
		RETURN_THROWS();
	}

	GET_IMAP_STREAM(imap_conn_struct, imap_conn_obj);

	CRX_IMAP_CHECK_MSGNO(msgno, 2);

	if (fromlength < 0 || fromlength > MAILTMPLEN) {
		crex_argument_value_error(3, "must be between 0 and %d", MAILTMPLEN);
		RETURN_THROWS();
	}

	if (subjectlength < 0 || subjectlength > MAILTMPLEN) {
		crex_argument_value_error(4, "must be between 0 and %d", MAILTMPLEN);
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_rfc822_parse_headers)
{
	crex_string *headers, *defaulthost = NULL;
	ENVELOPE *en;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S|S", &headers, &defaulthost) == FAILURE) {
		RETURN_THROWS();
	}
...
/* Author: CJH */
CRX_FUNCTION(imap_getsubscribed)
{
	zval *imap_conn_obj;
	crex_string *ref, *pat;
	crx_imap_object *imap_conn_struct;
...
CRX_FUNCTION(imap_fetchstructure)
{
	zval *imap_conn_obj;
	crex_long msgno, flags = 0;
	crx_imap_object *imap_conn_struct;
	...
	if (!body) {
		crx_error_docref(NULL, E_WARNING, "No body information available");
		RETURN_FALSE;
	}

	_crx_imap_add_body(return_value, body);
}
/* }}} */

/* {{{ Get a specific body section */
CRX_FUNCTION(imap_fetchbody)
{
	zval *imap_conn_obj;
	crex_long msgno, flags = 0;
	crx_imap_object *imap_conn_struct;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "OlS|l", &imap_conn_obj, crx_imap_ce, &msgno, &sec, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~(FT_UID|FT_PEEK|FT_INTERNAL)) != 0)) {
		crex_argument_value_error(4, "must be a bitmask of FT_UID, FT_PEEK, and FT_INTERNAL");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_fetchmime)
{
	...
	if (!body) {
		crx_error_docref(NULL, E_WARNING, "No body MIME information available");
		RETURN_FALSE;
	}
	RETVAL_STRINGL(body, len);
}
...
CRX_FUNCTION(imap_savebody)
{
	zval *imap_conn_obj;
	zval *out;
	crx_imap_object *imap_conn_struct = NULL;
	crx_stream *writer = NULL;
	crex_string *section = NULL;
	int close_stream = 1;
	crex_long msgno, flags = 0;

	if (SUCCESS != crex_parse_parameters(CREX_NUM_ARGS(), "Ozl|Sl", &imap_conn_obj, crx_imap_ce, &out, &msgno, &section, &flags)) {
		RETURN_THROWS();
	}

	GET_IMAP_STREAM(imap_conn_struct, imap_conn_obj);

	CRX_IMAP_CHECK_MSGNO_MAYBE_UID_PRE_FLAG_CHECKS(msgno, 3)

	if (flags && ((flags & ~(FT_UID|FT_PEEK|FT_INTERNAL)) != 0)) {
		crex_argument_value_error(5, "must be a bitmask of FT_UID, FT_PEEK, and FT_INTERNAL");
		RETURN_THROWS();
	}

	CRX_IMAP_CHECK_MSGNO_MAYBE_UID_POST_FLAG_CHECKS(msgno, 3, flags, FT_UID);
	...
	switch (C_TYPE_P(out))
	{
		...
		case IS_RESOURCE:
			close_stream = 0;
			crx_stream_from_zval(writer, out);
		break;
		...
		default:
			...
			writer = crx_stream_open_wrapper(C_STRVAL_P(out), "wb", REPORT_ERRORS, NULL);
		break;
	}
	...
	mail_parameters(NIL, SET_GETS, (void *) crx_mail_gets);
...
CRX_FUNCTION(imap_mailboxmsginfo)
{
	zval *imap_conn_obj;
	crx_imap_object *imap_conn_struct;
	unsigned long msgno;
	crex_ulong unreadmsg = 0, deletedmsg = 0, msize = 0;
...
CRX_FUNCTION(imap_rfc822_write_address)
{
	crex_string *mailbox, *host, *personal;
	ADDRESS *addr;
	crex_string *string;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "SSS", &mailbox, &host, &personal) == FAILURE) {
		RETURN_THROWS();
	}
	...
	string = _crx_rfc822_write_address(addr);
	if (string) {
		RETVAL_STR(string);
	} else {
		RETVAL_FALSE;
...
static void crx_imap_construct_address_object(zval *z_object, const ADDRESS *address)
{
	if (address->mailbox) {
		crex_update_property_string(
			C_OBJCE_P(z_object), C_OBJ_P(z_object),
...
static void crx_imap_construct_list_of_addresses(HashTable *list, const ADDRESS *const address_list)
{
	const ADDRESS *current_address = address_list;
	do {
		zval tmp_object;
		object_init(&tmp_object);
		crx_imap_construct_address_object(&tmp_object, current_address);
		crex_hash_next_index_insert_new(list, &tmp_object);
	} while ((current_address = current_address->next));
}

/* {{{ Parses an address string */
CRX_FUNCTION(imap_rfc822_parse_adrlist)
{
	crex_string *str, *defaulthost;
	char *str_copy;
	ADDRESS *address_list;
	ENVELOPE *env;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "SS", &str, &defaulthost) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (address_list) {
		crx_imap_construct_list_of_addresses(C_ARR_P(return_value), address_list);
	}
...
CRX_FUNCTION(imap_utf7_decode)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "S", &arg) == FAILURE) {
		RETURN_THROWS();
	}
	...
	for (endp = (inp = in) + inlen; inp < endp; inp++) {
		if (state == ST_NORMAL) {
			...
			if (SPECIAL(*inp)) {
				crx_error_docref(NULL, E_WARNING, "Invalid modified UTF-7 character: `%c'", *inp);
				RETURN_FALSE;
			} else if (*inp != '&') {
				outlen++;
			} else if (inp + 1 == endp) {
				crx_error_docref(NULL, E_WARNING, "Unexpected end of string");
				RETURN_FALSE;
			} else if (inp[1] != '-') {
				state = ST_DECODE0;
			} else {
			...
			if (state == ST_DECODE1) {
				crx_error_docref(NULL, E_WARNING, "Stray modified base64 character: `%c'", *--inp);
				RETURN_FALSE;
			}
			state = ST_NORMAL;
		} else if (!B64CHAR(*inp)) {
			crx_error_docref(NULL, E_WARNING, "Invalid modified base64 character: `%c'", *inp);
			RETURN_FALSE;
		} else {
			switch (state) {
				case ST_DECODE3:
	...
	*outp = 0;
	...
	if (outp - out != outlen) {
		crx_error_docref(NULL, E_WARNING, "outp - out [%zd] != outlen [%d]", outp - out, outlen);
	}
#endif
...
CRX_FUNCTION(imap_utf7_encode)
{
	...
	/* allocate output buffer */
	out = crex_string_safe_alloc(1, outlen, 0, 0);
...
static void crx_imap_mutf7(INTERNAL_FUNCTION_PARAMETERS, int mode) /* {{{ */
{
	crex_string *in;
	unsigned char *out;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S", &in) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_utf8_to_mutf7)
{
	crx_imap_mutf7(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Decode a modified UTF-7 string to UTF-8 */
CRX_FUNCTION(imap_mutf7_to_utf8)
{
	crx_imap_mutf7(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
#endif

/* {{{ Sets flags on messages */
CRX_FUNCTION(imap_setflag_full)
{
	zval *imap_conn_obj;
	crex_string *sequence, *flag;
	crex_long flags = 0;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSS|l", &imap_conn_obj, crx_imap_ce, &sequence, &flag, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~ST_UID) != 0)) {
		crex_argument_value_error(4, "must be ST_UID or 0");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_clearflag_full)
{
	zval *imap_conn_obj;
	crex_string *sequence, *flag;
	crex_long flags = 0;
...
CRX_FUNCTION(imap_sort)
{
	zval *imap_conn_obj;
	crex_string *criteria = NULL, *charset = NULL;
	crex_long sort, flags = 0;
	bool rev;
	crx_imap_object *imap_conn_struct;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Olb|lS!S!", &imap_conn_obj, crx_imap_ce, &sort, &rev, &flags, &criteria, &charset) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (!(sort == SORTDATE || sort == SORTARRIVAL || sort == SORTFROM || sort == SORTSUBJECT || sort == SORTTO ||
		...
		crex_argument_value_error(2, "must be one of the SORT* constants");
		RETURN_THROWS();
	}

	if (flags && ((flags & ~(SE_UID|SE_NOPREFETCH )) != 0)) {
		crex_argument_value_error(4, "must be a bitmask of SE_UID, and SE_NOPREFETCH");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_fetchheader)
{
	...
	if (flags && ((flags & ~(FT_UID|FT_INTERNAL|FT_PREFETCHTEXT)) != 0)) {
		crex_argument_value_error(3, "must be a bitmask of FT_UID, FT_PREFETCHTEXT, and FT_INTERNAL");
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_uid)
{
	zval *imap_conn_obj;
	crex_long msgno;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol", &imap_conn_obj, crx_imap_ce, &msgno) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(imap_msgno)
{
	zval *imap_conn_obj;
	crex_long msg_uid;
	crx_imap_object *imap_conn_struct;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol", &imap_conn_obj, crx_imap_ce, &msg_uid) == FAILURE) {
		RETURN_THROWS();
	}

	GET_IMAP_STREAM(imap_conn_struct, imap_conn_obj);

	/* Do NOT use the CRX_IMAP_CHECK_MSGNO() macro as UID cannot be checked for their upper bound. */
	if (msg_uid < 1) {
...
CRX_FUNCTION(imap_status)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "OSl", &imap_conn_obj, crx_imap_ce, &mbx, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~(SA_ALL)) != 0)) {
		crex_argument_value_error(3, "must be a bitmask of SA_* constants");
		RETURN_THROWS();
	}
...
static void crx_imap_populate_body_struct_object(zval *z_object, const BODY *body)
{
	...
	if (body->disposition.parameter) {
		...
		do {
			...
			crex_update_property_string(
				C_OBJCE_P(&z_disposition_parameter), C_OBJ_P(&z_disposition_parameter),
			...
			crex_hash_next_index_insert_new(C_ARR(z_disposition_parameter_list), &z_disposition_parameter);
	...
	if (body_parameters) {
		...
		do {
			...
			crex_update_property_string(
				C_OBJCE_P(&z_body_parameter), C_OBJ_P(&z_body_parameter),
				"attribute", strlen("attribute"),
			...
			crex_hash_next_index_insert_new(C_ARR(z_body_parameter_list), &z_body_parameter);
...
CRX_FUNCTION(imap_bodystruct)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "OlS", &imap_conn_obj, crx_imap_ce, &msgno, &section) == FAILURE) {
		RETURN_THROWS();
	}
	...
	object_init(return_value);
	crx_imap_populate_body_struct_object(return_value, body);
}

/* }}} */

static void crx_imap_update_property_with_full_address_str(
	zval *z_object, const char *property, size_t property_len, ADDRESS *address)
{
	ADDRESS *current = address;
	current->next = NULL;

	crex_string *full_address = _crx_rfc822_write_address(current);
	if (full_address) {
		crex_update_property_str(
			C_OBJCE_P(z_object), C_OBJ_P(z_object),
		...
		crex_string_release_ex(full_address, false);
	}
}

#define UPDATE_PROPERTY_FULL_STR_ADDRESS(z_object, name, envelope) if ((envelope)->name) { crx_imap_update_property_with_full_address_str((z_object), #name, strlen(#name), (envelope)->name); }

/* {{{ Read an overview of the information in the headers of the given message sequence */
CRX_FUNCTION(imap_fetch_overview)
{
	zval *imap_conn_obj;
	crex_string *sequence;
	crx_imap_object *imap_conn_struct;
	crex_long status, flags = 0L;
	...
	for (unsigned long  i = 1; i <= imap_conn_struct->imap_stream->nmsgs; i++) {
		...
		// TODO Use part _crx_make_header_object function?
		zval myoverview;
		object_init(&myoverview);
		if (env->subject) {
			crex_update_property_string(
				C_OBJCE_P(&myoverview), C_OBJ_P(&myoverview),
		...
		crex_hash_next_index_insert_new(C_ARR_P(return_value), &myoverview);
	}
}
/* }}} */

static bool header_injection(const crex_string *str, bool adrlist)
{
...
CRX_FUNCTION(imap_mail_compose)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "h/h/", &envelope, &body) == FAILURE) {
		RETURN_THROWS();
	}

	if (crex_hash_num_elements(body) == 0) {
...
#define CHECK_HEADER_INJECTION(zstr, adrlist, header) \
	if (header_injection(zstr, adrlist)) { \
		crx_error_docref(NULL, E_WARNING, "header injection attempt in " header); \
		RETVAL_FALSE; \
		goto done; \
	}

#define CRX_RFC822_PARSE_ADRLIST(target, value) \
	str_copy = estrndup(C_STRVAL_P(value), C_STRLEN_P(value)); \
	rfc822_parse_adrlist(target, str_copy, "NO HOST"); \
	efree(str_copy);

	env = mail_newenvelope();
	if ((pvalue = crex_hash_str_find(envelope, "remail", sizeof("remail") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "remail");
		env->remail = cpystr(C_STRVAL_P(pvalue));
	}
	if ((pvalue = crex_hash_str_find(envelope, "return_path", sizeof("return_path") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "return_path");
		CRX_RFC822_PARSE_ADRLIST(&env->return_path, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "date", sizeof("date") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "date");
		env->date = (unsigned char*)cpystr(C_STRVAL_P(pvalue));
	}
	if ((pvalue = crex_hash_str_find(envelope, "from", sizeof("from") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "from");
		CRX_RFC822_PARSE_ADRLIST(&env->from, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "reply_to", sizeof("reply_to") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "reply_to");
		CRX_RFC822_PARSE_ADRLIST(&env->reply_to, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "in_reply_to", sizeof("in_reply_to") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "in_reply_to");
		env->in_reply_to = cpystr(C_STRVAL_P(pvalue));
	}
	if ((pvalue = crex_hash_str_find(envelope, "subject", sizeof("subject") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "subject");
		env->subject = cpystr(C_STRVAL_P(pvalue));
	}
	if ((pvalue = crex_hash_str_find(envelope, "to", sizeof("to") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "to");
		CRX_RFC822_PARSE_ADRLIST(&env->to, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "cc", sizeof("cc") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "cc");
		CRX_RFC822_PARSE_ADRLIST(&env->cc, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "bcc", sizeof("bcc") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 1, "bcc");
		CRX_RFC822_PARSE_ADRLIST(&env->bcc, pvalue);
	}
	if ((pvalue = crex_hash_str_find(envelope, "message_id", sizeof("message_id") - 1)) != NULL) {
		convert_to_string(pvalue);
		CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "message_id");
		env->message_id=cpystr(C_STRVAL_P(pvalue));
	}

	if ((pvalue = crex_hash_str_find(envelope, "custom_headers", sizeof("custom_headers") - 1)) != NULL) {
		if (C_TYPE_P(pvalue) == IS_ARRAY) {
			custom_headers_param = tmp_param = NULL;
			SEPARATE_ARRAY(pvalue);
			CREX_HASH_FOREACH_VAL(C_ARRVAL_P(pvalue), env_data) {
				custom_headers_param = mail_newbody_parameter();
				convert_to_string(env_data);
				CHECK_HEADER_INJECTION(C_STR_P(env_data), 0, "custom_headers");
				custom_headers_param->value = (char *) fs_get(C_STRLEN_P(env_data) + 1);
				custom_headers_param->attribute = NULL;
				memcpy(custom_headers_param->value, C_STRVAL_P(env_data), C_STRLEN_P(env_data) + 1);
	...
	first = 1;
	CREX_HASH_FOREACH_VAL(body, data) {
		if (first) {
			first = 0;

			if (C_TYPE_P(data) != IS_ARRAY) {
				crex_argument_type_error(2, "individual body must be of type array, %s given",
					crex_zval_value_name(data));
				goto done;
			}
			if (crex_hash_num_elements(C_ARRVAL_P(data)) == 0) {
				crex_argument_value_error(2, "individual body cannot be empty");
				goto done;
			}
			SEPARATE_ARRAY(data);
			...
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "type", sizeof("type") - 1)) != NULL) {
				crex_long type = zval_get_long(pvalue);
				if (type >= 0 && type <= TYPEMAX && body_types[type] != NULL) {
					bod->type = (short) type;
				}
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "encoding", sizeof("encoding") - 1)) != NULL) {
				crex_long encoding = zval_get_long(pvalue);
				if (encoding >= 0 && encoding <= ENCMAX && body_encodings[encoding] != NULL) {
					bod->encoding = (short) encoding;
				}
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "charset", sizeof("charset") - 1)) != NULL) {
				convert_to_string(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body charset");
				tmp_param = mail_newbody_parameter();
				tmp_param->value = cpystr(C_STRVAL_P(pvalue));
				tmp_param->attribute = cpystr("CHARSET");
				tmp_param->next = bod->parameter;
				bod->parameter = tmp_param;
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "type.parameters", sizeof("type.parameters") - 1)) != NULL) {
				if(C_TYPE_P(pvalue) == IS_ARRAY && !HT_IS_PACKED(C_ARRVAL_P(pvalue))) {
					disp_param = tmp_param = NULL;
					SEPARATE_ARRAY(pvalue);
					CREX_HASH_MAP_FOREACH_STR_KEY_VAL(C_ARRVAL_P(pvalue), key, disp_data) {
						...
						CHECK_HEADER_INJECTION(C_STR_P(disp_data), 0, "body disposition value");
						disp_param->value = (char *) fs_get(C_STRLEN_P(disp_data) + 1);
						memcpy(disp_param->value, C_STRVAL_P(disp_data), C_STRLEN_P(disp_data) + 1);
			...
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "subtype", sizeof("subtype") - 1)) != NULL) {
				convert_to_string_ex(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body subtype");
				bod->subtype = cpystr(C_STRVAL_P(pvalue));
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "id", sizeof("id") - 1)) != NULL) {
				convert_to_string(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body id");
				bod->id = cpystr(C_STRVAL_P(pvalue));
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "description", sizeof("description") - 1)) != NULL) {
				convert_to_string(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body description");
				bod->description = cpystr(C_STRVAL_P(pvalue));
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "disposition.type", sizeof("disposition.type") - 1)) != NULL) {
				convert_to_string(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body disposition.type");
				bod->disposition.type = (char *) fs_get(C_STRLEN_P(pvalue) + 1);
				memcpy(bod->disposition.type, C_STRVAL_P(pvalue), C_STRLEN_P(pvalue)+1);
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "disposition", sizeof("disposition") - 1)) != NULL) {
				if (C_TYPE_P(pvalue) == IS_ARRAY && !HT_IS_PACKED(C_ARRVAL_P(pvalue))) {
					disp_param = tmp_param = NULL;
					SEPARATE_ARRAY(pvalue);
					CREX_HASH_MAP_FOREACH_STR_KEY_VAL(C_ARRVAL_P(pvalue), key, disp_data) {
						...
						disp_param->attribute = cpystr(ZSTR_VAL(key));
						convert_to_string(disp_data);
						CHECK_HEADER_INJECTION(C_STR_P(disp_data), 0, "body type.parameters value");
						disp_param->value = (char *) fs_get(C_STRLEN_P(disp_data) + 1);
						memcpy(disp_param->value, C_STRVAL_P(disp_data), C_STRLEN_P(disp_data) + 1);
						disp_param->next = tmp_param;
						tmp_param = disp_param;
			...
			if (bod->type == TYPEMESSAGE && bod->subtype && !strcmp(bod->subtype, "RFC822")) {
				...
				if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "contents.data", sizeof("contents.data") - 1)) != NULL) {
					convert_to_string(pvalue);
					bod->contents.text.data = fs_get(C_STRLEN_P(pvalue) + 1);
					memcpy(bod->contents.text.data, C_STRVAL_P(pvalue), C_STRLEN_P(pvalue)+1);
					bod->contents.text.size = C_STRLEN_P(pvalue);
				} else {
			...
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "lines", sizeof("lines") - 1)) != NULL) {
				bod->size.lines = zval_get_long(pvalue);
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "bytes", sizeof("bytes") - 1)) != NULL) {
				bod->size.bytes = zval_get_long(pvalue);
			}
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "md5", sizeof("md5") - 1)) != NULL) {
				convert_to_string(pvalue);
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body md5");
				bod->md5 = cpystr(C_STRVAL_P(pvalue));
			}
		} else if (C_TYPE_P(data) == IS_ARRAY && topbod->type == TYPEMULTIPART) {
			short type = 0;
			SEPARATE_ARRAY(data);
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "type", sizeof("type") - 1)) != NULL) {
				crex_long tmp_type = zval_get_long(pvalue);
			...
			if ((pvalue = crex_hash_str_find(C_ARRVAL_P(data), "charset", sizeof("charset") - 1)) != NULL) {
				...
				CHECK_HEADER_INJECTION(C_STR_P(pvalue), 0, "body charset");
				tmp_param = mail_newbody_parameter();
				tmp_param->value = (char *) fs_get(C_STRLEN_P(pvalue) + 1);
				memcpy(tmp_param->value, C_STRVAL_P(pvalue), C_STRLEN_P(pvalue) + 1);
				tmp_param->attribute = cpystr("CHARSET");
				tmp_param->next = bod->parameter;
				bod->parameter = tmp_param;
			}
	...
	if (bod && bod->type == TYPEMULTIPART && (!bod->nested.part || !bod->nested.part->next)) {
		crx_error_docref(NULL, E_WARNING, "Cannot generate multipart e-mail without components.");
		RETVAL_FALSE;
		goto done;
	}
	...
	if (bod && bod->type == TYPEMULTIPART) {
		...
		/* yucky default */
			if (!cookie) {
				...
				crx_error_docref(NULL, E_WARNING, "The boundary should be no longer than 4kb");
				RETVAL_FALSE;
				goto done;
			}
...
/* {{{ _crx_imap_mail */
bool _crx_imap_mail(crex_string *to, crex_string *subject, crex_string *message, crex_string *headers,
	crex_string *cc, crex_string *bcc, crex_string* rpath)
{
#ifdef CRX_WIN32
	int tsm_err;
#else
	...
	CREX_ASSERT(to && ZSTR_LEN(to) != 0);
	CREX_ASSERT(subject && ZSTR_LEN(subject) != 0);
	CREX_ASSERT(message);
...
#define CRX_IMAP_CLEAN	if (bufferTo) efree(bufferTo); if (bufferCc) efree(bufferCc); if (bufferBcc) efree(bufferBcc); if (bufferHeader) efree(bufferHeader);
#define CRX_IMAP_BAD_DEST CRX_IMAP_CLEAN; efree(tempMailTo); return (BAD_MSG_DESTINATION);
	...
	if (TSendMail(INI_STR("SMTP"), &tsm_err, &tsm_errmsg, bufferHeader, ZSTR_VAL(subject),
		...
		if (tsm_errmsg) {
			crx_error_docref(NULL, E_WARNING, "%s", tsm_errmsg);
			efree(tsm_errmsg);
		} else {
			crx_error_docref(NULL, E_WARNING, "%s", GetSMErrorText(tsm_err));
		}
		CRX_IMAP_CLEAN;
		return 0;
	}
	...
	if (sendmail) {
		...
		crx_error_docref(NULL, E_WARNING, "Could not execute mail delivery program");
		return 0;
	}
#endif
	return 1;
...
CRX_FUNCTION(imap_mail)
{
	crex_string *to=NULL, *message=NULL, *headers=NULL, *subject=NULL, *cc=NULL, *bcc=NULL, *rpath=NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "PPP|P!P!P!P!", &to, &subject, &message,
	...
	if (ZSTR_LEN(message) == 0) {
		...
		crx_error_docref(NULL, E_WARNING, "No message string in mail command");
	}

	if (_crx_imap_mail(to, subject, message, headers, cc, bcc, rpath)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
CRX_FUNCTION(imap_search)
{
	zval *imap_conn_obj;
	crex_string *criteria, *charset = NULL;
	crex_long flags = SE_FREE;
	crx_imap_object *imap_conn_struct;
	char *search_criteria;
	MESSAGELIST *cur;
	SEARCHPGM *pgm = NIL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "OS|lS", &imap_conn_obj, crx_imap_ce, &criteria, &flags, &charset) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (flags && ((flags & ~(SE_FREE | SE_UID)) != 0)) {
		crex_argument_value_error(3, "must be a bitmask of SE_FREE, and SE_UID");
		RETURN_THROWS();
	}
...
static void crx_imap_populate_mime_header_object(zval *z_object, const char *charset, const char *content)
{
	CREX_ASSERT(z_object && "Object must be initialized");
	crex_update_property_string(
		C_OBJCE_P(z_object), C_OBJ_P(z_object),
...
CRX_FUNCTION(imap_mime_header_decode)
{
	/* Author: Ted Parnefors <ted@mtv.se> */
	zval myobject;
	crex_string *str;
	char *string, *charset, encoding, *text, *decode;
	crex_long charset_token, encoding_token, end_token, end, offset=0, i;
	unsigned long newlength;
	...
	while (offset < end) {	/* Reached end of the string? */
		if ((charset_token = (crex_long)crx_memnstr(&string[offset], "=?", 2, string + end))) {	/* Is there anything encoded in the string? */
			charset_token -= (crex_long)string;
			if (offset != charset_token) {	/* Is there anything before the encoded data? */
				...
				crx_imap_populate_mime_header_object(&myobject, "default", text);
				crex_hash_next_index_insert(C_ARRVAL_P(return_value), &myobject);
			}
			if ((encoding_token = (crex_long)crx_memnstr(&string[charset_token+2], "?", 1, string+end))) {		/* Find token for encoding */
				encoding_token -= (crex_long)string;
				if ((end_token = (crex_long)crx_memnstr(&string[encoding_token+3], "?=", 2, string+end))) {	/* Find token for end of encoded data */
					end_token -= (crex_long)string;
					...
					object_init(&myobject);
					crx_imap_populate_mime_header_object(&myobject, charset, decode);
					crex_hash_next_index_insert(C_ARRVAL_P(return_value), &myobject);
...
/* {{{ _crx_rfc822_soutr */
static long _crx_rfc822_soutr (void *stream, char *string)
{
...
static size_t _crx_rfc822_len(const char *const str)
{
...
static size_t _crx_imap_address_size(const ADDRESS *const address_list)
{
	size_t total_size = 0;
	...
	if (current_address) do {
		total_size += _crx_rfc822_len(current_address->personal);
		total_size += _crx_rfc822_len(current_address->adl);
		total_size += _crx_rfc822_len(current_address->mailbox);
		total_size += _crx_rfc822_len(current_address->host);
		nb_addresses++;
	} while ((current_address = current_address->next));

	/*
	 * rfc822_write_address_full() needs some extra space for '<>,', etc.
	 * for this purpose we allocate additional CRX_IMAP_ADDRESS_SIZE_BUF bytes
	 * by default this buffer is 10 bytes long
	 */
	total_size += nb_addresses * CRX_IMAP_ADDRESS_SIZE_BUF;
...
static crex_string* _crx_rfc822_write_address(ADDRESS *addresslist)
{
	...
	if (_crx_imap_address_size(addresslist) >= SENDBUFLEN) {
		crex_throw_error(NULL, "Address buffer overflow");
		return NULL;
	}
	address[0] = 0;
	rfc822_write_address(address, addresslist);
	return crex_string_init(address, strlen(address), 0);
}
/* }}} */
#endif
/* {{{ _crx_imap_parse_address */
static crex_string* _crx_imap_parse_address (ADDRESS *address_list, zval *paddress)
{
	crex_string *fulladdress;
	ADDRESS *addresstmp;

	addresstmp = address_list;

	fulladdress = _crx_rfc822_write_address(addresstmp);

	crx_imap_construct_list_of_addresses(C_ARR_P(paddress), address_list);
	return fulladdress;
}
/* }}} */

static void crx_imap_update_property_with_parsed_full_address(
...
{
	zval address_parts;
	crex_string *full_address = NULL;

	array_init(&address_parts);
	full_address = _crx_imap_parse_address(address, &address_parts);
	if (full_address) {
		crex_update_property_str(
			C_OBJCE_P(z_object), C_OBJ_P(z_object),
...
#define UPDATE_PROPERTY_PARSED_ADDRESS(z_object, name, envelope) if ((envelope)->name) { \
	crx_imap_update_property_with_parsed_full_address( \
		(z_object), (envelope)->name, \
		#name "address", strlen(#name "address"), \
		#name, strlen(#name) \
	); }

/* {{{ _crx_make_header_object */
static void _crx_make_header_object(zval *myzvalue, ENVELOPE *en)
{
	...
	if (en->remail) {
		crex_update_property_string(
			C_OBJCE_P(myzvalue), C_OBJ_P(myzvalue),
...
/* {{{ _crx_imap_add_body */
void _crx_imap_add_body(zval *arg, const BODY *body)
{
	crx_imap_populate_body_struct_object(arg, body);
	...
	if (body->type == TYPEMULTIPART) {
		...
		for (const PART *content_part = body->CONTENT_PART; content_part; content_part = content_part->next) {
			...
			_crx_imap_add_body(&z_content_part, &content_part->body);
			crex_hash_next_index_insert_new(C_ARR(z_content_part_list), &z_content_part);
		}

		crex_update_property(
			C_OBJCE_P(arg), C_OBJ_P(arg),
			"parts", strlen("parts"),
			&z_content_part_list
		);
	...
	if ((body->type == TYPEMESSAGE) && (!strcasecmp(body->subtype, "rfc822"))) {
		...
		object_init(&message);
		_crx_imap_add_body(&message, message_body);
		crex_hash_next_index_insert_new(C_ARR(message_list), &message);
...
CRX_FUNCTION(imap_thread)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O|l", &imap_conn_obj, crx_imap_ce, &flags) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (top == NIL) {
		crx_error_docref(NULL, E_WARNING, "Function returned an empty tree");
		RETURN_FALSE;
	}
...
CRX_FUNCTION(imap_timeout)
{
	crex_long ttype, timeout=-1;
	int timeout_type;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|l", &ttype, &timeout) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (timeout == -1) {
		...
		timeout = (crex_long) mail_parameters(NIL, timeout_type, NIL);
		RETURN_LONG(timeout);
	} else if (timeout >= 0) {
		switch (ttype) {
			case 1:
		...
		timeout = (crex_long) mail_parameters(NIL, timeout_type, (void *) timeout);
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
	...
	if (IMAPG(gets_stream)) {
		...
		while (size) {
			...
			if (!f(stream, read, buf)) {
				crx_error_docref(NULL, E_WARNING, "Failed to read from socket");
				break;
			} else if (read != crx_stream_write(IMAPG(gets_stream), buf, read)) {
				crx_error_docref(NULL, E_WARNING, "Failed to write to stream");
				break;
			}
		}
		return NULL;
...
CRX_IMAP_EXPORT void mm_searched(MAILSTREAM *stream, unsigned long number)
{
	MESSAGELIST *cur = NIL;
...
CRX_IMAP_EXPORT void mm_exists(MAILSTREAM *stream, unsigned long number)
{
}

CRX_IMAP_EXPORT void mm_expunged(MAILSTREAM *stream, unsigned long number)
{
}

CRX_IMAP_EXPORT void mm_flags(MAILSTREAM *stream, unsigned long number)
{
}

/* Author: CJH */
CRX_IMAP_EXPORT void mm_notify(MAILSTREAM *stream, char *str, long errflg)
{
	STRINGLIST *cur = NIL;
...
CRX_IMAP_EXPORT void mm_list(MAILSTREAM *stream, DTYPE delimiter, char *mailbox, long attributes)
{
	STRINGLIST *cur=NIL;
...
CRX_IMAP_EXPORT void mm_lsub(MAILSTREAM *stream, DTYPE delimiter, char *mailbox, long attributes)
{
	STRINGLIST *cur=NIL;
	FOBJECTLIST *ocur=NIL;
...
CRX_IMAP_EXPORT void mm_status(MAILSTREAM *stream, char *mailbox, MAILSTATUS *status)
{
...
CRX_IMAP_EXPORT void mm_log(char *str, long errflg)
{
	ERRORLIST *cur = NIL;
...
CRX_IMAP_EXPORT void mm_dlog(char *str)
{
...
CRX_IMAP_EXPORT void mm_login(NETMBX *mb, char *user, char *pwd, long trial)
{
...
CRX_IMAP_EXPORT void mm_critical(MAILSTREAM *stream)
{
}

CRX_IMAP_EXPORT void mm_nocritical(MAILSTREAM *stream)
{
}

CRX_IMAP_EXPORT long mm_diskerror(MAILSTREAM *stream, long errcode, long serious)
{
	return 1;
}

CRX_IMAP_EXPORT void mm_fatal(char *str)
{
}
/* }}} */
