...
static int pdo_sqlite_stmt_execute(pdo_stmt_t *stmt)
{
	...
	switch (sqlite3_step(S->stmt)) {
		case SQLITE_ROW:
			S->pre_fetched = 1;
			crx_pdo_stmt_set_column_count(stmt, sqlite3_data_count(S->stmt));
			return 1;

		case SQLITE_DONE:
			crx_pdo_stmt_set_column_count(stmt, sqlite3_column_count(S->stmt));
				...
				switch (PDO_PARAM_TYPE(param->param_type)) {
					...
					case PDO_PARAM_BOOL:
						...
						if (C_TYPE_P(parameter) == IS_NULL) {
							...
							if (SQLITE_OK == sqlite3_bind_int64(S->stmt, param->paramno + 1, C_LVAL_P(parameter))) {
								return 1;
							}
#else
							if (SQLITE_OK == sqlite3_bind_int(S->stmt, param->paramno + 1, C_LVAL_P(parameter))) {
								return 1;
							}
#endif
						}
					...
					case PDO_PARAM_LOB:
						...
						} else if (C_TYPE_P(parameter) == IS_NULL) {
							if (sqlite3_bind_null(S->stmt, param->paramno + 1) == SQLITE_OK) {
								return 1;
							}
							pdo_sqlite_error_stmt(stmt);
...
static int pdo_sqlite_stmt_describe(pdo_stmt_t *stmt, int colno)
{
	...
	stmt->columns[colno].name = crex_string_init(str, strlen(str), 0);
		...
		pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type)
{
	...
	switch (sqlite3_column_type(S->stmt, colno)) {
		...
		case SQLITE_INTEGER: {
			...
			if (i > CREX_LONG_MAX || i < CREX_LONG_MIN) {
				ZVAL_STRINGL(result,
...
static int pdo_sqlite_stmt_col_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value)
{
	pdo_sqlite_stmt *S = (pdo_sqlite_stmt*)stmt->driver_data;
	const char *str;
	zval flags;
	...
	switch (sqlite3_column_type(S->stmt, colno)) {
		case SQLITE_NULL:
			add_assoc_str(return_value, "native_type", ZSTR_KNOWN(CREX_STR_NULL_LOWERCASE));
			add_assoc_long(return_value, "pdo_type", PDO_PARAM_NULL);
			break;

		case SQLITE_FLOAT:
			add_assoc_str(return_value, "native_type", ZSTR_KNOWN(CREX_STR_DOUBLE));
		...
		case SQLITE_BLOB:
			...
			CREX_FALLTHROUGH;
		case SQLITE_TEXT:
			add_assoc_str(return_value, "native_type", ZSTR_KNOWN(CREX_STR_STRING));
			add_assoc_long(return_value, "pdo_type", PDO_PARAM_STR);
			break;

		case SQLITE_INTEGER:
			add_assoc_str(return_value, "native_type", ZSTR_KNOWN(CREX_STR_INTEGER));
...
static int pdo_sqlite_stmt_get_attribute(pdo_stmt_t *stmt, crex_long attr, zval *val)
{
	pdo_sqlite_stmt *S = (pdo_sqlite_stmt*)stmt->driver_data;
