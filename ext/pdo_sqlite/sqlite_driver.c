...
static void pdo_sqlite_cleanup_callbacks(pdo_sqlite_db_handle *H)
{
	...
	while (H->funcs) {
		...
		if (!C_ISUNDEF(func->func)) {
			zval_ptr_dtor(&func->func);
		}
		if (!C_ISUNDEF(func->step)) {
			zval_ptr_dtor(&func->step);
		}
		if (!C_ISUNDEF(func->fini)) {
	...
	while (H->collations) {
		...
		if (!C_ISUNDEF(collation->callback)) {
...
static bool sqlite_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
...
static crex_long sqlite_handle_doer(pdo_dbh_t *dbh, const crex_string *sql)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
...
static crex_string *pdo_sqlite_last_insert_id(pdo_dbh_t *dbh, const crex_string *name)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;

	return crex_i64_to_str(sqlite3_last_insert_rowid(H->db));
}

/* NB: doesn't handle binary strings... use prepared stmts for that */
static crex_string* sqlite_handle_quoter(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype)
{
	char *quoted;
	...
	crex_string *quoted_str = crex_string_init(quoted, strlen(quoted), 0);
	efree(quoted);
	return quoted_str;
}
...
static int pdo_sqlite_get_attribute(pdo_dbh_t *dbh, crex_long attr, zval *return_value)
{
	switch (attr) {
...
static bool pdo_sqlite_set_attr(pdo_dbh_t *dbh, crex_long attr, zval *val)
{
	pdo_sqlite_db_handle *H = (pdo_sqlite_db_handle *)dbh->driver_data;
	crex_long lval;
		...
		int is_agg)
{
	...
	if ((ret = crex_call_function(&fc->fci, &fc->fcc)) == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "An error occurred while invoking the callback");
	}
	...
	if (!is_agg || !argv) {
		...
		if (!C_ISUNDEF(retval)) {
			switch (C_TYPE(retval)) {
				...
				default:
					...
					sqlite3_result_text(context, C_STRVAL(retval), C_STRLEN(retval), SQLITE_TRANSIENT);
					break;
			}
		} else {
		...
		if (agg_context) {
			...
			zval_ptr_dtor(C_REFVAL(agg_context->val));
			ZVAL_COPY_VALUE(C_REFVAL(agg_context->val), &retval);
			ZVAL_UNDEF(&retval);
		}
	}
...
static void crx_sqlite3_func_callback(sqlite3_context *context, int argc,
	sqlite3_value **argv)
{
...
static void crx_sqlite3_func_step_callback(sqlite3_context *context, int argc,
	sqlite3_value **argv)
{
	struct pdo_sqlite_func *func = (struct pdo_sqlite_func*)sqlite3_user_data(context);
...
static void crx_sqlite3_func_final_callback(sqlite3_context *context)
{
	struct pdo_sqlite_func *func = (struct pdo_sqlite_func*)sqlite3_user_data(context);
...
static int crx_sqlite3_collation_callback(void *context,
	...
	int string2_len, const void *string2)
{
	...
	if ((ret = crex_call_function(&collation->fc.fci, &collation->fc.fcc)) == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "An error occurred while invoking the callback");
...
CRX_METHOD(PDO_SQLite_Ext, sqliteCreateFunction)
{
	struct pdo_sqlite_func *func;
	crex_fcall_info fci;
	...
	CREX_PARSE_PARAMETERS_START(2, 4)
		C_PARAM_STRING(func_name, func_name_len)
	...
	ret = sqlite3_create_function(H->db, func_name, argc, flags | SQLITE_UTF8,
			func, crx_sqlite3_func_callback, NULL, NULL);
	if (ret == SQLITE_OK) {
...
CRX_METHOD(PDO_SQLite_Ext, sqliteCreateAggregate)
{
	struct pdo_sqlite_func *func;
	...
	CREX_PARSE_PARAMETERS_START(3, 4)
		C_PARAM_STRING(func_name, func_name_len)
		C_PARAM_FUNC(step_fci, step_fcc)
		C_PARAM_FUNC(fini_fci, fini_fcc)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(argc)
	CREX_PARSE_PARAMETERS_END();
	...
	ret = sqlite3_create_function(H->db, func_name, argc, SQLITE_UTF8,
			func, NULL, crx_sqlite3_func_step_callback, crx_sqlite3_func_final_callback);
	if (ret == SQLITE_OK) {
		func->funcname = estrdup(func_name);
...
CRX_METHOD(PDO_SQLite_Ext, sqliteCreateCollation)
{
	struct pdo_sqlite_collation *collation;
	crex_fcall_info fci;
	crex_fcall_info_cache fcc;
	...
	CREX_PARSE_PARAMETERS_START(2, 2)
		C_PARAM_STRING(collation_name, collation_name_len)
		C_PARAM_FUNC(fci, fcc)
	CREX_PARSE_PARAMETERS_END();
	...
	ret = sqlite3_create_collation(H->db, collation_name, SQLITE_UTF8, collation, crx_sqlite3_collation_callback);
	if (ret == SQLITE_OK) {
...
static const crex_function_entry *get_driver_methods(pdo_dbh_t *dbh, int kind)
{
	switch (kind) {
...
static void pdo_sqlite_get_gc(pdo_dbh_t *dbh, crex_get_gc_buffer *gc_buffer)
{
	pdo_sqlite_db_handle *H = dbh->driver_data;

	struct pdo_sqlite_func *func = H->funcs;
	while (func) {
		crex_get_gc_buffer_add_zval(gc_buffer, &func->func);
		crex_get_gc_buffer_add_zval(gc_buffer, &func->step);
		crex_get_gc_buffer_add_zval(gc_buffer, &func->fini);
		func = func->next;
	}

	struct pdo_sqlite_collation *collation = H->collations;
	while (collation) {
		crex_get_gc_buffer_add_zval(gc_buffer, &collation->callback);
...
static int pdo_sqlite_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	...
	if (!filename) {
		crex_throw_exception_ex(crx_pdo_get_exception(), 0,
