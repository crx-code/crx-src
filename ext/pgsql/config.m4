...
if test "$CRX_PGSQL" != "no"; then
  ...
  for i in $CRX_PGSQL $CRX_PGSQL/bin /usr/local/pgsql/bin /usr/local/bin /usr/bin ""; do
  ...
  if test -n "$PG_CONFIG"; then
    ...
    if test "$CRX_PGSQL" = "yes"; then
      PGSQL_SEARCH_PATHS="/usr /usr/local /usr/local/pgsql"
    else
      PGSQL_SEARCH_PATHS=$CRX_PGSQL
    fi
    ...
    for i in $PGSQL_SEARCH_PATHS; do
      ...
      for j in lib $CRX_LIBDIR/pgsql $CRX_LIBDIR/postgres $CRX_LIBDIR/postgresql ""; do
  ...
  CRX_ADD_LIBRARY_WITH_PATH(pq, $PGSQL_LIBDIR, PGSQL_SHARED_LIBADD)
  CRX_SUBST(PGSQL_SHARED_LIBADD)

  CRX_ADD_INCLUDE($PGSQL_INCLUDE)

  CRX_NEW_EXTENSION(pgsql, pgsql.c, $ext_shared,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)
fi
