...
static int pdo_odbc_utf82ucs2(pdo_stmt_t *stmt, int is_unicode, const char *buf,
	crex_ulong buflen, crex_ulong *outlen)
{
#ifdef CRX_WIN32
...
{
#ifdef CRX_WIN32
	CREX_ASSERT(C_TYPE_P(result) == IS_STRING);
	if (is_unicode && C_STRLEN_P(result) != 0) {
		pdo_odbc_stmt *S = (pdo_odbc_stmt*)stmt->driver_data;
		DWORD ret;

		ret = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR) C_STRVAL_P(result), C_STRLEN_P(result)/sizeof(WCHAR), NULL, 0, NULL, NULL);
		if (ret == 0) {
			return PDO_ODBC_CONV_FAIL;
		}

		crex_string *str = crex_string_alloc(ret, 0);
		ret = WideCharToMultiByte(CP_UTF8, 0, (LPCWSTR) C_STRVAL_P(result), C_STRLEN_P(result)/sizeof(WCHAR), ZSTR_VAL(str), ZSTR_LEN(str), NULL, NULL);
		if (ret == 0) {
			return PDO_ODBC_CONV_FAIL;
		}
		...
		if (rc == SQL_NEED_DATA) {
			...
			if (C_TYPE_P(parameter) != IS_RESOURCE) {
				...
				switch (pdo_odbc_utf82ucs2(stmt, P->is_unicode,
							C_STRVAL_P(parameter),
							C_STRLEN_P(parameter),
							&ulen)) {
					case PDO_ODBC_CONV_NOT_REQUIRED:
						rc1 = SQLPutData(S->stmt, C_STRVAL_P(parameter),
							C_STRLEN_P(parameter));
			...
			do {
				len = crx_stream_read(stm, buf, 8192);
				if (len == 0) {
					break;
				}
			...
			case PDO_PARAM_EVT_EXEC_PRE:
				...
				if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_LOB) {
					if (C_TYPE_P(parameter) == IS_RESOURCE) {
						...
						if (0 == crx_stream_stat(stm, &sb)) {
							if (P->outbuf) {
								...
								do {
									...
									if (amount > 8192)
										amount = 8192;
									len = crx_stream_read(stm, ptr, amount);
									if (len == 0) {
										break;
									}
									ptr += len;
						...
						convert_to_string(parameter);
						if (P->outbuf) {
							P->len = C_STRLEN_P(parameter);
							memcpy(P->outbuf, C_STRVAL_P(parameter), P->len);
						} else {
							P->len = SQL_LEN_DATA_AT_EXEC(C_STRLEN_P(parameter));
						}
					}
				} else if (C_TYPE_P(parameter) == IS_NULL || PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_NULL) {
					P->len = SQL_NULL_DATA;
				} else {
					convert_to_string(parameter);
					if (P->outbuf) {
...
static int odbc_stmt_describe(pdo_stmt_t *stmt, int colno)
{
	...
	col->name = crex_string_init(S->cols[colno].colname, colnamelen, 0);
...
static int odbc_stmt_get_column_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value)
{
...
static int odbc_stmt_get_col(pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type)
{
	...
	if (C->is_long) {
		...
		if (rc == SQL_SUCCESS_WITH_INFO || rc == SQL_SUCCESS) {
			...
			crex_string *str = crex_string_init(C->data, 256, 0);
			...
			do {
				...
				if (rc==SQL_SUCCESS_WITH_INFO || (rc==SQL_SUCCESS && C->fetched_len > 255)) {
					...
					str = crex_string_realloc(str, used + 256, 0);
					memcpy(ZSTR_VAL(str) + used, buf2, 256);
					used = used + 255;
				} else if (rc==SQL_SUCCESS) {
					str = crex_string_realloc(str, used + C->fetched_len, 0);
			...
			/* NULL terminate the buffer once, when finished, for use with the rest of CRX */
			ZSTR_VAL(str)[used] = '\0';
			ZVAL_STR(result, str);
			if (C->is_unicode) {
				goto unicode_conv;
...
static int odbc_stmt_set_param(pdo_stmt_t *stmt, crex_long attr, zval *val)
{
	SQLRETURN rc;
	pdo_odbc_stmt *S = (pdo_odbc_stmt*)stmt->driver_data;
	...
	switch (attr) {
		case PDO_ATTR_CURSOR_NAME:
			...
			rc = SQLSetCursorName(S->stmt, (SQLCHAR *) C_STRVAL_P(val), C_STRLEN_P(val));
...
static int odbc_stmt_get_attr(pdo_stmt_t *stmt, crex_long attr, zval *val)
{
	SQLRETURN rc;
	pdo_odbc_stmt *S = (pdo_odbc_stmt*)stmt->driver_data;
