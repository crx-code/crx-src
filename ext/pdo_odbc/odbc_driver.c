...
/* printf("@@ SQLSTATE[%s] %s\n", *pdo_err, einfo->last_err_msg); */
	if (!dbh->methods) {
		crex_throw_exception_ex(crx_pdo_get_exception(), einfo->last_error, "SQLSTATE[%s] %s: %d %s",
...
static bool odbc_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options)
{
	RETCODE rc;
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
...
static crex_long odbc_handle_doer(pdo_dbh_t *dbh, const crex_string *sql)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	RETCODE rc;
	SQLLEN row_count = -1;
...
static bool odbc_handle_set_attr(pdo_dbh_t *dbh, crex_long attr, zval *val)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	bool bval;
...
static int odbc_handle_get_attr(pdo_dbh_t *dbh, crex_long attr, zval *val)
{
	pdo_odbc_db_handle *H = (pdo_odbc_db_handle *)dbh->driver_data;
	switch (attr) {
...
static crex_result odbc_handle_check_liveness(pdo_dbh_t *dbh)
{
	RETCODE ret;
	UCHAR d_name[32];
	SQLSMALLINT len;
...
static int pdo_odbc_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	...
	if (strchr(dbh->data_source, '=')) {
		...
		if (is_uid_set && is_pwd_set) {
			...
			bool should_quote_uid = !crx_odbc_connstr_is_quoted(dbh->username) && crx_odbc_connstr_should_quote(dbh->username);
			bool should_quote_pwd = !crx_odbc_connstr_is_quoted(dbh->password) && crx_odbc_connstr_should_quote(dbh->password);
			if (should_quote_uid) {
				size_t estimated_length = crx_odbc_connstr_estimate_quote_length(dbh->username);
				uid = emalloc(estimated_length);
				crx_odbc_connstr_quote(uid, dbh->username, estimated_length);
			} else {
				uid = dbh->username;
			}
			if (should_quote_pwd) {
				size_t estimated_length = crx_odbc_connstr_estimate_quote_length(dbh->password);
				pwd = emalloc(estimated_length);
				crx_odbc_connstr_quote(pwd, dbh->password, estimated_length);
			} else {
				pwd = dbh->password;
			}
