...
if test "$CRX_FFI" != "no"; then
  PKG_CHECK_MODULES([FFI], [libffi >= 3.0.11])

  CRX_EVAL_INCLINE($FFI_CFLAGS)
  CRX_EVAL_LIBLINE($FFI_LIBS, FFI_SHARED_LIBADD)
  ...
  CRX_NEW_EXTENSION(ffi, ffi.c ffi_parser.c, $ext_shared,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)
  CRX_SUBST(FFI_SHARED_LIBADD)
fi
