...
if test "$CRX_TIDY" != "no"; then

  if test "$CRX_TIDY" != "yes"; then
    TIDY_SEARCH_DIRS=$CRX_TIDY
  else
  ...
  TIDY_LIBDIR=$TIDY_DIR/$CRX_LIBDIR
  ...
  CRX_CHECK_LIBRARY($TIDY_LIB_NAME,tidyOptGetDoc,
  [
    AC_DEFINE(HAVE_TIDYOPTGETDOC,1,[ ])
  ],[
    CRX_CHECK_LIBRARY(tidy5,tidyOptGetDoc,
    [
      TIDY_LIB_NAME=tidy5
      AC_DEFINE(HAVE_TIDYOPTGETDOC,1,[ ])
    ], [], [])
  ...
  CRX_CHECK_LIBRARY($TIDY_LIB_NAME,tidyReleaseDate,
  [
    AC_DEFINE(HAVE_TIDYRELEASEDATE,1,[ ])
  ], [], [])

  CRX_ADD_LIBRARY_WITH_PATH($TIDY_LIB_NAME, $TIDY_LIBDIR, TIDY_SHARED_LIBADD)
  CRX_ADD_INCLUDE($TIDY_INCDIR)

  dnl Add -Wno-ignored-qualifiers as this is an issue upstream
  TIDY_COMPILER_FLAGS="$TIDY_CFLAGS -Wno-ignored-qualifiers -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1"
  CRX_NEW_EXTENSION(tidy, tidy.c, $ext_shared,, $TIDY_COMPILER_FLAGS)
  CRX_SUBST(TIDY_SHARED_LIBADD)
