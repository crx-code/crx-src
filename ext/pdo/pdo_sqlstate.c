...
void pdo_sqlstate_init_error_table(void)
{
	...
	crex_hash_init(&err_hash, sizeof(err_initializer)/sizeof(err_initializer[0]), NULL, NULL, 1);

	for (i = 0; i < sizeof(err_initializer)/sizeof(err_initializer[0]); i++) {
		info = &err_initializer[i];

		crex_hash_str_add_ptr(&err_hash, info->state, sizeof(info->state), (void *)info);
	}
}
...
{
	const struct pdo_sqlstate_info *info;
	if ((info = crex_hash_str_find_ptr(&err_hash, state, sizeof(err_initializer[0].state))) != NULL) {
		return info->desc;
	}
	return NULL;
}
