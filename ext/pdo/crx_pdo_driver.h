...
#ifndef CRX_PDO_DRIVER_H
#define CRX_PDO_DRIVER_H
...
static inline crex_long pdo_attr_lval(zval *options, unsigned option_name, crex_long defval)
{
	zval *v;

	if (options && (v = crex_hash_index_find(C_ARRVAL_P(options), option_name))) {
		return zval_get_long(v);
	}
	return defval;
}
static inline crex_string *pdo_attr_strval(zval *options, unsigned option_name, crex_string *defval)
{
	zval *v;
	...
	return defval ? crex_string_copy(defval) : NULL;
}
/* }}} */
...
typedef struct {
	...
	crex_ulong	api_version; /* needs to be compatible with PDO */
...
#define PDO_DRIVER_HEADER(name)	\
	 ...
	 * data in the db, otherwise you will crash CRX when persistent connections
	 * are used.
	 */
...
typedef bool (*pdo_dbh_prepare_func)(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options);

/* execute a statement (that does not return a result set)
 * Return -1 on failure, otherwise the number of affected rows */
typedef crex_long (*pdo_dbh_do_func)(pdo_dbh_t *dbh, const crex_string *sql);

/* quote a string */
typedef crex_string* (*pdo_dbh_quote_func)(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype);
...
typedef bool (*pdo_dbh_set_attr_func)(pdo_dbh_t *dbh, crex_long attr, zval *val);

/* return last insert id.  NULL indicates error condition.
 * name MIGHT be NULL */
typedef crex_string *(*pdo_dbh_last_id_func)(pdo_dbh_t *dbh, const crex_string *name);

/* Fetch error information.
 * If stmt is not null, fetch information pertaining to the statement,
 * otherwise fetch global error information.
 * info is an initialized CRX array, if there are no messages leave it empty.
...
typedef int (*pdo_dbh_get_attr_func)(pdo_dbh_t *dbh, crex_long attr, zval *val);
...
typedef crex_result (*pdo_dbh_check_liveness_func)(pdo_dbh_t *dbh);
...
typedef void (*pdo_dbh_get_gc_func)(pdo_dbh_t *dbh, crex_get_gc_buffer *buffer);
...
typedef const crex_function_entry *(*pdo_dbh_get_driver_methods_func)(pdo_dbh_t *dbh, int kind);
...
typedef int (*pdo_stmt_fetch_func)(pdo_stmt_t *stmt,
	enum pdo_fetch_orientation ori, crex_long offset);
...
/* setting of attributes */
typedef int (*pdo_stmt_set_attr_func)(pdo_stmt_t *stmt, crex_long attr, zval *val);

/* fetching of attributes */
typedef int (*pdo_stmt_get_attr_func)(pdo_stmt_t *stmt, crex_long attr, zval *val);
 ...
 *   type => a string representation of the type, mapped to the CRX equivalent type name
 ...
 */
typedef int (*pdo_stmt_get_column_meta_func)(pdo_stmt_t *stmt, crex_long colno, zval *return_value);
...
struct _pdo_dbh_object_t {
	...
	/* these items must appear in this order at the beginning of the
       struct so that this can be cast as a crex_object.  we need this
...
static inline pdo_dbh_t *crx_pdo_dbh_fetch_inner(crex_object *obj) {
	return (pdo_dbh_t *)(((pdo_dbh_object_t *)((char*)(obj) - XtOffsetOf(pdo_dbh_object_t, std)))->inner);
}

static inline pdo_dbh_object_t *crx_pdo_dbh_fetch_object(crex_object *obj) {
	return (pdo_dbh_object_t *)((char*)(obj) - XtOffsetOf(pdo_dbh_object_t, std));
}

#define C_PDO_DBH_P(zv) crx_pdo_dbh_fetch_inner(C_OBJ_P((zv)))
#define C_PDO_OBJECT_P(zv) crx_pdo_dbh_fetch_object(C_OBJ_P((zv)))

/* describes a column */
struct pdo_column_data {
	crex_string *name;
	size_t maxlen;
	crex_ulong precision;
};
...
struct pdo_bound_param_data {
	...
	crex_long paramno; /* if -1, then it has a name, and we don't know the index *yet* */
	crex_string *name;

	crex_long max_value_len;	/* as a hint for pre-allocation */
...
struct _pdo_stmt_t {
	...
	/* keep track of CRX variables bound to named (or positional) columns
	 * in the result set */
	...
	crex_string *query_string;

	/* the copy of the query with expanded binds ONLY for emulated-prepare drivers */
	crex_string *active_query_string;
...
static inline pdo_stmt_t *crx_pdo_stmt_fetch_object(crex_object *obj) {
	return (pdo_stmt_t *)((char*)(obj) - XtOffsetOf(pdo_stmt_t, std));
}

#define C_PDO_STMT_P(zv) crx_pdo_stmt_fetch_object(C_OBJ_P((zv)))
...
PDO_API int crx_pdo_parse_data_source(const char *data_source,
		crex_ulong data_source_len, struct pdo_data_src_parser *parsed,
		int nparams);

PDO_API crex_class_entry *crx_pdo_get_dbh_ce(void);
PDO_API crex_class_entry *crx_pdo_get_exception(void);

PDO_API int pdo_parse_params(pdo_stmt_t *stmt, crex_string *inquery, crex_string **outquery);

PDO_API void pdo_raise_impl_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt,
	pdo_error_type sqlstate, const char *supp);

PDO_API void crx_pdo_dbh_addref(pdo_dbh_t *dbh);
PDO_API void crx_pdo_dbh_delref(pdo_dbh_t *dbh);
