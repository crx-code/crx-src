...
struct placeholder {
	const char *pos;
	size_t len;
	crex_string *quoted;	/* quoted value */
	int bindno;
	struct placeholder *next;
};

static void free_param_name(zval *el) {
	crex_string_release(C_PTR_P(el));
}
...
PDO_API int pdo_parse_params(pdo_stmt_t *stmt, crex_string *inquery, crex_string **outquery)
{
	...
	if (stmt->supports_placeholders == PDO_PLACEHOLDER_NONE && params && bindno != crex_hash_num_elements(params)) {
		/* extra bit of validation for instances when same params are bound more than once */
		if (query_type != PDO_PLACEHOLDER_POSITIONAL && bindno > crex_hash_num_elements(params)) {
			int ok = 1;
			for (plc = placeholders; plc; plc = plc->next) {
				if ((param = crex_hash_str_find_ptr(params, plc->pos, plc->len)) == NULL) {
					ok = 0;
					break;
				}
			}
	...
	if (stmt->supports_placeholders == PDO_PLACEHOLDER_NONE) {
		...
		for (plc = placeholders; plc && params; plc = plc->next) {
			...
			if (query_type == PDO_PLACEHOLDER_POSITIONAL) {
				param = crex_hash_index_find_ptr(params, plc->bindno);
			} else {
				param = crex_hash_str_find_ptr(params, plc->pos, plc->len);
			}
			if (param == NULL) {
			...
			if (stmt->dbh->methods->quoter) {
				...
				if (param->param_type == PDO_PARAM_LOB && C_TYPE_P(parameter) == IS_RESOURCE) {
					crx_stream *stm;
					...
					if (stm) {
						...
						buf = crx_stream_copy_to_mem(stm, CRX_STREAM_COPY_ALL, 0);
						if (!buf) {
					...
					switch (param_type) {
						case PDO_PARAM_BOOL:
							plc->quoted = crex_is_true(parameter) ? ZSTR_CHAR('1') : ZSTR_CHAR('0');
							break;

						case PDO_PARAM_INT:
							plc->quoted = crex_long_to_str(zval_get_long(parameter));
							break;

						case PDO_PARAM_NULL:
							plc->quoted = ZSTR_KNOWN(CREX_STR_NULL);
							break;
				...
				plc->quoted = crex_string_copy(C_STR_P(parameter));
			}
...
rewrite:
		...
		*outquery = crex_string_alloc(newbuffer_len, 0);
		...
		*newbuffer = '\0';
		...
		if (stmt->bound_param_map == NULL) {
			...
			crex_hash_init(stmt->bound_param_map, 13, NULL, free_param_name, 0);
		}
		...
		for (plc = placeholders; plc; plc = plc->next) {
			...
			crex_string *name = crex_string_init(plc->pos, plc->len, 0);

			/* check if bound parameter is already available */
			if (crex_string_equals_literal(name, "?") || (p = crex_hash_find_ptr(stmt->bound_param_map, name)) == NULL) {
				idxbuf = crex_strpprintf(0, tmpl, bind_no++);
			} else {
				idxbuf = crex_string_copy(p);
				skip_map = 1;
			}
			...
			if (!skip_map && stmt->named_rewrite_template) {
				/* create a mapping */
				crex_hash_update_ptr(stmt->bound_param_map, name, crex_string_copy(plc->quoted));
			}

			/* map number to name */
			crex_hash_index_update_ptr(stmt->bound_param_map, plc->bindno, crex_string_copy(plc->quoted));
		...
		for (plc = placeholders; plc; plc = plc->next) {
			crex_string *name = crex_string_init(plc->pos, plc->len, 0);
			crex_hash_index_update_ptr(stmt->bound_param_map, plc->bindno, name);
			plc->quoted = ZSTR_CHAR('?');
...
clean_up:
	...
	while (placeholders) {
		...
		if (plc->quoted) {
			crex_string_release_ex(plc->quoted, 0);
		}
		efree(plc);
	}
