...
static bool pdo_dbh_attribute_set(pdo_dbh_t *dbh, crex_long attr, zval *value);
...
void pdo_throw_exception(unsigned int driver_errcode, char *driver_errmsg, pdo_error_type *pdo_error)
{
		...
		object_init_ex(&pdo_exception, crx_pdo_get_exception());
		...
		crex_update_property(crx_pdo_get_exception(), C_OBJ(pdo_exception), "errorInfo", sizeof("errorInfo")-1, &error_info);
		crex_update_property_long(crx_pdo_get_exception(), C_OBJ(pdo_exception), "code", sizeof("code")-1, driver_errcode);
		crex_update_property_string(
			crx_pdo_get_exception(),
			C_OBJ(pdo_exception),
			"message",
			sizeof("message")-1,
		...
		crex_throw_exception_object(&pdo_exception);
}
...
void pdo_raise_impl_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, pdo_error_type sqlstate, const char *supp) /* {{{ */
{
	...
	if (dbh && dbh->error_mode != PDO_ERRMODE_EXCEPTION) {
		...
		zval ex, info;
		crex_class_entry *pdo_ex = crx_pdo_get_exception();

		object_init_ex(&ex, pdo_ex);

		crex_update_property_string(crex_ce_exception, C_OBJ(ex), "message", sizeof("message")-1, message);
		crex_update_property_string(crex_ce_exception, C_OBJ(ex), "code", sizeof("code")-1, *pdo_err);

		array_init(&info);

		add_next_index_string(&info, *pdo_err);
		add_next_index_long(&info, 0);
		crex_update_property(pdo_ex, C_OBJ(ex), "errorInfo", sizeof("errorInfo")-1, &info);
		zval_ptr_dtor(&info);

		crex_throw_exception_object(&ex);
	}
...
PDO_API void pdo_handle_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt) /* {{{ */
{
	...
	if (dbh->methods->fetch_err) {
		...
		if ((item = crex_hash_index_find(C_ARRVAL(info), 1)) != NULL
				&& C_TYPE_P(item) == IS_LONG) {
			native_code = C_LVAL_P(item);
		}

		if ((item = crex_hash_index_find(C_ARRVAL(info), 2)) != NULL) {
			supp = estrndup(C_STRVAL_P(item), C_STRLEN_P(item));
		}
	}

	if (native_code && supp) {
		message = strpprintf(0, "SQLSTATE[%s]: %s: " CREX_LONG_FMT " %s", *pdo_err, msg, native_code, supp);
	} else if (supp) {
	...
	if (dbh->error_mode == PDO_ERRMODE_WARNING) {
		crx_error_docref(NULL, E_WARNING, "%s", ZSTR_VAL(message));
		...
		crex_update_property_str(crex_ce_exception, C_OBJ(ex), "message", sizeof("message") - 1, message);
		crex_update_property_string(crex_ce_exception, C_OBJ(ex), "code", sizeof("code") - 1, *pdo_err);
...
static char *dsn_from_uri(char *uri, char *buf, size_t buflen) /* {{{ */
{
	...
	stream = crx_stream_open_wrapper(uri, "rb", REPORT_ERRORS, NULL);
	if (stream) {
		dsn = crx_stream_get_line(stream, buf, buflen, NULL);
...
CRX_METHOD(PDO, __main)
{
	...
	CREX_PARSE_PARAMETERS_START(1, 4)
		C_PARAM_STRING(data_source, data_source_len)
		C_PARAM_OPTIONAL
		C_PARAM_STRING_OR_NULL(username, usernamelen)
		C_PARAM_STRING_OR_NULL(password, passwordlen)
		C_PARAM_ARRAY_OR_NULL(options)
	...
	if (!colon) {
		/* let's see if this string has a matching dsn in the crx.ini */
		char *ini_dsn = NULL;

		snprintf(alt_dsn, sizeof(alt_dsn), "pdo.dsn.%s", data_source);
		if (FAILURE == cfg_get_string(alt_dsn, &ini_dsn)) {
			crex_argument_error(crx_pdo_get_exception(), 1, "must be a valid data source name");
			RETURN_THROWS();
		}
		...
		if (!colon) {
			crex_throw_exception_ex(crx_pdo_get_exception(), 0, "invalid data source name (via INI: %s)", alt_dsn);
			RETURN_THROWS();
		}
	}
	...
	if (!strncmp(data_source, "uri:", sizeof("uri:")-1)) {
		...
		if (!data_source) {
			crex_argument_error(crx_pdo_get_exception(), 1, "must be a valid data source URI");
			RETURN_THROWS();
		}
		colon = strchr(data_source, ':');
		if (!colon) {
			crex_argument_error(crx_pdo_get_exception(), 1, "must be a valid data source name (via URI)");
			RETURN_THROWS();
		}
	}
	...
	if (!driver) {
		...
		crex_throw_exception_ex(crx_pdo_get_exception(), 0, "could not find driver");
		RETURN_THROWS();
	}

	dbh = C_PDO_DBH_P(object);
	...
	if (options) {
		...
		if ((v = crex_hash_index_find_deref(C_ARRVAL_P(options), PDO_ATTR_PERSISTENT)) != NULL) {
			if (C_TYPE_P(v) == IS_STRING &&
				!is_numeric_string(C_STRVAL_P(v), C_STRLEN_P(v), NULL, NULL, 0) && C_STRLEN_P(v) > 0) {
				/* user specified key */
		...
		if (is_persistent) {
			...
			if ((le = crex_hash_str_find_ptr(&EG(persistent_list), hashkey, plen)) != NULL) {
				if (le->type == crx_pdo_list_entry()) {
		...
		if (pdbh) {
			...
			C_PDO_OBJECT_P(object)->inner = pdbh;
			pdbh->refcount++;
			dbh = pdbh;
		}
	...
	if (!dbh->data_source || (username && !dbh->username) || (password && !dbh->password)) {
		crx_error_docref(NULL, E_ERROR, "Out of memory");
	}
	...
	crex_replace_error_handling(EH_THROW, pdo_exception_ce, &zeh);
	...
	if (driver->db_handle_factory(dbh, options)) {
		...
		if (is_persistent) {
			...
			if ((crex_register_persistent_resource(
						(char*)dbh->persistent_id, dbh->persistent_id_len, dbh, crx_pdo_list_entry())) == NULL) {
				crx_error_docref(NULL, E_ERROR, "Failed to register persistent entry");
			}
		}
...
options:
		if (options) {
			zval *attr_value;
			crex_ulong long_key;
			crex_string *str_key = NULL;

			CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(options), long_key, str_key, attr_value) {
				if (str_key) {
					continue;
				}
	...
	crex_restore_error_handling(&zeh);
	if (!EG(exception)) {
		crex_throw_exception(pdo_exception_ce, "Constructor failed", 0);
	}
}
/* }}} */

static zval *pdo_stmt_instantiate(pdo_dbh_t *dbh, zval *object, crex_class_entry *dbstmt_ce, zval *ctor_args) /* {{{ */
{
	if (!C_ISUNDEF_P(ctor_args)) {
		/* This implies an error within PDO if this does not hold */
		CREX_ASSERT(C_TYPE_P(ctor_args) == IS_ARRAY);
	...
	if (UNEXPECTED(object_init_ex(object, dbstmt_ce) != SUCCESS)) {
		if (EXPECTED(!EG(exception))) {
			crex_throw_error(NULL, "Cannot instantiate user-supplied statement class");
		}
		return NULL;
	}
...
static void pdo_stmt_construct(crex_execute_data *execute_data, pdo_stmt_t *stmt, zval *object, crex_class_entry *dbstmt_ce, zval *ctor_args) /* {{{ */
{
	zval query_string;
	crex_string *key;
	...
	crex_std_write_property(C_OBJ_P(object), key, &query_string, NULL);
	...
	if (dbstmt_ce->constructor) {
		...
		crex_fcall_info_args(&fci, ctor_args);

		fcc.function_handler = dbstmt_ce->constructor;
		fcc.called_scope = C_OBJCE_P(object);
		fcc.object = C_OBJ_P(object);

		if (crex_call_function(&fci, &fcc) != FAILURE) {
...
CRX_METHOD(PDO, prepare)
{
	pdo_stmt_t *stmt;
	crex_string *statement;
	zval *options = NULL, *value, *item, ctor_args;
	crex_class_entry *dbstmt_ce, *pce;
	pdo_dbh_object_t *dbh_obj = C_PDO_OBJECT_P(CREX_THIS);
	pdo_dbh_t *dbh = dbh_obj->inner;

	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_STR(statement)
		C_PARAM_OPTIONAL
		C_PARAM_ARRAY(options)
	CREX_PARSE_PARAMETERS_END();
	...
	if (options && (value = crex_hash_index_find(C_ARRVAL_P(options), PDO_ATTR_STATEMENT_CLASS)) != NULL) {
		if (C_TYPE_P(value) != IS_ARRAY) {
			crex_type_error("PDO::ATTR_STATEMENT_CLASS value must be of type array, %s given",
				crex_zval_value_name(value));
			RETURN_THROWS();
		}
		if ((item = crex_hash_index_find(C_ARRVAL_P(value), 0)) == NULL) {
			crex_value_error("PDO::ATTR_STATEMENT_CLASS value must be an array with the format "
				"array(classname, constructor_args)");
			RETURN_THROWS();
		}
		if (C_TYPE_P(item) != IS_STRING || (pce = crex_lookup_class(C_STR_P(item))) == NULL) {
			crex_type_error("PDO::ATTR_STATEMENT_CLASS class must be a valid class");
			RETURN_THROWS();
		}
		dbstmt_ce = pce;
		if (!instanceof_function(dbstmt_ce, pdo_dbstmt_ce)) {
			crex_type_error("PDO::ATTR_STATEMENT_CLASS class must be derived from PDOStatement");
			RETURN_THROWS();
		}
		if (dbstmt_ce->constructor && !(dbstmt_ce->constructor->common.fn_flags & (CREX_ACC_PRIVATE|CREX_ACC_PROTECTED))) {
			crex_type_error("User-supplied statement class cannot have a public constructor");
			RETURN_THROWS();
		}
		if ((item = crex_hash_index_find(C_ARRVAL_P(value), 1)) != NULL) {
			if (C_TYPE_P(item) != IS_ARRAY) {
				crex_type_error("PDO::ATTR_STATEMENT_CLASS constructor_args must be of type ?array, %s given",
					crex_zval_value_name(value));
				RETURN_THROWS();
			}
	...
	stmt = C_PDO_STMT_P(return_value);

	/* unconditionally keep this for later reference */
	stmt->query_string = crex_string_copy(statement);
...
CRX_METHOD(PDO, beginTransaction)
{
	pdo_dbh_t *dbh = C_PDO_DBH_P(CREX_THIS);
	...
	if (pdo_is_in_transaction(dbh)) {
		crex_throw_exception_ex(crx_pdo_get_exception(), 0, "There is already an active transaction");
		RETURN_THROWS();
	}

	if (!dbh->methods->begin) {
		/* Throw an exception when the driver does not support transactions */
		crex_throw_exception_ex(crx_pdo_get_exception(), 0, "This driver doesn't support transactions");
		RETURN_THROWS();
	}
...
CRX_METHOD(PDO, commit)
{
	...
	if (!pdo_is_in_transaction(dbh)) {
		crex_throw_exception_ex(crx_pdo_get_exception(), 0, "There is no active transaction");
		RETURN_THROWS();
	}
...
PDO_API bool pdo_get_long_param(crex_long *lval, zval *value)
{
	switch (C_TYPE_P(value)) {
		...
		case IS_FALSE:
			*lval = zval_get_long(value);
			return true;
		case IS_STRING:
			if (IS_LONG == is_numeric_str_function(C_STR_P(value), lval, NULL)) {
				return true;
			}
			CREX_FALLTHROUGH;
		default:
			crex_type_error("Attribute value must be of type int for selected attribute, %s given", crex_zval_value_name(value));
			return false;
	}
}
PDO_API bool pdo_get_bool_param(bool *bval, zval *value)
{
	switch (C_TYPE_P(value)) {
		...
		default:
			crex_type_error("Attribute value must be of type bool for selected attribute, %s given", crex_zval_value_name(value));
			return false;
	}
}
...
static bool pdo_dbh_attribute_set(pdo_dbh_t *dbh, crex_long attr, zval *value) /* {{{ */
{
	...
	switch (attr) {
		case PDO_ATTR_ERRMODE:
			...
			switch (lval) {
				...
				case PDO_ERRMODE_EXCEPTION:
					...
					return true;
				default:
					crex_value_error("Error mode must be one of the PDO::ERRMODE_* constants");
					return false;
			}
			return false;
		...
		case PDO_ATTR_CASE:
			...
			switch (lval) {
				...
				case PDO_CASE_LOWER:
					...
					return true;
				default:
					crex_value_error("Case folding mode must be one of the PDO::CASE_* constants");
					return false;
			}
			return false;
		...
		case PDO_ATTR_DEFAULT_FETCH_MODE:
			if (C_TYPE_P(value) == IS_ARRAY) {
				zval *tmp;
				if ((tmp = crex_hash_index_find(C_ARRVAL_P(value), 0)) != NULL && C_TYPE_P(tmp) == IS_LONG) {
					if (C_LVAL_P(tmp) == PDO_FETCH_INTO || C_LVAL_P(tmp) == PDO_FETCH_CLASS) {
						crex_value_error("PDO::FETCH_INTO and PDO::FETCH_CLASS cannot be set as the default fetch mode");
						return false;
					}
				}
				lval = zval_get_long(value);
		...
		case PDO_ATTR_STATEMENT_CLASS: {
			...
			if (pce->constructor && !(pce->constructor->common.fn_flags & (CREX_ACC_PRIVATE|CREX_ACC_PROTECTED))) {
				crex_type_error("User-supplied statement class cannot have a public constructor");
				return false;
			}
			dbh->def_stmt_ce = pce;
			if (!C_ISUNDEF(dbh->def_stmt_ctor_args)) {
...
CRX_METHOD(PDO, getAttribute)
{
	...
	switch (attr) {
		...
		case PDO_ATTR_STATEMENT_CLASS:
			...
			add_next_index_str(return_value, crex_string_copy(dbh->def_stmt_ce->name));
			if (!C_ISUNDEF(dbh->def_stmt_ctor_args)) {
				C_TRY_ADDREF(dbh->def_stmt_ctor_args);
...
CRX_METHOD(PDO, lastInsertId)
{
	pdo_dbh_t *dbh = C_PDO_DBH_P(CREX_THIS);
	crex_string *name = NULL;
	crex_string *last_id = NULL;
...
CRX_METHOD(PDO, query)
{
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "S|l!*", &statement,
	...
	if (dbh->methods->preparer(dbh, statement, stmt, NULL)) {
		...
		/* something broke */
		dbh->query_stmt = stmt;
		ZVAL_OBJ(&dbh->query_stmt_zval, C_OBJ_P(return_value));
		C_DELREF(stmt->database_object_handle);
...
CRX_METHOD(PDO, quote)
{
	pdo_dbh_t *dbh = C_PDO_DBH_P(CREX_THIS);
	crex_string *str, *quoted;
	crex_long paramtype = PDO_PARAM_STR;

	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_STR(str)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(paramtype)
	CREX_PARSE_PARAMETERS_END();
...
CRX_METHOD(PDO, getAvailableDrivers)
{
...
static void cls_method_dtor(zval *el) /* {{{ */ {
	crex_function *func = (crex_function*)C_PTR_P(el);
	if (func->common.function_name) {
		crex_string_release_ex(func->common.function_name, 0);
	}
	if (CREX_MAP_PTR(func->common.run_time_cache)) {
		efree(CREX_MAP_PTR(func->common.run_time_cache));
	}
	efree(func);
}
/* }}} */
...
static void cls_method_pdtor(zval *el) /* {{{ */ {
	crex_function *func = (crex_function*)C_PTR_P(el);
	if (func->common.function_name) {
		crex_string_release_ex(func->common.function_name, 1);
	}
	if (CREX_MAP_PTR(func->common.run_time_cache)) {
		pefree(CREX_MAP_PTR(func->common.run_time_cache), 1);
	}
	pefree(func, 1);
}
/* }}} */
...
{
	const crex_function_entry *funcs;
	crex_internal_function func;
	size_t namelen;
	char *lc_name;
	pdo_dbh_t *dbh = dbh_obj->inner;
	...
	crex_hash_init(dbh->cls_methods[kind], 8, NULL,
			dbh->is_persistent? cls_method_pdtor : cls_method_dtor, dbh->is_persistent);

	memset(&func, 0, sizeof(func));

	size_t rt_cache_size = crex_internal_run_time_cache_reserved_size();
	while (funcs->fname) {
		func.type = CREX_INTERNAL_FUNCTION;
		func.handler = funcs->handler;
		func.function_name = crex_string_init(funcs->fname, strlen(funcs->fname), dbh->is_persistent);
		func.scope = dbh_obj->std.ce;
		func.prototype = NULL;
		CREX_MAP_PTR(func.run_time_cache) = rt_cache_size ? pecalloc(rt_cache_size, 1, dbh->is_persistent) : NULL;
		func.T = CREX_OBSERVER_ENABLED;
		if (funcs->flags) {
			func.fn_flags = funcs->flags | CREX_ACC_NEVER_CACHE;
		} else {
			func.fn_flags = CREX_ACC_PUBLIC | CREX_ACC_NEVER_CACHE;
		}
		if (funcs->arg_info) {
			crex_internal_function_info *info = (crex_internal_function_info*)funcs->arg_info;

			func.arg_info = (crex_internal_arg_info*)funcs->arg_info + 1;
			...
			if (CREX_ARG_SEND_MODE(info)) {
				func.fn_flags |= CREX_ACC_RETURN_REFERENCE;
			}
			if (CREX_ARG_IS_VARIADIC(&funcs->arg_info[funcs->num_args])) {
				func.fn_flags |= CREX_ACC_VARIADIC;
		...
		crex_set_function_arg_flags((crex_function*)&func);
		namelen = strlen(funcs->fname);
		lc_name = emalloc(namelen+1);
		crex_str_tolower_copy(lc_name, funcs->fname, namelen);
		crex_hash_str_add_mem(dbh->cls_methods[kind], lc_name, namelen, &func, sizeof(func));
		efree(lc_name);
		funcs++;
	}
...
static crex_function *dbh_method_get(crex_object **object, crex_string *method_name, const zval *key)
{
	crex_function *fbc = NULL;
	pdo_dbh_object_t *dbh_obj = crx_pdo_dbh_fetch_object(*object);
	crex_string *lc_method_name;

	if ((fbc = crex_std_get_method(object, method_name, key)) == NULL) {
		...
		fbc = crex_hash_find_ptr(dbh_obj->inner->cls_methods[PDO_DBH_DRIVER_METHOD_KIND_DBH], lc_method_name);
...
static HashTable *dbh_get_gc(crex_object *object, zval **gc_data, int *gc_count)
{
	pdo_dbh_t *dbh = crx_pdo_dbh_fetch_inner(object);
	crex_get_gc_buffer *gc_buffer = crex_get_gc_buffer_create();
	crex_get_gc_buffer_add_zval(gc_buffer, &dbh->def_stmt_ctor_args);
	if (dbh->methods && dbh->methods->get_gc) {
		dbh->methods->get_gc(dbh, gc_buffer);
	}
	crex_get_gc_buffer_use(gc_buffer, gc_data, gc_count);
	return crex_std_get_properties(object);
}

static crex_object_handlers pdo_dbh_object_handlers;
static void pdo_dbh_free_storage(crex_object *std);
...
void pdo_dbh_init(int module_number)
{
	...
	memcpy(&pdo_dbh_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	...
	pdo_dbh_object_handlers.compare = crex_objects_not_comparable;
...
static void dbh_free(pdo_dbh_t *dbh, bool free_persistent)
{
	...
	if (dbh->is_persistent) {
#if CREX_DEBUG
		CREX_ASSERT(!free_persistent || (dbh->refcount == 1));
#endif
	...
	for (i = 0; i < PDO_DBH_DRIVER_METHOD_KIND__MAX; i++) {
		if (dbh->cls_methods[i]) {
			crex_hash_destroy(dbh->cls_methods[i]);
...
static void pdo_dbh_free_storage(crex_object *std)
{
	pdo_dbh_t *dbh = crx_pdo_dbh_fetch_inner(std);
	...
	crex_object_std_dtor(std);
	dbh_free(dbh, 0);
}

crex_object *pdo_dbh_new(crex_class_entry *ce)
{
	pdo_dbh_object_t *dbh;

	dbh = crex_object_alloc(sizeof(pdo_dbh_object_t), ce);
	crex_object_std_init(&dbh->std, ce);
...
CREX_RSRC_DTOR_FUNC(crx_pdo_pdbh_dtor) /* {{{ */
{
	if (res->ptr) {
