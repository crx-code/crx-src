...
CREX_DECLARE_MODULE_GLOBALS(gmp)
static CREX_GINIT_FUNCTION(gmp);

/* {{{ gmp_module_entry */
crex_module_entry gmp_module_entry = {
	STANDARD_MODULE_HEADER,
	"gmp",
	ext_functions,
	CREX_MODULE_STARTUP_N(gmp),
	NULL,
	NULL,
	CREX_MODULE_DEACTIVATE_N(gmp),
	CREX_MODULE_INFO_N(gmp),
	CRX_GMP_VERSION,
	CREX_MODULE_GLOBALS(gmp),
	CREX_GINIT(gmp),
	NULL,
	NULL,
...
static crex_class_entry *gmp_ce;
static crex_object_handlers gmp_object_handlers;
...
#define IS_GMP(zval) \
	(C_TYPE_P(zval) == IS_OBJECT && instanceof_function(C_OBJCE_P(zval), gmp_ce))

#define GET_GMP_OBJECT_FROM_OBJ(obj) \
	crx_gmp_object_from_crex_object(obj)
#define GET_GMP_OBJECT_FROM_ZVAL(zv) \
	GET_GMP_OBJECT_FROM_OBJ(C_OBJ_P(zv))

#define GET_GMP_FROM_ZVAL(zval) \
	GET_GMP_OBJECT_FROM_OBJ(C_OBJ_P(zval))->num
...
static crex_result convert_to_gmp(mpz_t gmpnumber, zval *val, crex_long base, uint32_t arg_pos);
...
static void gmp_free_object_storage(crex_object *obj) /* {{{ */
{
...
static inline crex_object *gmp_create_object_ex(crex_class_entry *ce, mpz_ptr *gmpnum_target) /* {{{ */
{
	gmp_object *intern = emalloc(sizeof(gmp_object) + crex_object_properties_size(ce));
...
static crex_object *gmp_create_object(crex_class_entry *ce) /* {{{ */
{
...
static crex_result gmp_cast_object(crex_object *readobj, zval *writeobj, int type) /* {{{ */
{
	mpz_ptr gmpnum;
	switch (type) {
	case IS_STRING:
...
static HashTable *gmp_get_debug_info(crex_object *obj, int *is_temp) /* {{{ */
{
	HashTable *ht, *props = crex_std_get_properties(obj);
	...
	*is_temp = 1;
	...
	crex_hash_str_update(ht, "num", sizeof("num")-1, &zv);

	return ht;
}
/* }}} */

static crex_object *gmp_clone_obj(crex_object *obj) /* {{{ */
{
	gmp_object *old_object = GET_GMP_OBJECT_FROM_OBJ(obj);
	gmp_object *new_object = GET_GMP_OBJECT_FROM_OBJ(gmp_create_object(obj->ce));

	crex_objects_clone_members( &new_object->std, &old_object->std);
...
static void shift_operator_helper(gmp_binary_ui_op_t op, zval *return_value, zval *op1, zval *op2, uint8_t opcode) {
	crex_long shift = zval_get_long(op2);

	if (shift < 0) {
		crex_throw_error(
			crex_ce_value_error, "%s must be greater than or equal to 0",
			opcode == CREX_POW ? "Exponent" : "Shift"
		);
...
static crex_result gmp_do_operation_ex(uint8_t opcode, zval *result, zval *op1, zval *op2) /* {{{ */
{
	switch (opcode) {
	case CREX_ADD:
...
static crex_result gmp_do_operation(uint8_t opcode, zval *result, zval *op1, zval *op2) /* {{{ */
{
	zval op1_copy;
	int retval;
...
static int gmp_compare(zval *op1, zval *op2) /* {{{ */
{
	...
	/* result can only be a crex_long if gmp_cmp hasn't thrown an Error */
	CREX_ASSERT(C_TYPE(result) == IS_LONG);
	return C_LVAL(result);
}
/* }}} */

static int gmp_serialize(zval *object, unsigned char **buffer, size_t *buf_len, crex_serialize_data *data) /* {{{ */
{
	mpz_ptr gmpnum = GET_GMP_FROM_ZVAL(object);
	smart_str buf = {0};
	zval zv;
	crx_serialize_data_t serialize_data;

	CRX_VAR_SERIALIZE_INIT(serialize_data);

	gmp_strval(&zv, gmpnum, 10);
	crx_var_serialize(&buf, &zv, &serialize_data);
	zval_ptr_dtor_str(&zv);

	ZVAL_ARR(&zv, crex_std_get_properties(C_OBJ_P(object)));
	crx_var_serialize(&buf, &zv, &serialize_data);

	CRX_VAR_SERIALIZE_DESTROY(serialize_data);
	*buffer = (unsigned char *) estrndup(ZSTR_VAL(buf.s), ZSTR_LEN(buf.s));
	*buf_len = ZSTR_LEN(buf.s);
	crex_string_release_ex(buf.s, 0);

	return SUCCESS;
}
/* }}} */

static int gmp_unserialize(zval *object, crex_class_entry *ce, const unsigned char *buf, size_t buf_len, crex_unserialize_data *data) /* {{{ */
{
	mpz_ptr gmpnum;
	const unsigned char *p, *max;
	zval *zv;
	int retval = FAILURE;
	crx_unserialize_data_t unserialize_data;
	crex_object *zobj;

	CRX_VAR_UNSERIALIZE_INIT(unserialize_data);
	...
	if (!crx_var_unserialize(zv, &p, max, &unserialize_data)
		|| C_TYPE_P(zv) != IS_STRING
		|| convert_to_gmp(gmpnum, zv, 10, 0) == FAILURE
	) {
		crex_throw_exception(NULL, "Could not unserialize number", 0);
		goto exit;
	}

	zv = var_tmp_var(&unserialize_data);
	if (!crx_var_unserialize(zv, &p, max, &unserialize_data)
		|| C_TYPE_P(zv) != IS_ARRAY
	) {
		crex_throw_exception(NULL, "Could not unserialize properties", 0);
		goto exit;
	}

	if (crex_hash_num_elements(C_ARRVAL_P(zv)) != 0) {
		crex_hash_copy(
			crex_std_get_properties(zobj), C_ARRVAL_P(zv),
	...
	retval = SUCCESS;
exit:
	CRX_VAR_UNSERIALIZE_DESTROY(unserialize_data);
	return retval;
}
/* }}} */
...
CREX_MINIT_FUNCTION(gmp)
{
	...
	memcpy(&gmp_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
...
CREX_MODULE_INFO_D(gmp)
{
	...
	crx_info_print_table_row(2, "gmp support", "enabled");
#ifdef mpir_version
	crx_info_print_table_row(2, "MPIR version", mpir_version);
#else
	crx_info_print_table_row(2, "GMP version", gmp_version);
#endif
	crx_info_print_table_end();
}
/* }}} */

static crex_result convert_zstr_to_gmp(mpz_t gmp_number, const crex_string *val, crex_long base, uint32_t arg_pos)
{
	...
	if (-1 == gmp_ret) {
		if (arg_pos == 0) {
			crex_value_error("Number is not an integer string");
		} else {
			crex_argument_value_error(arg_pos, "is not an integer string");
		}
		return FAILURE;
	}
...
static crex_result convert_to_gmp(mpz_t gmpnumber, zval *val, crex_long base, uint32_t arg_pos)
{
	switch (C_TYPE_P(val)) {
	case IS_LONG:
		mpz_set_si(gmpnumber, C_LVAL_P(val));
		return SUCCESS;
	case IS_STRING: {
		return convert_zstr_to_gmp(gmpnumber, C_STR_P(val), base, arg_pos);
	}
	default: {
		crex_long lval;
		if (!crex_parse_arg_long_slow(val, &lval, arg_pos)) {
			if (arg_pos == 0) {
				crex_type_error(
					"Number must be of type GMP|string|int, %s given", crex_zval_value_name(val));
			} else {
				crex_argument_type_error(arg_pos,
					"must be of type GMP|string|int, %s given", crex_zval_value_name(val));
			}
			return FAILURE;
		}
...
static void gmp_strval(zval *result, mpz_t gmpnum, int base) /* {{{ */
{
	...
	str = crex_string_alloc(num_len, 0);
...
static void gmp_cmp(zval *return_value, zval *a_arg, zval *b_arg, bool is_operator) /* {{{ */
{
	...
	if (use_si) {
		res = mpz_cmp_si(gmpnum_a, C_LVAL_P(b_arg));
	} else {
...
static inline void gmp_zval_binary_ui_op(zval *return_value, zval *a_arg, zval *b_arg, gmp_binary_op_t gmp_op, gmp_binary_ui_op_t gmp_ui_op, bool check_b_zero, bool is_operator)
{
	...
	if (gmp_ui_op && C_TYPE_P(b_arg) == IS_LONG && C_LVAL_P(b_arg) >= 0) {
		gmpnum_b = NULL;
		temp_b.is_used = 0;
	} else {
		FETCH_GMP_ZVAL_DEP(gmpnum_b, b_arg, temp_b, temp_a, is_operator ? 0 : 2);
	...
	if (!gmpnum_b) {
		gmp_ui_op(gmpnum_result, gmpnum_a, (gmp_ulong) C_LVAL_P(b_arg));
	} else {
...
static inline void gmp_zval_binary_ui_op2(zval *return_value, zval *a_arg, zval *b_arg, gmp_binary_op2_t gmp_op, gmp_binary_ui_op2_t gmp_ui_op, int check_b_zero)
{
	...
	if (!gmpnum_b) {
		gmp_ui_op(gmpnum_result1, gmpnum_result2, gmpnum_a, (gmp_ulong) C_LVAL_P(b_arg));
	} else {
...
static inline void _gmp_binary_ui_op(INTERNAL_FUNCTION_PARAMETERS, gmp_binary_op_t gmp_op, gmp_binary_ui_op_t gmp_ui_op, int check_b_zero)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zz", &a_arg, &b_arg) == FAILURE){
		RETURN_THROWS();
	}
...
static inline void _gmp_unary_op(INTERNAL_FUNCTION_PARAMETERS, gmp_unary_op_t gmp_op)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z", &a_arg) == FAILURE){
		RETURN_THROWS();
	}
...
static bool gmp_verify_base(crex_long base, uint32_t arg_num)
{
	if (base && (base < 2 || base > GMP_MAX_BASE)) {
		crex_argument_value_error(arg_num, "must be between 2 and %d", GMP_MAX_BASE);
		return false;
	}
...
static crex_result gmp_initialize_number(mpz_ptr gmp_number, const crex_string *arg_str, crex_long arg_l, crex_long base)
{
	if (arg_str) {
...
{
	mpz_ptr gmp_number;
	crex_string *arg_str = NULL;
	crex_long arg_l = 0;
	crex_long base = 0;

	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_STR_OR_LONG(arg_str, arg_l)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(base)
...
int gmp_import_export_validate(crex_long size, crex_long options, int *order, int *endian)
{
	if (size < 1) {
	...
	switch (options & (GMP_LSW_FIRST | GMP_MSW_FIRST)) {
		...
		default:
			...
			crex_argument_value_error(3, "cannot use multiple word order options");
			return FAILURE;
	}
	...
	switch (options & (GMP_LITTLE_ENDIAN | GMP_BIG_ENDIAN | GMP_NATIVE_ENDIAN)) {
		...
		default:
			/* options argument is in second position */
			crex_argument_value_error(3, "cannot use multiple endian options");
			return FAILURE;
	}
...
CREX_FUNCTION(gmp_import)
{
	...
	size_t data_len;
	crex_long size = 1;
	crex_long options = GMP_MSW_FIRST | GMP_NATIVE_ENDIAN;
	int order, endian;
	mpz_ptr gmpnumber;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|ll", &data, &data_len, &size, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if ((data_len % size) != 0) {
		crex_argument_value_error(1, "must be a multiple of argument #2 ($word_size)");
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_export)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z|ll", &gmpnumber_arg, &size, &options) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (mpz_sgn(gmpnumber) == 0) {
		...
		crex_string *out_string = crex_string_safe_alloc(count, size, 0, 0);
...
CREX_FUNCTION(gmp_intval)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z", &gmpnumber_arg) == FAILURE){
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_strval)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z|l", &gmpnumber_arg, &base) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if ((base < 2 && base > -2) || base > GMP_MAX_BASE || base < -36) {
		crex_argument_value_error(2, "must be between 2 and %d, or -2 and -36", GMP_MAX_BASE);
		RETURN_THROWS();
	}
...
{
	zval *a_arg, *b_arg;
	crex_long round = GMP_ROUND_ZERO;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "zz|l", &a_arg, &b_arg, &round) == FAILURE) {
		RETURN_THROWS();
	}
	...
	case GMP_ROUND_MINUSINF:
		...
		break;
	default:
		crex_argument_value_error(3, "must be one of GMP_ROUND_ZERO, GMP_ROUND_PLUSINF, or GMP_ROUND_MINUSINF");
		RETURN_THROWS();
	}
}
/* }}} */
...
CREX_FUNCTION(gmp_binomial)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zl", &n_arg, &k) == FAILURE) {
		RETURN_THROWS();
	}
	...
	INIT_GMP_RETVAL(gmpnum_result);
	if (C_TYPE_P(n_arg) == IS_LONG && C_LVAL_P(n_arg) >= 0) {
		mpz_bin_uiui(gmpnum_result, (gmp_ulong) C_LVAL_P(n_arg), (gmp_ulong) k);
	} else {
		mpz_ptr gmpnum_n;
		gmp_temp_t temp_n;
...
CREX_FUNCTION(gmp_pow)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zl", &base_arg, &exp) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (C_TYPE_P(base_arg) == IS_LONG && C_LVAL_P(base_arg) >= 0) {
		INIT_GMP_RETVAL(gmpnum_result);
		mpz_ui_pow_ui(gmpnum_result, C_LVAL_P(base_arg), exp);
	} else {
		mpz_ptr gmpnum_base;
...
CREX_FUNCTION(gmp_powm)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zzz", &base_arg, &exp_arg, &mod_arg) == FAILURE){
		RETURN_THROWS();
	}

	FETCH_GMP_ZVAL(gmpnum_base, base_arg, temp_base, 1);

	if (C_TYPE_P(exp_arg) == IS_LONG && C_LVAL_P(exp_arg) >= 0) {
		use_ui = 1;
		temp_exp.is_used = 0;
	} else {
	...
	INIT_GMP_RETVAL(gmpnum_result);
	if (use_ui) {
		mpz_powm_ui(gmpnum_result, gmpnum_base, (crex_ulong) C_LVAL_P(exp_arg), gmpnum_mod);
	} else {
...
CREX_FUNCTION(gmp_root)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zl", &a_arg, &nth) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (nth % 2 == 0 && mpz_sgn(gmpnum_a) < 0) {
		crex_argument_value_error(2, "must be odd if argument #1 ($a) is negative");
		FREE_GMP_TEMP(temp_a);
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_prob_prime)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z|l", &gmpnumber_arg, &reps) == FAILURE) {
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_kronecker)
{
	...
	if (C_TYPE_P(a_arg) == IS_LONG && C_TYPE_P(b_arg) != IS_LONG) {
		use_a_si = 1;
		temp_a.is_used = 0;
	} else {
		FETCH_GMP_ZVAL(gmpnum_a, a_arg, temp_a, 1);
	...
	if (use_a_si) {
		CREX_ASSERT(use_b_si == 0);
		result = mpz_si_kronecker((gmp_long) C_LVAL_P(a_arg), gmpnum_b);
	} else if (use_b_si) {
		result = mpz_kronecker_si(gmpnum_a, (gmp_long) C_LVAL_P(b_arg));
	} else {
...
CREX_FUNCTION(gmp_random_seed)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "z", &seed) == FAILURE) {
		RETURN_THROWS();
	}

	gmp_init_random();

	if (C_TYPE_P(seed) == IS_LONG && C_LVAL_P(seed) >= 0) {
		gmp_randseed_ui(GMPG(rand_state), C_LVAL_P(seed));
	}
	else {
		mpz_ptr gmpnum_seed;
...
CREX_FUNCTION(gmp_random_bits)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "l", &bits) == FAILURE) {
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_random_range)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zz", &min_arg, &max_arg) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (C_TYPE_P(min_arg) == IS_LONG && C_LVAL_P(min_arg) >= 0) {
		if (mpz_cmp_ui(gmpnum_max, C_LVAL_P(min_arg)) <= 0) {
			FREE_GMP_TEMP(temp_a);
			crex_argument_value_error(1, "must be less than argument #2 ($maximum)");
			RETURN_THROWS();
		}
		...
		if (C_LVAL_P(min_arg) != 0) {
			mpz_sub_ui(gmpnum_range, gmpnum_max, C_LVAL_P(min_arg) - 1);
		} else {
		...
		if (C_LVAL_P(min_arg) != 0) {
			mpz_add_ui(gmpnum_result, gmpnum_result, C_LVAL_P(min_arg));
		}
...
CREX_FUNCTION(gmp_setbit)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol|b", &a_arg, gmp_ce, &index, &set) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (index / GMP_NUMB_BITS >= INT_MAX) {
		crex_argument_value_error(2, "must be less than %d * %d", INT_MAX, GMP_NUMB_BITS);
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_clrbit)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Ol", &a_arg, gmp_ce, &index) == FAILURE){
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_testbit)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zl", &a_arg, &index) == FAILURE){
		RETURN_THROWS();
	}
...
CREX_FUNCTION(gmp_scan0)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "zl", &a_arg, &start) == FAILURE){
		RETURN_THROWS();
	}
...
CREX_METHOD(GMP, __main)
{
	...
	mpz_ptr gmp_number = GET_GMP_FROM_ZVAL(CREX_THIS);
...
CREX_METHOD(GMP, __serialize)
{
	...
	mpz_ptr gmpnum = GET_GMP_FROM_ZVAL(CREX_THIS);
	gmp_strval(&zv, gmpnum, 16);
	crex_hash_next_index_insert(C_ARRVAL_P(return_value), &zv);

	HashTable *props = C_OBJ_P(CREX_THIS)->properties;
	if (props && crex_hash_num_elements(props) != 0) {
		ZVAL_ARR(&zv, crex_proptable_to_symtable(
			crex_std_get_properties(C_OBJ_P(CREX_THIS)), /* always duplicate */ 1));
		crex_hash_next_index_insert(C_ARRVAL_P(return_value), &zv);
	}
}
...
CREX_METHOD(GMP, __unserialize)
{
	...
	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_ARRAY_HT(data)
	CREX_PARSE_PARAMETERS_END();

	zval *num = crex_hash_index_find(data, 0);
	if (!num || C_TYPE_P(num) != IS_STRING ||
			convert_to_gmp(GET_GMP_FROM_ZVAL(CREX_THIS), num, 16, 0) == FAILURE) {
		crex_throw_exception(NULL, "Could not unserialize number", 0);
		RETURN_THROWS();
	}

	zval *props = crex_hash_index_find(data, 1);
	if (props) {
		if (C_TYPE_P(props) != IS_ARRAY) {
			crex_throw_exception(NULL, "Could not unserialize properties", 0);
			RETURN_THROWS();
		}

		object_properties_load(C_OBJ_P(CREX_THIS), C_ARRVAL_P(props));
	}
}
