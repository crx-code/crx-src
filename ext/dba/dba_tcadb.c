...
DBA_FETCH_FUNC(tcadb)
{
	...
	if (value) {
		fetched_val = crex_string_init(value, value_size, /* persistent */ false);
		tcfree(value);
	}
...
DBA_UPDATE_FUNC(tcadb)
{
	...
	crx_error_docref(NULL, E_WARNING, "Error updating data");
	return FAILURE;
}
...
DBA_FIRSTKEY_FUNC(tcadb)
{
	...
	if (value) {
		key = crex_string_init(value, value_size, /* persistent */ false);
		tcfree(value);
	}
