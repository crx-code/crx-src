...
DBA_FETCH_FUNC(gdbm)
{
	...
	if (gval.dptr) {
		fetched_val = crex_string_init(gval.dptr, gval.dsize, /* persistent */ false);
		free(gval.dptr);
	}
	return fetched_val;
}
...
DBA_UPDATE_FUNC(gdbm)
{
	...
	switch (gdbm_store(dba->dbf, gkey, gval, mode == 1 ? GDBM_INSERT : GDBM_REPLACE)) {
		...
		case -1:
			...
			crx_error_docref(NULL, E_WARNING, "%s", gdbm_strerror(gdbm_errno));
			return FAILURE;
		default:
			// TODO Convert this to an assertion failure
			crx_error_docref(NULL, E_WARNING, "Unknown return value");
			return FAILURE;
	}
}
...
DBA_FIRSTKEY_FUNC(gdbm)
{
	...
	if (gkey.dptr) {
		key = crex_string_init(gkey.dptr, gkey.dsize, /* persistent */ false);
		dba->nextkey = gkey;
	} else {
