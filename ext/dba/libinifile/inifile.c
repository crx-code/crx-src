...
inifile * inifile_alloc(crx_stream *fp, int readonly, int persistent)
{
	...
	if (!readonly) {
		if (!crx_stream_truncate_supported(fp)) {
...
static int inifile_read(inifile *dba, line_type *ln) {
	...
	inifile_val_free(&ln->val);
	while ((fline = crx_stream_gets(dba->fp, NULL, 0)) != NULL) {
		if (fline) {
			if (fline[0] == '[') {
...
val_type inifile_fetch(inifile *dba, const key_type *key, int skip) {
	...
	if (skip == -1 && dba->next.key.group && dba->next.key.name && !inifile_key_cmp(&dba->next.key, key)) {
		...
		crx_stream_seek(dba->fp, dba->next.pos, SEEK_SET);
	...
	while(inifile_read(dba, &ln)) {
		if (!(res=inifile_key_cmp(&ln.key, key))) {
			if (!skip) {
				...
				inifile_line_free(&dba->next);
				dba->next = ln;
				dba->next.pos = crx_stream_tell(dba->fp);
				return val;
			}
			skip--;
		} else if (res == 1) {
...
int inifile_nextkey(inifile *dba) {
	...
	/*inifile_line_free(&dba->next); ??? */
	crx_stream_seek(dba->fp, dba->curr.pos, SEEK_SET);
...
static int inifile_truncate(inifile *dba, size_t size)
{
	...
	if ((res=crx_stream_truncate_set_size(dba->fp, size)) != 0) {
		crx_error_docref(NULL, E_WARNING, "Error in ftruncate: %d", res);
		return FAILURE;
	}
	crx_stream_seek(dba->fp, size, SEEK_SET);
	return SUCCESS;
}
/* }}} */
...
static int inifile_find_group(inifile *dba, const key_type *key, size_t *pos_grp_start)
{
	...
	crx_stream_seek(dba->fp, 0, SEEK_SET);
	inifile_line_free(&dba->curr);
	inifile_line_free(&dba->next);
	...
	if (key->group && strlen(key->group)) {
		...
		while(inifile_read(dba, &ln)) {
			...
			*pos_grp_start = crx_stream_tell(dba->fp);
		}
		inifile_line_free(&ln);
	} else {
		*pos_grp_start = 0;
...
static int inifile_copy_to(inifile *dba, size_t pos_start, size_t pos_end, inifile **ini_copy)
{
	...
	if ((fp = crx_stream_temp_create(0, 64 * 1024)) == NULL) {
		crx_error_docref(NULL, E_WARNING, "Could not create temporary stream");
		*ini_copy = NULL;
		return FAILURE;
	}
	...
	crx_stream_seek(dba->fp, pos_start, SEEK_SET);
	if (SUCCESS != crx_stream_copy_to_stream_ex(dba->fp, fp, pos_end - pos_start, NULL)) {
		crx_error_docref(NULL, E_WARNING, "Could not copy group [%zu - %zu] to temporary stream", pos_start, pos_end);
		return FAILURE;
	}
	return SUCCESS;
}
...
static int inifile_filter(inifile *dba, inifile *from, const key_type *key, bool *found)
{
	...
	crx_stream_seek(from->fp, 0, SEEK_SET);
	crx_stream_seek(dba->fp, 0, SEEK_END);
	while(inifile_read(from, &ln)) {
		...
		case 0:
			...
			pos_curr = crx_stream_tell(from->fp);
			if (pos_start != pos_next) {
				crx_stream_seek(from->fp, pos_start, SEEK_SET);
				if (SUCCESS != crx_stream_copy_to_stream_ex(from->fp, dba->fp, pos_next - pos_start, NULL)) {
					crx_error_docref(NULL, E_WARNING, "Could not copy [%zu - %zu] from temporary stream", pos_next, pos_start);
					ret = FAILURE;
				}
				crx_stream_seek(from->fp, pos_curr, SEEK_SET);
			}
			pos_next = pos_start = pos_curr;
			break;
		case 1:
			pos_next = crx_stream_tell(from->fp);
			break;
		case 2:
...
static int inifile_delete_replace_append(inifile *dba, const key_type *key, const val_type *value, int append, bool *found)
{
	...
	/* 4 */
	if (ret == SUCCESS) {
		fp_tmp = crx_stream_temp_create(0, 64 * 1024);
		if (!fp_tmp) {
			crx_error_docref(NULL, E_WARNING, "Could not create temporary stream");
			ret = FAILURE;
		} else {
			crx_stream_seek(dba->fp, 0, SEEK_END);
			if (pos_grp_next != (size_t)crx_stream_tell(dba->fp)) {
				crx_stream_seek(dba->fp, pos_grp_next, SEEK_SET);
				if (SUCCESS != crx_stream_copy_to_stream_ex(dba->fp, fp_tmp, CRX_STREAM_COPY_ALL, NULL)) {
					crx_error_docref(NULL, E_WARNING, "Could not copy remainder to temporary stream");
					ret = FAILURE;
				}
			}
		}
	...
	if (ret == SUCCESS) {
		if (key->name && strlen(key->name)) {
			...
			if (value) {
				if (pos_grp_start == pos_grp_next && key->group && strlen(key->group)) {
					crx_stream_printf(dba->fp, "[%s]\n", key->group);
				}
				crx_stream_printf(dba->fp, "%s=%s\n", key->name, value->value ? value->value : "");
			}
		}
		 ...
		 * however next operation must be done.
		 */
		if (fp_tmp && crx_stream_tell(fp_tmp)) {
			crx_stream_seek(fp_tmp, 0, SEEK_SET);
			crx_stream_seek(dba->fp, 0, SEEK_END);
			if (SUCCESS != crx_stream_copy_to_stream_ex(fp_tmp, dba->fp, CRX_STREAM_COPY_ALL, NULL)) {
				crex_throw_error(NULL, "Could not copy from temporary stream - ini file truncated");
				ret = FAILURE;
			}
		}
	}
	...
	if (ini_tmp) {
		crx_stream_close(ini_tmp->fp);
		inifile_free(ini_tmp, 0);
	}
	if (fp_tmp) {
		crx_stream_close(fp_tmp);
	}
	crx_stream_flush(dba->fp);
	crx_stream_seek(dba->fp, 0, SEEK_SET);
