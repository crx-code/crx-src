...
#if DBA_CDB_BUILTIN
# define crx_cdb_read(cdb, buf, len, pos) cdb_read(cdb, buf, len, pos)
# define crx_cdb_findnext(cdb, key, len) cdb_findnext(cdb, key, len)
# define crx_cdb_find(cdb, key, len) cdb_find(cdb, key, len)
#else
# define crx_cdb_read(cdb, buf, len, pos) cdb_read(cdb, buf, len, pos)
# define crx_cdb_findnext(cdb, key, len) cdb_findnext(cdb, key, len)
# define crx_cdb_find(cdb, key, len) cdb_find(cdb, key, len)
...
#endif
	if (crx_cdb_find(&cdb->c, ZSTR_VAL(key), ZSTR_LEN(key)) == 1) {
		while(skip--) {
			if (crx_cdb_findnext(&cdb->c, ZSTR_VAL(key), ZSTR_LEN(key)) != 1) {
				return NULL;
			}
		}
		len = cdb_datalen(&cdb->c);
		fetched_val = crex_string_alloc(len, /* persistent */ false);

		if (crx_cdb_read(&cdb->c, ZSTR_VAL(fetched_val), len, cdb_datapos(&cdb->c)) == -1) {
			crex_string_release_ex(fetched_val, /* persistent */ false);
			return NULL;
		}
...
/* {{{ cdb_file_read */
#if DBA_CDB_BUILTIN
# define cdb_file_read(fildes, buf, size) crx_stream_read(fildes, buf, size)
#else
...
/* {{{ cdb_file_lseek
 crx_stream_seek does not return actual position */
#if DBA_CDB_BUILTIN
crex_off_t cdb_file_lseek(crx_stream *fp, crex_off_t offset, int whence) {
	crx_stream_seek(fp, offset, whence);
	return crx_stream_tell(fp);
}
#else
crex_off_t cdb_file_lseek(int fd, crex_off_t offset, int whence) {
...
#define CSEEK(n) do { \
	...
	if (cdb_file_lseek(cdb->file, (crex_off_t)n, SEEK_SET) != (crex_off_t) n) return NULL; \
} while (0)
...
DBA_FIRSTKEY_FUNC(cdb)
{
	...
	key = crex_string_alloc(klen, /* persistent */ false);
	if (cdb_file_read(cdb->file, ZSTR_VAL(key), klen) < klen) {
		crex_string_release_ex(key, /* persistent */ false);
		key = NULL;
	} else {
		ZSTR_VAL(key)[klen] = 0;
	}
