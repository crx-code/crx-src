...
static void crx_dba_db4_errcall_fcn(
...
/* This code suppresses a BDB 4.8+ error message, thus keeping CRX test compatibility */
	{
	...
	crx_error_docref(NULL, E_NOTICE, "%s%s", errpfx?errpfx:"", msg);
}
...
DBA_OPEN_FUNC(db4)
{
	...
	if ((err=db_create(&dbp, NULL, 0)) == 0) {
	    dbp->set_errcall(dbp, crx_dba_db4_errcall_fcn);
	    if (
...
DBA_FETCH_FUNC(db4)
{
	...
	DBT gval;
	DBT gkey;
	crex_string *fetched_value = NULL;
	...
	if (!dba->dbp->get(dba->dbp, NULL, &gkey, &gval, 0)) {
		fetched_value = crex_string_init(gval.data, gval.size, /* persistent */ false);
		if (info->flags & DBA_PERSISTENT) {
			free(gval.data);
		}
	}
...
DBA_NEXTKEY_FUNC(db4)
{
	...
	if (dba->cursor && dba->cursor->c_get(dba->cursor, &gkey, &gval, DB_NEXT) == 0) {
		if (gkey.data) {
			key = crex_string_init(gkey.data, gkey.size, /* persistent */ false);
		}
		if (info->flags & DBA_PERSISTENT) {
			if (gkey.data) {
				free(gkey.data);
