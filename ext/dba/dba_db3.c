...
static void crx_dba_db3_errcall_fcn(
...
DBA_OPEN_FUNC(db3)
{
	...
	if ((err=db_create(&dbp, NULL, 0)) == 0) {
	    dbp->set_errcall(dbp, crx_dba_db3_errcall_fcn);
		if(
...
DBA_FETCH_FUNC(db3)
{
	...
	if (!dba->dbp->get(dba->dbp, NULL, &gkey, &gval, 0)) {
		return crex_string_init(gval.data, gval.size, /* persistent */ false);
	}
	return NULL;
}
...
DBA_NEXTKEY_FUNC(db3)
{
	...
	if (dba->cursor->c_get(dba->cursor, &gkey, &gval, DB_NEXT) == 0) {
		if (gkey.data) {
			return crex_string_init(gkey.data, gkey.size, /* persistent */ false);
		}
	}
