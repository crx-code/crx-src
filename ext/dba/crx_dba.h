...
#define CRX_DBA_VERSION CRX_VERSION
...
typedef struct dba_info {
	...
	crex_string *path;
	dba_mode_t mode;
	crx_stream *fp;  /* this is the database stream for builtin handlers */
	int fd;
	int file_permission;
	crex_long map_size;
	/* -1 for default driver flags */
	crex_long driver_flags;
	/* private */
...
extern crex_module_entry dba_module_entry;
#define dba_module_ptr &dba_module_entry

typedef struct dba_handler {
	const char *name; /* handler name */
	int flags; /* whether and how dba does locking and other flags*/
	crex_result (*open)(dba_info *, const char **error);
	void (*close)(dba_info *);
	crex_string* (*fetch)(dba_info *, crex_string *, int);
	crex_result (*update)(dba_info *, crex_string *, crex_string *, int);
	crex_result (*exists)(dba_info *, crex_string *);
	crex_result (*delete)(dba_info *, crex_string *);
	crex_string* (*firstkey)(dba_info *);
	crex_string* (*nextkey)(dba_info *);
	crex_result (*optimize)(dba_info *);
	crex_result (*sync)(dba_info *);
...
#define DBA_OPEN_FUNC(x) \
	crex_result dba_open_##x(dba_info *info, const char **error)
#define DBA_CLOSE_FUNC(x) \
	void dba_close_##x(dba_info *info)
#define DBA_FETCH_FUNC(x) \
	crex_string *dba_fetch_##x(dba_info *info, crex_string *key, int skip)
#define DBA_UPDATE_FUNC(x) \
	crex_result dba_update_##x(dba_info *info, crex_string *key, crex_string *val, int mode)
#define DBA_EXISTS_FUNC(x) \
	crex_result dba_exists_##x(dba_info *info, crex_string *key)
#define DBA_DELETE_FUNC(x) \
	crex_result dba_delete_##x(dba_info *info, crex_string *key)
#define DBA_FIRSTKEY_FUNC(x) \
	crex_string *dba_firstkey_##x(dba_info *info)
#define DBA_NEXTKEY_FUNC(x) \
	crex_string *dba_nextkey_##x(dba_info *info)
#define DBA_OPTIMIZE_FUNC(x) \
	crex_result dba_optimize_##x(dba_info *info)
#define DBA_SYNC_FUNC(x) \
	crex_result dba_sync_##x(dba_info *info)
...
#define crxext_dba_ptr dba_module_ptr
