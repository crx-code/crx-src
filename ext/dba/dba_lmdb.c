...
#define LMDB_IT(it) (((struct crx_lmdb_info *)info->dbf)->it)
...
DBA_OPEN_FUNC(lmdb)
{
	...
	crex_long map_size = info->map_size;

	CREX_ASSERT(map_size >= 0);
	...
	if (info->driver_flags != DBA_DEFAULT_DRIVER_FLAGS) {
		CREX_ASSERT(info->driver_flags >= 0);
		switch (info->driver_flags) {
			...
			case MDB_NOSUBDIR:
				...
				break;
			default:
				crex_argument_value_error(6, "must be either DBA_LMDB_USE_SUB_DIR or DBA_LMDB_NO_SUB_DIR for LMDB driver");
				return FAILURE;
		}
	}
	...
	info->dbf = pemalloc(sizeof(struct crx_lmdb_info), info->flags & DBA_PERSISTENT);
	if (!info->dbf) {
		*error = "Failed to allocate crx_lmdb_info.";
		return FAILURE;
	}
	memset(info->dbf, 0, sizeof(struct crx_lmdb_info));
...
DBA_FETCH_FUNC(lmdb)
{
	...
	if (rc) {
		crx_error_docref(NULL, E_WARNING, "%s", mdb_strerror(rc));
		return NULL;
	}
	...
	if (v.mv_data) {
		ret = crex_string_init(v.mv_data, v.mv_size, /* persistent */ false);
	}
...
DBA_FIRSTKEY_FUNC(lmdb)
{
	...
	if (k.mv_data) {
		ret = crex_string_init(k.mv_data, k.mv_size, /* persistent */ false);
	}
