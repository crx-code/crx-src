...
static int cdb_make_write(struct cdb_make *c, char *buf, uint32 sz) {
	return crx_stream_write(c->fp, buf, sz) == sz ? 0 : -1;
}
...
/* {{{ cdb_make_start */
int cdb_make_start(struct cdb_make *c, crx_stream * f)
{
	c->head = 0;
	c->split = 0;
	c->hash = 0;
	...
	if (crx_stream_seek(f, c->pos, SEEK_SET) == -1) {
		crx_error_docref(NULL, E_NOTICE, "Fseek failed");
		return -1;
	}
	return crx_stream_tell(c->fp);
}
/* }}} */
...
int cdb_make_finish(struct cdb_make *c)
{
	...
	if (crx_stream_flush(c->fp) != 0)
		return -1;
	crx_stream_rewind(c->fp);
	if (crx_stream_tell(c->fp) != 0)
		return -1;
	if (cdb_make_write(c, c->final, sizeof(c->final)) != 0)
		return -1;
	return crx_stream_flush(c->fp);
}
/* }}} */
