CRX_ARG_WITH([gettext],
  [for GNU gettext support],
  [AS_HELP_STRING([[--with-gettext[=DIR]]],
    [Include GNU gettext support])])

if test "$CRX_GETTEXT" != "no"; then
  for i in $CRX_GETTEXT /usr/local /usr; do
  ...
  GETTEXT_LIBDIR=$GETTEXT_DIR/$CRX_LIBDIR
  ...
  CRX_NEW_EXTENSION(gettext, gettext.c, $ext_shared)
  CRX_SUBST(GETTEXT_SHARED_LIBADD)

  CRX_ADD_INCLUDE($GETTEXT_INCDIR)
  ...
  if test -n "$GETTEXT_LIBS"; then
    CRX_ADD_LIBRARY_WITH_PATH($GETTEXT_LIBS, $GETTEXT_LIBDIR, GETTEXT_SHARED_LIBADD)
  fi
fi
