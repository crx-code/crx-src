...
#include "crx_libxml.h"

#define CRX_LIBXML_LOADED_VERSION ((char *)xmlParserVersion)
#define CRX_LIBXML_ERROR 0
#define CRX_LIBXML_CTX_ERROR 1
#define CRX_LIBXML_CTX_WARNING 2

#include "libxml_arginfo.h"

/* a true global for initialization */
static int _crx_libxml_initialized = 0;
static int _crx_libxml_per_request_initialization = 1;
static xmlExternalEntityLoader _crx_libxml_default_entity_loader;

typedef struct _crx_libxml_func_handler {
	crx_libxml_export_node export_func;
} crx_libxml_func_handler;

static HashTable crx_libxml_exports;

static CREX_DECLARE_MODULE_GLOBALS(libxml)
static CRX_GINIT_FUNCTION(libxml);

static crex_class_entry *libxmlerror_class_entry;
...
#endif
CREX_GET_MODULE(libxml)
#endif /* COMPILE_DL_LIBXML */
/* }}} */

/* {{{ function prototypes */
static CRX_MINIT_FUNCTION(libxml);
static CRX_RINIT_FUNCTION(libxml);
static CRX_RSHUTDOWN_FUNCTION(libxml);
static CRX_MSHUTDOWN_FUNCTION(libxml);
static CRX_MINFO_FUNCTION(libxml);
static crex_result crx_libxml_post_deactivate(void);

/* }}} */

crex_module_entry libxml_module_entry = {
	STANDARD_MODULE_HEADER,
	"libxml",                /* extension name */
	ext_functions,           /* extension function list */
	CRX_MINIT(libxml),       /* extension-wide startup function */
	CRX_MSHUTDOWN(libxml),   /* extension-wide shutdown function */
	CRX_RINIT(libxml),       /* per-request startup function */
	CRX_RSHUTDOWN(libxml),   /* per-request shutdown function */
	CRX_MINFO(libxml),       /* information function */
	CRX_LIBXML_VERSION,
	CRX_MODULE_GLOBALS(libxml), /* globals descriptor */
	CRX_GINIT(libxml),          /* globals ctor */
	NULL,                       /* globals dtor */
	crx_libxml_post_deactivate, /* post deactivate */
	STANDARD_MODULE_PROPERTIES_EX
};

/* }}} */

static void crx_libxml_set_old_ns_list(xmlDocPtr doc, xmlNsPtr first, xmlNsPtr last)
{
	if (UNEXPECTED(doc == NULL)) {
		return;
	}

	CREX_ASSERT(last->next == NULL);
...
{
	crx_libxml_set_old_ns_list(doc, ns, ns);
}
...
static void crx_libxml_unlink_entity(void *data, void *table, const xmlChar *name)
#else
static void crx_libxml_unlink_entity(void *data, void *table, xmlChar *name)
#endif
{
...
static int crx_libxml_clear_object(crx_libxml_node_object *object)
{
	if (object->properties) {
		object->properties = NULL;
	}
	crx_libxml_decrement_node_ptr(object);
	return crx_libxml_decrement_doc_ref(object);
}

static void crx_libxml_unregister_node(xmlNodePtr nodep)
{
	crx_libxml_node_object *wrapper;

	crx_libxml_node_ptr *nodeptr = nodep->_private;

	if (nodeptr != NULL) {
		wrapper = nodeptr->_private;
		if (wrapper) {
			crx_libxml_clear_object(wrapper);
		} else {
...
static void crx_libxml_unlink_entity_decl(xmlEntityPtr entity)
{
...
static void crx_libxml_node_free(xmlNodePtr node)
{
	if(node) {
		if (node->_private != NULL) {
			((crx_libxml_node_ptr *) node->_private)->node = NULL;
		}
		switch (node->type) {
			...
			case XML_ENTITY_DECL: {
				...
				crx_libxml_unlink_entity_decl(entity);
				if (entity->orig != NULL) {
			...
			case XML_DTD_NODE: {
				...
				if (dtd->_private == NULL) {
					...
					xmlHashScan(dtd->entities, crx_libxml_unlink_entity, dtd->entities);
					xmlHashScan(dtd->pentities, crx_libxml_unlink_entity, dtd->pentities);
			...
			case XML_ELEMENT_NODE:
				if (node->nsDef && node->doc) {
					...
					crx_libxml_set_old_ns_list(node->doc, ns, last);
					node->nsDef = NULL;
				}
				xmlFreeNode(node);
				break;
		...
		while (curnode != NULL) {
			...
			switch (node->type) {
				...
				case XML_NOTATION_NODE:
					break;
				case XML_ENTITY_DECL:
					crx_libxml_unlink_entity_decl((xmlEntityPtr) node);
					break;
				case XML_ENTITY_REF_NODE:
					crx_libxml_node_free_list((xmlNodePtr) node->properties);
					break;
				case XML_ATTRIBUTE_NODE:
				...
				case XML_NAMESPACE_DECL:
				case XML_TEXT_NODE:
					crx_libxml_node_free_list(node->children);
					break;
				default:
					crx_libxml_node_free_list(node->children);
					crx_libxml_node_free_list((xmlNodePtr) node->properties);
			...
			curnode = node->next;
			xmlUnlinkNode(node);
			crx_libxml_unregister_node(node);
			crx_libxml_node_free(node);
		}
	}
}
...
/* Channel libxml file io layer through the CRX streams subsystem.
 * This allows use of ftps:// and https:// urls */

static void *crx_libxml_streams_IO_open_wrapper(const char *filename, const char *mode, const int read_only)
{
	...
	if (strstr(filename, "%00")) {
		crx_error_docref(NULL, E_WARNING, "URI must not contain percent-encoded NUL bytes");
		return NULL;
	}
	...
	if (uri && (uri->scheme == NULL ||
		...
		isescaped = 1;
#if LIBXML_VERSION >= 20902 && defined(CRX_WIN32)
		/* Libxml 2.9.2 prefixes local paths with file:/ instead of file://,
			thus the crx stream wrapper will fail on a valid case. For this
	...
	/* logic copied from _crx_stream_stat, but we only want to fail
	...
	wrapper = crx_stream_locate_url_wrapper(resolved_path, &path_to_open, 0);
	if (wrapper && read_only && wrapper->wops->url_stat) {
		if (wrapper->wops->url_stat(wrapper, path_to_open, CRX_STREAM_URL_STAT_QUIET, &ssbuf, NULL) == -1) {
			if (isescaped) {
				xmlFree(resolved_path);
			}
			return NULL;
	...
	context = crx_stream_context_from_zval(C_ISUNDEF(LIBXML(stream_context))? NULL : &LIBXML(stream_context), 0);

	ret_val = crx_stream_open_wrapper_ex(path_to_open, (char *)mode, REPORT_ERRORS, NULL, context);
	if (ret_val) {
		/* Prevent from closing this by fclose() */
		((crx_stream*)ret_val)->flags |= CRX_STREAM_FLAG_NO_FCLOSE;
	}
	if (isescaped) {
		xmlFree(resolved_path);
	}
...
static void *crx_libxml_streams_IO_open_read_wrapper(const char *filename)
{
	return crx_libxml_streams_IO_open_wrapper(filename, "rb", 1);
}

static void *crx_libxml_streams_IO_open_write_wrapper(const char *filename)
{
	return crx_libxml_streams_IO_open_wrapper(filename, "wb", 0);
}

static int crx_libxml_streams_IO_read(void *context, char *buffer, int len)
{
	return crx_stream_read((crx_stream*)context, buffer, len);
}

static int crx_libxml_streams_IO_write(void *context, const char *buffer, int len)
{
	return crx_stream_write((crx_stream*)context, buffer, len);
}

static int crx_libxml_streams_IO_close(void *context)
{
	return crx_stream_close((crx_stream*)context);
}

static xmlParserInputBufferPtr
crx_libxml_input_buffer_create_filename(const char *URI, xmlCharEncoding enc)
{
	...
	context = crx_libxml_streams_IO_open_read_wrapper(URI);
	...
	if (enc == XML_CHAR_ENCODING_NONE) {
		crx_stream *s  = (crx_stream *) context;

		if (C_TYPE(s->wrapperdata) == IS_ARRAY) {
			zval *header;

			CREX_HASH_FOREACH_VAL_IND(C_ARRVAL(s->wrapperdata), header) {
				const char buf[] = "Content-Type:";
				if (C_TYPE_P(header) == IS_STRING &&
						!crex_binary_strncasecmp(C_STRVAL_P(header), C_STRLEN_P(header), buf, sizeof(buf)-1, sizeof(buf)-1)) {
					char needle[] = "charset=";
					char *haystack = estrndup(C_STRVAL_P(header), C_STRLEN_P(header));
					char *encoding = crx_stristr(haystack, needle, C_STRLEN_P(header), strlen(needle));
	...
	if (ret != NULL) {
		ret->context = context;
		ret->readcallback = crx_libxml_streams_IO_read;
		ret->closecallback = crx_libxml_streams_IO_close;
	} else
		crx_libxml_streams_IO_close(context);

	return(ret);
}

static xmlOutputBufferPtr
crx_libxml_output_buffer_create_filename(const char *URI,
                              ...
                              int compression ATTRIBUTE_UNUSED)
{
	...
	if (unescaped != NULL) {
		context = crx_libxml_streams_IO_open_write_wrapper(unescaped);
		xmlFree(unescaped);
	}

	/* try with a non-escaped URI this may be a strange filename */
	if (context == NULL) {
		context = crx_libxml_streams_IO_open_write_wrapper(URI);
	}
	...
	if (ret != NULL) {
		ret->context = context;
		ret->writecallback = crx_libxml_streams_IO_write;
		ret->closecallback = crx_libxml_streams_IO_close;
	}
...
static void _crx_libxml_free_error(void *ptr)
{
	/* This will free the libxml alloc'd memory */
	xmlResetError((xmlErrorPtr) ptr);
}

static void _crx_list_set_error_structure(xmlErrorPtr error, const char *msg)
{
	xmlError error_copy;
	int ret;
	...
	if (ret == 0) {
		crex_llist_add_element(LIBXML(error_list), &error_copy);
	}
}

static void crx_libxml_ctx_error_level(int level, void *ctx, const char *msg)
{
	...
	if (parser != NULL && parser->input != NULL) {
		if (parser->input->filename) {
			crx_error_docref(NULL, level, "%s in %s, line: %d", msg, parser->input->filename, parser->input->line);
		} else {
			crx_error_docref(NULL, level, "%s in Entity, line: %d", msg, parser->input->line);
		}
	} else {
		crx_error_docref(NULL, E_WARNING, "%s", msg);
	}
}

void crx_libxml_issue_error(int level, const char *msg)
{
	if (LIBXML(error_list)) {
		_crx_list_set_error_structure(NULL, msg);
	} else {
		crx_error_docref(NULL, level, "%s", msg);
	}
}

static void crx_libxml_internal_error_handler(int error_type, void *ctx, const char **msg, va_list ap)
{
	char *buf;
	...
	if (output == 1) {
		if (LIBXML(error_list)) {
			_crx_list_set_error_structure(NULL, ZSTR_VAL(LIBXML(error_buffer).s));
		} else if (!EG(exception)) {
			/* Don't throw additional notices/warnings if an exception has already been thrown. */
			switch (error_type) {
				case CRX_LIBXML_CTX_ERROR:
					crx_libxml_ctx_error_level(E_WARNING, ctx, ZSTR_VAL(LIBXML(error_buffer).s));
					break;
				case CRX_LIBXML_CTX_WARNING:
					crx_libxml_ctx_error_level(E_NOTICE, ctx, ZSTR_VAL(LIBXML(error_buffer).s));
					break;
				default:
					crx_error_docref(NULL, E_WARNING, "%s", ZSTR_VAL(LIBXML(error_buffer).s));
			}
		}
...
static xmlParserInputPtr _crx_libxml_external_entity_loader(const char *URL,
		const char *ID, xmlParserCtxtPtr context)
{
	...
	if (!CREX_FCC_INITIALIZED(LIBXML(entity_loader_callback))) {
		return _crx_libxml_default_entity_loader(URL, ID, context);
	}
...
#undef ADD_NULL_OR_STRING_KEY
	...
	crex_call_known_fcc(&LIBXML(entity_loader_callback), &retval, 3, params, /* named_params */ NULL);

	if (C_ISUNDEF(retval)) {
		crx_libxml_ctx_error(context,
...
is_string:
			resource = C_STRVAL(retval);
		} else if (C_TYPE(retval) == IS_RESOURCE) {
			crx_stream *stream;
			crx_stream_from_zval_no_verify(stream, &retval);
			if (stream == NULL) {
				crx_libxml_ctx_error(context,
				...
				if (pib == NULL) {
					crx_libxml_ctx_error(context, "Could not allocate parser "
							"input buffer");
				} else {
					/* make stream not being closed when the zval is freed */
					GC_ADDREF(stream->res);
					pib->context = stream;
					pib->readcallback = crx_libxml_streams_IO_read;
					pib->closecallback = crx_libxml_streams_IO_close;
...
static xmlParserInputPtr _crx_libxml_pre_ext_ent_loader(const char *URL,
		const char *ID, xmlParserCtxtPtr context)
{

	/* Check whether we're running in a CRX context, since the entity loader
	 ...
	 * (if _crx_libxml_per_request_initialization */
	if (xmlGenericError == crx_libxml_error_handler && PG(modules_activated)) {
		return _crx_libxml_external_entity_loader(URL, ID, context);
	} else {
		return _crx_libxml_default_entity_loader(URL, ID, context);
	}
}
...
CRX_LIBXML_API void crx_libxml_ctx_error(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	crx_libxml_internal_error_handler(CRX_LIBXML_CTX_ERROR, ctx, &msg, args);
	va_end(args);
}
...
CRX_LIBXML_API void crx_libxml_ctx_warning(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	crx_libxml_internal_error_handler(CRX_LIBXML_CTX_WARNING, ctx, &msg, args);
	va_end(args);
}

static void crx_libxml_structured_error_handler(void *userData, xmlErrorPtr error)
{
	_crx_list_set_error_structure(error, NULL);
...
CRX_LIBXML_API void crx_libxml_error_handler(void *ctx, const char *msg, ...)
{
	va_list args;
	va_start(args, msg);
	crx_libxml_internal_error_handler(CRX_LIBXML_ERROR, ctx, &msg, args);
	va_end(args);
}

static void crx_libxml_exports_dtor(zval *zv)
{
	free(C_PTR_P(zv));
}
...
{
	if (!_crx_libxml_initialized) {
		/* we should be the only one's to ever init!! */
		CREX_IGNORE_LEAKS_BEGIN();
		xmlInitParser();
		CREX_IGNORE_LEAKS_END();

		_crx_libxml_default_entity_loader = xmlGetExternalEntityLoader();
		xmlSetExternalEntityLoader(_crx_libxml_pre_ext_ent_loader);

		crex_hash_init(&crx_libxml_exports, 0, NULL, crx_libxml_exports_dtor, 1);

		_crx_libxml_initialized = 1;
	}
}
...
#endif
		/* xmlCleanupParser(); */
		crex_hash_destroy(&crx_libxml_exports);

		xmlSetExternalEntityLoader(_crx_libxml_default_entity_loader);
		_crx_libxml_initialized = 0;
	}
}
...
static CRX_MINIT_FUNCTION(libxml)
{
	...
	if (sapi_module.name) {
		...
		for (sapi_name = supported_sapis; *sapi_name; sapi_name++) {
			if (strcmp(sapi_module.name, *sapi_name) == 0) {
				_crx_libxml_per_request_initialization = 0;
				break;
			}
		}
	}
	...
	if (!_crx_libxml_per_request_initialization) {
		...
		xmlSetGenericErrorFunc(NULL, crx_libxml_error_handler);
		xmlParserInputBufferCreateFilenameDefault(crx_libxml_input_buffer_create_filename);
		xmlOutputBufferCreateFilenameDefault(crx_libxml_output_buffer_create_filename);
	}
...
{
	if (CREX_FCC_INITIALIZED(LIBXML(entity_loader_callback))) {
		crex_fcc_dtor(&LIBXML(entity_loader_callback));
	}
...
static crex_result crx_libxml_post_deactivate(void)
{
	...
	smart_str_free(&LIBXML(error_buffer));
	if (LIBXML(error_list)) {
		crex_llist_destroy(LIBXML(error_list));
...
static CRX_MINFO_FUNCTION(libxml)
{
	...
	crx_info_print_table_row(2, "libXML support", "active");
	crx_info_print_table_row(2, "libXML Compiled Version", LIBXML_DOTTED_VERSION);
	crx_info_print_table_row(2, "libXML Loaded Version", (char *)xmlParserVersion);
	crx_info_print_table_row(2, "libXML streams", "enabled");
	crx_info_print_table_end();
}
/* }}} */

/* {{{ Set the streams context for the next libxml document load or write */
CRX_FUNCTION(libxml_set_streams_context)
{
	zval *arg;

	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_RESOURCE(arg)
	CREX_PARSE_PARAMETERS_END();

	if (!C_ISUNDEF(LIBXML(stream_context))) {
...
CRX_FUNCTION(libxml_use_internal_errors)
{
	...
	CREX_PARSE_PARAMETERS_START(0, 1)
		C_PARAM_OPTIONAL
		C_PARAM_BOOL_OR_NULL(use_errors, use_errors_is_null)
	CREX_PARSE_PARAMETERS_END();

	current_handler = xmlStructuredError;
	if (current_handler && current_handler == crx_libxml_structured_error_handler) {
		retval = 1;
	} else {
		retval = 0;
	}
	...
	if (use_errors == 0) {
		...
		xmlSetStructuredErrorFunc(NULL, crx_libxml_structured_error_handler);
		if (LIBXML(error_list) == NULL) {
			LIBXML(error_list) = (crex_llist *) emalloc(sizeof(crex_llist));
			crex_llist_init(LIBXML(error_list), sizeof(xmlError), _crx_libxml_free_error, 0);
		}
	}
	RETURN_BOOL(retval);
}
...
CRX_FUNCTION(libxml_get_last_error)
{
	xmlErrorPtr error;
...
CRX_FUNCTION(libxml_get_errors)
{
	...
	if (LIBXML(error_list)) {
		...
		error = crex_llist_get_first(LIBXML(error_list));
		...
		while (error != NULL) {
			...
			error = crex_llist_get_next(LIBXML(error_list));
		}
	} else {
		RETURN_EMPTY_ARRAY();
	}
...
CRX_FUNCTION(libxml_clear_errors)
{
	CREX_PARSE_PARAMETERS_NONE();

	xmlResetLastError();
	if (LIBXML(error_list)) {
		crex_llist_clean(LIBXML(error_list));
	}
}
/* }}} */
...
CRX_FUNCTION(libxml_disable_entity_loader)
{
	bool disable = 1;

	CREX_PARSE_PARAMETERS_START(0, 1)
		C_PARAM_OPTIONAL
		C_PARAM_BOOL(disable)
	CREX_PARSE_PARAMETERS_END();

	RETURN_BOOL(crx_libxml_disable_entity_loader(disable));
}
/* }}} */

/* {{{ Changes the default external entity loader */
CRX_FUNCTION(libxml_set_external_entity_loader)
{
	crex_fcall_info			fci;
	...
	if (CREX_FCI_INITIALIZED(fci)) { /* argument not null */
		...
		crex_fcc_dup(&LIBXML(entity_loader_callback), &fcc);
	}
	RETURN_TRUE;
}
/* }}} */
...
CRX_FUNCTION(libxml_get_external_entity_loader)
{
	CREX_PARSE_PARAMETERS_NONE();

	if (CREX_FCC_INITIALIZED(LIBXML(entity_loader_callback))) {
		zval tmp;
		crex_get_callable_zval_from_fcc(&LIBXML(entity_loader_callback), &tmp);
		RETVAL_COPY(&tmp);
		zval_ptr_dtor(&tmp);
		return;
	}
...
int crx_libxml_xmlCheckUTF8(const unsigned char *s)
{
	size_t i;
	unsigned char c;
...
zval *crx_libxml_register_export(crex_class_entry *ce, crx_libxml_export_node export_function)
{
	crx_libxml_func_handler export_hnd;

	/* Initialize in case this module hasn't been loaded yet */
	crx_libxml_initialize();
	export_hnd.export_func = export_function;

	return crex_hash_add_mem(&crx_libxml_exports, ce->name, &export_hnd, sizeof(export_hnd));
}
...
CRX_LIBXML_API xmlNodePtr crx_libxml_import_node(zval *object)
{
	...
	if (C_TYPE_P(object) == IS_OBJECT) {
		...
		if ((export_hnd = crex_hash_find_ptr(&crx_libxml_exports, ce->name))) {
...
CRX_LIBXML_API int crx_libxml_increment_node_ptr(crx_libxml_node_object *object, xmlNodePtr node, void *private_data)
{
	...
	if (object != NULL && node != NULL) {
		...
		if (node->_private != NULL) {
			...
			object->node = emalloc(sizeof(crx_libxml_node_ptr));
			ret_refcount = 1;
...
{
	int ret_refcount = -1;
	crx_libxml_node_ptr *obj_node;

	if (object != NULL && object->node != NULL) {
		obj_node = (crx_libxml_node_ptr *) object->node;
...
CRX_LIBXML_API int crx_libxml_increment_doc_ref(crx_libxml_node_object *object, xmlDocPtr docp)
{
	...
	if (object->document != NULL) {
		...
		ret_refcount = 1;
		object->document = emalloc(sizeof(crx_libxml_ref_obj));
		...
		if (ret_refcount == 0) {
			...
			if (object->document->doc_props != NULL) {
				if (object->document->doc_props->classmap) {
					crex_hash_destroy(object->document->doc_props->classmap);
	...
	switch (node->type) {
		...
		default:
			if (node->parent == NULL || node->type == XML_NAMESPACE_DECL) {
				crx_libxml_node_free_list((xmlNodePtr) node->children);
				switch (node->type) {
					/* Skip property freeing for the following types */
					case XML_ATTRIBUTE_DECL:
					case XML_DTD_NODE:
...
{
	if (object != NULL && object->node != NULL) {
		crx_libxml_node_ptr *obj_node = (crx_libxml_node_ptr *) object->node;
		xmlNodePtr nodep = obj_node->node;
		int ret_refcount = crx_libxml_decrement_node_ptr(object);
		if (ret_refcount == 0) {
			crx_libxml_node_free_resource(nodep);
		} else {
	...
	if (object != NULL && object->document != NULL) {
		...
		crx_libxml_decrement_doc_ref(object);
	}
}
/* }}} */

#if defined(CRX_WIN32) && defined(COMPILE_DL_LIBXML)
CRX_LIBXML_API BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
