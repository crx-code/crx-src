...
#define USE_MBSTRING crex_hash_str_exists(&module_registry, "mbstring", sizeof("mbstring")-1)
...
/* {{{ CRX_MINFO_FUNCTION */
CRX_MINFO_FUNCTION(exif)
{
	crx_info_print_table_start();
	crx_info_print_table_row(2, "EXIF Support", "enabled");
	crx_info_print_table_row(2, "Supported EXIF Version", "0220");
	crx_info_print_table_row(2, "Supported filetypes", "JPEG, TIFF");

	if (crex_hash_str_exists(&module_registry, "mbstring", sizeof("mbstring")-1)) {
		crx_info_print_table_row(2, "Multibyte decoding support using mbstring", "enabled");
	} else {
		crx_info_print_table_row(2, "Multibyte decoding support using mbstring", "disabled");
	}

	crx_info_print_table_row(2, "Extended EXIF tag formats", "Canon, Casio, Fujifilm, Nikon, Olympus, Samsung, Panasonic, DJI, Sony, Pentax, Minolta, Sigma, Foveon, Kyocera, Ricoh, AGFA, Epson");
...
CREX_BEGIN_MODULE_GLOBALS(exif)
	char * encode_unicode;
...
CREX_END_MODULE_GLOBALS(exif)

CREX_DECLARE_MODULE_GLOBALS(exif)
#define EXIF_G(v) CREX_MODULE_GLOBALS_ACCESSOR(exif, v)
...
CREX_INI_MH(OnUpdateEncode)
{
	if (new_value && ZSTR_LEN(new_value)) {
		const crex_encoding **return_list;
		size_t return_size;
		if (FAILURE == crex_multibyte_parse_encoding_list(ZSTR_VAL(new_value), ZSTR_LEN(new_value),
	&return_list, &return_size, 0)) {
			crx_error_docref(NULL, E_WARNING, "Illegal encoding ignored: '%s'", ZSTR_VAL(new_value));
			return FAILURE;
		}
...
CRX_INI_BEGIN()
	STD_CRX_INI_ENTRY("exif.encode_unicode",          "ISO-8859-15", CRX_INI_ALL, OnUpdateEncode, encode_unicode,    crex_exif_globals, exif_globals)
	STD_CRX_INI_ENTRY("exif.decode_unicode_motorola", "UCS-2BE",     CRX_INI_ALL, OnUpdateDecode, decode_unicode_be, crex_exif_globals, exif_globals)
	STD_CRX_INI_ENTRY("exif.decode_unicode_intel",    "UCS-2LE",     CRX_INI_ALL, OnUpdateDecode, decode_unicode_le, crex_exif_globals, exif_globals)
	STD_CRX_INI_ENTRY("exif.encode_jis",              "",            CRX_INI_ALL, OnUpdateEncode, encode_jis,        crex_exif_globals, exif_globals)
	STD_CRX_INI_ENTRY("exif.decode_jis_motorola",     "JIS",         CRX_INI_ALL, OnUpdateDecode, decode_jis_be,     crex_exif_globals, exif_globals)
	STD_CRX_INI_ENTRY("exif.decode_jis_intel",        "JIS",         CRX_INI_ALL, OnUpdateDecode, decode_jis_le,     crex_exif_globals, exif_globals)
CRX_INI_END()
/* }}} */

/* {{{ CRX_GINIT_FUNCTION */
static CRX_GINIT_FUNCTION(exif)
{
...
CRX_MSHUTDOWN_FUNCTION(exif)
{
	UNREGISTER_INI_ENTRIES();
	if (EXIF_G(tag_table_cache)) {
		crex_hash_destroy(EXIF_G(tag_table_cache));
...
/* {{{ exif dependencies */
static const crex_module_dep exif_module_deps[] = {
	CREX_MOD_REQUIRED("standard")
	CREX_MOD_OPTIONAL("mbstring")
	CREX_MOD_END
};
/* }}} */

/* {{{ exif_module_entry */
crex_module_entry exif_module_entry = {
...
CREX_GET_MODULE(exif)
#endif

/* crx_stream_read() may return early without reading all data, depending on the chunk size
...
static ssize_t exif_read_from_stream_file_looped(crx_stream *stream, char *buf, size_t count)
{
	ssize_t total_read = 0;
	while (total_read < count) {
		ssize_t ret = crx_stream_read(stream, buf + total_read, count - total_read);
		if (ret == -1) {
			return -1;
		}
		if (ret == 0) {
...
*/
static const int crx_tiff_bytes_per_format[] = {0, 1, 1, 2, 4, 8, 1, 1, 2, 4, 8, 4, 8, 1};
...
/* Remember to update CRX_MINFO if updated */
...
static HashTable *exif_make_tag_ht(tag_info_type *tag_table)
{
	...
	crex_hash_init(ht, 0, NULL, NULL, 1);
	while (tag_table->Tag != TAG_END_OF_LIST) {
		if (!crex_hash_index_add_ptr(ht, tag_table->Tag, tag_table->Desc)) {
			crex_error(E_CORE_ERROR, "Duplicate tag %x", tag_table->Tag);
		}
		tag_table++;
	}
	return ht;
...
static HashTable *exif_get_tag_ht(tag_info_type *tag_table)
{
	...
	if (!EXIF_G(tag_table_cache)) {
		...
		crex_hash_init(EXIF_G(tag_table_cache), 0, NULL, exif_tag_ht_dtor, 1);
	}

	ht = crex_hash_index_find_ptr(EXIF_G(tag_table_cache), (uintptr_t) tag_table);
	if (ht) {
		return ht;
	}

	ht = exif_make_tag_ht(tag_table);
	crex_hash_index_add_new_ptr(EXIF_G(tag_table_cache), (uintptr_t) tag_table, ht);
	return ht;
}
...
{
	return crex_hash_index_find_ptr(exif_get_tag_ht(tag_table), tag_num);
}
/* }}} */

static char *exif_get_tagname_debug(int tag_num, tag_table_type tag_table)
{
	char *desc = crex_hash_index_find_ptr(exif_get_tag_ht(tag_table), tag_num);
	if (desc) {
		return desc;
	}
	return "UndefinedTag";
...
*/
static int crx_jpg_get16(void *value)
{
...
static int crx_ifd_get16u(void *value, int motorola_intel)
{
	if (motorola_intel) {
		return (((uchar *)value)[0] << 8) | ((uchar *)value)[1];
	} else {
...
static signed short crx_ifd_get16s(void *value, int motorola_intel)
{
	return (signed short)crx_ifd_get16u(value, motorola_intel);
}
/* }}} */

/* {{{ crx_ifd_get32u
 * Convert a 32 bit unsigned value from file's native byte order */
static unsigned crx_ifd_get32u(void *void_value, int motorola_intel)
{
...
static uint64_t crx_ifd_get64u(void *void_value, int motorola_intel)
{
	uchar *value = (uchar *) void_value;
	if (motorola_intel) {
...
static unsigned crx_ifd_get32s(void *value, int motorola_intel)
{
	return (int) crx_ifd_get32u(value, motorola_intel);
}
/* }}} */

/* {{{ crx_ifd_set16u
 * Write 16 bit unsigned value to data */
static void crx_ifd_set16u(char *data, unsigned int value, int motorola_intel)
{
	if (motorola_intel) {
...
/* {{{ crx_ifd_set32u
 * Convert a 32 bit unsigned value from file's native byte order */
static void crx_ifd_set32u(char *data, size_t value, int motorola_intel)
{
	if (motorola_intel) {
...
static float crx_ifd_get_float(char *data) {
	union { uint32_t i; float f; } u;
	u.i = crx_ifd_get32u(data, 0);
	return u.f;
}

static double crx_ifd_get_double(char *data) {
	union { uint64_t i; double f; } u;
	u.i = crx_ifd_get64u(data, 0);
	return u.f;
}
	...
	while(components > 0) {
		switch(format) {
			...
			case TAG_FMT_SSHORT:
				...
				snprintf(dump + len, 6 + 1, "0x%04X", crx_ifd_get16s(value_ptr, motorola_intel));
				len += 6;
				value_ptr += 2;
				break;
			case TAG_FMT_ULONG:
			case TAG_FMT_SLONG:
				dump = erealloc(dump, len + 6 + 1);
				snprintf(dump + len, 6 + 1, "0x%04X", crx_ifd_get32s(value_ptr, motorola_intel));
				len += 6;
				value_ptr += 4;
				break;
			...
			case TAG_FMT_SRATIONAL:
				...
				snprintf(dump + len, 13 + 1, "0x%04X/0x%04X", crx_ifd_get32s(value_ptr, motorola_intel), crx_ifd_get32s(value_ptr+4, motorola_intel));
				len += 13;
				value_ptr += 8;
				break;
		}
...
static double exif_convert_any_format(void *value, int format, int motorola_intel)
{
	...
	switch(format) {
		...
		case TAG_FMT_USHORT:    return crx_ifd_get16u(value, motorola_intel);
		case TAG_FMT_ULONG:     return crx_ifd_get32u(value, motorola_intel);

		case TAG_FMT_URATIONAL:
			u_den = crx_ifd_get32u(4+(char *)value, motorola_intel);
			if (u_den == 0) {
				return 0;
			} else {
				return (double)crx_ifd_get32u(value, motorola_intel) / u_den;
			}

		case TAG_FMT_SRATIONAL:
			s_den = crx_ifd_get32s(4+(char *)value, motorola_intel);
			if (s_den == 0) {
				return 0;
			} else {
				return (double)crx_ifd_get32s(value, motorola_intel) / s_den;
			}

		case TAG_FMT_SSHORT:    return (signed short)crx_ifd_get16u(value, motorola_intel);
		case TAG_FMT_SLONG:     return crx_ifd_get32s(value, motorola_intel);

		/* Not sure if this is correct (never seen float used in Exif format) */
		case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
			crx_error_docref(NULL, E_NOTICE, "Found value of type single");
#endif
			return (double) crx_ifd_get_float(value);
		case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
			crx_error_docref(NULL, E_NOTICE, "Found value of type double");
#endif
			return crx_ifd_get_double(value);
	}
	return 0;
}
/* }}} */
...
static size_t exif_convert_any_to_int(void *value, int format, int motorola_intel)
{
	switch (format) {
		...
		case TAG_FMT_URATIONAL: {
			unsigned u_den = crx_ifd_get32u(4+(char *)value, motorola_intel);
			if (u_den == 0) {
				return 0;
			} else {
				return crx_ifd_get32u(value, motorola_intel) / u_den;
			}
		}

		case TAG_FMT_SRATIONAL: {
			int s_num = crx_ifd_get32s(value, motorola_intel);
			int s_den = crx_ifd_get32s(4+(char *)value, motorola_intel);
			if (s_den == 0) {
				return 0;
		...
		case TAG_FMT_SSHORT:    return crx_ifd_get16u(value, motorola_intel);
		case TAG_FMT_SLONG:     return crx_ifd_get32s(value, motorola_intel);
		...
		case TAG_FMT_SINGLE:
			...
			crx_error_docref(NULL, E_NOTICE, "Found value of type single");
#endif
			return float_to_size_t(crx_ifd_get_float(value));
		case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
			crx_error_docref(NULL, E_NOTICE, "Found value of type double");
#endif
			return double_to_size_t(crx_ifd_get_double(value));
	}
	return 0;
}
/* }}} */
...
static void exif_error_docref(const char *docref EXIFERR_DC, image_info_type *ImageInfo, int type, const char *format, ...)
{
...
#ifdef EXIF_DEBUG
	{
		...
		crx_verror(docref, ImageInfo && ImageInfo->FileName ? ImageInfo->FileName:"", type, buf, args);
		efree(buf);
	}
#else
	crx_verror(docref, ImageInfo && ImageInfo->FileName ? ImageInfo->FileName:"", type, format, args);
#endif
	va_end(args);
}
/* }}} */
...
static crex_always_inline bool ptr_offset_overflows(char *ptr, size_t offset) {
...
static inline void exif_offset_info_init(
	...
	CREX_ASSERT(!ptr_offset_overflows(valid_start, valid_length));
#ifdef __SANITIZE_ADDRESS__
	CREX_ASSERT(!__asan_region_is_poisoned(valid_start, valid_length));
#endif
...
static void exif_iif_add_value(image_info_type *image_info, int section_index, char *name, int tag, int format, int length, void* value, size_t value_len, int motorola_intel)
{
	...
	switch (format) {
		case TAG_FMT_STRING:
			...
			if (value) {
				length = (int)crex_strnlen(value, length);
		...
		case TAG_FMT_DOUBLE:
			...
			for (idex=0,vptr=value; idex<(size_t)length; idex++,vptr=(char *) vptr + crx_tiff_bytes_per_format[format]) {
				if ((char *) vptr_end - (char *) vptr < crx_tiff_bytes_per_format[format]) {
				...
				switch (format) {
					case TAG_FMT_USHORT:
						info_value->u = crx_ifd_get16u(vptr, motorola_intel);
						break;

					case TAG_FMT_ULONG:
						info_value->u = crx_ifd_get32u(vptr, motorola_intel);
						break;

					case TAG_FMT_URATIONAL:
						info_value->ur.num = crx_ifd_get32u(vptr, motorola_intel);
						info_value->ur.den = crx_ifd_get32u(4+(char *)vptr, motorola_intel);
						break;

					case TAG_FMT_SSHORT:
						info_value->i = crx_ifd_get16s(vptr, motorola_intel);
						break;

					case TAG_FMT_SLONG:
						info_value->i = crx_ifd_get32s(vptr, motorola_intel);
						break;

					case TAG_FMT_SRATIONAL:
						info_value->sr.num = crx_ifd_get32u(vptr, motorola_intel);
						info_value->sr.den = crx_ifd_get32u(4+(char *)vptr, motorola_intel);
						break;

					case TAG_FMT_SINGLE:
#ifdef EXIF_DEBUG
						crx_error_docref(NULL, E_WARNING, "Found value of type single");
#endif
						info_value->f = crx_ifd_get_float(value);
						break;
					case TAG_FMT_DOUBLE:
#ifdef EXIF_DEBUG
						crx_error_docref(NULL, E_WARNING, "Found value of type double");
#endif
						info_value->d = crx_ifd_get_double(value);
						break;
				}
			}
	}
...
static void exif_process_SOFn (uchar *Data, int marker, jpeg_sof_info *result)
{
	...
	result->height          = crx_jpg_get16(Data+3);
	result->width           = crx_jpg_get16(Data+5);
...
CRX_FUNCTION(exif_tagname)
{
	crex_long tag;
	char *szTemp;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l", &tag) == FAILURE) {
		RETURN_THROWS();
	}
...
static void* exif_ifd_make_value(image_info_data *info_data, int motorola_intel) {
	...
	byte_count = crx_tiff_bytes_per_format[info_data->format] * info_data->length;
	  ...
	  || (byte_count>1 && (info_data->format == TAG_FMT_BYTE || info_data->format == TAG_FMT_SBYTE))
		...
		*value_ptr = info_data->value.i;
		...
		for(i=0; i<info_data->length; i++) {
			...
			switch(info_data->format) {
				case TAG_FMT_USHORT:
					crx_ifd_set16u(data_ptr, info_value->u, motorola_intel);
					data_ptr += 2;
					break;
				case TAG_FMT_ULONG:
					crx_ifd_set32u(data_ptr, info_value->u, motorola_intel);
					data_ptr += 4;
					break;
				case TAG_FMT_SSHORT:
					crx_ifd_set16u(data_ptr, info_value->i, motorola_intel);
					data_ptr += 2;
					break;
				case TAG_FMT_SLONG:
					crx_ifd_set32u(data_ptr, info_value->i, motorola_intel);
					data_ptr += 4;
					break;
				case TAG_FMT_URATIONAL:
					crx_ifd_set32u(data_ptr,   info_value->sr.num, motorola_intel);
					crx_ifd_set32u(data_ptr+4, info_value->sr.den, motorola_intel);
					data_ptr += 8;
					break;
				case TAG_FMT_SRATIONAL:
					crx_ifd_set32u(data_ptr,   info_value->ur.num, motorola_intel);
					crx_ifd_set32u(data_ptr+4, info_value->ur.den, motorola_intel);
					data_ptr += 8;
					break;
				case TAG_FMT_SINGLE:
...
static void exif_thumbnail_build(image_info_type *ImageInfo) {
	...
	switch(ImageInfo->Thumbnail.filetype) {
		...
		case IMAGE_FILETYPE_TIFF_MM:
			...
			new_data += 8;
			crx_ifd_set16u(new_data, info_list->count, ImageInfo->motorola_intel);
			new_data += 2;
			for (i=0; i<info_list->count; i++) {
				info_data  = &info_list->list[i];
				byte_count = crx_tiff_bytes_per_format[info_data->format] * info_data->length;
				...
				if (info_data->tag==TAG_STRIP_OFFSETS || info_data->tag==TAG_JPEG_INTERCHANGE_FORMAT) {
					crx_ifd_set16u(new_data + 0, info_data->tag,    ImageInfo->motorola_intel);
					crx_ifd_set16u(new_data + 2, TAG_FMT_ULONG,     ImageInfo->motorola_intel);
					crx_ifd_set32u(new_data + 4, 1,                 ImageInfo->motorola_intel);
					crx_ifd_set32u(new_data + 8, new_move,          ImageInfo->motorola_intel);
				} else {
					crx_ifd_set16u(new_data + 0, info_data->tag,    ImageInfo->motorola_intel);
					crx_ifd_set16u(new_data + 2, info_data->format, ImageInfo->motorola_intel);
					crx_ifd_set32u(new_data + 4, info_data->length, ImageInfo->motorola_intel);
					value_ptr  = exif_ifd_make_value(info_data, ImageInfo->motorola_intel);
					if (byte_count <= 4) {
						memmove(new_data+8, value_ptr, 4);
					} else {
						crx_ifd_set32u(new_data+8, new_value, ImageInfo->motorola_intel);
#ifdef EXIF_DEBUG
...
static int exif_process_string(char **result, char *value, size_t byte_count) {
	 ...
	 * memory page...so we use crex_strnlen in conjunction with memcpy and add the NUL
	 * char.
	 * estrdup would sometimes allocate more memory and does not return length
	 */
	if ((byte_count=crex_strnlen(value, byte_count)) > 0) {
	...
	if (ByteCount>=8) {
		...
		if (!memcmp(szValuePtr, "UNICODE\0", 8)) {
			*pszEncoding = estrdup((const char*)szValuePtr);
			...
			to = crex_multibyte_fetch_encoding(ImageInfo->encode_unicode);
			from = crex_multibyte_fetch_encoding(decode);
			/* XXX this will fail again if encoding_converter returns on error something different than SIZE_MAX   */
			if (!to || !from || crex_multibyte_encoding_converter(
			...
			*pszEncoding = estrdup((const char*)szValuePtr);
			...
			ByteCount -= 8;
			/* XXX this will fail again if encoding_converter returns on error something different than SIZE_MAX   */
			to = crex_multibyte_fetch_encoding(ImageInfo->encode_jis);
			from = crex_multibyte_fetch_encoding(ImageInfo->motorola_intel ? ImageInfo->decode_jis_be : ImageInfo->decode_jis_le);
			if (!to || !from || crex_multibyte_encoding_converter(
					(unsigned char**)pszInfoPtr,
					&len,
					(unsigned char*)szValuePtr,
...
static int exif_process_unicode(image_info_type *ImageInfo, xp_field_type *xp_field, int tag, char *szValuePtr, int ByteCount)
{
	...
	xp_field->value = NULL;
	/* XXX this will fail again if encoding_converter returns on error something different than SIZE_MAX   */
	if (crex_multibyte_encoding_converter(
			(unsigned char**)&xp_field->value,
			&xp_field->size,
			(unsigned char*)szValuePtr,
			ByteCount,
			crex_multibyte_fetch_encoding(ImageInfo->encode_unicode),
			crex_multibyte_fetch_encoding(ImageInfo->motorola_intel ? ImageInfo->decode_unicode_be : ImageInfo->decode_unicode_le)
			) == (size_t)-1) {
...
static bool exif_process_IFD_in_MAKERNOTE(image_info_type *ImageInfo, char * value_ptr, int value_len, const exif_offset_info *info, size_t displacement)
{
	...
	NumDirEntries = crx_ifd_get16u(dir_start, ImageInfo->motorola_intel);
...
/*	NextDirOffset (must be NULL) = crx_ifd_get32u(dir_start+2+12*de, ImageInfo->motorola_intel);*/
#ifdef EXIF_DEBUG
...
static bool exif_process_IFD_TAG_impl(image_info_type *ImageInfo, char *dir_entry, const exif_offset_info *info, size_t displacement, int section_index, int ReadNextIFD, tag_table_type tag_table)
{
	...
	tag = crx_ifd_get16u(dir_entry, ImageInfo->motorola_intel);
	format = crx_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
	components = crx_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel);
	...
	byte_count_signed = (int64_t)components * crx_tiff_bytes_per_format[format];
	...
	if (byte_count > 4) {
		...
		offset_val = crx_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
		...
		if (!value_ptr) {
			...
			fpos = crx_stream_tell(ImageInfo->infile);
			crx_stream_seek(ImageInfo->infile, displacement+offset_val, SEEK_SET);
			fgot = crx_stream_tell(ImageInfo->infile);
			...
			crx_stream_seek(ImageInfo->infile, fpos, SEEK_SET);
			if (fgot != byte_count) {
				EFREE_IF(outside);
	...
	if (section_index==SECTION_THUMBNAIL) {
		...
		switch(tag) {
			case TAG_COPYRIGHT:
				...
				if (byte_count>1 && (length=crex_strnlen(value_ptr, byte_count)) > 0) {
			...
			case TAG_INTEROP_IFD_POINTER:
				if (ReadNextIFD) {
					...
					offset_val = crx_ifd_get32u(value_ptr, ImageInfo->motorola_intel);
...
static bool exif_process_IFD_in_JPEG(image_info_type *ImageInfo, char *dir_start, const exif_offset_info *info, size_t displacement, int section_index, int tag)
{
	...
	if (tag != TAG_EXIF_IFD_POINTER && tag != TAG_GPS_IFD_POINTER) {
		NextDirOffset = crx_ifd_get32u(dir_start+2+12*de, ImageInfo->motorola_intel);
	}
...
static void exif_process_TIFF_in_JPEG(image_info_type *ImageInfo, char *CharBuf, size_t length, size_t displacement)
{
	...
	exif_value_2a = crx_ifd_get16u(CharBuf+2, ImageInfo->motorola_intel);
	offset_of_ifd = crx_ifd_get32u(CharBuf+4, ImageInfo->motorola_intel);
...
static bool exif_scan_JPEG_header(image_info_type *ImageInfo)
{
	...
	while (true) {
		...
		do {
			if ((marker = crx_stream_getc(ImageInfo->infile)) == EOF) {
		...
		CREX_ASSERT(marker != 0xff);

		/* Read the length of the section. */
		if ((lh = crx_stream_getc(ImageInfo->infile)) == (unsigned int)EOF) {
			EXIF_ERRLOG_CORRUPT(ImageInfo)
			return false;
		}
		if ((ll = crx_stream_getc(ImageInfo->infile)) == (unsigned int)EOF) {
			EXIF_ERRLOG_CORRUPT(ImageInfo)
			return false;
		}
...
static bool exif_process_IFD_in_TIFF_impl(image_info_type *ImageInfo, size_t dir_offset, int section_index)
{
	...
	if (ImageInfo->FileSize >= 2 && ImageInfo->FileSize - 2 >= dir_offset) {
...
#endif
		crx_stream_seek(ImageInfo->infile, dir_offset, SEEK_SET); /* we do not know the order of sections */
		if (UNEXPECTED(exif_read_from_stream_file_looped(ImageInfo->infile, (char*)ImageInfo->file.list[sn].data, 2) != 2)) {
			return false;
		}
		num_entries = crx_ifd_get16u(ImageInfo->file.list[sn].data, ImageInfo->motorola_intel);
		...
		if (ImageInfo->FileSize >= dir_size && ImageInfo->FileSize - dir_size >= dir_offset) {
			...
			next_offset = crx_ifd_get32u(ImageInfo->file.list[sn].data + dir_size - 4, ImageInfo->motorola_intel);
#ifdef EXIF_DEBUG
			...
			for(i=0;i<num_entries;i++) {
				...
				entry_tag    = crx_ifd_get16u(dir_entry+0, ImageInfo->motorola_intel);
				entry_type   = crx_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
				if (entry_type > NUM_FORMATS) {
					...
					/* crx_ifd_set16u(dir_entry+2, entry_type, ImageInfo->motorola_intel);*/
				}
				entry_length = crx_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel) * crx_tiff_bytes_per_format[entry_type];
				if (entry_length <= 4) {
					switch(entry_type) {
						case TAG_FMT_USHORT:
							entry_value  = crx_ifd_get16u(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_SSHORT:
							entry_value  = crx_ifd_get16s(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_ULONG:
							entry_value  = crx_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
							break;
						case TAG_FMT_SLONG:
							entry_value  = crx_ifd_get32s(dir_entry+8, ImageInfo->motorola_intel);
							break;
					}
					switch(entry_tag) {
						case TAG_IMAGEWIDTH:
					...
					entry_offset = crx_ifd_get32u(dir_entry+8, ImageInfo->motorola_intel);
			...
			if (ImageInfo->FileSize >= ImageInfo->file.list[sn].size && ImageInfo->FileSize - ImageInfo->file.list[sn].size >= dir_offset) {
				...
				for(i=0;i<num_entries;i++) {
					...
					entry_tag    = crx_ifd_get16u(dir_entry+0, ImageInfo->motorola_intel);
					entry_type   = crx_ifd_get16u(dir_entry+2, ImageInfo->motorola_intel);
					/*entry_length = crx_ifd_get32u(dir_entry+4, ImageInfo->motorola_intel);*/
						...
						if (section_index!=SECTION_THUMBNAIL && entry_tag==TAG_SUB_IFD) {
							...
							&&  ImageInfo->read_thumbnail
								...
								if (!ImageInfo->Thumbnail.data) {
									...
									crx_stream_seek(ImageInfo->infile, ImageInfo->Thumbnail.offset, SEEK_SET);
...
static bool exif_scan_FILE_header(image_info_type *ImageInfo)
{
	...
	if (ImageInfo->FileSize >= 2) {
		crx_stream_seek(ImageInfo->infile, 0, SEEK_SET);
		...
		if ((file_header[0]==0xff) && (file_header[1]==M_SOI)) {
			...
			if (!memcmp(file_header, "II\x2A\x00", 4)) {
				...
				if (exif_process_IFD_in_TIFF(ImageInfo,
											 crx_ifd_get32u(file_header + 4, ImageInfo->motorola_intel),
											 SECTION_IFD0)) {
					ret = true;
				} else {
...
/* {{{ exif_read_from_impl */
static bool exif_read_from_impl(image_info_type *ImageInfo, crx_stream *stream, int read_thumbnail, int read_all)
{
	bool ret;
	crex_stat_t st = {0};
	...
	if (crx_stream_is(ImageInfo->infile, CRX_STREAM_IS_STDIO)) {
		if (VCWD_STAT(stream->orig_path, &st) >= 0) {
			...
			/* Store file name */
			base = crx_basename(stream->orig_path, strlen(stream->orig_path), NULL, 0);
			ImageInfo->FileName = estrndup(ZSTR_VAL(base), ZSTR_LEN(base));

			crex_string_release_ex(base, 0);
		...
		if (!ImageInfo->FileSize) {
			crx_stream_seek(ImageInfo->infile, 0, SEEK_END);
			ImageInfo->FileSize = crx_stream_tell(ImageInfo->infile);
			crx_stream_seek(ImageInfo->infile, 0, SEEK_SET);
		}
	}
...
static bool exif_read_from_stream(image_info_type *ImageInfo, crx_stream *stream, int read_thumbnail, int read_all)
{
	bool ret;
	off_t old_pos = crx_stream_tell(stream);
	...
	if (old_pos) {
		crx_stream_seek(stream, old_pos, SEEK_SET);
	}
...
static bool exif_read_from_file(image_info_type *ImageInfo, char *FileName, int read_thumbnail, int read_all)
{
	...
	stream = crx_stream_open_wrapper(FileName, "rb", STREAM_MUST_SEEK | IGNORE_PATH, NULL);
...
CRX_FUNCTION(exif_read_data)
{
	crex_string *z_sections_needed = NULL;
	...
	CREX_PARSE_PARAMETERS_START(1, 4)
		C_PARAM_ZVAL(stream)
		C_PARAM_OPTIONAL
		C_PARAM_STR_OR_NULL(z_sections_needed)
		C_PARAM_BOOL(sub_arrays)
		C_PARAM_BOOL(read_thumbnail)
	...
	if (C_TYPE_P(stream) == IS_RESOURCE) {
		crx_stream *p_stream = NULL;

		crx_stream_from_res(p_stream, C_RES_P(stream));
		...
		if (!C_STRLEN_P(stream)) {
			crex_argument_value_error(1, "cannot be empty");
			RETURN_THROWS();
		}

		if (CHECK_NULL_PATH(C_STRVAL_P(stream), C_STRLEN_P(stream))) {
			crex_argument_value_error(1, "must not contain any null bytes");
			RETURN_THROWS();
		}

		ret = exif_read_from_file(&ImageInfo, C_STRVAL_P(stream), read_thumbnail, read_all);
	}
	...
	exif_iif_add_str(&ImageInfo, SECTION_FILE, "MimeType",      (char*)crx_image_type_to_mime_type(ImageInfo.FileType));
	...
	if (ImageInfo.Thumbnail.size) {
		...
		exif_iif_add_str(&ImageInfo, SECTION_COMPUTED, "Thumbnail.MimeType", (char*)crx_image_type_to_mime_type(ImageInfo.Thumbnail.filetype));
	}
	if (ImageInfo.Thumbnail.width && ImageInfo.Thumbnail.height) {
...
#ifdef EXIF_DEBUG
	crx_error_docref1(NULL, (C_TYPE_P(stream) == IS_RESOURCE ? "<stream>" : C_STRVAL_P(stream)), E_NOTICE, "Done");
#endif
}
/* }}} */

/* {{{ Reads the embedded thumbnail */
CRX_FUNCTION(exif_thumbnail)
{
	bool ret;
	int arg_c = CREX_NUM_ARGS();
	image_info_type ImageInfo;
	zval *stream;
	...
	CREX_PARSE_PARAMETERS_START(1, 4)
		C_PARAM_ZVAL(stream)
		C_PARAM_OPTIONAL
		C_PARAM_ZVAL(z_width)
		C_PARAM_ZVAL(z_height)
		C_PARAM_ZVAL(z_imagetype)
	CREX_PARSE_PARAMETERS_END();
	...
	if (C_TYPE_P(stream) == IS_RESOURCE) {
		...
		ret = exif_read_from_file(&ImageInfo, C_STRVAL_P(stream), 1, 0);
	}
	...
	if (arg_c >= 3) {
		...
		CREX_TRY_ASSIGN_REF_LONG(z_width,  ImageInfo.Thumbnail.width);
		CREX_TRY_ASSIGN_REF_LONG(z_height, ImageInfo.Thumbnail.height);
	}
	if (arg_c >= 4)	{
		CREX_TRY_ASSIGN_REF_LONG(z_imagetype, ImageInfo.Thumbnail.filetype);
	}
...
CRX_FUNCTION(exif_imagetype)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "p", &imagefile, &imagefile_len) == FAILURE) {
		RETURN_THROWS();
	}

	stream = crx_stream_open_wrapper(imagefile, "rb", IGNORE_PATH|REPORT_ERRORS, NULL);

	if (stream == NULL) {
		RETURN_FALSE;
	}

	itype = crx_getimagetype(stream, imagefile, NULL);
