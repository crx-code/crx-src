...
typedef struct _crex_gdbjit_obj {
	crex_elf_header     hdr;
	crex_elf_sectheader sect[GDBJIT_SECT__MAX];
	crex_elf_symbol     sym[GDBJIT_SYM__MAX];
	uint8_t             space[4096];
} crex_gdbjit_obj;

static const crex_elf_header crex_elfhdr_template = {
	.emagic      = { 0x7f, 'E', 'L', 'F' },
#ifdef ELF64
	.eclass      = 2,
#else
...
# error "Unsupported target architecture"
	...
	.entry       = 0,
	.phofs       = 0,
	.shofs       = offsetof(crex_gdbjit_obj, sect),
	.flags       = 0,
	.ehsize      = sizeof(crex_elf_header),
	.phentsize   = 0,
	.phnum       = 0,
	.shentsize   = sizeof(crex_elf_sectheader),
	.shnum       = GDBJIT_SECT__MAX,
	.shstridx    = GDBJIT_SECT_shstrtab
};

/* Context for generating the ELF object for the GDB JIT API. */
typedef struct _crex_gdbjit_ctx {
	...
	crex_gdbjit_obj obj;     /* In-memory ELF object. */
} crex_gdbjit_ctx;

/* Add a zero-terminated string */
static uint32_t crex_gdbjit_strz(crex_gdbjit_ctx *ctx, const char *str)
{
	uint8_t *p = ctx->p;
...
/* Add a ULEB128 value */
static void crex_gdbjit_uleb128(crex_gdbjit_ctx *ctx, uint32_t v)
{
	uint8_t *p = ctx->p;
	for (; v >= 0x80; v >>= 7)
...
/* Add a SLEB128 value */
static void crex_gdbjit_sleb128(crex_gdbjit_ctx *ctx, int32_t v)
{
	uint8_t *p = ctx->p;
...
static void crex_gdbjit_secthdr(crex_gdbjit_ctx *ctx)
{
	crex_elf_sectheader *sect;

	*ctx->p++ = '\0';

#define SECTDEF(id, tp, al)                       \
	sect = &ctx->obj.sect[GDBJIT_SECT_##id];      \
	sect->name = crex_gdbjit_strz(ctx, "." #id);  \
	...
	sect->ofs = offsetof(crex_gdbjit_obj, sym);
	sect->size = sizeof(ctx->obj.sym);
	sect->link = GDBJIT_SECT_strtab;
	sect->entsize = sizeof(crex_elf_symbol);
...
static void crex_gdbjit_symtab(crex_gdbjit_ctx *ctx)
{
	crex_elf_symbol *sym;
...
typedef CREX_SET_ALIGNED(1, uintptr_t unaligned_uintptr_t);
...
#define DUV(x)      (ctx->p = p, crex_gdbjit_uleb128(ctx, (x)), p = ctx->p)
#define DSV(x)      (ctx->p = p, crex_gdbjit_sleb128(ctx, (x)), p = ctx->p)
#define DSTR(str)   (ctx->p = p, crex_gdbjit_strz(ctx, (str)), p = ctx->p)
#define DALIGNNOP(s)    while ((uintptr_t)p & ((s)-1)) *p++ = DW_CFA_nop
#define DSECT(name, stmt) \
	{ unaligned_uint32_t *szp_##name = (uint32_t *)p; p += 4; stmt \
		*szp_##name = (uint32_t)((p-(uint8_t *)szp_##name)-4); }

static void crex_gdbjit_ehframe(crex_gdbjit_ctx *ctx, uint32_t sp_offset, uint32_t sp_adjustment)
{
	uint8_t *p = ctx->p;
	uint8_t *framep = p;
...
static void crex_gdbjit_debuginfo(crex_gdbjit_ctx *ctx)
{
	uint8_t *p = ctx->p;
...
static void crex_gdbjit_debugabbrev(crex_gdbjit_ctx *ctx)
{
	uint8_t *p = ctx->p;
...
static void crex_gdbjit_debugline(crex_gdbjit_ctx *ctx)
{
	uint8_t *p = ctx->p;
...
typedef void (*crex_gdbjit_initf) (crex_gdbjit_ctx *ctx);

static void crex_gdbjit_initsect(crex_gdbjit_ctx *ctx, int sect)
{
	ctx->startp = ctx->p;
	ctx->obj.sect[sect].ofs = (uintptr_t)((char *)ctx->p - (char *)&ctx->obj);
}

static void crex_gdbjit_initsect_done(crex_gdbjit_ctx *ctx, int sect)
{
	ctx->obj.sect[sect].size = (uintptr_t)(ctx->p - ctx->startp);
}

static void crex_gdbjit_buildobj(crex_gdbjit_ctx *ctx, uint32_t sp_offset, uint32_t sp_adjustment)
{
	crex_gdbjit_obj *obj = &ctx->obj;

	/* Fill in ELF header and clear structures. */
	memcpy(&obj->hdr, &crex_elfhdr_template, sizeof(crex_elf_header));
	memset(&obj->sect, 0, sizeof(crex_elf_sectheader) * GDBJIT_SECT__MAX);
	memset(&obj->sym, 0, sizeof(crex_elf_symbol) * GDBJIT_SYM__MAX);

	/* Initialize sections. */
	ctx->p = obj->space;
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_shstrtab); crex_gdbjit_secthdr(ctx); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_shstrtab);
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_strtab); crex_gdbjit_symtab(ctx); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_strtab);
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_debug_info); crex_gdbjit_debuginfo(ctx); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_debug_info);
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_debug_abbrev); crex_gdbjit_debugabbrev(ctx); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_debug_abbrev);
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_debug_line); crex_gdbjit_debugline(ctx); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_debug_line);
	SECTALIGN(ctx->p, sizeof(uintptr_t));
	crex_gdbjit_initsect(ctx, GDBJIT_SECT_eh_frame); crex_gdbjit_ehframe(ctx, sp_offset, sp_adjustment); crex_gdbjit_initsect_done(ctx, GDBJIT_SECT_eh_frame);
	ctx->objsize = (size_t)((char *)ctx->p - (char *)obj);

	CREX_ASSERT(ctx->objsize < sizeof(crex_gdbjit_obj));
}
                          ...
                          uint32_t       sp_adjustment)
{
	...
	crex_gdbjit_buildobj(&ctx, sp_offset, sp_adjustment);

	return crex_gdb_register_code(&ctx.obj, ctx.objsize);
}

int crex_jit_gdb_unregister(void)
{
	crex_gdb_unregister_all();
	return 1;
}
...
void crex_jit_gdb_init(void)
{
	...
	if (crex_gdb_present()) {
		JIT_G(debug) |= CREX_JIT_DEBUG_GDB;
	}
#endif
}
