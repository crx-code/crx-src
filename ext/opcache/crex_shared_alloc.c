...
#define SEM_FILENAME_PREFIX ".CrexSem."
...
static const crex_shared_memory_handlers *g_shared_alloc_handler = NULL;
static const char *g_shared_model;
/* pointer to globals allocated in SHM and shared across processes */
crex_smm_shared_globals *smm_shared_globals;
...
static const crex_shared_memory_handler_entry handler_table[] = {
#ifdef USE_MMAP
	{ "mmap", &crex_alloc_mmap_handlers },
#endif
#ifdef USE_SHM
	{ "shm", &crex_alloc_shm_handlers },
#endif
#ifdef USE_SHM_OPEN
	{ "posix", &crex_alloc_posix_handlers },
#endif
#ifdef CREX_WIN32
	{ "win32", &crex_alloc_win32_handlers },
#endif
	{ NULL, NULL}
};

#ifndef CREX_WIN32
void crex_shared_alloc_create_lock(char *lockfile_path)
{
	int val;
	...
	if (lock_file == -1) {
		crex_accel_error_noreturn(ACCEL_LOG_FATAL, "Unable to create lock file: %s (%d)", strerror(errno), errno);
	}
...
{
	crex_accel_error_noreturn(ACCEL_LOG_FATAL, "Unable to allocate shared memory segment of %zu bytes: %s: %s (%d)", allocate_size, error?error:"unknown", strerror(errno), errno );
}

static void copy_shared_segments(void *to, void *from, int count, int size)
{
	crex_shared_segment **shared_segments_v = (crex_shared_segment **)to;
...
static int crex_shared_alloc_try(const crex_shared_memory_handler_entry *he, size_t requested_size, crex_shared_segment ***shared_segments_p, int *shared_segments_count, const char **error_in)
{
	int res;
...
{
	crex_shared_segment **tmp_shared_segments;
	size_t shared_segments_array_size;
	crex_smm_shared_globals tmp_shared_globals, *p_tmp_shared_globals;
	const char *error_in = NULL;
	const crex_shared_memory_handler_entry *he;
	int res = ALLOC_FAILURE;
	int i;

	/* shared_free must be valid before we call crex_shared_alloc()
...
#ifndef CREX_WIN32
	crex_shared_alloc_create_lock(ZCG(accel_directives).lockfile_path);
#else
	crex_shared_alloc_create_lock();
#endif
	...
	if (ZCG(accel_directives).memory_model && ZCG(accel_directives).memory_model[0]) {
		...
		for (he = handler_table; he->name; he++) {
			if (strcmp(model, he->name) == 0) {
				res = crex_shared_alloc_try(he, requested_size, &ZSMMG(shared_segments), &ZSMMG(shared_segments_count), &error_in);
				if (res) {
					/* this model works! */
					break;
				}
	...
	p_tmp_shared_globals = (crex_smm_shared_globals *) crex_shared_alloc(sizeof(crex_smm_shared_globals));
	if (!p_tmp_shared_globals) {
		crex_accel_error_noreturn(ACCEL_LOG_FATAL, "Insufficient shared memory!");
		return ALLOC_FAILURE;
	}
	memset(p_tmp_shared_globals, 0, sizeof(crex_smm_shared_globals));

	tmp_shared_segments = crex_shared_alloc(shared_segments_array_size + ZSMMG(shared_segments_count) * sizeof(void *));
	...
	*p_tmp_shared_globals = tmp_shared_globals;
	...
	ZSMMG(shared_memory_state).positions = (size_t *)crex_shared_alloc(sizeof(size_t) * ZSMMG(shared_segments_count));
...
{
	crex_shared_segment **tmp_shared_segments;
	crex_shared_segment *shared_segments_buf[16];
	size_t shared_segments_array_size;
	crex_smm_shared_globals tmp_shared_globals;
	int i;
...
static size_t crex_shared_alloc_get_largest_free_block(void)
{
	int i;
...
#define SHARED_ALLOC_FAILED() do {		\
		crex_accel_error(ACCEL_LOG_WARNING, "Not enough free shared space to allocate %zu bytes (%zu bytes free)", size, ZSMMG(shared_free)); \
		if (crex_shared_alloc_get_largest_free_block() < MIN_FREE_MEMORY) { \
...
{
	CREX_ASSERT(ZCG(locked));

	int i;
	unsigned int block_size = CREX_ALIGNED_SIZE(size);

	if (UNEXPECTED(block_size < size)) {
		crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Possible integer overflow in shared memory allocation (%zu + %zu)", size, PLATFORM_ALIGNMENT);
	}
	...
	for (i = 0; i < ZSMMG(shared_segments_count); i++) {
		if (ZSMMG(shared_segments)[i]->end - ZSMMG(shared_segments)[i]->pos >= block_size) { /* found a valid block */
			...
			CREX_ASSERT(((uintptr_t)retval & 0x7) == 0); /* should be 8 byte aligned */
			return retval;
		}
	}
	SHARED_ALLOC_FAILED();
...
static crex_always_inline crex_ulong crex_rotr3(crex_ulong key)
{
	return (key >> 3) | (key << ((sizeof(key) * 8) - 3));
}

int crex_shared_memdup_size(void *source, size_t size)
{
	void *old_p;
	crex_ulong key = (crex_ulong)source;

	key = crex_rotr3(key);
	if ((old_p = crex_hash_index_find_ptr(&ZCG(xlat_table), key)) != NULL) {
		/* we already duplicated this pointer */
		return 0;
	}
	crex_hash_index_add_new_ptr(&ZCG(xlat_table), key, source);
	return CREX_ALIGNED_SIZE(size);
}

static crex_always_inline void *_crex_shared_memdup(void *source, size_t size, bool get_xlat, bool set_xlat, bool free_source)
{
	void *old_p, *retval;
	crex_ulong key;

	if (get_xlat) {
		key = (crex_ulong)source;
		key = crex_rotr3(key);
		if ((old_p = crex_hash_index_find_ptr(&ZCG(xlat_table), key)) != NULL) {
			/* we already duplicated this pointer */
			return old_p;
	...
	retval = ZCG(mem);
	ZCG(mem) = (void*)(((char*)ZCG(mem)) + CREX_ALIGNED_SIZE(size));
	...
	if (set_xlat) {
		...
		crex_hash_index_add_new_ptr(&ZCG(xlat_table), key, retval);
	}
	if (free_source) {
		efree(source);
	}
...
{
	return _crex_shared_memdup(source, size, true, true, true);
}

void *crex_shared_memdup_put_free(void *source, size_t size)
{
	return _crex_shared_memdup(source, size, false, true, true);
}

void *crex_shared_memdup_free(void *source, size_t size)
{
	return _crex_shared_memdup(source, size, false, false, true);
}

void *crex_shared_memdup_get_put(void *source, size_t size)
{
	return _crex_shared_memdup(source, size, true, true, false);
}

void *crex_shared_memdup_put(void *source, size_t size)
{
	return _crex_shared_memdup(source, size, false, true, false);
}

void *crex_shared_memdup(void *source, size_t size)
{
	return _crex_shared_memdup(source, size, false, false, false);
}
...
void crex_shared_alloc_lock(void)
{
	...
	while (1) {
		if (fcntl(lock_file, F_SETLKW, &mem_write_lock) == -1) {
			...
			crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Cannot create lock - %s (%d)", strerror(errno), errno);
		}
		break;
	}
#else
	crex_shared_alloc_lock_win32();
#endif

	ZCG(locked) = 1;
}

void crex_shared_alloc_unlock(void)
{
	CREX_ASSERT(ZCG(locked));
	...
	if (fcntl(lock_file, F_SETLK, &mem_write_unlock) == -1) {
		crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Cannot remove lock - %s (%d)", strerror(errno), errno);
	}
#ifdef ZTS
	tsrm_mutex_unlock(zts_lock);
#endif
#else
	crex_shared_alloc_unlock_win32();
#endif
}
...
{
	crex_hash_discard(&ZCG(xlat_table), checkpoint);
}

void crex_shared_alloc_register_xlat_entry(const void *key_pointer, const void *value)
{
	crex_ulong key = (crex_ulong)key_pointer;

	key = crex_rotr3(key);
	crex_hash_index_add_new_ptr(&ZCG(xlat_table), key, (void*)value);
}

void *crex_shared_alloc_get_xlat_entry(const void *key_pointer)
{
	void *retval;
	crex_ulong key = (crex_ulong)key_pointer;

	key = crex_rotr3(key);
	if ((retval = crex_hash_index_find_ptr(&ZCG(xlat_table), key)) == NULL) {
		return NULL;
	}
	return retval;
}
...
void crex_accel_shared_protect(bool protected)
{
	...
	for (i = 0; i < ZSMMG(shared_segments_count); i++) {
		...
		if (!VirtualProtect(ZSMMG(shared_segments)[i]->p, ZSMMG(shared_segments)[i]->end, mode, &oldProtect)) {
			crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Failed to protect memory");
		}
	}
#endif
}
