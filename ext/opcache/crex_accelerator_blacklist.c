...
#define CREX_BLACKLIST_BLOCK_SIZE	32

struct _crex_regexp_list {
	pcre2_code       *re;
	crex_regexp_list *next;
};

crex_blacklist accel_blacklist;

void crex_accel_blacklist_init(crex_blacklist *blacklist)
{
	blacklist->pos = 0;
	blacklist->size = CREX_BLACKLIST_BLOCK_SIZE;

	if (blacklist->entries != NULL) {
		crex_accel_blacklist_shutdown(blacklist);
	}

	blacklist->entries = (crex_blacklist_entry *) calloc(sizeof(crex_blacklist_entry), blacklist->size);
	if (!blacklist->entries) {
		crex_accel_error_noreturn(ACCEL_LOG_FATAL, "Blacklist initialization: no memory\n");
		return;
	}
...
{
	crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Blacklist compilation failed (offset: %d), %s\n", pcre_error_offset, pcre_error);
}

static void crex_accel_blacklist_update_regexp(crex_blacklist *blacklist)
{
	PCRE2_UCHAR pcre_error[128];
	int i, errnumber;
	PCRE2_SIZE pcre_error_offset;
	crex_regexp_list **regexp_list_it, *it;
	char regexp[12*1024], *p, *end, *c, *backtrack = NULL;
	pcre2_compile_context *cctx = crx_pcre_cctx();
	...
	for (i = 0; i < blacklist->pos; ) {
		...
		if (*c || i == blacklist->pos - 1) {
			if (*c) {
				if (!backtrack) {
					crex_accel_error_noreturn(ACCEL_LOG_ERROR, "Too long blacklist entry\n");
				}
				p = backtrack;
			} else {
				i++;
			...
			*p++ = ')';
			...
			it = (crex_regexp_list*)malloc(sizeof(crex_regexp_list));
			if (!it) {
				crex_accel_error_noreturn(ACCEL_LOG_ERROR, "malloc() failed\n");
				return;
			}
			it->next = NULL;
			...
			if (PCRE_G(jit)) {
				if (0 > pcre2_jit_compile(it->re, PCRE2_JIT_COMPLETE)) {
					...
					pcre2_get_error_message(errnumber, pcre_error, sizeof(pcre_error));
					crex_accel_error(ACCEL_LOG_WARNING, "Blacklist JIT compilation failed, %s\n", pcre_error);
				}
			}
#endif
...
void crex_accel_blacklist_shutdown(crex_blacklist *blacklist)
{
	...
	crex_blacklist_entry *p = blacklist->entries, *end = blacklist->entries + blacklist->pos;
	while (p<end) {
		free(p->path);
		p++;
	}
	...
	if (blacklist->regexp_list) {
		crex_regexp_list *temp, *it = blacklist->regexp_list;
		while (it) {
...
static inline void crex_accel_blacklist_allocate(crex_blacklist *blacklist)
{
	if (blacklist->pos == blacklist->size) {
		blacklist->size += CREX_BLACKLIST_BLOCK_SIZE;
		blacklist->entries = (crex_blacklist_entry *) realloc(blacklist->entries, sizeof(crex_blacklist_entry)*blacklist->size);
	}
}

static void crex_accel_blacklist_loadone(crex_blacklist *blacklist, char *filename)
{
	...
	if ((fp = fopen(filename, "r")) == NULL) {
		crex_accel_error(ACCEL_LOG_WARNING, "Cannot load blacklist file: %s\n", filename);
		return;
	}

	crex_accel_error(ACCEL_LOG_DEBUG,"Loading blacklist file:  '%s'", filename);

	if (VCWD_REALPATH(filename, buf)) {
		blacklist_path_length = crex_dirname(buf, strlen(buf));
		blacklist_path = crex_strndup(buf, blacklist_path_length);
	}
	...
	while (fgets(buf, MAXPATHLEN, fp) != NULL) {
		...
		path_dup = crex_strndup(pbuf, path_length);
		if (blacklist_path) {
		...
		crex_accel_blacklist_allocate(blacklist);
...
void crex_accel_blacklist_load(crex_blacklist *blacklist, char *filename)
{
	...
	if (!globbuf.gl_pathc) {
#endif
		crex_accel_error(ACCEL_LOG_WARNING, "No blacklist file found matching: %s\n", filename);
	} else {
		for(i=0 ; i<globbuf.gl_pathc; i++) {
			crex_accel_blacklist_loadone(blacklist, globbuf.gl_pathv[i]);
		}
		globfree(&globbuf);
	}
#else
	crex_accel_blacklist_loadone(blacklist, filename);
#endif
	crex_accel_blacklist_update_regexp(blacklist);
}

bool crex_accel_blacklist_is_blacklisted(crex_blacklist *blacklist, char *verify_path, size_t verify_path_len)
{
	int ret = 0;
	crex_regexp_list *regexp_list_it = blacklist->regexp_list;
	...
	while (regexp_list_it != NULL) {
		pcre2_match_data *match_data = crx_pcre_create_match_data(0, regexp_list_it->re);
		if (!match_data) {
