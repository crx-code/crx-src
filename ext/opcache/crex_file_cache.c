...
#ifndef CREX_WIN32
#define crex_file_cache_unlink unlink
#define crex_file_cache_open open
#else
#define crex_file_cache_unlink crx_win32_ioutil_unlink
#define crex_file_cache_open crx_win32_ioutil_open
#endif
...
# define LOCK_EX 1
# define LOCK_UN 2
static int crex_file_cache_flock(int fd, int op)
{
...
# define crex_file_cache_flock flock
#else
# define LOCK_SH 0
# define LOCK_EX 1
# define LOCK_UN 2
static int crex_file_cache_flock(int fd, int type)
{
	return 0;
}
#endif
...
#define SERIALIZE_PTR(ptr) do { \
		if (ptr) { \
			CREX_ASSERT(IS_UNSERIALIZED(ptr)); \
			(ptr) = (void*)((char*)(ptr) - (char*)script->mem); \
		} \
	} while (0)
#define UNSERIALIZE_PTR(ptr) do { \
		if (ptr) { \
			CREX_ASSERT(IS_SERIALIZED(ptr)); \
...
#define SERIALIZE_STR(ptr) do { \
		if (ptr) { \
			if (IS_ACCEL_INTERNED(ptr)) { \
				(ptr) = crex_file_cache_serialize_interned((crex_string*)(ptr), info); \
			} else { \
				CREX_ASSERT(IS_UNSERIALIZED(ptr)); \
...
#define UNSERIALIZE_STR(ptr) do { \
		if (ptr) { \
			if (IS_SERIALIZED_INTERNED(ptr)) { \
				(ptr) = (void*)crex_file_cache_unserialize_interned((crex_string*)(ptr), !script->corrupted); \
			} else { \
				CREX_ASSERT(IS_SERIALIZED(ptr)); \
				(ptr) = (void*)((char*)buf + (size_t)(ptr)); \
				/* script->corrupted shows if the script in SHM or not */ \
...
#define SERIALIZE_ATTRIBUTES(attributes) do { \
	if ((attributes) && !IS_SERIALIZED(attributes)) { \
		...
		ht = (attributes); \
		UNSERIALIZE_PTR(ht); \
		crex_file_cache_serialize_hash(ht, script, info, buf, crex_file_cache_serialize_attribute); \
	} \
} while (0)
...
#define UNSERIALIZE_ATTRIBUTES(attributes) do { \
	if ((attributes) && !IS_UNSERIALIZED(attributes)) { \
		...
		ht = (attributes); \
		crex_file_cache_unserialize_hash(ht, script, buf, crex_file_cache_unserialize_attribute, NULL); \
	} \
} while (0)
...
typedef struct _crex_file_cache_metainfo {
	char         magic[8];
	char         system_id[32];
	size_t       mem_size;
	size_t       str_size;
	...
	uint32_t     checksum;
} crex_file_cache_metainfo;

static int crex_file_cache_mkdir(char *filename, size_t start)
{
...
#else
			if (crx_win32_ioutil_mkdir(filename, 0700) < 0 && errno != EEXIST) {
#endif
				*s = old;
				return FAILURE;
			}
...
static void crex_file_cache_serialize_zval(zval                     *zv,
                                           crex_persistent_script   *script,
                                           crex_file_cache_metainfo *info,
                                           void                     *buf);
static void crex_file_cache_unserialize_zval(zval                    *zv,
                                             crex_persistent_script  *script,
                                             void                    *buf);

static void *crex_file_cache_serialize_interned(crex_string              *str,
                                                crex_file_cache_metainfo *info)
{
	size_t len;
	void *ret;
	...
	ret = crex_shared_alloc_get_xlat_entry(str);
	if (ret) {
		return ret;
	}

	len = CREX_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(ZSTR_LEN(str)));
	ret = (void*)(info->str_size | C_UL(1));
	crex_shared_alloc_register_xlat_entry(str, ret);

	crex_string *s = (crex_string*)ZCG(mem);
	if (info->str_size + len > ZSTR_LEN(s)) {
		size_t new_len = info->str_size + len;
		s = crex_string_realloc(
			s,
	...
	crex_string *new_str = (crex_string *) (ZSTR_VAL(s) + info->str_size);
...
static void *crex_file_cache_unserialize_interned(crex_string *str, bool in_shm)
{
	str = (crex_string*)((char*)ZCG(mem) + ((size_t)(str) & ~C_UL(1)));
	if (!in_shm) {
		return str;
	}

	crex_string *ret = accel_new_interned_string(str);
	if (ret == str) {
		/* We have to create new SHM allocated string */
		size_t size = _ZSTR_STRUCT_SIZE(ZSTR_LEN(str));
		ret = crex_shared_alloc(size);
		if (!ret) {
...
static void crex_file_cache_serialize_hash(HashTable                *ht,
                                           crex_persistent_script   *script,
                                           crex_file_cache_metainfo *info,
                                           void                     *buf,
...
static void crex_file_cache_serialize_ast(crex_ast                 *ast,
                                          crex_persistent_script   *script,
                                          crex_file_cache_metainfo *info,
                                          void                     *buf)
{
	...
	if (ast->kind == CREX_AST_ZVAL || ast->kind == CREX_AST_CONSTANT) {
		crex_file_cache_serialize_zval(&((crex_ast_zval*)ast)->val, script, info, buf);
		...
		for (i = 0; i < list->children; i++) {
			if (list->child[i] && !IS_SERIALIZED(list->child[i])) {
				...
				tmp = list->child[i];
				UNSERIALIZE_PTR(tmp);
				crex_file_cache_serialize_ast(tmp, script, info, buf);
			}
		}
	} else {
                                           ...
                                           void                     *buf)
{
	switch (C_TYPE_P(zv)) {
		case IS_STRING:
			if (!IS_SERIALIZED(C_STR_P(zv))) {
				SERIALIZE_STR(C_STR_P(zv));
			}
			break;
		case IS_ARRAY:
			if (!IS_SERIALIZED(C_ARR_P(zv))) {
				HashTable *ht;

				SERIALIZE_PTR(C_ARR_P(zv));
				ht = C_ARR_P(zv);
				UNSERIALIZE_PTR(ht);
				crex_file_cache_serialize_hash(ht, script, info, buf, crex_file_cache_serialize_zval);
			}
			break;
		case IS_CONSTANT_AST:
			if (!IS_SERIALIZED(C_AST_P(zv))) {
				crex_ast_ref *ast;

				SERIALIZE_PTR(C_AST_P(zv));
				ast = C_AST_P(zv);
				UNSERIALIZE_PTR(ast);
				crex_file_cache_serialize_ast(GC_AST(ast), script, info, buf);
			}
			break;
		case IS_INDIRECT:
			/* Used by static properties. */
			SERIALIZE_PTR(C_INDIRECT_P(zv));
			break;
		default:
...
static void crex_file_cache_serialize_attribute(zval                     *zv,
                                                crex_persistent_script   *script,
                                                crex_file_cache_metainfo *info,
                                                void                     *buf)
{
	crex_attribute *attr = C_PTR_P(zv);
	uint32_t i;

	SERIALIZE_PTR(C_PTR_P(zv));
	attr = C_PTR_P(zv);
	...
	for (i = 0; i < attr->argc; i++) {
		...
		crex_file_cache_serialize_zval(&attr->args[i].value, script, info, buf);
	}
}

static void crex_file_cache_serialize_type(
		crex_type *type, crex_persistent_script *script, crex_file_cache_metainfo *info, void *buf)
{
	if (CREX_TYPE_HAS_LIST(*type)) {
		...
		crex_type *list_type;
		CREX_TYPE_LIST_FOREACH(list, list_type) {
			crex_file_cache_serialize_type(list_type, script, info, buf);
		} CREX_TYPE_LIST_FOREACH_END();
	} else if (CREX_TYPE_HAS_NAME(*type)) {
		crex_string *type_name = CREX_TYPE_NAME(*type);
		SERIALIZE_STR(type_name);
		CREX_TYPE_SET_PTR(*type, type_name);
	}
}

static void crex_file_cache_serialize_op_array(crex_op_array            *op_array,
                                               crex_persistent_script   *script,
                                               crex_file_cache_metainfo *info,
                                               void                     *buf)
{
	...
	if (IS_SERIALIZED(op_array->opcodes)) {
		CREX_ASSERT(op_array->scope && "Only method op_arrays should be shared");
		return;
	}

	if (op_array->scope) {
		if (UNEXPECTED(crex_shared_alloc_get_xlat_entry(op_array->opcodes))) {
	...
	if (op_array->literals) {
		...
		while (p < end) {
			crex_file_cache_serialize_zval(p, script, info, buf);
			p++;
		}
	}

	{
...
#endif
			crex_serialize_opcode_handler(opline);
			opline++;
		}
		...
		if (op_array->arg_info) {
			...
			while (p < end) {
				...
				crex_file_cache_serialize_type(&p->type, script, info, buf);
				p++;
			}
		}
		...
		if (op_array->num_dynamic_func_defs) {
			...
			for (uint32_t i = 0; i < op_array->num_dynamic_func_defs; i++) {
				...
				def = defs[i];
				UNSERIALIZE_PTR(def);
				crex_file_cache_serialize_op_array(def, script, info, buf);
			}
		}
...
static void crex_file_cache_serialize_func(zval                     *zv,
                                           crex_persistent_script   *script,
                                           crex_file_cache_metainfo *info,
                                           void                     *buf)
{
	...
	CREX_ASSERT(func->type == CREX_USER_FUNCTION);
	crex_file_cache_serialize_op_array(&func->op_array, script, info, buf);
}

static void crex_file_cache_serialize_prop_info(zval                     *zv,
                                                crex_persistent_script   *script,
                                                crex_file_cache_metainfo *info,
                                                void                     *buf)
{
	if (!IS_SERIALIZED(C_PTR_P(zv))) {
		...
		CREX_ASSERT(prop->ce != NULL && prop->name != NULL);
		if (!IS_SERIALIZED(prop->ce)) {
			...
			crex_file_cache_serialize_type(&prop->type, script, info, buf);
		}
	}
}

static void crex_file_cache_serialize_class_constant(zval                     *zv,
                                                     crex_persistent_script   *script,
                                                     crex_file_cache_metainfo *info,
                                                     void                     *buf)
{
	if (!IS_SERIALIZED(C_PTR_P(zv))) {
		...
		CREX_ASSERT(c->ce != NULL);
		if (!IS_SERIALIZED(c->ce)) {
			SERIALIZE_PTR(c->ce);

			crex_file_cache_serialize_zval(&c->value, script, info, buf);
			if (c->doc_comment) {
				SERIALIZE_STR(c->doc_comment);
			}

			SERIALIZE_ATTRIBUTES(c->attributes);
			crex_file_cache_serialize_type(&c->type, script, info, buf);
		}
	}
}

static void crex_file_cache_serialize_class(zval                     *zv,
                                            crex_persistent_script   *script,
                                            crex_file_cache_metainfo *info,
                                            void                     *buf)
{
	...
	crex_file_cache_serialize_hash(&ce->function_table, script, info, buf, crex_file_cache_serialize_func);
	...
	crex_file_cache_serialize_hash(&ce->constants_table, script, info, buf, crex_file_cache_serialize_class_constant);
	SERIALIZE_STR(ce->info.user.filename);
	SERIALIZE_STR(ce->info.user.doc_comment);
	SERIALIZE_ATTRIBUTES(ce->attributes);
	crex_file_cache_serialize_hash(&ce->properties_info, script, info, buf, crex_file_cache_serialize_prop_info);

	if (ce->properties_info_table) {
		uint32_t i;
		crex_property_info **table;
...
static void crex_file_cache_serialize_warnings(
		crex_persistent_script *script, crex_file_cache_metainfo *info, void *buf)
{
	if (script->warnings) {
...
static void crex_file_cache_serialize_early_bindings(
		crex_persistent_script *script, crex_file_cache_metainfo *info, void *buf)
{
	if (script->early_bindings) {
		SERIALIZE_PTR(script->early_bindings);
		crex_early_binding *early_bindings = script->early_bindings;
...
static void crex_file_cache_serialize(crex_persistent_script   *script,
                                      crex_file_cache_metainfo *info,
                                      void                     *buf)
{
	crex_persistent_script *new_script;

	memcpy(info->magic, "OPCACHE", 8);
	memcpy(info->system_id, crex_system_id, 32);
	...
	new_script = (crex_persistent_script*)((char*)buf + info->script_offset);
	SERIALIZE_STR(new_script->script.filename);

	crex_file_cache_serialize_hash(&new_script->script.class_table, script, info, buf, crex_file_cache_serialize_class);
	crex_file_cache_serialize_hash(&new_script->script.function_table, script, info, buf, crex_file_cache_serialize_func);
	crex_file_cache_serialize_op_array(&new_script->script.main_op_array, script, info, buf);
	crex_file_cache_serialize_warnings(new_script, info, buf);
	crex_file_cache_serialize_early_bindings(new_script, info, buf);

	new_script->mem = NULL;
}

static char *crex_file_cache_get_bin_file_path(crex_string *script_path)
{
	size_t len;
	char *filename;
	...
	filename[len] = '/';
	memcpy(filename + len + 1, crex_system_id, 32);
...
/**
 * Helper function for crex_file_cache_script_store().
 *
 * @return true on success, false on error and errno is set to indicate the cause of the error
 */
static bool crex_file_cache_script_write(int fd, const crex_persistent_script *script, const crex_file_cache_metainfo *info, const void *buf, const crex_string *s)
{
	ssize_t written;
	...
	return false;
#else
	if (UNEXPECTED(CREX_LONG_MAX < (crex_long)total_size)) {
# ifdef EFBIG
		errno = EFBIG;
# else
		errno = ERANGE;
...
int crex_file_cache_script_store(crex_persistent_script *script, bool in_shm)
{
	...
	filename = crex_file_cache_get_bin_file_path(script->script.filename);

	if (crex_file_cache_mkdir(filename, strlen(ZCG(accel_directives).file_cache)) != SUCCESS) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot create directory for file '%s', %s\n", filename, strerror(errno));
		efree(filename);
		return FAILURE;
	}

	fd = crex_file_cache_open(filename, O_CREAT | O_EXCL | O_RDWR | O_BINARY, S_IRUSR | S_IWUSR);
	if (fd < 0) {
		if (errno != EEXIST) {
			crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot create file '%s', %s\n", filename, strerror(errno));
		}
		efree(filename);
		return FAILURE;
	}

	if (crex_file_cache_flock(fd, LOCK_EX) != 0) {
		close(fd);
		efree(filename);
		return FAILURE;
	}
	...
	info.checksum = crex_adler32(ADLER32_INIT, buf, script->size);
	info.checksum = crex_adler32(info.checksum, (unsigned char*)ZSTR_VAL(s), info.str_size);

	if (!crex_file_cache_script_write(fd, script, &info, buf, s)) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot write to file '%s': %s\n", filename, strerror(errno));
		crex_string_release_ex(s, 0);
		close(fd);
		efree(mem);
		crex_file_cache_unlink(filename);
		efree(filename);
		return FAILURE;
	}
	...
	crex_string_release_ex(s, 0);
	efree(mem);
	if (crex_file_cache_flock(fd, LOCK_UN) != 0) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot unlock file '%s': %s\n", filename, strerror(errno));
	}
	close(fd);
	efree(filename);
...
static void crex_file_cache_unserialize_hash(HashTable               *ht,
                                             crex_persistent_script  *script,
                                             void                    *buf,
...
static void crex_file_cache_unserialize_ast(crex_ast                *ast,
                                            crex_persistent_script  *script,
                                            void                    *buf)
{
	uint32_t i;
	...
	if (ast->kind == CREX_AST_ZVAL || ast->kind == CREX_AST_CONSTANT) {
		crex_file_cache_unserialize_zval(&((crex_ast_zval*)ast)->val, script, buf);
	} else if (crex_ast_is_list(ast)) {
		crex_ast_list *list = crex_ast_get_list(ast);
		for (i = 0; i < list->children; i++) {
			if (list->child[i] && !IS_UNSERIALIZED(list->child[i])) {
				UNSERIALIZE_PTR(list->child[i]);
				crex_file_cache_unserialize_ast(list->child[i], script, buf);
			}
		}
	} else {
		uint32_t children = crex_ast_get_num_children(ast);
		for (i = 0; i < children; i++) {
			if (ast->child[i] && !IS_UNSERIALIZED(ast->child[i])) {
				...
				crex_file_cache_unserialize_ast(ast->child[i], script, buf);
			}
		}
	}
}
                                             ...
                                             void                    *buf)
{
	switch (C_TYPE_P(zv)) {
		case IS_STRING:
			...
			if (IS_SERIALIZED(C_STR_P(zv)) || IS_SERIALIZED_INTERNED(C_STR_P(zv))) {
				UNSERIALIZE_STR(C_STR_P(zv));
			}
			break;
		case IS_ARRAY:
			if (!IS_UNSERIALIZED(C_ARR_P(zv))) {
				HashTable *ht;

				UNSERIALIZE_PTR(C_ARR_P(zv));
				ht = C_ARR_P(zv);
				crex_file_cache_unserialize_hash(ht,
						script, buf, crex_file_cache_unserialize_zval, ZVAL_PTR_DTOR);
			}
			break;
		case IS_CONSTANT_AST:
			if (!IS_UNSERIALIZED(C_AST_P(zv))) {
				UNSERIALIZE_PTR(C_AST_P(zv));
				crex_file_cache_unserialize_ast(C_ASTVAL_P(zv), script, buf);
			}
			break;
		case IS_INDIRECT:
			/* Used by static properties. */
			UNSERIALIZE_PTR(C_INDIRECT_P(zv));
			break;
		default:
			CREX_ASSERT(C_TYPE_P(zv) < IS_STRING);
			break;
...
static void crex_file_cache_unserialize_attribute(zval *zv, crex_persistent_script *script, void *buf)
{
	crex_attribute *attr;
	uint32_t i;

	UNSERIALIZE_PTR(C_PTR_P(zv));
	attr = C_PTR_P(zv);
	...
	for (i = 0; i < attr->argc; i++) {
		...
		crex_file_cache_unserialize_zval(&attr->args[i].value, script, buf);
	}
}

static void crex_file_cache_unserialize_type(
		crex_type *type, crex_class_entry *scope, crex_persistent_script *script, void *buf)
{
	if (CREX_TYPE_HAS_LIST(*type)) {
		crex_type_list *list = CREX_TYPE_LIST(*type);
		UNSERIALIZE_PTR(list);
		...
		crex_type *list_type;
		CREX_TYPE_LIST_FOREACH(list, list_type) {
			crex_file_cache_unserialize_type(list_type, scope, script, buf);
		} CREX_TYPE_LIST_FOREACH_END();
	} else if (CREX_TYPE_HAS_NAME(*type)) {
		crex_string *type_name = CREX_TYPE_NAME(*type);
		...
		if (!script->corrupted) {
			...
			crex_alloc_ce_cache(type_name);
		}
	}
}

static void crex_file_cache_unserialize_op_array(crex_op_array           *op_array,
                                                 crex_persistent_script  *script,
                                                 void                    *buf)
{
	if (!script->corrupted) {
		if (op_array != &script->script.main_op_array) {
			...
			CREX_ASSERT(!(op_array->fn_flags & CREX_ACC_IMMUTABLE));
			CREX_MAP_PTR_INIT(op_array->run_time_cache, NULL);
		}
		if (op_array->static_variables) {
	...
	if (op_array->literals) {
		...
		while (p < end) {
			crex_file_cache_unserialize_zval(p, script, buf);
			p++;
		}
	}

	{
...
#endif
			crex_deserialize_opcode_handler(opline);
			opline++;
		}
		...
		if (op_array->arg_info) {
			...
			while (p < end) {
				...
				crex_file_cache_unserialize_type(&p->type, (op_array->fn_flags & CREX_ACC_CLOSURE) ? NULL : op_array->scope, script, buf);
				p++;
			}
		}
		...
		if (op_array->num_dynamic_func_defs) {
			...
			for (uint32_t i = 0; i < op_array->num_dynamic_func_defs; i++) {
				...
				crex_file_cache_unserialize_op_array(op_array->dynamic_func_defs[i], script, buf);
			}
		}
...
static void crex_file_cache_unserialize_func(zval                    *zv,
                                             crex_persistent_script  *script,
                                             void                    *buf)
{
	crex_function *func;
	...
	func = C_PTR_P(zv);
	CREX_ASSERT(func->type == CREX_USER_FUNCTION);
	crex_file_cache_unserialize_op_array(&func->op_array, script, buf);
}

static void crex_file_cache_unserialize_prop_info(zval                    *zv,
                                                  crex_persistent_script  *script,
                                                  void                    *buf)
{
	if (!IS_UNSERIALIZED(C_PTR_P(zv))) {
		crex_property_info *prop;
		...
		if (!IS_UNSERIALIZED(prop->ce)) {
			...
			crex_file_cache_unserialize_type(&prop->type, prop->ce, script, buf);
		}
	}
}

static void crex_file_cache_unserialize_class_constant(zval                    *zv,
                                                       crex_persistent_script  *script,
                                                       void                    *buf)
{
	if (!IS_UNSERIALIZED(C_PTR_P(zv))) {
		...
		if (!IS_UNSERIALIZED(c->ce)) {
			...
			crex_file_cache_unserialize_zval(&c->value, script, buf);

			if (c->doc_comment) {
				UNSERIALIZE_STR(c->doc_comment);
			}
			UNSERIALIZE_ATTRIBUTES(c->attributes);
			crex_file_cache_unserialize_type(&c->type, c->ce, script, buf);
		}
	}
}

static void crex_file_cache_unserialize_class(zval                    *zv,
                                              crex_persistent_script  *script,
                                              void                    *buf)
{
	crex_class_entry *ce;
	...
	crex_file_cache_unserialize_hash(&ce->function_table,
			script, buf, crex_file_cache_unserialize_func, CREX_FUNCTION_DTOR);
	if (ce->default_properties_table) {
		zval *p, *end;
	...
	crex_file_cache_unserialize_hash(&ce->constants_table,
			script, buf, crex_file_cache_unserialize_class_constant, NULL);
	UNSERIALIZE_STR(ce->info.user.filename);
	UNSERIALIZE_STR(ce->info.user.doc_comment);
	UNSERIALIZE_ATTRIBUTES(ce->attributes);
	crex_file_cache_unserialize_hash(&ce->properties_info,
			script, buf, crex_file_cache_unserialize_prop_info, NULL);
	...
	ce->default_object_handlers = ce->ce_flags & CREX_ACC_ENUM ? &crex_enum_object_handlers : &std_object_handlers;
}

static void crex_file_cache_unserialize_warnings(crex_persistent_script *script, void *buf)
{
	if (script->warnings) {
...
static void crex_file_cache_unserialize_early_bindings(crex_persistent_script *script, void *buf)
{
	if (script->early_bindings) {
...
static void crex_file_cache_unserialize(crex_persistent_script  *script,
                                        void                    *buf)
{
	script->mem = buf;
	...
	crex_file_cache_unserialize_hash(&script->script.class_table,
			script, buf, crex_file_cache_unserialize_class, CREX_CLASS_DTOR);
	crex_file_cache_unserialize_hash(&script->script.function_table,
			script, buf, crex_file_cache_unserialize_func, CREX_FUNCTION_DTOR);
	crex_file_cache_unserialize_op_array(&script->script.main_op_array, script, buf);
	crex_file_cache_unserialize_warnings(script, buf);
	crex_file_cache_unserialize_early_bindings(script, buf);
}

crex_persistent_script *crex_file_cache_script_load(crex_file_handle *file_handle)
{
	crex_string *full_path = file_handle->opened_path;
	int fd;
	char *filename;
	crex_persistent_script *script;
	crex_file_cache_metainfo info;
	...
	filename = crex_file_cache_get_bin_file_path(full_path);

	fd = crex_file_cache_open(filename, O_RDONLY | O_BINARY);
	if (fd < 0) {
		efree(filename);
		return NULL;
	}

	if (crex_file_cache_flock(fd, LOCK_SH) != 0) {
		close(fd);
		efree(filename);
		return NULL;
	}
	...
	if (read(fd, &info, sizeof(info)) != sizeof(info)) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot read from file '%s' (info)\n", filename);
		crex_file_cache_flock(fd, LOCK_UN);
		close(fd);
		crex_file_cache_unlink(filename);
		efree(filename);
		return NULL;
	...
	if (memcmp(info.magic, "OPCACHE", 8) != 0) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot read from file '%s' (wrong header)\n", filename);
		crex_file_cache_flock(fd, LOCK_UN);
		close(fd);
		crex_file_cache_unlink(filename);
		efree(filename);
	...
	if (memcmp(info.system_id, crex_system_id, 32) != 0) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot read from file '%s' (wrong \"system_id\")\n", filename);
		crex_file_cache_flock(fd, LOCK_UN);
		close(fd);
		crex_file_cache_unlink(filename);
		efree(filename);
	...
	if (ZCG(accel_directives).validate_timestamps &&
	    crex_get_file_handle_timestamp(file_handle, NULL) != info.timestamp) {
		if (crex_file_cache_flock(fd, LOCK_UN) != 0) {
			crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot unlock file '%s'\n", filename);
		}
		close(fd);
		crex_file_cache_unlink(filename);
		efree(filename);
...
#if defined(__AVX__) || defined(__SSE2__)
	/* Align to 64-byte boundary */
	mem = crex_arena_alloc(&CG(arena), info.mem_size + info.str_size + 64);
	mem = (void*)(((uintptr_t)mem + 63L) & ~63L);
#else
	mem = crex_arena_alloc(&CG(arena), info.mem_size + info.str_size);
#endif

	if (read(fd, mem, info.mem_size + info.str_size) != (ssize_t)(info.mem_size + info.str_size)) {
		crex_accel_error(ACCEL_LOG_WARNING, "opcache cannot read from file '%s' (mem)\n", filename);
		crex_file_cache_flock(fd, LOCK_UN);
		close(fd);
		crex_file_cache_unlink(filename);
	...
	if (ZCG(accel_directives).file_cache_consistency_checks &&
	    (actual_checksum = crex_adler32(ADLER32_INIT, mem, info.mem_size + info.str_size)) != info.checksum) {
		crex_accel_error(ACCEL_LOG_WARNING, "corrupted file '%s' excepted checksum: 0x%08x actual checksum: 0x%08x\n", filename, info.checksum, actual_checksum);
		crex_file_cache_unlink(filename);
		crex_arena_release(&CG(arena), checkpoint);
		efree(filename);
		return NULL;
	...
	if (!file_cache_only &&
		 ...
		 * exclusive lock) */
		bucket = crex_accel_hash_find_entry(&ZCSG(hash), full_path);
		if (bucket) {
			script = (crex_persistent_script *)bucket->data;
			if (!script->corrupted) {
		...
		buf = crex_shared_alloc_aligned(info.mem_size);
...
use_process_mem:
	...
	script = (crex_persistent_script*)((char*)buf + info.script_offset);
	script->corrupted = !cache_it; /* used to check if script restored to SHM or process memory */

	ok = true;
	crex_try {
		crex_file_cache_unserialize(script, buf);
	} crex_catch {
		ok = false;
	} crex_end_try();
	if (!ok) {
	...
	if (cache_it) {
		...
		crex_accel_hash_update(&ZCSG(hash), script->script.filename, 0, script);

		crex_shared_alloc_unlock();
		crex_accel_error(ACCEL_LOG_INFO, "File cached script loaded into memory '%s'", ZSTR_VAL(script->script.filename));
