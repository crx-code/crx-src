...
typedef struct  {
    ...
    int shm_id;
} crex_shared_segment_shm;

static int create_segments(size_t requested_size, crex_shared_segment_shm ***shared_segments_p, int *shared_segments_count, const char **error_in)
{
	int i;
	...
	struct shmid_ds sds;
	int shmget_flags;
	crex_shared_segment_shm *shared_segments;
	...
	*shared_segments_p = (crex_shared_segment_shm **) calloc(1, (*shared_segments_count) * sizeof(crex_shared_segment_shm) + sizeof(void *) * (*shared_segments_count));
	if (!*shared_segments_p) {
		*error_in = "calloc";
		return ALLOC_FAILURE;
	}
	shared_segments = (crex_shared_segment_shm *)((char *)(*shared_segments_p) + sizeof(void *) * (*shared_segments_count));
...
static int detach_segment(crex_shared_segment_shm *shared_segment)
{
...
{
	return sizeof(crex_shared_segment_shm);
}

const crex_shared_memory_handlers crex_alloc_shm_handlers = {
