...
#define ACCEL_FILEMAP_NAME "CrexOPcache.SharedMemoryArea"
#define ACCEL_MUTEX_NAME "CrexOPcache.SharedMemoryMutex"
#define ACCEL_EVENT_SOURCE "Crex OPcache"
...
static void crex_win_error_message(int type, char *msg, int err)
{
	HANDLE h;
	char *ev_msgs[2];
...
static char *create_name_with_username(char *name)
{
	...
	*(p++) = '@';
	memcpy(p, crex_system_id, 32);
	p += 32;
	*(p++) = '\0';
	CREX_ASSERT(p - newname <= sizeof(newname));
...
void crex_shared_alloc_create_lock(void)
{
	...
	if (!memory_mutex) {
		crex_accel_error(ACCEL_LOG_FATAL, "Cannot create mutex (error %u)", GetLastError());
		return;
	}
...
void crex_shared_alloc_lock_win32(void)
{
	...
	if (waitRes == WAIT_FAILED) {
		crex_accel_error(ACCEL_LOG_ERROR, "Cannot lock mutex");
	}
}
...
static int crex_shared_alloc_reattach(size_t requested_size, const char **error_in)
{
	int err;
	...
	if (mapping_base == NULL) {
		err = GetLastError();
		crex_win_error_message(ACCEL_LOG_FATAL, "Unable to read base address", err);
	...
	if (execute_ex_moved ||
		...
		if (ZCG(accel_directives).file_cache && ZCG(accel_directives).file_cache_fallback) {
			...
			if (execute_ex_moved) {
				...
				crex_win_error_message(ACCEL_LOG_WARNING, "Opcode handlers are unusable due to ASLR (fall-back to file cache)", err);
			} else {
				err = ERROR_INVALID_ADDRESS;
				crex_win_error_message(ACCEL_LOG_WARNING, "Base address marks unusable memory region (fall-back to file cache)", err);
			}

			pre_size = CREX_ALIGNED_SIZE(sizeof(crex_smm_shared_globals)) + CREX_ALIGNED_SIZE(sizeof(crex_shared_segment)) + CREX_ALIGNED_SIZE(sizeof(void *)) + CREX_ALIGNED_SIZE(sizeof(int));
			/* Map only part of SHM to have access to opcache shared globals */
			mapping_base = MapViewOfFileEx(memfile, FILE_MAP_ALL_ACCESS, 0, 0, pre_size + CREX_ALIGNED_SIZE(sizeof(crex_accel_shared_globals)), NULL);
			if (mapping_base == NULL) {
				err = GetLastError();
				crex_win_error_message(ACCEL_LOG_FATAL, "Unable to reattach to opcache shared globals", err);
				return ALLOC_FAILURE;
			}
			accel_shared_globals = (crex_accel_shared_globals *)((char *)((crex_smm_shared_globals *)mapping_base)->app_shared_globals + ((char *)mapping_base - (char *)wanted_mb_save));
		...
		if (execute_ex_moved) {
			err = ERROR_INVALID_ADDRESS;
			crex_win_error_message(ACCEL_LOG_FATAL, "Opcode handlers are unusable due to ASLR. Please setup opcache.file_cache and opcache.file_cache_fallback directives for more convenient Opcache usage", err);
		} else {
			err = ERROR_INVALID_ADDRESS;
			crex_win_error_message(ACCEL_LOG_FATAL, "Base address marks unusable memory region. Please setup opcache.file_cache and opcache.file_cache_fallback directives for more convenient Opcache usage", err);
		}
		return ALLOC_FAILURE;
	}
	...
	if (mapping_base == NULL) {
		...
		if (err == ERROR_INVALID_ADDRESS) {
			crex_win_error_message(ACCEL_LOG_FATAL, "Unable to reattach to base address", err);
			return ALLOC_FAILURE;
		}
		...
		if (!VirtualProtect(mapping_base, requested_size, PAGE_READWRITE, &old)) {
			err = GetLastError();
			crex_win_error_message(ACCEL_LOG_FATAL, "VirtualProtect() failed", err);
			return ALLOC_FAIL_MAPPING;
		}
	}

	smm_shared_globals = (crex_smm_shared_globals *) ((char*)mapping_base + ACCEL_BASE_POINTER_SIZE);

	return SUCCESSFULLY_REATTACHED;
}

static int create_segments(size_t requested_size, crex_shared_segment ***shared_segments_p, int *shared_segments_count, const char **error_in)
{
	int err = 0, ret;
	crex_shared_segment *shared_segment;
	int map_retries = 0;
	...
	/* TODO:
	  ...
	  desired. Not done yet, @crex refused but did not remember the exact reason, pls add info here if one of you know why :)
	*/
#if defined(_WIN64)
	...
	do {
		...
		ret =  crex_shared_alloc_reattach(requested_size, error_in);
	...
	if (map_retries == MAX_MAP_RETRIES) {
		crex_shared_alloc_unlock_win32();
		crex_win_error_message(ACCEL_LOG_FATAL, "Unable to open file mapping", err);
	...
	*shared_segments_p = (crex_shared_segment **) calloc(1, sizeof(crex_shared_segment)+sizeof(void *));
	if (!*shared_segments_p) {
		err = GetLastError();
		crex_shared_alloc_unlock_win32();
		crex_win_error_message(ACCEL_LOG_FATAL, "calloc() failed", err);
		*error_in = "calloc";
		return ALLOC_FAILURE;
	}
	shared_segment = (crex_shared_segment *)((char *)(*shared_segments_p) + sizeof(void *));
	...
	if (memfile == NULL) {
		err = GetLastError();
		crex_shared_alloc_unlock_win32();
		crex_win_error_message(ACCEL_LOG_FATAL, "Unable to create file mapping", err);
	...
	if (!ZCG(accel_directives).mmap_base || !*ZCG(accel_directives).mmap_base) {
		...
		if (sscanf(s, "%p", &default_mapping_base_set[0]) != 1) {
			crex_shared_alloc_unlock_win32();
			crex_win_error_message(ACCEL_LOG_FATAL, "Bad mapping address specified in opcache.mmap_base", err);
			return ALLOC_FAILURE;
		}
	}
	...
	if (mapping_base == NULL) {
		err = GetLastError();
		crex_shared_alloc_unlock_win32();
		crex_win_error_message(ACCEL_LOG_FATAL, "Unable to create view for file mapping", err);
...
static int detach_segment(crex_shared_segment *shared_segment)
{
	crex_shared_alloc_lock_win32();
	if (mapping_base) {
		UnmapViewOfFile(mapping_base);
...
{
	return sizeof(crex_shared_segment);
}

const crex_shared_memory_handlers crex_alloc_win32_handlers = {
	create_segments,
	detach_segment,
	segment_type_size
};
