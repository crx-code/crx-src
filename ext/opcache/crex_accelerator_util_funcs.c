...
{
	crex_persistent_script *persistent_script = (crex_persistent_script *) emalloc(sizeof(crex_persistent_script));
	memset(persistent_script, 0, sizeof(crex_persistent_script));

	crex_hash_init(&persistent_script->script.function_table, 0, NULL, CREX_FUNCTION_DTOR, 0);
	/* class_table is usually destroyed by free_persistent_script() that
	 * overrides destructor. CREX_CLASS_DTOR may be used by standard
	 * CRX compiler
	 */
	crex_hash_init(&persistent_script->script.class_table, 0, NULL, CREX_CLASS_DTOR, 0);
...
void free_persistent_script(crex_persistent_script *persistent_script, int destroy_elements)
{
	...
	crex_hash_destroy(&persistent_script->script.function_table);
	crex_hash_destroy(&persistent_script->script.class_table);

	if (persistent_script->script.filename) {
		crex_string_release_ex(persistent_script->script.filename, 0);
	}

	if (persistent_script->warnings) {
		for (uint32_t i = 0; i < persistent_script->num_warnings; i++) {
			crex_error_info *info = persistent_script->warnings[i];
	...
	crex_accel_free_delayed_early_binding_list(persistent_script);

	efree(persistent_script);
}

void crex_accel_move_user_functions(HashTable *src, uint32_t count, crex_script *script)
{
	Bucket *p, *end;
	HashTable *dst;
...
void crex_accel_move_user_classes(HashTable *src, uint32_t count, crex_script *script)
{
	Bucket *p, *end;
	HashTable *dst;
	crex_string *filename;
	...
	for (; p != end; p++) {
		if (UNEXPECTED(C_TYPE(p->val) == IS_UNDEF)) continue;
		ce = C_PTR(p->val);
		if (EXPECTED(ce->type == CREX_USER_CLASS)
		 && EXPECTED(ce->info.user.filename == filename)) {
			_crex_hash_append_ptr(dst, p->key, ce);
			crex_hash_del_bucket(src, p);
		}
	}
	src->pDestructor = orig_dtor;
...
static crex_always_inline void _crex_accel_function_hash_copy(HashTable *target, HashTable *source, bool call_observers)
{
	crex_function *function1, *function2;
	Bucket *p, *end;
	zval *t;

	crex_hash_extend(target, target->nNumUsed + source->nNumUsed, 0);
	p = source->arData;
	end = p + source->nNumUsed;
	for (; p != end; p++) {
		CREX_ASSERT(C_TYPE(p->val) != IS_UNDEF);
		CREX_ASSERT(p->key);
		t = crex_hash_find_known_hash(target, p->key);
		if (UNEXPECTED(t != NULL)) {
			goto failure;
		}
		_crex_hash_append_ptr_ex(target, p->key, C_PTR(p->val), 1);
		if (UNEXPECTED(call_observers) && *ZSTR_VAL(p->key)) { // if not rtd key
			_crex_observer_function_declared_notify(C_PTR(p->val), p->key);
		}
	}
...
failure:
	function1 = C_PTR(p->val);
	function2 = C_PTR_P(t);
	CG(in_compilation) = 1;
	crex_set_compiled_filename(function1->op_array.filename);
	CG(crex_lineno) = function1->op_array.opcodes[0].lineno;
	if (function2->type == CREX_USER_FUNCTION
		&& function2->op_array.last > 0) {
		crex_error(E_ERROR, "Cannot redeclare %s() (previously declared in %s:%d)",
				   ZSTR_VAL(function1->common.function_name),
				   ZSTR_VAL(function2->op_array.filename),
				   (int)function2->op_array.opcodes[0].lineno);
	} else {
		crex_error(E_ERROR, "Cannot redeclare %s()", ZSTR_VAL(function1->common.function_name));
	}
}

static crex_always_inline void crex_accel_function_hash_copy(HashTable *target, HashTable *source)
{
	_crex_accel_function_hash_copy(target, source, 0);
}

static crex_never_inline void crex_accel_function_hash_copy_notify(HashTable *target, HashTable *source)
{
	_crex_accel_function_hash_copy(target, source, 1);
}

static crex_always_inline void _crex_accel_class_hash_copy(HashTable *target, HashTable *source, bool call_observers)
{
	Bucket *p, *end;
	zval *t;
	...
	for (; p != end; p++) {
		...
		if (UNEXPECTED(t != NULL)) {
			if (EXPECTED(ZSTR_LEN(p->key) > 0) && EXPECTED(ZSTR_VAL(p->key)[0] == 0)) {
				...
				crex_class_entry *ce1 = C_PTR(p->val);
				if (!(ce1->ce_flags & CREX_ACC_ANON_CLASS)) {
					CG(in_compilation) = 1;
					crex_set_compiled_filename(ce1->info.user.filename);
					CG(crex_lineno) = ce1->info.user.line_start;
					crex_error(E_ERROR,
							"Cannot declare %s %s, because the name is already in use",
							crex_get_object_type(ce1), ZSTR_VAL(ce1->name));
					return;
				}
				continue;
			}
			...
			_crex_hash_append_ptr_ex(target, p->key, C_PTR(p->val), 1);
			if ((ce->ce_flags & CREX_ACC_LINKED) && ZSTR_VAL(p->key)[0]) {
				if (ZSTR_HAS_CE_CACHE(ce->name)) {
					ZSTR_SET_CE_CACHE_EX(ce->name, ce, 0);
				}
				if (UNEXPECTED(call_observers)) {
					_crex_observer_class_linked_notify(ce, p->key);
				}
			}
		}
	}
...
static crex_always_inline void crex_accel_class_hash_copy(HashTable *target, HashTable *source)
{
	_crex_accel_class_hash_copy(target, source, 0);
}

static crex_never_inline void crex_accel_class_hash_copy_notify(HashTable *target, HashTable *source)
{
	_crex_accel_class_hash_copy(target, source, 1);
}

void crex_accel_build_delayed_early_binding_list(crex_persistent_script *persistent_script)
{
	crex_op_array *op_array = &persistent_script->script.main_op_array;
	if (!(op_array->fn_flags & CREX_ACC_EARLY_BINDING)) {
		return;
	}

	crex_op *end = op_array->opcodes + op_array->last;
	for (crex_op *opline = op_array->opcodes; opline < end; opline++) {
		if (opline->opcode == CREX_DECLARE_CLASS_DELAYED) {
			persistent_script->num_early_bindings++;
		}
	}

	crex_early_binding *early_binding = persistent_script->early_bindings =
		emalloc(sizeof(crex_early_binding) * persistent_script->num_early_bindings);

	for (crex_op *opline = op_array->opcodes; opline < end; opline++) {
		if (opline->opcode == CREX_DECLARE_CLASS_DELAYED) {
			zval *lcname = RT_CONSTANT(opline, opline->op1);
			early_binding->lcname = crex_string_copy(C_STR_P(lcname));
			early_binding->rtd_key = crex_string_copy(C_STR_P(lcname + 1));
			early_binding->lc_parent_name =
				crex_string_copy(C_STR_P(RT_CONSTANT(opline, opline->op2)));
...
void crex_accel_finalize_delayed_early_binding_list(crex_persistent_script *persistent_script)
{
	...
	crex_early_binding *early_binding = persistent_script->early_bindings;
	crex_early_binding *early_binding_end = early_binding + persistent_script->num_early_bindings;
	crex_op_array *op_array = &persistent_script->script.main_op_array;
	crex_op *opline_end = op_array->opcodes + op_array->last;
	for (crex_op *opline = op_array->opcodes; opline < opline_end; opline++) {
		if (opline->opcode == CREX_DECLARE_CLASS_DELAYED) {
			crex_string *rtd_key = C_STR_P(RT_CONSTANT(opline, opline->op1) + 1);
			/* Skip early_binding entries that don't match, maybe their DECLARE_CLASS_DELAYED
			 * was optimized away. */
			while (!crex_string_equals(early_binding->rtd_key, rtd_key)) {
				early_binding++;
...
void crex_accel_free_delayed_early_binding_list(crex_persistent_script *persistent_script)
{
	if (persistent_script->num_early_bindings) {
		for (uint32_t i = 0; i < persistent_script->num_early_bindings; i++) {
			crex_early_binding *early_binding = &persistent_script->early_bindings[i];
			crex_string_release(early_binding->lcname);
			crex_string_release(early_binding->rtd_key);
			crex_string_release(early_binding->lc_parent_name);
		}
...
static void crex_accel_do_delayed_early_binding(
		crex_persistent_script *persistent_script, crex_op_array *op_array)
{
	CREX_ASSERT(!CREX_MAP_PTR(op_array->run_time_cache));
	CREX_ASSERT(op_array->fn_flags & CREX_ACC_HEAP_RT_CACHE);
	void *run_time_cache = emalloc(op_array->cache_size);

	CREX_MAP_PTR_INIT(op_array->run_time_cache, run_time_cache);
	memset(run_time_cache, 0, op_array->cache_size);

	crex_string *orig_compiled_filename = CG(compiled_filename);
	bool orig_in_compilation = CG(in_compilation);
	CG(compiled_filename) = persistent_script->script.filename;
	CG(in_compilation) = 1;
	for (uint32_t i = 0; i < persistent_script->num_early_bindings; i++) {
		crex_early_binding *early_binding = &persistent_script->early_bindings[i];
		crex_class_entry *ce = crex_hash_find_ex_ptr(EG(class_table), early_binding->lcname, 1);
		if (!ce) {
			zval *zv = crex_hash_find_known_hash(EG(class_table), early_binding->rtd_key);
			if (zv) {
				crex_class_entry *orig_ce = C_CE_P(zv);
				crex_class_entry *parent_ce = !(orig_ce->ce_flags & CREX_ACC_LINKED)
					? crex_hash_find_ex_ptr(EG(class_table), early_binding->lc_parent_name, 1)
					: NULL;
				if (parent_ce || (orig_ce->ce_flags & CREX_ACC_LINKED)) {
					ce = crex_try_early_bind(orig_ce, parent_ce, early_binding->lcname, zv);
				}
			}
...
crex_op_array* crex_accel_load_script(crex_persistent_script *persistent_script, int from_shared_memory)
{
	...
	op_array = (crex_op_array *) emalloc(sizeof(crex_op_array));
	*op_array = persistent_script->script.main_op_array;
	...
	if (EXPECTED(from_shared_memory)) {
		...
		if (persistent_script->compiler_halt_offset != 0 &&
			...
			name = crex_mangle_property_name(haltoff, sizeof(haltoff) - 1, ZSTR_VAL(persistent_script->script.filename), ZSTR_LEN(persistent_script->script.filename), 0);
			if (!crex_hash_exists(EG(crex_constants), name)) {
				crex_register_long_constant(ZSTR_VAL(name), ZSTR_LEN(name), persistent_script->compiler_halt_offset, 0, 0);
			}
			crex_string_release_ex(name, 0);
		}
	}

	if (crex_hash_num_elements(&persistent_script->script.function_table) > 0) {
		if (EXPECTED(!crex_observer_function_declared_observed)) {
			crex_accel_function_hash_copy(CG(function_table), &persistent_script->script.function_table);
		} else {
			crex_accel_function_hash_copy_notify(CG(function_table), &persistent_script->script.function_table);
		}
	}

	if (crex_hash_num_elements(&persistent_script->script.class_table) > 0) {
		if (EXPECTED(!crex_observer_class_linked_observed)) {
			crex_accel_class_hash_copy(CG(class_table), &persistent_script->script.class_table);
		} else {
			crex_accel_class_hash_copy_notify(CG(class_table), &persistent_script->script.class_table);
		}
	}

	if (persistent_script->num_early_bindings) {
		crex_accel_do_delayed_early_binding(persistent_script, op_array);
	}
...
/*
 * crex_adler32() is based on zlib implementation
...
static crex_always_inline void adler32_do16_loop(unsigned char *buf, unsigned char *end, unsigned int *s1_out, unsigned int *s2_out)
{
...
unsigned int crex_accel_script_checksum(crex_persistent_script *persistent_script)
{
	...
	if (mem < (unsigned char*)persistent_script) {
		checksum = crex_adler32(checksum, mem, (unsigned char*)persistent_script - mem);
		size -= (unsigned char*)persistent_script - mem;
		mem  += (unsigned char*)persistent_script - mem;
	}

	crex_adler32(checksum, mem, persistent_script_check_block_size);
	mem  += sizeof(*persistent_script);
	size -= sizeof(*persistent_script);

	if (size > 0) {
		checksum = crex_adler32(checksum, mem, size);
	}
	return checksum;
}
