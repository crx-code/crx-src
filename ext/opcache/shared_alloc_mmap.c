...
static void *find_prefered_mmap_base(size_t requested_size)
{
	...
	while (fgets(buffer, MAXPATHLEN, f) && sscanf(buffer, "%lx-%lx", &start, &end) == 2) {
		if ((uintptr_t)execute_ex >= start) {
			/* the current segment lays before CRX .text segment or CRX .text segment itself */
			...
			if (last_free_addr + requested_size <= start) {
				last_candidate = CREX_MM_ALIGNED_SIZE_EX(start - requested_size, huge_page_size);
				if (last_candidate + requested_size > start) {
					last_candidate -= huge_page_size;
				}
			}
			if ((uintptr_t)execute_ex < end) {
				/* the current segment is CRX .text segment itself */
			...
			/* the current segment lays after CRX .text segment */
			if (last_free_addr + requested_size - text_start > UINT32_MAX) {
				/* the current segment and the following segments lay too far from CRX .text segment */
				break;
			}
			if (last_free_addr + requested_size <= start) {
		...
		last_free_addr = CREX_MM_ALIGNED_SIZE_EX(end, huge_page_size);
				...
				while (start < end) {
					...
					if ((uintptr_t)execute_ex >= e_start) {
						...
						if (last_free_addr + requested_size <= e_start) {
							last_candidate = CREX_MM_ALIGNED_SIZE_EX(e_start - requested_size, huge_page_size);
					...
					last_free_addr = CREX_MM_ALIGNED_SIZE_EX(e_end, huge_page_size);
					start += sz;
				}
			}
			munmap(addr, s);
...
# if defined(__x86_64__) && defined(MAP_32BIT)
		...
		if (p != MAP_FAILED) {
			...
			p = (void*)(CREX_MM_ALIGNED_SIZE_EX((ptrdiff_t)p, huge_page_size));
...
const crex_shared_memory_handlers crex_alloc_mmap_handlers = {
	create_segments,
	detach_segment,
	segment_type_size
};
