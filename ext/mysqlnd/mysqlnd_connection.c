...
CRXAPI const char * const mysqlnd_server_gone = "MySQL server has gone away";
CRXAPI const char * const mysqlnd_out_of_sync = "Commands out of sync; you can't run this command now";
CRXAPI const char * const mysqlnd_out_of_memory = "Out of memory";

CRXAPI MYSQLND_STATS * mysqlnd_global_stats = NULL;
...
MYSQLND_METHOD(mysqlnd_error_info, reset)(MYSQLND_ERROR_INFO * const info)
{
	...
	crex_llist_clean(&info->error_list);
													 ...
													 const char * const error)
{
	...
	if (err_no) {
		...
		if (error_for_the_list.error) {
			...
			crex_llist_add_element(&info->error_list, &error_for_the_list);
		}
	} else {
		info->m->reset(info);
	}
...
mysqlnd_error_info_init(MYSQLND_ERROR_INFO * const info, const bool persistent)
{
	...
	crex_llist_init(&info->error_list, sizeof(MYSQLND_ERROR_LIST_ELEMENT), (llist_dtor_func_t) mysqlnd_error_list_pdtor, persistent);
...
MYSQLND_METHOD(mysqlnd_conn_data, free_options)(MYSQLND_CONN_DATA * conn)
{
	...
	if (conn->options->connect_attr) {
		crex_hash_destroy(conn->options->connect_attr);
...
MYSQLND_METHOD(mysqlnd_conn_data, get_updated_connect_flags)(MYSQLND_CONN_DATA * conn, unsigned int mysql_flags)
{
	...
	if (conn->options->connect_attr && crex_hash_num_elements(conn->options->connect_attr)) {
		...
		*/
		...
		if (!unix_socket && !named_pipe) {
			...
			if (unix_socket) {
				...
				crx_error_docref(NULL, E_WARNING, "Impossible. Should be either socket or a pipe. Report a bug!");
			}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_conn_data, statistic)(MYSQLND_CONN_DATA * conn, crex_string **message)
{
...
CRXAPI const char * mysqlnd_get_client_info(void)
{
	return CRX_MYSQLND_VERSION;
}
/* }}} */


/* {{{ mysqlnd_get_client_version */
CRXAPI unsigned long mysqlnd_get_client_version(void)
{
...
MYSQLND_METHOD(mysqlnd_conn_data, get_server_version)(const MYSQLND_CONN_DATA * const conn)
{
	...
	major = CREX_STRTOL(p, &p, 10);
	p += 1; /* consume the dot */
	minor = CREX_STRTOL(p, &p, 10);
	p += 1; /* consume the dot */
	patch = CREX_STRTOL(p, &p, 10);

	return (crex_ulong)(major * C_L(10000) + (crex_ulong)(minor * C_L(100) + patch));
}
/* }}} */
	...
	if (FAIL == ret) {
		...
		*/
		if (!conn->error_info->error_no) {
			...
			crx_error_docref(NULL, E_WARNING, "Serious error. PID=%d", getpid());
			SET_CONNECTION_STATE(&conn->state, CONN_QUIT_SENT);
			conn->m->send_close(conn);
		} else {
												...
												)
{
	...
	switch (option) {
		...
		case MYSQL_OPT_CONNECT_ATTR_RESET:
			if (conn->options->connect_attr) {
				DBG_INF_FMT("Before reset %d attribute(s)", crex_hash_num_elements(conn->options->connect_attr));
				crex_hash_clean(conn->options->connect_attr);
			}
			break;
		case MYSQL_OPT_CONNECT_ATTR_DELETE:
			if (conn->options->connect_attr && value) {
				DBG_INF_FMT("Before delete %d attribute(s)", crex_hash_num_elements(conn->options->connect_attr));
				crex_hash_str_del(conn->options->connect_attr, value, strlen(value));
				DBG_INF_FMT("%d left", crex_hash_num_elements(conn->options->connect_attr));
			}
			break;
#ifdef WHEN_SUPPORTED_BY_MYSQLI
	...
	switch (option) {
		case MYSQL_OPT_CONNECT_ATTR_ADD:
			if (!conn->options->connect_attr) {
				...
				crex_hash_init(conn->options->connect_attr, 0, NULL, conn->persistent ? ZVAL_INTERNAL_PTR_DTOR : ZVAL_PTR_DTOR, conn->persistent);
			}
			DBG_INF_FMT("Adding [%s][%s]", key, value);
			{
				...
				if (conn->persistent) {
					str = crex_string_init(key, strlen(key), 1);
					GC_MAKE_PERSISTENT_LOCAL(str);
					ZVAL_NEW_STR(&attrz, crex_string_init(value, strlen(value), 1));
					GC_MAKE_PERSISTENT_LOCAL(C_COUNTED(attrz));
				} else {
					str = crex_string_init(key, strlen(key), 0);
					ZVAL_NEW_STR(&attrz, crex_string_init(value, strlen(value), 0));
				}
				crex_hash_update(conn->options->connect_attr, str, &attrz);
...
MYSQLND_METHOD(mysqlnd_conn_data, get_connection_stats)(const MYSQLND_CONN_DATA * const conn,
														zval * return_value CREX_FILE_LINE_DC)
{
	DBG_ENTER("mysqlnd_conn_data::get_connection_stats");
	mysqlnd_fill_stats_hash(conn->stats, mysqlnd_stats_values_names, return_value CREX_FILE_LINE_CC);
	DBG_VOID_RETURN;
}
/* }}} */
		...
		while (1) {
				...
				v == '=')
			{
				*p_copy++ = v;
				...
				crx_error_docref(NULL, E_WARNING, "Transaction name has been truncated, since it can only contain the A-Z, a-z, 0-9, \"\\\", \"-\", \"_\", and \"=\" characters");
				warned = TRUE;
			}
			++p_orig;
		}
...
/* {{{ mysqlnd_stream_array_to_fd_set */
static unsigned int
mysqlnd_stream_array_to_fd_set(MYSQLND ** conn_array, fd_set * fds, crx_socket_t * max_fd)
{
	...
	while (*p) {
		...
		if (stream != NULL &&
			SUCCESS == crx_stream_cast(stream, CRX_STREAM_AS_FD_FOR_SELECT | CRX_STREAM_CAST_INTERNAL, (void*)&this_fd, 1) &&
			CREX_VALID_SOCKET(this_fd))
		{
...
mysqlnd_stream_array_from_fd_set(MYSQLND ** conn_array, fd_set * fds)
{
	...
	while (*fwd) {
		...
		if (stream != NULL && SUCCESS == crx_stream_cast(stream, CRX_STREAM_AS_FD_FOR_SELECT | CRX_STREAM_CAST_INTERNAL,
										(void*)&this_fd, 1) && CREX_VALID_SOCKET(this_fd)) {
...
mysqlnd_poll(MYSQLND **r_array, MYSQLND **e_array, MYSQLND ***dont_poll, long sec, long usec, int * desc_num)
{
	...
	if (sec < 0 || usec < 0) {
		crx_error_docref(NULL, E_WARNING, "Negative values passed for sec and/or usec");
		DBG_RETURN(FAIL);
	}
	...
	if (!sets) {
		crx_error_docref(NULL, E_WARNING, *dont_poll ? "All arrays passed are clear":"No stream arrays were passed");
...
/* {{{ mysqlnd_connect */
CRXAPI MYSQLND * mysqlnd_connection_connect(MYSQLND * conn_handle,
