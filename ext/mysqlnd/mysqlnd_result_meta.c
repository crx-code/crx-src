...
/* {{{ crx_mysqlnd_free_field_metadata */
static void
crx_mysqlnd_free_field_metadata(MYSQLND_FIELD *meta)
{
	if (meta) {
		meta->root = NULL;
		meta->def = NULL;
		if (meta->sname) {
			crex_string_release_ex(meta->sname, 0);
		}
	}
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_res_meta, read_metadata)(MYSQLND_RES_METADATA * const meta, MYSQLND_CONN_DATA * conn, MYSQLND_RES * result)
{
	...
	for (;i < meta->field_count; i++) {
		...
		CREX_ASSERT(meta->fields[i].root == NULL);
		...
		if (mysqlnd_ps_fetch_functions[meta->fields[i].type].func == NULL) {
			...
			crx_error_docref(NULL, E_WARNING, "Unknown type %u sent by the server. Please send a report to the developers", meta->fields[i].type);
			PACKET_FREE(&field_packet);
			DBG_RETURN(FAIL);
		}

		/* For BC we have to check whether the key is numeric and use it like this */
		if ((meta->fields[i].is_numeric = CREX_HANDLE_NUMERIC(field_packet.metadata->sname, idx))) {
...
MYSQLND_METHOD(mysqlnd_res_meta, free)(MYSQLND_RES_METADATA * meta)
{
	...
	if ((fields = meta->fields)) {
		...
		i = meta->field_count;
		while (i--) {
			crx_mysqlnd_free_field_metadata(fields++);
		}
		meta->fields = NULL;
	}
	...
	*/
	...
	for (i = 0; i < meta->field_count; i++) {
		...
		if (orig_fields[i].sname) {
			new_fields[i].sname = crex_string_copy(orig_fields[i].sname);
...
CRXAPI struct st_mysqlnd_res_meta_methods *
