...
MYSQLND_METHOD(mysqlnd_debug, open)(MYSQLND_DEBUG * self, bool reopen)
{
	...
	self->stream = crx_stream_open_wrapper(self->file_name,
								   ...
								   unsigned int level, const char * type, const char * message)
{
	...
	if (level == -1) {
		level = crex_stack_count(&self->call_stack);
	}
	...
	ret = crx_stream_write(self->stream, message_line, message_line_len)? PASS:FAIL;
										  ...
										  const char * const func_name, unsigned int func_name_len)
{
	...
	if ((uint32_t) crex_stack_count(&self->call_stack) >= self->nest_level_limit) {
		return FALSE;
	}
	...
	if ((self->flags & MYSQLND_DEBUG_TRACE_MEMORY_CALLS) == 0 && self->skip_functions) {
		...
		while (*p) {
			if (*p == func_name) {
				crex_stack_push(&self->call_stack, &mysqlnd_debug_empty_string);
#ifndef MYSQLND_PROFILING_DISABLED
				if (self->flags & MYSQLND_DEBUG_PROFILE_CALLS) {
					uint64_t some_time = 0;
					crex_stack_push(&self->call_time_stack, &some_time);
				}
#endif
				return FALSE;
			}
	...
	crex_stack_push(&self->call_stack, &func_name);
#ifndef MYSQLND_PROFILING_DISABLED
	if (self->flags & MYSQLND_DEBUG_PROFILE_CALLS) {
		uint64_t some_time = 0;
		crex_stack_push(&self->call_time_stack, &some_time);
	...
	if (crex_hash_num_elements(&self->not_filtered_functions) &&
		0 == crex_hash_str_exists(&self->not_filtered_functions, func_name, strlen(func_name)))
	{
		return FALSE;
	}

	self->m->log_va(self, line, file, crex_stack_count(&self->call_stack) - 1, NULL, ">%s", func_name);
	return TRUE;
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_debug, func_leave)(MYSQLND_DEBUG * self, unsigned int line, const char * const file, uint64_t call_time)
{
	...
	func_name = crex_stack_top(&self->call_stack);

#ifndef MYSQLND_PROFILING_DISABLED
	if (profile_calls) {
		mine_non_own_time_ptr = crex_stack_top(&self->call_time_stack);
		mine_non_own_time = *mine_non_own_time_ptr;
		crex_stack_del_top(&self->call_time_stack); /* callee - removing ourselves */
	}
#endif

	if ((*func_name)[0] == '\0') {
		; /* don't log that function */
	} else if (!crex_hash_num_elements(&self->not_filtered_functions) ||
			   1 == crex_hash_str_exists(&self->not_filtered_functions, (*func_name), strlen((*func_name))))
	{
#ifndef MYSQLND_PROFILING_DISABLED
		if (FALSE == profile_calls) {
			...
			self->m->log_va(self, line, file, crex_stack_count(&self->call_stack) - 1, NULL, "<%s (total=%u own=%u in_calls=%u)",
						*func_name, (unsigned int) call_time, (unsigned int) own_time, (unsigned int) mine_non_own_time
					);

			if ((f_profile = crex_hash_str_find_ptr(&self->function_profiles, *func_name, func_name_len)) != NULL) {
				/* found */
					if (f_profile) {
				...
				f_profile->calls = 1;
				crex_hash_str_add_mem(&self->function_profiles, *func_name, func_name_len, f_profile, sizeof(struct st_mysqlnd_dbg_function_profile));
			}
			if ((uint32_t) crex_stack_count(&self->call_time_stack)) {
				uint64_t parent_non_own_time = 0;

				parent_non_own_time_ptr = crex_stack_top(&self->call_time_stack);
				parent_non_own_time = *parent_non_own_time_ptr;
				parent_non_own_time += call_time;
				crex_stack_del_top(&self->call_time_stack); /* the caller */
				crex_stack_push(&self->call_time_stack, &parent_non_own_time); /* add back the caller */
			}
		}
#endif
	}

	crex_stack_del_top(&self->call_stack);
	return PASS;
}
/* }}} */
	...
	if (self->stream) {
		...
		if (!(self->flags & MYSQLND_DEBUG_FLUSH) && (self->flags & MYSQLND_DEBUG_PROFILE_CALLS)) {
			...
			self->m->log_va(self, __LINE__, __FILE__, 0, "info : ",
					"number of functions: %d", crex_hash_num_elements(&self->function_profiles));
			CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&self->function_profiles, string_key, f_profile) {
...
MYSQLND_METHOD(mysqlnd_debug, free)(MYSQLND_DEBUG * self)
{
	...
	crex_stack_destroy(&self->call_stack);
	crex_stack_destroy(&self->call_time_stack);
	crex_hash_destroy(&self->not_filtered_functions);
	crex_hash_destroy(&self->function_profiles);
	free(self);
	return PASS;
}
/* }}} */
...
MYSQLND_METHOD(mysqlnd_debug, set_mode)(MYSQLND_DEBUG * self, const char * const mode)
{
	...
	if (crex_hash_num_elements(&self->not_filtered_functions)) {
		crex_hash_destroy(&self->not_filtered_functions);
		crex_hash_init(&self->not_filtered_functions, 0, NULL, NULL, 0);
	}
		...
		switch (mode[i]) {
			...
			case ':':
				if (state != PARSER_WAIT_COLON) {
					crx_error_docref(NULL, E_WARNING, "Consecutive semicolons at position %u", i);
				}
					...
					while (j < mode_len) {
						...
						if (mode[j] == ',' || mode[j] == ':') {
							if (j > i + 2) {
								...
								crex_hash_str_add_empty_element(&self->not_filtered_functions,
			...
			default:
				if (state == PARSER_WAIT_MODIFIER) {
					crx_error_docref(NULL, E_WARNING, "Unrecognized format '%c'", mode[i]);
					if (i+1 < mode_len && mode[i+1] == ',') {
						i+= 2;
						while (i < mode_len) {
							if (mode[i] == ':') {
					...
					crx_error_docref(NULL, E_WARNING, "Colon expected, '%c' found", mode[i]);
				}
				break;
		}
	}
...
mysqlnd_debug_init(const char * skip_functions[])
{
	...
	ret->pid = getpid();
	crex_stack_init(&ret->call_stack, sizeof(char *));
	crex_stack_init(&ret->call_time_stack, sizeof(uint64_t));
	crex_hash_init(&ret->not_filtered_functions, 0, NULL, NULL, 0);
	crex_hash_init(&ret->function_profiles, 0, NULL, free_ptr, 0);
...
/* {{{ mysqlnd_debug */
CRXAPI void mysqlnd_debug(const char * mode)
{
#if CRX_DEBUG
	...
	if (dbg) {
		...
		while (crex_stack_count(&dbg->call_stack)) {
			crex_stack_del_top(&dbg->call_stack);
		}
		while (crex_stack_count(&dbg->call_time_stack)) {
			crex_stack_del_top(&dbg->call_time_stack);
		}
	}
#endif
}
