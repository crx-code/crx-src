...
ps_fetch_from_1_to_8_bytes(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len,
						   const crex_uchar ** row, unsigned int byte_count)
{
	...
	if (field->flags & UNSIGNED_FLAG) {
		...
		if (field->flags & ZEROFILL_FLAG) {
			...
			ZVAL_STR(zv, crex_strpprintf(0, "%0*" PRIu64, (int) field->length, uval));
		} else
#if SIZEOF_CREX_LONG==4
		if (uval > INT_MAX) {
			DBG_INF("stringify");
			ZVAL_STR(zv, crex_u64_to_str(uval));
		} else
#endif /* #if SIZEOF_LONG==4 */
		{
			if (byte_count < 8 || uval <= L64(9223372036854775807)) {
				ZVAL_LONG(zv, (crex_long) uval); /* the cast is safe, we are in the range */
			} else {
				DBG_INF("stringify");
				ZVAL_STR(zv, crex_u64_to_str(uval));
			}
		...
		if ((L64(2147483647) < (int64_t) lval) || (L64(-2147483648) > (int64_t) lval)) {
			DBG_INF("stringify");
			ZVAL_STR(zv, crex_i64_to_str(lval));
		} else
#endif /* SIZEOF */
		{
			ZVAL_LONG(zv, (crex_long) lval); /* the cast is safe, we are in the range */
		}
	}
...
/* {{{ ps_fetch_null */
static void
ps_fetch_null(zval *zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	ZVAL_NULL(zv);
}
/* }}} */
...
/* {{{ ps_fetch_int8 */
static void
ps_fetch_int8(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
...
/* {{{ ps_fetch_int16 */
static void
ps_fetch_int16(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
...
/* {{{ ps_fetch_int32 */
static void
ps_fetch_int32(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
...
/* {{{ ps_fetch_int64 */
static void
ps_fetch_int64(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
...
/* {{{ ps_fetch_float */
static void
ps_fetch_float(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	float fval;
	double dval;
...
/* {{{ ps_fetch_double */
static void
ps_fetch_double(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	double value;
...
/* {{{ ps_fetch_time */
static void
ps_fetch_time(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	struct st_mysqlnd_time t;
	crex_ulong length; /* First byte encodes the length */
	DBG_ENTER("ps_fetch_time");

	if ((length = crx_mysqlnd_net_field_length(row))) {
		const crex_uchar * to = *row;

		t.time_type = MYSQLND_TIMESTAMP_TIME;
		t.neg			= (bool) to[0];

		t.day			= (crex_ulong) sint4korr(to+1);
		t.hour			= (unsigned int) to[5];
		t.minute		= (unsigned int) to[6];
		t.second		= (unsigned int) to[7];
		t.second_part	= (length > 8) ? (crex_ulong) sint4korr(to+8) : 0;
		t.year			= t.month= 0;
		if (t.day) {
...
/* {{{ ps_fetch_date */
static void
ps_fetch_date(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	...
	ZVAL_STR(zv, crex_strpprintf(0, "%04u-%02u-%02u", t.year, t.month, t.day));
	DBG_VOID_RETURN;
}
/* }}} */
...
/* {{{ ps_fetch_datetime */
static void
ps_fetch_datetime(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	struct st_mysqlnd_time t;
	crex_ulong length; /* First byte encodes the length*/
	...
	if ((length = crx_mysqlnd_net_field_length(row))) {
		...
		t.second_part = (length > 7) ? (crex_ulong) sint4korr(to+7) : 0;
	...
	if (field->decimals > 0 && field->decimals < 7) {
		ZVAL_STR(zv, crex_strpprintf(0, "%04u-%02u-%02u %02u:%02u:%02u.%0*u",
			t.year, t.month, t.day, t.hour, t.minute, t.second, field->decimals,
			(uint32_t) (t.second_part / pow(10, 6 - field->decimals))));
	} else {
		ZVAL_STR(zv,  crex_strpprintf(0, "%04u-%02u-%02u %02u:%02u:%02u",
...
/* {{{ ps_fetch_string */
static void
ps_fetch_string(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	const crex_ulong length = crx_mysqlnd_net_field_length(row);
	DBG_ENTER("ps_fetch_string");
	DBG_INF_FMT("len = " CREX_ULONG_FMT, length);
...
/* {{{ ps_fetch_bit */
static void
ps_fetch_bit(zval * zv, const MYSQLND_FIELD * const field, const unsigned int pack_len, const crex_uchar ** row)
{
	const crex_ulong length = crx_mysqlnd_net_field_length(row);
...
void _mysqlnd_init_ps_fetch_subsystem(void)
{
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_TIME].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_DATE].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_NEWDATE].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_DATETIME].crx_type= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_TIMESTAMP].crx_type= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_JSON].crx_type = IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_TINY_BLOB].crx_type = IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_BLOB].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_MEDIUM_BLOB].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_LONG_BLOB].crx_type	= IS_STRING;

	mysqlnd_ps_fetch_functions[MYSQL_TYPE_BIT].func		= ps_fetch_bit;
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_BIT].pack_len	= 8;
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_BIT].crx_type	= IS_LONG;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_VAR_STRING].crx_type = IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_VARCHAR].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_STRING].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_DECIMAL].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_NEWDECIMAL].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_ENUM].crx_type	= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_SET].crx_type		= IS_STRING;
	...
	mysqlnd_ps_fetch_functions[MYSQL_TYPE_GEOMETRY].crx_type= IS_STRING;
}
/* }}} */
...
static enum_func_status
mysqlnd_stmt_execute_check_n_enlarge_buffer(crex_uchar **buf, crex_uchar **p, size_t * buf_len, crex_uchar * const provided_buffer, size_t needed_bytes)
{
...
mysqlnd_stmt_execute_prepare_param_types(MYSQLND_STMT_DATA * stmt, zval ** copies_param, int * resend_types_next_time)
{
	...
	for (i = 0; i < stmt->param_count; i++) {
		...
		if (!C_ISNULL_P(parameter) && (current_type == MYSQL_TYPE_LONG || current_type == MYSQL_TYPE_LONGLONG || current_type == MYSQL_TYPE_TINY)) {
			/* always copy the var, because we do many conversions */
			if (C_TYPE_P(parameter) != IS_LONG &&
			...
			*/
			if (C_TYPE_P(parameter) != IS_LONG) {
				zval *tmp_data = (*copies_param && !C_ISUNDEF((*copies_param)[i]))? &(*copies_param)[i]: parameter;
				/*
				...
				*/
				if (d >= (double) CREX_LONG_MAX || d < (double) CREX_LONG_MIN) {
...
static void
mysqlnd_stmt_execute_store_types(MYSQLND_STMT_DATA * stmt, zval * copies, crex_uchar ** p)
{
	unsigned int i;
	for (i = 0; i < stmt->param_count; i++) {
...
#endif
		ZVAL_DEREF(parameter);
		if (!C_ISNULL_P(parameter) && (current_type == MYSQL_TYPE_LONG || current_type == MYSQL_TYPE_LONGLONG)) {
			/*
			  if it doesn't fit in a long send it as a string.
			  Check bug #52891 : Wrong data inserted with mysqli/mysqlnd when using bind_param, value > LONG_MAX
			*/
			if (C_TYPE_P(parameter) != IS_LONG) {
				const zval *tmp_data = (copies && !C_ISUNDEF(copies[i]))? &copies[i] : parameter;
				/*
				...
				*/
				if (C_TYPE_P(tmp_data) == IS_STRING) {
...
mysqlnd_stmt_execute_calculate_param_values_size(MYSQLND_STMT_DATA * stmt, zval ** copies_param, size_t * data_size)
{
	...
	for (i = 0; i < stmt->param_count; i++) {
		...
		bind_var = the_var;
		ZVAL_DEREF(the_var);
		if ((stmt->param_bind[i].type != MYSQL_TYPE_LONG_BLOB && C_TYPE_P(the_var) == IS_NULL)) {
			continue;
		}

		if (C_ISREF_P(bind_var)) {
			for (j = i + 1; j < stmt->param_count; j++) {
				if (C_ISREF(stmt->param_bind[j].zv) && C_REFVAL(stmt->param_bind[j].zv) == the_var) {
					/* Double binding of the same zval, make a copy */
					if (!*copies_param || C_ISUNDEF((*copies_param)[i])) {
		...
		switch (stmt->param_bind[i].type) {
			...
			case MYSQL_TYPE_LONG:
				{
					zval *tmp_data = (*copies_param && !C_ISUNDEF((*copies_param)[i]))? &(*copies_param)[i]: the_var;
					if (C_TYPE_P(tmp_data) == IS_STRING) {
						goto use_string;
					}
					convert_to_long(tmp_data);
...
static void
mysqlnd_stmt_execute_store_param_values(MYSQLND_STMT_DATA * stmt, zval * copies, crex_uchar * buf, crex_uchar ** p, size_t null_byte_offset)
{
	unsigned int i;
	for (i = 0; i < stmt->param_count; i++) {
		...
		ZVAL_DEREF(parameter);
		data = (copies && !C_ISUNDEF(copies[i]))? &copies[i]: parameter;
		/* Handle long data */
		if (!C_ISUNDEF_P(parameter) && C_TYPE_P(data) == IS_NULL) {
			(buf + null_byte_offset)[i/8] |= (crex_uchar) (1 << (i & 7));
		} else {
			switch (stmt->param_bind[i].type) {
				case MYSQL_TYPE_DOUBLE:
					convert_to_double(data);
					float8store(*p, C_DVAL_P(data));
					(*p) += 8;
					break;
				case MYSQL_TYPE_LONGLONG:
				...
				case MYSQL_TYPE_LONG_BLOB:
					if (stmt->param_bind[i].flags & MYSQLND_PARAM_BIND_BLOB_USED) {
						...
						*p = crx_mysqlnd_net_store_length(*p, 0);
					}
					break;
				case MYSQL_TYPE_VAR_STRING:
send_string:
					{
						const size_t len = C_STRLEN_P(data);
						/* to is after p. The latter hasn't been moved */
						*p = crx_mysqlnd_net_store_length(*p, len);
						memcpy(*p, C_STRVAL_P(data), len);
						(*p) += len;
					}
					break;
				default:
...
static enum_func_status
mysqlnd_stmt_execute_store_params(MYSQLND_STMT * s, crex_uchar **buf, crex_uchar **p, size_t *buf_len )
{
	MYSQLND_STMT_DATA * stmt = s->data;
	crex_uchar * provided_buffer = *buf;
	size_t data_size = 0;
...
enum_func_status
mysqlnd_stmt_execute_generate_request(MYSQLND_STMT * const s, crex_uchar ** request, size_t *request_len, bool * free_buffer)
{
	MYSQLND_STMT_DATA * stmt = s->data;
	crex_uchar	*p = stmt->execute_cmd_buffer.buffer,
	...
	int1store(p, (crex_uchar) stmt->flags);
	p++;
