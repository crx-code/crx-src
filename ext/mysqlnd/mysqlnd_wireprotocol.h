...
#define MYSQLND_NULL_LENGTH	(crex_ulong) ~0

/* Used in mysqlnd_debug.c */
CRXAPI extern const char mysqlnd_read_header_name[];
CRXAPI extern const char mysqlnd_read_body_name[];
...
CRXAPI extern const char * const mysqlnd_command_to_text[COM_END];
...
typedef struct st_mysqlnd_packet_auth_response {
	...
	crex_uchar	*new_auth_protocol_data;
...
typedef struct st_mysqlnd_packet_command {
	MYSQLND_PACKET_HEADER			header;
	enum crx_mysqlnd_server_command	command;
...
crex_ulong		crx_mysqlnd_net_field_length(const crex_uchar **packet);
crex_uchar *	crx_mysqlnd_net_store_length(crex_uchar *packet, const uint64_t length);
size_t			crx_mysqlnd_net_store_length_size(uint64_t length);

CRXAPI extern const char * const mysqlnd_empty_string;

enum_func_status crx_mysqlnd_rowp_read_binary_protocol(MYSQLND_ROW_BUFFER * row_buffer, zval * fields,
										 unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
										 bool as_int_or_float, MYSQLND_STATS * stats);


enum_func_status crx_mysqlnd_rowp_read_text_protocol(MYSQLND_ROW_BUFFER * row_buffer, zval * fields,
										 unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
										 bool as_int_or_float, MYSQLND_STATS * stats);


CRXAPI MYSQLND_PROTOCOL_PAYLOAD_DECODER_FACTORY * mysqlnd_protocol_payload_decoder_factory_init(MYSQLND_CONN_DATA * conn, const bool persistent);
CRXAPI void mysqlnd_protocol_payload_decoder_factory_free(MYSQLND_PROTOCOL_PAYLOAD_DECODER_FACTORY * const payload_decoder_factory);
