...
#define BAIL_IF_NO_MORE_DATA \
	if (UNEXPECTED((size_t)(p - begin) > packet->header.size)) { \
		crx_error_docref(NULL, E_WARNING, "Premature end of data (mysqlnd_wireprotocol.c:%u)", __LINE__); \
		goto premature_end; \
	} \
...
/* {{{ crx_mysqlnd_net_field_length
   Get next field's length */
crex_ulong
crx_mysqlnd_net_field_length(const crex_uchar **packet)
{
	const crex_uchar *p= (const crex_uchar *)*packet;
	...
	switch (*p) {
		...
		case 252:
			(*packet) += 3;
			return (crex_ulong) uint2korr(p+1);
		case 253:
			(*packet) += 4;
			return (crex_ulong) uint3korr(p+1);
		default:
			(*packet) += 9;
			return (crex_ulong) uint4korr(p+1);
	}
}
/* }}} */


/* {{{ crx_mysqlnd_net_field_length_ll
   Get next field's length */
uint64_t
crx_mysqlnd_net_field_length_ll(const crex_uchar **packet)
{
	const crex_uchar *p = (crex_uchar *)*packet;
...
/* {{{ crx_mysqlnd_net_store_length */
crex_uchar *
crx_mysqlnd_net_store_length(crex_uchar *packet, const uint64_t length)
{
	if (length < (uint64_t) L64(251)) {
		*packet = (crex_uchar) length;
		return packet + 1;
	}
	...
	if (length < (uint64_t) L64(16777216)) {
		*packet++ = 253;
		int3store(packet,(crex_ulong) length);
		return packet + 3;
	}
	*packet++ = 254;
...
/* {{{ crx_mysqlnd_net_store_length_size */
size_t
crx_mysqlnd_net_store_length_size(uint64_t length)
{
	if (length < (uint64_t) L64(251)) {
		return 1;
	}
...
/* {{{ crx_mysqlnd_read_error_from_line */
static enum_func_status
crx_mysqlnd_read_error_from_line(const crex_uchar * const buf, const size_t buf_len,
								 char *error, const size_t error_buf_len,
								 unsigned int *error_no, char *sqlstate)
{
	const crex_uchar *p = buf;
	size_t error_msg_len = 0;

	DBG_ENTER("crx_mysqlnd_read_error_from_line");
...
{
	crex_uchar buffer[MYSQLND_HEADER_SIZE];
	...
	crx_error(E_WARNING, "Packets out of order. Expected %u received %u. Packet size=%zu",
			  pfc->data->packet_no, header->packet_no, header->size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
mysqlnd_read_packet_header_and_body(MYSQLND_PACKET_HEADER * packet_header,
									...
									crex_uchar * const buf, const size_t buf_size,
...
/* {{{ crx_mysqlnd_greet_read */
static enum_func_status
crx_mysqlnd_greet_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	const crex_uchar * pad_start = NULL;
	...
	crex_uchar * const buf = (crex_uchar *) pfc->cmd_buffer.buffer;
	const crex_uchar * p = buf;
	const crex_uchar * const begin = buf;

	DBG_ENTER("crx_mysqlnd_greet_read");
	...
	if (ERROR_MARKER == packet->protocol_version) {
		crx_mysqlnd_read_error_from_line(p, packet->header.size - 1,
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "GREET packet %zu bytes shorter than expected",
...
/* {{{ crx_mysqlnd_greet_free_mem */
static
void crx_mysqlnd_greet_free_mem(void * _packet)
{
...
/* {{{ crx_mysqlnd_auth_write */
static
size_t crx_mysqlnd_auth_write(MYSQLND_CONN_DATA * conn, void * _packet)
{
	crex_uchar buffer[AUTH_WRITE_BUFFER_LEN];
	crex_uchar *p = buffer + MYSQLND_HEADER_SIZE; /* start after the header */
	size_t len;
	...
	DBG_ENTER("crx_mysqlnd_auth_write");
...
/*!!!!! is the buffer big enough ??? */
		...
		if (packet->connect_attr && crex_hash_num_elements(packet->connect_attr)) {
			size_t ca_payload_len = 0;

			{
				crex_string * key;
				zval * entry_value;
				CREX_HASH_MAP_FOREACH_STR_KEY_VAL(packet->connect_attr, key, entry_value) {
					if (key) { /* HASH_KEY_IS_STRING */
						size_t value_len = C_STRLEN_P(entry_value);

						ca_payload_len += crx_mysqlnd_net_store_length_size(ZSTR_LEN(key));
						ca_payload_len += ZSTR_LEN(key);
						ca_payload_len += crx_mysqlnd_net_store_length_size(value_len);
						ca_payload_len += value_len;
					}
				} CREX_HASH_FOREACH_END();
			}

			if (sizeof(buffer) >= (ca_payload_len + crx_mysqlnd_net_store_length_size(ca_payload_len) + (p - buffer))) {
				p = crx_mysqlnd_net_store_length(p, ca_payload_len);

				{
					...
					CREX_HASH_MAP_FOREACH_STR_KEY_VAL(packet->connect_attr, key, entry_value) {
						if (key) { /* HASH_KEY_IS_STRING */
							...
							/* copy key */
							p = crx_mysqlnd_net_store_length(p, ZSTR_LEN(key));
							memcpy(p, ZSTR_VAL(key), ZSTR_LEN(key));
							p+= ZSTR_LEN(key);
							/* copy value */
							p = crx_mysqlnd_net_store_length(p, value_len);
							memcpy(p, C_STRVAL_P(entry_value), value_len);
							p+= value_len;
						}
					} CREX_HASH_FOREACH_END();
				}
...
/* {{{ crx_mysqlnd_auth_response_read */
static enum_func_status
crx_mysqlnd_auth_response_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_auth_response_read");
	...
	if (0xFE == packet->response_code) {
		...
		crex_ulong net_len;
		/* Everything was fine! */
		packet->affected_rows  = crx_mysqlnd_net_field_length_ll(&p);
		BAIL_IF_NO_MORE_DATA;

		packet->last_insert_id = crx_mysqlnd_net_field_length_ll(&p);
		BAIL_IF_NO_MORE_DATA;
		...
		/* There is a message */
		if (packet->header.size > (size_t) (p - buf) && (net_len = crx_mysqlnd_net_field_length(&p))) {
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "AUTH_RESPONSE packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_auth_response_free_mem */
static void
crx_mysqlnd_auth_response_free_mem(void * _packet)
{
...
/* {{{ crx_mysqlnd_change_auth_response_write */
static size_t
crx_mysqlnd_change_auth_response_write(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	crex_uchar * const buffer = pfc->cmd_buffer.length >= total_packet_size? pfc->cmd_buffer.buffer : mnd_emalloc(total_packet_size);
	crex_uchar * p = buffer + MYSQLND_HEADER_SIZE; /* start after the header */

	DBG_ENTER("crx_mysqlnd_change_auth_response_write");
...
/* {{{ crx_mysqlnd_ok_read */
static enum_func_status
crx_mysqlnd_ok_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
...
premature_end:
	DBG_ERR_FMT("OK packet %zu bytes shorter than expected", p - begin - packet->header.size);
	crx_error_docref(NULL, E_WARNING, "OK packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_ok_free_mem */
static void
crx_mysqlnd_ok_free_mem(void * _packet)
{
...
/* {{{ crx_mysqlnd_eof_read */
static enum_func_status
crx_mysqlnd_eof_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	/*
	...
	*/
	...
	DBG_ENTER("crx_mysqlnd_eof_read");
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "EOF packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_cmd_write */
size_t crx_mysqlnd_cmd_write(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_cmd_write");
	/*
	...
	*/
	...
	if (!packet->argument.s || !packet->argument.l) {
		crex_uchar buffer[MYSQLND_HEADER_SIZE + 1];
...
/* {{{ crx_mysqlnd_rset_header_read */
static enum_func_status
crx_mysqlnd_rset_header_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_rset_header_read");
	...
	*/
	...
	packet->field_count = crx_mysqlnd_net_field_length(&p);
	BAIL_IF_NO_MORE_DATA;
	...
	switch (packet->field_count) {
		...
		case 0x00:
			...
			if (packet->header.size  > (size_t) (p - buf) && (len = crx_mysqlnd_net_field_length(&p))) {
				packet->info_or_local_file.s = mnd_emalloc(len + 1);
				memcpy(packet->info_or_local_file.s, p, len);
				packet->info_or_local_file.s[len] = '\0';
				packet->info_or_local_file.l = len;
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "RSET_HEADER packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_rset_header_free_mem */
static
void crx_mysqlnd_rset_header_free_mem(void * _packet)
{
	MYSQLND_PACKET_RSET_HEADER *p= (MYSQLND_PACKET_RSET_HEADER *) _packet;
	DBG_ENTER("crx_mysqlnd_rset_header_free_mem");
...
#define READ_RSET_FIELD(field_name) do { \
		len = crx_mysqlnd_net_field_length(&p); \
...
/* {{{ crx_mysqlnd_rset_field_read */
static enum_func_status
crx_mysqlnd_rset_field_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_rset_field_read");
	...
	if (ERROR_MARKER == *p) {
		...
		crx_error_docref(NULL, E_WARNING, "Premature EOF in result field metadata");
		DBG_RETURN(FAIL);
	}
	...
	if (UNEXPECTED(12 != *p)) {
		...
		crx_error_docref(NULL, E_WARNING, "Protocol error. Server sent false length. Expected 12");
	}
	...
	*/
	...
	{
		BAIL_IF_NO_MORE_DATA;
		DBG_INF_FMT("Def found, length " CREX_ULONG_FMT, len);
	...
	if (EXPECTED(meta->name_length != 0)) {
		meta->sname = crex_string_init_interned(meta->name, meta->name_length, 0);
...
faulty_or_fake:
	...
	crx_error_docref(NULL, E_WARNING, "Protocol error. Server sent NULL_LENGTH."
					 " The server is faulty");
	DBG_RETURN(FAIL);
premature_end:
	DBG_ERR_FMT("RSET field packet %zu bytes shorter than expected", p - begin - packet->header.size);
	crx_error_docref(NULL, E_WARNING, "Result set field packet %zu bytes "
...
/* {{{ crx_mysqlnd_read_row_ex */
static enum_func_status
crx_mysqlnd_read_row_ex(MYSQLND_PFC * pfc,
						MYSQLND_VIO * vio,
						MYSQLND_STATS * stats,
						MYSQLND_ERROR_INFO * error_info,
						MYSQLND_CONNECTION_STATE * connection_state,
						...
						size_t * const data_size)
{
	...
	DBG_ENTER("crx_mysqlnd_read_row_ex");
	...
	*/
	...
	/*
	  We're allocating an extra byte, as crx_mysqlnd_rowp_read_text_protocol
	...
	*data_size = 0;
	if (UNEXPECTED(FAIL == mysqlnd_read_header(pfc, vio, &header, stats, error_info))) {
		...
		p = (crex_uchar *) buffer->ptr + *data_size;
		*data_size += header.size;
...
/* {{{ crx_mysqlnd_rowp_read_binary_protocol */
enum_func_status
crx_mysqlnd_rowp_read_binary_protocol(MYSQLND_ROW_BUFFER * row_buffer, zval * fields,
...
{
	unsigned int i;
	const crex_uchar * p = row_buffer->ptr;
	const crex_uchar * null_ptr;
	crex_uchar bit;
	zval *current_field, *end_field, *start_field;

	DBG_ENTER("crx_mysqlnd_rowp_read_binary_protocol");
	...
	for (i = 0, current_field = start_field; current_field < end_field; current_field++, i++) {
		...
		MYSQLND_INC_CONN_STATISTIC_W_VALUE2(stats, statistic, 1,
										...
										(C_TYPE_P(current_field) == IS_STRING)?
											C_STRLEN_P(current_field) : (size_t)(p - orig_p));
...
/* {{{ crx_mysqlnd_rowp_read_text_protocol */
enum_func_status
crx_mysqlnd_rowp_read_text_protocol(MYSQLND_ROW_BUFFER * row_buffer, zval * fields,
									unsigned int field_count, const MYSQLND_FIELD * fields_metadata,
									bool as_int_or_float, MYSQLND_STATS * stats)
{
	unsigned int i;
	zval *current_field, *end_field, *start_field;
	crex_uchar * p = row_buffer->ptr;
	const size_t data_size = row_buffer->size;
	const crex_uchar * const packet_end = (crex_uchar*) p + data_size;

	DBG_ENTER("crx_mysqlnd_rowp_read_text_protocol");
	...
	for (i = 0, current_field = start_field; current_field < end_field; current_field++, i++) {
		/* crx_mysqlnd_net_field_length() call should be after *this_field_len_pos = p; */
		const crex_ulong len = crx_mysqlnd_net_field_length((const crex_uchar **) &p);

		/* NULL or NOT NULL, this is the question! */
		if (len == MYSQLND_NULL_LENGTH) {
			ZVAL_NULL(current_field);
		} else if ((p + len) > packet_end) {
			crx_error_docref(NULL, E_WARNING, "Malformed server packet. Field length pointing %zu"
			...
			if (fields_metadata[i].type == MYSQL_TYPE_BIT) {
				...
				*/
				ps_fetch_from_1_to_8_bytes(current_field, &(fields_metadata[i]), 0, (const crex_uchar **) &p, len);
				/*
				  We have advanced in ps_fetch_from_1_to_8_bytes. We should go back because
				  later in this function there will be an advancement.
				*/
				p -= len;
				if (C_TYPE_P(current_field) == IS_LONG && !as_int_or_float) {
					/* we are using the text protocol, so convert to string */
					char tmp[22];
					const size_t tmp_len = sprintf((char *)&tmp, CREX_ULONG_FMT, C_LVAL_P(current_field));
					ZVAL_STRINGL(current_field, tmp, tmp_len);
				} else if (C_TYPE_P(current_field) == IS_STRING) {
					/* nothing to do here, as we want a string and ps_fetch_from_1_to_8_bytes() has given us one */
				}
			} else if (as_int_or_float && perm_bind.crx_type == IS_LONG
					&& !(fields_metadata[i].flags & ZEROFILL_FLAG)) {
				crex_uchar save = *(p + len);
				/* We have to make it ASCIIZ temporarily */
				*(p + len) = '\0';
				if (perm_bind.pack_len < SIZEOF_CREX_LONG) {
					/* direct conversion */
					int64_t v =
#ifndef CRX_WIN32
						atoll((char *) p);
						...
						_atoi64((char *) p);
#endif
					ZVAL_LONG(current_field, (crex_long) v); /* the cast is safe */
				} else {
					uint64_t v =
#ifndef CRX_WIN32
...
#error Need fix for this architecture
				...
				*(p + len) = save;
			} else if (as_int_or_float && perm_bind.crx_type == IS_DOUBLE) {
				crex_uchar save = *(p + len);
				/* We have to make it ASCIIZ temporarily */
				*(p + len) = '\0';
				ZVAL_DOUBLE(current_field, crex_strtod((char *) p, NULL));
				*(p + len) = save;
			} else {
...
/* {{{ crx_mysqlnd_rowp_read */
static enum_func_status
crx_mysqlnd_rowp_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_rowp_read");

	ret = crx_mysqlnd_read_row_ex(pfc, vio, stats, error_info, &conn->state,
	...
	*/
	...
	if (ERROR_MARKER == (*(p = packet->row_buffer.ptr))) {
		...
		*/
		ret = FAIL;
		crx_mysqlnd_read_error_from_line(p + 1, data_size - 1,
...
/* {{{ crx_mysqlnd_stats_read */
static enum_func_status
crx_mysqlnd_stats_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	MYSQLND_CONNECTION_STATE * connection_state = &conn->state;
	const size_t buf_len = pfc->cmd_buffer.length;
	crex_uchar *buf = (crex_uchar *) pfc->cmd_buffer.buffer;

	DBG_ENTER("crx_mysqlnd_stats_read");
...
/* {{{ crx_mysqlnd_stats_free_mem */
static
void crx_mysqlnd_stats_free_mem(void * _packet)
{
...
/* {{{ crx_mysqlnd_prepare_read */
static enum_func_status
crx_mysqlnd_prepare_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_prepare_read");
	...
	if (ERROR_MARKER == packet->error_code) {
		crx_mysqlnd_read_error_from_line(p, data_size - 1,
										 packet->error_info.error,
										 sizeof(packet->error_info.error),
										 &packet->error_info.error_no,
										 packet->error_info.sqlstate
	...
	if (data_size != PREPARE_RESPONSE_SIZE_41 &&
		...
		crx_error(E_WARNING, "Wrong COM_STMT_PREPARE response size. Received %u", data_size);
		DBG_RETURN(FAIL);
	}
	...
	DBG_INF_FMT("Prepare packet read: stmt_id=" CREX_ULONG_FMT " fields=%u params=%u",
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "PREPARE packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_chg_user_read */
static enum_func_status
crx_mysqlnd_chg_user_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_chg_user_read");
...
premature_end:
	...
	crx_error_docref(NULL, E_WARNING, "CHANGE_USER packet %zu bytes shorter than expected",
						 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_chg_user_free_mem */
static void
crx_mysqlnd_chg_user_free_mem(void * _packet)
{
...
/* {{{ crx_mysqlnd_sha256_pk_request_write */
static
size_t crx_mysqlnd_sha256_pk_request_write(MYSQLND_CONN_DATA * conn, void * _packet)
{
	MYSQLND_ERROR_INFO * error_info = conn->error_info;
	MYSQLND_PFC * pfc = conn->protocol_frame_codec;
	MYSQLND_VIO * vio = conn->vio;
	...
	DBG_ENTER("crx_mysqlnd_sha256_pk_request_write");
...
/* {{{ crx_mysqlnd_sha256_pk_request_response_read */
static enum_func_status
crx_mysqlnd_sha256_pk_request_response_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	DBG_ENTER("crx_mysqlnd_sha256_pk_request_response_read");
...
premature_end:
	DBG_ERR_FMT("OK packet %zu bytes shorter than expected", p - begin - packet->header.size);
	crx_error_docref(NULL, E_WARNING, "SHA256_PK_REQUEST_RESPONSE packet %zu bytes shorter than expected",
					 p - begin - packet->header.size);
	DBG_RETURN(FAIL);
}
/* }}} */
...
/* {{{ crx_mysqlnd_sha256_pk_request_response_free_mem */
static void
crx_mysqlnd_sha256_pk_request_response_free_mem(void * _packet)
{
...
static
size_t crx_mysqlnd_cached_sha2_result_write(MYSQLND_CONN_DATA * conn, void * _packet)
{
	...
	crex_uchar *buffer = do_alloca(MYSQLND_HEADER_SIZE + packet->password_len + 1, use_heap);
	size_t sent;

	DBG_ENTER("crx_mysqlnd_cached_sha2_result_write");
...
static enum_func_status
crx_mysqlnd_cached_sha2_result_read(MYSQLND_CONN_DATA * conn, void * _packet)
{
	MYSQLND_PACKET_CACHED_SHA2_RESULT * packet= (MYSQLND_PACKET_CACHED_SHA2_RESULT *) _packet;
	MYSQLND_ERROR_INFO * error_info = conn->error_info;
	MYSQLND_PFC * pfc = conn->protocol_frame_codec;
	...
	DBG_ENTER("crx_mysqlnd_cached_sha2_result_read");
...
mysqlnd_packet_methods packet_methods[PROT_LAST] =
{
	...
	{
		crx_mysqlnd_auth_response_read, /* read */
		NULL, /* write */
		crx_mysqlnd_auth_response_free_mem,
	}, /* PROT_AUTH_RESP_PACKET */
	{
		NULL, /* read */
		crx_mysqlnd_change_auth_response_write, /* write */
		NULL,
	}, /* PROT_CHANGE_AUTH_RESP_PACKET */
	{
		crx_mysqlnd_ok_read, /* read */
		NULL, /* write */
		crx_mysqlnd_ok_free_mem,
	}, /* PROT_OK_PACKET */
	{
		crx_mysqlnd_eof_read, /* read */
		NULL, /* write */
		NULL,
	}, /* PROT_EOF_PACKET */
	{
		NULL, /* read */
		crx_mysqlnd_cmd_write, /* write */
		NULL,
	}, /* PROT_CMD_PACKET */
	{
		crx_mysqlnd_rset_header_read, /* read */
		NULL, /* write */
		crx_mysqlnd_rset_header_free_mem,
	}, /* PROT_RSET_HEADER_PACKET */
	{
		crx_mysqlnd_rset_field_read, /* read */
		NULL, /* write */
		NULL,
	}, /* PROT_RSET_FLD_PACKET */
	{
		crx_mysqlnd_rowp_read, /* read */
		NULL, /* write */
		NULL,
	}, /* PROT_ROW_PACKET */
	{
		crx_mysqlnd_stats_read, /* read */
		NULL, /* write */
		crx_mysqlnd_stats_free_mem,
	}, /* PROT_STATS_PACKET */
	{
		crx_mysqlnd_prepare_read, /* read */
		NULL, /* write */
		NULL,
	}, /* PROT_PREPARE_RESP_PACKET */
	{
		crx_mysqlnd_chg_user_read, /* read */
		NULL, /* write */
		crx_mysqlnd_chg_user_free_mem,
	...
	{
		crx_mysqlnd_cached_sha2_result_read,
		crx_mysqlnd_cached_sha2_result_write,
		NULL
...
MYSQLND_METHOD(mysqlnd_protocol, send_command)(
		...
		const enum crx_mysqlnd_server_command command,
		const crex_uchar * const arg, const size_t arg_len,
		const bool silent,
		...
		void * send_close_ctx)
{
	...
	if (! PACKET_WRITE(payload_decoder_factory->conn, &cmd_packet)) {
		if (!silent && error_info->error_no != CR_SERVER_GONE_ERROR) {
			...
			crx_error(E_WARNING, "Error while sending %s packet. PID=%d", mysqlnd_command_to_text[command], getpid());
		}
						...
						MYSQLND_UPSERT_STATUS * const upsert_status)
{
	...
	if (FAIL == (ret = PACKET_READ(payload_decoder_factory->conn, &response))) {
		...
		crx_error_docref(NULL, E_WARNING, "EOF packet expected, field count wasn't 0xFE but 0x%2X", response.field_count);
	} else {
	...
	)
{
	...
	switch (ok_packet) {
		...
		default:
			SET_CLIENT_ERROR(error_info, CR_MALFORMED_PACKET, UNKNOWN_SQLSTATE, "Malformed packet");
			crx_error_docref(NULL, E_ERROR, "Wrong response packet %u passed to the function", ok_packet);
			break;
	}
	if (!silent && error_info->error_no == CR_MALFORMED_PACKET) {
		crx_error_docref(NULL, E_WARNING, "Error while reading %s's response packet. PID=%d", mysqlnd_command_to_text[command], getpid());
	}
	DBG_INF(ret == PASS ? "PASS":"FAIL");
	DBG_RETURN(ret);
}
...
CRXAPI MYSQLND_PROTOCOL_PAYLOAD_DECODER_FACTORY *
