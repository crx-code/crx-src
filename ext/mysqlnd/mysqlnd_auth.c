...
"store a new, and more secure, hash value in mysql.user. If this user is used in other scripts executed by CRX 5.2 or earlier you might need to remove the old-passwords "
			...
			)
{
	...
	do {
		...
		{
			crex_uchar * switch_to_auth_protocol_data = NULL;
			size_t switch_to_auth_protocol_data_len = 0;
			crex_uchar * scrambled_data = NULL;
	...
	if (conn->error_info->error_no) {
		...
		*/
		if (conn->m->get_server_version(conn) > 50113L &&conn->m->get_server_version(conn) < 50118L) {
			...
			DBG_INF_FMT("Server is " CREX_ULONG_FMT ", buggy, sends two ERR messages", conn->m->get_server_version(conn));
		}
	}
	if (ret == PASS) {
		CREX_ASSERT(conn->username.s != user && conn->password.s != passwd);
...
/* {{{ crx_mysqlnd_crypt */
static void
crx_mysqlnd_crypt(crex_uchar *buffer, const crex_uchar *s1, const crex_uchar *s2, size_t len)
{
	const crex_uchar *s1_end = s1 + len;
	while (s1 < s1_end) {
		*buffer++= *s1++ ^ *s2++;
	}
}
...
/* {{{ crx_mysqlnd_scramble */
void crx_mysqlnd_scramble(crex_uchar * const buffer, const crex_uchar * const scramble, const crex_uchar * const password, const size_t password_len)
{
	CRX_SHA1_CTX context;
	crex_uchar sha1[SHA1_MAX_LENGTH];
	crex_uchar sha2[SHA1_MAX_LENGTH];

	/* Phase 1: hash password */
	CRX_SHA1Init(&context);
	CRX_SHA1Update(&context, password, password_len);
	CRX_SHA1Final(sha1, &context);

	/* Phase 2: hash sha1 */
	CRX_SHA1Init(&context);
	CRX_SHA1Update(&context, (crex_uchar*)sha1, SHA1_MAX_LENGTH);
	CRX_SHA1Final(sha2, &context);

	/* Phase 3: hash scramble + sha2 */
	CRX_SHA1Init(&context);
	CRX_SHA1Update(&context, scramble, SCRAMBLE_LENGTH);
	CRX_SHA1Update(&context, (crex_uchar*)sha2, SHA1_MAX_LENGTH);
	CRX_SHA1Final(buffer, &context);

	/* let's crypt buffer now */
	crx_mysqlnd_crypt(buffer, (const crex_uchar *)buffer, (const crex_uchar *)sha1, SHA1_MAX_LENGTH);
}
/* }}} */
...
mysqlnd_native_auth_get_auth_data(struct st_mysqlnd_authentication_plugin * self,
								  ...
								  const size_t passwd_len, crex_uchar * auth_plugin_data, const size_t auth_plugin_data_len,
								  const MYSQLND_SESSION_OPTIONS * const session_options,
	...
	*auth_data_len = 0;
	...
	if (passwd && passwd_len) {
		...
		*auth_data_len = SCRAMBLE_LENGTH;
		...
		crx_mysqlnd_scramble((crex_uchar*)ret, auth_plugin_data, (crex_uchar*)passwd, passwd_len);
	}
	DBG_RETURN(ret);
}
/* }}} */
...
{
	{
		...
		CRX_MYSQLND_VERSION,
		"CRX License 3.01",
		"Andrey Hristov <andrey@crx.net>,  Ulf Wendel <uwendel@mysql.com>, Georg Richter <georg@mysql.com>",
		{
							  ...
							  )
{
	...
	/* copy pass*/
	if (passwd && passwd_len) {
		ret = (crex_uchar*) crex_strndup(passwd, passwd_len);
	}
	/*
...
{
	{
		...
		CRX_MYSQLND_VERSION,
		"CRX License 3.01",
		"Andrey Hristov <andrey@crx.net>,  Ulf Wendel <uw@crx.net>, Georg Richter <georg@crx.net>",
		{
			NULL, /* no statistics , will be filled later if there are some */
			NULL, /* no statistics */
		},
...
mysqlnd_sha256_public_encrypt(MYSQLND_CONN_DATA * conn, mysqlnd_rsa_t server_public_key, size_t passwd_len, size_t * auth_data_len, char *xor_str)
{
	...
	*auth_data_len = server_public_key_len;
	...
	if (!ctx || EVP_PKEY_encrypt_init(ctx) <= 0 ||
			...
			EVP_PKEY_encrypt(ctx, ret, &server_public_key_len, (crex_uchar *) xor_str, passwd_len + 1) <= 0) {
	...
	if (!fname || fname[0] == '\0') {
		...
		do {
			...
			if (! PACKET_WRITE(conn, &pk_req_packet)) {
				...
				crx_error(E_WARNING, "Error while sending public key request packet. PID=%d", getpid());
				SET_CONNECTION_STATE(&conn->state, CONN_QUIT_SENT);
				break;
			}
			if (FAIL == PACKET_READ(conn, &pk_resp_packet) || NULL == pk_resp_packet.public_key) {
				DBG_ERR_FMT("Error while receiving public key");
				crx_error(E_WARNING, "Error while receiving public key. PID=%d", getpid());
				SET_CONNECTION_STATE(&conn->state, CONN_QUIT_SENT);
				break;
			}
		...
		stream = crx_stream_open_wrapper((char *) fname, "rb", REPORT_ERRORS, NULL);

		if (stream) {
			if ((key_str = crx_stream_copy_to_mem(stream, CRX_STREAM_COPY_ALL, 0)) != NULL) {
				...
				crex_string_release_ex(key_str, 0);
			}
	...
	if (conn->vio->data->ssl) {
		...
		*auth_data_len = 0;
		...
		if (server_public_key) {
			 ...
			 * In a 5.5+ server we might get additional scramble data in crx_mysqlnd_greet_read, not used by this authentication method. */
...
{
	{
		...
		CRX_MYSQLND_VERSION,
		"CRX License 3.01",
		"Andrey Hristov <andrey@crx.net>,  Ulf Wendel <uwendel@mysql.com>",
		{
			NULL, /* no statistics , will be filled later if there are some */
			NULL, /* no statistics */
		},
...
/* {{{ crx_mysqlnd_scramble_sha2 */
void crx_mysqlnd_scramble_sha2(crex_uchar * const buffer, const crex_uchar * const scramble, const crex_uchar * const password, const size_t password_len)
{
	CRX_SHA256_CTX context;
	crex_uchar sha1[SHA256_LENGTH];
	crex_uchar sha2[SHA256_LENGTH];

	/* Phase 1: hash password */
	CRX_SHA256Init(&context);
	CRX_SHA256Update(&context, password, password_len);
	CRX_SHA256Final(sha1, &context);

	/* Phase 2: hash sha1 */
	CRX_SHA256Init(&context);
	CRX_SHA256Update(&context, (crex_uchar*)sha1, SHA256_LENGTH);
	CRX_SHA256Final(sha2, &context);

	/* Phase 3: hash scramble + sha2 */
	CRX_SHA256Init(&context);
	CRX_SHA256Update(&context, (crex_uchar*)sha2, SHA256_LENGTH);
	CRX_SHA256Update(&context, scramble, SCRAMBLE_LENGTH);
	CRX_SHA256Final(buffer, &context);

	/* let's crypt buffer now */
	crx_mysqlnd_crypt(buffer, (const crex_uchar *)sha1, (const crex_uchar *)buffer, SHA256_LENGTH);
}
/* }}} */
...
mysqlnd_caching_sha2_public_encrypt(MYSQLND_CONN_DATA * conn, mysqlnd_rsa_t server_public_key, size_t passwd_len, unsigned char **crypted, char *xor_str)
{
	...
	*crypted = emalloc(server_public_key_len);
	...
	if (!ctx || EVP_PKEY_encrypt_init(ctx) <= 0 ||
			EVP_PKEY_CTX_set_rsa_padding(ctx, RSA_PKCS1_OAEP_PADDING) <= 0 ||
			EVP_PKEY_encrypt(ctx, *crypted, &server_public_key_len, (crex_uchar *) xor_str, passwd_len + 1) <= 0) {
		DBG_ERR("encrypt failed");
	...
	*auth_data_len = 0;
	...
	if (passwd && passwd_len) {
		...
		*auth_data_len = SHA256_LENGTH;
		crx_mysqlnd_scramble_sha2((crex_uchar*)ret, auth_plugin_data, (crex_uchar*)passwd, passwd_len);
		ret[SHA256_LENGTH] = '\0';
...
mysqlnd_caching_sha2_get_and_use_key(MYSQLND_CONN_DATA *conn,
		const crex_uchar * auth_plugin_data, const size_t auth_plugin_data_len,
	...
	switch (result_packet.response_code) {
		...
		case 4:
			if (is_secure_transport(conn)) {
				...
				result_packet.password = (crex_uchar *)passwd;
...
{
	{
		...
		CRX_MYSQLND_VERSION,
		"CRX License 3.01",
		"Johannes Schlüter <johannes.schlueter@crx.net>",
		{
			NULL, /* no statistics , will be filled later if there are some */
			NULL, /* no statistics */
		},
