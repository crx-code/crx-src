...
static ssize_t write_compressed_packet(
		...
		crex_uchar *uncompressed_payload, size_t to_be_sent, crex_uchar *compress_buf) {
	...
	if (res == C_OK) {
		...
		crex_uchar * decompressed_data = mnd_emalloc(decompressed_size);
...
*/
static size_t
MYSQLND_METHOD(mysqlnd_pfc, send)(MYSQLND_PFC * const pfc, MYSQLND_VIO * const vio, crex_uchar * const buffer, const size_t count,
								  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	crex_uchar safe_buf[((MYSQLND_HEADER_SIZE) + (sizeof(crex_uchar)) - 1) / (sizeof(crex_uchar))];
	crex_uchar * safe_storage = safe_buf;
	size_t packets_sent = 1;
	size_t left = count;
	crex_uchar * p = (crex_uchar *) buffer;
	crex_uchar * compress_buf = NULL;
	size_t to_be_sent;
	ssize_t bytes_sent;
...
#ifdef MYSQLND_COMPRESSION_ENABLED
		if (pfc->data->compressed == TRUE) {
			crex_uchar * uncompressed_payload = p; /* should include the header */
		...
		(MYSQLND_PFC * pfc, MYSQLND_VIO * vio, size_t net_payload_size, MYSQLND_STATS * conn_stats, MYSQLND_ERROR_INFO * error_info)
{
	...
	crex_uchar * compressed_data = NULL;
	crex_uchar comp_header[COMPRESSED_HEADER_SIZE];
...
/* {{{ mysqlnd_pfc::decode */
static enum_func_status
MYSQLND_METHOD(mysqlnd_pfc, decode)(crex_uchar * uncompressed_data, const size_t uncompressed_data_len,
									const crex_uchar * const compressed_data, const size_t compressed_data_len)
{
#ifdef MYSQLND_COMPRESSION_ENABLED
	int error;
	...
	if (error != C_OK) {
		DBG_INF_FMT("decompression NOT successful. error=%d C_OK=%d C_BUF_ERROR=%d C_MEM_ERROR=%d", error, C_OK, C_BUF_ERROR, C_MEM_ERROR);
	}
	DBG_RETURN(error == C_OK? PASS:FAIL);
#else
	DBG_ENTER("mysqlnd_pfc::decode");
	DBG_RETURN(FAIL);
#endif
...
/* {{{ mysqlnd_pfc::encode */
static enum_func_status
MYSQLND_METHOD(mysqlnd_pfc, encode)(crex_uchar * compress_buffer, size_t * compress_buffer_len,
									const crex_uchar * const uncompressed_data, const size_t uncompressed_data_len)
{
#ifdef MYSQLND_COMPRESSION_ENABLED
	int error;
	...
	if (error != C_OK) {
		DBG_INF_FMT("compression NOT successful. error=%d C_OK=%d C_BUF_ERROR=%d C_MEM_ERROR=%d", error, C_OK, C_BUF_ERROR, C_MEM_ERROR);
	} else {
...
/* {{{ mysqlnd_pfc::receive */
static enum_func_status
MYSQLND_METHOD(mysqlnd_pfc, receive)(MYSQLND_PFC * const pfc, MYSQLND_VIO * const vio, crex_uchar * const buffer, const size_t count,
									 MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	if (pfc->data->compressed) {
		if (pfc->data->uncompressed_data) {
			...
			if (to_read_from_buffer) {
				pfc->data->uncompressed_data->read(pfc->data->uncompressed_data, to_read_from_buffer, (crex_uchar *) p);
		...
		if (to_read) {
			crex_uchar net_header[MYSQLND_HEADER_SIZE];
			...
			DBG_INF_FMT("HEADER: hwd_packet_no=%u size=%3u", packet_no, (crex_ulong) net_payload_size);
#endif
