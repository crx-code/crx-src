...
static unsigned int check_mb_utf8mb3_sequence(const char * const start, const char * const end)
{
	...
	if (c < 0xE0) {
		...
		if (!(((crex_uchar)start[1] ^ 0x80) < 0x40)) {
			return 0;
		}
		return 2;
	}
	if (c < 0xF0) {
		...
		if (!(((crex_uchar)start[1] ^ 0x80) < 0x40 && ((crex_uchar)start[2] ^ 0x80) < 0x40 &&
			(c >= 0xE1 || (crex_uchar)start[1] >= 0xA0))) {
...
static unsigned int check_mb_utf8_sequence(const char * const start, const char * const end)
{
	...
	if (c < 0xF5) {
		...
		*/
		...
		if (!(((crex_uchar)start[1] ^ 0x80) < 0x40 &&
			((crex_uchar)start[2] ^ 0x80) < 0x40 &&
			((crex_uchar)start[3] ^ 0x80) < 0x40 &&
				(c >= 0xf1 || (crex_uchar)start[1] >= 0x90) &&
				(c <= 0xf3 || (crex_uchar)start[1] <= 0x8F)))
		{
			return 0;	/* invalid utf8 character */
		}
		return 4;
...
/* {{{ big5 functions */
#define valid_big5head(c)	(0xA1 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0xF9)
#define valid_big5tail(c)	((0x40 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0x7E) || \
							(0xA1 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0xFE))
...
{
	return (valid_cp932head((crex_uchar)start[0]) && (end - start >  1) &&
			valid_cp932tail((crex_uchar)start[1])) ? 2 : 0;
}


static unsigned int mysqlnd_mbcharlen_cp932(const unsigned int cp932)
{
	return (valid_cp932head((crex_uchar)cp932)) ? 2 : 1;
}
/* }}} */


/* {{{ euckr functions */
#define valid_euckr(c)	((0xA1 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0xFE))
...
/* {{{ gb2312 functions */
#define valid_gb2312_head(c)	(0xA1 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0xF7)
#define valid_gb2312_tail(c)	(0xA1 <= (crex_uchar)(c) && (crex_uchar)(c) <= 0xFE)
...
/* {{{ gbk functions */
#define valid_gbk_head(c)	(0x81<=(crex_uchar)(c) && (crex_uchar)(c)<=0xFE)
#define valid_gbk_tail(c)	((0x40<=(crex_uchar)(c) && (crex_uchar)(c)<=0x7E) || (0x80<=(crex_uchar)(c) && (crex_uchar)(c)<=0xFE))
...
{
	return (valid_sjis_head((crex_uchar)start[0]) && (end - start) > 1 && valid_sjis_tail((crex_uchar)start[1])) ? 2 : 0;
}


static unsigned int mysqlnd_mbcharlen_sjis(const unsigned int sjis)
{
	return (valid_sjis_head((crex_uchar)sjis)) ? 2 : 1;
}
/* }}} */
...
/* {{{ utf16 functions */
#define UTF16_HIGH_HEAD(x)  ((((crex_uchar) (x)) & 0xFC) == 0xD8)
#define UTF16_LOW_HEAD(x)   ((((crex_uchar) (x)) & 0xFC) == 0xDC)
...
/* {{{ gb18030 functions */
#define is_gb18030_odd(c)          (0x81 <= (crex_uchar) (c) && (crex_uchar) (c) <= 0xFE)
#define is_gb18030_even_2(c)       ((0x40 <= (crex_uchar) (c) && (crex_uchar) (c) <= 0x7E) || (0x80 <= (crex_uchar) (c) && (crex_uchar) (c) <= 0xFE))
#define is_gb18030_even_4(c)       (0x30 <= (crex_uchar) (c) && (crex_uchar) (c) <= 0x39)
...
CRXAPI const MYSQLND_CHARSET * mysqlnd_find_charset_nr(const unsigned int charsetnr)
{
...
CRXAPI const MYSQLND_CHARSET * mysqlnd_find_charset_name(const char * const name)
{
	if (name) {
		const MYSQLND_CHARSET * c = mysqlnd_charsets;
		do {
...
CRXAPI crex_ulong mysqlnd_cset_escape_quotes(const MYSQLND_CHARSET * const cset, char * newstr,
											 const char * escapestr, const size_t escapestr_len)
{
	...
	*newstr = '\0';
	...
	if (escape_overflow) {
		DBG_RETURN((crex_ulong)~0);
	}
	DBG_RETURN((crex_ulong)(newstr - newstr_s));
}
/* }}} */
