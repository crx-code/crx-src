...
enum_func_status mysqlnd_stmt_execute_generate_request(MYSQLND_STMT * const s, crex_uchar ** request, size_t *request_len, bool * free_buffer);
enum_func_status mysqlnd_stmt_execute_batch_generate_request(MYSQLND_STMT * const s, crex_uchar ** request, size_t *request_len, bool * free_buffer);
		...
		const MYSQLND_STMT_DATA *stmt, unsigned max_rows)
{
	...
	crex_uchar buf[MYSQLND_STMT_ID_LENGTH /* statement id */ + 4 /* number of rows to fetch */];
...
MYSQLND_METHOD(mysqlnd_stmt, store_result)(MYSQLND_STMT * const s)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT, stmt->stmt_id);
...
/*	result->m.row_decoder = crx_mysqlnd_rowp_read_binary_protocol; */
...
MYSQLND_METHOD(mysqlnd_stmt, send_execute)(MYSQLND_STMT * const s, const enum_mysqlnd_send_execute_type type, zval * read_cb, zval * err_cb)
{
	...
	if (stmt->param_count) {
		...
		for (i = 0; i < stmt->param_count; i++) {
			if (C_ISUNDEF(stmt->param_bind[i].zv)) {
				not_bound++;
			}
		}
		if (not_bound) {
...
mysqlnd_fetch_stmt_row_cursor(MYSQLND_RES * result, zval **row_ptr, const unsigned int flags, bool * fetched_anything)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " flags=%u", stmt->stmt_id, flags);
		...
		if (ret == PASS && *fetched_anything) {
			...
			for (unsigned i = 0; i < field_count; i++) {
				...
				if (stmt->result_bind[i].bound == TRUE) {
					DBG_INF_FMT("i=%u type=%u", i, C_TYPE(row_data[i]));
					CREX_TRY_ASSIGN_VALUE_EX(resultzv, &row_data[i], 0);
				} else {
...
MYSQLND_METHOD(mysqlnd_stmt, send_long_data)(MYSQLND_STMT * const s, unsigned int param_no,
							 				 const char * const data, crex_ulong data_length)
{
	enum_func_status ret = FAIL;
	MYSQLND_STMT_DATA * stmt = s? s->data : NULL;
	MYSQLND_CONN_DATA * conn = stmt? stmt->conn : NULL;
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " param_no=%u data_len=" CREX_ULONG_FMT, stmt->stmt_id, param_no, data_length);
...
MYSQLND_METHOD(mysqlnd_stmt, bind_parameters)(MYSQLND_STMT * const s, MYSQLND_PARAM_BIND * const param_bind)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " param_count=%u", stmt->stmt_id, stmt->param_count);
	...
	if (stmt->param_count) {
		...
		for (i = 0; i < stmt->param_count; i++) {
			...
			C_TRY_ADDREF(stmt->param_bind[i].zv);
												 ...
												 zval * const zv, crex_uchar type)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " param_no=%u param_count=%u type=%u", stmt->stmt_id, param_no, stmt->param_count, type);
										  ...
										  MYSQLND_RESULT_BIND * const result_bind)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " field_count=%u", stmt->stmt_id, stmt->field_count);
	...
	if (stmt->field_count) {
		...
		for (i = 0; i < stmt->field_count; i++) {
			/* Prevent from freeing */
			C_TRY_ADDREF(stmt->result_bind[i].zv);

			DBG_INF_FMT("ref of %p = %u", &stmt->result_bind[i].zv,
					C_REFCOUNTED(stmt->result_bind[i].zv)? C_REFCOUNT(stmt->result_bind[i].zv) : 0);
			/*
			  Don't update is_ref !!! it's not our job
			  Otherwise either 009.crxt or mysqli_stmt_bind_result.crxt
			  will fail.
			*/
									   ...
									   const void * const value)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " attr_type=%u", stmt->stmt_id, attr_type);

	switch (attr_type) {
		case STMT_ATTR_UPDATE_MAX_LENGTH:{
			crex_uchar bval = *(crex_uchar *) value;
			/*
			  XXX : libmysql uses my_bool, but mysqli uses ulong as storage on the stack
			  and mysqlnd won't be used out of the scope of CRX -> use ulong.
			*/
...
mysqlnd_stmt_separate_result_bind(MYSQLND_STMT * const s)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " result_bind=%p field_count=%u", stmt->stmt_id, stmt->result_bind, stmt->field_count);
	...
	*/
	for (i = 0; i < stmt->field_count; i++) {
		...
		if (stmt->result_bind[i].bound == TRUE) {
			DBG_INF_FMT("%u has refcount=%u", i, C_REFCOUNTED(stmt->result_bind[i].zv)? C_REFCOUNT(stmt->result_bind[i].zv) : 0);
			zval_ptr_dtor(&stmt->result_bind[i].zv);
		}
	}
...
MYSQLND_METHOD(mysqlnd_stmt, free_stmt_result)(MYSQLND_STMT * const s)
{
	...
	*/
	...
	crex_llist_clean(&stmt->error_info->error_list);
...
MYSQLND_METHOD(mysqlnd_stmt, free_stmt_content)(MYSQLND_STMT * const s)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT " param_bind=%p param_count=%u", stmt->stmt_id, stmt->param_bind, stmt->param_count);
