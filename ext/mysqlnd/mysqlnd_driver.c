...
{
	const size_t pfc_alloc_size = CREX_MM_ALIGNED_SIZE(sizeof(MYSQLND_PFC) + mysqlnd_plugin_count() * sizeof(void *));
...
{
	const size_t vio_alloc_size = CREX_MM_ALIGNED_SIZE(sizeof(MYSQLND_VIO) + mysqlnd_plugin_count() * sizeof(void *));
...
CRXAPI MYSQLND_CLASS_METHODS_START(mysqlnd_object_factory)
