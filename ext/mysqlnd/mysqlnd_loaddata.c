...
int mysqlnd_local_infile_init(void ** ptr, const char * const filename)
{
	...
	*ptr = info;
	...
	/* check open_basedir */
	if (PG(open_basedir)) {
		if (crx_check_open_basedir_ex(filename, 0) == -1) {
	...
	info->fd = crx_stream_open_wrapper_ex((char *)filename, "r", 0, NULL, context);
...
static
int mysqlnd_local_infile_read(void * ptr, crex_uchar * buf, unsigned int buf_len)
{
	...
	count = (int) crx_stream_read(info->fd, (char *) buf, buf_len);
...
void mysqlnd_local_infile_end(void * ptr)
{
	...
	if (info) {
		/* crx_stream_close segfaults on NULL */
		if (info->fd) {
			crx_stream_close(info->fd);
			info->fd = NULL;
		}
		mnd_efree(info);
	}
...
{
	crex_uchar			*buf = NULL;
	crex_uchar			empty_packet[MYSQLND_HEADER_SIZE];
	...
	*/
	if (is_local_infile_dir_set) {
		crx_stream *stream = crx_stream_opendir(local_infile_directory, REPORT_ERRORS, NULL);
		if (stream) {
	...
	*/
	if (prerequisities_ok && !is_local_infile_enabled && is_local_infile_dir_set) {
		if (crx_check_specific_open_basedir(local_infile_directory, filename) == -1) {
			SET_CLIENT_ERROR(conn->error_info, CR_LOAD_DATA_LOCAL_INFILE_REJECTED, UNKNOWN_SQLSTATE,
	...
	buf = (crex_uchar *) mnd_ecalloc(1, buflen);
