...
MYSQLND_METHOD(mysqlnd_res, free_result_buffers)(MYSQLND_RES * result)
{
	...
	if (result->meta) {
		CREX_ASSERT(crex_arena_contains(result->memory_pool->arena, result->meta));
...
mysqlnd_query_read_result_set_header(MYSQLND_CONN_DATA * conn, MYSQLND_STMT * s)
{
	...
	DBG_INF_FMT("stmt=" CREX_ULONG_FMT, stmt? stmt->stmt_id:0);
	...
	do {
		...
		if (FAIL == (ret = PACKET_READ(conn, &rset_header))) {
			if (conn->error_info->error_no != CR_SERVER_GONE_ERROR) {
				crx_error_docref(NULL, E_WARNING, "Error reading result set's header");
			}
			break;
		}
...
/*
  Do lazy initialization for buffered results. As CRX strings have
  length inside, this function makes not much sense in the context
  of CRX, to be called as separate function. But let's have it for
  completeness.
*/
static const size_t *
			...
			*row_ptr = result->row_data;
			...
			if (lengths) {
				for (unsigned i = 0; i < field_count; i++) {
					...
					lengths[i] = C_TYPE_P(data) == IS_STRING ? C_STRLEN_P(data) : 0;
				}
			}
		}
		set->row_count++;
			...
			*row_ptr = result->row_data;
			...
			if (set->lengths) {
				for (unsigned i = 0; i < field_count; ++i) {
					...
					set->lengths[i] = C_TYPE_P(data) == IS_STRING ? C_STRLEN_P(data) : 0;
				}
			}
		}
										...
										zval *return_value CREX_FILE_LINE_DC)
{
	...
	HashTable *row_ht = C_ARRVAL_P(return_value);
	MYSQLND_FIELD *field = meta->fields;
	for (unsigned i = 0; i < meta->field_count; i++, field++) {
		zval *data = &row_data[i];

		if (flags & MYSQLND_FETCH_NUM) {
			if (crex_hash_index_add(row_ht, i, data) != NULL) {
				C_TRY_ADDREF_P(data);
			}
		}
		if (flags & MYSQLND_FETCH_ASSOC) {
			/* crex_hash_quick_update needs length + trailing zero */
			/* QQ: Error handling ? */
			/*
			  crex_hash_quick_update does not check, as add_assoc_zval_ex do, whether
			  the index is a numeric and convert it to it. This however means constant
			  hashing of the column name, which is not needed as it can be precomputed.
			*/
			C_TRY_ADDREF_P(data);
			if (meta->fields[i].is_numeric == FALSE) {
				crex_hash_update(row_ht, meta->fields[i].sname, data);
			} else {
				crex_hash_index_update(row_ht, meta->fields[i].num_key, data);
			}
		}
...
mysqlnd_result_unbuffered_init(MYSQLND_RES *result, const unsigned int field_count, MYSQLND_STMT_DATA *stmt)
{
	...
	if (stmt) {
		ret->m.row_decoder = crx_mysqlnd_rowp_read_binary_protocol;
		ret->m.fetch_lengths = NULL; /* makes no sense */
		ret->lengths = NULL;
	} else {
		ret->m.row_decoder = crx_mysqlnd_rowp_read_text_protocol;
