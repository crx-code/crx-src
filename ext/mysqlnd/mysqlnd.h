...
#define CRX_MYSQLND_VERSION "mysqlnd " CRX_VERSION
#define MYSQLND_VERSION_ID CRX_VERSION_ID
...
/* Library related */
CRXAPI void mysqlnd_library_init(void);
CRXAPI void mysqlnd_library_end(void);

CRXAPI unsigned int mysqlnd_plugin_register(void);
CRXAPI unsigned int mysqlnd_plugin_register_ex(struct st_mysqlnd_plugin_header * plugin);
CRXAPI unsigned int mysqlnd_plugin_count(void);
CRXAPI void * mysqlnd_plugin_find(const char * const name);

CRXAPI void mysqlnd_plugin_apply_with_argument(apply_func_arg_t apply_func, void * argument);
...
CRXAPI void mysqlnd_minfo_print_hash(zval *values);
#define mysqlnd_thread_safe()	true

CRXAPI const MYSQLND_CHARSET * mysqlnd_find_charset_nr(const unsigned int charsetno);
CRXAPI const MYSQLND_CHARSET * mysqlnd_find_charset_name(const char * const charsetname);
...
CRXAPI MYSQLND * mysqlnd_connection_init(const size_t client_flags, const bool persistent, MYSQLND_CLASS_METHODS_TYPE(mysqlnd_object_factory) *object_factory);
CRXAPI MYSQLND * mysqlnd_connection_connect(MYSQLND * conn,
...
/* Query */
#define mysqlnd_fetch_into(result, flags, ret_val)	(result)->m.fetch_into((result), (flags), (ret_val) CREX_FILE_LINE_CC)
...
#define mysqlnd_get_connection_stats(conn, values)		((conn)->data)->m->get_statistics((conn)->data,  (values) CREX_FILE_LINE_CC)
#define mysqlnd_get_client_stats(values)				_mysqlnd_get_client_stats(mysqlnd_global_stats, (values) CREX_FILE_LINE_CC)
...
CRXAPI enum_func_status mysqlnd_poll(MYSQLND **r_array, MYSQLND **e_array, MYSQLND ***dont_poll, long sec, long usec, int * desc_num);
...
CRXAPI void mysqlnd_efree_param_bind_dtor(MYSQLND_PARAM_BIND * param_bind);
CRXAPI void mysqlnd_efree_result_bind_dtor(MYSQLND_RESULT_BIND * result_bind);
CRXAPI void mysqlnd_free_param_bind_dtor(MYSQLND_PARAM_BIND * param_bind);
CRXAPI void mysqlnd_free_result_bind_dtor(MYSQLND_RESULT_BIND * result_bind);


/* LOAD DATA LOCAL */
CRXAPI void mysqlnd_local_infile_default(MYSQLND_CONN_DATA * conn);
...
CRXAPI crex_ulong mysqlnd_old_escape_string(char * newstr, const char * escapestr, size_t escapestr_len);
...
CRXAPI extern MYSQLND_STATS * mysqlnd_global_stats;
CRXAPI extern const MYSQLND_STRING mysqlnd_stats_values_names[];
CRXAPI void			_mysqlnd_get_client_stats(MYSQLND_STATS * stats, zval *return_value CREX_FILE_LINE_DC);
...
CREX_BEGIN_MODULE_GLOBALS(mysqlnd)
	...
	crex_long		net_cmd_buffer_size;
	crex_long		net_read_buffer_size;
	crex_long		log_mask;
	crex_long		net_read_timeout;
	crex_long		mempool_default_size;
	char *			sha256_server_public_key;
	bool		collect_statistics;
	bool		collect_memory_statistics;
CREX_END_MODULE_GLOBALS(mysqlnd)

CRXAPI CREX_EXTERN_MODULE_GLOBALS(mysqlnd)
#define MYSQLND_G(v) CREX_MODULE_GLOBALS_ACCESSOR(mysqlnd, v)
