...
static int
mysqlnd_set_sock_no_delay(crx_stream * stream)
{
	int socketd = ((crx_netstream_data_t*)stream->abstract)->socket;
	int ret = SUCCESS;
	int flag = 1;
...
static int
mysqlnd_set_sock_keepalive(crx_stream * stream)
{
	int socketd = ((crx_netstream_data_t*)stream->abstract)->socket;
	int ret = SUCCESS;
	int flag = 1;
...
MYSQLND_METHOD(mysqlnd_vio, network_read)(MYSQLND_VIO * const vio, crex_uchar * const buffer, const size_t count,
										  MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info)
{
	enum_func_status return_value = PASS;
	crx_stream * net_stream = vio->data->m.get_stream(vio);
	...
	while (to_read) {
		ssize_t ret = crx_stream_read(net_stream, (char *) p, to_read);
		if (ret <= 0) {
...
static ssize_t
MYSQLND_METHOD(mysqlnd_vio, network_write)(MYSQLND_VIO * const vio, const crex_uchar * const buffer, const size_t count,
										   MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info)
{
	ssize_t ret;
	DBG_ENTER("mysqlnd_vio::network_write");
	DBG_INF_FMT("sending %zu bytes", count);
	ret = crx_stream_write(vio->data->m.get_stream(vio), (char *)buffer, count);
	DBG_RETURN(ret);
}
/* }}} */
									   ...
									   MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	net_stream = crx_stream_open_wrapper(scheme.s + sizeof("pipe://") - 1, "r+", streams_options, NULL);
	if (!net_stream) {
	...
	*/
	...
	crex_hash_index_del(&EG(regular_list), net_stream->res->handle); /* ToDO: should it be res->handle, do streams register with addref ?*/
											  ...
											  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	DBG_INF_FMT("calling crx_stream_xport_create");
	net_stream = crx_stream_xport_create(scheme.s, scheme.l, streams_options, streams_flags,
	...
	if (hashed_details) {
		...
		*/
		...
		if ((le = crex_hash_str_find_ptr(&EG(persistent_list), hashed_details, hashed_details_len))) {
			...
			*/
			...
			crex_hash_str_del(&EG(persistent_list), hashed_details, hashed_details_len);
												  ...
												  MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	if (net_stream) {
		if (vio->data->options.timeout_read) {
			struct timeval tv;
			DBG_INF_FMT("setting %u as CRX_STREAM_OPTION_READ_TIMEOUT", vio->data->options.timeout_read);
			tv.tv_sec = vio->data->options.timeout_read;
			tv.tv_usec = 0;
			crx_stream_set_option(net_stream, CRX_STREAM_OPTION_READ_TIMEOUT, 0, &tv);
		}
		...
		net_stream->flags |= CRX_STREAM_FLAG_SUPPRESS_ERRORS;
	}
									 ...
									 MYSQLND_STATS * const conn_stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	if (open_stream) {
		crx_stream * net_stream = open_stream(vio, scheme, persistent, conn_stats, error_info);
...
size_t
MYSQLND_METHOD(mysqlnd_vio, consume_uneaten_data)(MYSQLND_VIO * const net, enum crx_mysqlnd_server_command cmd)
{
	...
	*/
	...
	int opt = CRX_STREAM_OPTION_BLOCKING;
	crx_stream * net_stream = net->data->get_stream(net);
	int was_blocked = net_stream->ops->set_option(net_stream, opt, 0, NULL);

	DBG_ENTER("mysqlnd_vio::consume_uneaten_data");

	if (CRX_STREAM_OPTION_RETURN_ERR != was_blocked) {
		/* Do a read of 1 byte */
		ssize_t bytes_consumed;

		do {
			bytes_consumed = crx_stream_read(net_stream, tmp_buf, sizeof(tmp_buf));
			if (bytes_consumed <= 0) {
				break;
			}
		...
		if (bytes_consumed) {
			...
			crx_error_docref(NULL, E_WARNING, "Skipped %u bytes. Last command %s hasn't "
...
MYSQLND_METHOD(mysqlnd_vio, enable_ssl)(MYSQLND_VIO * const net)
{
	...
	crx_stream_context * context = crx_stream_context_alloc();
	crx_stream * net_stream = net->data->m.get_stream(net);
	bool any_flag = FALSE;
	...
	if (net->data->options.ssl_key) {
		...
		crx_stream_context_set_option(context, "ssl", "local_pk", &key_zval);
	...
	if (net->data->options.ssl_cert) {
		...
		crx_stream_context_set_option(context, "ssl", "local_cert", &cert_zval);
		if (!net->data->options.ssl_key) {
			crx_stream_context_set_option(context, "ssl", "local_pk", &cert_zval);
		}
	...
	if (net->data->options.ssl_ca) {
		...
		crx_stream_context_set_option(context, "ssl", "cafile", &cafile_zval);
	...
	if (net->data->options.ssl_capath) {
		...
		crx_stream_context_set_option(context, "ssl", "capath", &capath_zval);
	...
	if (net->data->options.ssl_passphrase) {
		...
		crx_stream_context_set_option(context, "ssl", "passphrase", &passphrase_zval);
	...
	if (net->data->options.ssl_cipher) {
		...
		crx_stream_context_set_option(context, "ssl", "ciphers", &cipher_zval);
	...
	}
	{
		...
		crx_stream_context_set_option(context, "ssl", "verify_peer", &verify_peer_zval);
		crx_stream_context_set_option(context, "ssl", "verify_peer_name", &verify_peer_zval);
		if (net->data->options.ssl_verify_peer == MYSQLND_SSL_PEER_DONT_VERIFY) {
			ZVAL_TRUE(&verify_peer_zval);
			crx_stream_context_set_option(context, "ssl", "allow_self_signed", &verify_peer_zval);
		}
	}
	crx_stream_context_set(net_stream, context);
	/* crx_stream_context_set() increases the refcount of context, but we just want to transfer ownership
	 * hence the need to decrease the refcount so the refcount will be equal to 1. */
	CREX_ASSERT(GC_REFCOUNT(context->res) == 2);
	GC_DELREF(context->res);
	if (crx_stream_xport_crypto_setup(net_stream, STREAM_CRYPTO_METHOD_TLS_CLIENT, NULL) < 0 ||
	    crx_stream_xport_crypto_enable(net_stream, 1) < 0)
	{
	...
	*/
	crx_stream_context_set(net_stream, NULL);

	if (net->data->options.timeout_read) {
		struct timeval tv;
		DBG_INF_FMT("setting %u as CRX_STREAM_OPTION_READ_TIMEOUT", net->data->options.timeout_read);
...
MYSQLND_METHOD(mysqlnd_vio, close_stream)(MYSQLND_VIO * const net, MYSQLND_STATS * const stats, MYSQLND_ERROR_INFO * const error_info)
{
	...
	if (net && (net_stream = net->data->m.get_stream(net))) {
		...
		if (pers && EG(active)) {
			crx_stream_free(net_stream, CRX_STREAM_FREE_CLOSE_PERSISTENT | CRX_STREAM_FREE_RSRC_DTOR);
		} else {
			/*
			  otherwise we will crash because the EG(persistent_list) has been freed already,
			  before the modules are shut down
			*/
			crx_stream_free(net_stream, CRX_STREAM_FREE_CLOSE | CRX_STREAM_FREE_RSRC_DTOR);
		}
...
static enum_func_status
MYSQLND_METHOD(mysqlnd_vio, set_stream)(MYSQLND_VIO * const vio, crx_stream * net_stream)
{
