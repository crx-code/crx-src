...
struct st_mysqlnd_debug_methods
{
	...
	enum_func_status (*log_va)(
		...
		unsigned int level, const char * type, const char *format, ...)
			CREX_ATTRIBUTE_FORMAT(printf, 6, 7);
...
struct st_mysqlnd_debug
{
	...
	int pid;
	char * file_name;
	crex_stack call_stack;
	crex_stack call_time_stack;
...
CRXAPI MYSQLND_DEBUG * mysqlnd_debug_init(const char * skip_functions[]);
...
#else  /* defined(__GNUC__) || defined(CRX_WIN32) */
