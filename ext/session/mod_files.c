 ...
 *  - Refer to PS_* macros in crx_session.h for function/parameter definitions.
...
static char *ps_files_path_create(char *buf, size_t buflen, ps_files *data, const crex_string *key)
{
	const char *p;
	int i;
	size_t n;
...
static void ps_files_open(ps_files *data, /* const */ crex_string *key)
{
	char buf[MAXPATHLEN];
#if !defined(O_NOFOLLOW) || !defined(CRX_WIN32)
    struct stat sbuf = {0};
#endif
	int ret;

	if (data->fd < 0 || !data->last_key || !crex_string_equals(key, data->last_key)) {
		if (data->last_key) {
			crex_string_release_ex(data->last_key, /* persistent */ false);
			data->last_key = NULL;
		}

		ps_files_close(data);

		if (crx_session_valid_key(ZSTR_VAL(key)) == FAILURE) {
			crx_error_docref(NULL, E_WARNING, "Session ID is too long or contains illegal characters. Only the A-Z, a-z, 0-9, \"-\", and \",\" characters are allowed");
			return;
		}

		if (!ps_files_path_create(buf, sizeof(buf), data, key)) {
			crx_error_docref(NULL, E_WARNING, "Failed to create session data file path. Too short session ID, invalid save_path or path length exceeds %d characters", MAXPATHLEN);
			return;
		}

		data->last_key = crex_string_copy(key);
		...
		if(PG(open_basedir) && lstat(buf, &sbuf) == 0 && S_ISLNK(sbuf.st_mode) && crx_check_open_basedir(buf)) {
			return;
		}
		...
		if (data->fd != -1) {
			...
			/* check that this session file was created by us or root – we
			   ...
			   */
			if (crex_fstat(data->fd, &sbuf) || (sbuf.st_uid != 0 && sbuf.st_uid != getuid() && sbuf.st_uid != geteuid() && getuid() != 0)) {
				close(data->fd);
				data->fd = -1;
				crx_error_docref(NULL, E_WARNING, "Session data file is not created by your uid");
				return;
			}
#endif
			do {
...
# endif
			if (fcntl(data->fd, F_SETFD, FD_CLOEXEC)) {
				crx_error_docref(NULL, E_WARNING, "fcntl(%d, F_SETFD, FD_CLOEXEC) failed: %s (%d)", data->fd, strerror(errno), errno);
			}
#endif
		} else {
			crx_error_docref(NULL, E_WARNING, "open(%s, O_RDWR) failed: %s (%d)", buf, strerror(errno), errno);
		}
	}
}

static crex_result ps_files_write(ps_files *data, crex_string *key, crex_string *val)
{
	size_t n = 0;
	...
	if (ZSTR_LEN(val) < data->st_size) {
		crx_ignore_value(ftruncate(data->fd, 0));
	}
	...
	if (n != ZSTR_LEN(val)) {
		if (n == (size_t)-1) {
			crx_error_docref(NULL, E_WARNING, "Write failed: %s (%d)", strerror(errno), errno);
		} else {
			crx_error_docref(NULL, E_WARNING, "Write wrote less bytes than requested");
		}
		return FAILURE;
	}
...
static int ps_files_cleanup_dir(const crex_string *dirname, crex_long maxlifetime)
{
	DIR *dir;
	struct dirent *entry;
	crex_stat_t sbuf = {0};
	...
	if (!dir) {
		crx_error_docref(NULL, E_NOTICE, "ps_files_cleanup_dir: opendir(%s) failed: %s (%d)", ZSTR_VAL(dirname), strerror(errno), errno);
		return -1;
	}
	...
	if (ZSTR_LEN(dirname) >= MAXPATHLEN) {
		crx_error_docref(NULL, E_NOTICE, "ps_files_cleanup_dir: dirname(%s) is too long", ZSTR_VAL(dirname));
		closedir(dir);
		return -1;
	}
	...
	buf[ZSTR_LEN(dirname)] = CRX_DIR_SEPARATOR;
...
static crex_result ps_files_key_exists(ps_files *data, const crex_string *key)
{
	char buf[MAXPATHLEN];
	crex_stat_t sbuf = {0};
 ...
 * PARAMETERS: PS_OPEN_ARGS in crx_session.h
...
PS_OPEN_FUNC(files)
{
	...
	if (*save_path == '\0') {
		...
		save_path = crx_get_temporary_directory();

		if (crx_check_open_basedir(save_path)) {
			return FAILURE;
		}
	}
	...
	if (argc > 1) {
		errno = 0;
		dirdepth = (size_t) CREX_STRTOL(argv[0], NULL, 10);
		if (errno == ERANGE) {
			crx_error(E_WARNING, "The first parameter in session.save_path is invalid");
			return FAILURE;
		}
	}
	...
	if (argc > 2) {
		errno = 0;
		filemode = (int)CREX_STRTOL(argv[1], NULL, 8);
		if (errno == ERANGE || filemode < 0 || filemode > 07777) {
			crx_error(E_WARNING, "The second parameter in session.save_path is invalid");
			return FAILURE;
		}
	}
	...
	data->basedir = crex_string_init(save_path, strlen(save_path), /* persistent */ false);
 ...
 * PARAMETERS: PS_CLOSE_ARGS in crx_session.h
...
PS_CLOSE_FUNC(files)
{
	...
	crex_string_release_ex(data->basedir, /* persistent */ false);
	efree(data);
 ...
 * PARAMETERS: PS_READ_ARGS in crx_session.h
 * RETURN VALUE: SUCCESS or FAILURE. Must set non-NULL session data to (crex_string **val)
...
PS_READ_FUNC(files)
{
	...
	*val = crex_string_alloc(sbuf.st_size, 0);
	...
	if (n != (crex_long)sbuf.st_size) {
		if (n == -1) {
			crx_error_docref(NULL, E_WARNING, "Read failed: %s (%d)", strerror(errno), errno);
		} else {
			crx_error_docref(NULL, E_WARNING, "Read returned less bytes than requested");
		}
...
/*
 * Write session data.
 * PARAMETERS: PS_WRITE_ARGS in crx_session.h
 * RETURN VALUE: SUCCESS or FAILURE.
 *
 * PS_WRITE_FUNC() must write session data(crex_string *val) unconditionally.
 ...
 * PARAMETERS: PS_UPDATE_TIMESTAMP_ARGS in crx_session.h
 * RETURN VALUE: SUCCESS or FAILURE.
 *
...
/*
 * Delete session data.
 * PARAMETERS: PS_DESTROY_ARGS in crx_session.h
 * RETURN VALUE: SUCCESS or FAILURE.
 *
 ...
 * PARAMETERS: PS_GC_ARGS in crx_session.h
...
/*
 * Create session ID.
 * PARAMETERS: PS_CREATE_SID_ARGS in crx_session.h
 * RETURN VALUE: Valid session ID(crex_string *) or NULL for FAILURE.
 *
 * PS_CREATE_SID_FUNC() must check collision. i.e. Check session data if
 * new sid exists already.
 * *mod_data is guaranteed to have non-NULL value.
 * NOTE: Default crx_session_create_id() does not check collision. If
 * NULL is returned, session module create new ID by using crx_session_create_id().
 * If crx_session_create_id() fails due to invalid configuration, it raises E_ERROR.
 * NULL return value checks from crx_session_create_id() is not required generally.
 */
PS_CREATE_SID_FUNC(files)
{
	...
	do {
		sid = crx_session_create_id((void**)&data);
		if (!sid) {
			if (--maxfail < 0) {
				return NULL;
			} else {
 ...
 * PARAMETERS: PS_VALIDATE_SID_ARGS in crx_session.h
 * RETURN VALUE: SUCCESS or FAILURE.
 *
