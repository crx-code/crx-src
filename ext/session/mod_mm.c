...
static inline uint32_t ps_sd_hash(const crex_string *data)
{
	uint32_t h;
...
static ps_sd *ps_sd_new(ps_mm *data, crex_string *key)
{
	uint32_t hv, slot;
	ps_sd *sd;
	...
	if (!sd) {
		...
		crx_error_docref(NULL, E_WARNING, "mm_malloc failed, avail %ld, err %s", mm_available(data->mm), mm_error());
		return NULL;
	}
...
static void ps_sd_destroy(ps_mm *data, ps_sd *sd)
{
	...
	crex_string_release(sd->key);

	mm_free(data->mm, sd);
}

static ps_sd *ps_sd_lookup(ps_mm *data, const crex_string *key, bool rw)
{
	uint32_t hv, slot;
	ps_sd *ret, *prev;
	...
	for (prev = NULL, ret = data->hash[slot]; ret; prev = ret, ret = ret->next) {
		if (ret->hv == hv && crex_string_equals(ret->key, key)) {
			break;
		}
	}
...
static crex_result ps_mm_key_exists(ps_mm *data, const crex_string *key)
{
	ps_sd *sd;
...
static crex_result ps_mm_initialize(ps_mm *data, const char *path)
{
	data->owner = getpid();
...
CRX_MINIT_FUNCTION(ps_mm)
{
	...
	crx_session_register_module(&ps_mod_mm);
	return SUCCESS;
}
...
PS_WRITE_FUNC(mm)
{
	...
	if (sd) {
		if (val->len >= sd->alloclen) {
			...
			if (!sd->data) {
				ps_sd_destroy(data, sd);
				crx_error_docref(NULL, E_WARNING, "Cannot allocate new data segment");
				sd = NULL;
			}
		}
		if (sd) {
