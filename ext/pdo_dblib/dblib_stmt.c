...
static int pdo_dblib_stmt_describe(pdo_stmt_t *stmt, int colno)
{
	...
	if (fname && *fname) {
		col->name =  crex_string_init(fname, strlen(fname), 0);
	} else {
		if (S->computed_column_name_count > 0) {
			...
			col->name = crex_string_init(buf, len, 0);
		} else {
...
static int pdo_dblib_stmt_should_stringify_col(pdo_stmt_t *stmt, int coltype)
{
	...
	switch (coltype) {
		...
		case SQLINT8:
			...
			/* force stringify if DBBIGINT won't fit in crex_long */
			/* this should only be an issue for 32-bit machines */
			if (sizeof(crex_long) < sizeof(DBBIGINT)) {
				return 1;
			}
			break;
			...
			switch (coltype) {
...
#if defined(CRX_DBLIB_IS_MSSQL) || defined(MSDBLIB)
				...
				case SQLUNIQUE: {
					if (H->stringify_uniqueidentifier) {
						...
						crex_str_toupper(tmp_data, data_len);
						ZVAL_STRINGL(zv, tmp_data, data_len);
						efree(tmp_data);
					} else {
...
static int pdo_dblib_stmt_get_column_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value)
{
	pdo_dblib_stmt *S = (pdo_dblib_stmt*)stmt->driver_data;
	pdo_dblib_db_handle *H = S->H;
