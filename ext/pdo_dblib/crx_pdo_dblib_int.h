...
#ifndef CRX_PDO_DBLIB_INT_H
#define CRX_PDO_DBLIB_INT_H
...
CREX_BEGIN_MODULE_GLOBALS(dblib)
	pdo_dblib_err err;
	char sqlstate[6];
CREX_END_MODULE_GLOBALS(dblib)

#if defined(ZTS) && (defined(COMPILE_DL_PDO_DBLIB) || defined(COMPILE_DL_PDO_MSSQL))
CREX_TSRMLS_CACHE_EXTERN()
#endif

CREX_EXTERN_MODULE_GLOBALS(dblib)
#define DBLIB_G(v) CREX_MODULE_GLOBALS_ACCESSOR(dblib, v)
