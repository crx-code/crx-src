...
static bool dblib_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, pdo_stmt_t *stmt, zval *driver_options)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
...
static crex_long dblib_handle_doer(pdo_dbh_t *dbh, const crex_string *sql)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
	RETCODE ret, resret;
...
static crex_string* dblib_handle_quoter(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
...
crex_string *dblib_handle_last_id(pdo_dbh_t *dbh, const crex_string *name)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
	...
	ret_id = crex_string_init(id, len, 0);
	efree(id);
	return ret_id;
}

static bool dblib_set_attr(pdo_dbh_t *dbh, crex_long attr, zval *val)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
	crex_long lval;
	bool bval;
...
static int dblib_get_attribute(pdo_dbh_t *dbh, crex_long attr, zval *return_value)
{
	pdo_dblib_db_handle *H = (pdo_dblib_db_handle *)dbh->driver_data;
...
static int pdo_dblib_handle_factory(pdo_dbh_t *dbh, zval *driver_options)
{
	...
	struct pdo_data_src_parser vars[] = {
		{ "charset",	NULL,	0 }
		,{ "appname",	"CRX " PDO_DBLIB_FLAVOUR,	0 }
		,{ "host",		"127.0.0.1", 0 }
		,{ "dbname",	NULL,	0 }
	...
	crx_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, nvars);
...
cleanup:
	...
	if (!ret) {
		crex_throw_exception_ex(crx_pdo_get_exception(), DBLIB_G(err).dberr,
