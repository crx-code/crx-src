AC_DEFUN([CRX_LDAP_CHECKS], [
  if test -f $1/include/ldap.h; then
    LDAP_DIR=$1
    LDAP_INCDIR=$1/include
    LDAP_LIBDIR=$1/$CRX_LIBDIR
    ...
    OCISDKRPMINC=`echo "$1" | $SED -e 's!^/usr/lib/oracle/\(.*\)/client\('${CRX_OCI8_IC_LIBDIR_SUFFIX}'\)*/lib[/]*$!/usr/include/oracle/\1/client\2!'`
...
if test "$CRX_LDAP" != "no"; then

  CRX_NEW_EXTENSION(ldap, ldap.c, $ext_shared,, -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)

  if test "$CRX_LDAP" = "yes"; then
    for i in /usr/local /usr; do
      CRX_LDAP_CHECKS($i)
    done
  else
    CRX_LDAP_CHECKS($CRX_LDAP)
  fi
  ...
  if test -f $LDAP_LIBDIR/liblber.a || test -f $LDAP_LIBDIR/liblber.$SHLIB_SUFFIX_NAME || test -f $LDAP_LIBDIR/$MACHINE_INCLUDES/liblber.a || test -f $LDAP_LIBDIR/$MACHINE_INCLUDES/liblber.$SHLIB_SUFFIX_NAME; then
    CRX_ADD_LIBRARY_WITH_PATH(lber, $LDAP_LIBDIR, LDAP_SHARED_LIBADD)
    CRX_ADD_LIBRARY_WITH_PATH(ldap, $LDAP_LIBDIR, LDAP_SHARED_LIBADD)
    ...
    CRX_ADD_LIBRARY_WITH_PATH(clntsh, $LDAP_LIBDIR, LDAP_SHARED_LIBADD)
  ...
  CRX_ADD_INCLUDE($LDAP_INCDIR)
  CRX_SUBST(LDAP_SHARED_LIBADD)
  ...
  dnl SASL check
  if test "$CRX_LDAP_SASL" != "no"; then
    PKG_CHECK_MODULES([SASL], [libsasl2])

    CRX_EVAL_INCLINE($SASL_CFLAGS)
    CRX_EVAL_LIBLINE($SASL_LIBS, LDAP_SHARED_LIBADD)
