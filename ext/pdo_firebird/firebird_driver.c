...
static int firebird_alloc_prepare_stmt(pdo_dbh_t*, const crex_string*, XSQLDA*, isc_stmt_handle*,
	HashTable*);
...
int preprocess(const crex_string* sql, char* sql_out, HashTable* named_params)
{
		...
		case ttParamMark:
			...
			if (tok == ttIdent /*|| tok == ttString*/)
			{
				...
				if (named_params) {
					...
					crex_hash_str_update(named_params, pname, l, &tmp);
				}
...
void _firebird_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, char const *file, crex_long line) /* {{{ */
{
...
static bool firebird_handle_preparer(pdo_dbh_t *dbh, crex_string *sql, /* {{{ */
	pdo_stmt_t *stmt, zval *driver_options)
{
	...
	do {
		...
		ALLOC_HASHTABLE(np);
		crex_hash_init(np, 8, NULL, NULL, 0);
...
static crex_long firebird_handle_doer(pdo_dbh_t *dbh, const crex_string *sql) /* {{{ */
{
	pdo_firebird_db_handle *H = (pdo_firebird_db_handle *)dbh->driver_data;
...
static crex_string* firebird_handle_quoter(pdo_dbh_t *dbh, const crex_string *unquoted, enum pdo_param_type paramtype)
{
	int qcount = 0;
	char const *co, *l, *r;
	char *c;
	...
	quoted_str = crex_string_alloc(quotedlen, 0);
...
static int firebird_alloc_prepare_stmt(pdo_dbh_t *dbh, const crex_string *sql,
...
static bool firebird_handle_set_attribute(pdo_dbh_t *dbh, crex_long attr, zval *val) /* {{{ */
{
	pdo_firebird_db_handle *H = (pdo_firebird_db_handle *)dbh->driver_data;
	bool bval;
	...
	switch (attr) {
		...
		case PDO_FB_ATTR_DATE_FORMAT:
			{
				crex_string *str = zval_try_get_string(val);
				if (UNEXPECTED(!str)) {
					return false;
				}
				if (H->date_format) {
...
static int firebird_handle_get_attribute(pdo_dbh_t *dbh, crex_long attr, zval *val) /* {{{ */
{
	pdo_firebird_db_handle *H = (pdo_firebird_db_handle *)dbh->driver_data;
...
#if defined(__GNUC__) || defined(CRX_WIN32)
...
static void pdo_firebird_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info) /* {{{ */
{
	...
	char buf[400];
	crex_long i = 0, l, sqlcode = isc_sqlcode(s);
...
static int pdo_firebird_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	...
	crx_pdo_parse_data_source(dbh->data_source, dbh->data_source_len, vars, 6);
	...
	if (!dbh->methods) {
		...
		crex_throw_exception_ex(crx_pdo_get_exception(), H->isc_status[1], "SQLSTATE[%s] [%ld] %s",
