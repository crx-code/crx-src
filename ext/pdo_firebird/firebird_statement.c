...
static crex_always_inline ISC_INT64 get_isc_int64_from_sqldata(const ISC_SCHAR *sqldata)
{
	READ_AND_RETURN_USING_MEMCPY(ISC_INT64, sqldata);
}

static crex_always_inline ISC_LONG get_isc_long_from_sqldata(const ISC_SCHAR *sqldata)
{
	READ_AND_RETURN_USING_MEMCPY(ISC_LONG, sqldata);
}

static crex_always_inline double get_double_from_sqldata(const ISC_SCHAR *sqldata)
{
	READ_AND_RETURN_USING_MEMCPY(double, sqldata);
}

static crex_always_inline ISC_TIMESTAMP get_isc_timestamp_from_sqldata(const ISC_SCHAR *sqldata)
{
	READ_AND_RETURN_USING_MEMCPY(ISC_TIMESTAMP, sqldata);
}

static crex_always_inline ISC_QUAD get_isc_quad_from_sqldata(const ISC_SCHAR *sqldata)
{
...
static int firebird_stmt_dtor(pdo_stmt_t *stmt) /* {{{ */
{
	...
	crex_hash_destroy(S->named_params);
...
static int firebird_stmt_describe(pdo_stmt_t *stmt, int colno) /* {{{ */
{
	...
	col->name = crex_string_alloc(colname_len, 0);
...
static int firebird_stmt_get_column_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value)
{
	pdo_firebird_stmt *S = (pdo_firebird_stmt *) stmt->driver_data;
	XSQLVAR *var = &S->out_sqlda.sqlvar[colno];
	...
	if (*var->sqlind == -1) {
		...
		if (var->sqlscale < 0) {
			...
			if ((var->sqltype & ~1) == SQL_DOUBLE) {
				str = crex_strpprintf(0, "%.*F", -var->sqlscale, get_double_from_sqldata(var->sqldata));
			} else if (n >= 0) {
				str = crex_strpprintf(0, "%" LL_MASK "d.%0*" LL_MASK "d",
					n / f, -var->sqlscale, n % f);
			} else if (n <= -f) {
				str = crex_strpprintf(0, "%" LL_MASK "d.%0*" LL_MASK "d",
					n / f, -var->sqlscale, -n % f);
			 } else {
				str = crex_strpprintf(0, "-0.%0*" LL_MASK "d", -var->sqlscale, -n % f);
			}
			ZVAL_STR(result, str);
		} else {
			switch (var->sqltype & ~1) {
...
#else
					ZVAL_STR(result, crex_strpprintf(0, "%" LL_MASK "d", get_isc_int64_from_sqldata(var->sqldata)));
#endif
					break;
				case SQL_FLOAT:
					/* TODO: Why is this not returned as the native type? */
					ZVAL_STR(result, crex_strpprintf(0, "%F", *(float*)var->sqldata));
					break;
				case SQL_DOUBLE:
					/* TODO: Why is this not returned as the native type? */
					ZVAL_STR(result, crex_strpprintf(0, "%F", get_double_from_sqldata(var->sqldata)));
					break;
#ifdef SQL_BOOLEAN
				case SQL_BOOLEAN:
...
static int firebird_bind_blob(pdo_stmt_t *stmt, ISC_QUAD *blob_id, zval *param)
{
	...
	if (C_TYPE_P(param) != IS_STRING) {
		ZVAL_STR(&data, zval_get_string_func(param));
	} else {
		ZVAL_COPY_VALUE(&data, param);
	}

	for (rem_cnt = C_STRLEN(data); rem_cnt > 0; rem_cnt -= chunk_size) {
		chunk_size = rem_cnt > USHRT_MAX ? USHRT_MAX : (unsigned short)rem_cnt;
		if (isc_put_segment(H->isc_status, &h, chunk_size, &C_STRVAL(data)[put_cnt])) {
			RECORD_ERROR(stmt);
			result = 0;
			break;
		}
	...
	enum pdo_param_event event_type)
{
	...
	if (param->is_param && param->paramno == -1) {
		...
		if ((index = crex_hash_find(S->named_params, param->name)) != NULL) {
			param->paramno = C_LVAL_P(index);
		} else {
		...
		case PDO_PARAM_EVT_EXEC_PRE:
			...
			*var->sqlind = 0;
			...
			if (C_TYPE_P(parameter) == IS_RESOURCE) {
				...
				crx_stream_from_zval_no_verify(stm, parameter);
				if (stm) {
					crex_string *mem =  crx_stream_copy_to_mem(stm, CRX_STREAM_COPY_ALL, 0);
			...
			if ((var->sqltype & ~1) == SQL_BOOLEAN) {
				switch (C_TYPE_P(parameter)) {
					case IS_LONG:
					case IS_DOUBLE:
					case IS_TRUE:
					case IS_FALSE:
						*(FB_BOOLEAN*)var->sqldata = crex_is_true(parameter) ? FB_TRUE : FB_FALSE;
						break;
					case IS_STRING:
						{
							crex_long lval;
							...
							if (C_STRLEN_P(parameter) == 0) {
								*(FB_BOOLEAN*)var->sqldata = FB_FALSE;
								break;
							}

							switch (is_numeric_string(C_STRVAL_P(parameter), C_STRLEN_P(parameter), &lval, &dval, 0)) {
								case IS_LONG:
								...
								case IS_DOUBLE:
									*(FB_BOOLEAN*)var->sqldata = (dval != 0) ? FB_TRUE : FB_FALSE;
									break;
								default:
									if (!crex_binary_strncasecmp(C_STRVAL_P(parameter), C_STRLEN_P(parameter), "true", 4, 4)) {
										*(FB_BOOLEAN*)var->sqldata = FB_TRUE;
									} else if (!crex_binary_strncasecmp(C_STRVAL_P(parameter), C_STRLEN_P(parameter), "false", 5, 5)) {
										*(FB_BOOLEAN*)var->sqldata = FB_FALSE;
									} else {
										strcpy(stmt->error_code, "HY105");
			...
			switch (C_TYPE_P(parameter)) {
				...
				case IS_LONG:
					...
					var->sqltype = (sizeof(crex_long) == 8 ? SQL_INT64 : SQL_LONG) | (var->sqltype & 1);
					var->sqldata = (void*)&C_LVAL_P(parameter);
					var->sqllen = sizeof(crex_long);
					break;
				case IS_DOUBLE:
					/* keep the allow-NULL flag */
					var->sqltype = SQL_DOUBLE | (var->sqltype & 1);
					var->sqldata = (void*)&C_DVAL_P(parameter);
				...
				case IS_STRING:
					...
					switch (var->sqltype & ~1) {
						...
						case SQL_TYPE_DATE:
						case SQL_TYPE_TIME:
							force_null = (C_STRLEN_P(parameter) == 0);
					}
					if (!force_null) {
						/* keep the allow-NULL flag */
						var->sqltype = SQL_TEXT | (var->sqltype & 1);
						var->sqldata = C_STRVAL_P(parameter);
						var->sqllen = C_STRLEN_P(parameter);
						break;
					}
					CREX_FALLTHROUGH;
				case IS_NULL:
...
static int firebird_stmt_set_attribute(pdo_stmt_t *stmt, crex_long attr, zval *val) /* {{{ */
{
	pdo_firebird_stmt *S = (pdo_firebird_stmt*)stmt->driver_data;
	...
	switch (attr) {
		...
		case PDO_ATTR_CURSOR_NAME:
			...
			if (isc_dsql_set_cursor_name(S->H->isc_status, &S->stmt, C_STRVAL_P(val),0)) {
				RECORD_ERROR(stmt);
				return 0;
			}
			strlcpy(S->name, C_STRVAL_P(val), sizeof(S->name));
			break;
	}
	return 1;
}
...
static int firebird_stmt_get_attribute(pdo_stmt_t *stmt, crex_long attr, zval *val) /* {{{ */
{
	pdo_firebird_stmt *S = (pdo_firebird_stmt*)stmt->driver_data;
