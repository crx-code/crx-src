...
if test "$CRX_ICONV" != "no"; then

  CRX_SETUP_ICONV(ICONV_SHARED_LIBADD, [
    iconv_avail="yes";
  ],[
    iconv_avail="no";
  ])
...
int main(void) {
  ...
  return 0;
    ...
    case "$iconv_impl_name" in
      gnu_libiconv [)]
        AC_DEFINE([CRX_ICONV_IMPL],["libiconv"],[Which iconv implementation to use])
        AC_DEFINE([HAVE_LIBICONV],1,[Whether libiconv is used])
        ;;

      bsd [)]
        AC_DEFINE([HAVE_BSD_ICONV],1,[Konstantin Chuguev's iconv implementation])
        AC_DEFINE([CRX_ICONV_IMPL],["BSD iconv"],[Which iconv implementation to use])
        ;;

      glibc [)]
        AC_DEFINE([HAVE_GLIBC_ICONV],1,[glibc's iconv implementation])
        AC_DEFINE([CRX_ICONV_IMPL],["glibc"],[Which iconv implementation to use])
        ;;
      ibm [)]
        AC_DEFINE([HAVE_IBM_ICONV],1,[IBM iconv implementation])
        AC_DEFINE([CRX_ICONV_IMPL],["IBM iconv"],[Which iconv implementation to use])
        ;;
    esac
...
int main(void) {
  ...
  return 0;
    ...
    CRX_NEW_EXTENSION(iconv, iconv.c, $ext_shared,, [-DCREX_ENABLE_STATIC_TSRMLS_CACHE=1])
    CRX_SUBST(ICONV_SHARED_LIBADD)
    CRX_INSTALL_HEADERS([ext/iconv/])
  else
