...
CRX_METHOD(Spoofchecker, isSuspicious)
{
	int32_t ret, errmask;
	char *text;
	size_t text_len;
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "s|z", &text, &text_len, &error_code)) {
		RETURN_THROWS();
	}
	...
	if (U_FAILURE(SPOOFCHECKER_ERROR_CODE(co))) {
		crx_error_docref(NULL, E_WARNING, "(%d) %s", SPOOFCHECKER_ERROR_CODE(co), u_errorName(SPOOFCHECKER_ERROR_CODE(co)));
#if U_ICU_VERSION_MAJOR_NUM >= 58
		errmask = uspoof_getCheckResultChecks(co->uspoofres, SPOOFCHECKER_ERROR_CODE_P(co));

		if (errmask != ret) {
			crx_error_docref(NULL, E_WARNING, "unexpected error (%d), does not relate to the flags passed to setChecks (%d)", ret, errmask);
		}
#endif
		RETURN_TRUE;
	}
	...
	if (error_code) {
		...
		ZVAL_LONG(C_REFVAL_P(error_code), ret);
		C_TRY_ADDREF_P(error_code);
	}
	RETVAL_BOOL(ret != 0);
}
/* }}} */
...
CRX_METHOD(Spoofchecker, areConfusable)
{
	int ret;
	char *s1, *s2;
	...
	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "ss|z", &s1, &s1_len,
...
CRX_METHOD(Spoofchecker, setAllowedLocales)
{
	char *locales;
	size_t locales_len;
	SPOOFCHECKER_METHOD_INIT_VARS;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "s", &locales, &locales_len)) {
		RETURN_THROWS();
	}
...
CRX_METHOD(Spoofchecker, setChecks)
{
	crex_long checks;
	SPOOFCHECKER_METHOD_INIT_VARS;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "l", &checks)) {
		RETURN_THROWS();
	}
...
#if U_ICU_VERSION_MAJOR_NUM >= 58
/* TODO Document this method on CRX.net */
/* {{{ Set the loosest restriction level allowed for strings. */
CRX_METHOD(Spoofchecker, setRestrictionLevel)
{
	crex_long level;
	SPOOFCHECKER_METHOD_INIT_VARS;

	if (FAILURE == crex_parse_parameters(CREX_NUM_ARGS(), "l", &level)) {
		RETURN_THROWS();
	}
	...
	if (USPOOF_ASCII != level &&
		...
		crex_argument_value_error(1, "must be one of Spoofchecker::ASCII, Spoofchecker::SINGLE_SCRIPT_RESTRICTIVE, "
