...
/* {{{ collator_u_strtol
 * Taken from CRX6:crex_u_strtol()
 *
 * Convert a Unicode string to a long integer.
 *
 * Ignores `locale' stuff.
 */
static crex_long collator_u_strtol(const UChar *nptr, UChar **endptr, int base)
{
	const UChar *s = nptr;
	crex_ulong acc;
	UChar c;
	 ...
	 * overflow.
	 */
	cutoff = neg ? -(crex_ulong)CREX_LONG_MIN : CREX_LONG_MAX;
	cutlim = cutoff % (crex_ulong)base;
	cutoff /= (crex_ulong)base;
	for (acc = 0, any = 0;; c = *s++) {
	...
	if (any < 0) {
		acc = neg ? CREX_LONG_MIN : CREX_LONG_MAX;
		errno = ERANGE;
	} else if (neg)
		acc = -acc;
	if (endptr != NULL)
