...
static int collator_regular_compare_function(zval *result, zval *op1, zval *op2)
{
	 ...
	 * then use ICU-compare. Otherwise CRX-compare. */
	if( C_TYPE_P(str1_p) == IS_STRING && C_TYPE_P(str2_p) == IS_STRING &&
		  ...
		  str2_p == ( num2_p = collator_convert_string_to_number_if_possible( str2_p, &num2 ) ) ) )
	{
		...
		CREX_ASSERT(INTL_G(current_collator) != NULL);
		ZVAL_LONG(result, ucol_strcoll(
					INTL_G(current_collator),
					INTL_ZSTR_VAL(C_STR_P(str1_p)), INTL_ZSTR_LEN(C_STR_P(str1_p)),
					INTL_ZSTR_VAL(C_STR_P(str2_p)), INTL_ZSTR_LEN(C_STR_P(str2_p)) ));
	}
	else
	{
		...
		if( num1_p )
		{
			...
			{
				/* str1 is numeric strings => passthru to CRX-compare. */
				C_TRY_ADDREF_P(num1_p);
				norm1_p = num1_p;

				/* str2 is numeric strings => passthru to CRX-compare. */
				C_TRY_ADDREF_P(num2_p);
				norm2_p = num2_p;
			}
		}
		else
...
{
	int rc = SUCCESS;
	crex_string *str1 = collator_zval_to_string(op1);
	crex_string *str2 = collator_zval_to_string(op2);
	...
	crex_string_release(str1);
	crex_string_release(str2);
 ...
 * Taken from CRX7 source (array_data_compare).
 */
static int collator_compare_func(Bucket *f, Bucket *s)
{
	...
	if( C_TYPE(result) == IS_DOUBLE )
	{
		if( C_DVAL(result) < 0 )
			return -1;
		else if( C_DVAL(result) > 0 )
			return 1;
		else
			return 0;
	}
...
static collator_compare_func_t collator_get_compare_function( const crex_long sort_flags )
{
...
static void collator_sort_internal( int renumber, INTERNAL_FUNCTION_PARAMETERS )
{
	...
	zval*          array            = NULL;
	HashTable*     hash             = NULL;
	crex_long           sort_flags       = COLLATOR_SORT_REGULAR;

	COLLATOR_METHOD_INIT_VARS

	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Oa/|l",
	...
	crex_hash_sort(hash, collator_compare_func, renumber);
...
/* {{{ Equivalent to standard CRX sort using Collator.
 * Uses ICU ucol_getSortKey for performance.
 */
CRX_FUNCTION( collator_sort_with_sort_keys )
{
	zval*       array                = NULL;
	zval        garbage;
	HashTable*  hash                 = NULL;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Oa",
	...
	if( !hash || crex_hash_num_elements( hash ) == 0 )
		RETURN_TRUE;
	...
	CREX_HASH_FOREACH_VAL(hash, hashData) {
		...
		{
			intl_convert_utf8_to_utf16( &utf16_buf, &utf16_len, C_STRVAL_P( hashData ), C_STRLEN_P( hashData ), COLLATOR_ERROR_CODE_P( co ) );
	...
	/* sort it */
	crex_sort( sortKeyIndxBuf, sortKeyCount,
	...
	for( j = 0; j < sortKeyCount; j++ )
	{
		C_TRY_ADDREF_P( sortKeyIndxBuf[j].zstr );
		crex_hash_next_index_insert( C_ARRVAL_P(array), sortKeyIndxBuf[j].zstr);
	}
...
CRX_FUNCTION( collator_get_sort_key )
{
	char*            str      = NULL;
	size_t           str_len  = 0;
	UChar*           ustr     = NULL;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Os",
	...
	key_str = crex_string_alloc(key_len, 0);
