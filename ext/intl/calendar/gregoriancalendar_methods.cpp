...
#define CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(argument, zpp_arg_position) \
	if (UNEXPECTED(argument < INT32_MIN || argument > INT32_MAX)) { \
		crex_argument_value_error(zpp_arg_position, "must be between %d and %d", INT32_MIN, INT32_MAX); \
		RETURN_THROWS(); \
	}
...
static bool set_gregorian_calendar_time_zone(GregorianCalendar *gcal, UErrorCode status)
{
	...
	if (tzstr.isBogus()) {
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
		   ...
		   "from CRX's default timezone name (see date_default_timezone_get())",
		   0
		);
...
static void _crx_intlgregcal_constructor_body(
    INTERNAL_FUNCTION_PARAMETERS, bool is_constructor, crex_error_handling *error_handling, bool *error_handling_replaced)
{
	zval		*tz_object	= NULL;
	zval		args_a[6],
				*args		= &args_a[0];
	...
	if (CREX_NUM_ARGS() > 6 ||
			crex_get_parameters_array_ex(CREX_NUM_ARGS(), args) == FAILURE) {
		crex_argument_count_error("Too many arguments");
		RETURN_THROWS();
	}

	for (variant = CREX_NUM_ARGS();
		variant > 0 && C_TYPE(args[variant - 1]) == IS_NULL;
		variant--) {}
	if (variant == 4) {
		crex_argument_count_error("No variant with 4 arguments (excluding trailing NULLs)");
		RETURN_THROWS();
	}

	// argument parsing
	if (variant <= 2) {
		if (crex_parse_parameters(MIN(CREX_NUM_ARGS(), 2),
				"|z!s!", &tz_object, &locale, &locale_len) == FAILURE) {
			RETURN_THROWS();
		}
	}
	if (variant > 2 && crex_parse_parameters(CREX_NUM_ARGS(),
	...
	Calendar_object *co = C_INTL_CALENDAR_P(return_value);
	GregorianCalendar *gcal = NULL;

	if (co->ucal) {
		crex_throw_error(NULL, "IntlGregorianCalendar object is already constructed");
		RETURN_THROWS();
	}
	...
	if (variant <= 2) {
		...
		for (int i = 0; i < variant; i++) {
			CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(largs[i], getThis() ? (i-1) : i);
		}
...
U_CFUNC CRX_FUNCTION(intlgregcal_create_instance)
{
	intl_error_reset(NULL);

	object_init_ex(return_value, GregorianCalendar_ce_ptr);
	_crx_intlgregcal_constructor_body(INTERNAL_FUNCTION_PARAM_PASSTHRU, /* is_constructor */ 0, NULL, NULL);
}

U_CFUNC CRX_METHOD(IntlGregorianCalendar, __main)
{
	crex_error_handling error_handling;
	bool error_handling_replaced = 0;

	return_value = CREX_THIS;
	_crx_intlgregcal_constructor_body(INTERNAL_FUNCTION_PARAM_PASSTHRU, /* is_constructor */ 1, &error_handling, &error_handling_replaced);
	if (error_handling_replaced) {
		crex_restore_error_handling(&error_handling);
	}
}

U_CFUNC CRX_METHOD(IntlGregorianCalendar, createFromDate)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "lll", &year, &month, &day) == FAILURE) {
		RETURN_THROWS();
	}
	...
	object_init_ex(return_value, GregorianCalendar_ce_ptr);
	co = C_INTL_CALENDAR_P(return_value);
	co->ucal = gcal;
...
U_CFUNC CRX_METHOD(IntlGregorianCalendar, createFromDateTime)
{
	crex_long year, month, day, hour, minute, second;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "lllll|l!", &year, &month, &day, &hour, &minute, &second, &second_is_null) == FAILURE) {
		RETURN_THROWS();
	}
...
U_CFUNC CRX_FUNCTION(intlgregcal_set_gregorian_change)
{
	double date;
...
U_CFUNC CRX_FUNCTION(intlgregcal_get_gregorian_change)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlgregcal_is_leap_year)
{
	crex_long year;
	CALENDAR_METHOD_INIT_VARS;
	...
	if (UNEXPECTED(year < INT32_MIN || year > INT32_MAX)) {
		crex_argument_value_error(getThis() ? 1 : 2, "must be between %d and %d", INT32_MIN, INT32_MAX);
		RETURN_THROWS();
	}
