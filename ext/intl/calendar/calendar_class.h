...
static inline Calendar_object *crx_intl_calendar_fetch_object(crex_object *obj) {
	return (Calendar_object *)((char*)(obj) - XtOffsetOf(Calendar_object, zo));
}
#define C_INTL_CALENDAR_P(zv) crx_intl_calendar_fetch_object(C_OBJ_P(zv))
...
#define CALENDAR_METHOD_FETCH_OBJECT \
	...
	if (co->ucal == NULL) \
	{ \
		crex_throw_error(NULL, "Found unconstructed IntlCalendar"); \
		RETURN_THROWS(); \
	}

void calendar_object_create(zval *object, Calendar *calendar);

Calendar *calendar_fetch_native_calendar(crex_object *object);

void calendar_object_construct(zval *object, Calendar *calendar);

void calendar_register_IntlCalendar_class(void);

extern crex_class_entry *Calendar_ce_ptr, *GregorianCalendar_ce_ptr;

extern crex_object_handlers Calendar_handlers;
