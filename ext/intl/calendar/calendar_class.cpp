...
/* {{{ Global variables */
crex_class_entry *Calendar_ce_ptr;
crex_class_entry *GregorianCalendar_ce_ptr;
crex_object_handlers Calendar_handlers;
/* }}} */
...
U_CFUNC Calendar *calendar_fetch_native_calendar(crex_object *object)
{
	Calendar_object *co = crx_intl_calendar_fetch_object(object);
...
static crex_object *Calendar_clone_obj(crex_object *object)
{
	...
	co_orig = crx_intl_calendar_fetch_object(object);
	intl_error_reset(INTL_DATA_ERROR_P(co_orig));

	ret_val = Calendar_ce_ptr->create_object(object->ce);
	co_new  = crx_intl_calendar_fetch_object(ret_val);

	crex_objects_clone_members(&co_new->zo, &co_orig->zo);
	...
	if (co_orig->ucal != NULL) {
		...
		crex_throw_exception(NULL, "Cannot clone unconstructed IntlCalendar", 0);
	}
...
static HashTable *Calendar_get_debug_info(crex_object *object, int *is_temp)
{
	zval			zv,
					zfields;
	Calendar_object	*co;
	...
	*is_temp = 1;
	...
	co  = crx_intl_calendar_fetch_object(object);
	cal = co->ucal;
	...
	crex_hash_str_update(debug_info, "type", sizeof("type") - 1, &zv);
	{
		zval		   ztz,
					   ztz_debug;
		int			   is_tmp;
		...
		debug_info_tz = C_OBJ_HANDLER(ztz, get_debug_info)(C_OBJ(ztz), &is_tmp);
		assert(is_tmp == 1);

		array_init(&ztz_debug);
		crex_hash_copy(C_ARRVAL(ztz_debug), debug_info_tz, zval_add_ref);
		crex_hash_destroy(debug_info_tz);
		FREE_HASHTABLE(debug_info_tz);

		crex_hash_str_update(debug_info, "timeZone", sizeof("timeZone") - 1, &ztz_debug);
	}

	{
		...
		if (U_SUCCESS(uec)) {
			...
			crex_hash_str_update(debug_info, "locale", sizeof("locale") - 1, &zv);
		} else {
	...
	for (int i = 0;
		...
		if (U_SUCCESS(uec)) {
			add_assoc_long(&zfields, name, (crex_long)res);
		} else {
			add_assoc_string(&zfields, name, const_cast<char*>(u_errorName(uec)));
		}
	}

	crex_hash_str_update(debug_info, "fields", sizeof("fields") - 1, &zfields);
 ...
 * Initialize internals of Calendar_object not specific to crex standard objects.
 */
...
static void Calendar_objects_free(crex_object *object)
{
	Calendar_object* co = crx_intl_calendar_fetch_object(object);
...
static crex_object *Calendar_object_create(crex_class_entry *ce)
{
