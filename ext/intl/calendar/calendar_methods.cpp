...
#define CREX_VALUE_ERROR_INVALID_FIELD(argument, zpp_arg_position) \
	if (argument < 0 || argument >= UCAL_FIELD_COUNT) { \
		crex_argument_value_error(getThis() ? ((zpp_arg_position)-1) : zpp_arg_position, \
			"must be a valid field"); \
		RETURN_THROWS(); \
	}

#define CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(argument, zpp_arg_position) \
...
#define CREX_VALUE_ERROR_INVALID_DAY_OF_WEEK(argument, zpp_arg_position) \
...
U_CFUNC CRX_METHOD(IntlCalendar, __main)
{
...
U_CFUNC CRX_FUNCTION(intlcal_create_instance)
{
...
U_CFUNC CRX_FUNCTION(intlcal_get_keyword_values_for_locale)
{
	UErrorCode	status = U_ZERO_ERROR;
	char		*key,
				*locale;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ssb",
...
U_CFUNC CRX_FUNCTION(intlcal_get_now)
{
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intlcal_get_available_locales)
{
	intl_error_reset(NULL);
...
static void _crx_intlcal_field_uec_ret_in32t_method(
		...
		INTERNAL_FUNCTION_PARAMETERS)
{
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(),
			"Ol", &object, Calendar_ce_ptr, &field) == FAILURE) {
		RETURN_THROWS();
	}

	CREX_VALUE_ERROR_INVALID_FIELD(field, 2);
...
U_CFUNC CRX_FUNCTION(intlcal_get)
{
	_crx_intlcal_field_uec_ret_in32t_method(&Calendar::get,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_time)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_set_time)
{
	double	time_arg;
	CALENDAR_METHOD_INIT_VARS;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Od",
...
U_CFUNC CRX_FUNCTION(intlcal_add)
{
	crex_long	field,
			amount;
	CALENDAR_METHOD_INIT_VARS;
	...
	CREX_VALUE_ERROR_INVALID_FIELD(field, 2);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(amount, 3);
...
U_CFUNC CRX_FUNCTION(intlcal_set_time_zone)
{
	zval			*zv_timezone;
	TimeZone		*timeZone;
	CALENDAR_METHOD_INIT_VARS;
...
static void _crx_intlcal_before_after(
		...
		INTERNAL_FUNCTION_PARAMETERS)
{
	...
	when_co = C_INTL_CALENDAR_P(when_object);
	if (when_co->ucal == NULL) {
		crex_argument_error(NULL, 2, "is uninitialized");
		RETURN_THROWS();
	}
...
U_CFUNC CRX_FUNCTION(intlcal_after)
{
	_crx_intlcal_before_after(&Calendar::after, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_before)
{
	_crx_intlcal_before_after(&Calendar::before, INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_set)
{
	crex_long args[6];
	...
	int arg_num = CREX_NUM_ARGS() - (object ? 0 : 1);

	if (crex_parse_method_parameters(
		CREX_NUM_ARGS(), object, "Oll|llll",
	...
	for (int i = 0; i < arg_num; i++) {
		...
		CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(args[i], i + 1);
	}

	CALENDAR_METHOD_FETCH_OBJECT;

	if (arg_num == 2) {
		CREX_VALUE_ERROR_INVALID_FIELD(args[0], 2);
		co->ucal->set((UCalendarDateFields)args[0], (int32_t)args[1]);
	} else if (arg_num == 3) {
		co->ucal->set((int32_t)args[0], (int32_t)args[1], (int32_t)args[2]);
	} else if (arg_num == 4) {
		crex_argument_count_error("IntlCalendar::set() has no variant with exactly 4 parameters");
		RETURN_THROWS();
	} else if (arg_num == 5) {
...
U_CFUNC CRX_METHOD(IntlCalendar, setDate)
{
	crex_long year, month, day;

	CALENDAR_METHOD_INIT_VARS;

	object = getThis();

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), object, "Olll",
		&object, Calendar_ce_ptr, &year, &month, &day) == FAILURE) {
		RETURN_THROWS();
	}

	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(year, 1);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(month, 2);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(day, 3);
...
U_CFUNC CRX_METHOD(IntlCalendar, setDateTime)
{
	crex_long year, month, day, hour, minute, second = 0;
	...
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), object, "Olllll|l!",
	...
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(month, 2);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(day, 3);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(hour, 4);
	CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(minute, 5);
	...
	if (second_is_null) {
		...
		CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(second, 6);
		co->ucal->set((int32_t) year, (int32_t) month, (int32_t) day, (int32_t) hour, (int32_t) minute, (int32_t) second);
	}
}

U_CFUNC CRX_FUNCTION(intlcal_roll)
{
	zval *zvalue;
	crex_long field, value;
	CALENDAR_METHOD_INIT_VARS;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Olz", &object, Calendar_ce_ptr, &field, &zvalue) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (C_TYPE_P(zvalue) == IS_FALSE || C_TYPE_P(zvalue) == IS_TRUE) {
		value = C_TYPE_P(zvalue) == IS_TRUE ? 1 : -1;
		crx_error_docref(NULL, E_DEPRECATED, "Passing bool is deprecated, use 1 or -1 instead");
	} else {
		value = zval_get_long(zvalue);
		CREX_VALUE_ERROR_OUT_OF_BOUND_VALUE(value, 3);
	}
...
U_CFUNC CRX_FUNCTION(intlcal_clear)
{
	crex_long field;
	bool field_is_null = 1;
	CALENDAR_METHOD_INIT_VARS;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(),
...
U_CFUNC CRX_FUNCTION(intlcal_field_difference)
{
	crex_long	field;
	double	when;
	CALENDAR_METHOD_INIT_VARS;
	...
	CREX_VALUE_ERROR_INVALID_FIELD(field, 3);
...
U_CFUNC CRX_FUNCTION(intlcal_get_actual_maximum)
{
	_crx_intlcal_field_uec_ret_in32t_method(&Calendar::getActualMaximum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_actual_minimum)
{
	_crx_intlcal_field_uec_ret_in32t_method(&Calendar::getActualMinimum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_day_of_week_type)
{
	crex_long	dow;
	CALENDAR_METHOD_INIT_VARS;
	...
	CREX_VALUE_ERROR_INVALID_DAY_OF_WEEK(dow, 2);
...
U_CFUNC CRX_FUNCTION(intlcal_get_first_day_of_week)
{
	CALENDAR_METHOD_INIT_VARS;
...
static void _crx_intlcal_field_ret_in32t_method(
...
U_CFUNC CRX_FUNCTION(intlcal_get_greatest_minimum)
{
	_crx_intlcal_field_ret_in32t_method(&Calendar::getGreatestMinimum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_least_maximum)
{
	_crx_intlcal_field_ret_in32t_method(&Calendar::getLeastMaximum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_locale)
{
	...
	if (locale_type != ULOC_ACTUAL_LOCALE && locale_type != ULOC_VALID_LOCALE) {
		crex_argument_value_error(getThis() ? 1 : 2, "must be either Locale::ACTUAL_LOCALE or Locale::VALID_LOCALE");
		RETURN_THROWS();
	}
...
U_CFUNC CRX_FUNCTION(intlcal_get_maximum)
{
	_crx_intlcal_field_ret_in32t_method(&Calendar::getMaximum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_minimal_days_in_first_week)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_get_minimum)
{
	_crx_intlcal_field_ret_in32t_method(&Calendar::getMinimum,
		INTERNAL_FUNCTION_PARAM_PASSTHRU);
}

U_CFUNC CRX_FUNCTION(intlcal_get_time_zone)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_get_type)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_get_weekend_transition)
{
	crex_long	dow;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_in_daylight_time)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_is_equivalent_to)
{
	zval			*other_object;
	...
	other_co = C_INTL_CALENDAR_P(other_object);
...
U_CFUNC CRX_FUNCTION(intlcal_is_lenient)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_is_set)
{
	crex_long field;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_is_weekend)
{
	double date;
	bool date_is_null = 1;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_set_first_day_of_week)
{
	crex_long	dow;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_set_lenient)
{
	bool is_lenient;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_set_minimal_days_in_first_week)
{
	crex_long	num_days;
	CALENDAR_METHOD_INIT_VARS;
	...
	// Use CREX_VALUE_ERROR_INVALID_DAY_OF_WEEK ?
	if (num_days < 1 || num_days > 7) {
		crex_argument_value_error(getThis() ? 1 : 2, "must be between 1 and 7");
		RETURN_THROWS();
	}
...
U_CFUNC CRX_FUNCTION(intlcal_equals)
{
	zval			*other_object;
	Calendar_object	*other_co;
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_get_repeated_wall_time_option)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_get_skipped_wall_time_option)
{
	CALENDAR_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intlcal_set_repeated_wall_time_option)
{
	crex_long	option;
	CALENDAR_METHOD_INIT_VARS;
	...
	if (option != UCAL_WALLTIME_FIRST && option != UCAL_WALLTIME_LAST) {
		crex_argument_value_error(getThis() ? 1 : 2, "must be either IntlCalendar::WALLTIME_FIRST or "
...
U_CFUNC CRX_FUNCTION(intlcal_set_skipped_wall_time_option)
{
	crex_long	option;
	CALENDAR_METHOD_INIT_VARS;
	...
	if (option != UCAL_WALLTIME_FIRST && option != UCAL_WALLTIME_LAST
		...
		crex_argument_value_error(getThis() ? 1 : 2, "must be one of IntlCalendar::WALLTIME_FIRST, "
...
U_CFUNC CRX_FUNCTION(intlcal_from_date_time)
{
	crex_object     *date_obj;
	crex_string     *date_str;
	zval			zv_tmp, zv_arg, zv_timestamp;
	crx_date_obj	*datetime;
	...
	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_OBJ_OF_CLASS_OR_STR(date_obj, crx_date_get_date_ce(), date_str)
		C_PARAM_OPTIONAL
		C_PARAM_STRING_OR_NULL(locale_str, locale_str_len)
	CREX_PARSE_PARAMETERS_END();

	if (date_str) {
		object_init_ex(&zv_tmp, crx_date_get_date_ce());
		ZVAL_STR(&zv_arg, date_str);
		crex_call_known_instance_method_with_1_params(C_OBJCE(zv_tmp)->constructor, C_OBJ(zv_tmp), NULL, &zv_arg);
		date_obj = C_OBJ(zv_tmp);
		if (EG(exception)) {
			crex_object_store_ctor_failed(C_OBJ(zv_tmp));
			goto error;
		}
	}

	datetime = crx_date_obj_from_obj(date_obj);
	if (!datetime->time) {
	...
	crex_call_method_with_0_params(date_obj, crx_date_get_date_ce(), NULL, "gettimestamp", &zv_timestamp);
	if (C_TYPE(zv_timestamp) != IS_LONG) {
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
	...
	cal->setTime(((UDate)C_LVAL(zv_timestamp)) * 1000., status);
    if (U_FAILURE(status)) {
...
U_CFUNC CRX_FUNCTION(intlcal_to_date_time)
{
	zval retval;
	CALENDAR_METHOD_INIT_VARS;
	...
	object_init_ex(return_value, crx_date_get_date_ce());
	crex_call_known_instance_method_with_2_params(
		C_OBJCE_P(return_value)->constructor, C_OBJ_P(return_value), NULL, &ts_zval, timezone_zval);
	if (EG(exception)) {
	...
	crex_call_method_with_1_params(C_OBJ_P(return_value), NULL, NULL, "settimezone",
			&retval, timezone_zval);
	if (C_ISUNDEF(retval) || C_TYPE(retval) == IS_FALSE) {
		intl_errors_set(CALENDAR_ERROR_P(co), U_ILLEGAL_ARGUMENT_ERROR,
...
U_CFUNC CRX_FUNCTION(intlcal_get_error_code)
{
	CALENDAR_METHOD_INIT_VARS;
	...
	co = C_INTL_CALENDAR_P(object);
	if (co == NULL)
		RETURN_FALSE;

	RETURN_LONG((crex_long)CALENDAR_ERROR_CODE(co));
}

U_CFUNC CRX_FUNCTION(intlcal_get_error_message)
{
