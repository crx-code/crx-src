...
#define IC_METHOD(mname) CRX_METHOD(IntlChar, mname)

static inline int convert_cp(UChar32* pcp, crex_string *string_codepoint, crex_long int_codepoint) {
	if (string_codepoint != NULL) {
		int32_t i = 0;
		size_t string_codepoint_length = ZSTR_LEN(string_codepoint);

		if (CREX_SIZE_T_INT_OVFL(string_codepoint_length)) {
...
static crex_never_inline int parse_code_point_param(INTERNAL_FUNCTION_PARAMETERS, UChar32 *cp) {
	crex_string *string_codepoint;
	crex_long int_codepoint = 0;
	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_STR_OR_LONG(string_codepoint, int_codepoint)
	CREX_PARSE_PARAMETERS_END_EX(return FAILURE);
...
IC_METHOD(getIntPropertyMinValue) {
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "l", &prop) == FAILURE) {
		RETURN_THROWS();
	}
...
static UBool enumCharType_callback(enumCharType_data *context,
	...
	if (crex_call_function(&context->fci, &context->fci_cache) == FAILURE) {
...
IC_METHOD(enumCharTypes) {
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "f", &context.fci, &context.fci_cache) == FAILURE) {
		RETURN_THROWS();
	}
...
IC_METHOD(charName) {
	...
	crex_long nameChoice = U_UNICODE_CHAR_NAME;
	...
	buffer = crex_string_alloc(buffer_len, 0);
	error = U_ZERO_ERROR;
...
IC_METHOD(charFromName) {
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|l", &name, &name_len, &nameChoice) == FAILURE) {
		RETURN_THROWS();
	}
...
IC_METHOD(enumCharNames) {
	UChar32 start, limit;
	crex_string *string_start, *string_limit;
	crex_long int_start = 0, int_limit = 0;
	...
	CREX_PARSE_PARAMETERS_START(3, 4)
		C_PARAM_STR_OR_LONG(string_start, int_start)
		C_PARAM_STR_OR_LONG(string_limit, int_limit)
...
IC_METHOD(getPropertyName) {
	crex_long property;
	crex_long nameChoice = U_LONG_PROPERTY_NAME;
	const char *ret;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|l", &property, &nameChoice) == FAILURE) {
		RETURN_THROWS();
	}
...
IC_METHOD(getPropertyValueName) {
	crex_long property, value, nameChoice = U_LONG_PROPERTY_NAME;
	const char *ret;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "ll|l", &property, &value, &nameChoice) == FAILURE) {
		RETURN_THROWS();
	}
...
IC_METHOD(getPropertyValueEnum) {
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ls", &property, &name, &name_len) == FAILURE) {
		RETURN_THROWS();
	}
...
IC_METHOD(foldCase) {
	UChar32 cp, ret;
	crex_long options = U_FOLD_CASE_DEFAULT;
	crex_string *string_codepoint;
	crex_long int_codepoint = 0;
...
/* {{{ */
IC_METHOD(forDigit) {
	crex_long digit, radix = 10;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|l", &digit, &radix) == FAILURE) {
		RETURN_THROWS();
	}
...
int crx_uchar_minit(INIT_FUNC_ARGS) {
