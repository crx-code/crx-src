...
#if U_ICU_VERSION_MAJOR_NUM >= 56
static const UNormalizer2 *intl_get_normalizer(crex_long form, UErrorCode *err)
{/*{{{*/
	switch (form)
	{
...
static int32_t intl_normalize(crex_long form, const UChar *src, int32_t src_len, UChar *dst, int32_t dst_len, UErrorCode *err)
{/*{{{*/
...
static UBool intl_is_normalized(crex_long form, const UChar *uinput, int32_t uinput_len, UErrorCode *err)
{/*{{{*/
	const UNormalizer2 *norm = intl_get_normalizer(form, err);
...
/* {{{ Normalize a string. */
CRX_FUNCTION( normalizer_normalize )
{
	char*			input = NULL;
	/* form is optional, defaults to FORM_C */
	crex_long	    form = NORMALIZER_DEFAULT;
	size_t			input_len = 0;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "s|l",
	...
	switch(form) {
		...
		case NORMALIZER_FORM_KC_CF:
			...
			break;
		default:
			crex_argument_value_error(2, "must be a a valid normalization form");
			RETURN_THROWS();
	}
...
CRX_FUNCTION( normalizer_is_normalized )
{
	char*	 	input = NULL;
	/* form is optional, defaults to FORM_C */
	crex_long		form = NORMALIZER_DEFAULT;
...
#if U_ICU_VERSION_MAJOR_NUM >= 56
CRX_FUNCTION( normalizer_get_raw_decomposition )
{
	char* input = NULL;
	...
	if ((crex_parse_parameters(CREX_NUM_ARGS(), "s|l", &input, &input_length, &form) == FAILURE)) {
		RETURN_THROWS();
	}
