...
crex_class_entry *IntlException_ce_ptr;
...
void intl_error_set_custom_msg( intl_error* err, const char* msg, int copyMsg )
{
	...
	if( !err ) {
		if( INTL_G( error_level ) )
			crx_error_docref( NULL, INTL_G( error_level ), "%s", msg );
		if( INTL_G( use_exceptions ) )
			crex_throw_exception_ex( IntlException_ce_ptr, 0, "%s", msg );
	}
	if( !err && !( err = intl_g_error_get(  ) ) )
		return;
...
crex_string * intl_error_get_message( intl_error* err )
{
	const char *uErrorName = NULL;
	crex_string *errMessage = 0;
...
smart_str intl_parse_error_to_string( UParseError* pe )
{
	...
	if( pe->line > 0 )
	{
		...
		smart_str_append_long( &ret, (crex_long ) pe->line );
		any = 1;
	}
	if( pe->offset >= 0 ) {
		if( any )
		...
		smart_str_append_long( &ret, (crex_long ) pe->offset );
		any = 1;
	}
	...
	if (pe->preContext[0] != 0 ) {
		...
		if( !u8str )
		{
			...
			crex_string_release_ex( u8str, 0 );
		}
		smart_str_appends( &ret, "\"" );
		any = 1;
	}
