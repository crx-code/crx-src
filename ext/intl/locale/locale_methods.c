...
*/
static crex_off_t getStrrtokenPos(char* str, crex_off_t savedPos)
{
	crex_off_t result =-1;
	crex_off_t i;
...
*/
static crex_off_t getSingletonPos(const char* str)
{
	crex_off_t result =-1;
	size_t len = 0;
...
/* }}} */
/* {{{ Get default locale */
CRX_NAMED_FUNCTION(zif_locale_get_default)
{
...
/* }}} */
/* {{{ Set default locale */
CRX_NAMED_FUNCTION(zif_locale_set_default)
{
	crex_string* locale_name;
	crex_string *ini_name;
	char *default_locale = NULL;

	if(crex_parse_parameters( CREX_NUM_ARGS(),  "S", &locale_name) == FAILURE)
	{
		RETURN_THROWS();
	}
	...
	if (ZSTR_LEN(locale_name) == 0) {
		...
		locale_name = crex_string_init(default_locale, strlen(default_locale), 0);
	}

	ini_name = crex_string_init(LOCALE_INI_NAME, sizeof(LOCALE_INI_NAME) - 1, 0);
	crex_alter_ini_entry(ini_name, locale_name, CRX_INI_USER, CRX_INI_STAGE_RUNTIME);
	crex_string_release_ex(ini_name, 0);
	if (default_locale != NULL) {
		crex_string_release_ex(locale_name, 0);
	}
...
*/
static crex_string* get_icu_value_internal( const char* loc_name , char* tag_name, int* result , int fromParseLocale)
{
	crex_string* tag_value	    = NULL;
	...
	if( strcmp(tag_name, LOC_CANONICALIZE_TAG) != 0 ){
		...
		crex_off_t grOffset =  findOffset( LOC_GRANDFATHERED , loc_name );
		if( grOffset >= 0 ){
			if( strcmp(tag_name , LOC_LANG_TAG)==0 ){
				return crex_string_init(loc_name, strlen(loc_name), 0);
			} else {
	...
	do{
		if (tag_value) {
			tag_value = crex_string_realloc( tag_value , buflen, 0);
		} else {
			tag_value = crex_string_alloc( buflen, 0);
		}
		...
		if( U_FAILURE( status ) ) {
			...
			*result = 0;
			if( tag_value ){
				crex_string_release_ex( tag_value, 0 );
			}
			if( mod_loc_name ){
				efree( mod_loc_name);
			}
...
/* {{{
* Gets the value from ICU , called when CRX userspace function is called
* common code shared by get_primary_language,get_script or get_region or get_variant
*/
static void get_icu_value_src_crx( char* tag_name, INTERNAL_FUNCTION_PARAMETERS)
{
...
CRX_FUNCTION( locale_get_script )
{
	get_icu_value_src_crx( LOC_SCRIPT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */

/* {{{ gets the region for the $locale */
CRX_FUNCTION( locale_get_region )
{
	get_icu_value_src_crx( LOC_REGION_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */

/* {{{ gets the primary language for the $locale */
CRX_FUNCTION(locale_get_primary_language )
{
	get_icu_value_src_crx( LOC_LANG_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
static void get_icu_disp_value_src_crx( char* tag_name, INTERNAL_FUNCTION_PARAMETERS)
{
	const char* loc_name        	= NULL;
	size_t         loc_name_len    	= 0;
...
CRX_FUNCTION(locale_get_display_name)
{
	get_icu_disp_value_src_crx( DISP_NAME , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */

/* {{{ gets the language for the $locale in $in_locale or default_locale */
CRX_FUNCTION(locale_get_display_language)
{
	get_icu_disp_value_src_crx( LOC_LANG_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */

/* {{{ gets the script for the $locale in $in_locale or default_locale */
CRX_FUNCTION(locale_get_display_script)
{
	get_icu_disp_value_src_crx( LOC_SCRIPT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */

/* {{{ gets the region for the $locale in $in_locale or default_locale */
CRX_FUNCTION(locale_get_display_region)
{
	get_icu_disp_value_src_crx( LOC_REGION_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
* gets the variant for the $locale in $in_locale or default_locale
*/
CRX_FUNCTION(locale_get_display_variant)
{
	get_icu_disp_value_src_crx( LOC_VARIANT_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
 ...
 * pairs for this locale. The keys are keys to the array (doh!)
 */
CRX_FUNCTION( locale_get_keywords )
{
	UEnumeration*   e        = NULL;
	UErrorCode      status   = U_ZERO_ERROR;
	...
	if( e != NULL ) {
		...
		*/
		...
		while( ( kw_key = uenum_next( e, &kw_key_len, &status ) ) != NULL ){
			...
			crex_string *kw_value_str = crex_string_alloc(kw_value_len, 0);
			...
			if (status == U_BUFFER_OVERFLOW_ERROR) {
				status = U_ZERO_ERROR;
				kw_value_str = crex_string_extend(kw_value_str, kw_value_len, 0);
				kw_value_len=uloc_getKeywordValue( loc_name,kw_key, ZSTR_VAL(kw_value_str), kw_value_len+1, &status );
			} else if(!U_FAILURE(status)) {
				kw_value_str = crex_string_truncate(kw_value_str, kw_value_len, 0);
			}
			if (U_FAILURE(status)) {
				...
				if( kw_value_str){
					crex_string_efree( kw_value_str );
				}
...
CRX_FUNCTION(locale_canonicalize)
{
	get_icu_value_src_crx( LOC_CANONICALIZE_TAG , INTERNAL_FUNCTION_PARAM_PASSTHRU );
}
/* }}} */
...
static int append_key_value(smart_str* loc_name, HashTable* hash_arr, char* key_name)
{
	...
	if ((ele_value = crex_hash_str_find(hash_arr , key_name, strlen(key_name))) != NULL ) {
		if(C_TYPE_P(ele_value)!= IS_STRING ){
		...
		smart_str_appendl(loc_name, C_STRVAL_P(ele_value) , C_STRLEN_P(ele_value));
		return SUCCESS;
	}
...
static int append_multiple_key_values(smart_str* loc_name, HashTable* hash_arr, char* key_name)
{
	...
	if ((ele_value = crex_hash_str_find( hash_arr , key_name , strlen(key_name))) != NULL) {
		if( C_TYPE_P(ele_value) == IS_STRING ){
			...
			smart_str_appendl(loc_name, C_STRVAL_P(ele_value) , C_STRLEN_P(ele_value));
			return SUCCESS;
		} else if(C_TYPE_P(ele_value) == IS_ARRAY ) {
			HashTable *arr = C_ARRVAL_P(ele_value);
			zval *data;

			CREX_HASH_FOREACH_VAL(arr, data) {
				...
				smart_str_appendl(loc_name, SEPARATOR , sizeof(SEPARATOR)-1);
				smart_str_appendl(loc_name, C_STRVAL_P(data) , C_STRLEN_P(data));
		...
		for( i=0 ; i< max_value; i++ ){
			...
			if ((ele_value = crex_hash_str_find( hash_arr , cur_key_name , strlen(cur_key_name))) != NULL) {
				if( C_TYPE_P(ele_value)!= IS_STRING ){
...
CRX_FUNCTION(locale_compose)
{
	...
	hash_arr = C_ARRVAL_P( arr );

	if( !hash_arr || crex_hash_num_elements( hash_arr ) == 0 )
		RETURN_FALSE;
	...
	if( result == LOC_NOT_FOUND ){
		crex_argument_value_error(1, "must contain a \"%s\" key", LOC_LANG_TAG);
		smart_str_free(loc_name);
		RETURN_THROWS();
	}
	if( !handleAppendResult( result, loc_name)){
...
*/
static crex_string* get_private_subtags(const char* loc_name)
{
		...
		while( (singletonPos = getSingletonPos(mod_loc_name)) > -1){
			if( (*(mod_loc_name+singletonPos)=='x') || (*(mod_loc_name+singletonPos)=='X') ){
				...
				if( singletonPos + 2 ==  len){
					...
					result = crex_string_init(mod_loc_name + singletonPos+2  , (len -( singletonPos +2) ), 0);
				}
				break;
			}
			else{
...
static int add_array_entry(const char* loc_name, zval* hash_arr, char* key_name)
{
	...
	if( (strcmp(key_name , LOC_PRIVATE_TAG)==0) ||
		...
		if( result > 0 && key_value){
			...
			token = crx_strtok_r( key_value->val , DELIMITER ,&last_ptr);
			if( cur_key_name ){
				efree( cur_key_name);
			}
			...
			while( (token = crx_strtok_r(NULL , DELIMITER , &last_ptr)) && (strlen(token)>1) ){
				sprintf( cur_key_name , "%s%d", key_name , cnt++);
				add_assoc_string( hash_arr, cur_key_name , token);
			}
/*
...
*/
		...
		if (key_value) {
			crex_string_release_ex(key_value, 0);
		}
	} else {
		if( result == 1 ){
...
CRX_FUNCTION(locale_get_all_variants)
{
	const char*  	loc_name        = NULL;
	size_t    		loc_name_len    = 0;
	...
	/* Call ICU variant */
		...
		if( result > 0 && variant){
			/* Tokenize on the "_" or "-" */
			token = crx_strtok_r( variant->val , DELIMITER , &saved_ptr);
			add_next_index_stringl( return_value, token , strlen(token));
			/* tokenize on the "_" or "-" and stop  at singleton if any	*/
			while( (token = crx_strtok_r(NULL , DELIMITER, &saved_ptr)) && (strlen(token)>1) ){
				add_next_index_stringl( return_value, token , strlen(token));
			}
		}
		if( variant ){
			crex_string_release_ex( variant, 0 );
		}
	}
...
/* }}} */
/* {{{ Checks if a $langtag filter matches with $locale according to RFC 4647's basic filtering algorithm */
CRX_FUNCTION(locale_filter_matches)
{
	char*       	lang_tag        = NULL;
	...
	crex_string*   	can_lang_tag    = NULL;
	crex_string*   	can_loc_range   = NULL;
	...
	if(crex_parse_parameters( CREX_NUM_ARGS(), "ss|b",
	...
	if( boolCanonical ){
		...
		if( result == 0) {
			efree( cur_lang_tag );
			crex_string_release_ex( can_lang_tag, 0 );
			RETURN_FALSE;
		}
		...
		if( result == 0) {
			...
			crex_string_release_ex( can_lang_tag, 0 );
			efree( cur_loc_range );
			crex_string_release_ex( can_loc_range, 0 );
			RETURN_FALSE;
		}
...
/* {{{
* returns the lookup result to lookup_loc_range_src_crx
* internal function
*/
static crex_string* lookup_loc_range(const char* loc_range, HashTable* hash_arr, int canonicalize )
{
	int	i = 0;
	int	cur_arr_len = 0;
	int result = 0;

	crex_string* lang_tag = NULL;
	zval* ele_value = NULL;

	char* cur_loc_range	= NULL;
	crex_string* can_loc_range	= NULL;
	crex_off_t saved_pos = 0;

	crex_string* return_value = NULL;

	char **cur_arr = ecalloc(crex_hash_num_elements(hash_arr)*2, sizeof(char *));
	CREX_HASH_FOREACH_VAL(hash_arr, ele_value) {
	/* convert the array to lowercase , also replace hyphens with the underscore and store it in cur_arr */
		if(C_TYPE_P(ele_value)!= IS_STRING) {
			/* element value is not a string */
			crex_argument_type_error(2, "must only contain string values");
			LOOKUP_CLEAN_RETURN(NULL);
		}
		cur_arr[cur_arr_len*2] = estrndup(C_STRVAL_P(ele_value), C_STRLEN_P(ele_value));
		result = strToMatch(C_STRVAL_P(ele_value), cur_arr[cur_arr_len*2]);
		if(result == 0) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "lookup_loc_range: unable to canonicalize lang_tag", 0);
			LOOKUP_CLEAN_RETURN(NULL);
		}
		cur_arr[cur_arr_len*2+1] = C_STRVAL_P(ele_value);
		cur_arr_len++ ;
	} CREX_HASH_FOREACH_END(); /* end of for */
	...
	if(canonicalize) {
		for(i=0; i<cur_arr_len; i++) {
			...
			if(result != 1 || lang_tag == NULL || !lang_tag->val[0]) {
				if(lang_tag) {
					crex_string_release_ex(lang_tag, 0);
				}
				intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR, "lookup_loc_range: unable to canonicalize lang_tag" , 0);
				LOOKUP_CLEAN_RETURN(NULL);
			}
	...
	while(saved_pos > 0) {
		for(i=0; i< cur_arr_len; i++){
			if(cur_arr[i*2] != NULL && strlen(cur_arr[i*2]) == saved_pos && strncmp(cur_loc_range, cur_arr[i*2], saved_pos) == 0) {
				...
				return_value = crex_string_init(str, strlen(str), 0);
				efree(cur_loc_range);
...
* range
*/
CRX_FUNCTION(locale_lookup)
{
	crex_string*   	fallback_loc_str	= NULL;
	const char*    	loc_range      		= NULL;
	size_t        	loc_range_len  		= 0;
	...
	HashTable*	hash_arr		= NULL;
	bool	boolCanonical	= 0;
	crex_string* 	result_str	= NULL;

	intl_error_reset( NULL );

	if(crex_parse_parameters( CREX_NUM_ARGS(), "as|bS!", &arr, &loc_range, &loc_range_len,
	...
	if(result_str == NULL || ZSTR_VAL(result_str)[0] == '\0') {
		if( fallback_loc_str ) {
			result_str = crex_string_copy(fallback_loc_str);
		} else {
			RETURN_EMPTY_STRING();
		}
	}
...
/* }}} */
/* {{{ Tries to find out best available locale based on HTTP "Accept-Language" header */
CRX_FUNCTION(locale_accept_from_http)
{
	...
	if(crex_parse_parameters( CREX_NUM_ARGS(), "s", &http_accept, &http_accept_len) == FAILURE)
	{
		RETURN_THROWS();
	}
