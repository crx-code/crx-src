 ...
 *
 * Note that ICU uses int32_t as string length and CRX uses size_t. While
 ...
 * @return crex_string
 */
crex_string* intl_convert_utf16_to_utf8(
	...
	UErrorCode*  status )
{
	...
	*status = U_ZERO_ERROR;
	...
	/* Allocate memory for the destination buffer (it will be zero-terminated). */
	dst = crex_string_alloc(dst_len, 0);
