...
/* {{{ Format a number. */
CRX_FUNCTION( numfmt_format )
{
	zval *number;
	crex_long type = FORMAT_TYPE_DEFAULT;
	UChar format_buf[32];
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "On|l",
	...
	if(type == FORMAT_TYPE_DEFAULT) {
		switch(C_TYPE_P(number)) {
			case IS_LONG:
				/* take INT32 on 32-bit, int64 on 64-bit */
				type = (sizeof(crex_long) == 8)?FORMAT_TYPE_INT64:FORMAT_TYPE_INT32;
				break;
			case IS_DOUBLE:
	...
	switch(type) {
		case FORMAT_TYPE_INT32:
			...
			formatted_len = unum_format(FORMATTER_OBJECT(nfo), (int32_t)C_LVAL_P(number),
		...
		{
			int64_t value = (C_TYPE_P(number) == IS_DOUBLE)?(int64_t)C_DVAL_P(number):C_LVAL_P(number);
		...
		case FORMAT_TYPE_DOUBLE:
			...
			formatted_len = unum_formatDouble(FORMATTER_OBJECT(nfo), C_DVAL_P(number), formatted, formatted_len, NULL, &INTL_DATA_ERROR_CODE(nfo));
			if (INTL_DATA_ERROR_CODE(nfo) == U_BUFFER_OVERFLOW_ERROR) {
				intl_error_reset(INTL_DATA_ERROR_P(nfo));
				formatted = eumalloc(formatted_len);
				unum_formatDouble(FORMATTER_OBJECT(nfo), C_DVAL_P(number), formatted, formatted_len, NULL, &INTL_DATA_ERROR_CODE(nfo));
				if (U_FAILURE( INTL_DATA_ERROR_CODE(nfo) ) ) {
					efree(formatted);
				}
			}
		...
		case FORMAT_TYPE_CURRENCY:
			if (getThis()) {
				...
				crex_argument_value_error(2, "cannot be NumberFormatter::TYPE_CURRENCY constant, "
					"use %s%sformatCurrency() method instead", class_name, space);
			} else {
				crex_argument_value_error(3, "cannot be NumberFormatter::TYPE_CURRENCY constant, use numfmt_format_currency() function instead");
			}
			RETURN_THROWS();
		default:
			crex_argument_value_error(getThis() ? 2 : 3, "must be a NumberFormatter::TYPE_* constant");
			RETURN_THROWS();
	}
...
CRX_FUNCTION( numfmt_format_currency )
{
	double     number;
	UChar      format_buf[32];
	UChar*     formatted     = format_buf;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Ods",
