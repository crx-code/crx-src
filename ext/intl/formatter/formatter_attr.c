...
CRX_FUNCTION( numfmt_get_attribute )
{
	crex_long attribute, value;
...
/* {{{ Get formatter attribute value. */
CRX_FUNCTION( numfmt_get_text_attribute )
{
	crex_long   attribute;
	UChar   value_buf[64];
...
/* {{{ Get formatter attribute value. */
CRX_FUNCTION( numfmt_set_attribute )
{
	crex_long attribute;
	zval *value;
	FORMATTER_METHOD_INIT_VARS;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Oln",
...
/* {{{ Get formatter attribute value. */
CRX_FUNCTION( numfmt_set_text_attribute )
{
	int32_t slength = 0;
	UChar *svalue = NULL;
	crex_long attribute;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Ols",
...
CRX_FUNCTION( numfmt_get_symbol )
{
	crex_long symbol;
	UChar value_buf[4];
	UChar *value = value_buf;
...
CRX_FUNCTION( numfmt_set_symbol )
{
	crex_long  symbol;
	char*      value     = NULL;
	size_t     value_len = 0;
...
CRX_FUNCTION( numfmt_get_pattern )
{
	UChar   value_buf[64];
	uint32_t length = USIZE( value_buf );
	UChar*  value  = value_buf;
...
CRX_FUNCTION( numfmt_set_pattern )
{
	char*       value = NULL;
	size_t      value_len = 0;
	int32_t     slength = 0;
...
CRX_FUNCTION( numfmt_get_locale )
{
	crex_long type = ULOC_ACTUAL_LOCALE;
	char* loc;
	FORMATTER_METHOD_INIT_VARS;

	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "O|l",
