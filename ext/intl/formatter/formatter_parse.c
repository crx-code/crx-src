...
/* {{{ Parse a number. */
CRX_FUNCTION( numfmt_parse )
{
	crex_long type = FORMAT_TYPE_DOUBLE;
	UChar* sstr = NULL;
	int32_t sstr_len = 0;
	char* str = NULL;
	size_t str_len;
	...
	/* Parse parameters. */
	if (crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Os|lz!",
	...
	switch(type) {
		...
		case FORMAT_TYPE_INT64:
			...
			if(val64 > CREX_LONG_MAX || val64 < CREX_LONG_MIN) {
				RETVAL_DOUBLE(val64);
			} else {
				RETVAL_LONG((crex_long)val64);
			}
			break;
		...
		case FORMAT_TYPE_CURRENCY:
			if (getThis()) {
				...
				crex_argument_value_error(3, "cannot be NumberFormatter::TYPE_CURRENCY constant, use numfmt_parse_currency() function instead");
			}
			goto cleanup;
		default:
	...
	if (zposition) {
		CREX_TRY_ASSIGN_REF_LONG(zposition, position);
	}
...
CRX_FUNCTION( numfmt_parse_currency )
{
	double number;
	UChar currency[5] = {0};
	UChar* sstr = NULL;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Osz/|z!",
