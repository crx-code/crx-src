		...
		HashTable* hash_arr, char* key_name, intl_error *err)
{
	...
	if ((ele_value = crex_hash_str_find(hash_arr, key_name, strlen(key_name))) != NULL) {
		if(C_TYPE_P(ele_value) != IS_LONG) {
			...
			if (C_LVAL_P(ele_value) > INT32_MAX ||
					C_LVAL_P(ele_value) < INT32_MIN) {
				spprintf(&message, 0, "datefmt_format: value " CREX_LONG_FMT " is out of "
						"bounds for a 32-bit integer in key '%s'",
						C_LVAL_P(ele_value), key_name);
				intl_errors_set(err, U_ILLEGAL_ARGUMENT_ERROR, message, 1);
				efree(message);
			} else {
				result = C_LVAL_P(ele_value);
			}
		}
	}
...
#define INTL_GET_ELEM(elem) \
	...
	/* Month in ICU and CRX starts from January =0 */
...
CRX_FUNCTION(datefmt_format)
{
	...
	/* Parse parameters. */
	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Oz",
	...
	if (C_TYPE_P(zarg) == IS_ARRAY) {
		hash_arr = C_ARRVAL_P(zarg);
