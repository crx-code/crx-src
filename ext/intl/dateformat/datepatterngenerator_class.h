...
static inline IntlDatePatternGenerator_object *crx_intl_datepatterngenerator_fetch_object(crex_object *obj) {
	return (IntlDatePatternGenerator_object *)((char*)(obj) - XtOffsetOf(IntlDatePatternGenerator_object, zo));
}
#define C_INTL_DATEPATTERNGENERATOR_P(zv) crx_intl_datepatterngenerator_fetch_object(C_OBJ_P(zv))
...
#define DTPATTERNGEN_METHOD_FETCH_OBJECT \
	...
	if (dtpgo->dtpg == NULL) \
	{ \
		crex_throw_error(NULL, "Found unconstructed IntlDatePatternGenerator"); \
		RETURN_THROWS(); \
	}

void dateformat_register_IntlDatePatternGenerator_class(void);

extern crex_class_entry *IntlDatePatternGenerator_ce_ptr;

extern crex_object_handlers IntlDatePatternGenerator_handlers;
