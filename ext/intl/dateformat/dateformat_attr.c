...
CRX_FUNCTION( datefmt_get_datetype )
{
	DATE_FORMAT_METHOD_INIT_VARS;

	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "O", &object, IntlDateFormatter_ce_ptr ) == FAILURE )
	{
		RETURN_THROWS();
	}
...
CRX_FUNCTION( datefmt_get_timetype )
{
	DATE_FORMAT_METHOD_INIT_VARS;
...
CRX_FUNCTION( datefmt_get_pattern )
{
	UChar  value_buf[64];
...
CRX_FUNCTION( datefmt_set_pattern )
{
	char*       value = NULL;
	size_t      value_len = 0;
	int32_t     slength = 0;
...
CRX_FUNCTION( datefmt_get_locale )
{
	char *loc;
	crex_long  loc_type =ULOC_ACTUAL_LOCALE;
...
CRX_FUNCTION( datefmt_is_lenient )
{
...
CRX_FUNCTION( datefmt_set_lenient )
{
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Ob",
