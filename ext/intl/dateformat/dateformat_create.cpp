...
/* {{{ */
static crex_result datefmt_ctor(INTERNAL_FUNCTION_PARAMETERS, crex_error_handling *error_handling, bool *error_handling_replaced)
{
	zval		*object;
	char	*locale_str;
	size_t		locale_len	= 0;
	Locale		locale;
	crex_long	date_type = UDAT_FULL;
	crex_long	time_type = UDAT_FULL;
	crex_object *calendar_obj = NULL;
	crex_long calendar_long = 0;
	bool calendar_is_null = 1;
	Calendar *cal = NULL;
	crex_long	calendar_type;
	bool		calendar_owned;
	...
	CREX_PARSE_PARAMETERS_START(1, 6)
		C_PARAM_STRING_OR_NULL(locale_str, locale_len)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(date_type)
		C_PARAM_LONG(time_type)
		C_PARAM_ZVAL(timezone_zv)
		C_PARAM_OBJ_OF_CLASS_OR_LONG_OR_NULL(calendar_obj, Calendar_ce_ptr, calendar_long, calendar_is_null)
		C_PARAM_STRING_OR_NULL(pattern_str, pattern_str_len)
	...
	/* process timezone */
	explicit_tz = timezone_zv != NULL && C_TYPE_P(timezone_zv) != IS_NULL;
...
/* {{{ Create formatter. */
U_CFUNC CRX_FUNCTION( datefmt_create )
{
...
U_CFUNC CRX_METHOD( IntlDateFormatter, __main )
{
	...
	if (datefmt_ctor(INTERNAL_FUNCTION_PARAM_PASSTHRU, &error_handling, &error_handling_replaced) == FAILURE) {
		if (!EG(exception)) {
			crex_string *err = intl_error_get_message(NULL);
			crex_throw_exception(IntlException_ce_ptr, ZSTR_VAL(err), intl_error_get_code(NULL));
			crex_string_release_ex(err, 0);
		}
	}
