...
static inline IntlDateFormatter_object *crx_intl_dateformatter_fetch_object(crex_object *obj) {
	return (IntlDateFormatter_object *)((char*)(obj) - XtOffsetOf(IntlDateFormatter_object, zo));
}
#define C_INTL_DATEFORMATTER_P(zv) crx_intl_dateformatter_fetch_object(C_OBJ_P(zv))

void dateformat_register_IntlDateFormatter_class( void );
extern crex_class_entry *IntlDateFormatter_ce_ptr;
...
#define DATE_FORMAT_METHOD_FETCH_OBJECT				\
	...
	{												\
		crex_throw_error(NULL, "Found unconstructed IntlDateFormatter"); \
		RETURN_THROWS();								\
	}
