...
static bool valid_format(zval *z) {
	if (C_TYPE_P(z) == IS_LONG) {
		crex_long lval = C_LVAL_P(z);
		for (int i = 0; i < sizeof(valid_styles) / sizeof(*valid_styles); i++) {
			if ((crex_long)valid_styles[i] == lval) {
				return true;
			}
		}
	}
...
U_CFUNC CRX_FUNCTION(datefmt_format_object)
{
	zval				*object,
						*format = NULL;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "o|zs!",
	...
	if (format == NULL || C_TYPE_P(format) == IS_NULL) {
		//nothing
	} else if (C_TYPE_P(format) == IS_ARRAY) {
		HashTable *ht = C_ARRVAL_P(format);
		if (crex_hash_num_elements(ht) != 2) {
		...
		CREX_HASH_FOREACH_VAL(ht, z) {
			if (!valid_format(z)) {
				if (idx == 0) {
					...
					CREX_ASSERT(idx == 1 && "We checked that there are two elements above");
					intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
			...
			if (idx == 0) {
				dateStyle = (DateFormat::EStyle)C_LVAL_P(z);
			} else {
				CREX_ASSERT(idx == 1 && "We checked that there are two elements above");
				timeStyle = (DateFormat::EStyle)C_LVAL_P(z);
			}
			idx++;
		} CREX_HASH_FOREACH_END();
		CREX_ASSERT(idx == 2 && "We checked that there are two elements above");
	} else if (C_TYPE_P(format) == IS_LONG) {
		...
		dateStyle = timeStyle = (DateFormat::EStyle)C_LVAL_P(format);
	} else {
		if (!try_convert_to_string(format)) {
			RETURN_THROWS();
		}
		if (C_STRLEN_P(format) == 0) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
	...
	crex_class_entry *instance_ce = C_OBJCE_P(object);
	if (instanceof_function(instance_ce, Calendar_ce_ptr)) {
		Calendar *obj_cal = calendar_fetch_native_calendar(C_OBJ_P(object));
		if (obj_cal == NULL) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
		...
		cal = obj_cal->clone();
	} else if (instanceof_function(instance_ce, crx_date_get_interface_ce())) {
	...
	if (pattern) {
		StringPiece sp(C_STRVAL_P(format));
