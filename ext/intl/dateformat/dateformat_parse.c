...
static void internal_parse_to_timestamp(IntlDateFormatter_object *dfo, char* text_to_parse, size_t text_len, int32_t *parse_pos, zval *return_value)
{
	...
	if (result > (double)LONG_MAX || result < (double)LONG_MIN) {
		...
		ZVAL_LONG(return_value, (crex_long)result);
	}
}
/* }}} */

static void add_to_localtime_arr( IntlDateFormatter_object *dfo, zval* return_value, const UCalendar *parsed_calendar, crex_long calendar_field, char* key_name)
{
	crex_long calendar_field_val = ucal_get( parsed_calendar, calendar_field, &INTL_DATA_ERROR_CODE(dfo));
...
CRX_FUNCTION(datefmt_parse)
{
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "Os|z!",
	...
	if (z_parse_pos) {
		crex_long long_parse_pos;
		ZVAL_DEREF(z_parse_pos);
		long_parse_pos = zval_get_long(z_parse_pos);
		if (CREX_LONG_INT_OVFL(long_parse_pos)) {
...
CRX_FUNCTION(datefmt_localtime)
{
	char*           text_to_parse = NULL;
	size_t          text_len =0;
	zval*         	z_parse_pos = NULL;
