/*
   ...
   | http://www.crx.net/license/3_01.txt                                  |
...
crex_class_entry *IntlDatePatternGenerator_ce_ptr;
crex_object_handlers IntlDatePatternGenerator_handlers;

static crex_object *IntlDatePatternGenerator_object_clone(crex_object *object)
{
	intl_error_reset(NULL);

	IntlDatePatternGenerator_object *dtpgo_orig = crx_intl_datepatterngenerator_fetch_object(object);
	intl_error_reset(DTPATTERNGEN_ERROR_P(dtpgo_orig));

	crex_object *ret_val = IntlDatePatternGenerator_ce_ptr->create_object(object->ce);
	IntlDatePatternGenerator_object *dtpgo_new  = crx_intl_datepatterngenerator_fetch_object(ret_val);

	crex_objects_clone_members(&dtpgo_new->zo, &dtpgo_orig->zo);
	...
	if (dtpgo_orig->dtpg != NULL) {
		...
		crex_throw_exception(NULL, "Cannot clone unconstructed IntlDatePatternGenerator", 0);
	}
...
/*
 * Initialize internals of IntlDatePatternGenerator_object not specific to crex standard objects.
 */
...
static void IntlDatePatternGenerator_object_free(crex_object *object)
{
	IntlDatePatternGenerator_object* co = crx_intl_datepatterngenerator_fetch_object(object);
...
static crex_object *IntlDatePatternGenerator_object_create(crex_class_entry *ce)
{
	IntlDatePatternGenerator_object *intern =
		(IntlDatePatternGenerator_object*) crex_object_alloc(sizeof(IntlDatePatternGenerator_object), ce);
