 ...
 * We use it for the same purpose here. Prefix the methods with crx to
 * avoid clashes with any definitions in ICU.
 */
class MessageFormatAdapter {
public:
    static const Formattable::Type* crxGetArgTypeList(const MessageFormat& m,
                                                   int32_t& count);
    static const MessagePattern crxGetMessagePattern(MessageFormat* m);
};

const Formattable::Type*
MessageFormatAdapter::crxGetArgTypeList(const MessageFormat& m,
                                     int32_t& count) {
    return m.getArgTypeList(count);
}

const MessagePattern
MessageFormatAdapter::crxGetMessagePattern(MessageFormat* m) {
    return m->msgPattern;
}
U_NAMESPACE_END
...
{
	int32_t fmt_count = 0;
	MessageFormatAdapter::crxGetArgTypeList(*(const MessageFormat*)fmt, fmt_count);
	return fmt_count;
}
										 ...
										 intl_error& err)
{
	...
	const Formattable::Type *types = MessageFormatAdapter::crxGetArgTypeList(
	...
	ALLOC_HASHTABLE(ret);
	crex_hash_init(ret, parts_count, NULL, arg_types_dtor, 0);

	for (int i = 0; i < parts_count; i++) {
		const Formattable::Type t = types[i];
		crex_hash_index_update_mem(ret, (crex_ulong)i, (void*)&t, sizeof(t));
	}

	if (U_FAILURE(err.code)) {
		crex_hash_destroy(ret);
		efree(ret);
									...
									intl_error& err)
{
	 ...
	 * so no need for destructor */
	ALLOC_HASHTABLE(ret);
	crex_hash_init(ret, 32, NULL, arg_types_dtor, 0);
	...
	*/
	...
	for (int32_t i = 0; i < parts_count - 2 && U_SUCCESS(err.code); i++) {
		...
		if (name_part.getType() == UMSGPAT_PART_TYPE_ARG_NAME) {
			...
			if ((storedType = (Formattable::Type*)crex_hash_str_find_ptr(ret, (char*)argName.getBuffer(), argName.length() * sizeof(UChar))) == NULL) {
				/* not found already; create new entry in HT */
				Formattable::Type bogusType = Formattable::kObject;
				storedType = (Formattable::Type*)crex_hash_str_update_mem(ret, (char*)argName.getBuffer(), argName.length() * sizeof(UChar),
			...
			if ((storedType = (Formattable::Type*)crex_hash_index_find_ptr(ret, (crex_ulong)argNumber)) == NULL) {
				/* not found already; create new entry in HT */
				Formattable::Type bogusType = Formattable::kObject;
				storedType = (Formattable::Type*)crex_hash_index_update_mem(ret, (crex_ulong)argNumber, (void*)&bogusType, sizeof(bogusType));
			}
		} else {
								 ...
								 intl_error& err)
{
	...
	const MessagePattern mp = MessageFormatAdapter::crxGetMessagePattern(mf);
...
{
	int arg_count = crex_hash_num_elements(args);
	...
	CREX_HASH_FOREACH_KEY_VAL(args, num_index, str_index, elem) {
		ZVAL_DEREF(elem);
		...
		if (str_index == NULL) {
			...
			if (UNEXPECTED(num_index > (crex_ulong)INT32_MAX)) {
		   ...
		   storedArgType = (Formattable::Type*)crex_hash_index_find_ptr(types, (crex_ulong)num_index);
			...
			storedArgType = (Formattable::Type*)crex_hash_str_find_ptr(types, (char*)key.getBuffer(), key.length() * sizeof(UChar));
		}
	...
	string_arg:
					 ...
					 * and CRX ends up with a fatal error and calls longjmp
					 * as a result of that.
					 */
			...
			case Formattable::kLong:
				{
					...
					if (C_TYPE_P(elem) == IS_DOUBLE) {
						if (UNEXPECTED(C_DVAL_P(elem) > (double)INT32_MAX ||
								C_DVAL_P(elem) < (double)INT32_MIN)) {
							intl_errors_set(&err, U_ILLEGAL_ARGUMENT_ERROR,
								"Found CRX float with absolute value too large for "
								"32 bit integer argument", 0);
						} else {
							tInt32 = (int32_t)C_DVAL_P(elem);
						}
					} else if (C_TYPE_P(elem) == IS_LONG) {
						if (UNEXPECTED(C_LVAL_P(elem) > INT32_MAX ||
								C_LVAL_P(elem) < INT32_MIN)) {
							intl_errors_set(&err, U_ILLEGAL_ARGUMENT_ERROR,
								"Found CRX integer with absolute value too large "
								"for 32 bit integer argument", 0);
						} else {
							tInt32 = (int32_t)C_LVAL_P(elem);
						}
					} else {
			...
			case Formattable::kInt64:
				{
					...
					if (C_TYPE_P(elem) == IS_DOUBLE) {
						if (UNEXPECTED(C_DVAL_P(elem) > (double)U_INT64_MAX ||
								C_DVAL_P(elem) < (double)U_INT64_MIN)) {
							intl_errors_set(&err, U_ILLEGAL_ARGUMENT_ERROR,
								"Found CRX float with absolute value too large for "
								"64 bit integer argument", 0);
						} else {
							tInt64 = (int64_t)C_DVAL_P(elem);
						}
					} else if (C_TYPE_P(elem) == IS_LONG) {
						/* assume long is not wider than 64 bits */
						tInt64 = (int64_t)C_LVAL_P(elem);
					} else {
			...
			case Formattable::kDate:
				{
					...
					if (U_FAILURE(err.code)) {
						...
						if (u8key) {
							...
							intl_errors_set(&err, err.code, message, 1);
							crex_string_release_ex(u8key, 0);
							efree(message);
						}
						continue;
					}
			...
			switch (C_TYPE_P(elem)) {
			case IS_DOUBLE:
				formattable.setDouble(C_DVAL_P(elem));
				break;
			case IS_LONG:
				formattable.setInt64((int64_t)C_LVAL_P(elem));
				break;
			case IS_NULL:
			case IS_FALSE:
		...
		argNum++;
	} CREX_HASH_FOREACH_END(); // visiting each argument
	...
	*args = (zval *)safe_emalloc(*count, sizeof(zval), 0);
    ...
    for(int32_t i = 0; i < *count; i++) {
        ...
        case Formattable::kInt64:
			...
			if(aInt64 > CREX_LONG_MAX || aInt64 < -CREX_LONG_MAX) {
				ZVAL_DOUBLE(&(*args)[i], (double)aInt64);
			} else {
				ZVAL_LONG(&(*args)[i], (crex_long)aInt64);
			}
            break;
