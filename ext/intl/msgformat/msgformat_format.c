...
#ifndef C_ADDREF_P
#define C_ADDREF_P(z) ((z)->refcount++)
#endif
...
static void msgfmt_do_format(MessageFormatter_object *mfo, zval *args, zval *return_value)
{
	...
	umsg_format_helper(mfo, C_ARRVAL_P(args), &formatted, &formatted_len);
...
/* {{{ Format a message. */
CRX_FUNCTION( msgfmt_format_message )
{
	zval       *args;
	UChar      *spattern = NULL;
	int         spattern_len = 0;
	...
	/* Parse parameters. */
	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "ssa",
