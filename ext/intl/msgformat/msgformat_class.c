...
crex_class_entry *MessageFormatter_ce_ptr = NULL;
static crex_object_handlers MessageFormatter_handlers;
...
void MessageFormatter_object_free( crex_object *object )
{
	MessageFormatter_object* mfo = crx_intl_messageformatter_fetch_object(object);

	crex_object_std_dtor( &mfo->zo );
...
crex_object *MessageFormatter_object_create(crex_class_entry *ce)
{
	MessageFormatter_object*     intern;

	intern = crex_object_alloc(sizeof(MessageFormatter_object), ce);
...
crex_object *MessageFormatter_object_clone(crex_object *object)
{
	MessageFormatter_object *mfo, *new_mfo;
	crex_object *new_obj;

	mfo = crx_intl_messageformatter_fetch_object(object);
	intl_error_reset(INTL_DATA_ERROR_P(mfo));

	new_obj = MessageFormatter_ce_ptr->create_object(object->ce);
	new_mfo = crx_intl_messageformatter_fetch_object(new_obj);
	/* clone standard parts */
	crex_objects_clone_members(&new_mfo->zo, &mfo->zo);
	...
	if (MSG_FORMAT_OBJECT(mfo) != NULL) {
		...
		if (U_FAILURE(INTL_DATA_ERROR_CODE(mfo))) {
			...
			crex_throw_exception_ex(NULL, 0, "Failed to clone MessageFormatter object");
		}
	} else {
		crex_throw_exception_ex(NULL, 0, "Cannot clone unconstructed MessageFormatter");
	}
	return new_obj;
}
/* }}} */
