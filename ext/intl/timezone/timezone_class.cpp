...
/* {{{ Global variables */
U_CDECL_BEGIN
crex_class_entry *TimeZone_ce_ptr = NULL;
crex_object_handlers TimeZone_handlers;
U_CDECL_END
/* }}} */
...
U_CFUNC void timezone_object_construct(const TimeZone *zone, zval *object, int owned)
{
	...
	TIMEZONE_METHOD_FETCH_OBJECT_NO_CHECK; /* fetch crex object from zval "object" into "to" */
	to->utimezone = zone;
											   ...
											   const char *func, zval *ret)
{
	...
	object_init_ex(ret, crx_date_get_timezone_ce());
	tzobj = C_CRXTIMEZONE_P(ret);
	...
	if (id.compare(0, 3, UnicodeString("GMT", sizeof("GMT")-1, US_INV)) == 0) {
		...
		crex_call_known_instance_method_with_1_params(
			C_OBJCE_P(ret)->constructor, C_OBJ_P(ret), NULL, &arg);
		if (EG(exception)) {
			spprintf(&message, 0,
				"%s: DateTimeZone constructor threw exception", func);
			intl_errors_set(outside_error, U_ILLEGAL_ARGUMENT_ERROR,
				message, 1);
			crex_object_store_ctor_failed(C_OBJ_P(ret));
			zval_ptr_dtor(&arg);
			goto error;
		}
		zval_ptr_dtor(&arg);
													 ...
													 const char *func)
{
	...
	if (zv_timezone == NULL || C_TYPE_P(zv_timezone) == IS_NULL) {
	...
	if (C_TYPE_P(zv_timezone) == IS_OBJECT &&
			instanceof_function(C_OBJCE_P(zv_timezone), TimeZone_ce_ptr)) {
		TimeZone_object *to = C_INTL_TIMEZONE_P(zv_timezone);
	...
	} else if (C_TYPE_P(zv_timezone) == IS_OBJECT &&
			instanceof_function(C_OBJCE_P(zv_timezone), crx_date_get_timezone_ce())) {

		crx_timezone_obj *tzobj = C_CRXTIMEZONE_P(zv_timezone);
		...
		if (intl_stringFromChar(id, C_STRVAL_P(zv_timezone), C_STRLEN_P(zv_timezone),
				&status) == FAILURE) {
...
static crex_object *TimeZone_clone_obj(crex_object *object)
{
	...
	to_orig = crx_intl_timezone_fetch_object(object);
	intl_error_reset(TIMEZONE_ERROR_P(to_orig));

	ret_val = TimeZone_ce_ptr->create_object(object->ce);
	to_new  = crx_intl_timezone_fetch_object(ret_val);

	crex_objects_clone_members(&to_new->zo, &to_orig->zo);
	...
	if (to_orig->utimezone != NULL) {
		...
		crex_throw_exception(NULL, "Cannot clone unconstructed IntlTimeZone", 0);
	}
...
static int TimeZone_compare_objects(zval *object1, zval *object2)
{
	...
	to1 = C_INTL_TIMEZONE_P(object1);
	to2 = C_INTL_TIMEZONE_P(object2);

	if (to1->utimezone == NULL || to2->utimezone == NULL) {
		crex_throw_exception(NULL, "Comparison with at least one unconstructed "
...
static HashTable *TimeZone_get_debug_info(crex_object *object, int *is_temp)
{
	zval			zv;
	TimeZone_object	*to;
	const TimeZone	*tz;
	...
	*is_temp = 1;
	...
	to = crx_intl_timezone_fetch_object(object);
	tz = to->utimezone;

	if (tz == NULL) {
		ZVAL_FALSE(&zv);
		crex_hash_str_update(debug_info, "valid", sizeof("valid") - 1, &zv);
		return debug_info;
	}
	...
	crex_hash_str_update(debug_info, "id", sizeof("id") - 1, &zv);
	...
	ZVAL_LONG(&zv, (crex_long)rawOffset);
	crex_hash_str_update(debug_info,"rawOffset", sizeof("rawOffset") - 1, &zv);
	ZVAL_LONG(&zv, (crex_long)(rawOffset + dstOffset));
	crex_hash_str_update(debug_info,"currentOffset", sizeof("currentOffset") - 1, &zv);
 ...
 * Initialize internals of TImeZone_object not specific to crex standard objects.
 */
...
static void TimeZone_objects_free(crex_object *object)
{
	TimeZone_object* to = crx_intl_timezone_fetch_object(object);
	...
	crex_object_std_dtor(&to->zo);
}
/* }}} */

/* {{{ TimeZone_object_create */
static crex_object *TimeZone_object_create(crex_class_entry *ce)
{
