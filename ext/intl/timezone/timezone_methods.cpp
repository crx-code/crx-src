...
U_CFUNC CRX_METHOD(IntlTimeZone, __main)
{
...
U_CFUNC CRX_FUNCTION(intltz_create_time_zone)
{
	char	*str_id;
	size_t	 str_id_len;
	intl_error_reset(NULL);

	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &str_id, &str_id_len) == FAILURE) {
		RETURN_THROWS();
	}
...
U_CFUNC CRX_FUNCTION(intltz_from_date_time_zone)
{
	zval				*zv_timezone;
	TimeZone			*tz;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O",
			&zv_timezone, crx_date_get_timezone_ce()) == FAILURE) {
		RETURN_THROWS();
	}

	tzobj = C_CRXTIMEZONE_P(zv_timezone);
...
U_CFUNC CRX_FUNCTION(intltz_create_default)
{
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intltz_get_gmt)
{
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intltz_get_unknown)
{
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intltz_create_enumeration)
{
	zval				*arg = NULL;
	StringEnumeration	*se	  = NULL;
	intl_error_reset(NULL);

	/* double indirection to have the crex engine destroy the new zval that
	 * results from separation */
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|z", &arg) == FAILURE) {
		RETURN_THROWS();
	}

	if (arg == NULL || C_TYPE_P(arg) == IS_NULL) {
		se = TimeZone::createEnumeration();
	} else if (C_TYPE_P(arg) == IS_LONG) {
int_offset:
		if (UNEXPECTED(C_LVAL_P(arg) < (crex_long)INT32_MIN ||
				C_LVAL_P(arg) > (crex_long)INT32_MAX)) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
				"intltz_create_enumeration: value is out of range", 0);
			RETURN_FALSE;
		} else {
			se = TimeZone::createEnumeration((int32_t) C_LVAL_P(arg));
		}
	} else if (C_TYPE_P(arg) == IS_DOUBLE) {
double_offset:
		convert_to_long(arg);
		goto int_offset;
	} else if (C_TYPE_P(arg) == IS_OBJECT || C_TYPE_P(arg) == IS_STRING) {
		crex_long lval;
		double dval;
		if (!try_convert_to_string(arg)) {
			RETURN_THROWS();
		}
		switch (is_numeric_string(C_STRVAL_P(arg), C_STRLEN_P(arg), &lval, &dval, 0)) {
		case IS_DOUBLE:
			zval_ptr_dtor(arg);
		...
		se = TimeZone::createEnumeration(C_STRVAL_P(arg));
	} else {
		intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
...
U_CFUNC CRX_FUNCTION(intltz_count_equivalent_ids)
{
	char	*str_id;
	size_t	 str_id_len;
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intltz_create_time_zone_id_enumeration)
{
	crex_long zoneType,
			  offset_arg;
	char	 *region		= NULL;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "l|s!l!",
	...
	if (!arg3isnull) {
		if (UNEXPECTED(offset_arg < (crex_long)INT32_MIN || offset_arg > (crex_long)INT32_MAX)) {
			intl_error_set(NULL, U_ILLEGAL_ARGUMENT_ERROR,
...
U_CFUNC CRX_FUNCTION(intltz_get_canonical_id)
{
	char	*str_id;
	size_t	 str_id_len;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|z",
	...
	crex_string *u8str =intl_convert_utf16_to_utf8(result.getBuffer(), result.length(), &status);
...
U_CFUNC CRX_FUNCTION(intltz_get_region)
{
	char	*str_id;
	size_t	 str_id_len;
	char	 outbuf[3];
...
U_CFUNC CRX_FUNCTION(intltz_get_tz_data_version)
{
	intl_error_reset(NULL);
...
U_CFUNC CRX_FUNCTION(intltz_get_equivalent_id)
{
	char	   *str_id;
	size_t		str_id_len;
	crex_long	index;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sl", &str_id, &str_id_len, &index) == FAILURE) {
		RETURN_THROWS();
	}

	if (UNEXPECTED(index < (crex_long)INT32_MIN || index > (crex_long)INT32_MAX)) {
		RETURN_FALSE;
	}
...
U_CFUNC CRX_FUNCTION(intltz_get_id)
{
...
U_CFUNC CRX_FUNCTION(intltz_use_daylight_time)
{
	TIMEZONE_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intltz_get_offset)
{
	double		date;
	bool	local;
	zval		*rawOffsetArg,
...
U_CFUNC CRX_FUNCTION(intltz_get_raw_offset)
{
	TIMEZONE_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intltz_has_same_rules)
{
	zval			*other_object;
	...
	TIMEZONE_METHOD_FETCH_OBJECT;
	other_to = C_INTL_TIMEZONE_P(other_object);
...
U_CFUNC CRX_FUNCTION(intltz_get_display_name)
{
	bool	daylight		= 0;
	crex_long	display_type	= TimeZone::LONG;
	...
	crex_string *u8str = intl_convert_utf16_to_utf8(result.getBuffer(), result.length(), TIMEZONE_ERROR_CODE_P(to));
...
U_CFUNC CRX_FUNCTION(intltz_get_dst_savings)
{
	TIMEZONE_METHOD_INIT_VARS;
	...
	RETURN_LONG((crex_long)to->utimezone->getDSTSavings());
}

U_CFUNC CRX_FUNCTION(intltz_to_date_time_zone)
{
	zval tmp;
	TIMEZONE_METHOD_INIT_VARS;
...
U_CFUNC CRX_FUNCTION(intltz_get_error_code)
{
	TIMEZONE_METHOD_INIT_VARS
	...
	to = C_INTL_TIMEZONE_P(object);
	if (to == NULL)
		RETURN_FALSE;

	RETURN_LONG((crex_long)TIMEZONE_ERROR_CODE(to));
}

U_CFUNC CRX_FUNCTION(intltz_get_error_message)
{
 ...
 */
U_CFUNC CRX_FUNCTION(intltz_get_windows_id)
{
	crex_string *id, *winID;
	UnicodeString uID, uWinID;
	UErrorCode error;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S", &id) == FAILURE) {
		RETURN_THROWS();
	}
 ...
 */
U_CFUNC CRX_FUNCTION(intltz_get_id_for_windows_id)
{
	crex_string *winID, *region = NULL, *id;
	UnicodeString uWinID, uID;
	UErrorCode error;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S|S!", &winID, &region) == FAILURE) {
		RETURN_THROWS();
	}
