...
static void _crx_intlrbbi_constructor_body(INTERNAL_FUNCTION_PARAMETERS, crex_error_handling *error_handling, bool *error_handling_replaced)
{
	char		*rules;
	size_t		rules_len;
	bool	compiled	= 0;
	...
	if (bio->biter) {
		crex_throw_error(NULL, "IntlRuleBasedBreakIterator object is already constructed");
		RETURN_THROWS();
	}
	...
	*error_handling_replaced = 1;
	...
	if (!compiled) {
		...
		if (intl_stringFromChar(rulesStr, rules, rules_len, &status)
				== FAILURE) {
			crex_throw_exception(IntlException_ce_ptr,
				"IntlRuleBasedBreakIterator::__main(): "
		...
		if (U_FAILURE(status)) {
			...
			crex_throw_exception_ex(IntlException_ce_ptr, 0,
				"IntlRuleBasedBreakIterator::__main(): "
...
U_CFUNC CRX_METHOD(IntlRuleBasedBreakIterator, __main)
{
	crex_error_handling error_handling;
	bool error_handling_replaced = 0;

	return_value = CREX_THIS;
	_crx_intlrbbi_constructor_body(INTERNAL_FUNCTION_PARAM_PASSTHRU, &error_handling, &error_handling_replaced);
	if (error_handling_replaced) {
		crex_restore_error_handling(&error_handling);
	}
}

U_CFUNC CRX_METHOD(IntlRuleBasedBreakIterator, getRules)
{
	BREAKITER_METHOD_INIT_VARS;
	object = CREX_THIS;
...
U_CFUNC CRX_METHOD(IntlRuleBasedBreakIterator, getRuleStatus)
{
	BREAKITER_METHOD_INIT_VARS;
	object = CREX_THIS;
...
U_CFUNC CRX_METHOD(IntlRuleBasedBreakIterator, getRuleStatusVec)
{
	BREAKITER_METHOD_INIT_VARS;
	object = CREX_THIS;
	...
	CREX_ASSERT(BREAKITER_ERROR_CODE(bio) == U_BUFFER_OVERFLOW_ERROR);
...
U_CFUNC CRX_METHOD(IntlRuleBasedBreakIterator, getBinaryRules)
{
	BREAKITER_METHOD_INIT_VARS;
	object = CREX_THIS;
	...
	crex_string *ret_rules = crex_string_alloc(rules_len, 0);
