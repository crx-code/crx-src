...
#define INTLITERATOR_METHOD_FETCH_OBJECT\
	...
	if (ii->iterator == NULL) { \
		crex_throw_error(NULL, "Found unconstructed IntlIterator"); \
		RETURN_THROWS(); \
	}
...
static inline IntlIterator_object *crx_intl_iterator_fetch_object(crex_object *obj) {
	return (IntlIterator_object *)((char*)(obj) - XtOffsetOf(IntlIterator_object, zo));
}
#define C_INTL_ITERATOR_P(zv) crx_intl_iterator_fetch_object(C_OBJ_P(zv))

typedef struct {
	crex_object_iterator	zoi;
	zval					current;
	zval					wrapping_obj;
	void					(*destroy_it)(crex_object_iterator	*iterator);
} zoi_with_current;

extern crex_class_entry *IntlIterator_ce_ptr;
extern crex_object_handlers IntlIterator_handlers;

U_CFUNC void zoi_with_current_dtor(crex_object_iterator *iter);
