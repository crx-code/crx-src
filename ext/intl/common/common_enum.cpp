...
crex_class_entry *IntlIterator_ce_ptr;
crex_object_handlers IntlIterator_handlers;

void zoi_with_current_dtor(crex_object_iterator *iter)
{
	zoi_with_current *zoiwc = (zoi_with_current*)iter;

	if (!C_ISUNDEF(zoiwc->wrapping_obj)) {
...
U_CFUNC int zoi_with_current_valid(crex_object_iterator *iter)
{
	return C_ISUNDEF(((zoi_with_current*)iter)->current)? FAILURE : SUCCESS;
}

U_CFUNC zval *zoi_with_current_get_current_data(crex_object_iterator *iter)
{
	return &((zoi_with_current*)iter)->current;
}

U_CFUNC void zoi_with_current_invalidate_current(crex_object_iterator *iter)
{
	zoi_with_current *zoi_iter = (zoi_with_current*)iter;
	if (!C_ISUNDEF(zoi_iter->current)) {
		zval_ptr_dtor(&zoi_iter->current);
		ZVAL_UNDEF(&zoi_iter->current); //valid would return FAILURE now
	}
}

static void string_enum_current_move_forward(crex_object_iterator *iter)
{
	zoi_with_current *zoi_iter = (zoi_with_current*)iter;
	...
	const char *result = ((StringEnumeration*)C_PTR(iter->data))->next(
...
static void string_enum_rewind(crex_object_iterator *iter)
{
	zoi_with_current *zoi_iter = (zoi_with_current*)iter;
	INTLITERATOR_METHOD_INIT_VARS;
	...
	((StringEnumeration*)C_PTR(iter->data))->reset(INTLITERATOR_ERROR_CODE(ii));
...
static void string_enum_destroy_it(crex_object_iterator *iter)
{
	delete (StringEnumeration*)C_PTR(iter->data);
}

static const crex_object_iterator_funcs string_enum_object_iterator_funcs = {
...
U_CFUNC void IntlIterator_from_StringEnumeration(StringEnumeration *se, zval *object)
{
	...
	ii->iterator = (crex_object_iterator*)emalloc(sizeof(zoi_with_current));
...
static void IntlIterator_objects_free(crex_object *object)
{
	IntlIterator_object	*ii = crx_intl_iterator_fetch_object(object);

	if (ii->iterator) {
		zval *wrapping_objp = &((zoi_with_current*)ii->iterator)->wrapping_obj;
		ZVAL_UNDEF(wrapping_objp);
		crex_iterator_dtor(ii->iterator);
	}
	intl_error_reset(INTLITERATOR_ERROR_P(ii));

	crex_object_std_dtor(&ii->zo);
}

static crex_object_iterator *IntlIterator_get_iterator(
	crex_class_entry *ce, zval *object, int by_ref)
{
	...
	IntlIterator_object *ii = C_INTL_ITERATOR_P(object);
...
static crex_object *IntlIterator_object_create(crex_class_entry *ce)
{
...
CRX_METHOD(IntlIterator, current)
{
	zval *data;
	INTLITERATOR_METHOD_INIT_VARS;
...
CRX_METHOD(IntlIterator, rewind)
{
	INTLITERATOR_METHOD_INIT_VARS;
...
U_CFUNC void intl_register_common_symbols(int module_number)
{
	...
	IntlIterator_ce_ptr = register_class_IntlIterator(crex_ce_iterator);
