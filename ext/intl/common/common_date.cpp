												...
												const char *func)
{
	...
	switch (type) {
		case TIMELIB_ZONETYPE_ID:
			id = is_datetime
				? ((crx_date_obj*)object)->time->tz_info->name
				: ((crx_timezone_obj*)object)->tzi.tz->name;
			id_len = strlen(id);
			break;
		case TIMELIB_ZONETYPE_OFFSET: {
			int offset_mins = is_datetime
				? ((crx_date_obj*)object)->time->z / 60
				: (int)((crx_timezone_obj*)object)->tzi.utc_offset / 60,
		...
		case TIMELIB_ZONETYPE_ABBR:
			id = is_datetime
				? ((crx_date_obj*)object)->time->tz_abbr
				: ((crx_timezone_obj*)object)->tzi.z.abbr;
			id_len = strlen(id);
			break;
	}
		...
		intl_error *err, const char *func)
{
	...
	if (millis) {
		...
		datetime = C_CRXDATE_P(z);
		*millis = U_MILLIS_PER_SECOND * (double)C_LVAL(retval) + (datetime->time->us / 1000);
		zval_ptr_dtor(&zfuncname);
	}
	...
	if (tz) {
		...
		if (!datetime->time) {
			spprintf(&message, 0, "%s: the %s object is not properly "
					"initialized", func, ZSTR_VAL(C_OBJCE_P(z)->name));
...
try_again:
	switch (C_TYPE_P(z)) {
	case IS_STRING:
		type = is_numeric_string(C_STRVAL_P(z), C_STRLEN_P(z), &lv, &rv, 0);
		if (type == IS_DOUBLE) {
		...
		break;
	case IS_LONG:
		rv = U_MILLIS_PER_SECOND * (double)C_LVAL_P(z);
		break;
	case IS_DOUBLE:
		rv = U_MILLIS_PER_SECOND * C_DVAL_P(z);
		break;
	case IS_OBJECT:
		if (instanceof_function(C_OBJCE_P(z), crx_date_get_interface_ce())) {
			intl_datetime_decompose(z, &rv, NULL, err, func);
		} else if (instanceof_function(C_OBJCE_P(z), Calendar_ce_ptr)) {
			Calendar_object *co = C_INTL_CALENDAR_P(z);
			if (co->ucal == NULL) {
	...
	case IS_REFERENCE:
		...
		goto try_again;
	default:
		spprintf(&message, 0, "%s: invalid CRX type for date", func);
		intl_errors_set(err, U_ILLEGAL_ARGUMENT_ERROR,
			message, 1);
		efree(message);
		break;
