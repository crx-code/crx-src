...
CRX_FUNCTION(grapheme_strlen)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &string, &string_len) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(grapheme_strpos)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss|l", &haystack, &haystack_len, &needle, &needle_len, &loffset) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (offset >= 0 && grapheme_ascii_check((unsigned char *)haystack, haystack_len) >= 0) {
		...
		*/
		found = crx_memnstr(haystack + noffset, needle, needle_len, haystack + haystack_len);
...
CRX_FUNCTION(grapheme_stripos)
{
	...
	if ( is_ascii ) {
		...
		crex_str_tolower(needle_dup, needle_len);
		haystack_dup = estrndup(haystack, haystack_len);
		crex_str_tolower(haystack_dup, haystack_len);

		found = crx_memnstr(haystack_dup + noffset, needle_dup, needle_len, haystack_dup + haystack_len);
...
CRX_FUNCTION(grapheme_strripos)
{
	char *haystack, *needle;
	size_t haystack_len, needle_len;
	crex_long loffset = 0;
...
CRX_FUNCTION(grapheme_substr)
{
	char *str;
	crex_string *u8_sub_str;
	UChar *ustr;
	size_t str_len;
	int32_t ustr_len;
	crex_long lstart = 0, length = 0;
	int32_t start = 0;
	int iter_val;
	UErrorCode status;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sl|l!", &str, &str_len, &lstart, &length, &no_length) == FAILURE) {
		RETURN_THROWS();
	}
...
static void strstr_common_handler(INTERNAL_FUNCTION_PARAMETERS, int f_ignore_case)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "ss|b", &haystack, &haystack_len, &needle, &needle_len, &part) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if ( !f_ignore_case ) {
		...
		found = crx_memnstr(haystack, needle, needle_len, haystack + haystack_len);
...
CRX_FUNCTION(grapheme_extract)
{
	char *str, *pstr;
	UText ut = UTEXT_INITIALIZER;
	size_t str_len;
	crex_long size; /* maximum number of grapheme clusters, bytes, or characters (based on extract_type) to return */
	crex_long lstart = 0; /* starting position in str in bytes */
	int32_t start = 0;
	crex_long extract_type = GRAPHEME_EXTRACT_TYPE_COUNT;
	UErrorCode status;
	unsigned char u_break_iterator_buffer[U_BRK_SAFECLONE_BUFFERSIZE];
	UBreakIterator* bi = NULL;
	int ret_pos;
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sl|llz", &str, &str_len, &size, &extract_type, &lstart, &next) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if ( extract_type < GRAPHEME_EXTRACT_TYPE_MIN || extract_type > GRAPHEME_EXTRACT_TYPE_MAX ) {
		crex_argument_value_error(3, "must be one of GRAPHEME_EXTR_COUNT, GRAPHEME_EXTR_MAXBYTES, or GRAPHEME_EXTR_MAXCHARS");
		RETURN_THROWS();
	}
