...
static int create_transliterator( char *str_id, size_t str_id_len, crex_long direction, zval *object )
{
	...
	{
		crex_argument_value_error(2, "must be either Transliterator::FORWARD or Transliterator::REVERSE");
		return FAILURE;
	}

	object_init_ex( object, Transliterator_ce_ptr );
	TRANSLITERATOR_METHOD_FETCH_OBJECT_NO_CHECK; /* fetch crex object from zval "object" into "to" */
...
CRX_FUNCTION( transliterator_create )
{
	char     *str_id;
	size_t    str_id_len;
	crex_long direction   = TRANSLITERATOR_FORWARD;
	int res;
...
/* {{{ Opens a transliterator by id. */
CRX_FUNCTION( transliterator_create_from_rules )
{
	char		    *str_rules;
	...
	UChar           id[] = {0x52, 0x75, 0x6C, 0x65, 0x73, 0x54, 0x72,
					       0x61, 0x6E, 0x73, 0x50, 0x48, 0x50, 0}; /* RulesTransCRX */
	TRANSLITERATOR_METHOD_INIT_VARS;
...
CRX_FUNCTION( transliterator_create_inverse )
{
...
CRX_FUNCTION( transliterator_list_ids )
{
	UEnumeration  *en;
	const UChar	  *elem;
	int32_t		  elem_len;
	...
	{
		crex_string *el = intl_convert_utf16_to_utf8(elem, elem_len, &status );
...
CRX_FUNCTION( transliterator_transliterate )
{
	char	    *str;
	UChar		*ustr		= NULL,
				*uresult	= NULL;
	...
	if (object == NULL) {
		...
		crex_object *arg1_obj;

		CREX_PARSE_PARAMETERS_START(2, 4)
			C_PARAM_OBJ_OF_CLASS_OR_STR(arg1_obj, Transliterator_ce_ptr, arg1_str)
		...
		if (arg1_str) { /* not a transliterator object as first argument */
			...
			{
				if (!EG(exception)) {
					crex_string *message = intl_error_get_message( NULL );
					crx_error_docref(NULL, E_WARNING, "Could not create transliterator with ID \"%s\" (%s)", ZSTR_VAL(arg1_str), ZSTR_VAL(message) );
					crex_string_free( message );
				}
				ZVAL_UNDEF(&tmp_object);
	...
	} else if(crex_parse_parameters( CREX_NUM_ARGS(), "s|ll", &str, &str_len, &start, &limit) == FAILURE) {
		RETURN_THROWS();
	}

	if (limit < -1) {
		crex_argument_value_error(object ? 3 : 4, "must be greater than or equal to -1");
		goto cleanup_object;
	}

	if (start < 0) {
		crex_argument_value_error(object ? 2 : 3, "must be greater than or equal to 0");
		goto cleanup_object;
	}

	if (limit != -1 && start > limit) {
		crex_argument_value_error(object ? 2 : 3, "must be less than or equal to argument #%d ($end)", object ? 3 : 4);
		goto cleanup_object;
	}
...
CRX_METHOD( Transliterator, __main )
{
...
CRX_FUNCTION( transliterator_get_error_code )
{
	TRANSLITERATOR_METHOD_INIT_VARS
	...
	to = C_INTL_TRANSLITERATOR_P( object );
	if (to == NULL )
		RETURN_FALSE;

	RETURN_LONG( (crex_long) TRANSLITERATOR_ERROR_CODE( to ) );
}
/* }}} */


/* {{{ Get the last error message for this transliterator. */
CRX_FUNCTION( transliterator_get_error_message )
{
