...
crex_class_entry *Transliterator_ce_ptr = NULL;

crex_object_handlers Transliterator_handlers;
									 ...
									 UErrorCode *status )
{
	...
	crex_update_property(Transliterator_ce_ptr, C_OBJ_P(object),
		"id", sizeof( "id" ) - 1, &tmp );
	GC_DELREF(u8str);
	return SUCCESS;
}
...
static void Transliterator_objects_free( crex_object *object )
{
	Transliterator_object* to = crx_intl_transliterator_fetch_object(object);
...
static crex_object *Transliterator_object_create( crex_class_entry *ce )
{
	Transliterator_object* intern;

	intern = crex_object_alloc(sizeof(Transliterator_object), ce);
...
static crex_object *Transliterator_clone_obj( crex_object *object )
{
	...
	to_orig = crx_intl_transliterator_fetch_object( object );
	intl_error_reset( INTL_DATA_ERROR_P( to_orig ) );
	ret_val = Transliterator_ce_ptr->create_object( object->ce );
	to_new  = crx_intl_transliterator_fetch_object( ret_val );
	...
	if( to_orig->utrans != NULL )
	{
		...
		if( U_FAILURE( TRANSLITERATOR_ERROR_CODE( to_orig ) ) ) {
			...
			crex_throw_error( NULL, "%s", ZSTR_VAL(err_msg) );
			crex_string_free( err_msg ); /* if it's changed into a warning */
		} else {
	...
	else
	{
		...
		crex_throw_error(NULL, "Unconstructed Transliterator object cannot be cloned");
	}
