...
static void resourcebundle_iterator_read( ResourceBundle_iterator *iterator )
{
	...
	if (U_SUCCESS(icuerror)) {
		...
		// crex_throw_exception( spl_ce_OutOfRangeException, "Running past end of ResourceBundle", 0);
...
static void resourcebundle_iterator_invalidate( crex_object_iterator *iter )
{
...
static void resourcebundle_iterator_dtor( crex_object_iterator *iter )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
...
static int resourcebundle_iterator_has_more( crex_object_iterator *iter )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
...
static zval *resourcebundle_iterator_current( crex_object_iterator *iter )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
	if (C_ISUNDEF(iterator->current)) {
...
static void resourcebundle_iterator_key( crex_object_iterator *iter, zval *key )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
...
static void resourcebundle_iterator_step( crex_object_iterator *iter )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
...
static void resourcebundle_iterator_reset( crex_object_iterator *iter )
{
	ResourceBundle_iterator *iterator = (ResourceBundle_iterator *) iter;
...
static const crex_object_iterator_funcs resourcebundle_iterator_funcs = {
	resourcebundle_iterator_dtor,
	resourcebundle_iterator_has_more,
	resourcebundle_iterator_current,
	resourcebundle_iterator_key,
...
crex_object_iterator *resourcebundle_get_iterator( crex_class_entry *ce, zval *object, int byref )
{
	...
	if (byref) {
	     crx_error( E_ERROR, "ResourceBundle does not support writable iterators" );
	}
	 ...
	 * All other types are returned as crx primitives!
	 */
