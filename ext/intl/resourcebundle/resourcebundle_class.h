...
static inline ResourceBundle_object *crx_intl_resourcebundle_fetch_object(crex_object *obj) {
	return (ResourceBundle_object *)((char*)(obj) - XtOffsetOf(ResourceBundle_object, crex));
}
#define C_INTL_RESOURCEBUNDLE_P(zv) crx_intl_resourcebundle_fetch_object(C_OBJ_P(zv))
...
#define RESOURCEBUNDLE_METHOD_FETCH_OBJECT							\
	...
	if (RESOURCEBUNDLE_OBJECT(rb) == NULL) {						\
		crex_throw_error(NULL, "Found unconstructed ResourceBundle");	\
		RETURN_THROWS();											\
	}
...
extern crex_class_entry *ResourceBundle_ce_ptr;
