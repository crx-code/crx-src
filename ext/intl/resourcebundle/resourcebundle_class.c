...
crex_class_entry *ResourceBundle_ce_ptr = NULL;

static crex_object_handlers ResourceBundle_object_handlers;

/* {{{ ResourceBundle_object_free */
static void ResourceBundle_object_free( crex_object *object )
{
	ResourceBundle_object *rb = crx_intl_resourcebundle_fetch_object(object);
	...
	crex_object_std_dtor( &rb->crex );
}
/* }}} */

/* {{{ ResourceBundle_object_create */
static crex_object *ResourceBundle_object_create( crex_class_entry *ce )
{
	ResourceBundle_object *rb;

	rb = crex_object_alloc(sizeof(ResourceBundle_object), ce);

	crex_object_std_init( &rb->crex, ce );
	object_properties_init( &rb->crex, ce);
...
/* {{{ ResourceBundle_ctor */
static int resourcebundle_ctor(INTERNAL_FUNCTION_PARAMETERS, crex_error_handling *error_handling, bool *error_handling_replaced)
{
	...
	ResourceBundle_object *rb = C_INTL_RESOURCEBUNDLE_P( object );

	intl_error_reset( NULL );

	if( crex_parse_parameters( CREX_NUM_ARGS(), "s!s!|b",
	...
	if (rb->me) {
		crex_throw_error(NULL, "ResourceBundle object is already constructed");
		return FAILURE;
	}
...
CRX_METHOD( ResourceBundle, __main )
{
...
/* {{{ */
CRX_FUNCTION( resourcebundle_create )
{
...
static void resourcebundle_array_fetch(crex_object *object, zval *offset, zval *return_value, int fallback)
{
	int32_t     meindex = 0;
	char *      mekey = NULL;
	bool        is_numeric = 0;
	...
	rb = crx_intl_resourcebundle_fetch_object(object);
	intl_error_reset(NULL);
	intl_error_reset(INTL_DATA_ERROR_P(rb));

	if(C_TYPE_P(offset) == IS_LONG) {
		is_numeric = 1;
		meindex = (int32_t)C_LVAL_P(offset);
		rb->child = ures_getByIndex( rb->me, meindex, rb->child, &INTL_DATA_ERROR_CODE(rb) );
	} else if(C_TYPE_P(offset) == IS_STRING) {
		mekey = C_STRVAL_P(offset);
...
zval *resourcebundle_array_get(crex_object *object, zval *offset, int type, zval *rv)
{
	if(offset == NULL) {
		crx_error( E_ERROR, "Cannot apply [] to ResourceBundle object" );
	}
	ZVAL_NULL(rv);
...
CRX_FUNCTION( resourcebundle_get )
{
	bool   fallback = 1;
	zval *		offset;
	zval *      object;

	if (crex_parse_method_parameters(CREX_NUM_ARGS(), getThis(), "Oz|b",	&object, ResourceBundle_ce_ptr, &offset, &fallback ) == FAILURE) {
		RETURN_THROWS();
	}

	resourcebundle_array_fetch(C_OBJ_P(object), offset, return_value, fallback);
}
/* }}} */

/* {{{ resourcebundle_array_count */
static crex_result resourcebundle_array_count(crex_object *object, crex_long *count)
{
	ResourceBundle_object *rb = crx_intl_resourcebundle_fetch_object(object);
...
/* {{{ Get resources count */
CRX_FUNCTION( resourcebundle_count )
{
	int32_t                len;
	RESOURCEBUNDLE_METHOD_INIT_VARS;

	if( crex_parse_method_parameters( CREX_NUM_ARGS(), getThis(), "O", &object, ResourceBundle_ce_ptr ) == FAILURE ) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION( resourcebundle_locales )
{
	char * bundlename;
	size_t    bundlename_len = 0;
	const char * entry;
	...
	if( crex_parse_parameters(CREX_NUM_ARGS(), "s", &bundlename, &bundlename_len ) == FAILURE )
	{
		RETURN_THROWS();
	}
...
CRX_FUNCTION( resourcebundle_get_error_code )
{
	RESOURCEBUNDLE_METHOD_INIT_VARS;
	...
	rb = C_INTL_RESOURCEBUNDLE_P( object );
...
CRX_FUNCTION( resourcebundle_get_error_message )
{
...
CRX_METHOD(ResourceBundle, getIterator) {
...
{
	ResourceBundle_ce_ptr = register_class_ResourceBundle(crex_ce_aggregate, crex_ce_countable);
	...
	ResourceBundle_object_handlers.offset = XtOffsetOf(ResourceBundle_object, crex);
