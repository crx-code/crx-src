...
CREX_DECLARE_MODULE_GLOBALS( intl )
...
/* {{{ INI Settings */
CRX_INI_BEGIN()
	STD_CRX_INI_ENTRY(LOCALE_INI_NAME, NULL, CRX_INI_ALL, OnUpdateStringUnempty, default_locale, crex_intl_globals, intl_globals)
	STD_CRX_INI_ENTRY("intl.error_level", "0", CRX_INI_ALL, OnUpdateLong, error_level, crex_intl_globals, intl_globals)
	STD_CRX_INI_BOOLEAN("intl.use_exceptions", "0", CRX_INI_ALL, OnUpdateBool, use_exceptions, crex_intl_globals, intl_globals)
CRX_INI_END()
/* }}} */

static CRX_GINIT_FUNCTION(intl);

/* {{{ intl_module_entry */
crex_module_entry intl_module_entry = {
	...
	CRX_MINFO( intl ),
	CRX_INTL_VERSION,
	CRX_MODULE_GLOBALS(intl),   /* globals descriptor */
	CRX_GINIT(intl),            /* globals ctor */
	NULL,                       /* globals dtor */
...
#endif
	memset( intl_globals, 0, sizeof(crex_intl_globals) );
}
/* }}} */
...
{
	/* For the default locale crx.ini setting */
	REGISTER_INI_ENTRIES();

	register_crx_intl_symbols(module_number);

	/* Register collator symbols and classes */
	collator_register_Collator_symbols(module_number);

	/* Register 'NumberFormatter' CRX class */
	formatter_register_class(  );

	/* Register 'Normalizer' CRX class */
	normalizer_register_Normalizer_class(  );

	/* Register 'Locale' CRX class */
	locale_register_Locale_class(  );

	msgformat_register_class();

	/* Register 'DateFormat' CRX class */
	dateformat_register_IntlDateFormatter_class(  );

	/* Register 'IntlDateTimeFormatter' CRX class */
	dateformat_register_IntlDatePatternGenerator_class(  );

	/* Register 'ResourceBundle' CRX class */
	resourcebundle_register_class( );

	/* Register 'Transliterator' CRX class */
	transliterator_register_Transliterator_class(  );

	/* Register 'IntlTimeZone' CRX class */
	timezone_register_IntlTimeZone_class(  );

	/* Register 'IntlCalendar' CRX class */
	calendar_register_IntlCalendar_class(  );

	/* Register 'Spoofchecker' CRX class */
	spoofchecker_register_Spoofchecker_class(  );

	/* Register 'IntlException' CRX class */
	IntlException_ce_ptr = register_class_IntlException(crex_ce_exception);
	IntlException_ce_ptr->create_object = crex_ce_exception->create_object;
	...
	crx_converter_minit(INIT_FUNC_ARGS_PASSTHRU);

	/* IntlChar class */
	crx_uchar_minit(INIT_FUNC_ARGS_PASSTHRU);
...
CRX_MINFO_FUNCTION( intl )
{
	...
	crx_info_print_table_row( 2, "Internationalization support", "enabled" );
	crx_info_print_table_row( 2, "ICU version", U_ICU_VERSION );
#ifdef U_ICU_DATA_VERSION
	crx_info_print_table_row( 2, "ICU Data version", U_ICU_DATA_VERSION );
#endif
#if !UCONFIG_NO_FORMATTING
	tzdata_ver = ucal_getTZDataVersion(&status);
	if (U_ZERO_ERROR == status) {
		crx_info_print_table_row( 2, "ICU TZData version", tzdata_ver);
	}
#endif
	crx_info_print_table_row( 2, "ICU Unicode version", U_UNICODE_VERSION );
