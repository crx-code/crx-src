 ...
 * Essentially, it is set if you HAVE_FORK || CRX_WIN32
...
static crx_process_env _crx_array_to_envp(zval *environment)
{
	zval *element;
	crx_process_env env;
	crex_string *key, *str;
	...
	char *p;
	size_t sizeenv = 0;
	HashTable *env_hash; /* temporary CRX array used as helper */
	...
	uint32_t cnt = crex_hash_num_elements(C_ARRVAL_P(environment));
	...
	crex_hash_init(env_hash, cnt, NULL, NULL, 0);

	/* first, we have to get the size of all the elements in the hash */
	CREX_HASH_FOREACH_STR_KEY_VAL(C_ARRVAL_P(environment), key, element) {
		...
		if (key && ZSTR_LEN(key)) {
			...
			crex_hash_add_ptr(env_hash, key, str);
		} else {
			crex_hash_next_index_insert_ptr(env_hash, str);
		}
	...
	CREX_HASH_FOREACH_STR_KEY_PTR(env_hash, key, str) {
#ifndef CRX_WIN32
		*ep = p;
		++ep;
#endif
...
/* {{{ _crx_free_envp
 * Free the structures allocated by `_crx_array_to_envp` */
static void _crx_free_envp(crx_process_env env)
{
#ifndef CRX_WIN32
	if (env.envarray) {
		efree(env.envarray);
...
#if HAVE_SYS_WAIT_H
static pid_t waitpid_cached(crx_process_handle *proc, int *wait_status, int options)
{
...
static void proc_open_rsrc_dtor(crex_resource *rsrc)
{
	crx_process_handle *proc = (crx_process_handle*)rsrc->ptr;
#ifdef CRX_WIN32
	DWORD wstatus;
	...
	_crx_free_envp(proc->env);
	efree(proc->pipes);
	crex_string_release_ex(proc->command, false);
	efree(proc);
}
/* }}} */
...
CRX_MINIT_FUNCTION(proc_open)
{
	le_proc_open = crex_register_list_destructors_ex(proc_open_rsrc_dtor, NULL, "process",
		module_number);
	return SUCCESS;
}
/* }}} */
...
CRX_FUNCTION(proc_terminate)
{
	zval *zproc;
	crx_process_handle *proc;
	crex_long sig_no = SIGTERM;

	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_RESOURCE(zproc)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(sig_no)
	CREX_PARSE_PARAMETERS_END();

	proc = (crx_process_handle*)crex_fetch_resource(C_RES_P(zproc), "process", le_proc_open);
	if (proc == NULL) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(proc_get_status)
{
	...
	add_assoc_str(return_value, "command", crex_string_copy(proc->command));
	add_assoc_long(return_value, "pid", (crex_long)proc->child);
 ...
 * never changed */
SECURITY_ATTRIBUTES crx_proc_open_security = {
	.nLength = sizeof(SECURITY_ATTRIBUTES),
	.lpSecurityDescriptor = NULL,
	.bInheritHandle = TRUE
};

# define pipe(pair)		(CreatePipe(&pair[0], &pair[1], &crx_proc_open_security, 0) ? 0 : -1)
...
typedef struct _descriptorspec_item {
	...
	crx_file_descriptor_t childend;  /* FD # opened for use in child
	                                  * (will be copied to `index` in child) */
	crx_file_descriptor_t parentend; /* FD # opened for use in parent
	                                  * (for pipes only; will be 0 otherwise) */
	int mode_flags;                  /* mode for opening FDs: r/o, r/w, binary (on Win32), etc */
} descriptorspec_item;

static crex_string *get_valid_arg_string(zval *zv, int elem_num) {
	crex_string *str = zval_get_string(zv);
	if (!str) {
		return NULL;
	}

	if (elem_num == 1 && ZSTR_LEN(str) == 0) {
		crex_value_error("First element must contain a non-empty program name");
		crex_string_release(str);
		return NULL;
	}

	if (strlen(ZSTR_VAL(str)) != ZSTR_LEN(str)) {
		crex_value_error("Command array element %d contains a null byte", elem_num);
		crex_string_release(str);
		return NULL;
	}
...
static void append_win_escaped_arg(smart_str *str, crex_string *arg)
{
	size_t num_bs = 0;
...
static crex_string *create_win_command_from_args(HashTable *args)
{
	smart_str str = {0};
	zval *arg_zv;
	bool is_prog_name = 1;
	...
	CREX_HASH_FOREACH_VAL(args, arg_zv) {
		crex_string *arg_str = get_valid_arg_string(arg_zv, ++elem_num);
		if (!arg_str) {
			smart_str_free(&str);
			return NULL;
		}
...
{
	HashTable *opt_ary = C_ARRVAL_P(other_options);
	zval *item = crex_hash_str_find_deref(opt_ary, opt_name, opt_name_len);
	return item != NULL &&
		(C_TYPE_P(item) == IS_TRUE ||
		((C_TYPE_P(item) == IS_LONG) && C_LVAL_P(item)));
}
...
static crex_result convert_command_to_use_shell(wchar_t **cmdw, size_t cmdw_len)
{
	size_t len = sizeof(COMSPEC_NT) + sizeof(" /s /c ") + cmdw_len + 3;
	wchar_t *cmdw_shell = (wchar_t *)malloc(len * sizeof(wchar_t));

	if (cmdw_shell == NULL) {
		crx_error_docref(NULL, E_WARNING, "Command conversion failed");
		return FAILURE;
	}
...
static crex_string* get_command_from_array(HashTable *array, char ***argv, int num_elems)
{
	zval *arg_zv;
	crex_string *command = NULL;
	int i = 0;

	*argv = safe_emalloc(sizeof(char *), num_elems + 1, 0);

	CREX_HASH_FOREACH_VAL(array, arg_zv) {
		crex_string *arg_str = get_valid_arg_string(arg_zv, i + 1);
		if (!arg_str) {
			/* Terminate with NULL so exit_fail code knows how many entries to free */
			(*argv)[i] = NULL;
			if (command != NULL) {
				crex_string_release_ex(command, false);
			}
			return NULL;
		}

		if (i == 0) {
			command = crex_string_copy(arg_str);
		}
...
{
	uint32_t ndescriptors = crex_hash_num_elements(descriptorspec);
	return ecalloc(sizeof(descriptorspec_item), ndescriptors);
}

static crex_string* get_string_parameter(zval *array, int index, char *param_name)
{
	zval *array_item;
	if ((array_item = crex_hash_index_find(C_ARRVAL_P(array), index)) == NULL) {
		crex_value_error("Missing %s", param_name);
		return NULL;
	}
	return zval_try_get_string(array_item);
}

static crex_result set_proc_descriptor_to_blackhole(descriptorspec_item *desc)
{
#ifdef CRX_WIN32
	desc->childend = CreateFileA("nul", GENERIC_READ | GENERIC_WRITE,
		FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);
	if (desc->childend == NULL) {
		crx_error_docref(NULL, E_WARNING, "Failed to open nul");
		return FAILURE;
	}
#else
	desc->childend = open("/dev/null", O_RDWR);
	if (desc->childend < 0) {
		crx_error_docref(NULL, E_WARNING, "Failed to open /dev/null: %s", strerror(errno));
		return FAILURE;
	}
#endif
	return SUCCESS;
...
static crex_result set_proc_descriptor_to_pty(descriptorspec_item *desc, int *master_fd, int *slave_fd)
{
#ifdef HAVE_OPENPTY
	...
	if (*master_fd == -1) {
		if (openpty(master_fd, slave_fd, NULL, NULL, NULL)) {
			crx_error_docref(NULL, E_WARNING, "Could not open PTY (pseudoterminal): %s", strerror(errno));
			return FAILURE;
		}
	}
	...
	return SUCCESS;
#else
	crx_error_docref(NULL, E_WARNING, "PTY (pseudoterminal) not supported on this system");
	return FAILURE;
#endif
}

/* Mark the descriptor close-on-exec, so it won't be inherited by children */
static crx_file_descriptor_t make_descriptor_cloexec(crx_file_descriptor_t fd)
{
#ifdef CRX_WIN32
...
static crex_result set_proc_descriptor_to_pipe(descriptorspec_item *desc, crex_string *zmode)
{
	crx_file_descriptor_t newpipe[2];

	if (pipe(newpipe)) {
		crx_error_docref(NULL, E_WARNING, "Unable to create pipe %s", strerror(errno));
		return FAILURE;
	}

	desc->type = DESCRIPTOR_TYPE_PIPE;

	if (!crex_string_starts_with_literal(zmode, "w")) {
...
static crex_result set_proc_descriptor_to_socket(descriptorspec_item *desc)
{
	crx_socket_t sock[2];

	if (create_socketpair(sock)) {
		crex_string *err = crx_socket_error_str(crx_socket_errno());
		crx_error_docref(NULL, E_WARNING, "Unable to create socket pair: %s", ZSTR_VAL(err));
		crex_string_release(err);
		return FAILURE;
	}

	desc->type = DESCRIPTOR_TYPE_SOCKET;
	desc->parentend = make_descriptor_cloexec((crx_file_descriptor_t) sock[0]);

	/* Pass sock[1] to child because it will never use overlapped IO on Windows. */
	desc->childend = (crx_file_descriptor_t) sock[1];

	return SUCCESS;
}

static crex_result set_proc_descriptor_to_file(descriptorspec_item *desc, crex_string *file_path,
	crex_string *file_mode)
{
	crx_socket_t fd;

	/* try a wrapper */
	crx_stream *stream = crx_stream_open_wrapper(ZSTR_VAL(file_path), ZSTR_VAL(file_mode),
	...
	/* force into an fd */
	if (crx_stream_cast(stream, CRX_STREAM_CAST_RELEASE|CRX_STREAM_AS_FD, (void **)&fd,
...
static crex_result dup_proc_descriptor(crx_file_descriptor_t from, crx_file_descriptor_t *to,
	crex_ulong nindex)
{
#ifdef CRX_WIN32
	*to = dup_handle(from, TRUE, FALSE);
	if (*to == NULL) {
		crx_error_docref(NULL, E_WARNING, "Failed to dup() for descriptor " CREX_LONG_FMT, nindex);
		return FAILURE;
	}
#else
	*to = dup(from);
	if (*to < 0) {
		crx_error_docref(NULL, E_WARNING, "Failed to dup() for descriptor " CREX_LONG_FMT ": %s",
...
static crex_result redirect_proc_descriptor(descriptorspec_item *desc, int target,
	descriptorspec_item *descriptors, int ndesc, int nindex)
{
	crx_file_descriptor_t redirect_to = CRX_INVALID_FD;
	...
	if (redirect_to == CRX_INVALID_FD) { /* Didn't find the index we wanted */
		if (target < 0 || target > 2) {
			crx_error_docref(NULL, E_WARNING, "Redirection target %d not found", target);
			return FAILURE;
		}
...
static crex_result set_proc_descriptor_from_array(zval *descitem, descriptorspec_item *descriptors,
	int ndesc, int nindex, int *pty_master_fd, int *pty_slave_fd) {
	crex_string *ztype = get_string_parameter(descitem, 0, "handle qualifier");
	if (!ztype) {
		return FAILURE;
	}

	crex_string *zmode = NULL, *zfile = NULL;
	crex_result retval = FAILURE;

	if (crex_string_equals_literal(ztype, "pipe")) {
	...
	} else if (crex_string_equals_literal(ztype, "socket")) {
		/* Set descriptor to socketpair */
		retval = set_proc_descriptor_to_socket(&descriptors[ndesc]);
	} else if (crex_string_equals(ztype, ZSTR_KNOWN(CREX_STR_FILE))) {
	...
	} else if (crex_string_equals_literal(ztype, "redirect")) {
		/* Redirect descriptor to whatever another descriptor is set to */
		zval *ztarget = crex_hash_index_find_deref(C_ARRVAL_P(descitem), 1);
		if (!ztarget) {
			crex_value_error("Missing redirection target");
			goto finish;
		}
		if (C_TYPE_P(ztarget) != IS_LONG) {
			crex_value_error("Redirection target must be of type int, %s given", crex_zval_value_name(ztarget));
			goto finish;
		}

		retval = redirect_proc_descriptor(
			&descriptors[ndesc], (int)C_LVAL_P(ztarget), descriptors, ndesc, nindex);
	} else if (crex_string_equals(ztype, ZSTR_KNOWN(CREX_STR_NULL_LOWERCASE))) {
		/* Set descriptor to blackhole (discard all data written) */
		retval = set_proc_descriptor_to_blackhole(&descriptors[ndesc]);
	} else if (crex_string_equals_literal(ztype, "pty")) {
		/* Set descriptor to slave end of PTY */
		retval = set_proc_descriptor_to_pty(&descriptors[ndesc], pty_master_fd, pty_slave_fd);
	} else {
		crx_error_docref(NULL, E_WARNING, "%s is not a valid descriptor spec/mode", ZSTR_VAL(ztype));
		goto finish;
	}

finish:
	if (zmode) crex_string_release(zmode);
	if (zfile) crex_string_release(zfile);
	crex_string_release(ztype);
	return retval;
}

static crex_result set_proc_descriptor_from_resource(zval *resource, descriptorspec_item *desc, int nindex)
{
	/* Should be a stream - try and dup the descriptor */
	crx_stream *stream = (crx_stream*)crex_fetch_resource(C_RES_P(resource), "stream",
		crx_file_le_stream());
	if (stream == NULL) {
		return FAILURE;
	}
	...
	crx_socket_t fd;
	crex_result status = crx_stream_cast(stream, CRX_STREAM_AS_FD, (void **)&fd, REPORT_ERRORS);
	if (status == FAILURE) {
		return FAILURE;
	}

#ifdef CRX_WIN32
	crx_file_descriptor_t fd_t = (crx_file_descriptor_t)_get_osfhandle(fd);
#else
	crx_file_descriptor_t fd_t = fd;
#endif
...
static crex_result close_parentends_of_pipes(posix_spawn_file_actions_t * actions, descriptorspec_item *descriptors, int ndesc)
{
	int r;
	for (int i = 0; i < ndesc; i++) {
		if (descriptors[i].type != DESCRIPTOR_TYPE_STD) {
			...
			if (r != 0) {
				crx_error_docref(NULL, E_WARNING, "Cannot close file descriptor %d: %s", descriptors[i].parentend, strerror(r));
				return FAILURE;
			}
		}
		if (descriptors[i].childend != descriptors[i].index) {
			...
			if (r != 0) {
				crx_error_docref(NULL, E_WARNING, "Unable to copy file descriptor %d (for pipe) into "
			...
			if (r != 0) {
				crx_error_docref(NULL, E_WARNING, "Cannot close file descriptor %d: %s", descriptors[i].childend, strerror(r));
				return FAILURE;
			}
		}
	}
...
#else
static crex_result close_parentends_of_pipes(descriptorspec_item *descriptors, int ndesc)
{
...
CRX_FUNCTION(proc_open)
{
	crex_string *command_str;
	...
	CREX_PARSE_PARAMETERS_START(3, 6)
		C_PARAM_ARRAY_HT_OR_STR(command_ht, command_str)
		C_PARAM_ARRAY_HT(descriptorspec)
		C_PARAM_ZVAL(pipes)
		C_PARAM_OPTIONAL
		C_PARAM_STRING_OR_NULL(cwd, cwd_len)
		C_PARAM_ARRAY_OR_NULL(environment)
		C_PARAM_ARRAY_OR_NULL(other_options)
	CREX_PARSE_PARAMETERS_END();

	memset(&env, 0, sizeof(env));

	if (command_ht) {
		uint32_t num_elems = crex_hash_num_elements(command_ht);
		if (num_elems == 0) {
			crex_argument_value_error(1, "must have at least one element");
			RETURN_THROWS();
		}
		...
		crex_string_addref(command_str);
	}
	...
	if (environment) {
		env = _crx_array_to_envp(environment);
	}

	descriptors = alloc_descriptor_array(descriptorspec);

	/* Walk the descriptor spec and set up files/pipes */
	CREX_HASH_FOREACH_KEY_VAL(descriptorspec, nindex, str_index, descitem) {
		if (str_index) {
			crex_argument_value_error(2, "must be an integer indexed array");
			goto exit_fail;
		}
	...
	if (cwd == NULL) {
		...
		if (!getcwd_result) {
			crx_error_docref(NULL, E_WARNING, "Cannot get current directory");
			goto exit_fail;
		}
		cwd = cur_cwd;
	}
	cwdw = crx_win32_cp_any_to_w(cwd);
	if (!cwdw) {
		crx_error_docref(NULL, E_WARNING, "CWD conversion failed");
		goto exit_fail;
	}
	...
	envpw = crx_win32_cp_env_any_to_w(env.envp);
	if (envpw) {
		dwCreateFlags |= CREATE_UNICODE_ENVIRONMENT;
	} else  {
		if (env.envp) {
			crx_error_docref(NULL, E_WARNING, "ENV conversion failed");
			goto exit_fail;
		}
	}

	cmdw = crx_win32_cp_conv_any_to_w(ZSTR_VAL(command_str), ZSTR_LEN(command_str), &cmdw_len);
	if (!cmdw) {
		crx_error_docref(NULL, E_WARNING, "Command conversion failed");
		goto exit_fail;
	}
	...
	newprocok = CreateProcessW(NULL, cmdw, &crx_proc_open_security,
		&crx_proc_open_security, TRUE, dwCreateFlags, envpw, cwdw, &si, &pi);
	...
	if (newprocok == FALSE) {
		...
		crx_error_docref(NULL, E_WARNING, "CreateProcess failed, error code: %u", dw);
		goto exit_fail;
	}
	...
	if (cwd) {
		...
		if (r != 0) {
			crx_error_docref(NULL, E_WARNING, "posix_spawn_file_actions_addchdir_np() failed: %s", strerror(r));
		}
	}
	...
	if (r != 0) {
		close_all_descriptors(descriptors, ndesc);
		crx_error_docref(NULL, E_WARNING, "posix_spawn() failed: %s", strerror(r));
		goto exit_fail;
	}
#elif HAVE_FORK
	/* the Unix way */
	...
	if (child == 0) {
		...
		crx_error_docref(NULL, E_WARNING, "Exec failed: %s", strerror(errno));
		_exit(127);
	} else if (child < 0) {
		/* Failed to fork() */
		close_all_descriptors(descriptors, ndesc);
		crx_error_docref(NULL, E_WARNING, "Fork failed: %s", strerror(errno));
		goto exit_fail;
	}
#else
# error You lose (configure should not have let you get here)
	...
	pipes = crex_try_array_init(pipes);
	if (!pipes) {
		goto exit_fail;
	}

	proc = (crx_process_handle*) emalloc(sizeof(crx_process_handle));
	proc->command = crex_string_copy(command_str);
	proc->pipes = emalloc(sizeof(crex_resource *) * ndesc);
	proc->npipes = ndesc;
	proc->child = child;
#ifdef CRX_WIN32
	...
	for (i = 0; i < ndesc; i++) {
...
#ifdef CRX_WIN32
			stream = crx_stream_fopen_from_fd(_open_osfhandle((intptr_t)descriptors[i].parentend,
						descriptors[i].mode_flags), mode_string, NULL);
			crx_stream_set_option(stream, CRX_STREAM_OPTION_PIPE_BLOCKING, blocking_pipes, NULL);
#else
			stream = crx_stream_fopen_from_fd(descriptors[i].parentend, mode_string, NULL);
#endif
		} else if (descriptors[i].type == DESCRIPTOR_TYPE_SOCKET) {
			stream = crx_stream_sock_open_from_socket((crx_socket_t) descriptors[i].parentend, NULL);
		} else {
			proc->pipes[i] = NULL;
		}
		...
		if (stream) {
			...
			crx_stream_to_zval(stream, &retfp);
			add_index_zval(pipes, descriptors[i].index, &retfp);

			proc->pipes[i] = C_RES(retfp);
			C_ADDREF(retfp);
		}
	}

	if (1) {
		RETVAL_RES(crex_register_resource(proc, le_proc_open));
	} else {
exit_fail:
		_crx_free_envp(env);
		RETVAL_FALSE;
	...
	crex_string_release_ex(command_str, false);
#ifdef CRX_WIN32
	free(cwdw);
	free(cmdw);
	free(envpw);
...
#endif /* CRX_CAN_SUPPORT_PROC_OPEN */
