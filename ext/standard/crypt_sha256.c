...
char * __crx_stpncpy(char *dst, const char *src, size_t len)
{
	size_t n = strlen(src);
	if (n > len) {
		n = len;
...
void * __crx_mempcpy(void * dst, const void * src, size_t len)
{
...
char * crx_sha256_crypt_r(const char *key, const char *salt, char *buffer, int buflen)
{
#ifdef CRX_WIN32
	CREX_SET_ALIGNED(32, unsigned char alt_result[32]);
	CREX_SET_ALIGNED(32, unsigned char temp_result[32]);
#else
	CREX_SET_ALIGNED(__alignof__ (uint32_t), unsigned char alt_result[32]);
	CREX_SET_ALIGNED(__alignof__ (uint32_t), unsigned char temp_result[32]);
#endif
	...
	for (cnt = key_len; cnt >= 32; cnt -= 32) {
		cp = __crx_mempcpy((void *)cp, (const void *)temp_result, 32);
	}
	...
	for (cnt = salt_len; cnt >= 32; cnt -= 32) {
		cp = __crx_mempcpy(cp, temp_result, 32);
	}
	memcpy(cp, temp_result, cnt);
	...
	parts.  */
	cp = __crx_stpncpy(buffer, sha256_salt_prefix, MAX(0, buflen));
	buflen -= sizeof(sha256_salt_prefix) - 1;

	if (rounds_custom) {
#ifdef CRX_WIN32
		int n = _snprintf(cp, MAX(0, buflen), "%s" CREX_ULONG_FMT "$", sha256_rounds_prefix, rounds);
#else
...
/* This entry point is equivalent to the `crypt' function in Unix
   libcs.  */
char * crx_sha256_crypt(const char *key, const char *salt)
{
	...
	return crx_sha256_crypt_r(key, salt, buffer, buflen);
}
...
int main(void) {
	...
	for (cnt = 0; cnt < ntests2; ++cnt) {
		char *cp = crx_sha256_crypt(tests2[cnt].input, tests2[cnt].salt);
