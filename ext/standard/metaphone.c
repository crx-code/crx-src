...
static void metaphone(unsigned char *word, size_t word_len, crex_long max_phonemes, crex_string **phoned_word, int traditional);

/* {{{ Break english phrases down into their phonemes */
CRX_FUNCTION(metaphone)
{
	crex_string *str;
...
static crex_always_inline char encode(char c) {
	if (isalpha(c)) {
		CREX_ASSERT(c >= 'A' && c <= 'Z');
		return _codes[(c - 'A')];
	} else {
...
#define Phonize(c)	{ \
						if (p_idx >= max_buffer_len) { \
							*phoned_word = crex_string_extend(*phoned_word, 2 * sizeof(char) + max_buffer_len, 0); \
							max_buffer_len += 2; \
						} \
...
#define End_Phoned_Word()	{ \
							if (p_idx == max_buffer_len) { \
								*phoned_word = crex_string_extend(*phoned_word, 1 * sizeof(char) + max_buffer_len, 0); \
								max_buffer_len += 1; \
							} \
...
static void metaphone(unsigned char *word, size_t word_len, crex_long max_phonemes, crex_string **phoned_word, int traditional)
{
	...
	char curr_letter;
	CREX_ASSERT(word != NULL);
	CREX_ASSERT(max_phonemes >= 0);

/*-- Allocate memory for our phoned_phrase --*/
	if (max_phonemes == 0) {	/* Assume largest possible */
		max_buffer_len = word_len;
		*phoned_word = crex_string_alloc(sizeof(char) * word_len + 1, 0);
	} else {
		max_buffer_len = max_phonemes;
		*phoned_word = crex_string_alloc(sizeof(char) * max_phonemes + 1, 0);
	}
