...
#define CRX_URL_REDIRECT_MAX		20
...
static crx_stream *crx_stream_url_wrap_http_ex(crx_stream_wrapper *wrapper,
		const char *path, const char *mode, int options, crex_string **opened_path,
		crx_stream_context *context, int redirect_max, int flags,
		zval *response_header STREAMS_DC) /* {{{ */
{
	...
	crx_stream_filter *transfer_encoding = NULL;
	int response_code;
	smart_str req_buf = {0};
	...
	if (redirect_max < 1) {
		crx_stream_wrapper_log_error(wrapper, options, "Redirection limit reached, aborting");
		return NULL;
	}
	...
	CREX_ASSERT(resource->scheme);
	if (!crex_string_equals_literal_ci(resource->scheme, "http") &&
		!crex_string_equals_literal_ci(resource->scheme, "https")) {
		if (!context ||
			(tmpzval = crx_stream_context_get_option(context, wrapper->wops->label, "proxy")) == NULL ||
			C_TYPE_P(tmpzval) != IS_STRING ||
			C_STRLEN_P(tmpzval) == 0) {
			crx_url_free(resource);
			return crx_stream_open_wrapper_ex(path, mode, REPORT_ERRORS, NULL, context);
		}
		/* Called from a non-http wrapper with http proxying requested (i.e. ftp) */
		request_fulluri = 1;
		use_ssl = 0;
		use_proxy = 1;
		transport_string = crex_string_copy(C_STR_P(tmpzval));
	} else {
		/* Normal http request (possibly with proxy) */

		if (strpbrk(mode, "awx+")) {
			crx_stream_wrapper_log_error(wrapper, options, "HTTP wrapper does not support writeable connections");
			crx_url_free(resource);
			return NULL;
		}
		...
		if (context &&
			...
			transport_string = crex_strpprintf(0, "%s://%s:%d", use_ssl ? "ssl" : "tcp", ZSTR_VAL(resource->host), resource->port);
		}
	}

	if (context && (tmpzval = crx_stream_context_get_option(context, wrapper->wops->label, "timeout")) != NULL) {
	...
	stream = crx_stream_xport_create(ZSTR_VAL(transport_string), ZSTR_LEN(transport_string), options,
			STREAM_XPORT_CLIENT | STREAM_XPORT_CONNECT,
			NULL, &timeout, context, &errstr, NULL);

	if (stream) {
		crx_stream_set_option(stream, CRX_STREAM_OPTION_READ_TIMEOUT, 0, &timeout);
	}

	if (errstr) {
		crx_stream_wrapper_log_error(wrapper, options, "%s", ZSTR_VAL(errstr));
		crex_string_release_ex(errstr, 0);
		errstr = NULL;
	}

	crex_string_release(transport_string);

	if (stream && use_proxy && use_ssl) {
		smart_str header = {0};

		/* Set peer_name or name verification will try to use the proxy server name */
		if (!context || (tmpzval = crx_stream_context_get_option(context, "ssl", "peer_name")) == NULL) {
		...
		if (context && (tmpzval = crx_stream_context_get_option(context, "http", "header")) != NULL) {
			char *s, *p;

			if (C_TYPE_P(tmpzval) == IS_ARRAY) {
				zval *tmpheader = NULL;

				CREX_HASH_FOREACH_VAL(C_ARRVAL_P(tmpzval), tmpheader) {
					if (C_TYPE_P(tmpheader) == IS_STRING) {
						s = C_STRVAL_P(tmpheader);
						do {
							while (*s == ' ' || *s == '\t') s++;
							p = s;
							while (*p != 0 && *p != ':' && *p != '\r' && *p !='\n') p++;
							if (*p == ':') {
								...
								if (p - s == sizeof("Proxy-Authorization:") - 1 &&
								    crex_binary_strcasecmp(s, sizeof("Proxy-Authorization:") - 1,
			...
			} else if (C_TYPE_P(tmpzval) == IS_STRING && C_STRLEN_P(tmpzval)) {
				s = C_STRVAL_P(tmpzval);
				do {
					while (*s == ' ' || *s == '\t') s++;
					p = s;
					while (*p != 0 && *p != ':' && *p != '\r' && *p !='\n') p++;
...
finish:
		...
		if (crx_stream_write(stream, ZSTR_VAL(header.s), ZSTR_LEN(header.s)) != ZSTR_LEN(header.s)) {
			crx_stream_wrapper_log_error(wrapper, options, "Cannot connect to HTTPS server through proxy");
		...
		if (stream) {
			...
			/* get response header */
			while (crx_stream_gets(stream, header_line, HTTP_HEADER_BLOCK_SIZE-1) != NULL) {
				if (header_line[0] == '\n' ||
				    header_line[0] == '\r' ||
				    header_line[0] == '\0') {
				  break;
		...
		if (stream) {
			if (crx_stream_xport_crypto_setup(stream, STREAM_CRYPTO_METHOD_SSLv23_CLIENT, NULL) < 0 ||
	...
	if (options & STREAM_WILL_CAST)
		chunk_size = crx_stream_set_chunk_size(stream, 1);

	/* avoid problems with auto-detecting when reading the headers -> the headers
	 * are always in canonical \r\n format */
	eol_detect = stream->flags & (CRX_STREAM_FLAG_DETECT_EOL | CRX_STREAM_FLAG_EOL_MAC);
	stream->flags &= ~(CRX_STREAM_FLAG_DETECT_EOL | CRX_STREAM_FLAG_EOL_MAC);

	crx_stream_context_set(stream, context);

	crx_stream_notify_info(context, CRX_STREAM_NOTIFY_CONNECT, NULL, 0);

	if (header_init && context && (tmpzval = crx_stream_context_get_option(context, "http", "max_redirects")) != NULL) {
		redirect_max = (int)zval_get_long(tmpzval);
	}

	custom_request_method = 0;
	if (context && (tmpzval = crx_stream_context_get_option(context, "http", "method")) != NULL) {
		if (C_TYPE_P(tmpzval) == IS_STRING && C_STRLEN_P(tmpzval) > 0) {
			/* As per the RFC, automatically redirected requests MUST NOT use other methods than
			 * GET and HEAD unless it can be confirmed by the user. */
			if (!redirected || redirect_keep_method
				|| crex_string_equals_literal(C_STR_P(tmpzval), "GET")
				|| crex_string_equals_literal(C_STR_P(tmpzval), "HEAD")
			) {
				custom_request_method = 1;
				smart_str_append(&req_buf, C_STR_P(tmpzval));
	...
	if (!request_fulluri && context &&
		(tmpzval = crx_stream_context_get_option(context, "http", "request_fulluri")) != NULL) {
		request_fulluri = crex_is_true(tmpzval);
	}
	...
	if (context && (tmpzval = crx_stream_context_get_option(context, "http", "protocol_version")) != NULL) {
	...
	if (context && (tmpzval = crx_stream_context_get_option(context, "http", "header")) != NULL) {
		...
		if (C_TYPE_P(tmpzval) == IS_ARRAY) {
			...
			CREX_HASH_FOREACH_VAL(C_ARRVAL_P(tmpzval), tmpheader) {
				if (C_TYPE_P(tmpheader) == IS_STRING) {
					smart_str_append(&tmpstr, C_STR_P(tmpheader));
			...
			if (tmpstr.s) {
				tmp = crx_trim(tmpstr.s, NULL, 0, 3);
				smart_str_free(&tmpstr);
			}
		} else if (C_TYPE_P(tmpzval) == IS_STRING && C_STRLEN_P(tmpzval)) {
			/* Remove newlines and spaces from start and end crx_trim will estrndup() */
			tmp = crx_trim(C_STR_P(tmpzval), NULL, 0, 3);
		}
		if (tmp && ZSTR_LEN(tmp)) {
			char *s;
			char *t;
			...
			if (ZSTR_IS_INTERNED(tmp)) {
				tmp = crex_string_init(ZSTR_VAL(tmp), ZSTR_LEN(tmp), 0);
			...
			crex_str_tolower(ZSTR_VAL(tmp), ZSTR_LEN(tmp));
			t = ZSTR_VAL(tmp);
	...
	if (((have_header & HTTP_HEADER_AUTH) == 0) && resource->user) {
		...
		crx_url_decode(ZSTR_VAL(resource->user), ZSTR_LEN(resource->user));
		...
		if (resource->pass) {
			crx_url_decode(ZSTR_VAL(resource->pass), ZSTR_LEN(resource->pass));
			strcat(scratch, ZSTR_VAL(resource->pass));
		}

		stmp = crx_base64_encode((unsigned char*)scratch, strlen(scratch));

		smart_str_appends(&req_buf, "Authorization: Basic ");
		smart_str_appends(&req_buf, ZSTR_VAL(stmp));
		smart_str_appends(&req_buf, "\r\n");

		crx_stream_notify_info(context, CRX_STREAM_NOTIFY_AUTH_REQUIRED, NULL, 0);

		crex_string_free(stmp);
		efree(scratch);
	}
	...
	if (context &&
	    (ua_zval = crx_stream_context_get_option(context, "http", "user_agent")) != NULL &&
		C_TYPE_P(ua_zval) == IS_STRING) {
		ua_str = C_STRVAL_P(ua_zval);
...
#define _UA_HEADER "User-Agent: %s\r\n"
		...
		if (ua_len > sizeof(_UA_HEADER)) {
			...
			if ((ua_len = slprintf(ua, ua_len, _UA_HEADER, ua_str)) > 0) {
				...
				crx_error_docref(NULL, E_WARNING, "Cannot construct User-agent header");
			}
			efree(ua);
		}
	}
	...
	if (user_headers) {
		...
		if (
				...
				context &&
				!(have_header & HTTP_HEADER_CONTENT_LENGTH) &&
				(tmpzval = crx_stream_context_get_option(context, "http", "content")) != NULL &&
				C_TYPE_P(tmpzval) == IS_STRING && C_STRLEN_P(tmpzval) > 0
		) {
			smart_str_appends(&req_buf, "Content-Length: ");
			smart_str_append_unsigned(&req_buf, C_STRLEN_P(tmpzval));
			smart_str_appends(&req_buf, "\r\n");
			have_header |= HTTP_HEADER_CONTENT_LENGTH;
		}
	...
	if ((header_init || redirect_keep_method) && context &&
		...
		if (!(have_header & HTTP_HEADER_TYPE)) {
			...
			crx_error_docref(NULL, E_NOTICE, "Content-type not specified assuming application/x-www-form-urlencoded");
		}
		smart_str_appends(&req_buf, "\r\n");
		smart_str_appendl(&req_buf, C_STRVAL_P(tmpzval), C_STRLEN_P(tmpzval));
	} else {
		smart_str_appends(&req_buf, "\r\n");
	}

	/* send it */
	crx_stream_write(stream, ZSTR_VAL(req_buf.s), ZSTR_LEN(req_buf.s));

	location[0] = '\0';

	if (C_ISUNDEF_P(response_header)) {
...

	{
		...
		if (!crx_stream_eof(stream) &&
			crx_stream_get_line(stream, tmp_line, sizeof(tmp_line) - 1, &tmp_line_len) != NULL) {
			zval http_response;
			...
			if (context && NULL != (tmpzval = crx_stream_context_get_option(context, "http", "ignore_errors"))) {
				ignore_errors = crex_is_true(tmpzval);
			}
			...
			if (response_code >= 200 && response_code < 400) {
				...
				switch(response_code) {
					case 403:
						crx_stream_notify_error(context, CRX_STREAM_NOTIFY_AUTH_RESULT,
					...
					default:
						...
						crx_stream_notify_error(context, CRX_STREAM_NOTIFY_FAILURE,
								tmp_line, response_code);
				}
			}
			if (tmp_line_len >= 1 && tmp_line[tmp_line_len - 1] == '\n') {
				...
				char *line = crx_stream_get_line(stream, NULL, 0, NULL);
				efree(line);
			}
			ZVAL_STRINGL(&http_response, tmp_line, tmp_line_len);
			crex_hash_next_index_insert(C_ARRVAL_P(response_header), &http_response);
		} else {
			crx_stream_close(stream);
			stream = NULL;
			crx_stream_wrapper_log_error(wrapper, options, "HTTP request failed!");
			goto out;
		}
	}
	...
	while (!crx_stream_eof(stream)) {
		...
		if ((http_header_line = crx_stream_get_line(stream, NULL, 0, &http_header_line_length)) && *http_header_line != '\n' && *http_header_line != '\r') {
			...
			*e = '\0';
			...
			if (!strncasecmp(http_header_line, "Location:", sizeof("Location:")-1)) {
				if (context && (tmpzval = crx_stream_context_get_option(context, "http", "follow_location")) != NULL) {
				...
				crx_stream_notify_info(context, CRX_STREAM_NOTIFY_MIME_TYPE_IS, http_header_value, 0);
			} else if (!strncasecmp(http_header_line, "Content-Length:", sizeof("Content-Length:")-1)) {
				file_size = atoi(http_header_value);
				crx_stream_notify_file_size(context, file_size, http_header_line, 0);
			} else if (
				...
				if (!(options & STREAM_ONLY_GET_HEADERS)) {
					...
					if (context && (tmpzval = crx_stream_context_get_option(context, "http", "auto_decode")) != NULL) {
						decode = crex_is_true(tmpzval);
					}
					if (decode) {
						transfer_encoding = crx_stream_filter_create("dechunk", NULL, crx_stream_is_persistent(stream));
...

			{
				...
				crex_hash_next_index_insert(C_ARRVAL_P(response_header), &http_header);
			}
		} else {
			break;
		}
	...
	if (!reqok || (location[0] != '\0' && follow_location)) {
		...
		if (location[0] != '\0')
			crx_stream_notify_info(context, CRX_STREAM_NOTIFY_REDIRECTED, location, 0);

		crx_stream_close(stream);
		stream = NULL;

		if (transfer_encoding) {
			crx_stream_filter_free(transfer_encoding);
		...
		if (location[0] != '\0') {
			...
			*new_path='\0';
					...
					if (*(location+1) != '\0' && resource->path) {
						...
						if (!s) {
							...
							if (!ZSTR_LEN(resource->path)) {
								crex_string_release_ex(resource->path, 0);
			...
			if ((resource = crx_url_parse(new_path)) == NULL) {
				crx_stream_wrapper_log_error(wrapper, options, "Invalid redirect URL! %s", new_path);
				goto out;
			}
...
#define CHECK_FOR_CNTRL_CHARS(val) { \
	if (val) { \
		unsigned char *s, *e; \
		ZSTR_LEN(val) = crx_url_decode(ZSTR_VAL(val), ZSTR_LEN(val)); \
		...
		while (s < e) { \
			...
			stream = crx_stream_url_wrap_http_ex(
				wrapper, new_path, mode, options, opened_path, context,
				--redirect_max, new_flags, response_header STREAMS_CC);
		} else {
			crx_stream_wrapper_log_error(wrapper, options, "HTTP request failed! %s", tmp_line);
		}
	}
out:
	...
	if (stream) {
		...
		if (options & STREAM_WILL_CAST)
			crx_stream_set_chunk_size(stream, (int)chunk_size);
		...
		if (transfer_encoding) {
			crx_stream_filter_append(&stream->readfilters, transfer_encoding);
		}
		...
		if (stream->writepos > stream->readpos) {
			crx_stream_notify_progress_increment(context, stream->writepos - stream->readpos, 0);
		}
	}
...
crx_stream *crx_stream_url_wrap_http(crx_stream_wrapper *wrapper, const char *path, const char *mode, int options, crex_string **opened_path, crx_stream_context *context STREAMS_DC) /* {{{ */
{
	...
	stream = crx_stream_url_wrap_http_ex(
		...
		CRX_URL_REDIRECT_MAX, HTTP_WRAPPER_HEADER_INIT, &headers STREAMS_CC);

	if (!C_ISUNDEF(headers)) {
		if (FAILURE == crex_set_local_var_str(
...
static int crx_stream_http_stream_stat(crx_stream_wrapper *wrapper, crx_stream *stream, crx_stream_statbuf *ssb) /* {{{ */
{
...
static const crx_stream_wrapper_ops http_stream_wops = {
	crx_stream_url_wrap_http,
	NULL, /* stream_close */
	crx_stream_http_stream_stat,
	NULL, /* stat_url */
	NULL, /* opendir */
	"http",
	NULL, /* unlink */
...
CRXAPI const crx_stream_wrapper crx_stream_http_wrapper = {
	&http_stream_wops,
	NULL,
	1 /* is_url */
};
