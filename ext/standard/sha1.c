...
/* This code is heavily based on the CRX md5 implementation */
...
CRX_FUNCTION(sha1)
{
	...
	CRX_SHA1Update(&context, (unsigned char *) ZSTR_VAL(arg), ZSTR_LEN(arg));
	CRX_SHA1Final(digest, &context);
	if (raw_output) {
		RETURN_STRINGL((char *) digest, 20);
	} else {
		RETVAL_NEW_STR(crex_string_alloc(40, 0));
		make_digest_ex(C_STRVAL_P(return_value), digest, 20);
	}
...
CRX_FUNCTION(sha1_file)
{
	...
	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_PATH(arg, arg_len)
		C_PARAM_OPTIONAL
		C_PARAM_BOOL(raw_output)
	CREX_PARSE_PARAMETERS_END();

	stream = crx_stream_open_wrapper(arg, "rb", REPORT_ERRORS, NULL);
	if (!stream) {
		RETURN_FALSE;
	}
	...
	while ((n = crx_stream_read(stream, (char *) buf, sizeof(buf))) > 0) {
		CRX_SHA1Update(&context, buf, n);
	}
 ...
 */
CRXAPI void CRX_SHA1InitArgs(CRX_SHA1_CTX * context, CREX_ATTRIBUTE_UNUSED HashTable *args)
{
...
/* {{{ CRX_SHA1Update
   ...
   context.
 */
CRXAPI void CRX_SHA1Update(CRX_SHA1_CTX * context, const unsigned char *input,
			   size_t inputLen)
{
 ...
 */
CRXAPI void CRX_SHA1Final(unsigned char digest[20], CRX_SHA1_CTX * context)
{
	...
	CRX_SHA1Update(context, PADDING, padLen);

	/* Append length (before padding) */
	CRX_SHA1Update(context, bits, 8);
