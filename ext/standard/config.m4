...
CRX_ARG_WITH([external-libcrypt],
...
if test "$CRX_EXTERNAL_LIBCRYPT" != "no"; then
  CRX_CHECK_FUNC(crypt, crypt)
  CRX_CHECK_FUNC(crypt_r, crypt)
...
int main(void) {
  ...
  AC_DEFINE_UNQUOTED(CRX_USE_CRX_CRYPT_R, 0, [Whether CRX has to use its own crypt_r])
else
  AC_DEFINE_UNQUOTED(CRX_USE_CRX_CRYPT_R, 1, [Whether CRX has to use its own crypt_r])

  CRX_ADD_SOURCES(CRX_EXT_DIR(standard), crypt_freesec.c crypt_blowfish.c crypt_sha512.c crypt_sha256.c crx_crypt_r.c)
fi
...
AC_CHECK_FUNCS(fork CreateProcess, [
  crx_can_support_proc_open=yes
  break
],[
  crx_can_support_proc_open=no
])
AC_MSG_CHECKING([if your OS can spawn processes with inherited handles])
if test "$crx_can_support_proc_open" = "yes"; then
  AC_MSG_RESULT(yes)
  AC_DEFINE(CRX_CAN_SUPPORT_PROC_OPEN,1, [Define if your system has fork/vfork/CreateProcess])
else
  AC_MSG_RESULT(no)
fi

CRX_ENABLE_CHROOT_FUNC=no
case "$CRX_SAPI" in
  embed)
    CRX_ENABLE_CHROOT_FUNC=yes
  ;;

  none)
    for PROG in $CRX_BINARIES; do
      case "$PROG" in
        cgi|cli|crxdbg)
          CRX_ENABLE_CHROOT_FUNC=yes
        ;;
...
if test "$CRX_ENABLE_CHROOT_FUNC" = "yes"; then
  AC_DEFINE(ENABLE_CHROOT_FUNC, 1, [Whether to enable chroot() function])
fi

dnl
dnl Detect library functions needed by crx dns_xxx functions
dnl ext/standard/crx_dns.h will collect these in a single define
dnl HAVE_FULL_DNS_FUNCS
dnl
CRX_CHECK_FUNC(res_nsearch, resolv, bind, socket)
CRX_CHECK_FUNC(res_ndestroy, resolv, bind, socket)
CRX_CHECK_FUNC(dns_search, resolv, bind, socket)
CRX_CHECK_FUNC(dn_expand, resolv, bind, socket)
CRX_CHECK_FUNC(dn_skipname, resolv, bind, socket)

dnl
dnl These are old deprecated functions
dnl

CRX_CHECK_FUNC(res_search, resolv, bind, socket)

CRX_CHECK_FUNC(posix_spawn_file_actions_addchdir_np)
...
dnl
CRX_ARG_WITH([password-argon2],
  [for Argon2 support],
  [AS_HELP_STRING([[--with-password-argon2]],
    [Include Argon2 support in password_*])])

if test "$CRX_PASSWORD_ARGON2" != "no"; then
  PKG_CHECK_MODULES([ARGON2], [libargon2])
  CRX_EVAL_INCLINE($ARGON2_CFLAGS)
  CRX_EVAL_LIBLINE($ARGON2_LIBS)
...
dnl
CRX_NEW_EXTENSION(standard, array.c base64.c basic_functions.c browscap.c crc32.c crypt.c \
                            ...
                            http_fopen_wrapper.c crx_fopen_wrapper.c credits.c css.c \
                            var_unserializer.c ftok.c sha1.c user_filters.c uuencode.c \
                            filters.c proc_open.c streamsfuncs.c http.c password.c \
                            net.c hrtime.c crc32_x86.c libavifinfo/avifinfo.c,,,
			    -DCREX_ENABLE_STATIC_TSRMLS_CACHE=1)

CRX_ADD_BUILD_DIR($ext_builddir/libavifinfo)

CRX_ADD_MAKEFILE_FRAGMENT
CRX_INSTALL_HEADERS([ext/standard/])
