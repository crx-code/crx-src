...
#ifdef AF_INET6
		case AF_INET6: {
			crex_string *ret = crex_string_alloc(INET6_ADDRSTRLEN, 0);
...
#endif
		case AF_INET: {
			crex_string *ret = crex_string_alloc(INET_ADDRSTRLEN, 0);
...
#endif
		case AF_INET: {
			crex_string *ret = crex_string_alloc(NI_MAXHOST, 0);
...
#if defined(CRX_WIN32) || defined(HAVE_GETIFADDRS) || defined(__PASE__)
static void iface_append_unicast(zval *unicast, crex_long flags,
	...
	if (addr) {
		...
		if ((host = crx_inet_ntop(addr))) {
			add_assoc_str(&u, "address", host);
		}
	}
	if ((host = crx_inet_ntop(netmask))) {
		add_assoc_str(&u, "netmask", host);
	}

	if ((host = crx_inet_ntop(broadcast))) {
		add_assoc_str(&u, "broadcast", host);
	}

	if ((host = crx_inet_ntop(ptp))) {
...
*/
CRX_FUNCTION(net_get_interfaces) {
#ifdef CRX_WIN32
...
# define FREE(x) HeapFree(GetProcessHeap(), 0, (x))
	...
	if (pAddresses == NULL) {
		crex_error(E_WARNING, "Memory allocation failed for IP_ADAPTER_ADDRESSES struct");
		RETURN_FALSE;
	}
	...
	if (NO_ERROR != dwRetVal) {
		char *buf = crx_win32_error_to_msg(GetLastError());
		crex_error(E_WARNING, "GetAdaptersAddresses failed: %s", buf);
	...
	for (p = pAddresses; p; p = p->Next) {
		...
		if (p->PhysicalAddressLength > 0) {
			crex_string *mac = crex_string_alloc(p->PhysicalAddressLength * 3, 0);
			char *s = ZSTR_VAL(mac);
			ULONG i;
...
#undef MALLOC
#undef FREE
#elif HAVE_GETIFADDRS || defined(__PASE__) /* !CRX_WIN32 */
	struct ifaddrs *addrs = NULL, *p;

	CREX_PARSE_PARAMETERS_NONE();

	if (getifaddrs(&addrs)) {
		crx_error(E_WARNING, "getifaddrs() failed %d: %s", errno, strerror(errno));
		RETURN_FALSE;
	}

	array_init(return_value);
	for (p = addrs; p; p = p->ifa_next) {
		zval *iface = crex_hash_str_find(C_ARR_P(return_value), p->ifa_name, strlen(p->ifa_name));
		zval *unicast, *status;

		if (!iface) {
			zval newif;
			array_init(&newif);
			iface = crex_hash_str_add(C_ARR_P(return_value), p->ifa_name, strlen(p->ifa_name), &newif);
		}

		unicast = crex_hash_str_find(C_ARR_P(iface), "unicast", sizeof("unicast") - 1);
		if (!unicast) {
			zval newuni;
			array_init(&newuni);
			unicast = crex_hash_str_add(C_ARR_P(iface), "unicast", sizeof("unicast") - 1, &newuni);
		}
		...
		status = crex_hash_str_find(C_ARR_P(iface), "up", sizeof("up") - 1);
		if (!status) {
