...
CRXAPI crex_string *crx_quot_print_decode(const unsigned char *str, size_t length, int replace_us_by_ws) /* {{{ */
{
	size_t i;
	...
	retval = crex_string_alloc(buf_size, 0);
...
#define CRX_QPRINT_MAXL 75

CRXAPI crex_string *crx_quot_print_encode(const unsigned char *str, size_t length) /* {{{ */
{
	crex_ulong lp = 0;
	unsigned char c, *d;
	...
	ret = crex_string_safe_alloc(3, (length + (((3 * length)/(CRX_QPRINT_MAXL-9)) + 1)), 0, 0);
		...
		if (((c = *str++) == '\015') && (*str == '\012') && length > 0) {
			...
			*d++ = *str++;
			...
			if (iscntrl (c) || (c == 0x7f) || (c & 0x80) || (c == '=') || ((c == ' ') && (*str == '\015'))) {
				if ((((lp+= 3) > CRX_QPRINT_MAXL) && (c <= 0x7f))
				    || ((c > 0x7f) && (c <= 0xdf) && ((lp + 3) > CRX_QPRINT_MAXL))
				    || ((c > 0xdf) && (c <= 0xef) && ((lp + 6) > CRX_QPRINT_MAXL))
				    || ((c > 0xef) && (c <= 0xf4) && ((lp + 9) > CRX_QPRINT_MAXL))) {
					*d++ = '=';
					*d++ = '\015';
					*d++ = '\012';
					lp = 3;
	...
	*d = '\0';
	ret = crex_string_truncate(ret, d - (unsigned char*)ZSTR_VAL(ret), 0);
	return ret;
}
/* }}} */
...
CRX_FUNCTION(quoted_printable_decode)
{
	crex_string *arg1;
	char *str_in;
	crex_string *str_out;
	...
	str_out = crex_string_alloc(ZSTR_LEN(arg1), 0);
	while (str_in[i]) {
		switch (str_in[i]) {
		case '=':
			...
			{
				ZSTR_VAL(str_out)[j++] = (crx_hex2int((int) str_in[i + 1]) << 4)
						+ crx_hex2int((int) str_in[i + 2]);
				i += 3;
...
/* {{{ */
CRX_FUNCTION(quoted_printable_encode)
{
	crex_string *str;
	crex_string *new_str;
	...
	new_str = crx_quot_print_encode((unsigned char *)ZSTR_VAL(str), ZSTR_LEN(str));
	RETURN_STR(new_str);
}
/* }}} */
