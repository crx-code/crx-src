...
CRXAPI uint32_t crx_crc32_bulk_update(uint32_t crc, const char *p, size_t nr)
{
#ifdef HAVE_AARCH64_CRC32
	if (has_crc32_insn()) {
...
CRXAPI int crx_crc32_stream_bulk_update(uint32_t *crc, crx_stream *fp, size_t nr)
{
	size_t handled = 0, n;
	char buf[1024];
	...
	while (handled < nr) {
		...
		n = crx_stream_read(fp, buf, n);
		if (n > 0) {
			*crc = crx_crc32_bulk_update(*crc, buf, n);
			handled += n;
		} else { /* EOF */
			return FAILURE;
		}
...
CRX_FUNCTION(crc32)
{
	...
	crc = crx_crc32_bulk_update(crc, p, nr);

	RETURN_LONG(crx_crc32_bulk_end(crc));
}
/* }}} */
