...
#define INC_OUTPUTPOS(a,b) \
	if ((a) < 0 || ((INT_MAX - outputpos)/((int)b)) < (a)) { \
		efree(formatcodes);	\
		efree(formatargs);	\
		crex_value_error("Type %c: integer overflow in format string", code); \
		RETURN_THROWS(); \
	} \
	outputpos += (a)*(b);
...
typedef CREX_SET_ALIGNED(1, uint16_t unaligned_uint16_t);
typedef CREX_SET_ALIGNED(1, uint32_t unaligned_uint32_t);
typedef CREX_SET_ALIGNED(1, uint64_t unaligned_uint64_t);
typedef CREX_SET_ALIGNED(1, unsigned int unaligned_uint);
typedef CREX_SET_ALIGNED(1, int unaligned_int);
...
/* {{{ crx_pack */
static void crx_pack(zval *val, size_t size, int *map, char *output)
{
	size_t i;
	char *v;
...
CREX_ATTRIBUTE_CONST static inline uint16_t crx_pack_reverse_int16(uint16_t arg)
{
	return ((arg & 0xFF) << 8) | ((arg >> 8) & 0xFF);
}

/* {{{ crx_pack_reverse_int32 */
CREX_ATTRIBUTE_CONST static inline uint32_t crx_pack_reverse_int32(uint32_t arg)
{
	uint32_t result;
...
/* {{{ crx_pack */
static inline uint64_t crx_pack_reverse_int64(uint64_t arg)
{
	union Swap64 {
		uint64_t i;
		uint32_t ul[2];
	...
	tmp.i = arg;
	result.ul[0] = crx_pack_reverse_int32(tmp.ul[1]);
	result.ul[1] = crx_pack_reverse_int32(tmp.ul[0]);
...
/* {{{ crx_pack_copy_float */
static void crx_pack_copy_float(int is_little_endian, void * dst, float f)
{
	union Copy32 {
		float f;
		uint32_t i;
...
#ifdef WORDS_BIGENDIAN
	if (is_little_endian) {
		m.i = crx_pack_reverse_int32(m.i);
	}
...
/* {{{ crx_pack_copy_double */
static void crx_pack_copy_double(int is_little_endian, void * dst, double d)
{
	union Copy64 {
		double d;
		uint64_t i;
...
#ifdef WORDS_BIGENDIAN
	if (is_little_endian) {
		m.i = crx_pack_reverse_int64(m.i);
	}
#else /* WORDS_BIGENDIAN */
	if (!is_little_endian) {
		m.i = crx_pack_reverse_int64(m.i);
...
/* {{{ crx_pack_parse_float */
static float crx_pack_parse_float(int is_little_endian, void * src)
{
	union Copy32 {
		float f;
		uint32_t i;
...
/* {{{ crx_pack_parse_double */
static double crx_pack_parse_double(int is_little_endian, void * src)
{
	union Copy64 {
		double d;
		uint64_t i;
...
CRX_FUNCTION(pack)
{
	...
	CREX_PARSE_PARAMETERS_START(1, -1)
		C_PARAM_STRING(format, formatlen)
		C_PARAM_VARIADIC('*', argv, num_args)
	...
	for (i = 0; i < formatlen; formatcount++) {
		...
		switch ((int) code) {
			...
			case '@':
				if (arg < 0) {
					crx_error_docref(NULL, E_WARNING, "Type %c: '*' ignored", code);
					arg = 1;
				}
				break;
			...
			case 'H':
				if (currentarg >= num_args) {
					efree(formatcodes);
					efree(formatargs);
					crex_value_error("Type %c: not enough arguments", code);
					RETURN_THROWS();
				}
			...
			case 'P':
					...
					efree(formatcodes);
					efree(formatargs);
					crex_value_error("64-bit format codes are not available for 32-bit versions of CRX");
					RETURN_THROWS();
#endif
			case 'c':
			case 'C':
...
too_few_args:
					efree(formatcodes);
					efree(formatargs);
					crex_value_error("Type %c: too few arguments", code);
					RETURN_THROWS();
				}
				break;
			...
			default:
				efree(formatcodes);
				efree(formatargs);
				crex_value_error("Type %c: unknown format code", code);
				RETURN_THROWS();
		}
	...
	if (currentarg < num_args) {
		crx_error_docref(NULL, E_WARNING, "%d arguments unused", (num_args - currentarg));
	}
	...
	for (i = 0; i < formatcount; i++) {
		...
		switch ((int) code) {
			...
			case 'X':
				...
				if (outputpos < 0) {
					crx_error_docref(NULL, E_WARNING, "Type %c: outside of string", code);
					outputpos = 0;
				}
				break;
	...
	output = crex_string_alloc(outputsize, 0);
	outputpos = 0;
	currentarg = 0;
	...
	for (i = 0; i < formatcount; i++) {
		...
		switch ((int) code) {
			...
			case 'Z': {
				...
				crex_string *tmp_str;
				crex_string *str = zval_get_tmp_string(&argv[currentarg++], &tmp_str);
			...
			case 'H': {
				...
				if ((size_t)arg > ZSTR_LEN(str)) {
					crx_error_docref(NULL, E_WARNING, "Type %c: not enough characters in string", code);
					arg = ZSTR_LEN(str);
				}
				...
				while (arg-- > 0) {
					...
					if (n >= '0' && n <= '9') {
						...
						crx_error_docref(NULL, E_WARNING, "Type %c: illegal hex digit %c", code, n);
						n = 0;
					}
			...
			case 'C':
				while (arg-- > 0) {
					crx_pack(&argv[currentarg++], 1, byte_map, &ZSTR_VAL(output)[outputpos]);
					outputpos++;
				}
				break;
			...
			case 'v': {
				...
				while (arg-- > 0) {
					crx_pack(&argv[currentarg++], 2, map, &ZSTR_VAL(output)[outputpos]);
					outputpos += 2;
				}
				break;
			}
			...
			case 'I':
				while (arg-- > 0) {
					crx_pack(&argv[currentarg++], sizeof(int), int_map, &ZSTR_VAL(output)[outputpos]);
			...
			case 'V': {
				...
				while (arg-- > 0) {
					crx_pack(&argv[currentarg++], 4, map, &ZSTR_VAL(output)[outputpos]);
					outputpos += 4;
				}
				break;
			}
			...
			case 'P': {
				...
				while (arg-- > 0) {
					crx_pack(&argv[currentarg++], 8, map, &ZSTR_VAL(output)[outputpos]);
					outputpos += 8;
				}
				break;
			}
			...
			case 'g': {
				...
				while (arg-- > 0) {
					float v = (float) zval_get_double(&argv[currentarg++]);
					crx_pack_copy_float(1, &ZSTR_VAL(output)[outputpos], v);
					outputpos += sizeof(v);
				}
			...
			case 'G': {
				...
				while (arg-- > 0) {
					float v = (float) zval_get_double(&argv[currentarg++]);
					crx_pack_copy_float(0, &ZSTR_VAL(output)[outputpos], v);
					outputpos += sizeof(v);
				}
				break;
			}
			...
			case 'e': {
				...
				while (arg-- > 0) {
					double v = (double) zval_get_double(&argv[currentarg++]);
					crx_pack_copy_double(1, &ZSTR_VAL(output)[outputpos], v);
					outputpos += sizeof(v);
				}
				break;
			}
			...
			case 'E': {
				...
				while (arg-- > 0) {
					double v = (double) zval_get_double(&argv[currentarg++]);
					crx_pack_copy_double(0, &ZSTR_VAL(output)[outputpos], v);
					outputpos += sizeof(v);
				}
				break;
			}
...
{
	char *format, *input;
	crex_string *formatarg, *inputarg;
	crex_long formatlen, inputpos, inputlen;
	int i;
	crex_long offset = 0;

	CREX_PARSE_PARAMETERS_START(2, 3)
		C_PARAM_STR(formatarg)
		C_PARAM_STR(inputarg)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(offset)
	CREX_PARSE_PARAMETERS_END();
	...
	if (offset < 0 || offset > inputlen) {
		crex_argument_value_error(3, "must be contained in argument #2 ($data)");
		RETURN_THROWS();
	}
	...
	while (formatlen-- > 0) {
		...
		if (formatlen > 0) {
			...
			if (c >= '0' && c <= '9') {
				...
				if (errno || tmp < INT_MIN || tmp > INT_MAX) {
					crx_error_docref(NULL, E_WARNING, "Type %c: integer overflow", type);
		...
		switch ((int) type) {
			...
			case 'X':
				size = -1;
				if (repetitions < 0) {
					crx_error_docref(NULL, E_WARNING, "Type %c: '*' ignored", type);
					repetitions = 1;
				}
				break;
			...
			default:
				crex_value_error("Invalid format type %c", type);
				RETURN_THROWS();
		}
		...
		for (i = 0; i != repetitions; i++ ) {
			...
			if ((inputpos + size) <= inputlen) {
				...
				crex_string* real_name;
				zval val;

				if (repetitions == 1 && namelen > 0) {
					/* Use a part of the formatarg argument directly as the name. */
					real_name = crex_string_init_fast(name, namelen);

				} else {
					/* Need to add the 1-based element number to the name */
					char buf[MAX_LENGTH_OF_LONG + 1];
					char *res = crex_print_ulong_to_buf(buf + sizeof(buf) - 1, i+1);
					size_t digits = buf + sizeof(buf) - 1 - res;
					real_name = crex_string_concat2(name, namelen, res, digits);
				}

				switch ((int) type) {
					case 'a': {
						/* a will not strip any trailing whitespace or null padding */
						crex_long len = inputlen - inputpos;	/* Remaining string */
						...
						crex_symtable_update(C_ARRVAL_P(return_value), real_name, &val);
						break;
					}
					case 'A': {
					...
					case 'h':
					case 'H': {
						crex_long len = (inputlen - inputpos) * 2;	/* Remaining */
					...
					case 'G': /* big endian float*/
					{
						...
						if (type == 'g') {
							v = crx_pack_parse_float(1, &input[inputpos]);
						} else if (type == 'G') {
							v = crx_pack_parse_float(0, &input[inputpos]);
						} else {
					...
					case 'E': /* big endian float */
					{
						double v;
						if (type == 'e') {
							v = crx_pack_parse_double(1, &input[inputpos]);
						} else if (type == 'E') {
							v = crx_pack_parse_double(0, &input[inputpos]);
						} else {
					...
					case 'X':
						if (inputpos < size) {
							...
							if (repetitions >= 0) {
								crx_error_docref(NULL, E_WARNING, "Type %c: outside of string", type);
							}
						}
						break;
				...
				crx_error_docref(NULL, E_WARNING, "Type %c: not enough input values, need %d values but only " CREX_LONG_FMT " %s provided", type, size, inputlen - inputpos, inputlen - inputpos == 1 ? "was" : "were");
				crex_array_destroy(C_ARR_P(return_value));
				RETURN_FALSE;
			}
		}
...
CRX_MINIT_FUNCTION(pack)
{
	...
	if (MACHINE_LITTLE_ENDIAN) {
		...
		zval val;
		int size = sizeof(C_LVAL(val));
		C_LVAL(val)=0; /*silence a warning*/
