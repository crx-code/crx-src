/*
   ...
   +----------------------------------------------------------------------+
   | Authors: Pierre A. Joye <pierre@crx.net>                             |
   +----------------------------------------------------------------------+
 */
...
CRX_FUNCTION(dns_get_mx) /* {{{ */
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "sz|z", &hostname, &hostname_len, &mx_list, &weight_list) == FAILURE) {
		RETURN_THROWS();
	}
...
CRX_FUNCTION(dns_check_record)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|S", &hostname, &hostname_len, &rectype) == FAILURE) {
		RETURN_THROWS();
	}
	...
	if (rectype) {
		     if (crex_string_equals_literal_ci(rectype,     "A")) type = DNS_TYPE_A;
		else if (crex_string_equals_literal_ci(rectype,    "NS")) type = DNS_TYPE_NS;
		else if (crex_string_equals_literal_ci(rectype,    "MX")) type = DNS_TYPE_MX;
		else if (crex_string_equals_literal_ci(rectype,   "PTR")) type = DNS_TYPE_PTR;
		else if (crex_string_equals_literal_ci(rectype,   "ANY")) type = DNS_TYPE_ANY;
		else if (crex_string_equals_literal_ci(rectype,   "SOA")) type = DNS_TYPE_SOA;
		else if (crex_string_equals_literal_ci(rectype,   "TXT")) type = DNS_TYPE_TEXT;
		else if (crex_string_equals_literal_ci(rectype, "CNAME")) type = DNS_TYPE_CNAME;
		else if (crex_string_equals_literal_ci(rectype,  "AAAA")) type = DNS_TYPE_AAAA;
		else if (crex_string_equals_literal_ci(rectype,   "SRV")) type = DNS_TYPE_SRV;
		else if (crex_string_equals_literal_ci(rectype, "NAPTR")) type = DNS_TYPE_NAPTR;
		else if (crex_string_equals_literal_ci(rectype,    "A6")) type = DNS_TYPE_A6;
		else {
...
/* {{{ crx_parserr */
static void crx_parserr(PDNS_RECORD pRec, int type_to_fetch, int store, bool raw, zval *subarray)
{
	int type;
	u_long ttl;
		...
		case DNS_TYPE_TEXT:
			{
				...
				txt = crex_string_alloc(txt_len, 0);
...
CRX_FUNCTION(dns_get_record)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s|lz!z!b",
	...
	for ( ;
		...
		switch (type) {
			case -1: /* raw */
				...
				break;
			case 0:
				type_to_fetch = type_param&CRX_DNS_A     ? DNS_TYPE_A     : 0;
				break;
			case 1:
				type_to_fetch = type_param&CRX_DNS_NS    ? DNS_TYPE_NS    : 0;
				break;
			case 2:
				type_to_fetch = type_param&CRX_DNS_CNAME ? DNS_TYPE_CNAME : 0;
				break;
			case 3:
				type_to_fetch = type_param&CRX_DNS_SOA   ? DNS_TYPE_SOA   : 0;
				break;
			case 4:
				type_to_fetch = type_param&CRX_DNS_PTR   ? DNS_TYPE_PTR   : 0;
				break;
			case 5:
				type_to_fetch = type_param&CRX_DNS_HINFO ? DNS_TYPE_HINFO : 0;
				break;
			case 6:
				type_to_fetch = type_param&CRX_DNS_MX    ? DNS_TYPE_MX    : 0;
				break;
			case 7:
				type_to_fetch = type_param&CRX_DNS_TXT   ? DNS_TYPE_TEXT   : 0;
				break;
			case 8:
				type_to_fetch = type_param&CRX_DNS_AAAA	 ? DNS_TYPE_AAAA  : 0;
				break;
			case 9:
				type_to_fetch = type_param&CRX_DNS_SRV   ? DNS_TYPE_SRV   : 0;
				break;
			case 10:
				type_to_fetch = type_param&CRX_DNS_NAPTR ? DNS_TYPE_NAPTR : 0;
				break;
			case 11:
				type_to_fetch = type_param&CRX_DNS_A6	 ? DNS_TYPE_A6 : 0;
				break;
		...
		if (type_to_fetch) {
			...
			for (pRec = pResult; pRec; pRec = pRec->pNext) {
				...
				if (pRec->Flags.S.Section == DnsSectionAnswer) {
					crx_parserr(pRec, type_to_fetch, store_results, raw, &retval);
					if (!C_ISUNDEF(retval) && store_results) {
				...
				if (authns && pRec->Flags.S.Section == DnsSectionAuthority) {
					...
					crx_parserr(pRec, type_to_fetch, 1, raw, &retval);
					if (!C_ISUNDEF(retval)) {
