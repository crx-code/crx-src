...
/* Windows VC++ port by Pierre Joye <pierre@crx.net> */
...
extern void * __crx_mempcpy(void * dst, const void * src, size_t len);
extern char * __crx_stpncpy(char *dst, const char *src, size_t len);
...
#if defined(CRX_WIN32) || (!defined(WORDS_BIGENDIAN))
# define SWAP(n) \
  (((n) << 56)					\
   | (((n) & 0xff00) << 40)			\
   | (((n) & 0xff0000) << 24)			\
...
char *
crx_sha512_crypt_r(const char *key, const char *salt, char *buffer, int buflen) {
#ifdef CRX_WIN32
	CREX_SET_ALIGNED(64, unsigned char alt_result[64]);
	CREX_SET_ALIGNED(64, unsigned char temp_result[64]);
#else
	CREX_SET_ALIGNED(__alignof__ (uint64_t), unsigned char alt_result[64]);
	CREX_SET_ALIGNED(__alignof__ (uint64_t), unsigned char temp_result[64]);
#endif
	...
	if (strncmp(salt, sha512_rounds_prefix, sizeof(sha512_rounds_prefix) - 1) == 0) {
		...
		char *endp;
		crex_ulong srounds = CREX_STRTOUL(num, &endp, 10);
	...
	for (cnt = key_len; cnt >= 64; cnt -= 64) {
		cp = __crx_mempcpy((void *) cp, (const void *)temp_result, 64);
	}
	...
	for (cnt = salt_len; cnt >= 64; cnt -= 64) {
		cp = __crx_mempcpy(cp, temp_result, 64);
	}
	memcpy(cp, temp_result, cnt);
	...
	/* Now we can construct the result string.  It consists of three
	 parts.  */
	cp = __crx_stpncpy(buffer, sha512_salt_prefix, MAX(0, buflen));
	buflen -= sizeof(sha512_salt_prefix) - 1;

	if (rounds_custom) {
#ifdef CRX_WIN32
	  int n = _snprintf(cp, MAX(0, buflen), "%s" CREX_ULONG_FMT "$", sha512_rounds_prefix, rounds);
#else
	...
	cp = __crx_stpncpy(cp, salt, MIN((size_t) MAX(0, buflen), salt_len));
...
#define b64_from_24bit(B2, B1, B0, N)                    \
  do {									                 \
	...
	*cp++ = b64t[w & 0x3f];						         \
	...
	sha512_init_ctx(&ctx);
	sha512_finish_ctx(&ctx, alt_result);
	CREX_SECURE_ZERO(temp_result, sizeof(temp_result));
	CREX_SECURE_ZERO(p_bytes, key_len);
	CREX_SECURE_ZERO(s_bytes, salt_len);
	CREX_SECURE_ZERO(&ctx, sizeof(ctx));
	CREX_SECURE_ZERO(&alt_ctx, sizeof(alt_ctx));
	if (copied_key != NULL) {
		CREX_SECURE_ZERO(copied_key, key_len);
	}
	if (copied_salt != NULL) {
		CREX_SECURE_ZERO(copied_salt, salt_len);
	}
	if (tmp_key != NULL) {
...
/* This entry point is equivalent to the `crypt' function in Unix
   libcs.  */
char *
crx_sha512_crypt(const char *key, const char *salt) {
	/* We don't want to have an arbitrary limit in the size of the
	 password.  We can compute an upper bound for the size of the
	 result in advance and so we can prepare the buffer we pass to
	 `sha512_crypt_r'.  */
	CREX_TLS char *buffer;
	CREX_TLS int buflen = 0;
	...
	return crx_sha512_crypt_r (key, salt, buffer, buflen);
}
...
int main (void) {
	...
	for (cnt = 0; cnt < ntests2; ++cnt) {
		char *cp = crx_sha512_crypt(tests2[cnt].input, tests2[cnt].salt);
