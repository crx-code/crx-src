...
typedef crex_long (*int_string_formater)(const char*, char**, int);
 ...
 *
 * Side effects:
 *     May set crx_error based on abnormal conditions.
 *
 * Parameters :
 ...
 *----------------------------------------------------------------------
*/
CRXAPI int ValidateFormat(char *format, int numVars, int *totalSubs)
{
#define STATIC_LIST_SIZE 16
	...
	while (*format != '\0') {
		...
		if ( isdigit( (int)*ch ) ) {
			 ...
			 */
			value = CREX_STRTOUL(format-1, &end, 10);
			if (*end != '$') {
				goto notXpg;
			}
			format = end+1;
...
notXpg:
		...
		if (gotXpg) {
mixedXPG:
			crex_value_error("%s", "cannot mix \"%\" and \"%n$\" conversion specifiers");
			goto error;
		}
...
xpgCheckDone:
		 ...
		 */
		if (isdigit(UCHAR(*ch))) {
			value = CREX_STRTOUL(format-1, &format, 10);
			flags |= SCAN_WIDTH;
			ch = format++;
		}
		...
		switch (*ch) {
			...
			case 'c':
				...
				/* ANSI. since Crex auto allocates space for vars, this is no */
				/* problem - cc                                               */
				/*
				if (flags & SCAN_WIDTH) {
					crx_error_docref(NULL, E_WARNING, "Field width may not be specified in %c conversion");
					goto error;
				}
				*/
				break;
			...
			case '[':
				...
				break;
badSet:
				crex_value_error("Unmatched [ in format string");
				goto error;

			default: {
				crex_value_error("Bad scan conversion character \"%c\"", *ch);
				goto error;
			}
		}
	...
	for (i = 0; i < numVars; i++) {
		if (nassign[i] > 1) {
			crex_value_error("%s", "Variable is assigned by multiple \"%n$\" conversion specifiers");
			goto error;
			 ...
			 */
			crex_value_error("Variable is not assigned by any conversion specifiers");
			goto error;
		}
	}
...
badIndex:
	if (gotXpg) {
		crex_value_error("%s", "\"%n$\" argument index out of range");
	} else {
		crex_value_error("Different numbers of variable names and field specifiers");
	}
...
CRXAPI int crx_sscanf_internal( char *string, char *format,
				...
				int varStart, zval *return_value)
{
	...
	if (numVars) {
		for (i = varStart;i < argCount;i++){
			CREX_ASSERT(C_ISREF(args[i]) && "Parameter must be passed by reference");
		}
	}
...
literal:
		 ...
		 */
		if ( isdigit(UCHAR(*ch))) {
			width = CREX_STRTOUL(format-1, &format, 10);
			ch = format++;
		} else {
			width = 0;
		}
		...
		switch (*ch) {
			case 'n':
				if (!(flags & SCAN_SUPPRESS)) {
					if (numVars && objIndex >= argCount) {
						...
						CREX_TRY_ASSIGN_REF_LONG(current, (crex_long) (string - baseString));
					} else {
			...
			case 'D':
				op = 'i';
				base = 10;
				fn = (int_string_formater)CREX_STRTOL_PTR;
				break;
			case 'i':
				op = 'i';
				base = 0;
			...
			case 'u':
				...
				base = 10;
				flags |= SCAN_UNSIGNED;
				fn = (int_string_formater)CREX_STRTOUL_PTR;
				break;
			...
			case 's':
				...
				if (!(flags & SCAN_SUPPRESS)) {
					if (numVars && objIndex >= argCount) {
						...
						current = args + objIndex++;
						CREX_TRY_ASSIGN_REF_STRINGL(current, string, end - string);
					} else {
...
addToInt:
				 ...
				 * to a string since CRX only supports signed values.
				 */
				if (!(flags & SCAN_SUPPRESS)) {
					*end = '\0';
					value = (crex_long) (*fn)(buf, NULL, base);
					if ((flags & SCAN_UNSIGNED) && (value < 0)) {
						snprintf(buf, sizeof(buf), CREX_ULONG_FMT, value); /* INTL: ISO digit */
						if (numVars && objIndex >= argCount) {
							break;
						} else if (numVars) {
							 /* change passed value type to string */
							current = args + objIndex++;
							CREX_TRY_ASSIGN_REF_STRING(current, buf);
						} else {
						...
						if (numVars && objIndex >= argCount) {
							...
							current = args + objIndex++;
							CREX_TRY_ASSIGN_REF_LONG(current, value);
						} else {
...
addToFloat:
				...
				if (!(flags & SCAN_SUPPRESS)) {
					double dvalue;
					*end = '\0';
					dvalue = crex_strtod(buf, NULL);
					if (numVars && objIndex >= argCount) {
						break;
					} else if (numVars) {
						current = args + objIndex++;
						CREX_TRY_ASSIGN_REF_DOUBLE(current, dvalue);
					} else {
