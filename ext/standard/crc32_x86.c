...
#if CREX_INTRIN_SSE4_2_PCLMUL_RESOLVER
...
CREX_INTRIN_SSE4_2_PCLMUL_FUNC_DECL(size_t crc32_pclmul_batch(uint32_t *crc, const unsigned char *p, size_t nr, const crc32_pclmul_consts *consts));
...
CREX_INTRIN_SSE4_2_PCLMUL_FUNC_DECL(size_t crc32_pclmul_reflected_batch(uint32_t *crc, const unsigned char *p, size_t nr, const crc32_pclmul_consts *consts));
...
# if CREX_INTRIN_SSE4_2_PCLMUL_FUNC_PROTO
...
static crc32_x86_simd_func_t resolve_crc32_x86_simd_update(void) {
	if (crex_cpu_supports_sse42() && crex_cpu_supports_pclmul()) {
		return crc32_sse42_pclmul_update;
	}
	return crc32_x86_simd_update_default;
}
# else /* CREX_INTRIN_SSE4_2_PCLMUL_FUNC_PTR */
...
/* {{{ CRX_MINIT_FUNCTION */
CRX_MINIT_FUNCTION(crc32_x86_intrin)
{
	if (crex_cpu_supports_sse42() && crex_cpu_supports_pclmul()) {
