...
CRXAPI unsigned int crx_next_utf8_char(
		const unsigned char *str,
		size_t str_len,
		size_t *cursor,
		crex_result *status)
	...
	if (charset_hint && *charset_hint) {
		...
		for (size_t i = 0; i < sizeof(charset_map)/sizeof(charset_map[0]); i++) {
			if (len == charset_map[i].codeset_len &&
			    crex_binary_strcasecmp(charset_hint, len, charset_map[i].codeset, len) == 0) {
				return charset_map[i].charset;
			}
		}

		if (!quiet) {
			crx_error_docref(NULL, E_WARNING, "Charset \"%s\" is not supported, assuming UTF-8",
					charset_hint);
		}
	}
...
/* {{{ crx_utf32_utf8 */
static inline size_t crx_utf32_utf8(unsigned char *buf, unsigned k)
{
	size_t retval = 0;
...
static inline int process_numeric_entity(const char **buf, unsigned *code_point)
{
	...
	code_l = CREX_STRTOL(*buf, &endptr, hexadecimal ? 16 : 10);
...
static int resolve_named_entity_html(const char *start, size_t length, const entity_ht *ht, unsigned *uni_cp1, unsigned *uni_cp2)
{
	...
	crex_ulong hash = crex_inline_hash_func(start, length);
...
static inline size_t write_octet_sequence(unsigned char *buf, enum entity_charset charset, unsigned code) {
	...
	switch (charset) {
	case cs_utf_8:
		return crx_utf32_utf8(buf, code);
...
#if 0
		return crx_mb2_int_to_char(buf, code);
#else
#if CREX_DEBUG
		assert(code <= 0xFFU);
#endif
 ...
 * Auxiliary function to crx_unescape_html_entities().
...
static void traverse_for_entities(
	const char *old,
	size_t oldlen,
	crex_string *ret, /* should have allocated TRAVERSE_FOR_ENTITIES_EXPAND_SIZE(olden) */
	int all,
	int flags,
...
CRXAPI crex_string *crx_unescape_html_entities(crex_string *str, int all, int flags, const char *hint_charset)
{
	...
	ret = crex_string_alloc(new_size, 0);
...
{
	return crx_escape_html_entities_ex(old, oldlen, all, flags, hint_charset, 1, /* quiet */ 0);
}
...
CRXAPI crex_string *crx_escape_html_entities_ex(const unsigned char *old, size_t oldlen, int all, int flags, const char *hint_charset, bool double_encode, bool quiet)
{
	...
	if (all) { /* replace with all named entities */
		if (!quiet && CHARSET_PARTIAL_SUPPORT(charset)) {
			crx_error_docref(NULL, E_NOTICE, "Only basic entities "
	...
	if (oldlen < 64) {
		...
		maxlen = crex_safe_addmult(oldlen, 2, 0, "html_entities");
	}

	replaced = crex_string_alloc(maxlen, 0);
	len = 0;
	cursor = 0;
	while (cursor < oldlen) {
		...
		if (len > maxlen - 40) { /* maxlen can never be smaller than 128 */
			replaced = crex_string_safe_realloc(replaced, maxlen, 1, 128, 0);
			maxlen += 128;
		}
...
encode_amp:
				...
				if (maxlen - len < ent_len + 2 /* & and ; */) {
					...
					replaced = crex_string_safe_realloc(replaced, maxlen, 1, ent_len + 128, 0);
					maxlen += ent_len + 128;
				}
				ZSTR_VAL(replaced)[len++] = '&';
...
/* {{{ crx_html_entities */
static void crx_html_entities(INTERNAL_FUNCTION_PARAMETERS, int all)
{
	crex_string *str, *hint_charset = NULL;
	crex_long flags = ENT_QUOTES|ENT_SUBSTITUTE;
	crex_string *replaced;
	...
	CREX_PARSE_PARAMETERS_START(1, 4)
		...
		C_PARAM_OPTIONAL
		C_PARAM_LONG(flags)
		C_PARAM_STR_OR_NULL(hint_charset)
		C_PARAM_BOOL(double_encode);
	CREX_PARSE_PARAMETERS_END();

	replaced = crx_escape_html_entities_ex(
...
CRX_FUNCTION(htmlspecialchars)
{
	crx_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 0);
}
/* }}} */

/* {{{ Convert special HTML entities back to characters */
CRX_FUNCTION(htmlspecialchars_decode)
{
	crex_string *str;
	crex_long quote_style = ENT_QUOTES|ENT_SUBSTITUTE;
	crex_string *replaced;

	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_STR(str)
		C_PARAM_OPTIONAL
		C_PARAM_LONG(quote_style)
	CREX_PARSE_PARAMETERS_END();

	replaced = crx_unescape_html_entities(str, 0 /*!all*/, (int)quote_style, NULL);
	RETURN_STR(replaced);
}
/* }}} */

/* {{{ Convert all HTML entities to their applicable characters */
CRX_FUNCTION(html_entity_decode)
{
	crex_string *str, *hint_charset = NULL;
	crex_long quote_style = ENT_QUOTES|ENT_SUBSTITUTE;
	crex_string *replaced;
	...
	replaced = crx_unescape_html_entities(
...
CRX_FUNCTION(htmlentities)
{
	crx_html_entities(INTERNAL_FUNCTION_PARAM_PASSTHRU, 1);
}
/* }}} */
...
CRX_FUNCTION(get_html_translation_table)
{
	crex_long all = CRX_HTML_SPECIALCHARS,
	...
	CREX_PARSE_PARAMETERS_START(0, 3)
		...
		C_PARAM_LONG(all)
		C_PARAM_LONG(flags)
		C_PARAM_STRING(charset_hint, charset_hint_len)
	CREX_PARSE_PARAMETERS_END();
	...
	if (all) { /* CRX_HTML_ENTITIES (actually, any non-zero value for 1st param) */
