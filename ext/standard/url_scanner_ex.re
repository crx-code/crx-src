...
static int crx_ini_on_update_tags(crex_ini_entry *entry, crex_string *new_value, void *mh_arg1, void *mh_arg2, void *mh_arg3, int stage, int type)
{
	...
	crex_hash_init(ctx->tags, 0, NULL, tag_dtor, 1);

	for (key = crx_strtok_r(tmp, ",", &lasts);
		 key;
		 key = crx_strtok_r(NULL, ",", &lasts)) {
		char *val;
		...
		if (val) {
			...
			*val++ = '\0';
			...
			keylen = q - key;
			str = crex_string_init(key, keylen, 1);
			GC_MAKE_PERSISTENT_LOCAL(str);
			crex_hash_add_mem(ctx->tags, str, val, strlen(val)+1);
...
static CRX_INI_MH(OnUpdateSessionTags)
{
	return crx_ini_on_update_tags(entry, new_value, mh_arg1, mh_arg2, mh_arg3, stage, 1);
}

static CRX_INI_MH(OnUpdateOutputTags)
{
	return crx_ini_on_update_tags(entry, new_value, mh_arg1, mh_arg2, mh_arg3, stage, 0);
}

static int crx_ini_on_update_hosts(crex_ini_entry *entry, crex_string *new_value, void *mh_arg1, void *mh_arg2, void *mh_arg3, int stage, int type)
{
	HashTable *hosts;
	char *key;
	char *tmp;
	...
	for (key = crx_strtok_r(tmp, ",", &lasts);
		...
		keylen = q - key;
		if (keylen > 0) {
			tmp_key = crex_string_init(key, keylen, 0);
			crex_hash_add_empty_element(hosts, tmp_key);
			crex_string_release_ex(tmp_key, 0);
		}
	}
	efree(tmp);
...
static CRX_INI_MH(OnUpdateSessionHosts)
{
	return crx_ini_on_update_hosts(entry, new_value, mh_arg1, mh_arg2, mh_arg3, stage, 1);
}

static CRX_INI_MH(OnUpdateOutputHosts)
{
	return crx_ini_on_update_hosts(entry, new_value, mh_arg1, mh_arg2, mh_arg3, stage, 0);
}

/* FIXME: OnUpdate*Hosts cannot set default to $_SERVER['HTTP_HOST'] at startup */
CRX_INI_BEGIN()
	STD_CRX_INI_ENTRY("session.trans_sid_tags", "a=href,area=href,frame=src,form=", CRX_INI_ALL, OnUpdateSessionTags, url_adapt_session_ex, crx_basic_globals, basic_globals)
	STD_CRX_INI_ENTRY("session.trans_sid_hosts", "", CRX_INI_ALL, OnUpdateSessionHosts, url_adapt_session_hosts_ht, crx_basic_globals, basic_globals)
	STD_CRX_INI_ENTRY("url_rewriter.tags", "form=", CRX_INI_ALL, OnUpdateOutputTags, url_adapt_session_ex, crx_basic_globals, basic_globals)
	STD_CRX_INI_ENTRY("url_rewriter.hosts", "", CRX_INI_ALL, OnUpdateOutputHosts, url_adapt_session_hosts_ht, crx_basic_globals, basic_globals)
CRX_INI_END()
...
static inline void append_modified_url(smart_str *url, smart_str *dest, smart_str *url_app, const char *separator)
{
	...
	smart_str_0(url); /* FIXME: Bug #70480 crx_url_parse_ex() crashes by processing chars exceed len */
	url_parts = crx_url_parse_ex(ZSTR_VAL(url->s), ZSTR_LEN(url->s));
	...
	if (url_parts->fragment && '#' == ZSTR_VAL(url->s)[0]) {
		smart_str_append_smart_str(dest, url);
		crx_url_free(url_parts);
		return;
	}

	/* Check protocol. Only http/https is allowed. */
	if (url_parts->scheme
		&& !crex_string_equals_literal_ci(url_parts->scheme, "http")
		&& !crex_string_equals_literal_ci(url_parts->scheme, "https")) {
		smart_str_append_smart_str(dest, url);
		crx_url_free(url_parts);
		return;
	}
	...
	if (url_parts->host) {
		crex_string *tmp = crex_string_tolower(url_parts->host);
		if (!crex_hash_exists(&BG(url_adapt_session_hosts_ht), tmp)) {
...
{
	zval *host, *tmp;
	crex_string *host_tmp;
	char *colon;

	if ((tmp = crex_hash_find(&EG(symbol_table), ZSTR_KNOWN(CREX_STR_AUTOGLOBAL_SERVER))) &&
		C_TYPE_P(tmp) == IS_ARRAY &&
		(host = crex_hash_str_find(C_ARRVAL_P(tmp), CREX_STRL("HTTP_HOST"))) &&
		C_TYPE_P(host) == IS_STRING) {
		host_tmp = crex_string_init(C_STRVAL_P(host), C_STRLEN_P(host), 0);
		...
		if (!strcasecmp(ZSTR_VAL(host_tmp), target)) {
			crex_string_release_ex(host_tmp, 0);
			return SUCCESS;
		}
		crex_string_release_ex(host_tmp, 0);
	}
...
static int check_host_whitelist(url_adapt_state_ex_t *ctx)
{
	...
	if (url_parts->scheme) {
		...
		if (!crex_string_equals_literal_ci(url_parts->scheme, "http") &&
			!crex_string_equals_literal_ci(url_parts->scheme, "https")) {
		crx_url_free(url_parts);
		return FAILURE;
		}
	...
	if (!crex_hash_num_elements(allowed_hosts) &&
		check_http_host(ZSTR_VAL(url_parts->host)) == SUCCESS) {
		crx_url_free(url_parts);
		return SUCCESS;
	}
	if (!crex_hash_find(allowed_hosts, url_parts->host)) {
		crx_url_free(url_parts);
		return FAILURE;
	}
	crx_url_free(url_parts);
...
static inline void handle_tag(STD_PARA)
{
	...
	if ((ctx->lookup_data = crex_hash_str_find_ptr(ctx->tags, ZSTR_VAL(ctx->tag.s), ZSTR_LEN(ctx->tag.s))) != NULL) {
		ok = 1;
...
static int crx_url_scanner_ex_activate(int type)
{
	url_adapt_state_ex_t *ctx;
...
static int crx_url_scanner_ex_deactivate(int type)
{
	url_adapt_state_ex_t *ctx;
...
static inline void crx_url_scanner_session_handler_impl(char *output, size_t output_len, char **handled_output, size_t *handled_output_len, int mode, int type)
{
	size_t len;
	...
	if (ZSTR_LEN(url_state->url_app.s) != 0) {
		*handled_output = url_adapt_ext(output, output_len, &len, (bool) (mode & (CRX_OUTPUT_HANDLER_END | CRX_OUTPUT_HANDLER_CONT | CRX_OUTPUT_HANDLER_FLUSH | CRX_OUTPUT_HANDLER_FINAL) ? 1 : 0), url_state);
...
static void crx_url_scanner_session_handler(char *output, size_t output_len, char **handled_output, size_t *handled_output_len, int mode)
{
	crx_url_scanner_session_handler_impl(output, output_len, handled_output, handled_output_len, mode, 1);
}

static void crx_url_scanner_output_handler(char *output, size_t output_len, char **handled_output, size_t *handled_output_len, int mode)
{
	crx_url_scanner_session_handler_impl(output, output_len, handled_output, handled_output_len, mode, 0);
}

static inline int crx_url_scanner_add_var_impl(const char *name, size_t name_len, const char *value, size_t value_len, int encode, int type)
{
	smart_str sname = {0};
	smart_str svalue = {0};
	smart_str hname = {0};
	...
	crex_string *encoded;
	url_adapt_state_ex_t *url_state;
	crx_output_handler_func_t handler;

	if (type) {
		url_state = &BG(url_adapt_session_ex);
		handler = crx_url_scanner_session_handler;
	} else {
		url_state = &BG(url_adapt_output_ex);
		handler = crx_url_scanner_output_handler;
	}

	if (!url_state->active) {
		crx_url_scanner_ex_activate(type);
		crx_output_start_internal(CREX_STRL("URL-Rewriter"), handler, 0, CRX_OUTPUT_HANDLER_STDFLAGS);
		url_state->active = 1;
	}
	...
	if (encode) {
		encoded = crx_raw_url_encode(name, name_len);
		smart_str_appendl(&sname, ZSTR_VAL(encoded), ZSTR_LEN(encoded)); crex_string_free(encoded);
		encoded = crx_raw_url_encode(value, value_len);
		smart_str_appendl(&svalue, ZSTR_VAL(encoded), ZSTR_LEN(encoded)); crex_string_free(encoded);
		encoded = crx_escape_html_entities_ex((const unsigned char *) name, name_len, 0, ENT_QUOTES|ENT_SUBSTITUTE, NULL, /* double_encode */ 0, /* quiet */ 1);
		smart_str_appendl(&hname, ZSTR_VAL(encoded), ZSTR_LEN(encoded)); crex_string_free(encoded);
		encoded = crx_escape_html_entities_ex((const unsigned char *) value, value_len, 0, ENT_QUOTES|ENT_SUBSTITUTE, NULL, /* double_encode */ 0, /* quiet */ 1);
		smart_str_appendl(&hvalue, ZSTR_VAL(encoded), ZSTR_LEN(encoded)); crex_string_free(encoded);
	} else {
...
{
	return crx_url_scanner_add_var_impl(name, name_len, value, value_len, encode, 1);
}


CRXAPI int crx_url_scanner_add_var(const char *name, size_t name_len, const char *value, size_t value_len, int encode)
{
	return crx_url_scanner_add_var_impl(name, name_len, value, value_len, encode, 0);
}


static inline void crx_url_scanner_reset_vars_impl(int type) {
	url_adapt_state_ex_t *url_state;
...
{
	crx_url_scanner_reset_vars_impl(1);
	return SUCCESS;
}
...
{
	crx_url_scanner_reset_vars_impl(0);
	return SUCCESS;
}


static inline int crx_url_scanner_reset_var_impl(crex_string *name, int encode, int type)
{
	...
	if (encode) {
		encoded = crx_raw_url_encode(ZSTR_VAL(name), ZSTR_LEN(name));
		smart_str_appendl(&sname, ZSTR_VAL(encoded), ZSTR_LEN(encoded));
		crex_string_free(encoded);
		encoded = crx_escape_html_entities_ex((const unsigned char *) ZSTR_VAL(name), ZSTR_LEN(name), 0, ENT_QUOTES|ENT_SUBSTITUTE, SG(default_charset), /* double_encode */ 0, /* quiet */ 1);
	...
	/* Short circuit check. Only check url_app. */
	start = (char *) crx_memnstr(ZSTR_VAL(url_state->url_app.s),
	...
	if (ZSTR_LEN(url_state->url_app.s) == end - start) {
		crx_url_scanner_reset_vars_impl(type);
		goto finish;
	}
	...
	/* Remove form var */
	start = (char *) crx_memnstr(ZSTR_VAL(url_state->form_app.s),
...
{
	return crx_url_scanner_reset_var_impl(name, encode, 1);
}


CRXAPI int crx_url_scanner_reset_var(crex_string *name, int encode)
{
	return crx_url_scanner_reset_var_impl(name, encode, 0);
}


CRX_MINIT_FUNCTION(url_scanner)
{
	REGISTER_INI_ENTRIES();
	return SUCCESS;
}

CRX_MSHUTDOWN_FUNCTION(url_scanner)
{
...
CRX_RINIT_FUNCTION(url_scanner)
{
...
CRX_RSHUTDOWN_FUNCTION(url_scanner)
{
	if (BG(url_adapt_session_ex).active) {
		crx_url_scanner_ex_deactivate(1);
		BG(url_adapt_session_ex).active    = 0;
		BG(url_adapt_session_ex).tag_type  = 0;
		BG(url_adapt_session_ex).attr_type = 0;
	}
	...
	if (BG(url_adapt_output_ex).active) {
		crx_url_scanner_ex_deactivate(0);
		BG(url_adapt_output_ex).active    = 0;
		BG(url_adapt_output_ex).tag_type  = 0;
		BG(url_adapt_output_ex).attr_type = 0;
	}
