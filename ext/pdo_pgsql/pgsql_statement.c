...
static int pgsql_stmt_dtor(pdo_stmt_t *stmt)
{
	...
	bool server_obj_usable = !C_ISUNDEF(stmt->database_object_handle)
		&& IS_OBJ_VALID(EG(objects_store).object_buckets[C_OBJ_HANDLE(stmt->database_object_handle)])
		&& !(OBJ_FLAGS(C_OBJ(stmt->database_object_handle)) & IS_OBJ_FREE_CALLED);
...
stmt_retry:
			...
			switch (status) {
				...
				default: {
					 ...
					 * crx disconnects. if crx dies (no chance to run RSHUTDOWN) during execution it has no
		...
		S->result = PQexecPrepared(H->server, S->stmt_name,
				stmt->bound_params ?
					crex_hash_num_elements(stmt->bound_params) :
					0,
	...
	if (status == PGRES_COMMAND_OK) {
		stmt->row_count = CREX_ATOL(PQcmdTuples(S->result));
		H->pgoid = PQoidValue(S->result);
	} else {
		stmt->row_count = (crex_long)PQntuples(S->result);
	}
	...
	if (stmt->supports_placeholders == PDO_PLACEHOLDER_NAMED && param->is_param) {
		switch (event_type) {
			...
			case PDO_PARAM_EVT_NORMALIZE:
				...
				if (param->name) {
					if (ZSTR_VAL(param->name)[0] == '$') {
						param->paramno = CREX_ATOL(ZSTR_VAL(param->name) + 1);
					} else {
						/* resolve parameter name to rewritten name */
						crex_string *namevar;

						if (stmt->bound_param_map && (namevar = crex_hash_find_ptr(stmt->bound_param_map,
								param->name)) != NULL) {
							param->paramno = CREX_ATOL(ZSTR_VAL(namevar) + 1);
							param->paramno--;
						} else {
			...
			case PDO_PARAM_EVT_ALLOC:
				...
				if (!crex_hash_index_exists(stmt->bound_param_map, param->paramno)) {
			...
			case PDO_PARAM_EVT_EXEC_PRE:
				...
				if (!S->param_values) {
					S->param_values = ecalloc(
							crex_hash_num_elements(stmt->bound_param_map),
							sizeof(char*));
				...
				if (param->paramno >= 0) {
					...
					/*
					if (param->paramno >= crex_hash_num_elements(stmt->bound_params)) {
					...
					*/
					...
					if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_LOB &&
							C_TYPE_P(parameter) == IS_RESOURCE) {
						crx_stream *stm;
						crx_stream_from_zval_no_verify(stm, parameter);
						if (stm) {
							if (crx_stream_is(stm, &pdo_pgsql_lob_stream_ops)) {
								...
								crex_string *str = crx_stream_copy_to_mem(stm, CRX_STREAM_COPY_ALL, 0);
								if (str != NULL) {
					...
					if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_NULL ||
							C_TYPE_P(parameter) == IS_NULL) {
						S->param_values[param->paramno] = NULL;
						S->param_lengths[param->paramno] = 0;
					} else if (C_TYPE_P(parameter) == IS_FALSE || C_TYPE_P(parameter) == IS_TRUE) {
						S->param_values[param->paramno] = C_TYPE_P(parameter) == IS_TRUE ? "t" : "f";
						S->param_lengths[param->paramno] = 1;
						S->param_formats[param->paramno] = 0;
					} else {
						convert_to_string(parameter);
						S->param_values[param->paramno] = C_STRVAL_P(parameter);
						S->param_lengths[param->paramno] = C_STRLEN_P(parameter);
						S->param_formats[param->paramno] = 0;
					}
		...
		if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_BOOL &&
			...
			const char *s = crex_is_true(&param->parameter) ? "t" : "f";
	...
	enum pdo_fetch_orientation ori, crex_long offset)
{
	...
	if (S->cursor_name) {
		...
		switch (ori) {
			...
			case PDO_FETCH_ORI_ABS:		spprintf(&ori_str, 0, "ABSOLUTE " CREX_LONG_FMT, offset); break;
			case PDO_FETCH_ORI_REL:		spprintf(&ori_str, 0, "RELATIVE " CREX_LONG_FMT, offset); break;
			default:
				return 0;
		}
...
static int pgsql_stmt_describe(pdo_stmt_t *stmt, int colno)
{
	...
	cols[colno].name = crex_string_init(str, strlen(str), 0);
...
static int pgsql_stmt_get_col(pdo_stmt_t *stmt, int colno, zval *result, enum pdo_param_type *type)
{
	...
	if (PQgetisnull(S->result, S->current_row - 1, colno)) { /* Check if we got NULL */
		...
		switch (S->cols[colno].pgsql_type) {
			...
			case INT8OID:
#endif
				ZVAL_LONG(result, CREX_ATOL(ptr));
				break;
			...
			case OIDOID: {
				...
				if (type && *type == PDO_PARAM_LOB) {
					...
					if (loid >= 0) {
						crx_stream *stream = pdo_pgsql_create_lob_stream(&stmt->database_object_handle, loid, oid);
						if (stream) {
							crx_stream_to_zval(stream, result);
							return 1;
						}
					}
					return 0;
			...
			case BYTEAOID: {
				...
				crex_string *str = crex_string_init(tmp_ptr, tmp_len, 0);
				crx_stream *stream = crx_stream_memory_open(TEMP_STREAM_READONLY, str);
				crx_stream_to_zval(stream, result);
...
static crex_always_inline char * pdo_pgsql_translate_oid_to_table(Oid oid, PGconn *conn)
{
...
static int pgsql_stmt_get_column_meta(pdo_stmt_t *stmt, crex_long colno, zval *return_value)
{
	pdo_pgsql_stmt *S = (pdo_pgsql_stmt*)stmt->driver_data;
	PGresult *res;
	char *q=NULL;
