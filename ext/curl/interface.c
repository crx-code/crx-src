...
#  define CRX_CURL_NEED_OPENSSL_TSL
...
#ifdef __GNUC__
/* don't complain about deprecated CURLOPT_* we're exposing to CRX; we
   need to keep using those to avoid breaking CRX API compatibiltiy */
# pragma GCC diagnostic ignored "-Wdeprecated-declarations"
#endif

#include "curl_arginfo.h"

#ifdef CRX_CURL_NEED_OPENSSL_TSL /* {{{ */
static MUTEX_T *crx_curl_openssl_tsl = NULL;

/* Locking callbacks are no longer used since OpenSSL 1.1. Mark the functions as unused to
 * avoid warnings due to this. */
static CREX_ATTRIBUTE_UNUSED void crx_curl_ssl_lock(int mode, int n, const char * file, int line)
{
	if (mode & CRYPTO_LOCK) {
		tsrm_mutex_lock(crx_curl_openssl_tsl[n]);
	} else {
		tsrm_mutex_unlock(crx_curl_openssl_tsl[n]);
	}
}

static CREX_ATTRIBUTE_UNUSED unsigned long crx_curl_ssl_id(void)
{
...
#define CAAL(s, v) add_assoc_long_ex(return_value, s, sizeof(s) - 1, (crex_long) v);
#define CAAD(s, v) add_assoc_double_ex(return_value, s, sizeof(s) - 1, (double) v);
#define CAAS(s, v) add_assoc_string_ex(return_value, s, sizeof(s) - 1, (char *) (v ? v : ""));
#define CAASTR(s, v) add_assoc_str_ex(return_value, s, sizeof(s) - 1, \
		v ? crex_string_copy(v) : ZSTR_EMPTY_ALLOC());
#define CAAZ(s, v) add_assoc_zval_ex(return_value, s, sizeof(s) -1 , (zval *) v);

#if defined(CRX_WIN32) || defined(__GNUC__)
# define crx_curl_ret(__ret) RETVAL_FALSE; return __ret;
#else
# define crx_curl_ret(__ret) RETVAL_FALSE; return;
#endif

static crex_result crx_curl_option_str(crx_curl *ch, crex_long option, const char *str, const size_t len)
{
	if (crex_char_has_nul_byte(str, len)) {
		crex_value_error("%s(): cURL option must not contain any null bytes", get_active_function_name());
		return FAILURE;
	}
...
static crex_result crx_curl_option_url(crx_curl *ch, const crex_string *url) /* {{{ */
{
...
#if LIBCURL_VERSION_NUM > 0x073800 && defined(CRX_WIN32)
	if (
		crex_string_starts_with_literal_ci(url, "file://")
		...
		return crx_curl_option_str(ch, CURLOPT_URL, _tmp, ZSTR_LEN(url) + 1);
	}
#endif

	return crx_curl_option_str(ch, CURLOPT_URL, ZSTR_VAL(url), ZSTR_LEN(url));
}
/* }}} */
...
void _crx_curl_verify_handlers(crx_curl *ch, bool reporterror) /* {{{ */
{
	...
	if (!C_ISUNDEF(ch->handlers.std_err)) {
		stream = (crx_stream *)crex_fetch_resource2_ex(&ch->handlers.std_err, NULL, crx_file_le_stream(), crx_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				crx_error_docref(NULL, E_WARNING, "CURLOPT_STDERR resource has gone away, resetting to stderr");
			}
	...
	if (ch->handlers.read && !C_ISUNDEF(ch->handlers.read->stream)) {
		stream = (crx_stream *)crex_fetch_resource2_ex(&ch->handlers.read->stream, NULL, crx_file_le_stream(), crx_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				crx_error_docref(NULL, E_WARNING, "CURLOPT_INFILE resource has gone away, resetting to default");
			}
	...
	if (ch->handlers.write_header && !C_ISUNDEF(ch->handlers.write_header->stream)) {
		stream = (crx_stream *)crex_fetch_resource2_ex(&ch->handlers.write_header->stream, NULL, crx_file_le_stream(), crx_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				crx_error_docref(NULL, E_WARNING, "CURLOPT_WRITEHEADER resource has gone away, resetting to default");
			}
			zval_ptr_dtor(&ch->handlers.write_header->stream);
			ZVAL_UNDEF(&ch->handlers.write_header->stream);
			ch->handlers.write_header->fp = 0;

			ch->handlers.write_header->method = CRX_CURL_IGNORE;
			curl_easy_setopt(ch->cp, CURLOPT_WRITEHEADER, (void *) ch);
		}
	}
	if (ch->handlers.write && !C_ISUNDEF(ch->handlers.write->stream)) {
		stream = (crx_stream *)crex_fetch_resource2_ex(&ch->handlers.write->stream, NULL, crx_file_le_stream(), crx_file_le_pstream());
		if (stream == NULL) {
			if (reporterror) {
				crx_error_docref(NULL, E_WARNING, "CURLOPT_FILE resource has gone away, resetting to default");
			}
			zval_ptr_dtor(&ch->handlers.write->stream);
			ZVAL_UNDEF(&ch->handlers.write->stream);
			ch->handlers.write->fp = 0;

			ch->handlers.write->method = CRX_CURL_STDOUT;
...
/* {{{ curl_module_entry */
crex_module_entry curl_module_entry = {
...
CREX_GET_MODULE (curl)
#endif

/* CurlHandle class */

crex_class_entry *curl_ce;
crex_class_entry *curl_share_ce;
static crex_object_handlers curl_object_handlers;

static crex_object *curl_create_object(crex_class_entry *class_type);
static void curl_free_obj(crex_object *object);
static HashTable *curl_get_gc(crex_object *object, zval **table, int *n);
static crex_function *curl_get_constructor(crex_object *object);
static crex_object *curl_clone_obj(crex_object *object);
crx_curl *init_curl_handle_into_zval(zval *curl);
static inline crex_result build_mime_structure_from_hash(crx_curl *ch, zval *zpostfields);

/* {{{ CRX_INI_BEGIN */
CRX_INI_BEGIN()
	CRX_INI_ENTRY("curl.cainfo", "", CRX_INI_SYSTEM, NULL)
CRX_INI_END()
/* }}} */
...
CRX_MINFO_FUNCTION(curl)
{
	...
	crx_info_print_table_row(2, "cURL support",    "enabled");
	crx_info_print_table_row(2, "cURL Information", d->version);
	sprintf(str, "%d", d->age);
	crx_info_print_table_row(2, "Age", str);
	...
	if (d->features) {
		...
		crx_info_print_table_row(1, "Features");
		for(i=0; i<sizeof(feats)/sizeof(feats[0]); i++) {
			if (feats[i].name) {
				crx_info_print_table_row(2, feats[i].name, d->features & feats[i].bitmask ? "Yes" : "No");
			}
		}
	}
	...
	crx_info_print_table_row(2, "Protocols", str);

	crx_info_print_table_row(2, "Host", d->host);

	if (d->ssl_version) {
		crx_info_print_table_row(2, "SSL Version", d->ssl_version);
	}

	if (d->libz_version) {
		crx_info_print_table_row(2, "ZLib Version", d->libz_version);
	}

#if defined(CURLVERSION_SECOND) && CURLVERSION_NOW >= CURLVERSION_SECOND
	if (d->ares) {
		crx_info_print_table_row(2, "ZLib Version", d->ares);
	}
#endif

#if defined(CURLVERSION_THIRD) && CURLVERSION_NOW >= CURLVERSION_THIRD
	if (d->libidn) {
		crx_info_print_table_row(2, "libIDN Version", d->libidn);
	}
#endif

	if (d->iconv_ver_num) {
		crx_info_print_table_row(2, "IconV Version", d->iconv_ver_num);
	}

	if (d->libssh_version) {
		crx_info_print_table_row(2, "libSSH Version", d->libssh_version);
	}
...
CRX_MINIT_FUNCTION(curl)
{
	...
	if (!CRYPTO_get_id_callback()) {
		...
		crx_curl_openssl_tsl = malloc(c * sizeof(MUTEX_T));
		if (!crx_curl_openssl_tsl) {
			return FAILURE;
		}

		for (i = 0; i < c; ++i) {
			crx_curl_openssl_tsl[i] = tsrm_mutex_alloc();
		}

		CRYPTO_set_id_callback(crx_curl_ssl_id);
		CRYPTO_set_locking_callback(crx_curl_ssl_lock);
	}
#endif
	...
	memcpy(&curl_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	curl_object_handlers.offset = XtOffsetOf(crx_curl, std);
	...
	curl_object_handlers.compare = crex_objects_not_comparable;
...
static crex_object *curl_create_object(crex_class_entry *class_type) {
	crx_curl *intern = crex_object_alloc(sizeof(crx_curl), class_type);
...
static crex_function *curl_get_constructor(crex_object *object) {
	crex_throw_error(NULL, "Cannot directly construct CurlHandle, use curl_init() instead");
	return NULL;
}
...
static crex_object *curl_clone_obj(crex_object *object) {
	...
	if (!cp) {
		crex_throw_exception(NULL, "Failed to clone CurlHandle", 0);
		return &clone_ch->std;
	}

	clone_ch->cp = cp;
	_crx_setup_easy_copy_handlers(clone_ch, ch);

	postfields = &clone_ch->postfields;
	if (C_TYPE_P(postfields) != IS_UNDEF) {
...
static HashTable *curl_get_gc(crex_object *object, zval **table, int *n)
{
	crx_curl *curl = curl_from_obj(object);

	crex_get_gc_buffer *gc_buffer = crex_get_gc_buffer_create();

	crex_get_gc_buffer_add_zval(gc_buffer, &curl->postfields);
	if (curl->handlers.read) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.read->func_name);
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.read->stream);
	}

	if (curl->handlers.write) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.write->func_name);
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.write->stream);
	}

	if (curl->handlers.write_header) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.write_header->func_name);
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.write_header->stream);
	}

	if (curl->handlers.progress) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.progress->func_name);
	}

#if LIBCURL_VERSION_NUM >= 0x072000
	if (curl->handlers.xferinfo) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.xferinfo->func_name);
	}
#endif

	if (curl->handlers.fnmatch) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.fnmatch->func_name);
	}

#if LIBCURL_VERSION_NUM >= 0x075400
	if (curl->handlers.sshhostkey) {
		crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.sshhostkey->func_name);
	}
#endif

	crex_get_gc_buffer_add_zval(gc_buffer, &curl->handlers.std_err);
	crex_get_gc_buffer_add_zval(gc_buffer, &curl->private_data);
...
crex_result curl_cast_object(crex_object *obj, zval *result, int type)
{
	...
	return crex_std_cast_object_tostring(obj, result, type);
}
...
CRX_MSHUTDOWN_FUNCTION(curl)
{
	...
	if (crx_curl_openssl_tsl) {
		...
		for (i = 0; i < c; ++i) {
			tsrm_mutex_free(crx_curl_openssl_tsl[i]);
		}

		free(crx_curl_openssl_tsl);
		crx_curl_openssl_tsl = NULL;
	}
#endif
...
/* {{{ curl_write_nothing
 * Used as a work around. See _crx_curl_close_ex
 */
...
{
	crx_curl *ch = (crx_curl *) ctx;
	crx_curl_write *t = ch->handlers.write;
	...
	switch (t->method) {
		case CRX_CURL_STDOUT:
			CRXWRITE(data, length);
			break;
		case CRX_CURL_FILE:
			return fwrite(data, size, nmemb, t->fp);
		case CRX_CURL_RETURN:
			if (length > 0) {
		...
		case CRX_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				crx_error_docref(NULL, E_WARNING, "Could not call the CURLOPT_WRITEFUNCTION");
				length = -1;
...
{
	crx_curl *ch = (crx_curl *) ctx;
	crx_curl_callback *t = ch->handlers.fnmatch;
	...
	ch->in_callback = 0;
	if (error == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "Cannot call the CURLOPT_FNMATCH_FUNCTION");
	} else if (!C_ISUNDEF(retval)) {
		_crx_curl_verify_handlers(ch, /* reporterror */ true);
...
{
	crx_curl *ch = (crx_curl *)clientp;
	crx_curl_callback *t = ch->handlers.progress;
	size_t	rval = 0;
	...
	GC_ADDREF(&ch->std);
	ZVAL_OBJ(&argv[0], &ch->std);
	ZVAL_LONG(&argv[1], (crex_long)dltotal);
	ZVAL_LONG(&argv[2], (crex_long)dlnow);
	ZVAL_LONG(&argv[3], (crex_long)ultotal);
	ZVAL_LONG(&argv[4], (crex_long)ulnow);
	...
	ch->in_callback = 0;
	if (error == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "Cannot call the CURLOPT_PROGRESSFUNCTION");
	} else if (!C_ISUNDEF(retval)) {
		_crx_curl_verify_handlers(ch, /* reporterror */ true);
...
{
	crx_curl *ch = (crx_curl *)clientp;
	crx_curl_callback *t = ch->handlers.xferinfo;
	size_t rval = 0;
	...
	ch->in_callback = 0;
	if (error == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "Cannot call the CURLOPT_XFERINFOFUNCTION");
	} else if (!C_ISUNDEF(retval)) {
		_crx_curl_verify_handlers(ch, /* reporterror */ true);
		if (0 != zval_get_long(&retval)) {
			rval = 1;
...
{
	crx_curl *ch = (crx_curl *)clientp;
	crx_curl_callback *t = ch->handlers.sshhostkey;
	...
	ch->in_callback = 0;
	if (error == FAILURE) {
		crx_error_docref(NULL, E_WARNING, "Cannot call the CURLOPT_SSH_HOSTKEYFUNCTION");
	} else if (!C_ISUNDEF(retval)) {
		_crx_curl_verify_handlers(ch, /* reporterror */ true);
		if (C_TYPE(retval) == IS_LONG) {
			crex_long retval_long = C_LVAL(retval);
			if (retval_long == CURLKHMATCH_OK || retval_long == CURLKHMATCH_MISMATCH) {
				rval = retval_long;
			} else {
				crex_throw_error(NULL, "The CURLOPT_SSH_HOSTKEYFUNCTION callback must return either CURLKHMATCH_OK or CURLKHMATCH_MISMATCH");
			}
		} else {
			crex_throw_error(NULL, "The CURLOPT_SSH_HOSTKEYFUNCTION callback must return either CURLKHMATCH_OK or CURLKHMATCH_MISMATCH");
		}
...
{
	crx_curl *ch = (crx_curl *)ctx;
	crx_curl_read *t = ch->handlers.read;
	int length = 0;

	switch (t->method) {
		case CRX_CURL_DIRECT:
			if (t->fp) {
		...
		case CRX_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				crx_error_docref(NULL, E_WARNING, "Cannot call the CURLOPT_READFUNCTION");
				length = CURL_READFUNC_ABORT;
			} else if (!C_ISUNDEF(retval)) {
				_crx_curl_verify_handlers(ch, /* reporterror */ true);
				if (C_TYPE(retval) == IS_STRING) {
					length = MIN((int) (size * nmemb), C_STRLEN(retval));
					memcpy(data, C_STRVAL(retval), length);
				} else if (C_TYPE(retval) == IS_LONG) {
					length = C_LVAL_P(&retval);
				}
...
{
	crx_curl *ch = (crx_curl *) ctx;
	crx_curl_write *t = ch->handlers.write_header;
	size_t length = size * nmemb;
	...
	switch (t->method) {
		case CRX_CURL_STDOUT:
			 ...
			 */
			if (ch->handlers.write->method == CRX_CURL_RETURN && length > 0) {
		...
		case CRX_CURL_USER: {
			...
			ch->in_callback = 0;
			if (error == FAILURE) {
				crx_error_docref(NULL, E_WARNING, "Could not call the CURLOPT_HEADERFUNCTION");
				length = -1;
			} else if (!C_ISUNDEF(retval)) {
				_crx_curl_verify_handlers(ch, /* reporterror */ true);
				length = zval_get_long(&retval);
...
static int curl_debug(CURL *cp, curl_infotype type, char *buf, size_t buf_len, void *ctx) /* {{{ */
{
	...
	if (type == CURLINFO_HEADER_OUT) {
		if (ch->header.str) {
			crex_string_release_ex(ch->header.str, 0);
		}
		ch->header.str = crex_string_init(buf, buf_len, 0);
	}
...
static void curl_free_cb_arg(void **cb_arg_p)
{
	...
	CREX_ASSERT(cb_arg->stream == NULL);
	crex_string_release(cb_arg->filename);
	efree(cb_arg);
}
/* }}} */
...
{
	crex_string_release((crex_string *) *buffer);
}
/* }}} */
#endif
...
{
	curl_slist_free_all(((struct curl_slist *)C_PTR_P(el)));
}
/* }}} */

/* {{{ Return cURL version information. */
CRX_FUNCTION(curl_version)
{
	curl_version_info_data *d;
...
{
	ch->to_free = ecalloc(1, sizeof(struct _crx_curl_free));
	ch->handlers.write = ecalloc(1, sizeof(crx_curl_write));
	ch->handlers.write_header = ecalloc(1, sizeof(crx_curl_write));
	ch->handlers.read = ecalloc(1, sizeof(crx_curl_read));
	...
	*ch->clone = 1;
	...
	memset(&ch->err, 0, sizeof(struct _crx_curl_error));

	crex_llist_init(&ch->to_free->post,  sizeof(struct HttpPost *), (llist_dtor_func_t)curl_free_post,   0);
	crex_llist_init(&ch->to_free->stream, sizeof(struct mime_data_cb_arg *), (llist_dtor_func_t)curl_free_cb_arg, 0);

#if LIBCURL_VERSION_NUM < 0x073800 /* 7.56.0 */
	crex_llist_init(&ch->to_free->buffers, sizeof(crex_string *), (llist_dtor_func_t)curl_free_buffers, 0);
#endif

	ch->to_free->slist = emalloc(sizeof(HashTable));
	crex_hash_init(ch->to_free->slist, 4, NULL, curl_free_slist, 0);
			...
			for (slist = ci->certinfo[i]; slist; slist = slist->next) {
				...
				if(tmp) {
					*tmp = '\0';
					...
					crx_error_docref(NULL, E_WARNING, "Could not extract hash key from certificate info");
				}
			}
...
/* {{{ _crx_curl_set_default_options()
   Set default options for a handle */
static void _crx_curl_set_default_options(crx_curl *ch)
{
	char *cainfo;
...
CRX_FUNCTION(curl_init)
{
	crx_curl *ch;
	CURL 	 *cp;
	crex_string *url = NULL;

	CREX_PARSE_PARAMETERS_START(0,1)
		C_PARAM_OPTIONAL
		C_PARAM_STR_OR_NULL(url)
	CREX_PARSE_PARAMETERS_END();

	cp = curl_easy_init();
	if (!cp) {
		crx_error_docref(NULL, E_WARNING, "Could not initialize a new cURL handle");
		RETURN_FALSE;
	}
	...
	ch->handlers.write->method = CRX_CURL_STDOUT;
	ch->handlers.read->method  = CRX_CURL_DIRECT;
	ch->handlers.write_header->method = CRX_CURL_IGNORE;

	_crx_curl_set_default_options(ch);

	if (url) {
		if (crx_curl_option_url(ch, url) == FAILURE) {
...
static void _crx_copy_callback(crx_curl *ch, crx_curl_callback **new_callback, crx_curl_callback *source_callback, CURLoption option)
{
	if (source_callback) {
		*new_callback = ecalloc(1, sizeof(crx_curl_callback));
		if (!C_ISUNDEF(source_callback->func_name)) {
...
{
	if (!C_ISUNDEF(source->handlers.write->stream)) {
		C_ADDREF(source->handlers.write->stream);
	}
	ch->handlers.write->stream = source->handlers.write->stream;
	ch->handlers.write->method = source->handlers.write->method;
	if (!C_ISUNDEF(source->handlers.read->stream)) {
		C_ADDREF(source->handlers.read->stream);
	}
	...
	if (!C_ISUNDEF(source->handlers.write_header->stream)) {
		C_ADDREF(source->handlers.write_header->stream);
	}
	...
	if (!C_ISUNDEF(source->handlers.write->func_name)) {
		ZVAL_COPY(&ch->handlers.write->func_name, &source->handlers.write->func_name);
	}
	if (!C_ISUNDEF(source->handlers.read->func_name)) {
		ZVAL_COPY(&ch->handlers.read->func_name, &source->handlers.read->func_name);
	}
	if (!C_ISUNDEF(source->handlers.write_header->func_name)) {
	...
	_crx_copy_callback(ch, &ch->handlers.progress, source->handlers.progress, CURLOPT_PROGRESSDATA);
#if LIBCURL_VERSION_NUM >= 0x072000
	_crx_copy_callback(ch, &ch->handlers.xferinfo, source->handlers.xferinfo, CURLOPT_XFERINFODATA);
#endif
	_crx_copy_callback(ch, &ch->handlers.fnmatch, source->handlers.fnmatch, CURLOPT_FNMATCH_DATA);
#if LIBCURL_VERSION_NUM >= 0x075400
	_crx_copy_callback(ch, &ch->handlers.sshhostkey, source->handlers.sshhostkey, CURLOPT_SSH_HOSTKEYDATA);
#endif
...
static size_t read_cb(char *buffer, size_t size, size_t nitems, void *arg) /* {{{ */
{
	...
	if (cb_arg->stream == NULL) {
		if (!(cb_arg->stream = crx_stream_open_wrapper(ZSTR_VAL(cb_arg->filename), "rb", IGNORE_PATH, NULL))) {
			return CURL_READFUNC_ABORT;
		}
	}
	numread = crx_stream_read(cb_arg->stream, buffer, nitems * size);
	if (numread < 0) {
		crx_stream_close(cb_arg->stream);
		cb_arg->stream = NULL;
		return CURL_READFUNC_ABORT;
	}
	return numread;
...
static int seek_cb(void *arg, curl_off_t offset, int origin) /* {{{ */
{
	...
	res = crx_stream_seek(cb_arg->stream, offset, origin);
...
#if LIBCURL_VERSION_NUM >= 0x073800 /* 7.56.0 */
static inline CURLcode add_simple_field(curl_mime *mime, crex_string *string_key, zval *current)
#else
static inline CURLcode add_simple_field(struct HttpPost **first, struct HttpPost **last, crex_string *string_key, zval *current)
#endif
{
...
#endif
	crex_string *postval, *tmp_postval;
	...
	if (part == NULL) {
		crex_tmp_string_release(tmp_postval);
		crex_string_release_ex(string_key, 0);
...
static inline crex_result build_mime_structure_from_hash(crx_curl *ch, zval *zpostfields) /* {{{ */
{
	HashTable *postfields = C_ARRVAL_P(zpostfields);
	CURLcode error = CURLE_OK;
	zval *current;
...
#if LIBCURL_VERSION_NUM >= 0x073800 /* 7.56.0 */
	if (crex_hash_num_elements(postfields) > 0) {
	...
	CREX_HASH_FOREACH_KEY_VAL(postfields, num_key, string_key, current) {
		crex_string *postval;
		/* Pretend we have a string_key here */
		if (!string_key) {
			string_key = crex_long_to_str(num_key);
		} else {
			crex_string_addref(string_key);
		}

		ZVAL_DEREF(current);
		if (C_TYPE_P(current) == IS_OBJECT &&
				instanceof_function(C_OBJCE_P(current), curl_CURLFile_class)) {
			...
			prop = crex_read_property_ex(curl_CURLFile_class, C_OBJ_P(current), ZSTR_KNOWN(CREX_STR_NAME), /* silent */ false, &rv);
			ZVAL_DEREF(prop);
			if (C_TYPE_P(prop) != IS_STRING) {
				crx_error_docref(NULL, E_WARNING, "Invalid filename for key %s", ZSTR_VAL(string_key));
			} else {
				postval = C_STR_P(prop);

				if (crx_check_open_basedir(ZSTR_VAL(postval))) {
					return FAILURE;
				}

				prop = crex_read_property(curl_CURLFile_class, C_OBJ_P(current), "mime", sizeof("mime")-1, 0, &rv);
				ZVAL_DEREF(prop);
				if (C_TYPE_P(prop) == IS_STRING && C_STRLEN_P(prop) > 0) {
					type = C_STRVAL_P(prop);
				}
				prop = crex_read_property(curl_CURLFile_class, C_OBJ_P(current), "postname", sizeof("postname")-1, 0, &rv);
				ZVAL_DEREF(prop);
				if (C_TYPE_P(prop) == IS_STRING && C_STRLEN_P(prop) > 0) {
					filename = C_STRVAL_P(prop);
				}
				...
				if ((stream = crx_stream_open_wrapper(ZSTR_VAL(postval), "rb", STREAM_MUST_SEEK, NULL))) {
					if (!stream->readfilters.head && !crx_stream_stat(stream, &ssb)) {
				...
				cb_arg->filename = crex_string_copy(postval);
				...
				crex_llist_add_element(&ch->to_free->stream, &cb_arg);
#else
				form_error = curl_formadd(&first, &last,
								CURLFORM_COPYNAME, ZSTR_VAL(string_key),
								CURLFORM_NAMELENGTH, ZSTR_LEN(string_key),
		...
		if (C_TYPE_P(current) == IS_OBJECT && instanceof_function(C_OBJCE_P(current), curl_CURLStringFile_class)) {
			/* new-style file upload from string */
			zval *prop, rv;
			char *type = NULL, *filename = NULL;

			prop = crex_read_property(curl_CURLStringFile_class, C_OBJ_P(current), "postname", sizeof("postname")-1, 0, &rv);
			if (EG(exception)) {
				crex_string_release_ex(string_key, 0);
				return FAILURE;
			}
			ZVAL_DEREF(prop);
			CREX_ASSERT(C_TYPE_P(prop) == IS_STRING);

			filename = C_STRVAL_P(prop);

			prop = crex_read_property(curl_CURLStringFile_class, C_OBJ_P(current), "mime", sizeof("mime")-1, 0, &rv);
			if (EG(exception)) {
				crex_string_release_ex(string_key, 0);
				return FAILURE;
			}
			...
			prop = crex_read_property(curl_CURLStringFile_class, C_OBJ_P(current), "data", sizeof("data")-1, 0, &rv);
			if (EG(exception)) {
				crex_string_release_ex(string_key, 0);
				return FAILURE;
			}
...
#else
			postval = crex_string_copy(postval);
			crex_llist_add_element(&ch->to_free->buffers, &postval);
		...
		if (C_TYPE_P(current) == IS_ARRAY) {
			...
			CREX_HASH_FOREACH_VAL(HASH_OF(current), current_element) {
#if LIBCURL_VERSION_NUM >= 0x073800 /* 7.56.0 */
	...
	if ((*ch->clone) == 1) {
		crex_llist_clean(&ch->to_free->post);
	}
#if LIBCURL_VERSION_NUM >= 0x073800 /* 7.56.0 */
	crex_llist_add_element(&ch->to_free->post, &mime);
	error = curl_easy_setopt(ch->cp, CURLOPT_MIMEPOST, mime);
#else
	crex_llist_add_element(&ch->to_free->post, &first);
...
CRX_FUNCTION(curl_copy_handle)
{
	...
	CREX_PARSE_PARAMETERS_START(1,1)
		C_PARAM_OBJECT_OF_CLASS(zid, curl_ce)
	CREX_PARSE_PARAMETERS_END();
	...
	cp = curl_easy_duphandle(ch->cp);
	if (!cp) {
		crx_error_docref(NULL, E_WARNING, "Cannot duplicate cURL handle");
		RETURN_FALSE;
	}
	...
	_crx_setup_easy_copy_handlers(dupch, ch);

	postfields = &ch->postfields;
	if (C_TYPE_P(postfields) != IS_UNDEF) {
		if (build_mime_structure_from_hash(dupch, postfields) == FAILURE) {
			zval_ptr_dtor(return_value);
			crx_error_docref(NULL, E_WARNING, "Cannot rebuild mime structure");
			RETURN_FALSE;
		}
	}
}
...
static crex_result _crx_curl_setopt(crx_curl *ch, crex_long option, zval *zvalue, bool is_array_config) /* {{{ */
{
	CURLcode error = CURLE_OK;
	crex_long lval;
	...
	switch (option) {
		...
		case CURLOPT_SSL_VERIFYHOST:
			...
			if (lval == 1) {
				crx_error_docref(NULL, E_NOTICE, "CURLOPT_SSL_VERIFYHOST no longer accepts the value 1, value 2 will be used instead");
		...
		case CURLOPT_QUICK_EXIT:
			...
			if ((option == CURLOPT_PROTOCOLS || option == CURLOPT_REDIR_PROTOCOLS) &&
				(PG(open_basedir) && *PG(open_basedir)) && (lval & CURLPROTO_FILE)) {
					crx_error_docref(NULL, E_WARNING, "CURLPROTO_FILE cannot be activated when an open_basedir is set");
					return FAILURE;
			}
# if defined(ZTS)
			if (option == CURLOPT_DNS_USE_GLOBAL_CACHE && lval) {
				crx_error_docref(NULL, E_WARNING, "CURLOPT_DNS_USE_GLOBAL_CACHE cannot be activated when thread safety is enabled");
				return FAILURE;
			}
# endif
		...
		case CURLOPT_SAFE_UPLOAD:
			if (!crex_is_true(zvalue)) {
				crex_value_error("%s(): Disabling safe uploads is no longer supported", get_active_function_name());
				return FAILURE;
			}
			break;
		...
		{
			crex_string *tmp_str;
			crex_string *str = zval_get_tmp_string(zvalue, &tmp_str);
#if LIBCURL_VERSION_NUM >= 0x075500 /* Available since 7.85.0 */
			if ((option == CURLOPT_PROTOCOLS_STR || option == CURLOPT_REDIR_PROTOCOLS_STR) &&
				(PG(open_basedir) && *PG(open_basedir)) && crx_memnistr(ZSTR_VAL(str), "file", sizeof("file") - 1, ZSTR_VAL(str) + ZSTR_LEN(str)) != NULL) {
					crx_error_docref(NULL, E_WARNING, "The FILE protocol cannot be activated when an open_basedir is set");
					return FAILURE;
			}
#endif
			crex_result ret = crx_curl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str));
		...
		case CURLOPT_URL:
		{
			crex_string *tmp_str;
			crex_string *str = zval_get_tmp_string(zvalue, &tmp_str);
			crex_result ret = crx_curl_option_url(ch, str);
			crex_tmp_string_release(tmp_str);
			return ret;
		}
		...
		case CURLOPT_WRITEHEADER: {
			FILE *fp = NULL;
			crx_stream *what = NULL;

			if (C_TYPE_P(zvalue) != IS_NULL) {
				what = (crx_stream *)crex_fetch_resource2_ex(zvalue, "File-Handle", crx_file_le_stream(), crx_file_le_pstream());
				if (!what) {
					return FAILURE;
				}

				if (FAILURE == crx_stream_cast(what, CRX_STREAM_AS_STDIO, (void *) &fp, REPORT_ERRORS)) {
					return FAILURE;
				}
			...
			switch (option) {
				case CURLOPT_FILE:
					if (!what) {
						if (!C_ISUNDEF(ch->handlers.write->stream)) {
							zval_ptr_dtor(&ch->handlers.write->stream);
							ZVAL_UNDEF(&ch->handlers.write->stream);
						}
						...
						ch->handlers.write->method = CRX_CURL_FILE;
						ZVAL_COPY(&ch->handlers.write->stream, zvalue);
					} else {
						crex_value_error("%s(): The provided file handle must be writable", get_active_function_name());
						return FAILURE;
					}
					break;
				case CURLOPT_WRITEHEADER:
					if (!what) {
						if (!C_ISUNDEF(ch->handlers.write_header->stream)) {
							zval_ptr_dtor(&ch->handlers.write_header->stream);
							ZVAL_UNDEF(&ch->handlers.write_header->stream);
						}
						...
						ch->handlers.write_header->method = CRX_CURL_FILE;
				...
				case CURLOPT_INFILE:
					if (!what) {
						if (!C_ISUNDEF(ch->handlers.read->stream)) {
							zval_ptr_dtor(&ch->handlers.read->stream);
							ZVAL_UNDEF(&ch->handlers.read->stream);
						}
						...
						ch->handlers.read->res = C_RES_P(zvalue);
...
#endif
		{
			...
			if (C_TYPE_P(zvalue) != IS_ARRAY) {
				...
				crex_type_error("%s(): The %s option must have an array value", get_active_function_name(), name);
				return FAILURE;
			}

			ph = C_ARRVAL_P(zvalue);
			CREX_HASH_FOREACH_VAL(ph, current) {
				ZVAL_DEREF(current);
				val = zval_get_tmp_string(current, &tmp_val);
				slist = curl_slist_append(slist, ZSTR_VAL(val));
				crex_tmp_string_release(tmp_val);
				if (!slist) {
					crx_error_docref(NULL, E_WARNING, "Could not build curl_slist");
					return FAILURE;
				}
			} CREX_HASH_FOREACH_END();
			...
			if (slist) {
				if ((*ch->clone) == 1) {
					crex_hash_index_update_ptr(ch->to_free->slist, option, slist);
				} else {
					crex_hash_next_index_insert_ptr(ch->to_free->slist, slist);
				}
			}
		...
		case CURLOPT_FOLLOWLOCATION:
			lval = crex_is_true(zvalue);
			error = curl_easy_setopt(ch->cp, option, lval);
			break;

		case CURLOPT_HEADERFUNCTION:
			if (!C_ISUNDEF(ch->handlers.write_header->func_name)) {
				zval_ptr_dtor(&ch->handlers.write_header->func_name);
				ch->handlers.write_header->fci_cache = empty_fcall_info_cache;
			}
			ZVAL_COPY(&ch->handlers.write_header->func_name, zvalue);
			ch->handlers.write_header->method = CRX_CURL_USER;
			break;

		case CURLOPT_POSTFIELDS:
			if (C_TYPE_P(zvalue) == IS_ARRAY) {
				if (crex_hash_num_elements(HASH_OF(zvalue)) == 0) {
		...
		case CURLOPT_PROGRESSFUNCTION:
			...
			if (ch->handlers.progress == NULL) {
				ch->handlers.progress = ecalloc(1, sizeof(crx_curl_callback));
			} else if (!C_ISUNDEF(ch->handlers.progress->func_name)) {
		...
		case CURLOPT_SSH_HOSTKEYFUNCTION:
			...
			if (ch->handlers.sshhostkey == NULL) {
				ch->handlers.sshhostkey = ecalloc(1, sizeof(crx_curl_callback));
			} else if (!C_ISUNDEF(ch->handlers.sshhostkey->func_name)) {
		...
		case CURLOPT_READFUNCTION:
			if (!C_ISUNDEF(ch->handlers.read->func_name)) {
				zval_ptr_dtor(&ch->handlers.read->func_name);
				ch->handlers.read->fci_cache = empty_fcall_info_cache;
			}
			ZVAL_COPY(&ch->handlers.read->func_name, zvalue);
			ch->handlers.read->method = CRX_CURL_USER;
			break;

		case CURLOPT_RETURNTRANSFER:
			if (crex_is_true(zvalue)) {
				ch->handlers.write->method = CRX_CURL_RETURN;
			} else {
				ch->handlers.write->method = CRX_CURL_STDOUT;
			}
			break;
		...
		case CURLOPT_XFERINFOFUNCTION:
			...
			if (ch->handlers.xferinfo == NULL) {
				ch->handlers.xferinfo = ecalloc(1, sizeof(crx_curl_callback));
			} else if (!C_ISUNDEF(ch->handlers.xferinfo->func_name)) {
		...
		case CURLOPT_SSH_KNOWNHOSTS:
		{
			...
			if (ZSTR_LEN(str) && crx_check_open_basedir(ZSTR_VAL(str))) {
				crex_tmp_string_release(tmp_str);
				return FAILURE;
			}

			ret = crx_curl_option_str(ch, option, ZSTR_VAL(str), ZSTR_LEN(str));
			crex_tmp_string_release(tmp_str);
			return ret;
		}
		...
		case CURLOPT_SHARE:
			{
				if (C_TYPE_P(zvalue) == IS_OBJECT && C_OBJCE_P(zvalue) == curl_share_ce) {
					crx_curlsh *sh = C_CURL_SHARE_P(zvalue);
		...
		case CURLOPT_FNMATCH_FUNCTION:
			...
			if (ch->handlers.fnmatch == NULL) {
				ch->handlers.fnmatch = ecalloc(1, sizeof(crx_curl_callback));
			} else if (!C_ISUNDEF(ch->handlers.fnmatch->func_name)) {
		...
		default:
			if (is_array_config) {
				crex_argument_value_error(2, "must contain only valid cURL options");
			} else {
				crex_argument_value_error(2, "is not a valid cURL option");
			}
...
CRX_FUNCTION(curl_setopt)
{
	...
	if (_crx_curl_setopt(ch, options, zvalue, 0) == SUCCESS) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
CRX_FUNCTION(curl_setopt_array)
{
	zval		*zid, *arr, *entry;
	crx_curl	*ch;
	crex_ulong	option;
	...
	CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(arr), option, string_key, entry) {
		if (string_key) {
			crex_argument_value_error(2, "contains an invalid cURL option");
			RETURN_THROWS();
		}

		ZVAL_DEREF(entry);
		if (_crx_curl_setopt(ch, (crex_long) option, entry, 1) == FAILURE) {
			RETURN_FALSE;
		}
	} CREX_HASH_FOREACH_END();
...
void _crx_curl_cleanup_handle(crx_curl *ch)
{
...
CRX_FUNCTION(curl_exec)
{
	...
	if (ch->handlers.write->method == CRX_CURL_RETURN && ch->handlers.write->buf.s) {
		smart_str_0(&ch->handlers.write->buf);
		RETURN_STR_COPY(ch->handlers.write->buf.s);
	}

	/* flush the file handle, so any remaining data is synched to disk */
	if (ch->handlers.write->method == CRX_CURL_FILE && ch->handlers.write->fp) {
		fflush(ch->handlers.write->fp);
	}
	if (ch->handlers.write_header->method == CRX_CURL_FILE && ch->handlers.write_header->fp) {
...
CRX_FUNCTION(curl_getinfo)
{
	...
	CREX_PARSE_PARAMETERS_START(1, 2)
		C_PARAM_OBJECT_OF_CLASS(zid, curl_ce)
		C_PARAM_OPTIONAL
		C_PARAM_LONG_OR_NULL(option, option_is_null)
	CREX_PARSE_PARAMETERS_END();
	...
	if (option_is_null) {
		...
		/* libcurl expects long datatype. So far no cases are known where
		   it would be an issue. Using crex_long would truncate a 64-bit
...
CRX_FUNCTION(curl_close)
{
	...
	if (ch->in_callback) {
		crex_throw_error(NULL, "%s(): Attempt to close cURL handle from a callback", get_active_function_name());
		RETURN_THROWS();
	}
}
/* }}} */

static void _crx_curl_free_callback(crx_curl_callback* callback)
{
	if (callback) {
...
static void curl_free_obj(crex_object *object)
{
	crx_curl *ch = curl_from_obj(object);
	...
	if (--(*ch->clone) == 0) {
		crex_llist_clean(&ch->to_free->post);
		crex_llist_clean(&ch->to_free->stream);

#if LIBCURL_VERSION_NUM < 0x073800 /* 7.56.0 */
		crex_llist_clean(&ch->to_free->buffers);
#endif

		crex_hash_destroy(ch->to_free->slist);
		efree(ch->to_free->slist);
		efree(ch->to_free);
		efree(ch->clone);
	}
	...
	_crx_curl_free_callback(ch->handlers.progress);
#if LIBCURL_VERSION_NUM >= 0x072000
	_crx_curl_free_callback(ch->handlers.xferinfo);
#endif
	_crx_curl_free_callback(ch->handlers.fnmatch);
#if LIBCURL_VERSION_NUM >= 0x075400
	_crx_curl_free_callback(ch->handlers.sshhostkey);
#endif
...
/* {{{ _crx_curl_reset_handlers()
   Reset all handlers of a given crx_curl */
static void _crx_curl_reset_handlers(crx_curl *ch)
{
	if (!C_ISUNDEF(ch->handlers.write->stream)) {
		zval_ptr_dtor(&ch->handlers.write->stream);
		ZVAL_UNDEF(&ch->handlers.write->stream);
...
CRX_FUNCTION(curl_reset)
{
	...
	if (ch->in_callback) {
		crex_throw_error(NULL, "%s(): Attempt to reset cURL handle from a callback", get_active_function_name());
		RETURN_THROWS();
	}

	curl_easy_reset(ch->cp);
	_crx_curl_reset_handlers(ch);
	_crx_curl_set_default_options(ch);
}
/* }}} */

/* {{{ URL encodes the given string */
CRX_FUNCTION(curl_escape)
{
	crex_string *str;
	char        *res;
	zval        *zid;
	...
	if (CREX_SIZE_T_INT_OVFL(ZSTR_LEN(str))) {
		RETURN_FALSE;
	}
