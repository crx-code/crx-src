...
if test "$CRX_CURL" != "no"; then
  PKG_CHECK_MODULES([CURL], [libcurl >= 7.29.0])
  PKG_CHECK_VAR([CURL_FEATURES], [libcurl], [supported_features])

  CRX_EVAL_LIBLINE($CURL_LIBS, CURL_SHARED_LIBADD)
  CRX_EVAL_INCLINE($CURL_CFLAGS)
...
int main(int argc, char *argv[])
{
  ...
  return 1;
      ...
      PKG_CHECK_MODULES([OPENSSL], [openssl], [
        CRX_EVAL_LIBLINE($OPENSSL_LIBS, CURL_SHARED_LIBADD)
        CRX_EVAL_INCLINE($OPENSSL_CFLAGS)
  ...
  CRX_CHECK_LIBRARY(curl,curl_easy_perform,
  [
  ...
  CRX_NEW_EXTENSION(curl, interface.c multi.c share.c curl_file.c, $ext_shared)
  CRX_INSTALL_HEADERS([ext/curl], [crx_curl.h])
  CRX_SUBST(CURL_SHARED_LIBADD)
fi
