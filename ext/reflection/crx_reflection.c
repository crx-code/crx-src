...
#include "crx_reflection_arginfo.h"

/* Key used to avoid leaking addresses in ReflectionProperty::getId() */
#define REFLECTION_KEY_LEN 16
CREX_BEGIN_MODULE_GLOBALS(reflection)
	bool key_initialized;
	unsigned char key[REFLECTION_KEY_LEN];
CREX_END_MODULE_GLOBALS(reflection)
CREX_DECLARE_MODULE_GLOBALS(reflection)

#define REFLECTION_G(v) CREX_MODULE_GLOBALS_ACCESSOR(reflection, v)

static crex_always_inline zval *reflection_prop_name(zval *object) {
	/* $name is always in the first property slot. */
	CREX_ASSERT(C_OBJCE_P(object)->default_properties_count >= 1);
	return &C_OBJ_P(object)->properties_table[0];
}

static crex_always_inline zval *reflection_prop_class(zval *object) {
	/* $class is always in the second property slot. */
	CREX_ASSERT(C_OBJCE_P(object)->default_properties_count >= 2);
	return &C_OBJ_P(object)->properties_table[1];
}

/* Class entry pointers */
CRXAPI crex_class_entry *reflector_ptr;
CRXAPI crex_class_entry *reflection_exception_ptr;
CRXAPI crex_class_entry *reflection_ptr;
CRXAPI crex_class_entry *reflection_function_abstract_ptr;
CRXAPI crex_class_entry *reflection_function_ptr;
CRXAPI crex_class_entry *reflection_generator_ptr;
CRXAPI crex_class_entry *reflection_parameter_ptr;
CRXAPI crex_class_entry *reflection_type_ptr;
CRXAPI crex_class_entry *reflection_named_type_ptr;
CRXAPI crex_class_entry *reflection_intersection_type_ptr;
CRXAPI crex_class_entry *reflection_union_type_ptr;
CRXAPI crex_class_entry *reflection_class_ptr;
CRXAPI crex_class_entry *reflection_object_ptr;
CRXAPI crex_class_entry *reflection_method_ptr;
CRXAPI crex_class_entry *reflection_property_ptr;
CRXAPI crex_class_entry *reflection_class_constant_ptr;
CRXAPI crex_class_entry *reflection_extension_ptr;
CRXAPI crex_class_entry *reflection_crex_extension_ptr;
CRXAPI crex_class_entry *reflection_reference_ptr;
CRXAPI crex_class_entry *reflection_attribute_ptr;
CRXAPI crex_class_entry *reflection_enum_ptr;
CRXAPI crex_class_entry *reflection_enum_unit_case_ptr;
CRXAPI crex_class_entry *reflection_enum_backed_case_ptr;
CRXAPI crex_class_entry *reflection_fiber_ptr;

/* Exception throwing macro */
#define _DO_THROW(msg) \
	crex_throw_exception(reflection_exception_ptr, msg, 0);

#define GET_REFLECTION_OBJECT() do { \
	intern = C_REFLECTION_P(CREX_THIS); \
	if (intern->ptr == NULL) { \
		if (EG(exception) && EG(exception)->ce == reflection_exception_ptr) { \
			RETURN_THROWS(); \
		} \
		crex_throw_error(NULL, "Internal error: Failed to retrieve the reflection object"); \
		RETURN_THROWS(); \
	} \
} while (0)
...
typedef struct _parameter_reference {
	uint32_t offset;
	bool required;
	struct _crex_arg_info *arg_info;
	crex_function *fptr;
} parameter_reference;
...
static inline reflection_object *reflection_object_from_obj(crex_object *obj) {
	return (reflection_object*)((char*)(obj) - XtOffsetOf(reflection_object, zo));
}

#define C_REFLECTION_P(zv)  reflection_object_from_obj(C_OBJ_P((zv)))
/* }}} */

static crex_object_handlers reflection_object_handlers;

static crex_always_inline uint32_t prop_get_flags(property_reference *ref) {
	return ref->prop ? ref->prop->flags : CREX_ACC_PUBLIC;
}

static inline bool is_closure_invoke(crex_class_entry *ce, crex_string *lcname) {
	return ce == crex_ce_closure
		&& crex_string_equals_literal(lcname, CREX_INVOKE_FUNC_NAME);
}

static crex_function *_copy_function(crex_function *fptr) /* {{{ */
{
	if (fptr
		&& (fptr->internal_function.fn_flags & CREX_ACC_CALL_VIA_TRAMPOLINE))
	{
		crex_function *copy_fptr;
		copy_fptr = emalloc(sizeof(crex_function));
		memcpy(copy_fptr, fptr, sizeof(crex_function));
		copy_fptr->internal_function.function_name = crex_string_copy(fptr->internal_function.function_name);
		return copy_fptr;
	} else {
		/* no copy needed */
		return fptr;
...
static void _free_function(crex_function *fptr) /* {{{ */
{
	if (fptr
		&& (fptr->internal_function.fn_flags & CREX_ACC_CALL_VIA_TRAMPOLINE))
	{
		crex_string_release_ex(fptr->internal_function.function_name, 0);
		crex_free_trampoline(fptr);
	}
}
/* }}} */

static void reflection_free_objects_storage(crex_object *object) /* {{{ */
{
	...
	if (intern->ptr) {
		...
		case REF_TYPE_TYPE:
		{
			...
			if (CREX_TYPE_HAS_NAME(type_ref->type)) {
				crex_string_release(CREX_TYPE_NAME(type_ref->type));
			}
			efree(type_ref);
			break;
		}
		...
		case REF_TYPE_PROPERTY:
			...
			crex_string_release_ex(prop_reference->unmangled_name, 0);
			efree(intern->ptr);
			break;
		case REF_TYPE_ATTRIBUTE: {
			attribute_reference *attr_ref = intern->ptr;
			if (attr_ref->filename) {
				crex_string_release(attr_ref->filename);
			}
			efree(intern->ptr);
			break;
		}
...
static HashTable *reflection_get_gc(crex_object *obj, zval **gc_data, int *gc_data_count) /* {{{ */
{
...
static crex_object *reflection_objects_new(crex_class_entry *class_type) /* {{{ */
{
	reflection_object *intern = crex_object_alloc(sizeof(reflection_object), class_type);
...
static zval *reflection_instantiate(crex_class_entry *pce, zval *object) /* {{{ */
{
...
static void _function_string(smart_str *str, crex_function *fptr, crex_class_entry *scope, char* indent);
static void _property_string(smart_str *str, crex_property_info *prop, const char *prop_name, char* indent);
static void _class_const_string(smart_str *str, crex_string *name, crex_class_constant *c, char* indent);
static void _class_string(smart_str *str, crex_class_entry *ce, zval *obj, char *indent);
static void _extension_string(smart_str *str, crex_module_entry *module, char *indent);
static void _crex_extension_string(smart_str *str, crex_extension *extension, char *indent);

/* {{{ _class_string */
static void _class_string(smart_str *str, crex_class_entry *ce, zval *obj, char *indent)
{
	int count, count_static_props = 0, count_static_funcs = 0, count_shadow_props = 0;
	crex_string *sub_indent = strpprintf(0, "%s    ", indent);

	/* TBD: Repair indenting of doc comment (or is this to be done in the parser?) */
	if (ce->type == CREX_USER_CLASS && ce->info.user.doc_comment) {
		smart_str_append_printf(str, "%s%s", indent, ZSTR_VAL(ce->info.user.doc_comment));
		smart_str_appendc(str, '\n');
	}

	if (obj && C_TYPE_P(obj) == IS_OBJECT) {
		smart_str_append_printf(str, "%sObject of class [ ", indent);
	} else {
		char *kind = "Class";
		if (ce->ce_flags & CREX_ACC_INTERFACE) {
			kind = "Interface";
		} else if (ce->ce_flags & CREX_ACC_TRAIT) {
			kind = "Trait";
		}
		smart_str_append_printf(str, "%s%s [ ", indent, kind);
	}
	smart_str_append_printf(str, (ce->type == CREX_USER_CLASS) ? "<user" : "<internal");
	if (ce->type == CREX_INTERNAL_CLASS && ce->info.internal.module) {
	...
	if (ce->ce_flags & CREX_ACC_INTERFACE) {
		...
		if (ce->ce_flags & (CREX_ACC_IMPLICIT_ABSTRACT_CLASS|CREX_ACC_EXPLICIT_ABSTRACT_CLASS)) {
			smart_str_append_printf(str, "abstract ");
		}
		if (ce->ce_flags & CREX_ACC_FINAL) {
	...
	/* Constants */
	smart_str_append_printf(str, "\n");
	count = crex_hash_num_elements(&ce->constants_table);
	smart_str_append_printf(str, "%s  - Constants [%d] {\n", indent, count);
	if (count > 0) {
		crex_string *key;
		crex_class_constant *c;

		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(CE_CONSTANTS_TABLE(ce), key, c) {
			_class_const_string(str, key, c, ZSTR_VAL(sub_indent));
			if (UNEXPECTED(EG(exception))) {
				crex_string_release(sub_indent);
				return;
			}
	...
	count = crex_hash_num_elements(&ce->properties_info);
	if (count > 0) {
		crex_property_info *prop;

		CREX_HASH_MAP_FOREACH_PTR(&ce->properties_info, prop) {
			if ((prop->flags & CREX_ACC_PRIVATE) && prop->ce != ce) {
				count_shadow_props++;
			} else if (prop->flags & CREX_ACC_STATIC) {
				count_static_props++;
			}
		} CREX_HASH_FOREACH_END();
	}
	...
	if (count_static_props > 0) {
		...
		CREX_HASH_MAP_FOREACH_PTR(&ce->properties_info, prop) {
			if ((prop->flags & CREX_ACC_STATIC) && (!(prop->flags & CREX_ACC_PRIVATE) || prop->ce == ce)) {
	...
	count = crex_hash_num_elements(&ce->function_table);
	if (count > 0) {
		crex_function *mptr;

		CREX_HASH_MAP_FOREACH_PTR(&ce->function_table, mptr) {
			if ((mptr->common.fn_flags & CREX_ACC_STATIC)
				&& ((mptr->common.fn_flags & CREX_ACC_PRIVATE) == 0 || mptr->common.scope == ce))
			{
				count_static_funcs++;
			}
		} CREX_HASH_FOREACH_END();
	...
	count = crex_hash_num_elements(&ce->properties_info) - count_static_props - count_shadow_props;
	smart_str_append_printf(str, "\n%s  - Properties [%d] {\n", indent, count);
	if (count > 0) {
		crex_property_info *prop;

		CREX_HASH_MAP_FOREACH_PTR(&ce->properties_info, prop) {
			if (!(prop->flags & CREX_ACC_STATIC)
			 && (!(prop->flags & CREX_ACC_PRIVATE) || prop->ce == ce)) {
				_property_string(str, prop, NULL, ZSTR_VAL(sub_indent));
			}
		} CREX_HASH_FOREACH_END();
	}
	...
	if (obj && C_TYPE_P(obj) == IS_OBJECT) {
		HashTable    *properties = C_OBJ_HT_P(obj)->get_properties(C_OBJ_P(obj));
		crex_string  *prop_name;
		smart_str prop_str = {0};

		count = 0;
		if (properties && crex_hash_num_elements(properties)) {
			CREX_HASH_MAP_FOREACH_STR_KEY(properties, prop_name) {
				if (prop_name && ZSTR_LEN(prop_name) && ZSTR_VAL(prop_name)[0]) { /* skip all private and protected properties */
					if (!crex_hash_exists(&ce->properties_info, prop_name)) {
						count++;
	...
	/* Non static methods */
	count = crex_hash_num_elements(&ce->function_table) - count_static_funcs;
	if (count > 0) {
		crex_function *mptr;
		...
		count = 0;
		CREX_HASH_MAP_FOREACH_PTR(&ce->function_table, mptr) {
			if ((mptr->common.fn_flags & CREX_ACC_STATIC) == 0
				&& ((mptr->common.fn_flags & CREX_ACC_PRIVATE) == 0 || mptr->common.scope == ce))
			{
				crex_function *closure;
				/* see if this is a closure */
				if (obj && is_closure_invoke(ce, mptr->common.function_name)
					&& (closure = crex_get_closure_invoke_method(C_OBJ_P(obj))) != NULL)
				{
					mptr = closure;
				} else {
					closure = NULL;
	...
	smart_str_append_printf(str, "%s}\n", indent);
	crex_string_release_ex(sub_indent, 0);
}
/* }}} */
...
static void _const_string(smart_str *str, char *name, zval *value, char *indent)
{
	const char *type = crex_zval_type_name(value);
	...
	if (C_TYPE_P(value) == IS_ARRAY) {
		...
		smart_str_append_printf(str, "%s    Constant [ %s %s ] { %s }\n",
						indent, type, name, C_STRVAL_P(value));
	} else {
		crex_string *tmp_value_str;
		crex_string *value_str = zval_get_tmp_string(value, &tmp_value_str);
		smart_str_append_printf(str, "%s    Constant [ %s %s ] { %s }\n",
						indent, type, name, ZSTR_VAL(value_str));
		crex_tmp_string_release(tmp_value_str);
	}
}
/* }}} */
...
static void _class_const_string(smart_str *str, crex_string *name, crex_class_constant *c, char *indent)
{
	if (C_TYPE(c->value) == IS_CONSTANT_AST && crex_update_class_constant(c, name, c->ce) == FAILURE) {
		return;
	}

	const char *visibility = crex_visibility_string(CREX_CLASS_CONST_FLAGS(c));
	const char *final = CREX_CLASS_CONST_FLAGS(c) & CREX_ACC_FINAL ? "final " : "";
	crex_string *type_str = CREX_TYPE_IS_SET(c->type) ? crex_type_to_string(c->type) : NULL;
	const char *type = type_str ? ZSTR_VAL(type_str) : crex_zval_type_name(&c->value);
	smart_str_append_printf(str, "%sConstant [ %s%s %s %s ] { ",
		indent, final, visibility, type, ZSTR_VAL(name));
	if (C_TYPE(c->value) == IS_ARRAY) {
		smart_str_appends(str, "Array");
	} else if (C_TYPE(c->value) == IS_OBJECT) {
		smart_str_appends(str, "Object");
	} else {
		crex_string *tmp_value_str;
		crex_string *value_str = zval_get_tmp_string(&c->value, &tmp_value_str);
...
static crex_op *get_recv_op(crex_op_array *op_array, uint32_t offset)
{
	crex_op *op = op_array->opcodes;
	crex_op *end = op + op_array->last;

	++offset;
	while (op < end) {
		if ((op->opcode == CREX_RECV || op->opcode == CREX_RECV_INIT
		    || op->opcode == CREX_RECV_VARIADIC) && op->op1.num == offset)
		{
			return op;
		}
		++op;
	}
	CREX_ASSERT(0 && "Failed to find op");
	return NULL;
}

static zval *get_default_from_recv(crex_op_array *op_array, uint32_t offset) {
	crex_op *recv = get_recv_op(op_array, offset);
	if (!recv || recv->opcode != CREX_RECV_INIT) {
		return NULL;
	}
...
static int format_default_value(smart_str *str, zval *value) {
	if (C_TYPE_P(value) <= IS_STRING) {
	...
	} else if (C_TYPE_P(value) == IS_ARRAY) {
		crex_string *str_key;
		crex_long num_key;
		zval *zv;
		bool is_list = crex_array_is_list(C_ARRVAL_P(value));
		bool first = true;
		smart_str_appendc(str, '[');
		CREX_HASH_FOREACH_KEY_VAL(C_ARRVAL_P(value), num_key, str_key, zv) {
			if (!first) {
				smart_str_appends(str, ", ");
			}
			first = false;
		...
		smart_str_appendc(str, ']');
	} else if (C_TYPE_P(value) == IS_OBJECT) {
		/* This branch may only be reached for default properties, which don't support arbitrary objects. */
		crex_object *obj = C_OBJ_P(value);
		crex_class_entry *class = obj->ce;
		CREX_ASSERT(class->ce_flags & CREX_ACC_ENUM);
		smart_str_append(str, class->name);
		smart_str_appends(str, "::");
		smart_str_append(str, C_STR_P(crex_enum_fetch_case_name(obj)));
	} else {
		CREX_ASSERT(C_TYPE_P(value) == IS_CONSTANT_AST);
		crex_string *ast_str = crex_ast_export("", C_ASTVAL_P(value), "");
		smart_str_append(str, ast_str);
		crex_string_release(ast_str);
	}
	return SUCCESS;
}

static inline bool has_internal_arg_info(const crex_function *fptr) {
	return fptr->type == CREX_INTERNAL_FUNCTION
		&& !(fptr->common.fn_flags & CREX_ACC_USER_ARG_INFO);
}

/* {{{ _parameter_string */
static void _parameter_string(smart_str *str, crex_function *fptr, struct _crex_arg_info *arg_info, uint32_t offset, bool required, char* indent)
{
	...
	if (CREX_ARG_IS_VARIADIC(arg_info)) {
		smart_str_appends(str, "...");
	}
	smart_str_append_printf(str, "$%s", has_internal_arg_info(fptr)
		? ((crex_internal_arg_info*)arg_info)->name : ZSTR_VAL(arg_info->name));

	if (!required && !CREX_ARG_IS_VARIADIC(arg_info)) {
		if (fptr->type == CREX_INTERNAL_FUNCTION) {
			smart_str_appends(str, " = ");
			...
			if (has_internal_arg_info(fptr)
					&& ((crex_internal_arg_info*)arg_info)->default_value) {
				smart_str_appends(str, ((crex_internal_arg_info*)arg_info)->default_value);
			} else {
				smart_str_appends(str, "<default>");
			}
		} else {
			zval *default_value = get_default_from_recv((crex_op_array*)fptr, offset);
			if (default_value) {
				smart_str_appends(str, " = ");
...
static void _function_parameter_string(smart_str *str, crex_function *fptr, char* indent)
{
	struct _crex_arg_info *arg_info = fptr->common.arg_info;
	...
	if (fptr->common.fn_flags & CREX_ACC_VARIADIC) {
		num_args++;
	}
	smart_str_appendc(str, '\n');
...
static void _function_closure_string(smart_str *str, crex_function *fptr, char* indent)
{
	uint32_t i, count;
	crex_string *key;
	HashTable *static_variables;

	if (fptr->type != CREX_USER_FUNCTION || !fptr->op_array.static_variables) {
		return;
	}

	static_variables = CREX_MAP_PTR_GET(fptr->op_array.static_variables_ptr);
	count = crex_hash_num_elements(static_variables);
	...
	smart_str_append_printf(str, "\n");
	smart_str_append_printf(str, "%s- Bound Variables [%d] {\n", indent, crex_hash_num_elements(static_variables));
	i = 0;
	CREX_HASH_MAP_FOREACH_STR_KEY(static_variables, key) {
...
static void _function_string(smart_str *str, crex_function *fptr, crex_class_entry *scope, char* indent)
{
	 ...
	 */
	if (fptr->type == CREX_USER_FUNCTION && fptr->op_array.doc_comment) {
		smart_str_append_printf(str, "%s%s\n", indent, ZSTR_VAL(fptr->op_array.doc_comment));
	}

	smart_str_appendl(str, indent, strlen(indent));
	smart_str_append_printf(str, fptr->common.fn_flags & CREX_ACC_CLOSURE ? "Closure [ " : (fptr->common.scope ? "Method [ " : "Function [ "));
	smart_str_append_printf(str, (fptr->type == CREX_USER_FUNCTION) ? "<user" : "<internal");
	if (fptr->common.fn_flags & CREX_ACC_DEPRECATED) {
		smart_str_appends(str, ", deprecated");
	}
	if (fptr->type == CREX_INTERNAL_FUNCTION && ((crex_internal_function*)fptr)->module) {
		smart_str_append_printf(str, ":%s", ((crex_internal_function*)fptr)->module->name);
	}
	...
	if (scope && fptr->common.scope) {
		if (fptr->common.scope != scope) {
			...
			lc_name = crex_string_tolower(fptr->common.function_name);
			if ((overwrites = crex_hash_find_ptr(&fptr->common.scope->parent->function_table, lc_name)) != NULL) {
				if (fptr->common.scope != overwrites->common.scope && !(overwrites->common.fn_flags & CREX_ACC_PRIVATE)) {
	...
	if (fptr->common.fn_flags & CREX_ACC_CTOR) {
		smart_str_appends(str, ", ctor");
	}
	smart_str_appends(str, "> ");

	if (fptr->common.fn_flags & CREX_ACC_ABSTRACT) {
		smart_str_appends(str, "abstract ");
	}
	if (fptr->common.fn_flags & CREX_ACC_FINAL) {
	...
	if (fptr->common.scope) {
		...
		switch (fptr->common.fn_flags & CREX_ACC_PPP_MASK) {
			case CREX_ACC_PUBLIC:
				smart_str_appends(str, "public ");
				break;
			case CREX_ACC_PRIVATE:
				smart_str_appends(str, "private ");
				break;
			case CREX_ACC_PROTECTED:
	...
	if (fptr->op_array.fn_flags & CREX_ACC_RETURN_REFERENCE) {
		smart_str_appendc(str, '&');
	}
	smart_str_append_printf(str, "%s ] {\n", ZSTR_VAL(fptr->common.function_name));
	/* The information where a function is declared is only available for user classes */
	if (fptr->type == CREX_USER_FUNCTION) {
		smart_str_append_printf(str, "%s  @@ %s %d - %d\n", indent,
	...
	if (fptr->common.fn_flags & CREX_ACC_CLOSURE) {
	...
	if ((fptr->op_array.fn_flags & CREX_ACC_HAS_RETURN_TYPE)) {
		smart_str_append_printf(str, "  %s- %s [ ", indent, CREX_ARG_TYPE_IS_TENTATIVE(&fptr->common.arg_info[-1]) ? "Tentative return" : "Return");
		if (CREX_TYPE_IS_SET(fptr->common.arg_info[-1].type)) {
			crex_string *type_str = crex_type_to_string(fptr->common.arg_info[-1].type);
...
static zval *property_get_default(crex_property_info *prop_info) {
	crex_class_entry *ce = prop_info->ce;
...
static void _property_string(smart_str *str, crex_property_info *prop, const char *prop_name, char* indent)
{
	...
	if (!prop) {
		...
		/* These are mutually exclusive */
		switch (prop->flags & CREX_ACC_PPP_MASK) {
			case CREX_ACC_PUBLIC:
				smart_str_appends(str, "public ");
				break;
			case CREX_ACC_PRIVATE:
		...
		if (prop->flags & CREX_ACC_READONLY) {
			smart_str_appends(str, "readonly ");
		}
		if (CREX_TYPE_IS_SET(prop->type)) {
		...
		if (!prop_name) {
			...
			crex_unmangle_property_name(prop->name, &class_name, &prop_name);
		}
		smart_str_append_printf(str, "$%s", prop_name);

		zval *default_value = property_get_default(prop);
		if (!C_ISUNDEF_P(default_value)) {
			smart_str_appends(str, " = ");
			if (format_default_value(str, default_value) == FAILURE) {
				return;
			}
...
static void _extension_ini_string(crex_ini_entry *ini_entry, smart_str *str, char *indent, int number) /* {{{ */
{
	char *comma = "";

	if (number == ini_entry->module_number) {
		smart_str_append_printf(str, "    %sEntry [ %s <", indent, ZSTR_VAL(ini_entry->name));
		if (ini_entry->modifiable == CREX_INI_ALL) {
			smart_str_appends(str, "ALL");
		} else {
			if (ini_entry->modifiable & CREX_INI_USER) {
				smart_str_appends(str, "USER");
				comma = ",";
			}
			if (ini_entry->modifiable & CREX_INI_PERDIR) {
				smart_str_append_printf(str, "%sPERDIR", comma);
				comma = ",";
			}
			if (ini_entry->modifiable & CREX_INI_SYSTEM) {
...
static void _extension_class_string(crex_class_entry *ce, crex_string *key, smart_str *str, char *indent, crex_module_entry *module, int *num_classes) /* {{{ */
{
	if (ce->type == CREX_INTERNAL_CLASS && ce->info.internal.module && !strcasecmp(ce->info.internal.module->name, module->name)) {
		/* dump class if it is not an alias */
		if (crex_string_equals_ci(ce->name, key)) {
			smart_str_append_printf(str, "\n");
...
static void _extension_string(smart_str *str, crex_module_entry *module, char *indent) /* {{{ */
{
...

	{
		...
		CREX_HASH_MAP_FOREACH_PTR(EG(crex_constants), constant) {
			if (CREX_CONSTANT_MODULE_NUMBER(constant) == module->module_number) {
...

	{
		...
		CREX_HASH_MAP_FOREACH_PTR(CG(function_table), fptr) {
			if (fptr->common.type==CREX_INTERNAL_FUNCTION
...

	{
		...
		CREX_HASH_MAP_FOREACH_STR_KEY_PTR(EG(class_table), key, ce) {
...
static void reflection_attribute_factory(zval *object, HashTable *attributes, crex_attribute *data,
		crex_class_entry *scope, uint32_t target, crex_string *filename)
{
	reflection_object *intern;
	attribute_reference *reference;

	reflection_instantiate(reflection_attribute_ptr, object);
	intern  = C_REFLECTION_P(object);
	reference = (attribute_reference*) emalloc(sizeof(attribute_reference));
	reference->attributes = attributes;
	reference->data = data;
	reference->scope = scope;
	reference->filename = filename ? crex_string_copy(filename) : NULL;
...
static int read_attributes(zval *ret, HashTable *attributes, crex_class_entry *scope,
		uint32_t offset, uint32_t target, crex_string *name, crex_class_entry *base, crex_string *filename) /* {{{ */
{
	CREX_ASSERT(attributes != NULL);
	...
	if (name) {
		...
		crex_string *filter = crex_string_tolower(name);

		CREX_HASH_PACKED_FOREACH_PTR(attributes, attr) {
			if (attr->offset == offset && crex_string_equals(attr->lcname, filter)) {
				reflection_attribute_factory(&tmp, attributes, attr, scope, target, filename);
				add_next_index_zval(ret, &tmp);
			}
		} CREX_HASH_FOREACH_END();

		crex_string_release(filter);
		return SUCCESS;
	}
	...
	CREX_HASH_PACKED_FOREACH_PTR(attributes, attr) {
		...
		if (base) {
			// Base type filtering.
			crex_class_entry *ce = crex_lookup_class_ex(attr->name, attr->lcname, 0);
...
static void reflect_attributes(INTERNAL_FUNCTION_PARAMETERS, HashTable *attributes,
		uint32_t offset, crex_class_entry *scope, uint32_t target, crex_string *filename) /* {{{ */
{
	crex_string *name = NULL;
	crex_long flags = 0;
	crex_class_entry *base = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|S!l", &name, &flags) == FAILURE) {
		RETURN_THROWS();
	}

	if (flags & ~REFLECTION_ATTRIBUTE_IS_INSTANCEOF) {
		crex_argument_value_error(2, "must be a valid attribute filter flag");
		RETURN_THROWS();
	}

	if (name && (flags & REFLECTION_ATTRIBUTE_IS_INSTANCEOF)) {
		if (NULL == (base = crex_lookup_class(name))) {
			if (!EG(exception)) {
				crex_throw_error(NULL, "Class \"%s\" not found", ZSTR_VAL(name));
			}
...
static void _crex_extension_string(smart_str *str, crex_extension *extension, char *indent) /* {{{ */
{
	smart_str_append_printf(str, "%sCrex Extension [ %s ", indent, extension->name);
...
/* {{{ crex_reflection_class_factory */
CRXAPI void crex_reflection_class_factory(crex_class_entry *ce, zval *object)
{
	reflection_object *intern;

	crex_class_entry *reflection_ce =
		ce->ce_flags & CREX_ACC_ENUM ? reflection_enum_ptr : reflection_class_ptr;
...
static void reflection_extension_factory(zval *object, const char *name_str)
{
	...
	crex_str_tolower_copy(ZSTR_VAL(lcname), name_str, name_len);
...
static void reflection_parameter_factory(crex_function *fptr, zval *closure_object, struct _crex_arg_info *arg_info, uint32_t offset, bool required, zval *object)
{
	reflection_object *intern;
	parameter_reference *reference;
	zval *prop_name;
	...
	if (closure_object) {
		ZVAL_OBJ_COPY(&intern->obj, C_OBJ_P(closure_object));
	}

	prop_name = reflection_prop_name(object);
	if (has_internal_arg_info(fptr)) {
		ZVAL_STRING(prop_name, ((crex_internal_arg_info*)arg_info)->name);
	} else {
 ...
 * what doesn't. */
static reflection_type_kind get_type_kind(crex_type type) {
	uint32_t type_mask_without_null = CREX_TYPE_PURE_MASK_WITHOUT_NULL(type);
	...
	if (CREX_TYPE_IS_COMPLEX(type)) {
		...
		if (UNEXPECTED(CREX_TYPE_IS_ITERABLE_FALLBACK(type))) {
			return NAMED_TYPE;
		}
	...
	if (type_mask_without_null == MAY_BE_BOOL || CREX_TYPE_PURE_MASK(type) == MAY_BE_ANY) {
		return NAMED_TYPE;
	}
...
static void reflection_type_factory(crex_type type, zval *object, bool legacy_behavior)
{
	reflection_object *intern;
	type_reference *reference;
	reflection_type_kind type_kind = get_type_kind(type);
	bool is_mixed = CREX_TYPE_PURE_MASK(type) == MAY_BE_ANY;
	bool is_only_null = (CREX_TYPE_PURE_MASK(type) == MAY_BE_NULL && !CREX_TYPE_IS_COMPLEX(type));
	...
	if (CREX_TYPE_HAS_NAME(type)) {
		crex_string_addref(CREX_TYPE_NAME(type));
	}
}
/* }}} */

/* {{{ reflection_function_factory */
static void reflection_function_factory(crex_function *function, zval *closure_object, zval *object)
{
	reflection_object *intern;
...
static void reflection_method_factory(crex_class_entry *ce, crex_function *method, zval *closure_object, zval *object)
{
	reflection_object *intern;
...
static void reflection_property_factory(crex_class_entry *ce, crex_string *name, crex_property_info *prop, zval *object)
{
	reflection_object *intern;
	...
	reference->prop = prop;
	reference->unmangled_name = crex_string_copy(name);
	intern->ptr = reference;
...
static void reflection_property_factory_str(crex_class_entry *ce, const char *name_str, size_t name_len, crex_property_info *prop, zval *object)
{
	crex_string *name = crex_string_init(name_str, name_len, 0);
	reflection_property_factory(ce, name, prop, object);
	crex_string_release(name);
}

/* {{{ reflection_class_constant_factory */
static void reflection_class_constant_factory(crex_string *name_str, crex_class_constant *constant, zval *object)
{
	reflection_object *intern;
...
static void reflection_enum_case_factory(crex_class_entry *ce, crex_string *name_str, crex_class_constant *constant, zval *object)
{
	reflection_object *intern;

	crex_class_entry *case_reflection_class = ce->enum_backing_type == IS_UNDEF
...
static int get_parameter_default(zval *result, parameter_reference *param) {
	if (param->fptr->type == CREX_INTERNAL_FUNCTION) {
		if (param->fptr->common.fn_flags & CREX_ACC_USER_ARG_INFO) {
			/* We don't have a way to determine the default value for this case right now. */
			return FAILURE;
		}
		return crex_get_default_from_internal_arg_info(
			result, (crex_internal_arg_info *) param->arg_info);
	} else {
		zval *default_value = get_default_from_recv((crex_op_array *) param->fptr, param->offset);
		if (!default_value) {
			return FAILURE;
		}
...
CREX_METHOD(Reflection, getModifierNames)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "l", &modifiers) == FAILURE) {
		RETURN_THROWS();
	}

	array_init(return_value);

	if (modifiers & (CREX_ACC_ABSTRACT | CREX_ACC_EXPLICIT_ABSTRACT_CLASS)) {
		add_next_index_stringl(return_value, "abstract", sizeof("abstract")-1);
	}
	if (modifiers & CREX_ACC_FINAL) {
		add_next_index_stringl(return_value, "final", sizeof("final")-1);
	}

	/* These are mutually exclusive */
	switch (modifiers & CREX_ACC_PPP_MASK) {
		case CREX_ACC_PUBLIC:
	...
	if (modifiers & CREX_ACC_STATIC) {
		add_next_index_str(return_value, ZSTR_KNOWN(CREX_STR_STATIC));
	}

	if (modifiers & (CREX_ACC_READONLY | CREX_ACC_READONLY_CLASS)) {
...
{
	zval *object;
	crex_object *closure_obj = NULL;
	reflection_object *intern;
	crex_function *fptr;
	crex_string *fname, *lcname;

	object = CREX_THIS;
	intern = C_REFLECTION_P(object);

	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_OBJ_OF_CLASS_OR_STR(closure_obj, crex_ce_closure, fname)
	CREX_PARSE_PARAMETERS_END();

	if (closure_obj) {
		fptr = (crex_function*)crex_get_closure_method_def(closure_obj);
	} else {
		if (UNEXPECTED(ZSTR_VAL(fname)[0] == '\\')) {
			/* Ignore leading "\" */
			ALLOCA_FLAG(use_heap)
			ZSTR_ALLOCA_ALLOC(lcname, ZSTR_LEN(fname) - 1, use_heap);
			crex_str_tolower_copy(ZSTR_VAL(lcname), ZSTR_VAL(fname) + 1, ZSTR_LEN(fname) - 1);
			fptr = crex_fetch_function(lcname);
			ZSTR_ALLOCA_FREE(lcname, use_heap);
		} else {
			lcname = crex_string_tolower(fname);
			fptr = crex_fetch_function(lcname);
			crex_string_release(lcname);
		}

		if (fptr == NULL) {
			crex_throw_exception_ex(reflection_exception_ptr, 0,
...
CREX_METHOD(ReflectionFunctionAbstract, isClosure)
{
	...
	GET_REFLECTION_OBJECT_PTR(fptr);
	RETURN_BOOL(fptr->common.fn_flags & CREX_ACC_CLOSURE);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getClosureThis)
{
	...
	GET_REFLECTION_OBJECT();
	if (!C_ISUNDEF(intern->obj)) {
		closure_this = crex_get_closure_this_ptr(&intern->obj);
		if (!C_ISUNDEF_P(closure_this)) {
			RETURN_OBJ_COPY(C_OBJ_P(closure_this));
		}
	}
}
/* }}} */
...
{
	reflection_object *intern;
	const crex_function *closure_func;
	...
	GET_REFLECTION_OBJECT();
	if (!C_ISUNDEF(intern->obj)) {
		closure_func = crex_get_closure_method_def(C_OBJ(intern->obj));
		if (closure_func && closure_func->common.scope) {
			crex_reflection_class_factory(closure_func->common.scope, return_value);
		}
	}
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getClosureCalledClass)
{
	...
	if (!C_ISUNDEF(intern->obj)) {
		...
		crex_function *closure_func;
		crex_object *object;
		if (C_OBJ_HANDLER(intern->obj, get_closure)
		 && C_OBJ_HANDLER(intern->obj, get_closure)(C_OBJ(intern->obj), &called_scope, &closure_func, &object, 1) == SUCCESS
		 && closure_func && (called_scope || closure_func->common.scope)) {
			crex_reflection_class_factory(called_scope ? (crex_class_entry *) called_scope : closure_func->common.scope, return_value);
		}
	}
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getClosureUsedVariables)
{
	...
	if (!C_ISUNDEF(intern->obj)) {
		...
		if (closure_func == NULL ||
			closure_func->type != CREX_USER_FUNCTION ||
			closure_func->op_array.static_variables == NULL) {
			return;
		}

		const crex_op_array *ops = &closure_func->op_array;

		HashTable *static_variables = CREX_MAP_PTR_GET(ops->static_variables_ptr);

		if (!static_variables) {
			return;
		}

		crex_op *opline = ops->opcodes + ops->num_args;
		if (ops->fn_flags & CREX_ACC_VARIADIC) {
			opline++;
		}

		for (; opline->opcode == CREX_BIND_STATIC; opline++)  {
			...
			Bucket *bucket = (Bucket*)
				...
				(opline->extended_value & ~(CREX_BIND_REF|CREX_BIND_IMPLICIT|CREX_BIND_EXPLICIT)));

			if (C_ISUNDEF(bucket->val)) {
				continue;
			}

			crex_hash_add_new(C_ARRVAL_P(return_value), bucket->key, &bucket->val);
			C_TRY_ADDREF(bucket->val);
		}
	}
} /* }}} */
...
CREX_METHOD(ReflectionFunction, getClosure)
{
	...
	if (!C_ISUNDEF(intern->obj)) {
		...
		RETURN_OBJ_COPY(C_OBJ(intern->obj));
	} else {
		crex_create_fake_closure(return_value, fptr, NULL, NULL, NULL);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, isInternal)
{
	...
	GET_REFLECTION_OBJECT_PTR(fptr);
	RETURN_BOOL(fptr->type == CREX_INTERNAL_FUNCTION);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, isUserDefined)
{
	...
	GET_REFLECTION_OBJECT_PTR(fptr);
	RETURN_BOOL(fptr->type == CREX_USER_FUNCTION);
}
/* }}} */
...
CREX_METHOD(ReflectionFunction, isAnonymous)
{
	...
	GET_REFLECTION_OBJECT_PTR(fptr);
	RETURN_BOOL((fptr->common.fn_flags & (CREX_ACC_CLOSURE | CREX_ACC_FAKE_CLOSURE)) == CREX_ACC_CLOSURE);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getAttributes)
{
	...
	if (fptr->common.scope && (fptr->common.fn_flags & (CREX_ACC_CLOSURE|CREX_ACC_FAKE_CLOSURE)) != CREX_ACC_CLOSURE) {
		target = CREX_ATTRIBUTE_TARGET_METHOD;
	} else {
		target = CREX_ATTRIBUTE_TARGET_FUNCTION;
	}

	reflect_attributes(INTERNAL_FUNCTION_PARAM_PASSTHRU,
		fptr->common.attributes, 0, fptr->common.scope, target,
		fptr->type == CREX_USER_FUNCTION ? fptr->op_array.filename : NULL);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getStaticVariables)
{
	...
	if (fptr->type == CREX_USER_FUNCTION && fptr->op_array.static_variables != NULL) {
		HashTable *ht;

		array_init(return_value);
		ht = CREX_MAP_PTR_GET(fptr->op_array.static_variables_ptr);
		if (!ht) {
			ht = crex_array_dup(fptr->op_array.static_variables);
			CREX_MAP_PTR_SET(fptr->op_array.static_variables_ptr, ht);
		}
		crex_hash_copy(C_ARRVAL_P(return_value), ht, zval_add_ref);
	} else {
		RETURN_EMPTY_ARRAY();
	}
}
...
CREX_METHOD(ReflectionFunction, invoke)
{
	...
	CREX_PARSE_PARAMETERS_START(0, -1)
		C_PARAM_VARIADIC_WITH_NAMED(params, num_args, named_params)
	CREX_PARSE_PARAMETERS_END();
	...
	if (!C_ISUNDEF(intern->obj)) {
		C_OBJ_HT(intern->obj)->get_closure(
			C_OBJ(intern->obj), &fcc.called_scope, &fcc.function_handler, &fcc.object, 0);
	}

	crex_call_known_fcc(&fcc, &retval, num_args, params, named_params);
...
CREX_METHOD(ReflectionFunction, invokeArgs)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "h", &params) == FAILURE) {
		RETURN_THROWS();
	}
	...
	crex_call_known_fcc(&fcc, &retval, /* num_params */ 0, /* params */ NULL, params);
...
CREX_METHOD(ReflectionFunctionAbstract, returnsReference)
{
	...
	RETURN_BOOL((fptr->op_array.fn_flags & CREX_ACC_RETURN_REFERENCE) != 0);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getParameters)
{
	...
	for (i = 0; i < num_args; i++) {
		...
		reflection_parameter_factory(
			_copy_function(fptr),
			C_ISUNDEF(intern->obj) ? NULL : &intern->obj,
			arg_info,
			i,
			i < fptr->common.required_num_args,
			&parameter
		);
		crex_hash_next_index_insert_new(C_ARRVAL_P(return_value), &parameter);
...
CREX_METHOD(ReflectionFunctionAbstract, getExtension)
{
	reflection_object *intern;
	crex_function *fptr;
	crex_internal_function *internal;
	...
	internal = (crex_internal_function *)fptr;
	if (internal->module) {
...
CREX_METHOD(ReflectionGenerator, __main)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "O", &generator, crex_ce_generator) == FAILURE) {
		RETURN_THROWS();
	}

	ex = ((crex_generator *) C_OBJ_P(generator))->execute_data;
	if (!ex) {
	...
	ZVAL_OBJ_COPY(&intern->obj, C_OBJ_P(generator));
	intern->ce = crex_ce_generator;
}
/* }}} */
...
CREX_METHOD(ReflectionGenerator, getTrace)
{
	...
	crex_generator *generator = (crex_generator *) C_OBJ(C_REFLECTION_P(CREX_THIS)->obj);
	crex_generator *root_generator;
	crex_execute_data *ex_backup = EG(current_execute_data);
	crex_execute_data *ex = generator->execute_data;
	crex_execute_data *root_prev = NULL, *cur_prev;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "|l", &options) == FAILURE) {
		RETURN_THROWS();
	}

	REFLECTION_CHECK_VALID_GENERATOR(ex)

	root_generator = crex_generator_get_current(generator);
	...
	crex_fetch_debug_backtrace(return_value, 0, options, 0);
...
CREX_METHOD(ReflectionGenerator, getFunction)
{
	...
	if (ex->func->common.fn_flags & CREX_ACC_CLOSURE) {
		zval closure;
		ZVAL_OBJ(&closure, CREX_CLOSURE_OBJECT(ex->func));
...
CREX_METHOD(ReflectionGenerator, getExecutingGenerator)
{
	...
	current = crex_generator_get_current(generator);
...
CREX_METHOD(ReflectionParameter, __main)
{
	...
	CREX_PARSE_PARAMETERS_START(2, 2)
		C_PARAM_ZVAL(reference)
		C_PARAM_STR_OR_LONG(arg_name, position)
	CREX_PARSE_PARAMETERS_END();
	...
	switch (C_TYPE_P(reference)) {
		case IS_STRING:
			{
				crex_string *lcname = crex_string_tolower(C_STR_P(reference));
				fptr = crex_hash_find_ptr(EG(function_table), lcname);
				crex_string_release(lcname);
				if (!fptr) {
					crex_throw_exception_ex(reflection_exception_ptr, 0,
						"Function %s() does not exist", C_STRVAL_P(reference));
					RETURN_THROWS();
				}
		...
		case IS_ARRAY: {
				...
				if (((classref = crex_hash_index_find(C_ARRVAL_P(reference), 0)) == NULL)
					|| ((method = crex_hash_index_find(C_ARRVAL_P(reference), 1)) == NULL))
				{
					_DO_THROW("Expected array($object, $method) or array($classname, $method)");
					RETURN_THROWS();
				}

				if (C_TYPE_P(classref) == IS_OBJECT) {
					ce = C_OBJCE_P(classref);
				} else {
					name = zval_try_get_string(classref);
					if (UNEXPECTED(!name)) {
						return;
					}
					if ((ce = crex_lookup_class(name)) == NULL) {
						crex_throw_exception_ex(reflection_exception_ptr, 0,
				...
				if (C_TYPE_P(classref) == IS_OBJECT && is_closure_invoke(ce, lcname)
					&& (fptr = crex_get_closure_invoke_method(C_OBJ_P(classref))) != NULL)
				{
					/* nothing to do. don't set is_closure since is the invoke handler,
					   not the closure itself */
				} else if ((fptr = crex_hash_find_ptr(&ce->function_table, lcname)) == NULL) {
					crex_throw_exception_ex(reflection_exception_ptr, 0,
		...
		case IS_OBJECT: {
				ce = C_OBJCE_P(reference);

				if (instanceof_function(ce, crex_ce_closure)) {
					fptr = (crex_function *)crex_get_closure_method_def(C_OBJ_P(reference));
					C_ADDREF_P(reference);
					is_closure = 1;
				} else if ((fptr = crex_hash_find_ptr(&ce->function_table, ZSTR_KNOWN(CREX_STR_MAGIC_INVOKE))) == NULL) {
					crex_throw_exception_ex(reflection_exception_ptr, 0,
						"Method %s::%s() does not exist", ZSTR_VAL(ce->name), CREX_INVOKE_FUNC_NAME);
					RETURN_THROWS();
				}
			}
			break;
		...
		default:
			crex_argument_error(reflection_exception_ptr, 1, "must be a string, an array(class, method), or a callable object, %s given", crex_zval_value_name(reference));
			RETURN_THROWS();
	}
	...
	if (arg_name != NULL) {
		...
		if (has_internal_arg_info(fptr)) {
			for (i = 0; i < num_args; i++) {
				if (arg_info[i].name) {
					if (strcmp(((crex_internal_arg_info*)arg_info)[i].name, ZSTR_VAL(arg_name)) == 0) {
						position = i;
						break;
					}
				}
			...
			for (i = 0; i < num_args; i++) {
				if (arg_info[i].name) {
					if (crex_string_equals(arg_name, arg_info[i].name)) {
						position = i;
						break;
					}
				}
	...
	prop_name = reflection_prop_name(object);
	if (has_internal_arg_info(fptr)) {
		ZVAL_STRING(prop_name, ((crex_internal_arg_info*)arg_info)[position].name);
	} else {
...
failure:
	if (fptr->common.fn_flags & CREX_ACC_CALL_VIA_TRAMPOLINE) {
		crex_string_release_ex(fptr->common.function_name, 0);
		crex_free_trampoline(fptr);
	}
	if (is_closure) {
...
CREX_METHOD(ReflectionParameter, getName)
{
	...
	if (has_internal_arg_info(param->fptr)) {
		RETURN_STRING(((crex_internal_arg_info *) param->arg_info)->name);
	} else {
...
CREX_METHOD(ReflectionParameter, getDeclaringFunction)
{
	...
	if (!param->fptr->common.scope) {
		reflection_function_factory(_copy_function(param->fptr), C_ISUNDEF(intern->obj)? NULL : &intern->obj, return_value);
	} else {
		reflection_method_factory(param->fptr->common.scope, _copy_function(param->fptr), C_ISUNDEF(intern->obj)? NULL : &intern->obj, return_value);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, getDeclaringClass)
{
	...
	if (param->fptr->common.scope) {
		crex_reflection_class_factory(param->fptr->common.scope, return_value);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, getClass)
{
	...
	if (CREX_TYPE_HAS_NAME(param->arg_info->type)) {
		...
		class_name = CREX_TYPE_NAME(param->arg_info->type);
		if (crex_string_equals_literal_ci(class_name, "self")) {
			...
			ce = crex_lookup_class(class_name);
			if (!ce) {
				crex_throw_exception_ex(reflection_exception_ptr, 0,
		...
		crex_reflection_class_factory(ce, return_value);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, hasType)
{
	...
	RETVAL_BOOL(CREX_TYPE_IS_SET(param->arg_info->type));
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, getType)
{
	...
	if (!CREX_TYPE_IS_SET(param->arg_info->type)) {
		RETURN_NULL();
	}
...
CREX_METHOD(ReflectionParameter, isArray)
{
	...
	/* BC For iterable */
	if (CREX_TYPE_IS_ITERABLE_FALLBACK(param->arg_info->type)) {
		RETURN_FALSE;
	}

	type_mask = CREX_TYPE_PURE_MASK_WITHOUT_NULL(param->arg_info->type);
...
CREX_METHOD(ReflectionParameter, allowsNull)
{
	...
	RETVAL_BOOL(!CREX_TYPE_IS_SET(param->arg_info->type)
		|| CREX_TYPE_ALLOW_NULL(param->arg_info->type));
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, isPassedByReference)
{
	...
	RETVAL_BOOL(CREX_ARG_SEND_MODE(param->arg_info));
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, canBePassedByValue)
{
	...
	/* true if it's CREX_SEND_BY_VAL or CREX_SEND_PREFER_REF */
	RETVAL_BOOL(CREX_ARG_SEND_MODE(param->arg_info) != CREX_SEND_BY_REF);
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, getAttributes)
{
	...
	crex_class_entry *scope = param->fptr->common.scope;

	reflect_attributes(INTERNAL_FUNCTION_PARAM_PASSTHRU,
		attributes, param->offset + 1, scope, CREX_ATTRIBUTE_TARGET_PARAMETER,
		param->fptr->type == CREX_USER_FUNCTION ? param->fptr->op_array.filename : NULL);
}
...
CREX_METHOD(ReflectionParameter, isDefaultValueAvailable)
{
	...
	if (param->fptr->type == CREX_INTERNAL_FUNCTION) {
		RETURN_BOOL(!(param->fptr->common.fn_flags & CREX_ACC_USER_ARG_INFO)
			&& ((crex_internal_arg_info*) (param->arg_info))->default_value);
	} else {
		zval *default_value = get_default_from_recv((crex_op_array *)param->fptr, param->offset);
...
CREX_METHOD(ReflectionParameter, isDefaultValueConstant)
{
	...
	if (C_TYPE(default_value) == IS_CONSTANT_AST) {
		crex_ast *ast = C_ASTVAL(default_value);
		RETVAL_BOOL(ast->kind == CREX_AST_CONSTANT
...
CREX_METHOD(ReflectionParameter, getDefaultValueConstantName)
{
	...
	if (ast->kind == CREX_AST_CONSTANT) {
		RETVAL_STR_COPY(crex_ast_get_constant_name(ast));
	} else if (ast->kind == CREX_AST_CONSTANT_CLASS) {
		RETVAL_STRINGL("__CLASS__", sizeof("__CLASS__")-1);
	} else if (ast->kind == CREX_AST_CLASS_CONST) {
		crex_string *class_name = crex_ast_get_str(ast->child[0]);
		crex_string *const_name = crex_ast_get_str(ast->child[1]);
		RETVAL_NEW_STR(crex_string_concat3(
...
CREX_METHOD(ReflectionParameter, isVariadic)
{
	...
	RETVAL_BOOL(CREX_ARG_IS_VARIADIC(param->arg_info));
}
/* }}} */
...
CREX_METHOD(ReflectionParameter, isPromoted)
{
	...
	RETVAL_BOOL(CREX_ARG_IS_PROMOTED(param->arg_info));
}
/* }}} */
...
CREX_METHOD(ReflectionType, allowsNull)
{
	...
	RETVAL_BOOL(CREX_TYPE_ALLOW_NULL(param->type));
}
/* }}} */

/* For BC with iterable for named types */
static crex_string *crex_named_reflection_type_to_string(crex_type type) {
	if (CREX_TYPE_IS_ITERABLE_FALLBACK(type)) {
		crex_string *iterable = ZSTR_KNOWN(CREX_STR_ITERABLE);
		if (CREX_TYPE_FULL_MASK(type) & MAY_BE_NULL) {
			return crex_string_concat2("?", strlen("?"), ZSTR_VAL(iterable), ZSTR_LEN(iterable));
		}
		return iterable;
	}
	return crex_type_to_string(type);
}

static crex_string *crex_type_to_string_without_null(crex_type type) {
	CREX_TYPE_FULL_MASK(type) &= ~MAY_BE_NULL;
	return crex_named_reflection_type_to_string(type);
}
...
CREX_METHOD(ReflectionType, __toString)
{
	...
	RETURN_STR(crex_named_reflection_type_to_string(param->type));
}
/* }}} */
...
CREX_METHOD(ReflectionNamedType, getName)
{
	...
	if (param->legacy_behavior) {
		RETURN_STR(crex_type_to_string_without_null(param->type));
	}
	RETURN_STR(crex_named_reflection_type_to_string(param->type));
}
/* }}} */
...
CREX_METHOD(ReflectionNamedType, isBuiltin)
{
	...
	if (CREX_TYPE_IS_ITERABLE_FALLBACK(param->type)) {
		RETURN_TRUE;
	}

	/* Treat "static" as a class type for the purposes of reflection. */
	RETVAL_BOOL(CREX_TYPE_IS_ONLY_MASK(param->type)
		&& !(CREX_TYPE_FULL_MASK(param->type) & MAY_BE_STATIC));
}
/* }}} */

static void append_type(zval *return_value, crex_type type) {
	zval reflection_type;
	/* Drop iterable BC bit for type list */
	if (CREX_TYPE_IS_ITERABLE_FALLBACK(type)) {
		CREX_TYPE_FULL_MASK(type) &= ~_CREX_TYPE_ITERABLE_BIT;
	}

	reflection_type_factory(type, &reflection_type, 0);
	crex_hash_next_index_insert(C_ARRVAL_P(return_value), &reflection_type);
}

static void append_type_mask(zval *return_value, uint32_t type_mask) {
	append_type(return_value, (crex_type) CREX_TYPE_INIT_MASK(type_mask));
}
...
CREX_METHOD(ReflectionUnionType, getTypes)
{
	...
	array_init(return_value);
	if (CREX_TYPE_HAS_LIST(param->type)) {
		crex_type *list_type;
		CREX_TYPE_LIST_FOREACH(CREX_TYPE_LIST(param->type), list_type) {
			append_type(return_value, *list_type);
		} CREX_TYPE_LIST_FOREACH_END();
	} else if (CREX_TYPE_HAS_NAME(param->type)) {
		crex_string *name = CREX_TYPE_NAME(param->type);
		append_type(return_value, (crex_type) CREX_TYPE_INIT_CLASS(name, 0, 0));
	}

	type_mask = CREX_TYPE_PURE_MASK(param->type);
	CREX_ASSERT(!(type_mask & MAY_BE_VOID));
	CREX_ASSERT(!(type_mask & MAY_BE_NEVER));
...
CREX_METHOD(ReflectionIntersectionType, getTypes)
{
	...
	CREX_ASSERT(CREX_TYPE_HAS_LIST(param->type));
...
{
	crex_object *arg1_obj = NULL;
	crex_string *arg1_str;
	crex_string *arg2_str = NULL;

	crex_object *orig_obj = NULL;
	crex_class_entry *ce = NULL;
	...
	if (is_constructor) {
		CREX_PARSE_PARAMETERS_START(1, 2)
			C_PARAM_OBJ_OR_STR(arg1_obj, arg1_str)
			C_PARAM_OPTIONAL
			C_PARAM_STR_OR_NULL(arg2_str)
		CREX_PARSE_PARAMETERS_END();
	} else {
		CREX_PARSE_PARAMETERS_START(1, 1)
			C_PARAM_STR(arg1_str)
		CREX_PARSE_PARAMETERS_END();
	}

	if (arg1_obj) {
		if (!arg2_str) {
			crex_argument_value_error(2, "cannot be null when argument #1 ($objectOrMethod) is an object");
			RETURN_THROWS();
		}
		...
		class_name = crex_string_copy(arg1_str);
		method_name = ZSTR_VAL(arg2_str);
		method_name_len = ZSTR_LEN(arg2_str);
	} else {
		char *tmp;
		...
		if ((tmp = strstr(name, "::")) == NULL) {
			crex_argument_error(reflection_exception_ptr, 1, "must be a valid method name");
			RETURN_THROWS();
		}
		tmp_len = tmp - name;

		class_name = crex_string_init(name, tmp_len, 0);
		method_name = tmp + 2;
	...
	if (class_name) {
		if ((ce = crex_lookup_class(class_name)) == NULL) {
			if (!EG(exception)) {
				crex_throw_exception_ex(reflection_exception_ptr, 0, "Class \"%s\" does not exist", ZSTR_VAL(class_name));
			}
	...
	lcname = crex_str_tolower_dup(method_name, method_name_len);

	if (ce == crex_ce_closure && orig_obj && (method_name_len == sizeof(CREX_INVOKE_FUNC_NAME)-1)
		&& memcmp(lcname, CREX_INVOKE_FUNC_NAME, sizeof(CREX_INVOKE_FUNC_NAME)-1) == 0
		&& (mptr = crex_get_closure_invoke_method(orig_obj)) != NULL)
	{
		/* do nothing, mptr already set */
	} else if ((mptr = crex_hash_str_find_ptr(&ce->function_table, lcname, method_name_len)) == NULL) {
		efree(lcname);
		crex_throw_exception_ex(reflection_exception_ptr, 0,
...
CREX_METHOD(ReflectionMethod, getClosure)
{
	...
	if (mptr->common.fn_flags & CREX_ACC_STATIC)  {
		crex_create_fake_closure(return_value, mptr, mptr->common.scope, mptr->common.scope, NULL);
	} else {
		if (!obj) {
			crex_argument_value_error(1, "cannot be null for non-static methods");
			RETURN_THROWS();
		}

		if (!instanceof_function(C_OBJCE_P(obj), mptr->common.scope)) {
			_DO_THROW("Given object is not an instance of the class this method was declared in");
			RETURN_THROWS();
		}

		/* This is an original closure object and __invoke is to be called. */
		if (C_OBJCE_P(obj) == crex_ce_closure &&
			(mptr->internal_function.fn_flags & CREX_ACC_CALL_VIA_TRAMPOLINE))
		{
			RETURN_OBJ_COPY(C_OBJ_P(obj));
		} else {
			crex_create_fake_closure(return_value, mptr, mptr->common.scope, C_OBJCE_P(obj), obj);
		}
	}
}
/* }}} */
...
static void reflection_method_invoke(INTERNAL_FUNCTION_PARAMETERS, int variadic)
{
	...
	reflection_object *intern;
	crex_function *mptr, *callback;
	uint32_t argc = 0;
	crex_class_entry *obj_ce;

	GET_REFLECTION_OBJECT_PTR(mptr);

	if (mptr->common.fn_flags & CREX_ACC_ABSTRACT) {
		crex_throw_exception_ex(reflection_exception_ptr, 0,
	...
	if (variadic) {
		CREX_PARSE_PARAMETERS_START(1, -1)
			C_PARAM_OBJECT_OR_NULL(object)
			C_PARAM_VARIADIC_WITH_NAMED(params, argc, named_params)
		CREX_PARSE_PARAMETERS_END();
	} else {
		if (crex_parse_parameters(CREX_NUM_ARGS(), "o!h", &object, &named_params) == FAILURE) {
			RETURN_THROWS();
		}
	}
	...
	/* Copy the crex_function when calling via handler (e.g. Closure::__invoke()) */
	callback = _copy_function(mptr);
	crex_call_known_function(callback, (object ? C_OBJ_P(object) : NULL), intern->ce, &retval, argc, params, named_params);
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_FINAL);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_ABSTRACT);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PUBLIC);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PRIVATE);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PROTECTED);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_DEPRECATED);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_GENERATOR);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_VARIADIC);
}
/* }}} */
...
{
	_function_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_STATIC);
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, inNamespace)
{
	...
	crex_string *name = fptr->common.function_name;
	const char *backslash = crex_memrchr(ZSTR_VAL(name), '\\', ZSTR_LEN(name));
...
CREX_METHOD(ReflectionFunctionAbstract, hasReturnType)
{
	...
	RETVAL_BOOL((fptr->op_array.fn_flags & CREX_ACC_HAS_RETURN_TYPE) && !CREX_ARG_TYPE_IS_TENTATIVE(&fptr->common.arg_info[-1]));
}
/* }}} */
...
CREX_METHOD(ReflectionFunctionAbstract, getReturnType)
{
	...
	if (!(fptr->op_array.fn_flags & CREX_ACC_HAS_RETURN_TYPE) || CREX_ARG_TYPE_IS_TENTATIVE(&fptr->common.arg_info[-1])) {
		RETURN_NULL();
	}
...
CREX_METHOD(ReflectionMethod, isConstructor)
{
	...
	RETURN_BOOL((mptr->common.fn_flags & CREX_ACC_CTOR) && intern->ce->constructor && intern->ce->constructor->common.scope == mptr->common.scope);
}
/* }}} */
...
CREX_METHOD(ReflectionMethod, isDestructor)
{
	...
	GET_REFLECTION_OBJECT_PTR(mptr);
	RETURN_BOOL(crex_string_equals_literal_ci(
		mptr->common.function_name, CREX_DESTRUCTOR_FUNC_NAME));
}
/* }}} */
...
CREX_METHOD(ReflectionMethod, getModifiers)
{
	reflection_object *intern;
	crex_function *mptr;
	uint32_t keep_flags = CREX_ACC_PPP_MASK
		| CREX_ACC_STATIC | CREX_ACC_ABSTRACT | CREX_ACC_FINAL;
...
CREX_METHOD(ReflectionMethod, getDeclaringClass)
{
	...
	crex_reflection_class_factory(mptr->common.scope, return_value);
}
/* }}} */
...
CREX_METHOD(ReflectionMethod, setAccessible)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "b", &visible) == FAILURE) {
		RETURN_THROWS();
	}
}
/* }}} */
...
{
	zval *object;
	crex_string *classname_str;
	crex_object *classname_obj;
	crex_string *constname;
	reflection_object *intern;
	crex_class_entry *ce;
	crex_class_constant *constant = NULL;

	CREX_PARSE_PARAMETERS_START(2, 2)
		C_PARAM_OBJ_OR_STR(classname_obj, classname_str)
		C_PARAM_STR(constname)
	CREX_PARSE_PARAMETERS_END();

	if (classname_obj) {
		ce = classname_obj->ce;
	} else {
		if ((ce = crex_lookup_class(classname_str)) == NULL) {
			crex_throw_exception_ex(reflection_exception_ptr, 0, "Class \"%s\" does not exist", ZSTR_VAL(classname_str));
			RETURN_THROWS();
		}
	}
	...
	if ((constant = crex_hash_find_ptr(CE_CONSTANTS_TABLE(ce), constname)) == NULL) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Constant %s::%s does not exist", ZSTR_VAL(ce->name), ZSTR_VAL(constname));
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionClassConstant, __toString)
{
	...
	zval *name = reflection_prop_name(CREX_THIS);
	if (C_ISUNDEF_P(name)) {
	...
	ZVAL_DEREF(name);
	CREX_ASSERT(C_TYPE_P(name) == IS_STRING);

	_class_const_string(&str, C_STR_P(name), ref, "");
	RETURN_STR(smart_str_extract(&str));
}
/* }}} */
...
CREX_METHOD(ReflectionClassConstant, hasType)
{
	...
	GET_REFLECTION_OBJECT_PTR(ref);
	RETVAL_BOOL(CREX_TYPE_IS_SET(ref->type));
}
...
static void _class_constant_check_flag(INTERNAL_FUNCTION_PARAMETERS, int mask) /* {{{ */
{
	...
	GET_REFLECTION_OBJECT_PTR(ref);
	RETURN_BOOL(CREX_CLASS_CONST_FLAGS(ref) & mask);
}
/* }}} */
...
{
	_class_constant_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PUBLIC);
}
/* }}} */
...
{
	_class_constant_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PRIVATE);
}
/* }}} */
...
{
	_class_constant_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PROTECTED);
}
/* }}} */
...
{
	_class_constant_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_FINAL);
}
...
CREX_METHOD(ReflectionClassConstant, getModifiers)
{
	reflection_object *intern;
	crex_class_constant *ref;
	uint32_t keep_flags = CREX_ACC_FINAL | CREX_ACC_PPP_MASK;
	...
	RETURN_LONG(CREX_CLASS_CONST_FLAGS(ref) & keep_flags);
}
/* }}} */
...
CREX_METHOD(ReflectionClassConstant, getValue)
{
	...
	if (C_TYPE(ref->value) == IS_CONSTANT_AST) {
		crex_result result = crex_update_class_constant(ref, C_STR_P(name), ref->ce);
		if (result == FAILURE) {
			RETURN_THROWS();
		}
	}
...
CREX_METHOD(ReflectionClassConstant, getDeclaringClass)
{
	...
	crex_reflection_class_factory(ref->ce, return_value);
}
/* }}} */
...
CREX_METHOD(ReflectionClassConstant, getAttributes)
{
	...
	reflect_attributes(INTERNAL_FUNCTION_PARAM_PASSTHRU,
		ref->attributes, 0, ref->ce, CREX_ATTRIBUTE_TARGET_CLASS_CONST,
		ref->ce->type == CREX_USER_CLASS ? ref->ce->info.user.filename : NULL);
}
/* }}} */
...
CREX_METHOD(ReflectionClassConstant, isEnumCase)
{
	...
	RETURN_BOOL(CREX_CLASS_CONST_FLAGS(ref) & CREX_CLASS_CONST_IS_CASE);
}
...
static void reflection_class_object_ctor(INTERNAL_FUNCTION_PARAMETERS, int is_object)
{
	...
	if (is_object) {
		...
		CREX_PARSE_PARAMETERS_START(1, 1)
			C_PARAM_OBJ_OR_STR(arg_obj, arg_class)
		CREX_PARSE_PARAMETERS_END();
	}
	...
	if (arg_obj) {
		...
		if ((ce = crex_lookup_class(arg_class)) == NULL) {
			if (!EG(exception)) {
				crex_throw_exception_ex(reflection_exception_ptr, -1, "Class \"%s\" does not exist", ZSTR_VAL(arg_class));
			}
			RETURN_THROWS();
		}
...
/* {{{ add_class_vars */
static void add_class_vars(crex_class_entry *ce, bool statics, zval *return_value)
{
	...
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&ce->properties_info, key, prop_info) {
		...
		bool is_static = (prop_info->flags & CREX_ACC_STATIC) != 0;
		...
		* properties, returned to user */
		if (C_TYPE(prop_copy) == IS_CONSTANT_AST) {
			if (UNEXPECTED(zval_update_constant_ex(&prop_copy, ce) != SUCCESS)) {
				return;
			}
		}

		crex_hash_update(C_ARRVAL_P(return_value), key, &prop_copy);
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getStaticProperties)
{
	...
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&ce->properties_info, key, prop_info) {
		...
		if ((prop_info->flags & CREX_ACC_STATIC) == 0) {
			continue;
		}
		...
		if (CREX_TYPE_IS_SET(prop_info->type) && C_ISUNDEF_P(prop)) {
			continue;
		}
		...
		crex_hash_update(C_ARRVAL_P(return_value), key, prop);
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
{
	reflection_object *intern;
	crex_class_entry *ce, *old_scope;
	crex_string *name;
	zval *prop, *def_value = NULL;

	if (crex_parse_parameters(CREX_NUM_ARGS(), "S|z", &name, &def_value) == FAILURE) {
		RETURN_THROWS();
	}
	...
	EG(fake_scope) = ce;
	prop = crex_std_get_static_property(ce, name, BP_VAR_IS);
...
CREX_METHOD(ReflectionClass, setStaticPropertyValue)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "Sz", &name, &value) == FAILURE) {
		RETURN_THROWS();
	}
	...
	old_scope = EG(fake_scope);
	EG(fake_scope) = ce;
	variable_ptr =  crex_std_get_static_property_with_info(ce, name, BP_VAR_W, &prop_info);
	EG(fake_scope) = old_scope;
	if (!variable_ptr) {
	...
	if (C_ISREF_P(variable_ptr)) {
		crex_reference *ref = C_REF_P(variable_ptr);
		variable_ptr = C_REFVAL_P(variable_ptr);

		if (!crex_verify_ref_assignable_zval(ref, value, 0)) {
			return;
		}
	}

	if (CREX_TYPE_IS_SET(prop_info->type) && !crex_verify_property_type(prop_info, value, 0)) {
		return;
	}
...
CREX_METHOD(ReflectionClass, isInternal)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	RETURN_BOOL(ce->type == CREX_INTERNAL_CLASS);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, isUserDefined)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	RETURN_BOOL(ce->type == CREX_USER_CLASS);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, isAnonymous)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	RETURN_BOOL(ce->ce_flags & CREX_ACC_ANON_CLASS);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getAttributes)
{
	...
	reflect_attributes(INTERNAL_FUNCTION_PARAM_PASSTHRU,
		ce->attributes, 0, ce, CREX_ATTRIBUTE_TARGET_CLASS,
		ce->type == CREX_USER_CLASS ? ce->info.user.filename : NULL);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, hasMethod)
{
	...
	RETVAL_BOOL(crex_hash_exists(&ce->function_table, lc_name) || is_closure_invoke(ce, lc_name));
...
CREX_METHOD(ReflectionClass, getMethod)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	lc_name = crex_string_tolower(name);
	if (!C_ISUNDEF(intern->obj) && is_closure_invoke(ce, lc_name)
		&& (mptr = crex_get_closure_invoke_method(C_OBJ(intern->obj))) != NULL)
	{
		/* don't assign closure_object since we only reflect the invoke handler
		   method and not the closure definition itself */
		reflection_method_factory(ce, mptr, NULL, return_value);
	} else if (C_ISUNDEF(intern->obj) && is_closure_invoke(ce, lc_name)
		&& object_init_ex(&obj_tmp, ce) == SUCCESS && (mptr = crex_get_closure_invoke_method(C_OBJ(obj_tmp))) != NULL) {
		/* don't assign closure_object since we only reflect the invoke handler
		   method and not the closure definition itself */
		reflection_method_factory(ce, mptr, NULL, return_value);
		zval_ptr_dtor(&obj_tmp);
	} else if ((mptr = crex_hash_find_ptr(&ce->function_table, lc_name)) != NULL) {
		reflection_method_factory(ce, mptr, NULL, return_value);
	} else {
		crex_throw_exception_ex(reflection_exception_ptr, 0,
				"Method %s::%s() does not exist", ZSTR_VAL(ce->name), ZSTR_VAL(name));
...
/* {{{ _addmethod */
static bool _addmethod(crex_function *mptr, crex_class_entry *ce, HashTable *ht, crex_long filter)
{
	if ((mptr->common.fn_flags & CREX_ACC_PRIVATE) && mptr->common.scope != ce) {
		return 0;
	}
	...
	if (mptr->common.fn_flags & filter) {
		...
		crex_hash_next_index_insert_new(ht, &method);
		return 1;
	}
	return 0;
}
...
CREX_METHOD(ReflectionClass, getMethods)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|l!", &filter, &filter_is_null) == FAILURE) {
		RETURN_THROWS();
	}

	if (filter_is_null) {
		filter = CREX_ACC_PPP_MASK | CREX_ACC_ABSTRACT | CREX_ACC_FINAL | CREX_ACC_STATIC;
	}
	...
	array_init(return_value);
	CREX_HASH_MAP_FOREACH_PTR(&ce->function_table, mptr) {
		_addmethod(mptr, ce, C_ARRVAL_P(return_value), filter);
	} CREX_HASH_FOREACH_END();

	if (instanceof_function(ce, crex_ce_closure)) {
		bool has_obj = C_TYPE(intern->obj) != IS_UNDEF;
		zval obj_tmp;
		crex_object *obj;
		if (!has_obj) {
			...
			obj = C_OBJ(intern->obj);
		}
		crex_function *closure = crex_get_closure_invoke_method(obj);
		if (closure) {
			if (!_addmethod(closure, ce, C_ARRVAL_P(return_value), filter)) {
				_free_function(closure);
			}
		}
		if (!has_obj) {
...
CREX_METHOD(ReflectionClass, hasProperty)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	if ((property_info = crex_hash_find_ptr(&ce->properties_info, name)) != NULL) {
		if ((property_info->flags & CREX_ACC_PRIVATE) && property_info->ce != ce) {
			RETURN_FALSE;
		}
		RETURN_TRUE;
	} else {
		if (C_TYPE(intern->obj) != IS_UNDEF) {
			if (C_OBJ_HANDLER(intern->obj, has_property)(C_OBJ(intern->obj), name, 2, NULL)) {
				RETURN_TRUE;
			}
		}
		RETURN_FALSE;
...
{
	reflection_object *intern;
	crex_class_entry *ce, *ce2;
	crex_property_info *property_info;
	crex_string *name, *classname;
	char *tmp, *str_name;
	...
	} else if (C_TYPE(intern->obj) != IS_UNDEF) {
		/* Check for dynamic properties */
		if (crex_hash_exists(C_OBJ_HT(intern->obj)->get_properties(C_OBJ(intern->obj)), name)) {
	...
	if ((tmp = strstr(ZSTR_VAL(name), "::")) != NULL) {
		...
		classname = crex_string_alloc(classname_len, 0);
		crex_str_tolower_copy(ZSTR_VAL(classname), ZSTR_VAL(name), classname_len);
		ZSTR_VAL(classname)[classname_len] = '\0';
		str_name_len = ZSTR_LEN(name) - (classname_len + 2);
		str_name = tmp + 2;

		ce2 = crex_lookup_class(classname);
		if (!ce2) {
			if (!EG(exception)) {
				crex_throw_exception_ex(reflection_exception_ptr, -1, "Class \"%s\" does not exist", ZSTR_VAL(classname));
			}
		...
		if (!instanceof_function(ce, ce2)) {
			crex_throw_exception_ex(reflection_exception_ptr, -1, "Fully qualified property name %s::$%s does not specify a base class of %s", ZSTR_VAL(ce2->name), str_name, ZSTR_VAL(ce->name));
			RETURN_THROWS();
		}
		ce = ce2;

		property_info = crex_hash_str_find_ptr(&ce->properties_info, str_name, str_name_len);
	...
	crex_throw_exception_ex(reflection_exception_ptr, 0, "Property %s::$%s does not exist", ZSTR_VAL(ce->name), str_name);
}
/* }}} */

/* {{{ _addproperty */
static void _addproperty(crex_property_info *pptr, crex_string *key, crex_class_entry *ce, HashTable *ht, long filter)
{
	if ((pptr->flags & CREX_ACC_PRIVATE) && pptr->ce != ce) {
		return;
	}
	...
	if (pptr->flags	& filter) {
		...
		crex_hash_next_index_insert_new(ht, &property);
	}
}
/* }}} */

/* {{{ _adddynproperty */
static void _adddynproperty(zval *ptr, crex_string *key, crex_class_entry *ce, zval *retval)
{
	zval property;
...
CREX_METHOD(ReflectionClass, getProperties)
{
	...
	if (filter_is_null) {
		filter = CREX_ACC_PPP_MASK | CREX_ACC_STATIC;
	}
	...
	array_init(return_value);
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(&ce->properties_info, key, prop_info) {
		_addproperty(prop_info, key, ce, C_ARRVAL_P(return_value), filter);
	} CREX_HASH_FOREACH_END();

	if (C_TYPE(intern->obj) != IS_UNDEF && (filter & CREX_ACC_PUBLIC) != 0) {
		HashTable *properties = C_OBJ_HT(intern->obj)->get_properties(C_OBJ(intern->obj));
		zval *prop;
		CREX_HASH_FOREACH_STR_KEY_VAL(properties, key, prop) {
...
CREX_METHOD(ReflectionClass, hasConstant)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	if (crex_hash_exists(&ce->constants_table, name)) {
		RETURN_TRUE;
	} else {
		RETURN_FALSE;
	}
...
CREX_METHOD(ReflectionClass, getConstants)
{
	...
	if (filter_is_null) {
		filter = CREX_ACC_PPP_MASK;
	}

	GET_REFLECTION_OBJECT_PTR(ce);

	array_init(return_value);
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(CE_CONSTANTS_TABLE(ce), key, constant) {
		if (UNEXPECTED(C_TYPE(constant->value) == IS_CONSTANT_AST && crex_update_class_constant(constant, key, constant->ce) != SUCCESS)) {
			RETURN_THROWS();
		}

		if (CREX_CLASS_CONST_FLAGS(constant) & filter) {
...
CREX_METHOD(ReflectionClass, getReflectionConstants)
{
	...
	array_init(return_value);
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(CE_CONSTANTS_TABLE(ce), name, constant) {
		if (CREX_CLASS_CONST_FLAGS(constant) & filter) {
			zval class_const;
			reflection_class_constant_factory(name, constant, &class_const);
			crex_hash_next_index_insert_new(C_ARRVAL_P(return_value), &class_const);
		}
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getConstant)
{
	...
	CREX_HASH_MAP_FOREACH_STR_KEY_PTR(constants_table, key, c) {
		if (UNEXPECTED(C_TYPE(c->value) == IS_CONSTANT_AST && crex_update_class_constant(c, key, c->ce) != SUCCESS)) {
			RETURN_THROWS();
		}
	} CREX_HASH_FOREACH_END();
	if ((c = crex_hash_find_ptr(constants_table, name)) == NULL) {
		RETURN_FALSE;
	}
...
CREX_METHOD(ReflectionClass, getReflectionConstant)
{
	...
	if ((constant = crex_hash_find_ptr(CE_CONSTANTS_TABLE(ce), name)) == NULL) {
		RETURN_FALSE;
	}
...
CREX_METHOD(ReflectionClass, isInstantiable)
{
	...
	GET_REFLECTION_OBJECT_PTR(ce);
	if (ce->ce_flags & (CREX_ACC_INTERFACE | CREX_ACC_TRAIT | CREX_ACC_EXPLICIT_ABSTRACT_CLASS | CREX_ACC_IMPLICIT_ABSTRACT_CLASS | CREX_ACC_ENUM)) {
		RETURN_FALSE;
	}
	...
	RETURN_BOOL(ce->constructor->common.fn_flags & CREX_ACC_PUBLIC);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, isCloneable)
{
	...
	if (!C_ISUNDEF(intern->obj)) {
		if (ce->clone) {
			RETURN_BOOL(ce->clone->common.fn_flags & CREX_ACC_PUBLIC);
		} else {
			RETURN_BOOL(C_OBJ_HANDLER(intern->obj, clone_obj) != NULL);
		}
	} else {
		if (ce->clone) {
			RETURN_BOOL(ce->clone->common.fn_flags & CREX_ACC_PUBLIC);
			...
			crex_object_store_ctor_failed(C_OBJ(obj));
			RETVAL_BOOL(C_OBJ_HANDLER(obj, clone_obj) != NULL);
			zval_ptr_dtor(&obj);
		}
	}
}
...
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_INTERFACE);
}
/* }}} */
...
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_TRAIT);
}
/* }}} */

CREX_METHOD(ReflectionClass, isEnum)
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_ENUM);
}

/* {{{ Returns whether this class is final */
CREX_METHOD(ReflectionClass, isFinal)
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_FINAL);
}
/* }}} */
...
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_READONLY_CLASS);
}

/* {{{ Returns whether this class is abstract */
CREX_METHOD(ReflectionClass, isAbstract)
{
	_class_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_IMPLICIT_ABSTRACT_CLASS|CREX_ACC_EXPLICIT_ABSTRACT_CLASS);
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getModifiers)
{
	reflection_object *intern;
	crex_class_entry *ce;
	uint32_t keep_flags = CREX_ACC_FINAL | CREX_ACC_EXPLICIT_ABSTRACT_CLASS | CREX_ACC_READONLY_CLASS;
...
CREX_METHOD(ReflectionClass, isInstance)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "o", &object) == FAILURE) {
		RETURN_THROWS();
	}
	GET_REFLECTION_OBJECT_PTR(ce);
	RETURN_BOOL(instanceof_function(C_OBJCE_P(object), ce));
}
/* }}} */
...
CREX_METHOD(ReflectionClass, newInstance)
{
	...
	old_scope = EG(fake_scope);
	EG(fake_scope) = ce;
	constructor = C_OBJ_HT_P(return_value)->get_constructor(C_OBJ_P(return_value));
	EG(fake_scope) = old_scope;
	...
	if (constructor) {
		...
		if (!(constructor->common.fn_flags & CREX_ACC_PUBLIC)) {
			crex_throw_exception_ex(reflection_exception_ptr, 0, "Access to non-public constructor of class %s", ZSTR_VAL(ce->name));
		...
		crex_call_known_function(
			constructor, C_OBJ_P(return_value), C_OBJCE_P(return_value), NULL,
			num_args, params, named_params);

		if (EG(exception)) {
			crex_object_store_ctor_failed(C_OBJ_P(return_value));
		}
	} else if (CREX_NUM_ARGS()) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Class %s does not have a constructor, so you cannot pass any constructor arguments", ZSTR_VAL(ce->name));
	}
}
/* }}} */
...
CREX_METHOD(ReflectionClass, newInstanceWithoutConstructor)
{
	...
	if (ce->type == CREX_INTERNAL_CLASS
			&& ce->create_object != NULL && (ce->ce_flags & CREX_ACC_FINAL)) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Class %s is an internal class marked as final that cannot be instantiated without invoking its constructor", ZSTR_VAL(ce->name));
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionClass, newInstanceArgs)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "|h", &args) == FAILURE) {
		RETURN_THROWS();
	}

	if (args) {
		argc = crex_hash_num_elements(args);
	}
	...
	if (constructor) {
		...
		crex_call_known_function(
			constructor, C_OBJ_P(return_value), C_OBJCE_P(return_value), NULL, 0, NULL, args);
...
CREX_METHOD(ReflectionClass, getInterfaces)
{
	...
	if (ce->num_interfaces) {
		...
		for (i=0; i < ce->num_interfaces; i++) {
			zval interface;
			crex_reflection_class_factory(ce->interfaces[i], &interface);
			crex_hash_update(C_ARRVAL_P(return_value), ce->interfaces[i]->name, &interface);
		}
	} else {
		RETURN_EMPTY_ARRAY();
	}
...
CREX_METHOD(ReflectionClass, getInterfaceNames)
{
	...
	for (i=0; i < ce->num_interfaces; i++) {
		add_next_index_str(return_value, crex_string_copy(ce->interfaces[i]->name));
	}
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getTraits)
{
	...
	for (i=0; i < ce->num_traits; i++) {
		zval trait;
		crex_class_entry *trait_ce;

		trait_ce = crex_fetch_class_by_name(ce->trait_names[i].name,
			ce->trait_names[i].lc_name, CREX_FETCH_CLASS_TRAIT);
		CREX_ASSERT(trait_ce);
		crex_reflection_class_factory(trait_ce, &trait);
		crex_hash_update(C_ARRVAL_P(return_value), ce->trait_names[i].name, &trait);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getTraitNames)
{
	...
	for (i=0; i < ce->num_traits; i++) {
		add_next_index_str(return_value, crex_string_copy(ce->trait_names[i].name));
	}
}
/* }}} */
...
CREX_METHOD(ReflectionClass, getTraitAliases)
{
	...
	if (ce->trait_aliases) {
		...
		while (ce->trait_aliases[i]) {
			crex_string *mname;
			crex_trait_method_reference *cur_ref = &ce->trait_aliases[i]->trait_method;

			if (ce->trait_aliases[i]->alias) {
				crex_string *class_name = cur_ref->class_name;

				if (!class_name) {
					uint32_t j = 0;
					crex_string *lcname = crex_string_tolower(cur_ref->method_name);

					for (j = 0; j < ce->num_traits; j++) {
						crex_class_entry *trait =
							crex_hash_find_ptr(CG(class_table), ce->trait_names[j].lc_name);
						CREX_ASSERT(trait && "Trait must exist");
						if (crex_hash_exists(&trait->function_table, lcname)) {
							class_name = trait->name;
							break;
						}
					}
					crex_string_release_ex(lcname, 0);
					CREX_ASSERT(class_name != NULL);
				}

				mname = crex_string_alloc(ZSTR_LEN(class_name) + ZSTR_LEN(cur_ref->method_name) + 2, 0);
...
CREX_METHOD(ReflectionClass, getParentClass)
{
	...
	if (ce->parent) {
		crex_reflection_class_factory(ce->parent, return_value);
	} else {
		RETURN_FALSE;
	}
}
...
CREX_METHOD(ReflectionClass, isSubclassOf)
{
	...
	crex_class_entry *ce, *class_ce;
	crex_string *class_str;
	crex_object *class_obj;

	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_OBJ_OF_CLASS_OR_STR(class_obj, reflection_class_ptr, class_str)
	CREX_PARSE_PARAMETERS_END();

	if (class_obj) {
		argument = reflection_object_from_obj(class_obj);
		if (argument->ptr == NULL) {
			crex_throw_error(NULL, "Internal error: Failed to retrieve the argument's reflection object");
			RETURN_THROWS();
		}

		class_ce = argument->ptr;
	} else {
		if ((class_ce = crex_lookup_class(class_str)) == NULL) {
			crex_throw_exception_ex(reflection_exception_ptr, 0, "Class \"%s\" does not exist", ZSTR_VAL(class_str));
			RETURN_THROWS();
		}
	}
...
{
	reflection_object *intern, *argument;
	crex_string *interface_str;
	crex_class_entry *ce, *interface_ce;
	crex_object *interface_obj;

	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_OBJ_OF_CLASS_OR_STR(interface_obj, reflection_class_ptr, interface_str)
	CREX_PARSE_PARAMETERS_END();
	...
	if (interface_obj) {
		...
		if ((interface_ce = crex_lookup_class(interface_str)) == NULL) {
			crex_throw_exception_ex(reflection_exception_ptr, 0, "Interface \"%s\" does not exist", ZSTR_VAL(interface_str));
			RETURN_THROWS();
		}
	}

	if (!(interface_ce->ce_flags & CREX_ACC_INTERFACE)) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "%s is not an interface", ZSTR_VAL(interface_ce->name));
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionClass, isIterable)
{
	...
	if (ce->ce_flags & (CREX_ACC_INTERFACE | CREX_ACC_IMPLICIT_ABSTRACT_CLASS |
	                    CREX_ACC_TRAIT     | CREX_ACC_EXPLICIT_ABSTRACT_CLASS)) {
		RETURN_FALSE;
	}

	RETURN_BOOL(ce->get_iterator || instanceof_function(ce, crex_ce_traversable));
}
/* }}} */
...
CREX_METHOD(ReflectionProperty, __main)
{
	...
	reflection_object *intern;
	crex_class_entry *ce;
	crex_property_info *property_info = NULL;
	property_reference *reference;
	...
	property_info = crex_hash_find_ptr(&ce->properties_info, name);
	if (property_info == NULL
	 || ((property_info->flags & CREX_ACC_PRIVATE)
	  && property_info->ce != ce)) {
		/* Check for dynamic properties */
		if (property_info == NULL && classname_obj) {
			if (crex_hash_exists(classname_obj->handlers->get_properties(classname_obj), name)) {
				dynam_prop = 1;
			}
		}
		if (dynam_prop == 0) {
			crex_throw_exception_ex(reflection_exception_ptr, 0, "Property %s::$%s does not exist", ZSTR_VAL(ce->name), ZSTR_VAL(name));
			RETURN_THROWS();
		}
	}
...
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PUBLIC);
}
/* }}} */
...
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PRIVATE);
}
/* }}} */
...
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PROTECTED);
}
/* }}} */
...
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_STATIC);
}
/* }}} */

CREX_METHOD(ReflectionProperty, isReadOnly)
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_READONLY);
}
...
{
	_property_check_flag(INTERNAL_FUNCTION_PARAM_PASSTHRU, CREX_ACC_PROMOTED);
}
/* }}} */
...
CREX_METHOD(ReflectionProperty, getModifiers)
{
	reflection_object *intern;
	property_reference *ref;
	uint32_t keep_flags = CREX_ACC_PPP_MASK | CREX_ACC_STATIC | CREX_ACC_READONLY;
...
CREX_METHOD(ReflectionProperty, getValue)
{
	...
	if (prop_get_flags(ref) & CREX_ACC_STATIC) {
		member_p = crex_read_static_property_ex(intern->ce, ref->unmangled_name, 0);
		if (member_p) {
		...
		if (!object) {
			crex_argument_type_error(1, "must be provided for instance properties");
			RETURN_THROWS();
		}

		/* TODO: Should this always use intern->ce? */
		if (!instanceof_function(C_OBJCE_P(object), ref->prop ? ref->prop->ce : intern->ce)) {
			_DO_THROW("Given object is not an instance of the class this property was declared in");
			RETURN_THROWS();
		}

		member_p = crex_read_property_ex(intern->ce, C_OBJ_P(object), ref->unmangled_name, 0, &rv);
		if (member_p != &rv) {
			RETURN_COPY_DEREF(member_p);
		} else {
			if (C_ISREF_P(member_p)) {
				crex_unwrap_reference(member_p);
			}
...
CREX_METHOD(ReflectionProperty, setValue)
{
	...
	if (prop_get_flags(ref) & CREX_ACC_STATIC) {
		if (crex_parse_parameters_ex(CREX_PARSE_PARAMS_QUIET, CREX_NUM_ARGS(), "z", &value) == FAILURE) {
			if (crex_parse_parameters(CREX_NUM_ARGS(), "zz", &tmp, &value) == FAILURE) {
				RETURN_THROWS();
			}

			if (C_TYPE_P(tmp) != IS_NULL && C_TYPE_P(tmp) != IS_OBJECT) {
				crex_string *method_name = get_active_function_or_method_name();
				crex_error(E_DEPRECATED, "Calling %s() with a 1st argument which is not null or an object is deprecated", ZSTR_VAL(method_name));
			...
			crex_string *method_name = get_active_function_or_method_name();
			crex_error(E_DEPRECATED, "Calling %s() with a single argument is deprecated", ZSTR_VAL(method_name));
			crex_string_release(method_name);
			if (UNEXPECTED(EG(exception))) {
				RETURN_THROWS();
			}
		...
		crex_update_static_property_ex(intern->ce, ref->unmangled_name, value);
	} else {
		if (crex_parse_parameters(CREX_NUM_ARGS(), "oz", &object, &value) == FAILURE) {
			RETURN_THROWS();
		}

		crex_update_property_ex(intern->ce, C_OBJ_P(object), ref->unmangled_name, value);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionProperty, isInitialized)
{
	...
	if (prop_get_flags(ref) & CREX_ACC_STATIC) {
		member_p = crex_read_static_property_ex(intern->ce, ref->unmangled_name, 1);
		if (member_p) {
			RETURN_BOOL(!C_ISUNDEF_P(member_p));
		}
		RETURN_FALSE;
	} else {
		crex_class_entry *old_scope;
		int retval;
		...
		retval = C_OBJ_HT_P(object)->has_property(C_OBJ_P(object), ref->unmangled_name, CREX_PROPERTY_EXISTS, NULL);
		EG(fake_scope) = old_scope;
...
CREX_METHOD(ReflectionProperty, getAttributes)
{
	...
	reflect_attributes(INTERNAL_FUNCTION_PARAM_PASSTHRU,
		ref->prop->attributes, 0, ref->prop->ce, CREX_ATTRIBUTE_TARGET_PROPERTY,
		ref->prop->ce->type == CREX_USER_CLASS ? ref->prop->ce->info.user.filename : NULL);
}
/* }}} */
...
CREX_METHOD(ReflectionProperty, getType)
{
	...
	if (!ref->prop || !CREX_TYPE_IS_SET(ref->prop->type)) {
		RETURN_NULL();
	}
...
CREX_METHOD(ReflectionProperty, hasType)
{
	...
	RETVAL_BOOL(ref->prop && CREX_TYPE_IS_SET(ref->prop->type));
}
/* }}} */
...
CREX_METHOD(ReflectionExtension, __main)
{
	...
	if (crex_parse_parameters(CREX_NUM_ARGS(), "s", &name_str, &name_len) == FAILURE) {
		RETURN_THROWS();
	}
	...
	crex_str_tolower_copy(lcname, name_str, name_len);
	if ((module = crex_hash_str_find_ptr(&module_registry, lcname, name_len)) == NULL) {
...
CREX_METHOD(ReflectionExtension, getFunctions)
{
	...
	CREX_HASH_MAP_FOREACH_PTR(CG(function_table), fptr) {
		if (fptr->common.type==CREX_INTERNAL_FUNCTION
			...
			crex_hash_update(C_ARRVAL_P(return_value), fptr->common.function_name, &function);
		}
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
CREX_METHOD(ReflectionExtension, getConstants)
{
	...
	array_init(return_value);
	CREX_HASH_MAP_FOREACH_PTR(EG(crex_constants), constant) {
		if (module->module_number == CREX_CONSTANT_MODULE_NUMBER(constant)) {
			zval const_val;
			ZVAL_COPY_OR_DUP(&const_val, &constant->value);
			crex_hash_update(C_ARRVAL_P(return_value), constant->name, &const_val);
		}
	} CREX_HASH_FOREACH_END();
}
/* }}} */
...
/* {{{ _addinientry */
static void _addinientry(crex_ini_entry *ini_entry, zval *retval, int number)
{
	if (number == ini_entry->module_number) {
		zval zv;
		if (ini_entry->value) {
		...
		crex_symtable_update(C_ARRVAL_P(retval), ini_entry->name, &zv);
	}
}
/* }}} */
...
/* {{{ add_extension_class */
static void add_extension_class(crex_class_entry *ce, crex_string *key, zval *class_array, crex_module_entry *module, bool add_reflection_class)
{
	if (ce->type == CREX_INTERNAL_CLASS && ce->info.internal.module && !strcasecmp(ce->info.internal.module->name, module->name)) {
		crex_string *name;
		...
		if (add_reflection_class) {
			zval zclass;
			crex_reflection_class_factory(ce, &zclass);
			crex_hash_update(C_ARRVAL_P(class_array), name, &zclass);
		} else {
			add_next_index_str(class_array, crex_string_copy(name));
		}
	}
}
/* }}} */
...
CREX_METHOD(ReflectionExtension, getDependencies)
{
	...
	while(dep->name) {
		...
		relation = crex_string_alloc(len, 0);
...
/* {{{ Prints crxinfo block for the extension */
...
/* {{{ Constructor. Throws an Exception in case the given Crex extension does not exist */
CREX_METHOD(ReflectionCrexExtension, __main)
{
	...
	extension = crex_get_extension(name_str);
	if (!extension) {
		crex_throw_exception_ex(reflection_exception_ptr, 0,
				"Crex Extension \"%s\" does not exist", name_str);
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionCrexExtension, __toString)
{
	...
	_crex_extension_string(&str, extension, "");
	RETURN_STR(smart_str_extract(&str));
}
/* }}} */

/* {{{ Returns the name of this Crex extension */
...
/* {{{ Returns the version information of this Crex extension */
...
/* {{{ Returns the name of this Crex extension's author */
...
/* {{{ Returns this Crex extension's URL*/
...
/* {{{ Returns this Crex extension's copyright information */
...
static bool is_ignorable_reference(HashTable *ht, zval *ref) {
	if (C_REFCOUNT_P(ref) != 1) {
		return 0;
	}

	/* Directly self-referential arrays are treated as proper references
	 * in crex_array_dup() despite rc=1. */
	return C_TYPE_P(C_REFVAL_P(ref)) != IS_ARRAY || C_ARRVAL_P(C_REFVAL_P(ref)) != ht;
}
...
CREX_METHOD(ReflectionReference, fromArrayElement)
{
	...
	CREX_PARSE_PARAMETERS_START(2, 2)
		C_PARAM_ARRAY_HT(ht)
		C_PARAM_STR_OR_LONG(string_key, int_key)
	CREX_PARSE_PARAMETERS_END();

	if (string_key) {
		item = crex_hash_find(ht, string_key);
	} else {
		item = crex_hash_index_find(ht, int_key);
	}
	...
	if (C_TYPE_P(item) != IS_REFERENCE || is_ignorable_reference(ht, item)) {
		RETURN_NULL();
	}

	object_init_ex(return_value, reflection_reference_ptr);
	intern = C_REFLECTION_P(return_value);
...
CREX_METHOD(ReflectionReference, getId)
{
	...
	intern = C_REFLECTION_P(getThis());
	if (C_TYPE(intern->obj) != IS_REFERENCE) {
		_DO_THROW("Corrupted ReflectionReference object");
		RETURN_THROWS();
	}

	if (!REFLECTION_G(key_initialized)) {
		if (crx_random_bytes_throw(&REFLECTION_G(key), 16) == FAILURE) {
			RETURN_THROWS();
		}
	...
	CRX_SHA1Update(&context, (unsigned char *) &C_REF(intern->obj), sizeof(crex_reference *));
	CRX_SHA1Update(&context, REFLECTION_G(key), REFLECTION_KEY_LEN);
...
CREX_METHOD(ReflectionAttribute, isRepeated)
{
	...
	RETURN_BOOL(crex_is_attribute_repeated(attr->attributes, attr->data));
}
/* }}} */
...
CREX_METHOD(ReflectionAttribute, getArguments)
{
	...
	for (i = 0; i < attr->data->argc; i++) {
		if (FAILURE == crex_get_attribute_value(&tmp, attr->data, i, attr->scope)) {
			RETURN_THROWS();
		}

		if (attr->data->args[i].name) {
			/* We ensured at compile-time that there are no duplicate parameter names. */
			crex_hash_add_new(C_ARRVAL_P(return_value), attr->data->args[i].name, &tmp);
		} else {
...
static int call_attribute_constructor(
	crex_attribute *attr, crex_class_entry *ce, crex_object *obj,
	zval *args, uint32_t argc, HashTable *named_params, crex_string *filename)
{
	crex_function *ctor = ce->constructor;
	crex_execute_data *call = NULL;
	CREX_ASSERT(ctor != NULL);

	if (!(ctor->common.fn_flags & CREX_ACC_PUBLIC)) {
		crex_throw_error(NULL, "Attribute constructor of class %s must be public", ZSTR_VAL(ce->name));
		return FAILURE;
	}
	...
	if (filename) {
		...
		crex_function dummy_func;
		crex_op *opline;

		memset(&dummy_func, 0, sizeof(crex_function));

		call = crex_vm_stack_push_call_frame_ex(
			CREX_MM_ALIGNED_SIZE_EX(sizeof(crex_execute_data), sizeof(zval)) +
			CREX_MM_ALIGNED_SIZE_EX(sizeof(crex_op), sizeof(zval)) +
			CREX_MM_ALIGNED_SIZE_EX(sizeof(crex_function), sizeof(zval)),
			0, &dummy_func, 0, NULL);

		opline = (crex_op*)(call + 1);
		memset(opline, 0, sizeof(crex_op));
		...
		call->func = (crex_function*)(call->opline + 1);
		call->prev_execute_data = EG(current_execute_data);

		memset(call->func, 0, sizeof(crex_function));
		call->func->type = CREX_USER_FUNCTION;
		call->func->op_array.fn_flags =
			attr->flags & CREX_ATTRIBUTE_STRICT_TYPES ? CREX_ACC_STRICT_TYPES : 0;
		call->func->op_array.fn_flags |= CREX_ACC_CALL_VIA_TRAMPOLINE;
		call->func->op_array.filename = filename;

		EG(current_execute_data) = call;
	}

	crex_call_known_function(ctor, obj, obj->ce, NULL, argc, args, named_params);
		...
		zval *obj, zval *args, uint32_t argc, HashTable *named_params) /* {{{ */
{
	...
	if (named_params) {
		crex_array_destroy(named_params);
	}
}
/* }}} */
...
CREX_METHOD(ReflectionAttribute, newInstance)
{
	...
	if (NULL == (ce = crex_lookup_class(attr->data->name))) {
		crex_throw_error(NULL, "Attribute class \"%s\" not found", ZSTR_VAL(attr->data->name));
		RETURN_THROWS();
	}

	if (NULL == (marker = crex_get_attribute_str(ce->attributes, CREX_STRL("attribute")))) {
		crex_throw_error(NULL, "Attempting to use non-attribute class \"%s\" as attribute", ZSTR_VAL(attr->data->name));
		RETURN_THROWS();
	}

	if (ce->type == CREX_USER_CLASS) {
		uint32_t flags = CREX_ATTRIBUTE_TARGET_ALL;

		if (marker->argc > 0) {
			zval tmp;

			if (FAILURE == crex_get_attribute_value(&tmp, marker, 0, ce)) {
				RETURN_THROWS();
			}

			flags = (uint32_t) C_LVAL(tmp);
		}

		if (!(attr->target & flags)) {
			crex_string *location = crex_get_attribute_target_names(attr->target);
			crex_string *allowed = crex_get_attribute_target_names(flags);

			crex_throw_error(NULL, "Attribute \"%s\" cannot target %s (allowed targets: %s)",
				ZSTR_VAL(attr->data->name), ZSTR_VAL(location), ZSTR_VAL(allowed)
			);

			crex_string_release(location);
			crex_string_release(allowed);

			RETURN_THROWS();
		}

		if (!(flags & CREX_ATTRIBUTE_IS_REPEATABLE)) {
			if (crex_is_attribute_repeated(attr->attributes, attr->data)) {
				crex_throw_error(NULL, "Attribute \"%s\" must not be repeated", ZSTR_VAL(attr->data->name));
				RETURN_THROWS();
			}
		}
	}
	...
	if (attr->data->argc) {
		...
		for (uint32_t i = 0; i < attr->data->argc; i++) {
			zval val;
			if (FAILURE == crex_get_attribute_value(&val, attr->data, i, attr->scope)) {
				attribute_ctor_cleanup(&obj, args, argc, named_params);
				RETURN_THROWS();
			}
			if (attr->data->args[i].name) {
				if (!named_params) {
					named_params = crex_new_array(0);
				}
				crex_hash_add_new(named_params, attr->data->args[i].name, &val);
			} else {
	...
	if (ce->constructor) {
		if (FAILURE == call_attribute_constructor(attr->data, ce, C_OBJ(obj), args, argc, named_params, attr->filename)) {
			attribute_ctor_cleanup(&obj, args, argc, named_params);
			RETURN_THROWS();
		}
	} else if (argc || named_params) {
		attribute_ctor_cleanup(&obj, args, argc, named_params);
		crex_throw_error(NULL, "Attribute class %s does not have a constructor, cannot pass arguments", ZSTR_VAL(ce->name));
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionEnum, __main)
{
	...
	if (!(ce->ce_flags & CREX_ACC_ENUM)) {
		crex_throw_exception_ex(reflection_exception_ptr, -1, "Class \"%s\" is not an enum", ZSTR_VAL(ce->name));
		RETURN_THROWS();
	}
}
...
CREX_METHOD(ReflectionEnum, hasCase)
{
	...
	crex_class_constant *class_const = crex_hash_find_ptr(&ce->constants_table, name);
	if (class_const == NULL) {
		RETURN_FALSE;
	}

	RETURN_BOOL(CREX_CLASS_CONST_FLAGS(class_const) & CREX_CLASS_CONST_IS_CASE);
}
...
CREX_METHOD(ReflectionEnum, getCase)
{
	...
	crex_class_constant *constant = crex_hash_find_ptr(CE_CONSTANTS_TABLE(ce), name);
	if (constant == NULL) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Case %s::%s does not exist", ZSTR_VAL(ce->name), ZSTR_VAL(name));
		RETURN_THROWS();
	}
	if (!(CREX_CLASS_CONST_FLAGS(constant) & CREX_CLASS_CONST_IS_CASE)) {
		crex_throw_exception_ex(reflection_exception_ptr, 0, "%s::%s is not a case", ZSTR_VAL(ce->name), ZSTR_VAL(name));
		RETURN_THROWS();
	}
...
CREX_METHOD(ReflectionEnum, getBackingType)
{
	...
	if (ce->enum_backing_type == IS_UNDEF) {
		...
		crex_type type = CREX_TYPE_INIT_CODE(ce->enum_backing_type, 0, 0);
...
{
	CREX_MN(ReflectionClassConstant___main)(INTERNAL_FUNCTION_PARAM_PASSTHRU);
	if (EG(exception)) {
		RETURN_THROWS();
	}
	...
	if (!(CREX_CLASS_CONST_FLAGS(ref) & CREX_CLASS_CONST_IS_CASE)) {
		zval *case_name = reflection_prop_name(CREX_THIS);
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Constant %s::%s is not a case", ZSTR_VAL(ref->ce->name), C_STRVAL_P(case_name));
		RETURN_THROWS();
	}
}
...
{
	CREX_MN(ReflectionEnumUnitCase___main)(INTERNAL_FUNCTION_PARAM_PASSTHRU);
	if (EG(exception)) {
		RETURN_THROWS();
	}
	...
	if (ref->ce->enum_backing_type == IS_UNDEF) {
		zval *case_name = reflection_prop_name(CREX_THIS);
		crex_throw_exception_ex(reflection_exception_ptr, 0, "Enum case %s::%s is not a backed case", ZSTR_VAL(ref->ce->name), C_STRVAL_P(case_name));
		RETURN_THROWS();
	}
}
...
CREX_METHOD(ReflectionEnumBackedCase, getBackingValue)
{
	...
	CREX_ASSERT(intern->ce->enum_backing_type != IS_UNDEF);
	zval *member_p = crex_enum_fetch_case_value(C_OBJ(ref->value));

	ZVAL_COPY_OR_DUP(return_value, member_p);
}

/* {{{ proto ReflectionFiber::__main(Fiber $fiber) */
CREX_METHOD(ReflectionFiber, __main)
{
	...
	CREX_PARSE_PARAMETERS_START(1, 1)
		C_PARAM_OBJECT_OF_CLASS(fiber, crex_ce_fiber)
	CREX_PARSE_PARAMETERS_END();
	...
	ZVAL_OBJ_COPY(&intern->obj, C_OBJ_P(fiber));
	intern->ce = crex_ce_fiber;
}
/* }}} */
...
CREX_METHOD(ReflectionFiber, getFiber)
{
	...
	RETURN_OBJ_COPY(C_OBJ(C_REFLECTION_P(CREX_THIS)->obj));
}

#define REFLECTION_CHECK_VALID_FIBER(fiber) do { \
		if (fiber == NULL || fiber->context.status == CREX_FIBER_STATUS_INIT || fiber->context.status == CREX_FIBER_STATUS_DEAD) { \
			crex_throw_error(NULL, "Cannot fetch information from a fiber that has not been started or is terminated"); \
			RETURN_THROWS(); \
		} \
	} while (0)
...
{
	crex_fiber *fiber = (crex_fiber *) C_OBJ(C_REFLECTION_P(CREX_THIS)->obj);
	crex_long options = DEBUG_BACKTRACE_PROVIDE_OBJECT;
...
CREX_METHOD(ReflectionFiber, getExecutingLine)
{
	...
	while (prev_execute_data && (!prev_execute_data->func || !CREX_USER_CODE(prev_execute_data->func->common.type))) {
		prev_execute_data = prev_execute_data->prev_execute_data;
	}
	if (prev_execute_data && prev_execute_data->func && CREX_USER_CODE(prev_execute_data->func->common.type)) {
...
CREX_METHOD(ReflectionFiber, getCallable)
{
	...
	if (fiber == NULL || fiber->context.status == CREX_FIBER_STATUS_DEAD) {
		crex_throw_error(NULL, "Cannot fetch the callable from a fiber that has terminated"); \
		RETURN_THROWS();
	}
...
static zval *_reflection_write_property(crex_object *object, crex_string *name, zval *value, void **cache_slot)
{
	if (crex_hash_exists(&object->ce->properties_info, name)
		&& (crex_string_equals(name, ZSTR_KNOWN(CREX_STR_NAME)) || crex_string_equals(name, ZSTR_KNOWN(CREX_STR_CLASS))))
	{
		crex_throw_exception_ex(reflection_exception_ptr, 0,
...
CRX_MINIT_FUNCTION(reflection) /* {{{ */
{
	memcpy(&reflection_object_handlers, &std_object_handlers, sizeof(crex_object_handlers));
	...
	reflection_exception_ptr = register_class_ReflectionException(crex_ce_exception);

	reflection_ptr = register_class_Reflection();

	reflector_ptr = register_class_Reflector(crex_ce_stringable);
	...
	reflection_type_ptr = register_class_ReflectionType(crex_ce_stringable);
	...
	reflection_crex_extension_ptr = register_class_ReflectionCrexExtension(reflector_ptr);
	reflection_crex_extension_ptr->create_object = reflection_objects_new;
	reflection_crex_extension_ptr->default_object_handlers = &reflection_object_handlers;
...
CRX_MINFO_FUNCTION(reflection) /* {{{ */
{
	crx_info_print_table_start();
	crx_info_print_table_row(2, "Reflection", "enabled");
	crx_info_print_table_end();
} /* }}} */

crex_module_entry reflection_module_entry = { /* {{{ */
	STANDARD_MODULE_HEADER,
	"Reflection",
	NULL,
	CRX_MINIT(reflection),
	NULL,
	NULL,
	NULL,
	CRX_MINFO(reflection),
	CRX_REFLECTION_VERSION,
	CREX_MODULE_GLOBALS(reflection),
	NULL,
	NULL,
	NULL,
