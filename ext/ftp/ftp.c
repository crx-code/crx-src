...
static int		my_send(ftpbuf_t *ftp, crx_socket_t s, void *buf, size_t len);
static int		my_recv(ftpbuf_t *ftp, crx_socket_t s, void *buf, size_t len);
static int		my_accept(ftpbuf_t *ftp, crx_socket_t s, struct sockaddr *addr, socklen_t *addrlen);
...
static void		ftp_ssl_shutdown(ftpbuf_t *ftp, crx_socket_t fd, SSL *ssl_handle);
#endif
...
/* {{{ ftp_open */
ftpbuf_t*
ftp_open(const char *host, short port, crex_long timeout_sec)
{
	ftpbuf_t		*ftp;
	socklen_t		 size;
	struct timeval tv;
	...
	ftp->fd = crx_network_connect_socket_to_host(host,
	...
	if (getsockname(ftp->fd, (struct sockaddr*) &ftp->localaddr, &size) != 0) {
		crx_error_docref(NULL, E_WARNING, "getsockname failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
...
ftp_login(ftpbuf_t *ftp, const char *user, const size_t user_len, const char *pass, const size_t pass_len)
{
	...
	if (ftp->use_ssl && !ftp->ssl_active) {
		...
		if (ctx == NULL) {
			crx_error_docref(NULL, E_WARNING, "Failed to create the SSL context");
			return 0;
		}
		...
		if (ftp->ssl_handle == NULL) {
			crx_error_docref(NULL, E_WARNING, "Failed to create the SSL handle");
			return 0;
		}
		...
		do {
			...
			switch (err) {
				...
				default:
					crx_error_docref(NULL, E_WARNING, "SSL/TLS handshake failed");
					SSL_shutdown(ftp->ssl_handle);
...
ftp_mkdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len)
{
	...
	if ((mkd = strchr(ftp->inbuf, '"')) == NULL) {
		return crex_string_init(dir, dir_len, 0);
	}
	if ((end = strrchr(++mkd, '"')) == NULL) {
		return NULL;
	}
	*end = 0;
	ret = crex_string_init(mkd, end - mkd, 0);
	*end = '"';
...
/* {{{ ftp_alloc */
int
ftp_alloc(ftpbuf_t *ftp, const crex_long size, crex_string **response)
{
	char buffer[64];
	int buffer_len;
	...
	buffer_len = snprintf(buffer, sizeof(buffer) - 1, CREX_LONG_FMT, size);
	...
	if (response) {
		*response = crex_string_init(ftp->inbuf, strlen(ftp->inbuf), 0);
	}
...
ftp_mlsd_parse_line(HashTable *ht, const char *input) {
	...
	if (!sp) {
		crx_error_docref(NULL, E_WARNING, "Missing pathname in MLSD response");
		return FAILURE;
	}

	/* Extract pathname */
	ZVAL_STRINGL(&zstr, sp + 1, end - sp - 1);
	crex_hash_update(ht, ZSTR_KNOWN(CREX_STR_NAME), &zstr);
	end = sp;
	...
	while (input < end) {
		...
		if (!semi) {
			crx_error_docref(NULL, E_WARNING, "Malformed fact in MLSD response");
			return FAILURE;
		}
		...
		crex_hash_str_update(ht, input, eq - input, &zstr);
		input = semi + 1;
	}
...
/* {{{ ftp_get */
int
ftp_get(ftpbuf_t *ftp, crx_stream *outstream, const char *path, const size_t path_len, ftptype_t type, crex_long resumepos)
{
	databuf_t		*data = NULL;
	size_t			rcvd;
	...
	if (resumepos > 0) {
		int arg_len = snprintf(arg, sizeof(arg), CREX_LONG_FMT, resumepos);
	...
	while ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		if (type == FTPTYPE_ASCII) {
			 ...
			 */
#ifdef CRX_WIN32
			crx_stream_write(outstream, ptr, (e - ptr));
			ptr = e;
#else
			while (e > ptr && (s = memchr(ptr, '\r', (e - ptr)))) {
				crx_stream_write(outstream, ptr, (s - ptr));
				if (*(s + 1) == '\n') {
					s++;
					crx_stream_putc(outstream, '\n');
				}
				ptr = s + 1;
			}
#endif
		...
		} else if (rcvd != crx_stream_write(outstream, data->buf, rcvd)) {
			goto bail;
		}
	}
...
/* {{{ ftp_put */
int
ftp_put(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type, crex_long startpos)
{
	databuf_t		*data = NULL;
	crex_long			size;
	char			*ptr;
	...
	if (startpos > 0) {
		int arg_len = snprintf(arg, sizeof(arg), CREX_LONG_FMT, startpos);
	...
	size = 0;
	ptr = data->buf;
	while (!crx_stream_eof(instream) && (ch = crx_stream_getc(instream))!=EOF) {
		/* flush if necessary */
...
/* {{{ ftp_append */
int
ftp_append(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type)
{
	databuf_t		*data = NULL;
	crex_long			size;
	char			*ptr;
...
int single_send(ftpbuf_t *ftp, crx_socket_t s, void *buf, size_t size) {
#ifdef HAVE_FTP_SSL
	int err;
	bool retry = 0;
	SSL *handle = NULL;
	...
	do {
		...
		switch (err) {
			...
			default:
				crx_error_docref(NULL, E_WARNING, "SSL write failed");
				return -1;
		}
	} while (retry);
	return sent;
...
/* {{{ my_send */
int
my_send(ftpbuf_t *ftp, crx_socket_t s, void *buf, size_t len)
{
	crex_long size, sent;
	int       n;
	...
	size = len;
	while (size) {
		n = crx_pollfd_for_ms(s, POLLOUT, ftp->timeout_sec * 1000);
		...
		if (n < 1) {
			...
			crx_error_docref(NULL, E_WARNING, "%s", crx_socket_strerror(errno, buf, sizeof buf));
			return -1;
		}
...
/* {{{ my_recv */
int
my_recv(ftpbuf_t *ftp, crx_socket_t s, void *buf, size_t len)
{
	int		n, nr_bytes;
#ifdef HAVE_FTP_SSL
	int err;
	...
	crx_socket_t fd;
#endif
	n = crx_pollfd_for_ms(s, CRX_POLLREADABLE, ftp->timeout_sec * 1000);
	if (n < 1) {
		char buf[256];
		if (n == 0) {
#ifdef CRX_WIN32
	...
	if (handle) {
		do {
			...
			switch (err) {
				...
				default:
					crx_error_docref(NULL, E_WARNING, "SSL read failed");
					return -1;
			}
		} while (retry);
	} else {
...
/* {{{ data_available */
int
data_available(ftpbuf_t *ftp, crx_socket_t s)
{
	int		n;

	n = crx_pollfd_for_ms(s, CRX_POLLREADABLE, 1000);
	if (n < 1) {
		char buf[256];
		if (n == 0) {
#ifdef CRX_WIN32
...
/* {{{ data_writeable */
int
data_writeable(ftpbuf_t *ftp, crx_socket_t s)
{
	int		n;

	n = crx_pollfd_for_ms(s, POLLOUT, 1000);
	if (n < 1) {
		char buf[256];
		if (n == 0) {
#ifdef CRX_WIN32
...
/* {{{ my_accept */
int
my_accept(ftpbuf_t *ftp, crx_socket_t s, struct sockaddr *addr, socklen_t *addrlen)
{
	int		n;
...
ftp_getdata(ftpbuf_t *ftp)
{
	...
	if ((fd = socket(sa->sa_family, SOCK_STREAM, 0)) == SOCK_ERR) {
		crx_error_docref(NULL, E_WARNING, "socket() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
	...
	if (ftp->pasv) {
		...
		size = crx_sockaddr_size(&ftp->pasvaddr);
		tv.tv_sec = ftp->timeout_sec;
		tv.tv_usec = 0;
		if (crx_connect_nonb(fd, (struct sockaddr*) &ftp->pasvaddr, size, &tv) == -1) {
			crx_error_docref(NULL, E_WARNING, "crx_connect_nonb() failed: %s (%d)", strerror(errno), errno);
			goto bail;
		}
	...
	crx_any_addr(sa->sa_family, &addr, 0);
	size = crx_sockaddr_size(&addr);

	if (bind(fd, (struct sockaddr*) &addr, size) != 0) {
		crx_error_docref(NULL, E_WARNING, "bind() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
	...
	if (listen(fd, 5) != 0) {
		crx_error_docref(NULL, E_WARNING, "listen() failed: %s (%d)", strerror(errno), errno);
		goto bail;
	}
...
data_accepted:
	...
	if (ftp->use_ssl && ftp->use_ssl_for_data) {
		...
		if (ctx == NULL) {
			crx_error_docref(NULL, E_WARNING, "data_accept: failed to retrieve the existing SSL context");
			return 0;
		}

		data->ssl_handle = SSL_new(ctx);
		if (data->ssl_handle == NULL) {
			crx_error_docref(NULL, E_WARNING, "data_accept: failed to create the SSL handle");
			return 0;
		}
		...
		if (session == NULL) {
			crx_error_docref(NULL, E_WARNING, "data_accept: failed to retrieve the existing SSL session");
		...
		if (res == 0) {
			crx_error_docref(NULL, E_WARNING, "data_accept: failed to set the existing SSL session");
			SSL_free(data->ssl_handle);
			return 0;
		}
		...
		do {
			...
			switch (err) {
				...
				default:
					crx_error_docref(NULL, E_WARNING, "data_accept: SSL/TLS handshake failed");
					SSL_shutdown(data->ssl_handle);
					SSL_free(data->ssl_handle);
					return 0;
			}
...
static void ftp_ssl_shutdown(ftpbuf_t *ftp, crx_socket_t fd, SSL *ssl_handle) {
	...
	*/
	...
	if (err < 0) {
		crx_error_docref(NULL, E_WARNING, "SSL_shutdown failed");
	}
	else if (err == 0) {
			...
			switch (err) {
				...
				default:
					if ((sslerror = ERR_get_error())) {
						...
						crx_error_docref(NULL, E_WARNING, "SSL_read on shutdown: %s", buf);
					} else if (errno) {
						crx_error_docref(NULL, E_WARNING, "SSL_read on shutdown: %s (%d)", strerror(errno), errno);
					}
					done = 1;
					break;
			}
...
ftp_genlist(ftpbuf_t *ftp, const char *cmd, const size_t cmd_len, const char *path, const size_t path_len)
{
	...
	if ((tmpstream = crx_stream_fopen_tmpfile()) == NULL) {
	...
	while ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		crx_stream_write(tmpstream, data->buf, rcvd);
	...
	*entry = text;
	lastch = 0;
	while ((ch = crx_stream_getc(tmpstream)) != EOF) {
		if (ch == '\n' && lastch == '\r') {
			*(text - 1) = 0;
			*++entry = text;
		} else {
...
/* {{{ ftp_nb_get */
int
ftp_nb_get(ftpbuf_t *ftp, crx_stream *outstream, const char *path, const size_t path_len, ftptype_t type, crex_long resumepos)
{
	databuf_t		*data = NULL;
	char			arg[MAX_LENGTH_OF_LONG];

	if (ftp == NULL) {
		return CRX_FTP_FAILED;
	}
...
ftp_nb_continue_read(ftpbuf_t *ftp)
{
	...
	if ((rcvd = my_recv(ftp, data->fd, data->buf, FTP_BUFSIZE))) {
		...
		if (type == FTPTYPE_ASCII) {
			for (ptr = data->buf; rcvd; rcvd--, ptr++) {
				if (lastch == '\r' && *ptr != '\n') {
					crx_stream_putc(ftp->stream, '\r');
				}
				if (*ptr != '\r') {
					crx_stream_putc(ftp->stream, *ptr);
				}
				lastch = *ptr;
			}
		} else if (rcvd != crx_stream_write(ftp->stream, data->buf, rcvd)) {
			goto bail;
		}
...
/* {{{ ftp_nb_put */
int
ftp_nb_put(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type, crex_long startpos)
{
	databuf_t		*data = NULL;
	char			arg[MAX_LENGTH_OF_LONG];
...
ftp_nb_continue_write(ftpbuf_t *ftp)
{
	...
	size = 0;
	ptr = ftp->data->buf;
	while (!crx_stream_eof(ftp->stream) && (ch = crx_stream_getc(ftp->stream)) != EOF) {
