...
#define CRX_FTP_FAILED			0
#define CRX_FTP_FINISHED		1
#define CRX_FTP_MOREDATA		2
...
typedef struct databuf
{
	...
	crx_socket_t		fd;			/* data connection */
...
{
	crx_socket_t		fd;			/* control connection */
	crx_sockaddr_storage	localaddr;	/* local address */
	...
	crx_sockaddr_storage	pasvaddr;	/* passive mode address */
	crex_long	timeout_sec;	/* User configurable timeout (seconds) */
	...
	crx_stream		*stream; /* output stream for "nonblocking" transfers */
 ...
 */
ftpbuf_t*	ftp_open(const char *host, short port, crex_long timeout_sec);
 ...
 */
crex_string* ftp_mkdir(ftpbuf_t *ftp, const char *dir, const size_t dir_len);
...
int		ftp_alloc(ftpbuf_t *ftp, const crex_long size, crex_string **response);
 ...
 * returns true on success, false on error
 */
int		ftp_get(ftpbuf_t *ftp, crx_stream *outstream, const char *path, const size_t path_len, ftptype_t type, crex_long resumepos);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_put(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type, crex_long startpos);

/* append the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_append(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type);

/* returns the size of the given file, or -1 on error */
crex_long		ftp_size(ftpbuf_t *ftp, const char *path, const size_t path_len);
 ...
 * returns true on success, false on error
 */
int		ftp_nb_get(ftpbuf_t *ftp, crx_stream *outstream, const char *path, const size_t path_len, ftptype_t type, crex_long resumepos);

/* stores the data from a file, socket, or process as a file on the remote server
 * returns true on success, false on error
 */
int		ftp_nb_put(ftpbuf_t *ftp, const char *path, const size_t path_len, crx_stream *instream, ftptype_t type, crex_long startpos);
