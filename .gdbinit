...
define ____executor_globals
	if basic_functions_module.zts
		...
		set $eg = ((crex_executor_globals*) (*((void ***) $tsrm_ls))[executor_globals_id-1])
		set $cg = ((crex_compiler_globals*) (*((void ***) $tsrm_ls))[compiler_globals_id-1])
		set $eg_ptr = $eg
	else
		set $eg = executor_globals
		set $cg = compiler_globals
		set $eg_ptr = (crex_executor_globals*) &executor_globals
	end
end
...
define print_cvs
	if $argc == 0
		...
		set $cv_ex_ptr = (crex_execute_data *)$arg0
	end
	set $cv_count = $cv_ex_ptr.func.op_array.last_var
	set $cv = $cv_ex_ptr.func.op_array.vars
	set $cv_idx = 0
	set $callFrameSize = (sizeof(crex_execute_data) + sizeof(zval) - 1) / sizeof(zval)
...
document print_cvs
	...
	If a crex_execute_data pointer is set this will print the compiled
	variables of that scope. If no parameter is used it will use
	current_execute_data for scope.

	usage: print_cvs [crex_execute_data *]
end
...
define ____printzv_contents
	...
	if $type == 8
		...
		if ! $arg1
			if $handlers->get_properties == &crex_std_get_properties
				if $zobj->properties
					printf "\nProperties "
					...
					while $k < $num
						...
						set $name = $p->key
						set $prop = (crex_property_info*)$p->val.value.ptr
...
document print_const_table
	...
	Example: print_const_table executor_globals.crex_constants
end
...
define ____print_ht
	...
	while $i < $num
		...
		if $val->u1.v.type > 0
			...
			if $arg1 == 3
				set $func = (crex_function*)$val->value.ptr
				printf "\"%s\"\n", $func->common.function_name->val
			end
			if $arg1 == 4
				set $const = (crex_constant *)$val->value.ptr
				____printzv $const 1
			end
		end
		set $i = $i + 1
...
define print_pi
	set $pi = (crex_property_info *)$arg0
	set $initial_offset = ((uint32_t)(uintptr_t)(&((crex_object*)0)->properties_table[(0)]))
	...
	if $pi->flags & 0x100
		printf "CREX_ACC_PUBLIC"
	else
		if $pi->flags & 0x200
			printf "CREX_ACC_PROTECTED"
		else
			if $pi->flags & 0x400
				printf "CREX_ACC_PRIVATE"
			else
				if $pi->flags & 0x800
					printf "CREX_ACC_EARLY_BINDING"
				else
					if $pi->flags & 0x20000
						printf "CREX_ACC_SHADOW"
					end
				end
			end
		end
...
document print_zstr
	print the length and contents of a crex string
