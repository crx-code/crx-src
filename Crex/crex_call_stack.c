...
CREX_API void crex_call_stack_init(void) {
	if (!crex_call_stack_get(&EG(call_stack))) {
		EG(call_stack) = (crex_call_stack){0};
	}

	switch (EG(max_allowed_stack_size)) {
		case CREX_MAX_ALLOWED_STACK_SIZE_DETECT: {
			void *base = EG(call_stack).base;
			size_t size = EG(call_stack).max_size;
			if (UNEXPECTED(base == (void*)0)) {
				base = crex_call_stack_position();
				size = crex_call_stack_default_size();
				/* base is not the actual stack base */
				size -= 32 * 1024;
			}
			EG(stack_base) = base;
			EG(stack_limit) = crex_call_stack_limit(base, size, EG(reserved_stack_size));
			break;
		}
		case CREX_MAX_ALLOWED_STACK_SIZE_UNCHECKED: {
			EG(stack_base) = (void*)0;
			EG(stack_limit) = (void*)0;
			break;
		}
		default: {
			CREX_ASSERT(EG(max_allowed_stack_size) > 0);
			void *base = EG(call_stack).base;
			if (UNEXPECTED(base == (void*)0)) {
				base = crex_call_stack_position();
			}
			EG(stack_base) = base;
			EG(stack_limit) = crex_call_stack_limit(base, EG(max_allowed_stack_size), EG(reserved_stack_size));
			break;
		}
	}
}
...
#ifdef __linux__
static bool crex_call_stack_is_main_thread(void) {
# ifdef HAVE_GETTID
...
static bool crex_call_stack_get_linux_pthread(crex_call_stack *stack)
{
	pthread_attr_t attr;
	int error;
	void *addr;
	...
	CREX_ASSERT(!crex_call_stack_is_main_thread());
...
static bool crex_call_stack_get_linux_proc_maps(crex_call_stack *stack)
{
	FILE *f;
	char buffer[4096];
...
static bool crex_call_stack_get_linux(crex_call_stack *stack)
{
	if (crex_call_stack_is_main_thread()) {
		return crex_call_stack_get_linux_proc_maps(stack);
	}

	return crex_call_stack_get_linux_pthread(stack);
}
#else /* __linux__ */
static bool crex_call_stack_get_linux(crex_call_stack *stack)
{
...
static bool crex_call_stack_get_freebsd_pthread(crex_call_stack *stack)
{
	pthread_attr_t attr;
	int error;
	void *addr;
...
static bool crex_call_stack_get_freebsd_sysctl(crex_call_stack *stack)
{
	void *stack_base;
...
static bool crex_call_stack_get_freebsd(crex_call_stack *stack)
{
	if (crex_call_stack_is_main_thread()) {
		return crex_call_stack_get_freebsd_sysctl(stack);
	}

	return crex_call_stack_get_freebsd_pthread(stack);
}
#else
static bool crex_call_stack_get_freebsd(crex_call_stack *stack)
{
...
#ifdef CREX_WIN32
static bool crex_call_stack_get_win32(crex_call_stack *stack)
{
	...
	CREX_ASSERT(result_size >= sizeof(uncommitted_region));
...
static bool crex_call_stack_get_macos(crex_call_stack *stack)
{
...
static bool crex_call_stack_get_openbsd_pthread(crex_call_stack *stack)
{
	stack_t ss;
...
static bool crex_call_stack_get_openbsd_vm(crex_call_stack *stack)
{
...
static bool crex_call_stack_get_openbsd(crex_call_stack *stack)
{
	...
	if (!TIB_GET()->tib_thread || (TIB_GET()->tib_thread_flags & 0x002) != 0) {
		return crex_call_stack_get_openbsd_vm(stack);
	}

	return crex_call_stack_get_openbsd_pthread(stack);
}
...
{
	if (crex_call_stack_get_linux(stack)) {
		return true;
	}

	if (crex_call_stack_get_freebsd(stack)) {
		return true;
	}

	if (crex_call_stack_get_win32(stack)) {
		return true;
	}

	if (crex_call_stack_get_macos(stack)) {
		return true;
	}

	if (crex_call_stack_get_openbsd(stack)) {
		return true;
	}
