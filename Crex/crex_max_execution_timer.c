...
CREX_API void crex_max_execution_timer_init(void) /* {{{ */
{
	...
	// Measure wall time instead of CPU time as originally planned now that it is possible https://github.com/crx/crx-src/pull/6504#issuecomment-1370303727
	if (timer_create(CLOCK_BOOTTIME, &sev, &EG(max_execution_timer_timer)) != 0) {
		crex_strerror_noreturn(E_ERROR, errno, "Could not create timer");
	}
...
# endif
	...
	if (timer_settime(timer, 0, &its, NULL) != 0) {
		crex_strerror_noreturn(E_ERROR, errno, "Could not set timer");
	}
}
/* }}} */
...
# endif
	...
	if (err != 0) {
		crex_strerror_noreturn(E_ERROR, errno, "Could not delete timer");
	}
}
/* }}}} */
