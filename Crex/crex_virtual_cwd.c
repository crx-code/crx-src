...
#if defined(CREX_WIN32) && defined(ZTS)
MUTEX_T cwd_mutex;
#endif
...
static int crx_is_dir_ok(const cwd_state *state)  /* {{{ */
{
	crex_stat_t buf = {0};

	if (crx_sys_stat(state->cwd, &buf) == 0 && S_ISDIR(buf.st_mode))
		return (0);
...
static int crx_is_file_ok(const cwd_state *state)  /* {{{ */
{
	crex_stat_t buf = {0};

	if (crx_sys_stat(state->cwd, &buf) == 0 && S_ISREG(buf.st_mode))
		return (0);
...
static void realpath_cache_clean_helper(uint32_t max_entries, realpath_cache_bucket **cache, crex_long *cache_size)
{
	uint32_t i;
...
void virtual_cwd_main_cwd_init(uint8_t reinit) /* {{{ */
{
	...
	result = crx_win32_ioutil_getcwd(cwd, sizeof(cwd));
#else
...
#ifdef CREX_WIN32
static inline crex_ulong realpath_cache_key(const char *path, size_t path_len) /* {{{ */
{
	crex_ulong h;
	...
	for (h = C_UL(2166136261); bucket_key < e;) {
		h *= C_UL(16777619);
		h ^= *bucket_key++;
	}
...
{
	crex_ulong key = realpath_cache_key(path, path_len);
	crex_ulong n = key % (sizeof(CWDG(realpath_cache)) / sizeof(CWDG(realpath_cache)[0]));
...
{
	crex_long size = sizeof(realpath_cache_bucket) + path_len + 1;
	int same = 1;
...
CWD_API crex_long realpath_cache_size(void)
{
	return CWDG(realpath_cache_size);
}

CWD_API crex_long realpath_cache_max_buckets(void)
{
...
retry_reparse_point:
		...
		if (save) {
			...
			CRX_WIN32_IOUTIL_CHECK_PATH_W(pathw, (size_t)-1, 1);
...
retry_reparse_tag_cloud:
		if(save &&
			...
			if(pbuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK) {
...
#if VIRTUAL_CWD_DEBUG
				printname = crx_win32_ioutil_w_to_any(reparsetarget + pbuffer->MountPointReparseBuffer.PrintNameOffset  / sizeof(WCHAR));
				if (!printname) {
					free_alloca(pbuffer, use_heap_large);
					free_alloca(tmp, use_heap);
					FREE_PATHW()
				...
				substitutename = crx_win32_cp_conv_w_to_any(tmpsubstname, substitutename_len, &substitutename_len);
			...
			if(isabsolute && substitutename_len > 4) {
				/* Do not resolve volumes (for now). A mounted point can
				   ...
				   all IO functions we use in crx and its deps support
...
#else
		if (save && crx_sys_lstat(path, &st) < 0) {
			if (use_realpath == CWD_REALPATH) {
				/* file not found */
				return (size_t)-1;
			}
		...
		if (save && S_ISLNK(st.st_mode)) {
			if (++(*ll) > LINK_MAX || (j = (size_t)crx_sys_readlink(tmp, path, MAXPATHLEN)) == (size_t)-1) {
			...
			if (save) {
				size_t sz;
				char *tmp_path = crx_win32_ioutil_conv_w_to_any(dataw.cFileName, CRX_WIN32_CP_IGNORE_LEN, &sz);
				if (!tmp_path) {
					free_alloca(tmp, use_heap);
					FREE_PATHW()
					return (size_t)-1;
...
CWD_API crex_result virtual_chdir(const char *path) /* {{{ */
{
	return virtual_file_ex(&CWDG(cwd), path, crx_is_dir_ok, CWD_REALPATH) ? FAILURE : SUCCESS;
}
/* }}} */
...
{
	return virtual_filepath_ex(path, filepath, crx_is_file_ok);
}
/* }}} */
...
#ifdef CREX_WIN32
	f = crx_win32_ioutil_fopen(new_state.cwd, mode);
#else
...
CWD_API int virtual_chmod(const char *filename, mode_t mode) /* {{{ */
{
...
#ifdef CREX_WIN32
	{
		...
		ret = crx_win32_ioutil_chmod(new_state.cwd, mode);
	}
#else
...
#ifdef CREX_WIN32
		f = crx_win32_ioutil_open(new_state.cwd, flags, mode);
#else
		f = open(new_state.cwd, flags, mode);
#endif
	} else {
#ifdef CREX_WIN32
		f = crx_win32_ioutil_open(new_state.cwd, flags);
#else
...
CWD_API int virtual_rename(const char *oldname, const char *newname) /* {{{ */
{
	...
	retval = crx_win32_ioutil_rename(oldname, newname);
#else
...
CWD_API int virtual_stat(const char *path, crex_stat_t *buf) /* {{{ */
{
	cwd_state new_state;
	int retval;
	...
	retval = crx_sys_stat(new_state.cwd, buf);
...
CWD_API int virtual_lstat(const char *path, crex_stat_t *buf) /* {{{ */
{
	cwd_state new_state;
	int retval;
	...
	retval = crx_sys_lstat(new_state.cwd, buf);
...
#ifdef CREX_WIN32
	retval = crx_win32_ioutil_unlink(new_state.cwd);
#else
...
#ifdef CREX_WIN32
	retval = crx_win32_ioutil_mkdir(new_state.cwd, mode);
#else
...
#ifdef CREX_WIN32
	retval = crx_win32_ioutil_rmdir(new_state.cwd);
#else
