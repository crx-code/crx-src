...
CREX_API crex_op_array *(*crex_dtrace_compile_file)(crex_file_handle *file_handle, int type);
CREX_API void (*crex_dtrace_execute)(crex_op_array *op_array);
CREX_API void (*crex_dtrace_execute_internal)(crex_execute_data *execute_data, zval *return_value);
...
static inline const char *dtrace_get_executed_filename(void)
{
	...
	if (ex) {
		...
		return crex_get_executed_filename();
	}
}
