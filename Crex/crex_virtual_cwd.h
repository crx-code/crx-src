...
#ifdef CREX_WIN32
# define crx_sys_stat_ex crx_win32_ioutil_stat_ex
# define crx_sys_stat crx_win32_ioutil_stat
# define crx_sys_lstat crx_win32_ioutil_lstat
# define crx_sys_fstat crx_win32_ioutil_fstat
# define crx_sys_readlink crx_win32_ioutil_readlink
# define crx_sys_symlink crx_win32_ioutil_symlink
# define crx_sys_link crx_win32_ioutil_link
#else
# define crx_sys_stat stat
# define crx_sys_lstat lstat
# define crx_sys_fstat fstat
# ifdef HAVE_SYMLINK
# define crx_sys_readlink(link, target, target_len) readlink(link, target, target_len)
# define crx_sys_symlink symlink
# define crx_sys_link link
# endif
#endif
...
#define REALPATH_CACHE_SIZE 0      /* disabled while crx.ini isn't loaded */
...
typedef struct _virtual_cwd_globals {
	cwd_state cwd;
	crex_long                   realpath_cache_size;
	crex_long                   realpath_cache_size_limit;
	crex_long                   realpath_cache_ttl;
...
# define CWDG(v) CREX_TSRMG_FAST(cwd_globals_offset, virtual_cwd_globals *, v)
#else
...
#if defined(CREX_WIN32)
#define VCWD_FOPEN(path, mode)  crx_win32_ioutil_fopen(path, mode)
#define VCWD_OPEN(path, flags) crx_win32_ioutil_open(path, flags)
#define VCWD_OPEN_MODE(path, flags, mode) crx_win32_ioutil_open(path, flags, mode)
# define VCWD_RENAME(oldname, newname) crx_win32_ioutil_rename(oldname, newname)
#define VCWD_MKDIR(pathname, mode) crx_win32_ioutil_mkdir(pathname, mode)
#define VCWD_RMDIR(pathname) crx_win32_ioutil_rmdir(pathname)
#define VCWD_UNLINK(path) crx_win32_ioutil_unlink(path)
#define VCWD_CHDIR(path) crx_win32_ioutil_chdir(path)
#define VCWD_ACCESS(pathname, mode) tsrm_win32_access(pathname, mode)
#define VCWD_GETCWD(buff, size) crx_win32_ioutil_getcwd(buff, size)
#define VCWD_CHMOD(path, mode) crx_win32_ioutil_chmod(path, mode)
#else
...
#define VCWD_STAT(path, buff) crx_sys_stat(path, buff)
