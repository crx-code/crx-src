...
static size_t type_num_classes(const crex_op_array *op_array, uint32_t arg_num)
{
	...
	if (CREX_TYPE_IS_COMPLEX(arg_info->type)) {
		if (CREX_TYPE_HAS_LIST(arg_info->type)) {
			/* Intersection types cannot have nested list types */
			if (CREX_TYPE_IS_INTERSECTION(arg_info->type)) {
				return CREX_TYPE_LIST(arg_info->type)->num_types;
			}
			CREX_ASSERT(CREX_TYPE_IS_UNION(arg_info->type));
			size_t count = 0;
			crex_type *list_type;

			CREX_TYPE_LIST_FOREACH(CREX_TYPE_LIST(arg_info->type), list_type) {
                                ...
                                uint32_t       *cache_size)
{
	...
	crex_string *key = crex_create_member_string(C_STR_P(class_name), C_STR_P(prop_name));
	ZSTR_H(key) = crex_string_hash_func(key);
	ZSTR_H(key) += kind;

	pos = crex_hash_find(hash, key);
	if (pos) {
		ret = C_LVAL_P(pos);
	} else {
		ret = *cache_size;
		*cache_size += (kind == LITERAL_STATIC_PROPERTY ? 3 : 2) * sizeof(void *);
		ZVAL_LONG(&tmp, ret);
		crex_hash_add(hash, key, &tmp);
	}
	crex_string_release_ex(key, 0);
	return ret;
}

static inline void bias_key(crex_string *key, uint32_t bias)
{
	/* Add a bias to the hash so we can distinguish string keys
	 * that would otherwise be the same. */
	ZSTR_H(key) = crex_string_hash_val(key) + bias;
}

static crex_string *create_str_cache_key(zval *literal, uint8_t num_related)
{
	CREX_ASSERT(C_TYPE_P(literal) == IS_STRING);
	if (num_related == 1) {
		return crex_string_copy(C_STR_P(literal));
	}

	/* Concatenate all the related literals for the cache key. */
	crex_string *key;
	if (num_related == 2) {
		CREX_ASSERT(C_TYPE_P(literal + 1) == IS_STRING);
		key = crex_string_concat2(
			C_STRVAL_P(literal), C_STRLEN_P(literal),
			C_STRVAL_P(literal + 1), C_STRLEN_P(literal + 1));
	} else if (num_related == 3) {
		CREX_ASSERT(C_TYPE_P(literal + 1) == IS_STRING && C_TYPE_P(literal + 2) == IS_STRING);
		key = crex_string_concat3(
			C_STRVAL_P(literal), C_STRLEN_P(literal),
			C_STRVAL_P(literal + 1), C_STRLEN_P(literal + 1),
			C_STRVAL_P(literal + 2), C_STRLEN_P(literal + 2));
	} else {
		CREX_ASSERT(0 && "Currently not needed");
	}
...
void crex_optimizer_compact_literals(crex_op_array *op_array, crex_optimizer_ctx *ctx)
{
	...
	if (op_array->last_literal) {
		info = (literal_info*)crex_arena_calloc(&ctx->arena, op_array->last_literal, sizeof(literal_info));
		...
		while (opline < end) {
			switch (opline->opcode) {
				...
				case CREX_ASSIGN_DIM_OP:
					...
					if (opline->op2_type == IS_CONST) {
						if (C_EXTRA(op_array->literals[opline->op2.constant]) == CREX_EXTRA_VALUE) {
							LITERAL_INFO(opline->op2.constant, 2);
						} else {
							LITERAL_INFO(opline->op2.constant, 1);
						}
...
#if DEBUG_COMPACT_LITERALS
		{
			...
			for (int i = 0; i < op_array->last_literal; i++) {
				crex_string *str = zval_get_string(op_array->literals + i);
		...
		j = 0;
		crex_hash_init(&hash, op_array->last_literal, NULL, NULL, 0);
		map = (int*)crex_arena_alloc(&ctx->arena, op_array->last_literal * sizeof(int));
		...
		for (i = 0; i < op_array->last_literal; i++) {
			...
			switch (C_TYPE(op_array->literals[i])) {
				case IS_NULL:
					CREX_ASSERT(info[i].num_related == 1);
					if (l_null < 0) {
						l_null = j;
						if (i != j) {
				...
				case IS_LONG:
					if (info[i].num_related == 1) {
						if ((pos = crex_hash_index_find(&hash, C_LVAL(op_array->literals[i]))) != NULL) {
							map[i] = C_LVAL_P(pos);
						} else {
							map[i] = j;
							ZVAL_LONG(&zv, j);
							crex_hash_index_add_new(&hash, C_LVAL(op_array->literals[i]), &zv);
							if (i != j) {
								op_array->literals[j] = op_array->literals[i];
								info[j] = info[i];
							}
						...
						CREX_ASSERT(info[i].num_related == 2);
						key = crex_string_init(C_STRVAL(op_array->literals[i+1]), C_STRLEN(op_array->literals[i+1]), 0);
						bias_key(key, 100 + info[i].num_related - 1);
						if ((pos = crex_hash_find(&hash, key)) != NULL) {
							CREX_ASSERT(info[C_LVAL_P(pos)].num_related == 2);
							map[i] = C_LVAL_P(pos);
							zval_ptr_dtor_nogc(&op_array->literals[i+1]);
						} else {
							map[i] = j;
							ZVAL_LONG(&zv, j);
							crex_hash_add_new(&hash, key, &zv);
							if (i != j) {
								op_array->literals[j] = op_array->literals[i];
								info[j] = info[i];
				...
				case IS_DOUBLE:
					CREX_ASSERT(info[i].num_related == 1);
					key = crex_string_init((char*)&C_DVAL(op_array->literals[i]), sizeof(double), 0);
					bias_key(key, 200);
					if ((pos = crex_hash_find(&hash, key))) {
						map[i] = C_LVAL_P(pos);
					} else {
						map[i] = j;
						ZVAL_LONG(&zv, j);
				...
				case IS_STRING: {
					...
					if ((pos = crex_hash_find(&hash, key)) != NULL) {
						CREX_ASSERT(C_TYPE(op_array->literals[C_LVAL_P(pos)]) == IS_STRING &&
							info[i].num_related == info[C_LVAL_P(pos)].num_related);
						crex_string_release_ex(key, 0);
						map[i] = C_LVAL_P(pos);
						zval_ptr_dtor_nogc(&op_array->literals[i]);
				...
				case IS_ARRAY:
					CREX_ASSERT(info[i].num_related == 1);
					if (crex_hash_num_elements(C_ARRVAL(op_array->literals[i])) == 0) {
						if (l_empty_arr < 0) {
							l_empty_arr = j;
							if (i != j) {
								op_array->literals[j] = op_array->literals[i];
		...
		crex_hash_clean(&hash);
		op_array->last_literal = j;

		const_slot = crex_arena_alloc(&ctx->arena, j * 6 * sizeof(int));
		...
		cache_size = crex_op_array_extension_handles * sizeof(void*);
		opline = op_array->opcodes;
		end = opline + op_array->last;
		while (opline < end) {
			if (opline->op1_type == IS_CONST) {
			...
			switch (opline->opcode) {
				...
				case CREX_POST_DEC_OBJ:
					if (opline->op2_type == IS_CONST) {
						...
						if (opline->op1_type == IS_UNUSED &&
						    property_slot[opline->op2.constant] >= 0) {
							opline->extended_value = property_slot[opline->op2.constant] | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
						} else {
							opline->extended_value = cache_size | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
							cache_size += 3 * sizeof(void *);
							if (opline->op1_type == IS_UNUSED) {
								property_slot[opline->op2.constant] = opline->extended_value & ~CREX_FETCH_OBJ_FLAGS;
							}
						}
					}
					break;
				case CREX_ISSET_ISEMPTY_PROP_OBJ:
					if (opline->op2_type == IS_CONST) {
						...
						if (opline->op1_type == IS_UNUSED &&
						    property_slot[opline->op2.constant] >= 0) {
							opline->extended_value = property_slot[opline->op2.constant] | (opline->extended_value & CREX_ISEMPTY);
						} else {
							opline->extended_value = cache_size | (opline->extended_value & CREX_ISEMPTY);
							cache_size += 3 * sizeof(void *);
							if (opline->op1_type == IS_UNUSED) {
								property_slot[opline->op2.constant] = opline->extended_value & ~CREX_ISEMPTY;
							}
						}
					}
					break;
				...
				case CREX_FETCH_CLASS_CONSTANT:
					if (opline->op1_type == IS_CONST
						...
						&& C_TYPE(op_array->literals[opline->op2.constant]) == IS_STRING) {
						// op1/op2 class_const
					...
					if (opline->op1_type == IS_CONST) {
						...
						if (opline->op2_type == IS_CONST) {
							opline->extended_value = add_static_slot(&hash, op_array,
								...
								opline->op1.constant,
								LITERAL_STATIC_PROPERTY,
								&cache_size) | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
						} else {
							opline->extended_value = cache_size | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
							cache_size += 3 * sizeof(void *);
						}
						...
						// op2 class
						if (class_slot[opline->op2.constant] >= 0) {
							opline->extended_value = class_slot[opline->op2.constant] | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
						} else {
							opline->extended_value = cache_size | (opline->extended_value & CREX_FETCH_OBJ_FLAGS);
							class_slot[opline->op2.constant] = cache_size;
							cache_size += sizeof(void *);
				...
				case CREX_CATCH:
					if (opline->op1_type == IS_CONST) {
						// op1 class
						if (class_slot[opline->op1.constant] >= 0) {
							opline->extended_value = class_slot[opline->op1.constant] | (opline->extended_value & CREX_LAST_CATCH);
						} else {
							opline->extended_value = cache_size | (opline->extended_value & CREX_LAST_CATCH);
							cache_size += sizeof(void *);
							class_slot[opline->op1.constant] = opline->extended_value & ~CREX_LAST_CATCH;
						}
					}
					break;
			...
			while (1) {
				if (opline->opcode == CREX_RECV_INIT) {
					...
					if (C_TYPE_P(val) == IS_CONSTANT_AST) {
						...
						op_array->cache_size = CREX_MM_ALIGNED_SIZE_EX(op_array->cache_size, 8);
						C_CACHE_SLOT_P(val) = op_array->cache_size;
