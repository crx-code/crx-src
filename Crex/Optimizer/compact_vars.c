/*
   +----------------------------------------------------------------------+
   | Crex Engine, Removing unused variables                               |
   +----------------------------------------------------------------------+
...
void crex_optimizer_compact_vars(crex_op_array *op_array) {
	...
	uint32_t used_vars_len = crex_bitset_len(op_array->last_var + op_array->T);
	crex_bitset used_vars = CREX_BITSET_ALLOCA(used_vars_len, use_heap1);
	...
	crex_bitset_clear(used_vars, used_vars_len);
	for (i = 0; i < op_array->last; i++) {
		crex_op *opline = &op_array->opcodes[i];
		if (opline->op1_type & (IS_CV|IS_VAR|IS_TMP_VAR)) {
			crex_bitset_incl(used_vars, VAR_NUM(opline->op1.var));
		}
		if (opline->op2_type & (IS_CV|IS_VAR|IS_TMP_VAR)) {
			crex_bitset_incl(used_vars, VAR_NUM(opline->op2.var));
		}
		if (opline->result_type & (IS_CV|IS_VAR|IS_TMP_VAR)) {
			crex_bitset_incl(used_vars, VAR_NUM(opline->result.var));
			if (opline->opcode == CREX_ROPE_INIT) {
				uint32_t num = ((opline->extended_value * sizeof(crex_string*)) + (sizeof(zval) - 1)) / sizeof(zval);
				while (num > 1) {
					num--;
					crex_bitset_incl(used_vars, VAR_NUM(opline->result.var) + num);
				}
			}
		}
	}
	...
	CREX_ASSERT(num_cvs <= op_array->last_var);
	CREX_ASSERT(num_tmps <= op_array->T);
	...
	if (num_cvs != op_array->last_var) {
		if (num_cvs) {
			crex_string **names = safe_emalloc(sizeof(crex_string *), num_cvs, 0);
			for (i = 0; i < op_array->last_var; i++) {
