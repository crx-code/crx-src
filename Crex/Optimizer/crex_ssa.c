...
static bool will_rejoin(
		const crex_cfg *cfg, const crex_dfg *dfg, const crex_basic_block *block,
...
static bool needs_pi(const crex_op_array *op_array, const crex_dfg *dfg, const crex_ssa *ssa, int from, int to, int var) /* {{{ */
{
	const crex_basic_block *from_block, *to_block;
	int other_successor;
	...
	CREX_ASSERT(from_block->successors_count == 2);
...
static crex_ssa_phi *add_pi(
		crex_arena **arena, const crex_op_array *op_array, crex_dfg *dfg, crex_ssa *ssa,
		int from, int to, int var) /* {{{ */
{
	...
	phi = crex_arena_calloc(arena, 1,
		CREX_MM_ALIGNED_SIZE(sizeof(crex_ssa_phi)) +
		CREX_MM_ALIGNED_SIZE(sizeof(int) * ssa->cfg.blocks[to].predecessors_count) +
	...
	phi->use_chains = (crex_ssa_phi**)(((char*)phi->sources) + CREX_MM_ALIGNED_SIZE(sizeof(int) * ssa->cfg.blocks[to].predecessors_count));
...
static void pi_range(
		crex_ssa_phi *phi, int min_var, int max_var, crex_long min, crex_long max,
		char underflow, char overflow, char negative) /* {{{ */
{
	crex_ssa_range_constraint *constraint = &phi->constraint.range;
...
static inline void pi_range_equals(crex_ssa_phi *phi, int var, crex_long val) {
	pi_range(phi, var, var, val, val, 0, 0, 0);
}
static inline void pi_range_not_equals(crex_ssa_phi *phi, int var, crex_long val) {
	pi_range(phi, var, var, val, val, 0, 0, 1);
}
static inline void pi_range_min(crex_ssa_phi *phi, int var, crex_long val) {
	pi_range(phi, var, -1, val, CREX_LONG_MAX, 0, 1, 0);
}
static inline void pi_range_max(crex_ssa_phi *phi, int var, crex_long val) {
	pi_range(phi, -1, var, CREX_LONG_MIN, val, 1, 0, 0);
}

static void pi_type_mask(crex_ssa_phi *phi, uint32_t type_mask) {
...
static inline void pi_not_type_mask(crex_ssa_phi *phi, uint32_t type_mask) {
...
static int find_adjusted_tmp_var(const crex_op_array *op_array, uint32_t build_flags, crex_op *opline, uint32_t var_num, crex_long *adjustment) /* {{{ */
{
	crex_op *op = opline;
	zval *zv;
	...
	while (op != op_array->opcodes) {
		...
		if (op->opcode == CREX_POST_DEC) {
			if (op->op1_type == IS_CV) {
				*adjustment = -1;
				return EX_VAR_TO_NUM(op->op1.var);
			}
		} else if (op->opcode == CREX_POST_INC) {
			if (op->op1_type == IS_CV) {
				*adjustment = 1;
				return EX_VAR_TO_NUM(op->op1.var);
			}
		} else if (op->opcode == CREX_ADD) {
			if (op->op1_type == IS_CV && op->op2_type == IS_CONST) {
				zv = CRT_CONSTANT_EX(op_array, op, op->op2);
				if (C_TYPE_P(zv) == IS_LONG
				 && C_LVAL_P(zv) != CREX_LONG_MIN) {
					*adjustment = -C_LVAL_P(zv);
					return EX_VAR_TO_NUM(op->op1.var);
				}
...
static void place_essa_pis(
		crex_arena **arena, const crex_script *script, const crex_op_array *op_array,
		uint32_t build_flags, crex_ssa *ssa, crex_dfg *dfg) /* {{{ */ {
	...
	for (j = 0; j < blocks_count; j++) {
		...
		if ((blocks[j].flags & CREX_BB_REACHABLE) == 0 || blocks[j].len == 0) {
			continue;
		}
		...
		if (opline->op1_type == IS_TMP_VAR &&
		    ((opline-1)->opcode == CREX_IS_EQUAL ||
		     (opline-1)->opcode == CREX_IS_NOT_EQUAL ||
		     (opline-1)->opcode == CREX_IS_SMALLER ||
		     (opline-1)->opcode == CREX_IS_SMALLER_OR_EQUAL) &&
...
//			long val = 0;
			...
			if (var1 >= 0 && var2 >= 0) {
				if (!crex_sub_will_overflow(val1, val2) && !crex_sub_will_overflow(val2, val1)) {
					crex_long tmp = val1;
					val1 -= val2;
					val2 -= tmp;
				} else {
				...
				if (!crex_add_will_overflow(val2, add_val2)) {
					val2 += add_val2;
				} else {
					var1 = -1;
				}
			} else if (var1 < 0 && var2 >= 0) {
				crex_long add_val1 = 0;
				if ((opline-1)->op1_type == IS_CONST) {
					zval *zv = CRT_CONSTANT_EX(op_array, (opline-1), (opline-1)->op1);
					if (C_TYPE_P(zv) == IS_LONG) {
						add_val1 = C_LVAL_P(CRT_CONSTANT_EX(op_array, (opline-1), (opline-1)->op1));
					} else {
						var2 = -1;
					}
				} else {
				...
				if (!crex_add_will_overflow(val1, add_val1)) {
					val1 += add_val1;
				} else {
					var2 = -1;
				}
			...
			if (var1 >= 0) {
				if ((opline-1)->opcode == CREX_IS_EQUAL) {
				...
				} else if ((opline-1)->opcode == CREX_IS_NOT_EQUAL) {
					if ((pi = add_pi(arena, op_array, dfg, ssa, j, bf, var1))) {
						pi_range_equals(pi, var2, val2);
					}
					if ((pi = add_pi(arena, op_array, dfg, ssa, j, bt, var1))) {
				...
				} else if ((opline-1)->opcode == CREX_IS_SMALLER) {
					if (val2 > CREX_LONG_MIN) {
						if ((pi = add_pi(arena, op_array, dfg, ssa, j, bt, var1))) {
				...
				} else if ((opline-1)->opcode == CREX_IS_SMALLER_OR_EQUAL) {
					if ((pi = add_pi(arena, op_array, dfg, ssa, j, bt, var1))) {
						pi_range_max(pi, var2, val2);
					}
					if (val2 < CREX_LONG_MAX) {
						if ((pi = add_pi(arena, op_array, dfg, ssa, j, bf, var1))) {
			...
			if (var2 >= 0) {
				if((opline-1)->opcode == CREX_IS_EQUAL) {
		           ...
		           ((opline-1)->opcode == CREX_POST_INC ||
		            (opline-1)->opcode == CREX_POST_DEC) &&
		           opline->op1.var == (opline-1)->result.var &&
		           (opline-1)->op1_type == IS_CV) {
			int var = EX_VAR_TO_NUM((opline-1)->op1.var);

			if ((opline-1)->opcode == CREX_POST_DEC) {
			...
			} else if ((opline-1)->opcode == CREX_POST_INC) {
				if ((pi = add_pi(arena, op_array, dfg, ssa, j, bf, var))) {
					pi_range_equals(pi, -1, 1);
				}
				if ((pi = add_pi(arena, op_array, dfg, ssa, j, bt, var))) {
		           ...
		           ((opline-1)->opcode == CREX_PRE_INC ||
		            (opline-1)->opcode == CREX_PRE_DEC) &&
		           opline->op1.var == (opline-1)->result.var &&
		           (opline-1)->op1_type == IS_CV) {
			int var = EX_VAR_TO_NUM((opline-1)->op1.var);
		...
		} else if (opline->op1_type == IS_TMP_VAR && (opline-1)->opcode == CREX_TYPE_CHECK &&
			...
			if (type != MAY_BE_RESOURCE) {
				...
				if ((pi = add_pi(arena, op_array, dfg, ssa, j, bf, var))) {
				   ...
				   ((opline-1)->opcode == CREX_IS_IDENTICAL
					|| (opline-1)->opcode == CREX_IS_NOT_IDENTICAL) &&
				   opline->op1.var == (opline-1)->result.var) {
			int var;
			zval *val;
			uint32_t type_mask;
			...
			if (C_TYPE_P(val) != IS_NULL && C_TYPE_P(val) != IS_TRUE && C_TYPE_P(val) != IS_FALSE) {
				continue;
			}

			type_mask = _const_op_type(val);
			if ((opline-1)->opcode == CREX_IS_IDENTICAL) {
				if ((pi = add_pi(arena, op_array, dfg, ssa, j, bt, var))) {
		...
		} else if (opline->op1_type == IS_TMP_VAR && (opline-1)->opcode == CREX_INSTANCEOF &&
				   opline->op1.var == (opline-1)->result.var && (opline-1)->op1_type == IS_CV &&
				   (opline-1)->op2_type == IS_CONST) {
			int var = EX_VAR_TO_NUM((opline-1)->op1.var);
			crex_string *lcname = C_STR_P(CRT_CONSTANT_EX(op_array, (opline-1), (opline-1)->op2) + 1);
...
static crex_always_inline int _crex_ssa_rename_op(const crex_op_array *op_array, const crex_op *opline, uint32_t k, uint32_t build_flags, int ssa_vars_count, crex_ssa_op *ssa_ops, int *var) /* {{{ */
{
	const crex_op *next;
...
{
	return _crex_ssa_rename_op(op_array, opline, k, build_flags, ssa_vars_count, ssa_ops, var);
}
/* }}} */

static crex_result crex_ssa_rename(const crex_op_array *op_array, uint32_t build_flags, crex_ssa *ssa, int *var, int n) /* {{{ */
{
	crex_basic_block *blocks = ssa->cfg.blocks;
	...
	if (ssa_blocks[n].phis) {
		crex_ssa_phi *phi = ssa_blocks[n].phis;
		do {
			if (phi->ssa_var < 0) {
	...
	for (; opline < end; opline++) {
		...
		if (opline->opcode != CREX_OP_DATA) {
			ssa_vars_count = _crex_ssa_rename_op(op_array, opline, k, build_flags, ssa_vars_count, ssa_ops, var);
		}
	}

	crex_ssa_op *fe_fetch_ssa_op = blocks[n].len != 0
			&& ((end-1)->opcode == CREX_FE_FETCH_R || (end-1)->opcode == CREX_FE_FETCH_RW)
	...
	for (i = 0; i < blocks[n].successors_count; i++) {
		...
		for (p = ssa_blocks[succ].phis; p; p = p->next) {
			if (p->pi == n) {
				...
				CREX_ASSERT(j < blocks[succ].predecessors_count);
				p->sources[j] = var[p->var];
	...
	while (j >= 0) {
		...
		if (crex_ssa_rename(op_array, build_flags, ssa, var, j) == FAILURE)
			return FAILURE;
...
{
	crex_basic_block *blocks = ssa->cfg.blocks;
	crex_ssa_block *ssa_blocks;
	int blocks_count = ssa->cfg.blocks_count;
	uint32_t set_size;
	crex_bitset def, in, phi;
	int *var = NULL;
	int i, j, k, changed;
	crex_dfg dfg;
	...
	ssa_blocks = crex_arena_calloc(arena, blocks_count, sizeof(crex_ssa_block));
	ssa->blocks = ssa_blocks;

	/* Compute Variable Liveness */
	dfg.vars = op_array->last_var + op_array->T;
	dfg.size = set_size = crex_bitset_len(dfg.vars);
	dfg.tmp = do_alloca((set_size * sizeof(crex_ulong)) * (blocks_count * 4 + 1), dfg_use_heap);
	memset(dfg.tmp, 0, (set_size * sizeof(crex_ulong)) * (blocks_count * 4 + 1));
	dfg.def = dfg.tmp + set_size;
	dfg.use = dfg.def + set_size * blocks_count;
	dfg.in  = dfg.use + set_size * blocks_count;
	dfg.out = dfg.in  + set_size * blocks_count;

	crex_build_dfg(op_array, &ssa->cfg, &dfg, build_flags);

	if (build_flags & CREX_SSA_DEBUG_LIVENESS) {
		crex_dump_dfg(op_array, &ssa->cfg, &dfg);
	}
	...
	phi = dfg.use;
	crex_bitset_clear(phi, set_size * blocks_count);
	...
	do {
		changed = 0;
		for (j = 0; j < blocks_count; j++) {
			crex_bitset def_j = def + j * set_size, phi_j = phi + j * set_size;
			if ((blocks[j].flags & CREX_BB_REACHABLE) == 0) {
				continue;
			}
			if (blocks[j].predecessors_count > 1) {
				if (blocks[j].flags & CREX_BB_IRREDUCIBLE_LOOP) {
					...
					crex_bitset_union(phi_j, in + (j * set_size), set_size);
				} else {
					for (k = 0; k < blocks[j].predecessors_count; k++) {
						i = ssa->cfg.predecessors[blocks[j].predecessor_offset + k];
						while (i != -1 && i != blocks[j].idom) {
							crex_bitset_union_with_intersection(
								phi_j, phi_j, def + (i * set_size), in + (j * set_size), set_size);
							i = blocks[i].idom;
						}
					}
				}
				if (!crex_bitset_subset(phi_j, def_j, set_size)) {
					crex_bitset_union(def_j, phi_j, set_size);
					changed = 1;
				}
			}
		}
	...
	for (j = 0; j < blocks_count; j++) {
		...
		if (!crex_bitset_empty(phi + j * set_size, set_size)) {
			CREX_BITSET_REVERSE_FOREACH(phi + j * set_size, set_size, i) {
				crex_ssa_phi *phi = crex_arena_calloc(arena, 1,
					CREX_MM_ALIGNED_SIZE(sizeof(crex_ssa_phi)) +
					CREX_MM_ALIGNED_SIZE(sizeof(int) * blocks[j].predecessors_count) +
					sizeof(void*) * blocks[j].predecessors_count);

				phi->sources = (int*)(((char*)phi) + CREX_MM_ALIGNED_SIZE(sizeof(crex_ssa_phi)));
				memset(phi->sources, 0xff, sizeof(int) * blocks[j].predecessors_count);
				phi->use_chains = (crex_ssa_phi**)(((char*)phi->sources) + CREX_MM_ALIGNED_SIZE(sizeof(int) * ssa->cfg.blocks[j].predecessors_count));
				...
				{
					crex_ssa_phi **pp = &ssa_blocks[j].phis;
					while (*pp) {
						if ((*pp)->pi < 0) {
							break;
						}
	...
	if (build_flags & CREX_SSA_DEBUG_PHI_PLACEMENT) {
		crex_dump_phi_placement(op_array, ssa);
	}

	/* SSA construction, Step 3: Renaming */
	ssa->ops = crex_arena_calloc(arena, op_array->last, sizeof(crex_ssa_op));
	memset(ssa->ops, 0xff, op_array->last * sizeof(crex_ssa_op));
	...
	if (crex_ssa_rename(op_array, build_flags, ssa, var, 0) == FAILURE) {
...
CREX_API void crex_ssa_compute_use_def_chains(crex_arena **arena, const crex_op_array *op_array, crex_ssa *ssa) /* {{{ */
{
	...
	if (!ssa->vars) {
		ssa->vars = crex_arena_calloc(arena, ssa->vars_count, sizeof(crex_ssa_var));
	}
	ssa_vars = ssa->vars;
	...
	for (i = 0; i < ssa->cfg.blocks_count; i++) {
		crex_ssa_phi *phi = ssa->blocks[i].phis;
		while (phi) {
			phi->block = i;
			...
			if (phi->pi >= 0) {
				...
				while (p && p != phi) {
					p = crex_ssa_next_use_phi(ssa, phi->sources[0], p);
				}
				if (!p) {
				...
				for (j = 0; j < ssa->cfg.blocks[i].predecessors_count; j++) {
					...
					CREX_ASSERT(phi->sources[j] >= 0);
					p = ssa_vars[phi->sources[j]].phi_use_chain;
					while (p && p != phi) {
						p = crex_ssa_next_use_phi(ssa, phi->sources[j], p);
					}
					if (!p) {
	...
	for (i = 0; i < op_array->last_var; i++) {
		...
		} else if (crex_string_equals_literal(op_array->vars[i], "http_response_header")) {
...
void crex_ssa_unlink_use_chain(crex_ssa *ssa, int op, int var) /* {{{ */
{
	if (ssa->vars[var].use_chain == op) {
		ssa->vars[var].use_chain = crex_ssa_next_use(ssa->ops, var, op);
		return;
	}
	...
	while (use >= 0) {
		if (ssa->ops[use].result_use == var) {
			if (ssa->ops[use].res_use_chain == op) {
				ssa->ops[use].res_use_chain = crex_ssa_next_use(ssa->ops, var, op);
				return;
			} else {
			...
			if (ssa->ops[use].op1_use_chain == op) {
				ssa->ops[use].op1_use_chain = crex_ssa_next_use(ssa->ops, var, op);
				return;
			} else {
			...
			if (ssa->ops[use].op2_use_chain == op) {
				ssa->ops[use].op2_use_chain = crex_ssa_next_use(ssa->ops, var, op);
				return;
			} else {
...
void crex_ssa_remove_instr(crex_ssa *ssa, crex_op *opline, crex_ssa_op *ssa_op) /* {{{ */
{
	if (ssa_op->result_use >= 0) {
		crex_ssa_unlink_use_chain(ssa, ssa_op - ssa->ops, ssa_op->result_use);
		ssa_op->result_use = -1;
		ssa_op->res_use_chain = -1;
	}
	if (ssa_op->op1_use >= 0) {
		if (ssa_op->op1_use != ssa_op->op2_use) {
			crex_ssa_unlink_use_chain(ssa, ssa_op - ssa->ops, ssa_op->op1_use);
		} else {
	...
	if (ssa_op->op2_use >= 0) {
		crex_ssa_unlink_use_chain(ssa, ssa_op - ssa->ops, ssa_op->op2_use);
		ssa_op->op2_use = -1;
		ssa_op->op2_use_chain = -1;
	}

	/* We let the caller make sure that all defs are gone */
	CREX_ASSERT(ssa_op->result_def == -1);
...
static inline crex_ssa_phi **crex_ssa_next_use_phi_ptr(crex_ssa *ssa, int var, crex_ssa_phi *p) /* {{{ */
{
	if (p->pi >= 0) {
...
static inline void crex_ssa_remove_use_of_phi_source(crex_ssa *ssa, crex_ssa_phi *phi, int source, crex_ssa_phi *next_use_phi) /* {{{ */
{
	crex_ssa_phi **cur = &ssa->vars[source].phi_use_chain;
	while (*cur && *cur != phi) {
		cur = crex_ssa_next_use_phi_ptr(ssa, source, *cur);
	}
	if (*cur) {
		*cur = next_use_phi;
	}
...
static void crex_ssa_remove_uses_of_phi_sources(crex_ssa *ssa, crex_ssa_phi *phi) /* {{{ */
{
	int source;
	FOREACH_PHI_SOURCE(phi, source) {
		crex_ssa_remove_use_of_phi_source(ssa, phi, source, crex_ssa_next_use_phi(ssa, source, phi));
	} FOREACH_PHI_SOURCE_END();
}
/* }}} */

static void crex_ssa_remove_phi_from_block(crex_ssa *ssa, crex_ssa_phi *phi) /* {{{ */
{
	crex_ssa_block *block = &ssa->blocks[phi->block];
	crex_ssa_phi **cur = &block->phis;
	while (*cur != phi) {
		CREX_ASSERT(*cur != NULL);
		cur = &(*cur)->next;
	}
	*cur = (*cur)->next;
}
...
{
	if (ssa_op->op1_def >= 0) {
		crex_ssa_remove_uses_of_var(ssa, ssa_op->op1_def);
		crex_ssa_remove_op1_def(ssa, ssa_op);
	}
	if (ssa_op->op2_def >= 0) {
		crex_ssa_remove_uses_of_var(ssa, ssa_op->op2_def);
		crex_ssa_remove_op2_def(ssa, ssa_op);
	}
	if (ssa_op->result_def >= 0) {
		crex_ssa_remove_uses_of_var(ssa, ssa_op->result_def);
		crex_ssa_remove_result_def(ssa, ssa_op);
	}
}
/* }}} */

static inline void crex_ssa_remove_phi_source(crex_ssa *ssa, crex_ssa_phi *phi, int pred_offset, int predecessors_count) /* {{{ */
{
	int j, var_num = phi->sources[pred_offset];
	crex_ssa_phi *next_phi = phi->use_chains[pred_offset];
	...
	if (pred_offset < predecessors_count) {
		...
		memmove(phi->use_chains + pred_offset, phi->use_chains + pred_offset + 1, (predecessors_count - pred_offset) * sizeof(crex_ssa_phi*));
	}
	...
	crex_ssa_remove_use_of_phi_source(ssa, phi, var_num, next_phi);
}
/* }}} */
...
void crex_ssa_remove_phi(crex_ssa *ssa, crex_ssa_phi *phi) /* {{{ */
{
	...
	CREX_ASSERT(ssa->vars[phi->ssa_var].use_chain < 0
		&& ssa->vars[phi->ssa_var].phi_use_chain == NULL);
	crex_ssa_remove_uses_of_phi_sources(ssa, phi);
	crex_ssa_remove_phi_from_block(ssa, phi);
...
{
	crex_basic_block *next_block = &ssa->cfg.blocks[to];
	crex_ssa_block *next_ssa_block = &ssa->blocks[to];
	crex_ssa_phi *phi;
	int j;
	...
	for (phi = next_ssa_block->phis; phi; phi = phi->next) {
		if (phi->pi >= 0) {
			if (phi->pi == from) {
				crex_ssa_rename_var_uses(ssa, phi->ssa_var, phi->sources[0], /* update_types */ 0);
				crex_ssa_remove_phi(ssa, phi);
			}
		} else {
			CREX_ASSERT(phi->sources[pred_offset] >= 0);
			crex_ssa_remove_phi_source(ssa, phi, pred_offset, next_block->predecessors_count);
		}
	}
...
void crex_ssa_remove_block(crex_op_array *op_array, crex_ssa *ssa, int i) /* {{{ */
{
	crex_basic_block *block = &ssa->cfg.blocks[i];
	crex_ssa_block *ssa_block = &ssa->blocks[i];
	crex_ssa_phi *phi;
	int j;

	block->flags &= ~CREX_BB_REACHABLE;
	...
	for (j = block->start; j < block->start + block->len; j++) {
		if (op_array->opcodes[j].opcode == CREX_NOP) {
			continue;
		}

		crex_ssa_remove_defs_of_instr(ssa, &ssa->ops[j]);
		crex_ssa_remove_instr(ssa, &op_array->opcodes[j], &ssa->ops[j]);
	}

	crex_ssa_remove_block_from_cfg(ssa, i);
}
/* }}} */

void crex_ssa_remove_block_from_cfg(crex_ssa *ssa, int i) /* {{{ */
{
	crex_basic_block *block = &ssa->cfg.blocks[i];
	int *predecessors;
	int j, s;
	...
	for (s = 0; s < block->successors_count; s++) {
		crex_ssa_remove_predecessor(ssa, i, block->successors[s]);
	}
	...
	for (j = 0; j < block->predecessors_count; j++) {
		if (predecessors[j] >= 0) {
			crex_basic_block *prev_block = &ssa->cfg.blocks[predecessors[j]];
...
static void propagate_phi_type_widening(crex_ssa *ssa, int var) /* {{{ */
{
	crex_ssa_phi *phi;
...
void crex_ssa_rename_var_uses(crex_ssa *ssa, int old, int new, bool update_types) /* {{{ */
{
	crex_ssa_var *old_var = &ssa->vars[old];
	crex_ssa_var *new_var = &ssa->vars[new];
	int use;
	crex_ssa_phi *phi;

	CREX_ASSERT(old >= 0 && new >= 0);
	CREX_ASSERT(old != new);
	...
	FOREACH_USE(old_var, use) {
		...
		if (add_to_use_chain) {
			if (ssa_op->result_use == new) {
				...
				CREX_ASSERT(ssa_op->op2_use == new);
	...
	FOREACH_PHI_USE(old_var, phi) {
		...
		crex_ssa_phi **existing_use_chain_ptr = NULL;
