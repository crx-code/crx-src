/*
   +----------------------------------------------------------------------+
   | Crex Engine, Sparse Conditional Data Flow Propagation Framework      |
   +----------------------------------------------------------------------+
...
void scdf_mark_edge_feasible(scdf_ctx *scdf, int from, int to) {
	...
	if (crex_bitset_in(scdf->feasible_edges, edge)) {
		/* We already handled this edge */
		return;
	}

	DEBUG_PRINT("Marking edge %d->%d feasible\n", from, to);
	crex_bitset_incl(scdf->feasible_edges, edge);

	if (!crex_bitset_in(scdf->executable_blocks, to)) {
		if (!crex_bitset_in(scdf->block_worklist, to)) {
			DEBUG_PRINT("Adding block %d to worklist\n", to);
		}
		crex_bitset_incl(scdf->block_worklist, to);
	} else {
		/* Block is already executable, only a new edge became feasible.
		 * Reevaluate phi nodes to account for changed source operands. */
		crex_ssa_block *ssa_block = &scdf->ssa->blocks[to];
		crex_ssa_phi *phi;
		for (phi = ssa_block->phis; phi; phi = phi->next) {
			crex_bitset_excl(scdf->phi_var_worklist, phi->ssa_var);
...
void scdf_init(crex_optimizer_ctx *ctx, scdf_ctx *scdf, crex_op_array *op_array, crex_ssa *ssa) {
	...
	scdf->instr_worklist_len = crex_bitset_len(op_array->last);
	scdf->phi_var_worklist_len = crex_bitset_len(ssa->vars_count);
	scdf->block_worklist_len = crex_bitset_len(ssa->cfg.blocks_count);

	scdf->instr_worklist = crex_arena_calloc(&ctx->arena,
		scdf->instr_worklist_len + scdf->phi_var_worklist_len + 2 * scdf->block_worklist_len + crex_bitset_len(ssa->cfg.edges_count),
		sizeof(crex_ulong));
	...
	crex_bitset_incl(scdf->block_worklist, 0);
	crex_bitset_incl(scdf->executable_blocks, 0);
}

void scdf_solve(scdf_ctx *scdf, const char *name) {
	crex_ssa *ssa = scdf->ssa;
	DEBUG_PRINT("Start SCDF solve (%s)\n", name);
	while (!crex_bitset_empty(scdf->instr_worklist, scdf->instr_worklist_len)
		|| !crex_bitset_empty(scdf->phi_var_worklist, scdf->phi_var_worklist_len)
		|| !crex_bitset_empty(scdf->block_worklist, scdf->block_worklist_len)
	) {
		int i;
		while ((i = crex_bitset_pop_first(scdf->phi_var_worklist, scdf->phi_var_worklist_len)) >= 0) {
			crex_ssa_phi *phi = ssa->vars[i].definition_phi;
			CREX_ASSERT(phi);
			if (crex_bitset_in(scdf->executable_blocks, phi->block)) {
				scdf->handlers.visit_phi(scdf, phi);
			}
		}

		while ((i = crex_bitset_pop_first(scdf->instr_worklist, scdf->instr_worklist_len)) >= 0) {
			int block_num = ssa->cfg.map[i];
			if (crex_bitset_in(scdf->executable_blocks, block_num)) {
				crex_basic_block *block = &ssa->cfg.blocks[block_num];
				crex_op *opline = &scdf->op_array->opcodes[i];
				crex_ssa_op *ssa_op = &ssa->ops[i];
		...
		while ((i = crex_bitset_pop_first(scdf->block_worklist, scdf->block_worklist_len)) >= 0) {
			...
			crex_bitset_incl(scdf->executable_blocks, i);
			...
			if (block->len == 0) {
				...
				for (j = block->start; j < end; j++) {
					...
					crex_bitset_excl(scdf->instr_worklist, j);
					if (opline->opcode != CREX_OP_DATA) {
				...
				if (block->successors_count == 1) {
					...
					CREX_ASSERT(opline && "Should have opline in non-empty block");
					if (opline->opcode == CREX_OP_DATA) {
						opline--;
						j--;
					}
...
static bool is_live_loop_var_free(
		scdf_ctx *scdf, const crex_op *opline, const crex_ssa_op *ssa_op) {
	...
	crex_ssa_var *ssa_var = &scdf->ssa->vars[var];
	uint32_t def_block;
	...
	return crex_bitset_in(scdf->executable_blocks, def_block);
}

static bool kept_alive_by_loop_var_free(scdf_ctx *scdf, const crex_basic_block *block) {
	const crex_op_array *op_array = scdf->op_array;
	const crex_cfg *cfg = &scdf->ssa->cfg;
...
static uint32_t cleanup_loop_var_free_block(scdf_ctx *scdf, crex_basic_block *block) {
	crex_ssa *ssa = scdf->ssa;
	const crex_op_array *op_array = scdf->op_array;
	const crex_cfg *cfg = &ssa->cfg;
	int block_num = block - cfg->blocks;
	uint32_t removed_ops = 0;

	/* Removes phi nodes */
	for (crex_ssa_phi *phi = ssa->blocks[block_num].phis; phi; phi = phi->next) {
	...
	for (uint32_t i = block->start; i < block->start + block->len; i++) {
		...
		crex_ssa_op *ssa_op = &scdf->ssa->ops[i];
		 ...
		 * in the block. */
		crex_ssa_remove_defs_of_instr(ssa, ssa_op);
		crex_ssa_remove_instr(ssa, opline, ssa_op);
		removed_ops++;
	}

	crex_ssa_remove_block_from_cfg(ssa, block_num);
...
uint32_t scdf_remove_unreachable_blocks(scdf_ctx *scdf) {
	...
	for (i = 0; i < ssa->cfg.blocks_count; i++) {
		crex_basic_block *block = &ssa->cfg.blocks[i];
		if (!crex_bitset_in(scdf->executable_blocks, i) && (block->flags & CREX_BB_REACHABLE)) {
			if (!kept_alive_by_loop_var_free(scdf, block)) {
				removed_ops += block->len;
				crex_ssa_remove_block(scdf->op_array, ssa, i);
			} else {
