...
#ifndef CREX_DEBUG_DFA
# define CREX_DEBUG_DFA CREX_DEBUG
#endif
...
crex_result crex_dfa_analyze_op_array(crex_op_array *op_array, crex_optimizer_ctx *ctx, crex_ssa *ssa)
{
	...
	crex_build_cfg(&ctx->arena, op_array, CREX_CFG_NO_ENTRY_PREDECESSORS, &ssa->cfg);

	if ((ssa->cfg.flags & CREX_FUNC_INDIRECT_VAR_ACCESS)) {
		/* TODO: we can't analyze functions with indirect variable access ??? */
		return FAILURE;
	}

	crex_cfg_build_predecessors(&ctx->arena, &ssa->cfg);

	if (ctx->debug_level & CREX_DUMP_DFA_CFG) {
		crex_dump_op_array(op_array, CREX_DUMP_CFG, "dfa cfg", &ssa->cfg);
	}

	/* Compute Dominators Tree */
	crex_cfg_compute_dominators_tree(op_array, &ssa->cfg);

	/* Identify reducible and irreducible loops */
	crex_cfg_identify_loops(op_array, &ssa->cfg);

	if (ctx->debug_level & CREX_DUMP_DFA_DOMINATORS) {
		crex_dump_dominators(op_array, &ssa->cfg);
	}

	build_flags = 0;
	if (ctx->debug_level & CREX_DUMP_DFA_LIVENESS) {
		build_flags |= CREX_SSA_DEBUG_LIVENESS;
	}
	if (ctx->debug_level & CREX_DUMP_DFA_PHI) {
		build_flags |= CREX_SSA_DEBUG_PHI_PLACEMENT;
	}
	if (crex_build_ssa(&ctx->arena, ctx->script, op_array, build_flags, ssa) == FAILURE) {
		return FAILURE;
	}

	if (ctx->debug_level & CREX_DUMP_DFA_SSA) {
		crex_dump_op_array(op_array, CREX_DUMP_SSA, "dfa ssa", ssa);
	}


	crex_ssa_compute_use_def_chains(&ctx->arena, op_array, ssa);

	crex_ssa_find_false_dependencies(op_array, ssa);

	crex_ssa_find_sccs(op_array, ssa);

	if (crex_ssa_inference(&ctx->arena, op_array, ctx->script, ssa, ctx->optimization_level) == FAILURE) {
		return FAILURE;
	}

	if (crex_ssa_escape_analysis(ctx->script, op_array, ssa) == FAILURE) {
		return FAILURE;
	}

	if (ctx->debug_level & CREX_DUMP_DFA_SSA_VARS) {
		crex_dump_ssa_variables(op_array, ssa, 0);
	}

	return SUCCESS;
}

static void crex_ssa_remove_nops(crex_op_array *op_array, crex_ssa *ssa, crex_optimizer_ctx *ctx)
{
	crex_basic_block *blocks = ssa->cfg.blocks;
	crex_basic_block *blocks_end = blocks + ssa->cfg.blocks_count;
	crex_basic_block *b;
	...
	if (func_info) {
		...
		while ((*call_info)) {
			if ((*call_info)->caller_init_opline->opcode == CREX_NOP) {
		...
		if (b->flags & (CREX_BB_REACHABLE|CREX_BB_UNREACHABLE_FREE)) {
			if (b->len) {
				...
				if (b->flags & CREX_BB_UNREACHABLE_FREE) {
					...
					CREX_ASSERT(op_array->opcodes[b->start].opcode == CREX_FREE
							|| op_array->opcodes[b->start].opcode == CREX_FE_FREE);
					b->len = 1;
				}
				...
				while (i < old_end) {
					shiftlist[i] = i - target;
					if (EXPECTED(op_array->opcodes[i].opcode != CREX_NOP)) {
						if (i != target) {
	...
	if (target != op_array->last) {
		...
		for (b = blocks; b < blocks_end; b++) {
			if ((b->flags & CREX_BB_REACHABLE) && b->len != 0) {
				crex_op *opline = op_array->opcodes + b->start + b->len - 1;
...
static inline bool can_elide_list_type(
	const crex_script *script, const crex_op_array *op_array,
	const crex_ssa_var_info *use_info, crex_type type)
{
	...
	bool is_intersection = CREX_TYPE_IS_INTERSECTION(type);
	CREX_TYPE_FOREACH(type, single_type) {
		...
		if (CREX_TYPE_HAS_NAME(*single_type)) {
			crex_string *lcname = crex_string_tolower(CREX_TYPE_NAME(*single_type));
			crex_class_entry *ce = crex_optimizer_get_class_entry(script, op_array, lcname);
...
static inline bool can_elide_return_type_check(
		const crex_script *script, crex_op_array *op_array, crex_ssa *ssa, crex_ssa_op *ssa_op) {
	crex_arg_info *arg_info = &op_array->arg_info[-1];
	crex_ssa_var_info *use_info = &ssa->var_info[ssa_op->op1_use];
	...
	uint32_t disallowed_types = use_type & ~CREX_TYPE_PURE_MASK(arg_info->type);
	if (!disallowed_types) {
		/* Only contains allowed types. */
		return true;
	}

	if (disallowed_types == MAY_BE_OBJECT && use_info->ce && CREX_TYPE_IS_COMPLEX(arg_info->type)) {
...
static bool opline_supports_assign_contraction(
		crex_op_array *op_array, crex_ssa *ssa, crex_op *opline, int src_var, uint32_t cv_var) {
	if (opline->opcode == CREX_NEW) {
		/* see Crex/tests/generators/aborted_yield_during_new.crxt */
		return 0;
	}

	if (opline->opcode == CREX_DO_ICALL || opline->opcode == CREX_DO_UCALL
			|| opline->opcode == CREX_DO_FCALL || opline->opcode == CREX_DO_FCALL_BY_NAME) {
	...
	if (opline->opcode == CREX_POST_INC || opline->opcode == CREX_POST_DEC) {
	...
	if ((opline->opcode == CREX_ASSIGN_OP
	 ...
	 && crex_may_throw(opline, &ssa->ops[ssa->vars[src_var].definition], op_array, ssa)) {
		return 0;
	}
...
static bool variable_defined_or_used_in_range(crex_ssa *ssa, int var, int start, int end)
{
	while (start < end) {
		const crex_ssa_op *ssa_op = &ssa->ops[start];
...
int crex_dfa_optimize_calls(crex_op_array *op_array, crex_ssa *ssa)
{
	...
	if (func_info->callee_info) {
		...
		do {
			if (call_info->caller_call_opline
			 && call_info->caller_call_opline->opcode == CREX_DO_ICALL
			 && call_info->callee_func
			 && crex_string_equals_literal(call_info->callee_func->common.function_name, "in_array")
			 && (call_info->caller_init_opline->extended_value == 2
			  || (call_info->caller_init_opline->extended_value == 3
			   && (call_info->caller_call_opline - 1)->opcode == CREX_SEND_VAL
			   && (call_info->caller_call_opline - 1)->op1_type == IS_CONST))) {
				...
				if (call_info->caller_init_opline->extended_value == 2) {
					...
					if (crex_is_true(CT_CONSTANT_EX(op_array, (call_info->caller_call_opline - 1)->op1.constant))) {
						strict = 1;
					}
				...
				if (send_array->opcode == CREX_SEND_VAL
				 && send_array->op1_type == IS_CONST
				 && C_TYPE_P(CT_CONSTANT_EX(op_array, send_array->op1.constant)) == IS_ARRAY
				 && (send_needly->opcode == CREX_SEND_VAL
				  || send_needly->opcode == CREX_SEND_VAR)
			    ) {
					bool ok = 1;

					HashTable *src = C_ARRVAL_P(CT_CONSTANT_EX(op_array, send_array->op1.constant));
					HashTable *dst;
					zval *val, tmp;
					crex_ulong idx;

					ZVAL_TRUE(&tmp);
					dst = crex_new_array(crex_hash_num_elements(src));
					if (strict) {
						...
						CREX_HASH_FOREACH_VAL(src, val) {
							if (C_TYPE_P(val) != IS_STRING || CREX_HANDLE_NUMERIC(C_STR_P(val), idx)) {
								crex_array_destroy(dst);
								ok = 0;
								break;
							}
					...
					if (ok) {
						...
						crex_ssa_op *ssa_op = ssa->ops + op_num;
						...
						if (ssa_op->op1_use >= 0) {
							...
							CREX_ASSERT(ssa_op->op1_def < 0);
							crex_ssa_unlink_use_chain(ssa, op_num, ssa_op->op1_use);
							ssa_op->op1_use = -1;
						...
						/* Update opcode */
						call_info->caller_call_opline->opcode = CREX_IN_ARRAY;
						...
						call_info->caller_call_opline->op2.constant = crex_optimizer_add_literal(op_array, &tmp);
						...
						if (ssa_op->result_def >= 0) {
							...
							if (ssa->vars[var].phi_use_chain == NULL
								...
								&& (op_array->opcodes[use].opcode == CREX_JMPZ
									|| op_array->opcodes[use].opcode == CREX_JMPNZ)) {
...
static crex_always_inline void take_successor_0(crex_ssa *ssa, int block_num, crex_basic_block *block)
{
	if (block->successors_count == 2) {
		if (block->successors[1] != block->successors[0]) {
			crex_ssa_remove_predecessor(ssa, block_num, block->successors[1]);
		}
		block->successors_count = 1;
	}
}

static crex_always_inline void take_successor_1(crex_ssa *ssa, int block_num, crex_basic_block *block)
{
	if (block->successors_count == 2) {
		if (block->successors[1] != block->successors[0]) {
			crex_ssa_remove_predecessor(ssa, block_num, block->successors[0]);
...
static crex_always_inline void take_successor_ex(crex_ssa *ssa, int block_num, crex_basic_block *block, int target_block)
{
	int i;

	for (i = 0; i < block->successors_count; i++) {
		if (block->successors[i] != target_block) {
			crex_ssa_remove_predecessor(ssa, block_num, block->successors[i]);
		}
	}
...
static void compress_block(crex_op_array *op_array, crex_basic_block *block)
{
	while (block->len > 0) {
		crex_op *opline = &op_array->opcodes[block->start + block->len - 1];
...
static void replace_predecessor(crex_ssa *ssa, int block_id, int old_pred, int new_pred) {
	crex_basic_block *block = &ssa->cfg.blocks[block_id];
...
static void crex_ssa_replace_control_link(crex_op_array *op_array, crex_ssa *ssa, int from, int to, int new_to)
{
	crex_basic_block *src = &ssa->cfg.blocks[from];
	crex_basic_block *old = &ssa->cfg.blocks[to];
	crex_basic_block *dst = &ssa->cfg.blocks[new_to];
	int i;
	crex_op *opline;
	...
	if (src->len > 0) {
		...
		switch (opline->opcode) {
			case CREX_JMP:
			case CREX_FAST_CALL:
				CREX_ASSERT(CREX_OP1_JMP_ADDR(opline) == op_array->opcodes + old->start);
				CREX_SET_OP_JMP_ADDR(opline, opline->op1, op_array->opcodes + dst->start);
				break;
			case CREX_JMPZ:
			case CREX_JMPNZ:
			case CREX_JMPC_EX:
			...
			case CREX_BIND_INIT_STATIC_OR_JMP:
				if (CREX_OP2_JMP_ADDR(opline) == op_array->opcodes + old->start) {
					CREX_SET_OP_JMP_ADDR(opline, opline->op2, op_array->opcodes + dst->start);
				}
				break;
			case CREX_CATCH:
			...
			case CREX_FE_FETCH_RW:
				if (CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, opline->extended_value) == old->start) {
					opline->extended_value = CREX_OPLINE_NUM_TO_OFFSET(op_array, opline, dst->start);
				}
				break;
			...
			case CREX_MATCH:
				{
					...
					CREX_HASH_FOREACH_VAL(jumptable, zv) {
						if (CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, C_LVAL_P(zv)) == old->start) {
							C_LVAL_P(zv) = CREX_OPLINE_NUM_TO_OFFSET(op_array, opline, dst->start);
						}
					} CREX_HASH_FOREACH_END();
					if (CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, opline->extended_value) == old->start) {
						opline->extended_value = CREX_OPLINE_NUM_TO_OFFSET(op_array, opline, dst->start);
...
static void crex_ssa_unlink_block(crex_op_array *op_array, crex_ssa *ssa, crex_basic_block *block, int block_num)
{
	if (block->predecessors_count == 1 && ssa->blocks[block_num].phis == NULL) {
		int *predecessors, i;
		crex_basic_block *fe_fetch_block = NULL;

		CREX_ASSERT(block->successors_count == 1);
		predecessors = &ssa->cfg.predecessors[block->predecessor_offset];
		if (block->predecessors_count == 1 && (block->flags & CREX_BB_FOLLOW)) {
			crex_basic_block *pred_block = &ssa->cfg.blocks[predecessors[0]];

			if (pred_block->len > 0 && (pred_block->flags & CREX_BB_REACHABLE)) {
				if ((op_array->opcodes[pred_block->start + pred_block->len - 1].opcode == CREX_FE_FETCH_R
				 || op_array->opcodes[pred_block->start + pred_block->len - 1].opcode == CREX_FE_FETCH_RW)
		...
		for (i = 0; i < block->predecessors_count; i++) {
			crex_ssa_replace_control_link(op_array, ssa, predecessors[i], block_num, block->successors[0]);
		}
		crex_ssa_remove_block(op_array, ssa, block_num);
		if (fe_fetch_block && fe_fetch_block->successors[0] == fe_fetch_block->successors[1]) {
			...
			if (ssa_var >= 0) {
				crex_ssa_remove_uses_of_var(ssa, ssa_var);
			}
		}
	}
}

static int crex_dfa_optimize_jmps(crex_op_array *op_array, crex_ssa *ssa)
{
	int removed_ops = 0;
	int block_num = 0;

	for (block_num = 1; block_num < ssa->cfg.blocks_count; block_num++) {
		crex_basic_block *block = &ssa->cfg.blocks[block_num];
		...
		if (block->len == 0) {
			crex_ssa_unlink_block(op_array, ssa, block, block_num);
		}
	}

	block_num = 0;
	while (block_num < ssa->cfg.blocks_count
		&& !(ssa->cfg.blocks[block_num].flags & CREX_BB_REACHABLE)) {
		block_num++;
	}
	while (block_num < ssa->cfg.blocks_count) {
		...
		while (next_block_num < ssa->cfg.blocks_count
			&& !(ssa->cfg.blocks[next_block_num].flags & CREX_BB_REACHABLE)) {
			if (ssa->cfg.blocks[next_block_num].flags & CREX_BB_UNREACHABLE_FREE) {
				can_follow = 0;
			}
			next_block_num++;
		}
...
optimize_jmpz:
					if (opline->op1_type == IS_CONST) {
						if (crex_is_true(CT_CONSTANT_EX(op_array, opline->op1.constant))) {
							MAKE_NOP(opline);
							removed_ops++;
...
optimize_jmpnz:
				...
				case CREX_COALESCE:
				{
					...
					if (opline->op1_type == IS_CONST
						...
						if (C_TYPE_P(CT_CONSTANT_EX(op_array, opline->op1.constant)) == IS_NULL) {
							crex_ssa_remove_result_def(ssa, ssa_op);
							MAKE_NOP(opline);
							removed_ops++;
							take_successor_1(ssa, block_num, block);
				...
				case CREX_MATCH:
					if (opline->op1_type == IS_CONST) {
						...
						if (!correct_type && opline->opcode != CREX_MATCH) {
							removed_ops++;
							MAKE_NOP(opline);
						...
						if (correct_type) {
							...
							zval *jmp_zv = type == IS_LONG
								? crex_hash_index_find(jmptable, C_LVAL_P(zv))
								: crex_hash_find(jmptable, C_STR_P(zv));

							if (jmp_zv) {
								target = CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, C_LVAL_P(jmp_zv));
							} else {
								target = CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, opline->extended_value);
							}
						} else {
							CREX_ASSERT(opline->opcode == CREX_MATCH);
							target = CREX_OFFSET_TO_OPLINE_NUM(op_array, opline, opline->extended_value);
						}
						opline->opcode = CREX_JMP;
						opline->extended_value = 0;
						SET_UNUSED(opline->op1);
						CREX_SET_OP_JMP_ADDR(opline, opline->op1, op_array->opcodes + target);
...
static bool crex_dfa_try_to_replace_result(crex_op_array *op_array, crex_ssa *ssa, int def, int cv_var)
{
	...
	if (result_var >= 0
	 ...
	 && ssa->vars[result_var].sym_use_chain == NULL) {
		...
		if (use >= 0
		 && crex_ssa_next_use(ssa->ops, result_var, use) < 0
		 && op_array->opcodes[use].opcode != CREX_FREE
		 && op_array->opcodes[use].opcode != CREX_SEND_VAL
		 && op_array->opcodes[use].opcode != CREX_SEND_VAL_EX
		 && op_array->opcodes[use].opcode != CREX_VERIFY_RETURN_TYPE
		 && op_array->opcodes[use].opcode != CREX_YIELD) {
			if (use > def) {
				int i = use;
				const crex_op *opline = &op_array->opcodes[use];
...
{
	if (ctx->debug_level & CREX_DUMP_BEFORE_DFA_PASS) {
		crex_dump_op_array(op_array, CREX_DUMP_SSA, "before dfa pass", ssa);
	}
	...
	if (ssa->var_info) {
		...
		if (CREX_OPTIMIZER_PASS_8 & ctx->optimization_level) {
			if (sccp_optimize_op_array(ctx, op_array, ssa, call_map)) {
				remove_nops = 1;
			}

			if (crex_dfa_optimize_jmps(op_array, ssa)) {
				remove_nops = 1;
			}
			...
			if (CREX_FUNC_INFO(op_array)) {
				if (crex_dfa_optimize_calls(op_array, ssa)) {
					remove_nops = 1;
				}
			}
			if (ctx->debug_level & CREX_DUMP_AFTER_PASS_8) {
				crex_dump_op_array(op_array, CREX_DUMP_SSA, "after sccp pass", ssa);
			}
#if CREX_DEBUG_DFA
		...
		if (CREX_OPTIMIZER_PASS_14 & ctx->optimization_level) {
			...
			if (ctx->debug_level & CREX_DUMP_AFTER_PASS_14) {
				crex_dump_op_array(op_array, CREX_DUMP_SSA, "after dce pass", ssa);
			}
#if CREX_DEBUG_DFA
...
// op_1: ASSIGN ? -> #v [use_as_double], long(?) => ASSIGN ? -> #v, double(?)
					...
					CREX_ASSERT(C_TYPE_INFO_P(zv) == IS_LONG);
...
// op_1: QM_ASSIGN #v [use_as_double], long(?) => QM_ASSIGN #v, double(?)
					...
					CREX_ASSERT(C_TYPE_INFO_P(zv) == IS_LONG);
					ZVAL_DOUBLE(&tmp, zval_get_double(zv));
					opline->op1.constant = crex_optimizer_add_literal(op_array, &tmp);
				}
				...
				if (opline->opcode == CREX_ADD
				 ...
				 || opline->opcode == CREX_IS_NOT_EQUAL
				 || opline->opcode == CREX_IS_SMALLER
				 || opline->opcode == CREX_IS_SMALLER_OR_EQUAL
				) {
...
// op_1: #v.? = MUL 2, #x.? [double,long] => #v.? = ADD #x.?, #x.?
						...
						if ((OP1_INFO() & MAY_BE_ANY) == MAY_BE_DOUBLE
						 && C_TYPE_INFO_P(CT_CONSTANT_EX(op_array, opline->op2.constant)) == IS_LONG) {
...
// op_1: #v.? = ADD #?.? [double], long(?) => #v.? = ADD #?.? [double], double(?)
						...
						if (opline->opcode == CREX_ADD || opline->opcode == CREX_SUB) {
...
// op_1: #v.? = MUL #x.? [double,long], 2 => #v.? = ADD #x.?, #x.?
				...
				} else if (opline->opcode == CREX_VERIFY_RETURN_TYPE
				 && opline->op1_type != IS_CONST
				 && ssa->ops[op_1].op1_def == v
...
// op_1: VERIFY_RETURN_TYPE #orig_var.? [T] -> #v.? [T] => NOP
						...
						crex_ssa_unlink_use_chain(ssa, op_1, orig_var);
...
// op_1: VERIFY_RETURN_TYPE #orig_var.? [T] -> #v.? [T] => NOP
						...
						crex_ssa_replace_use_chain(ssa, op_1, ret, orig_var);
...
// op_1: QM_ASSIGN #src_var.T #orig_var.CV [undef,scalar] -> #v.CV,    NOP
					...
					if (op_array->opcodes[op_2].opcode == CREX_SUB
					 ...
					 && C_TYPE_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op2.constant)) == IS_LONG
					 && C_LVAL_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op2.constant)) == 1
					 ...
					 && !(ssa->var_info[ssa->ops[op_2].op1_use].type & (MAY_BE_FALSE|MAY_BE_TRUE|MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_REF))) {
						...
						op_array->opcodes[op_2].opcode = CREX_PRE_DEC;
					...
					} else if (op_array->opcodes[op_2].opcode == CREX_ADD
					 && op_array->opcodes[op_2].op1_type == op_array->opcodes[op_2].result_type
					 && op_array->opcodes[op_2].op1.var == op_array->opcodes[op_2].result.var
					 && op_array->opcodes[op_2].op2_type == IS_CONST
					 && C_TYPE_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op2.constant)) == IS_LONG
					 ...
					 && !(ssa->var_info[ssa->ops[op_2].op1_use].type & (MAY_BE_FALSE|MAY_BE_TRUE|MAY_BE_STRING|MAY_BE_ARRAY|MAY_BE_OBJECT|MAY_BE_RESOURCE|MAY_BE_REF))) {
						...
						op_array->opcodes[op_2].opcode = CREX_PRE_INC;
						SET_UNUSED(op_array->opcodes[op_2].op2);
						SET_UNUSED(op_array->opcodes[op_2].result);
					 ...
					 && C_TYPE_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op1.constant)) == IS_LONG
					 && C_LVAL_P(CT_CONSTANT_EX(op_array, op_array->opcodes[op_2].op1.constant)) == 1
			...
			if (ssa->ops[op_1].op1_def == v
			 && RETURN_VALUE_USED(opline)) {
				if (opline->opcode == CREX_ASSIGN
				 || opline->opcode == CREX_ASSIGN_OP
				 || opline->opcode == CREX_PRE_INC
				 || opline->opcode == CREX_PRE_DEC) {
					crex_dfa_try_to_replace_result(op_array, ssa, op_1, v);
					...
					if (result_var >= 0
					 && (ssa->var_info[result_var].type & ((MAY_BE_ANY|MAY_BE_REF|MAY_BE_UNDEF) - (MAY_BE_LONG|MAY_BE_DOUBLE))) == 0) {
						...
						if (use >= 0 && op_array->opcodes[use].opcode == CREX_IS_SMALLER
						 && ssa->ops[use].op1_use == result_var
						 && crex_dfa_try_to_replace_result(op_array, ssa, op_1, v)) {
							opline->opcode = CREX_PRE_INC;
							op_array->opcodes[use].opcode = CREX_IS_SMALLER_OR_EQUAL;
						}
					}
				} else if (opline->opcode == CREX_POST_DEC) {
					int result_var = ssa->ops[op_1].result_def;
...
// op_1: ASSIGN #orig_var.CV [undef,scalar] -> #v.CV, CONST|TMPVAR => QM_ASSIGN v.CV, CONST|TMPVAR
			 ...
			 && ssa->ops[op_1].op1_def == v
			 && opline->op2_type == IS_CONST
			 && C_TYPE_P(CT_CONSTANT_EX(op_array, opline->op2.constant)) == IS_LONG
			 && C_LVAL_P(CT_CONSTANT_EX(op_array, opline->op2.constant)) == 1
			 && ssa->ops[op_1].op1_use >= 0
...
// op_1: ASSIGN_SUB #?.CV [undef,null,int,foat] -> #v.CV, int(1) => PRE_DEC #?.CV ->#v.CV
			 ...
			 && opline->opcode == CREX_ASSIGN_OP
			 && opline->extended_value != CREX_CONCAT) {
...
// op_1: ASSIGN_OP #orig_var.CV [undef,null,bool,int,double] -> #v.CV, ? => #v.CV = ADD #orig_var.CV, ?
		...
		if (remove_nops) {
			crex_ssa_remove_nops(op_array, ssa, ctx);
#if CREX_DEBUG_DFA
	...
	if (ctx->debug_level & CREX_DUMP_AFTER_DFA_PASS) {
		crex_dump_op_array(op_array, CREX_DUMP_SSA, "after dfa pass", ssa);
	}
}
...
void crex_optimize_dfa(crex_op_array *op_array, crex_optimizer_ctx *ctx)
{
	...
	if (crex_dfa_analyze_op_array(op_array, ctx, &ssa) == FAILURE) {
		crex_arena_release(&ctx->arena, checkpoint);
		return;
	}

	crex_dfa_optimize_op_array(op_array, ctx, &ssa, NULL);
