...
static inline bool is_in_use_chain(crex_ssa *ssa, int var, int check) {
	int use;
...
static inline bool is_in_phi_use_chain(crex_ssa *ssa, int var, crex_ssa_phi *check) {
	crex_ssa_phi *phi;
...
static inline bool is_used_by_op(crex_ssa *ssa, int op, int check) {
	crex_ssa_op *ssa_op = &ssa->ops[op];
	return (ssa_op->op1_use == check)
		|| (ssa_op->op2_use == check)
		|| (ssa_op->result_use == check);
}

static inline bool is_defined_by_op(crex_ssa *ssa, int op, int check) {
	crex_ssa_op *ssa_op = &ssa->ops[op];
...
static inline bool is_in_phi_sources(crex_ssa *ssa, crex_ssa_phi *phi, int check) {
	int source;
...
static inline bool is_in_predecessors(crex_cfg *cfg, crex_basic_block *block, int check) {
...
static inline bool is_in_successors(crex_basic_block *block, int check) {
	int s;
...
static inline bool is_defined(const crex_ssa *ssa, const crex_op_array *op_array, int var) {
	const crex_ssa_var *ssa_var = &ssa->vars[var];
...
#define INSTRFMT "%d (%s)"
#define INSTR(i) \
	(i), (crex_get_opcode_name(op_array->opcodes[i].opcode))

void ssa_verify_integrity(crex_op_array *op_array, crex_ssa *ssa, const char *extra) {
	crex_cfg *cfg = &ssa->cfg;
	crex_ssa_phi *phi;
	int i;
	...
	for (i = 0; i < cfg->blocks_count; i++) {
		crex_basic_block *block = &cfg->blocks[i];
		...
		for (j = 0; j < block->predecessors_count; j++) {
			if (predecessors[j] >= 0) {
				int k;
				crex_basic_block *prev_block = &cfg->blocks[predecessors[j]];
				if (!(prev_block->flags & CREX_BB_REACHABLE)) {
...
finish:
	if (status == FAILURE) {
		crex_dump_op_array(op_array, CREX_DUMP_SSA, "at SSA integrity verification", ssa);
		CREX_ASSERT(0 && "SSA integrity verification failed");
	}
}
