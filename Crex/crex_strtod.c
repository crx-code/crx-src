...
#endif
{
	...
	if (k <= Kmax && (rv = freelist[k]))
		...
		if (!rv) {
			FREE_DTOA_LOCK(0);
			crex_error_noreturn(E_ERROR, "Balloc() failed to allocate memory");
		}
#else
...
CREX_API void
#ifdef KR_headers
crex_freedtoa(s) char *s;
#else
crex_freedtoa(char *s)
#endif
{
...
CREX_API char *crex_dtoa(double dd, int mode, int ndigits, int *decpt, bool *sign, char **rve)
{
...
CREX_API char *crex_gcvt(double value, int ndigit, char dec_point, char exponent, char *buf)
{
	...
	digits = crex_dtoa(value, mode, ndigit, &decpt, &sign, NULL);
	if (decpt == 9999) {
		/*
