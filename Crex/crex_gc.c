 ...
 *
 * A node MAY be added to possible roots when CREX_UNSET_VAR happens or
 * crex_assign_to_variable is called only when possible garbage node is
 * produced.
 * gc_possible_root() will be called to add the nodes to possible roots.
 *
 *
 * For objects, we call their get_gc handler (by default 'crex_std_get_gc') to
...
#ifndef CREX_GC_DEBUG
# define CREX_GC_DEBUG 0
#endif
...
# define GC_TRACE_REF(ref, format, ...) \
	do { \
		gc_trace_ref((crex_refcounted *) ref); \
...
/* Weak maps */
#define C_FROM_WEAKMAP_KEY		(1<<0)
#define C_FROM_WEAKMAP			(1<<1)
...
#define GC_FROM_WEAKMAP_KEY(zv) \
	(C_TYPE_INFO_P((zv)) & (C_FROM_WEAKMAP_KEY << C_TYPE_INFO_EXTRA_SHIFT))

#define GC_SET_FROM_WEAKMAP_KEY(zv) do {									   \
	zval *_z = (zv);														   \
	C_TYPE_INFO_P(_z) = C_TYPE_INFO_P(_z) | (C_FROM_WEAKMAP_KEY << C_TYPE_INFO_EXTRA_SHIFT); \
} while (0)
...
#define GC_FROM_WEAKMAP(zv) \
	(C_TYPE_INFO_P((zv)) & (C_FROM_WEAKMAP << C_TYPE_INFO_EXTRA_SHIFT))

#define GC_SET_FROM_WEAKMAP(zv) do {									       \
	zval *_z = (zv);														   \
	C_TYPE_INFO_P(_z) = C_TYPE_INFO_P(_z) | (C_FROM_WEAKMAP << C_TYPE_INFO_EXTRA_SHIFT); \
} while (0)
...
CREX_API int (*gc_collect_cycles)(void);

typedef struct _gc_root_buffer {
	crex_refcounted  *ref;
} gc_root_buffer;

typedef struct _crex_gc_globals {
...
#define GC_G(v) CREX_TSRMG_FAST(gc_globals_offset, crex_gc_globals *, v)
#else
#define GC_G(v) (gc_globals.v)
static crex_gc_globals gc_globals;
#endif
...
#define GC_STACK_SEGMENT_SIZE (((4096 - CREX_MM_OVERHEAD) / sizeof(void*)) - 2)
...
struct _gc_stack {
	gc_stack        *prev;
	gc_stack        *next;
	crex_refcounted *data[GC_STACK_SEGMENT_SIZE];
};
...
static crex_never_inline gc_stack* gc_stack_next(gc_stack *stack)
{
...
static crex_always_inline void gc_stack_push(gc_stack **stack, size_t *top, crex_refcounted *ref)
{
...
static crex_always_inline crex_refcounted* gc_stack_pop(gc_stack **stack, size_t *top)
{
	if (UNEXPECTED((*top) == 0)) {
		if (!(*stack)->prev) {
			return NULL;
...
static crex_always_inline uint32_t gc_compress(uint32_t idx)
{
...
static crex_always_inline gc_root_buffer* gc_decompress(crex_refcounted *ref, uint32_t idx)
{
	...
	while (1) {
		...
		CREX_ASSERT(idx < GC_G(first_unused));
		root = GC_IDX2PTR(idx);
...
static crex_always_inline uint32_t gc_fetch_unused(void)
{
	uint32_t idx;
	gc_root_buffer *root;

	CREX_ASSERT(GC_HAS_UNUSED());
	idx = GC_G(unused);
	root = GC_IDX2PTR(idx);
	CREX_ASSERT(GC_IS_UNUSED(root->ref));
	GC_G(unused) = GC_LIST2IDX(root->ref);
	return idx;
}

static crex_always_inline void gc_link_unused(gc_root_buffer *root)
{
	root->ref = GC_IDX2LIST(GC_G(unused));
	GC_G(unused) = GC_PTR2IDX(root);
}

static crex_always_inline uint32_t gc_fetch_next_unused(void)
{
	uint32_t idx;

	CREX_ASSERT(GC_HAS_NEXT_UNUSED());
...
static void gc_trace_ref(crex_refcounted *ref) {
	if (GC_TYPE(ref) == IS_OBJECT) {
		crex_object *obj = (crex_object *) ref;
		...
		crex_array *arr = (crex_array *) ref;
		fprintf(stderr, "[%p] rc=%d addr=%d %s array(%d) ",
			ref, GC_REFCOUNT(ref), GC_REF_ADDRESS(ref),
			gc_color_name(GC_REF_COLOR(ref)),
			crex_hash_num_elements(arr));
	} else {
		fprintf(stderr, "[%p] rc=%d addr=%d %s %s ",
			ref, GC_REFCOUNT(ref), GC_REF_ADDRESS(ref),
			gc_color_name(GC_REF_COLOR(ref)),
			GC_TYPE(ref) == IS_REFERENCE
				? "reference" : crex_get_type_by_const(GC_TYPE(ref)));
	}
}
#endif

static crex_always_inline void gc_remove_from_roots(gc_root_buffer *root)
{
	GC_LINK_UNUSED(root);
	GC_G(num_roots)--;
...
static void root_buffer_dtor(crex_gc_globals *gc_globals)
{
	if (gc_globals->buf) {
		free(gc_globals->buf);
		gc_globals->buf = NULL;
...
static void gc_globals_ctor_ex(crex_gc_globals *gc_globals)
{
...
{
#ifdef ZTS
	ts_allocate_fast_id(&gc_globals_id, &gc_globals_offset, sizeof(crex_gc_globals), (ts_allocate_ctor) gc_globals_ctor_ex, (ts_allocate_dtor) root_buffer_dtor);
#else
...
static void gc_grow_root_buffer(void)
{
	...
	if (GC_G(buf_size) >= GC_MAX_BUF_SIZE) {
		if (!GC_G(gc_full)) {
			crex_error(E_WARNING, "GC buffer overflow (GC disabled)\n");
			GC_G(gc_active) = 1;
			GC_G(gc_protected) = 1;
			GC_G(gc_full) = 1;
			return;
...
static crex_never_inline void CREX_FASTCALL gc_possible_root_when_full(crex_refcounted *ref)
{
	uint32_t idx;
	gc_root_buffer *newRoot;

	CREX_ASSERT(GC_TYPE(ref) == IS_ARRAY || GC_TYPE(ref) == IS_OBJECT);
	CREX_ASSERT(GC_INFO(ref) == 0);
...
static void CREX_FASTCALL gc_extra_root(crex_refcounted *ref)
{
	uint32_t idx;
	gc_root_buffer *newRoot;
	...
	CREX_ASSERT(GC_TYPE(ref) == IS_ARRAY || GC_TYPE(ref) == IS_OBJECT);
	CREX_ASSERT(GC_REF_ADDRESS(ref) == 0);
...
static crex_never_inline void CREX_FASTCALL gc_remove_compressed(crex_refcounted *ref, uint32_t idx)
{
...
static void gc_scan_black(crex_refcounted *ref, gc_stack *stack)
{
	HashTable *ht;
	Bucket *p;
	zval *zv;
	...
	if (GC_TYPE(ref) == IS_OBJECT) {
		...
		if (EXPECTED(!(OBJ_FLAGS(ref) & IS_OBJ_FREE_CALLED))) {
			...
			if (UNEXPECTED(GC_FLAGS(obj) & IS_OBJ_WEAKLY_REFERENCED)) {
				crex_weakmap_get_object_key_entry_gc(obj, &table, &len);
				n = len;
				zv = table;
				for (; n != 0; n-=2) {
					CREX_ASSERT(C_TYPE_P(zv) == IS_PTR);
					zval *entry = (zval*) C_PTR_P(zv);
					zval *weakmap = zv+1;
					CREX_ASSERT(C_REFCOUNTED_P(weakmap));
					if (C_OPT_REFCOUNTED_P(entry)) {
						GC_UNSET_FROM_WEAKMAP_KEY(entry);
						if (GC_REF_CHECK_COLOR(C_COUNTED_P(weakmap), GC_GREY)) {
							/* Weakmap was scanned in gc_mark_roots, we must
							 * ensure that it's eventually scanned in
							 * gc_scan_roots as well. */
							if (!GC_REF_ADDRESS(C_COUNTED_P(weakmap))) {
								gc_extra_root(C_COUNTED_P(weakmap));
							}
						} else if (/* GC_REF_CHECK_COLOR(C_COUNTED_P(weakmap), GC_BLACK) && */ !GC_FROM_WEAKMAP(entry)) {
							 ...
							 * entry */
							CREX_ASSERT(GC_REF_CHECK_COLOR(C_COUNTED_P(weakmap), GC_BLACK));
							ref = C_COUNTED_P(entry);
							GC_ADDREF(ref);
							if (!GC_REF_CHECK_COLOR(ref, GC_BLACK)) {
			...
			if (UNEXPECTED(obj->handlers->get_gc == crex_weakmap_get_gc)) {
				crex_weakmap_get_key_entry_gc(obj, &table, &len);
				n = len;
				zv = table;
				for (; n != 0; n-=2) {
					CREX_ASSERT(C_TYPE_P(zv+1) == IS_PTR);
					zval *key = zv;
					zval *entry = (zval*) C_PTR_P(zv+1);
					if (C_OPT_REFCOUNTED_P(entry)) {
						GC_UNSET_FROM_WEAKMAP(entry);
						if (GC_REF_CHECK_COLOR(C_COUNTED_P(key), GC_GREY)) {
							/* Key was scanned in gc_mark_roots, we must
							 * ensure that it's eventually scanned in
							 * gc_scan_roots as well. */
							if (!GC_REF_ADDRESS(C_COUNTED_P(key))) {
								gc_extra_root(C_COUNTED_P(key));
							}
						} else if (/* GC_REF_CHECK_COLOR(C_COUNTED_P(key), GC_BLACK) && */ !GC_FROM_WEAKMAP_KEY(entry)) {
							/* Both the entry weakmap and key are BLACK, so we
							 * can mark the entry BLACK as well.
							 ...
							 * scanned), so it's our responsibility to mark the
							 * entry */
							CREX_ASSERT(GC_REF_CHECK_COLOR(C_COUNTED_P(key), GC_BLACK));
							ref = C_COUNTED_P(entry);
							GC_ADDREF(ref);
							if (!GC_REF_CHECK_COLOR(ref, GC_BLACK)) {
								GC_REF_SET_BLACK(ref);
...
handle_zvals:
			for (; n != 0; n--) {
				if (C_REFCOUNTED_P(zv)) {
					...
					if (!GC_REF_CHECK_COLOR(ref, GC_BLACK)) {
						...
						while (--n) {
							if (C_REFCOUNTED_P(zv)) {
								crex_refcounted *ref = C_COUNTED_P(zv);
								GC_ADDREF(ref);
								if (!GC_REF_CHECK_COLOR(ref, GC_BLACK)) {
									GC_REF_SET_BLACK(ref);
									GC_STACK_PUSH(ref);
		...
		CREX_ASSERT((crex_array*)ref != &EG(symbol_table));
		ht = (crex_array*)ref;
handle_ht:
		n = ht->nNumUsed;
		zv = ht->arPacked;
		...
		if (C_REFCOUNTED(((crex_reference*)ref)->val)) {
			ref = C_COUNTED(((crex_reference*)ref)->val);
			GC_ADDREF(ref);
			if (!GC_REF_CHECK_COLOR(ref, GC_BLACK)) {
				GC_REF_SET_BLACK(ref);
				goto tail_call;
...
static void gc_mark_grey(crex_refcounted *ref, gc_stack *stack)
{
	HashTable *ht;
	Bucket *p;
	zval *zv;
...
tail_call:
	...
	if (GC_TYPE(ref) == IS_OBJECT) {
		...
		if (EXPECTED(!(OBJ_FLAGS(ref) & IS_OBJ_FREE_CALLED))) {
			...
			if (UNEXPECTED(obj->handlers->get_gc == crex_weakmap_get_gc)) {
				crex_weakmap_get_entry_gc(obj, &table, &len);
				n = len;
				zv = table;
				for (; n != 0; n--) {
					CREX_ASSERT(C_TYPE_P(zv) == IS_PTR);
...
static void gc_scan(crex_refcounted *ref, gc_stack *stack)
{
	HashTable *ht;
	Bucket *p;
	zval *zv;
...
tail_call:
	...
	if (GC_TYPE(ref) == IS_OBJECT) {
		...
		if (EXPECTED(!(OBJ_FLAGS(ref) & IS_OBJ_FREE_CALLED))) {
			...
			if (UNEXPECTED(GC_FLAGS(obj) & IS_OBJ_WEAKLY_REFERENCED)) {
				crex_weakmap_get_object_entry_gc(obj, &table, &len);
				n = len;
				zv = table;
				for (; n != 0; n--) {
					CREX_ASSERT(C_TYPE_P(zv) == IS_PTR);
			...
			if (UNEXPECTED(ht)) {
				if (GC_REF_CHECK_COLOR(ht, GC_GREY)) {
					...
					GC_STACK_PUSH((crex_refcounted *) ht);
					for (; n != 0; n--) {
						if (C_REFCOUNTED_P(zv)) {
							ref = C_COUNTED_P(zv);
							if (GC_REF_CHECK_COLOR(ref, GC_GREY)) {
...
static void gc_add_garbage(crex_refcounted *ref)
{
	uint32_t idx;
	gc_root_buffer *buf;
...
static int gc_collect_white(crex_refcounted *ref, uint32_t *flags, gc_stack *stack)
{
	int count = 0;
	HashTable *ht;
	Bucket *p;
	...
	if (GC_TYPE(ref) == IS_OBJECT) {
		...
		if (EXPECTED(!(OBJ_FLAGS(ref) & IS_OBJ_FREE_CALLED))) {
			...
			if (!(OBJ_FLAGS(obj) & IS_OBJ_DESTRUCTOR_CALLED)
			 && (obj->handlers->dtor_obj != crex_objects_destroy_object
			...
			if (UNEXPECTED(GC_FLAGS(obj) & IS_OBJ_WEAKLY_REFERENCED)) {
				...
				for (; n != 0; n--) {
					CREX_ASSERT(C_TYPE_P(zv) == IS_PTR);
					zval *entry = (zval*) C_PTR_P(zv);
					if (C_REFCOUNTED_P(entry) && GC_FROM_WEAKMAP_KEY(entry)) {
						GC_UNSET_FROM_WEAKMAP_KEY(entry);
						GC_UNSET_FROM_WEAKMAP(entry);
						ref = C_COUNTED_P(entry);
						GC_ADDREF(ref);
			...
			if (UNEXPECTED(obj->handlers->get_gc == crex_weakmap_get_gc)) {
				...
				for (; n != 0; n--) {
					CREX_ASSERT(C_TYPE_P(zv) == IS_PTR);
					zval *entry = (zval*) C_PTR_P(zv);
					if (C_REFCOUNTED_P(entry) && GC_FROM_WEAKMAP(entry)) {
						GC_UNSET_FROM_WEAKMAP_KEY(entry);
						GC_UNSET_FROM_WEAKMAP(entry);
						ref = C_COUNTED_P(entry);
						GC_ADDREF(ref);
...
static int gc_remove_nested_data_from_buffer(crex_refcounted *ref, gc_root_buffer *root, gc_stack *stack)
{
	HashTable *ht;
	Bucket *p;
	zval *zv;
...
static void crex_get_gc_buffer_release(void);
static void crex_gc_root_tmpvars(void);
...
CREX_API int crex_gc_collect_cycles(void)
{
	...
	crex_hrtime_t start_time = crex_hrtime();
...
rerun_gc:
	if (GC_G(num_roots)) {
		...
		if (GC_G(gc_active)) {
			GC_G(collector_time) += crex_hrtime() - start_time;
			return 0;
		}
		...
		if (gc_flags & GC_HAS_DESTRUCTORS) {
			...
			crex_hrtime_t dtor_start_time = crex_hrtime();
			idx = GC_FIRST_ROOT;
			while (idx != end) {
				current = GC_IDX2PTR(idx);
				if (GC_IS_DTOR_GARBAGE(current->ref)) {
			...
			GC_G(dtor_time) += crex_hrtime() - dtor_start_time;

			if (GC_G(gc_protected)) {
				/* something went wrong */
				crex_get_gc_buffer_release();
		...
		crex_hrtime_t free_start_time = crex_hrtime();
		idx = GC_FIRST_ROOT;
		while (idx != end) {
			current = GC_IDX2PTR(idx);
			if (GC_IS_GARBAGE(current->ref)) {
				...
				if (GC_TYPE(p) == IS_OBJECT) {
					...
					CREX_OBJECTS_STORE_ADD_TO_FREE_LIST(obj->handle);
				} else if (GC_TYPE(p) == IS_ARRAY) {
					crex_array *arr = (crex_array*)p;
		...
		GC_G(free_time) += crex_hrtime() - free_start_time;
...
CREX_API void crex_gc_get_status(crex_gc_status *status)
{
	...
	status->application_time = crex_hrtime() - GC_G(activated_at);
...
CREX_API crex_get_gc_buffer *crex_get_gc_buffer_create(void) {
	...
	crex_get_gc_buffer *gc_buffer = &EG(get_gc_buffer);
	...
	for (; ex; ex = ex->prev_execute_data) {
		...
		for (uint32_t i = 0; i < func->op_array.last_live_range; i++) {
			...
			if (kind == CREX_LIVE_TMPVAR || kind == CREX_LIVE_LOOP) {
				...
				zval *var = CREX_CALL_VAR(ex, var_num);
				if (C_REFCOUNTED_P(var)) {
					gc_check_possible_root(C_COUNTED_P(var));
				}
			}
		}
	}
